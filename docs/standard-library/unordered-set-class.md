...
|[модуль хэширования](#hasher)|Тип хэш-функции.|
...
Первые две функции-члены возвращают прямой итератор, указывающий на первый элемент последовательности (или на место сразу за концом пустой последовательности). Последние две функции-члены возвращают прямой итератор, указывающий на первый элемент сегмента *nbucket* (или просто за концом пустого сегмента).
...
Функция-член возвращает номер контейнера, в настоящий момент соответствует значению ключа *keyval*.
...
Объект **const** итератор прямого доступа, который указывает конец диапазона.
...
Объект `pair` которого **bool** компонент возвращает значение true, если вставка была выполнена и значение false, если `unordered_set` уже содержал элемент, ключ которого имел эквивалентное значение порядка, и компонент итератора которого возвращает адрес, где был вставлен новый элемент или где уже находился элемент.

Для доступа к компоненту итератора пары `pr`, возвращенной этой функцией-членом, используйте `pr.first` и разыменуйте ее с помощью `*(pr.first)`. Чтобы получить доступ к **bool** компонент пары `pr` возвращенной этой функцией-членом, используйте `pr.second`.
...
Функция-член возвращает пару итераторов `X` таким образом, чтобы`[X.first, X.second)` отделяет только те элементы управляемой последовательности, которые имеют эквивалентное упорядочение с *keyval*. Если таких элементов не существует, оба итератора имеют значение `end()`.
...
Функции-члены одного элемента, (1) и (2) возвращают[пары](../standard-library/pair-structure.md) которого **bool** компонент является значение true, если вставка была выполнена и значение false, если unordered_set уже содержало элемент с ключом эквивалентное значение порядка. Компонент итератора пары возвращаемых значений указывает на вставленный элемент, если **bool** компонент равно true, или на существующий элемент Если **bool** компонента имеет значение false.
...
Для доступа к компоненту-итератору `pair` `pr` возвращаемый функциями-членами, используйте `pr.first`; для разыменования итератора в возвращенной паре используйте`*pr.first`, сам элемент. Чтобы получить доступ к **bool** компонент, используйте `pr.second`. См. пример кода далее в этой статье.

[Value_type](../standard-library/map-class.md#value_type) контейнер — это определение, которое принадлежит к контейнеру, а для набора, `unordered_set<V>::value_type` является типом `const V`.

Функция-член диапазоном (5) вставляет последовательность значений элементов в unordered_set, соответствующее каждому элементу, адресованному итератором в диапазоне `[First, Last)`; таким образом, *последнего* не вставляется. Контейнер функции-члена `end()` ссылается на позицию сразу после последнего элемента в контейнере. Например, оператор `s.insert(v.begin(), v.end());` пытается вставить все элементы `v` в `s`. Вставляются только элементы с уникальными значениями в диапазоне. Повторяющиеся значения игнорируются. Чтобы увидеть, какие элементы отклонены, используйте одноэлементные версии `insert`.
...
Для вставки элемента, созданного на месте, то есть операции копирования или перемещения не выполняются, см. в разделе[set::emplace](../standard-library/set-class.md#emplace) и[set::emplace_hint](../standard-library/set-class.md#emplace_hint).
...
Первая функция-член возвращает сохраненный коэффициент максимальной нагрузки. Вторая функция-член заменяет сохраненный коэффициент максимальной нагрузки с *идентификации*.
...
После удаления всех существующих элементов в `unordered_set`, `operator=` копирует или перемещает содержимое *правой* в `unordered_set`.
...
Функция-член устанавливает число сегментов не менее *nbuckets* и перестраивает хэш-таблицу по мере необходимости.
...
Функция-член меняет местами управляемые последовательности между `*this` и *правой*. Если [unordered_set::get_allocator](#get_allocator)`() == right.get_allocator()`, она делает это в константном времени, создает исключение только в результате копирования сохраненного объекта признаков типа `Tr`, и делает недействительными не ссылки, указатели, или итераторы, обозначающие элементы в двух управляемых последовательностях. В противном случае она выполняет ряд назначений элементов и вызовов конструктора, пропорционально количеству элементов в двух управляемых последовательностях.
...
Первый конструктор определяет копию последовательности, управляемой *справа*. Второй конструктор определяет управляемую пустую последовательность. Третий конструктор задает копию последовательности путем перемещения *справа* конструкторы с четвертого по восьмой используют initializer_list для указания копируемых элементов. Девятый конструктор добавляет последовательность значений элементов `[first, last)`.

Все конструкторы также инициализируют ряд сохраненных значений. Для конструктора копии значения извлекаются из *справа*. В противном случае:

Минимальное число блоков — это аргумент *bucket_count*, если он существует; в противном случае это значение по умолчанию, представленное здесь как значение, определяемое реализацией `N0`.

Объект хэш-функции — это аргумент *хэш*, если он существует; в противном случае это `Hash()`.
