...
|||
|-|-|
|[unordered_set:оператор](#op_eq)|Копирует хэш-таблицу.|

## <a name="remarks"></a>Remarks

Объект заказывает последовательность, которую он контролирует, вызывая два сохраненных объекта, объект функции сравнения типа [unordered_set::key_equal](#key_equal) и объект функции хэша типа [unordered_set::hasher](#hasher). Вы получаете доступ к первому сохраненному объекту, позвонив функции участника [unordered_set::key_eq;](#key_eq)`()` и вы получаете доступ ко второму сохраненному объекту, позвонив функции участника [unordered_set::hash_function](#hash)`()`. В частности, для всех значений `X` и `Y` типа `Key` вызов `key_eq()(X, Y)` возвращает значение true, только если два значения аргументов имеют соответствующий порядок; вызов `hash_function()(keyval)` создает распределение значений типа `size_t`. В отличие от шаблона класса `unordered_set` unordered_multiset `key_eq()(X, Y)` [класса,](../standard-library/unordered-multiset-class.md)объект типа гарантирует, что это всегда ложно для любых двух элементов контролируемой последовательности. (Ключи уникальны).

Объект также хранит максимальный коэффициент нагрузки, который определяет максимальное желаемое среднее количество элементов в блоке. Если вставка элемента приводит к [unordered_set:load_factor](#load_factor) `()` превышению максимального коэффициента нагрузки, контейнер увеличивает количество ведер и по мере необходимости восстанавливает хэш-таблицу.
...
Объект выделяет и освобождает хранилище для последовательности, которая он контролирует через сохраненный объект распределительного типа [unordered_set::allocator_type](#allocator_type). Такой объект разлесть должен иметь тот же внешний `allocator`интерфейс, что и объект типа. Обратите внимание, что сохраненный объект распределителя не копируется, когда назначается объект контейнера.
...
## <a name="bucket"></a><a name="bucket"></a>Ведро
...
Функция участника возвращает номер ведра, в настоящее время соответствующий *ключевому ключу*значения.
...
Функция участника вызывает [unordered_set::erase](#erase) `(` [unordered_set::start](#begin) `(),` [unordered_set::end](#end)`())`.
...
## <a name="count"></a><a name="count"></a>Рассчитывать
...
Функция участника возвращает количество элементов в диапазоне, делимитированных [unordered_set::equal_range](#equal_range)`(keyval)`.
...
Компонент `pair` **bool** которого возвращается верно, если вставка `unordered_set` была сделана и ложна, если уже содержал элемент, ключ которого имел эквивалентное значение в заказе, и чей компонент итератора возвращает адрес, по которому был вставлен новый элемент или где элемент уже находится.

Для доступа к компоненту итератора пары `pr`, возвращенной этой функцией-членом, используйте `pr.first` и разыменуйте ее с помощью `*(pr.first)`. Чтобы получить доступ к компоненту **bool** `pr` пары, `pr.second`возвращенной этой функцией члена, используйте .
...
Для примера кода [см.](../standard-library/set-class.md#emplace)
...
## <a name="find"></a><a name="find"></a>Найти
...
Функция участника [возвращается unordered_set::equal_range](#equal_range)`(keyval).first`.
...
## <a name="hasher"></a><a name="hasher"></a>Домработника
...
*ValTy*\
Параметры шаблона, который определяет тип аргумента, который unordered_set может использовать для построения элемента [value_type,](../standard-library/map-class.md#value_type)и совершенный вперед *Val* в качестве аргумента.
...
*Ilist*\
В [initializer_list,](../standard-library/initializer-list.md) из которых можно скопировать элементы.

### <a name="return-value"></a>Возвращаемое значение

Функции одного элемента(1) и (2) возвращают [пару,](../standard-library/pair-structure.md) компонент **bool** которой верен, если вставка была сделана, и ложный, если unordered_set уже содержал элемент, ключ которого имел эквивалентное значение в заказе. Компонент итератора пары возврата значения указывает на вновь вставленный элемент, если компонент **bool** верен, или к существующему элементу, если компонент **bool** является ложным.
...
Чтобы получить доступ к компоненту `pair` `pr` итератора, который возвращается функциями одного элемента, используйте; `pr.first` для dereference итератора в возвращенной`*pr.first`паре, использование, давая вам элемент. Чтобы получить доступ к `pr.second`компоненту **bool,** используйте . См. пример кода далее в этой статье.
...
Функция члена диапазона (5) вставляет последовательность значений элементов в unordered_set, которая соответствует каждому `[First, Last)`элементу, адресованному итератором в диапазоне; таким образом, *Last* не вставляется. Контейнер функции-члена `end()` ссылается на позицию сразу после последнего элемента в контейнере. Например, оператор `s.insert(v.begin(), v.end());` пытается вставить все элементы `v` в `s`. Вставляются только элементы с уникальными значениями в диапазоне. Повторяющиеся значения игнорируются. Чтобы увидеть, какие элементы отклонены, используйте одноэлементные версии `insert`.

Функция члена списка инициализаторов (6) использует [initializer_list](../standard-library/initializer-list.md) для копирования элементов в unordered_set.
...
Для примера кода [см., см.: вставить](../standard-library/set-class.md#insert).

## <a name="iterator"></a><a name="iterator"></a>Итератор

Тип, обеспечивающий постоянный [итератор вперед,](../standard-library/forward-iterator-tag-struct.md) который может читать элементы в unordered_set.
...
Например, [например,](../standard-library/set-class.md#begin) можно примером того, как объявить и использовать**итератор.**
...
Функция участника`(float)`[возвращается unordered_set::размер](#size)`() / (float)`[unordered_set::bucket_count](#bucket_count)`()`, среднее количество элементов на ведро.
...
После стирания любых `unordered_set`существующих `operator=` элементов в , либо копии или перемещает содержимое *право* в `unordered_set`.
...
## <a name="rehash"></a><a name="rehash"></a>Переделывать
...
Первый конструктор указывает копию последовательности, контролируемой *Right.* Второй конструктор определяет управляемую пустую последовательность. Третий конструктор определяет копию последовательности, перемещая *право* Четвертый по восьмой конструкторы используют initializer_list, чтобы указать элементы для копирования. Девятый конструктор добавляет последовательность значений элементов `[first, last)`.
