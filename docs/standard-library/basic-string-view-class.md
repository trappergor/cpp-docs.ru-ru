...
Функция, принимающая параметр типа string_view, может быть выполнена для работы с любым типом, подобным строковым, без создания функции в шаблон или ограничения функции на определенное подмножество строковых типов. Единственное требование заключается в том, что для string_view существует неявное преобразование из строкового типа. Все стандартные строковые типы неявно преобразуются в string_view, который содержит тот же тип элемента. Иными словами, объект `std::string` преобразуется `string_view` в, но не в `wstring_view`.

В следующем примере показана функция `f` , не относящаяся к шаблону, которая принимает параметр типа. `wstring_view` Его можно вызвать с аргументами типа `std::wstring`, `wchar_t*`и `winrt::hstring`.
...
*CharType*\
Тип символов, которые хранятся в string_view. C++ Стандартная библиотека предоставляет следующие определения типов для специализаций этого шаблона.
...
|Оператор|Описание|
|-|-|
|[оператор=](#op_eq)|Присваивает string_view или преобразуемый строковый объект другому string_view.|
...
|-|-|
|[at](#at)|Возвращает const_reference для элемента в указанном расположении.|
|[back](#back)|Возвращает const_reference к последнему элементу.|
...
|[compare](#compare)|Сравнивает string_view с указанным string_view, чтобы определить, равны ли они или если один из них лексикографически меньше другого.|
...
|[find_first_not_of](#find_first_not_of)|Выполняет поиск первого символа, не любого элемента указанного string_view или преобразуемого строкового объекта.|
...
|[front](#front)|Возвращает const_reference к первому элементу.|
...
|[rfind](#rfind)|Поиск первого вхождения подстроки, совпадающей с указанной последовательностью символов, в string_view в обратную.|
...
Возвращает const_reference к символу по указанному индексу (от 0).
...
Объект const_reference для символа в позиции, указанной в индексе параметра.
...
В общем случае рекомендуется не использовать **для таких** последовательностей `std::vector` , как и string_view. Недопустимый индекс, переданный в последовательность, — это логическая ошибка, которая должна быть обнаружена и исправлена во время разработки. Если программа не имеет уверенности в том, что ее индексы являются допустимыми, следует протестировать их, а не вызывать в () и использовать исключения для защиты от неосторожного программирования.
...
Объект const_reference к последнему элементу в string_view.

### <a name="remarks"></a>Примечания

Создает исключение, если string_view пуст.

Помните, что после изменения string_view, например путем вызова `remove_suffix`, элемент, возвращаемый этой функцией, больше не является последним элементом в базовых данных.
...
Выполняет сравнение с учетом регистра с указанным string_view (или преобразуемым строковым типом), чтобы определить, равны ли два объекта, или если один из них лексикографически меньше другого. Операторы string_view > используют эту функцию члена для выполнения сравнений. [ \<](string-view-operators.md)
...
*стрв*\
String_view, который будет сравниваться с этим string_view.
...
*указатель*\
Строка C, сравниваемая с этим string_view.

### <a name="return-value"></a>Возвращаемое значение

Отрицательное значение, если string_view меньше *стрв* или *ptr*; нуль, если две последовательности символов равны; или положительное значение, если string_view больше *стрв* или *ptr*.

### <a name="remarks"></a>Примечания

Функции `compare` -члены выполняют сравнение с учетом регистра либо всей последовательности символов, либо ее части. 
...
*расчета*\
Число символов, которые будут копироваться не более чем из источника string_view.

*собой*\
Начальное расположение в источнике string_view, из которого должны быть сделаны копии.
...
Объект const_reverse_iterator, который обращается к первому элементу в обращенном string_view. 
...
Возвращает объект const_reverse_iterator, который обращается за одним после конца обращенного string_view.
...
Последовательность string_view символов не обязательно завершается нулем. Возвращаемый тип для `data` не является допустимой строкой C, так как не добавляется символ null. Символ null "\ 0" не имеет особого смысла в объекте типа string_view и может быть частью объекта string_view, как и любой другой символ.
...
`end`используется для проверки того, достиг ли const_iterator конца своего string_view. Значение, возвращаемое `end`, не должно быть подвергнуто удалению ссылки.

## <a name="find"></a>  basic_string_view::find

Выполняет поиск string_view в прямом направлении для первого вхождения символа или подстроки, совпадающей с указанной последовательностью символов.
...
Объект const_reference для первого элемента.
...
Присваивает string_view или преобразуемый строковый объект другому string_view.
...
Предоставляет const_reference для символа с указанным индексом.
...
`operator[]`не проверяет, является ли допустимым индекс, переданный в качестве аргумента. Недопустимый индекс, `operator[]` переданный в, приводит к неопределенному поведению.
...
Возвращает итератор произвольного доступа к первому элементу в обращенном string_view с адресацией последнего элемента в соответствующем необращенном string_view.
...
Оставляет базовые данные без изменений. Перемещает указатель string_view вперед на n элементов и устанавливает для элемента `size` закрытых данных размер-n.
...
Оставляет базовые данные и указатель на него без изменений. Задает для элемента `size` закрытых данных размер-n.
...
`rend`используется с обратным string_view точно так же, как [End](#end) используется с string_view. `rend`можно использовать для проверки того, достиг ли обратный итератор конца его string_view. Значение, возвращаемое `rend`, не должно быть подвергнуто удалению ссылки.
...
Индекс первого символа подстроки после успешного выполнения; в `npos`противном случае — значение.
...
String_view может изменить свою длину, например, на `remove_prefix` и. `remove_suffix` Так как это не изменяет базовые строковые данные, размер string_view не обязательно должен быть равен размеру базовых данных.

## <a name="substr"></a>  basic_string_view::substr

Возвращает string_view, представляющий указанное число символов из указанной позиции (не более).
...
*календар*\
Источник string_view, значения указателей и размера которых должны быть заменены на значение целевого string_view.
