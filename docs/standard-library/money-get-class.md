...
`InputIterator` Тип итератора, из которого функции получения считывают своих входные данные.
...
`Intl` Логическое значение, указывающее тип обозначение денежной единицы, ожидаемого в последовательности: **true** Если международные, **false** Если внутренние.
...
`State` Устанавливает соответствующие элементы битовой маски для состояния потока согласно ли операция успешно или нет.

`val` Строка, хранение преобразованную последовательность.
...
`Iosbase` Формат флаг, который при наборе указывает, что символ валюты является необязательным. в противном случае не требуется
