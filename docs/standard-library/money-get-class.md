...
ms.locfileid: "81375902"
---
# <a name="money_get-class"></a>Класс money_get

Шаблон класса описывает объект, который может служить в качестве аспекта локализации для управления преобразованиями последовательностей типа `CharType` к денежным значениям.
...
Первая защищенная виртуальная функция-член пытается сопоставить последовательные элементы, начиная с первого в последовательности [ `first`, `last`), пока не распознает полное, непустое денежное поле ввода. В случае успеха, он преобразует это поле в последовательность одной или нескольких десятичных `-`цифр, по желанию предшествует минус знак ( ), чтобы представить сумму и сохраняет результат в [string_type](#string_type) объект *val*. Она возвращает итератор, обозначающий первый элемент после денежного поля ввода. В противном случае функция *val* хранит пустую `ios_base::failbit` последовательность в val и устанавливает в *положении.* Она возвращает итератор, обозначающий первый элемент после любого префикса допустимого денежного поля ввода. В любом случае, если возвращаемое значение равно `last`, функция устанавливает `ios_base::eofbit` в `State`.

Вторая виртуальная защищенная функция члена ведет себя так же, как и первая, за исключением того, что в случае успеха она преобразует опционально подписанную последовательность цифр в значение длиннее **двойника** типа и хранит значение в *val*.
...
Обе функции члена возвращают [сятdo_get](#do_get)`(first, last, Intl, Iosbase, State, val)`.
...
Конструктор инициализирует свой базовый объект с **помощью локализации::**[грань](../standard-library/locale-class.md#facet_class)*(_Refs).*
