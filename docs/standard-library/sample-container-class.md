...
- Если контейнер сохраняет объект распределителя *Al*, а *Al* вызывает исключение `al.allocate`, отличное от результата вызова, результирующее состояние объекта контейнера не определено.
...
Для *всех* классов контейнеров, C++ определенных стандартной библиотекой, если исключение возникает во время вызовов следующих `insert` `push_back`функций-членов,, или `push_front`, контейнер остается неизменным, а исключение — снова.
...
Для *всех* классов контейнеров, определенных C++ стандартной библиотекой, функция `Alloc get_allocator const;` -член возвращает копию сохраненного объекта распределителя. Обратите внимание, что сохраненный объект-распределитель *не* копируется, если назначается объект контейнера. Все конструкторы инициализируют значение, хранящееся `allocator`в, `Alloc` в, если конструктор не содержит параметр распределителя.
...
- Тип `Alloc::const_pointer`совпадаетс. `const Ty *`

- Тип `Alloc::const_reference`совпадаетс. `const Ty&`

- Тип `Alloc::pointer`совпадаетс. `Ty *`

- Тип `Alloc::reference`совпадаетс. `Ty&`
...
- Тип `Alloc::const_pointer` не обязательно должен совпадать `const Ty *`с типом. (Константный указатель может быть классом.)

- Тип `Alloc::pointer` не обязательно должен совпадать `Ty *`с типом. (Указатель может быть классом.)
