...
ms.locfileid: "72687825"
---
# <a name="list-class"></a>Класс list

Класс C++ списка стандартной библиотеки — это шаблон класса последовательностей, который сохраняет свои элементы в линейном упорядочении и позволяет выполнять операции вставки и удаления в любом расположении в последовательности. Последовательность сохраняется в виде двунаправленного связанного списка элементов, каждый из которых содержит член какого-либо типа *Type*.
...
*Распределитель* \
Тип, представляющий сохраненный объект распределителя, содержащий сведения о распределении списка и отмене распределения памяти. Этот аргумент является необязательным, и значение по умолчанию — **распределитель** \<*тип*>.
...
Включите стандартный C++ заголовок стандартной библиотеки \<list >, чтобы определить список шаблонов классов [контейнеров](../standard-library/stl-containers.md) и несколько вспомогательных шаблонов.
...
|[begin](#cbegin)|Возвращает константный итератор, адресующий первый элемент в списке.|
...
И список аргументов, и список назначения должны быть упорядочены с использованием того же отношения сравнения, по которому должно быть упорядочена готовая последовательность. По умолчанию для первой функции-члена используется порядок по возрастанию. Вторая функция-член накладывает заданную *пользователем операцию сравнения* `Traits` класса.
...
После стирания любых существующих элементов в `list` оператор копирует или перемещает содержимое *непосредственно* в `list`.
...
Если размер списка больше запрошенного размера, то элементы, ближайшие к концу списка, удаляются, пока список не достигнет размера *_Newsize*.
...
Функция шаблона элемента упорядочивает элементы в соответствии с заданной *пользователем операцией* сравнения `Traits` класса.
...
Третья пара функций-членов вставляет диапазон, обозначенный [`First`, `Last`), перед элементом в конечном списке, на который ссылается, *и удаляет* этот диапазон элементов из исходного списка. (Если `&Source == this`, диапазон `[First, Last)` не должен включать элемент, на который указывает, *где*.)
...
Вторая функция члена удаляет каждый элемент, который *удовлетворяет функции* предиката, когда он сравнивается с предыдущим элементом. Можно использовать любой объект бинарной функции, объявленный в заголовке \<functional *> аргумента, или* можно создать собственный.
