...
*Тип* тип данных элемента для сохранения в списке.

*Распределитель* тип, представляющий сохраненный объект распределителя, инкапсулирующий сведения о выделении и освобождении памяти списка. Этот аргумент является необязательным, и значение по умолчанию — **распределителя**\<*тип*>.
...
|[const_pointer](#const_pointer)|Тип, предоставляющий указатель на **const** элемента в списке.|
...
|[reference](#reference)|Тип, предоставляющий ссылку на элемент **const**, хранящийся в списке, для чтения и выполнения операций **const**.|
...
|Оператор|Описание:|
|-|-|
|[list::operator=](#op_eq)|Заменяет элементы списка копией другого списка.|
...
*Первый* позицию первого элемента в диапазоне элементов для копирования из списка аргументов.

*Последний* положение первого элемента после диапазона элементов, копируемых из списка аргументов.

*Число* количество копий элемента, вставляемого в списке.

*Val* значение элемента, вставляемого в список.

*IList* список initializer_list содержит вставляемые элементы.
...
Если возвращаемое значение `begin` назначается `const_iterator`, элементы в объект списка невозможно изменить. Если возвращаемое значение `begin` назначается `iterator`, можно изменить элементы в объекте списка.
...
Предоставляет указатель на **const** элемента в списке.
...
`end` используется для проверки, достиг ли итератор конца списка.
...
*Где* положение элемента, удаляемого из списка.

*Первый* положение первого элемента, удаляемого из списка.

*последний* положение сразу после последнего элемента, удаляемого из списка.
...
|*Val*|Значение элемента, вставляемого в список.|
|*Количество*|Количество элементов, вставляемых в список.|
|*Первый*|Положение первого элемента в диапазоне элементов в копируемом списке аргументов.|
|*последний*|Положение первого элемента после диапазона элементов в копируемом списке аргументов.|
...
Третий конструктор задает повторение указанного числа (*число*) элементов со значением по умолчанию для класса `Type`.

Четвертый и пятый конструкторы указывают повтор (*число*) элементов со значением *Val*.

Шестой конструктор задает копию списка *справа*.

Седьмой конструктор перемещает список *справа*.
...
*правом* список аргументов для слияния со списком назначения.

*comp* оператор сравнения, использованный для упорядочения элементов списка назначения.

### <a name="remarks"></a>Примечания

Список аргументов *правой* объединяется со списком назначения.

И список аргументов, и список назначения должны быть упорядочены с использованием того же отношения сравнения, по которому должно быть упорядочена готовая последовательность. По умолчанию для первой функции-члена используется порядок по возрастанию. Вторая функция-член, налагается использование операции сравнения, определяемые пользователем *comp* класса `Traits`.
...
После удаления всех существующих элементов в `list`, оператор копирует или перемещает содержимое *правой* в `list`.
...
*Val* значение, которое этого элемента, приведет к удалению этого элемента из списка.
...
*Pred* унарный предикат, который, если совпадения с элементом, приводит к удалению этого элемента из списка.
...
*_Newsize* новый размер списка.

*Val* значение новых элементов, которое необходимо добавить в список, если новый размер больше, исходный размер. Если значение не задано, новым элементам назначается значение по умолчанию для класса.

### <a name="remarks"></a>Примечания

Если размер списка меньше запрошенного размера, *_Newsize*, элементы добавляются в список, пока не достигнет требуемого размера.

Если размер списка больше запрошенного размера, ближайшие к конец списка элементы удаляются, пока список не достигнет размера *_Newsize*.
...
*comp* оператор сравнения, использованный для упорядочивания последовательных элементов.

### <a name="remarks"></a>Примечания

Первая функция-член по умолчанию помещает элементы в порядке возрастания.

Шаблон функций-членов упорядочивает элементы в соответствии с операцией сравнения, определяемое пользователем *comp* класса `Traits`.
...
*Где* положение в целевом списке, перед которым следует вставить.

*Источник* исходный список, который должен быть вставлен в целевой список.

*Iter* элемент, который будет вставлен из исходного списка.

*Первый* первого элемента в диапазоне, который будет вставлен из исходного списка.

*Последний* первое положение после последнего элемента в диапазоне, который будет вставлен из исходного списка.

### <a name="remarks"></a>Примечания

Первая пара функций-членов вставляет все элементы в списке источников в целевой список перед позицией, на который ссылается *где* и удаляет все элементы из исходного списка. (`&Source` не должно быть равно `this`.)

Вторая пара функций-членов вставляет элемент, на который ссылается *Iter* перед положение в целевом списке ссылается *где* и удаляет *Iter* из список источников. (Если `Where == Iter || Where == ++Iter`, изменения не происходят.)

Третья пара функций-членов вставляет диапазон, обозначенный [ `First`, `Last`) перед элементом в целевом списке ссылается *где* и удаляет этот диапазон элементов из исходного списка. (Если `&Source == this`, диапазон `[First, Last)` не должен включать элемент, на которые указывают *где*.)

Если срез с диапазоном вставляет `N` элементов и `&Source != this`, объект класса [iterator](../standard-library/forward-list-class.md#iterator) `N` раз увеличивается.
...
*правом* список, предоставляющий элементы для обмена местами, или список, элементы которого должны быть заменены элементами списка *левой*.

*слева* список, элементы которого должны быть заменены элементами списка *правой*.
...
*Pred* двоичный предикат, используемый для сравнения последовательных элементов.
...
Вторая функция-член удаляет каждый элемент, удовлетворяющий функции предиката *pred* по сравнению с предшествующим элементом. Можно использовать любые объекты бинарной функции, объявленные в \<функциональной > заголовок для аргумента *pred* или создать свой собственный.
