...
Включите стандартный C++ заголовок стандартной библиотеки \<> списка, чтобы определить список шаблонов классов [контейнеров](../standard-library/stl-containers.md) и несколько вспомогательных шаблонов.
...
Вторая функция члена удаляет каждый элемент, который *удовлетворяет функции* предиката, когда он сравнивается с предыдущим элементом. Можно использовать любой объект двоичной функции, объявленный в заголовке \<функционального *> аргумента, или* можно создать собственный.
