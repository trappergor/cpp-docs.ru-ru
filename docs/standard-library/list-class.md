...
`Allocator` Тип, представляющий сохраненный объект распределителя, инкапсулирующий сведения о выделении и освобождении памяти в списке. Этот аргумент является необязательным, и значение по умолчанию — **распределителя**\<*тип*>.
...
`First` Положение первого элемента в диапазоне элементов, копируемых из списка аргументов.

`Last` Положение первого элемента после диапазона элементов, копируемых из списка аргументов.

`Count` Количество копий элемента, вставляемого в списке.
...
`Where` Положение элемента, удаляемого из списка.

`first` Положение первого элемента, удаляются из списка.

`last` Положение сразу после последнего элемента, удаляемого из списка.
...
`right` Список аргументов для объединения со списком назначения.

`comp` Оператор сравнения, используемый для упорядочения элементов в целевой список.
...
`val` Значение, которое, если с помощью элемента приведет к удалению этого элемента из списка.
...
`pred` Унарный предикат, который, если выполняется с помощью элемента приводит к удалению этого элемента из списка.
...
`val` Значение новых элементов для добавления в список, если новый размер больше, исходный размер. Если значение не задано, новым элементам назначается значение по умолчанию для класса.
...
`comp` Оператор сравнения, используемый для упорядочения последовательных элементов.
...
`Where` Позиция в целевом списке, после которого необходимо вставить.

`Source` Исходный список, который должен быть вставлен в целевой список.

`Iter` Элемент, который будет вставлен из исходного списка.

`First` Первый элемент в диапазоне, который будет вставлен из исходного списка.

`Last` Первая позиция за последним элементом в диапазоне, который будет вставлен из исходного списка.
...
`right` В списке, предоставляющий элементы, которые следует поменять местами, либо список, элементы которого должны поменяться местами с элементами списка `left`.

`left` Список, элементы которого должны поменяться местами с элементами списка `right`.
...
Вторая функция-член удаляет каждый элемент, удовлетворяющий функции предиката *pred* по сравнению с предшествующим элементом. Можно использовать любые объекты двоичных функций, объявленных в \<функциональной > заголовок для аргумента *pred* или создавать свои собственные.
