...
Типы `FT, T1, T2, ..., TN` должны быть Copy-конструируемым, а `INVOKE(fn, t1, ..., tN)` должны быть допустимыми выражениями для некоторых значений `w1, w2, ..., wN`.
...
Если значение `std::is_bind_expression<Ti>::value` равно **true** , аргумент `vi` является `ti(u1, u2, ..., uM)`, а его тип `Vi` — `result_of<Ti` `cv` `(U1&, U2&, ..., UN&>::type`;

Если значение `j` `std::is_placeholder<Ti>::value` не равно нулю, аргумент `vi` `uj`, а его тип `Vi` — `Uj&`;

в противном случае аргумент `vi` `ti`, а его тип `Vi` `Ti` `cv` `&`.
...
Число аргументов в вызове `bind` и аргументе `fn` должны быть равны числу аргументов, которые могут быть переданы в вызываемый объект `fn`. Например, `bind(cos, 1.0)` является правильным, и `bind(cos)` и `bind(cos, _1, 0.0)` неверны.

Число аргументов в вызове функции, отправляемом в оболочку вызова, возвращаемом методом `bind`, должен быть не меньше наивысшего нумерованного значения `is_placeholder<PH>::value` для всех аргументов-местозаполнителей в вызове в `bind`. Например, `bind(cos, _2)(0.0, 1.0)` является правильным (и возвращает `cos(1.0)`), а `bind(cos, _2)(0.0)` является неправильным.
...
Если *Func* является объектом типа `Operation` и `c` является константой, то `bind1st( func, c )` является таким же, как конструктор класса [binder1st](../standard-library/binder1st-class.md) `binder1st<Operation>(func, c)`и более удобен в использовании.
...
Если *Func* является объектом типа `Operation` и `c` является константой, то `bind2nd(func, c)` совпадает с конструктором класса [binder2nd](../standard-library/binder2nd-class.md) `binder2nd<Operation>(func, c)`и более удобным в использовании.
...
Стандартный объект функции, который выполняет побитовую операцию AND (binary `operator&`) о своих аргументах.
...
Стандартный объект функции, выполняющий операцию побитового дополнения (NOT) (унарный `operator~`) для аргумента. Добавлено в C++ 14.
...
Стандартный объект функции, который выполняет побитовую операцию OR (`operator|`) в своих аргументах.
...
Стандартный объект функции, который выполняет побитовую операцию XOR (binary `operator^`) о своих аргументах.
...
\ *спецификации*
`std::is_nothrow_invocable_v<Callable, Args>)`спецификации **except** .

### <a name="remarks"></a>Remarks

Вызывает вызываемый объект *fn* с помощью *аргументов*параметров. Фактически, `INVOKE(std::forward<Callable>(fn), std::forward<Args>(args)...)`, где псевдо-функция `INVOKE(f, t1, t2, ..., tN)` означает одно из следующих действий:
...
- `t1.get().*f`, если N = = 1 и `f` являются указателем на данные члена класса `T` а `std::decay_t<decltype(t1)>` является специализацией `std::reference_wrapper`.

- `(*t1).*f`, если N = = 1 и `f` являются указателем на данные члена класса `T` и `t1` не являются одним из предыдущих типов.
...
Функция шаблона возвращает простой упаковщик вызова `cw`с нестрогим типом результата, так что выражение `cw(t, a2, ..., aN)` совпадает с `INVOKE(pm, t, a2, ..., aN)`. Исключения не вызываются.

Возвращаемая обертка вызова является производной от `std::unary_function<cv Ty*, RTy>` (и определения вложенного типа `result_type` как синоним для *РТИ* , а вложенный тип `argument_type` как синоним для `cv Ty*`) только в том случае, если тип *Ty* является указателем на функцию-член с квалификатором ОПС `cv`, не принимающим аргументов.

Возвращаемая обертка вызова является производной от `std::binary_function<cv Ty*, T2, RTy>` (и определения вложенного типа `result_type` как синоним для *РТИ*, вложенный тип `first argument_type` как синоним для `cv Ty*`, а вложенный тип `second argument_type` как синоним для `T2`), только если тип *Ty* является указателем на функцию-член с квалификатором ОПС `cv`, принимающим один аргумент типа `T2`.
...
Если `unary_negate` создается из унарного предиката `predicate(x)`, он возвращает `!predicate(x)`.
...
Если `binary_negate` создается из бинарного предиката `binary_predicate(x, y)`, он возвращает `!binary_predicate(x, y)`.
...
Шаблон функции `not_fn` принимает вызываемый объект и возвращает вызываемый объект. Если возвращаемый вызываемый объект впоследствии вызывается с некоторыми аргументами, он передается в исходный вызываемый объект и логически инвертирует результат. Он сохраняет квалификацию квалификатора const и значение категории значений для упакованного вызываемого объекта. `not_fn` впервые появились в C++ 17 и заменяет устаревшие `std::not1`, `std::not2`, `std::unary_negate`и `std::binary_negate`.
...
Явный конструктор для вызываемого объекта *Func* требует тип `std::decay_t<Callable>` для удовлетворения требований `MoveConstructible`, а `is_constructible_v<FD, Callable>` должен иметь значение true. Он инициализирует упакованный вызываемый объект `fd` от `std::forward<Callable>(func)`и выдает любое исключение, созданное конструкцией `fd`.
...
Первые два — то же, что и `return !std::invoke(fd, std::forward<Args>(args)...)`. Второй — то же, что и `return !std::invoke(std::move(fd), std::forward<Args>(args)...)`.
