...
Распределитель — это шаблон класса, описывающий объект, который управляет выделением и освобождением памяти для объектов и массивов объектов определенного типа. Объекты распределителя используются несколькими шаблонами классов контейнеров в C++ стандартной библиотеке.
...
где аргумент шаблона `Type` — это тип, управляемый экземпляром распределителя. Стандартная библиотека предоставляет распределитель по умолчанию, [распределителя](../standard-library/allocator-class.md)шаблонов классов, который определен в [\<memory >.](../standard-library/memory.md) C++ Заголовок \<allocators> предоставляет следующие распределители:
...
|Макрос|Описание|
|-|-|
|[ALLOCATOR_DECL](../standard-library/allocators-functions.md#allocator_decl)|Возвращает шаблон класса распределителя.|
...
|[allocator_newdel](../standard-library/allocator-newdel-class.md)|Реализует распределитель, использующий **оператор DELETE** для освобождения блока памяти и **оператора New** для выделения блока памяти.|
...
|[rts_alloc](../standard-library/rts-alloc-class.md)|Шаблон класса rts_alloc описывает [Фильтр](../standard-library/allocators-header.md) , содержащий массив экземпляров кэша, и определяет, какой экземпляр следует использовать для выделения и освобождения в среде выполнения, а не во время компиляции.|
