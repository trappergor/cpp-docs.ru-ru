...
ms.locfileid: "68453255"
---
# <a name="valarray-class"></a>Класс valarray

Класс шаблона описывает объект, управляющий последовательностью элементов типа `Type` , которые хранятся в виде массива, предназначенного для выполнения высокоскоростных математических операций и оптимизированных для вычислительной производительности.
...
Функция-член возвращает объект класса [valarray](../standard-library/valarray-class.md) **\<Type > типа** [length,](#size)каждый из *которых имеет значение* `_Func((*this)[I])`.
...
Функция члена сравнивает значения, применяя **оператор\<**  или **оператор >** между парами элементов класса `Type`, для которых должны быть предоставлены операторы для элемента. `Type`
...
Объект valarray, элементы которого являются логическими `AND` элементами операнда valarray по *правому* краю.
...
Побитовое и имеет ту же таблицу истинности, `AND` что и логическая, но применяется к типу данных на уровне отдельных битов. В заданных битах *b*1 и *b*2, `AND` *b*1 *b*2 имеет **значение true** , если оба бита имеют значение true; **значение false** , если хотя бы одно значение равно false.
...
Исключающее логическое или, называемое **XOR**, имеет следующую семантику: Указанные элементы *e*1 и *e*2, *e*1 **XOR** *e*2 имеют **значение true** , если только один из элементов имеет значение true; **значение false** , если оба элемента имеют значение false или если оба элемента имеют значение true.
...
Если длина больше единицы, функция-член добавляет значения к сумме, применяя `operator+=` между парами элементов класса `Type`, поэтому для элементов типа `Type`необходимо предоставить соответствующий оператор.
