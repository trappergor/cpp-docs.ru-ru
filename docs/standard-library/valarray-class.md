...
- Он определяет различные интересные способы создания подстрочного `valarray<Type>` объекта, путем перегрузки [оператора&#91;](#op_at).
...
Функция-член возвращает объект класса [valarray](../standard-library/valarray-class.md) **\<> типа** [length,](#size)каждый *из элементов которого* `_Func((*this)[I])`.
...
Функция члена сравнивает значения, применяя **оператор\<** или **operator >** между парами элементов класса `Type`, для которых необходимо предоставить операторы для элемента `Type`.
...
Существует также битовый оператор **не**[valarray:: operator ~](#op_dtor) , который инвертирует уровень отдельных битов в двоичном представлении элементов **char** и **int** элемента valarray.
...
*справа*\
Valarray или значение типа элемента, идентичное значению операнда valarray, который должен быть объединен, поэлементно логически `AND` с операндом valarray.

### <a name="return-value"></a>Возвращаемое значение

Элемент valarray, элементы которого являются логическими `AND` операнда valarray по *правому* краю.
...
Побитовое и имеет ту же таблицу истинности, что и логическая `AND` но применяется к типу данных на уровне отдельных битов. В заданных битах *b*1 и *b*2, *b*1 `AND` *b*2 имеет **значение true** , если оба бита имеют значение true; **значение false** , если хотя бы одно значение равно false.
...
## <a name="op_gt_gt_eq"></a>&gt;&gt;оператора =
...
## <a name="op_lt_lt_eq"></a>&lt;&lt;оператора =
...
Оператор-член перегружается, чтобы предоставить несколько способов выбора последовательностей элементов из тех, которые управляются <strong>\*этом</strong>. Первая группа из пяти операторов-членов совместно с различными перегрузками [operator=](#op_eq) (и другими операторами присваивания) позволяют осуществить выборочную замену (фрагментирование) в управляемой последовательности. Выбранные элементы должны существовать.

При компиляции с флагом [_ITERATOR_DEBUG_LEVEL](../standard-library/iterator-debug-level.md), для которого задано значение 1 или 2, возникнет ошибка времени выполнения при попытке доступа к элементу за пределами объекта valarray.  Дополнительные сведения см. в разделе [Checked Iterators](../standard-library/checked-iterators.md) .
...
Исключающее логическое или, называемое **XOR**, имеет следующую семантику: заданные элементы *e*1 и *e*2, *e*1 **XOR** *e*2 имеют **значение true** , если только один из элементов имеет значение true; **значение false** , если оба элемента имеют значение false или если оба элемента имеют значение true.
...
Объект valarray, элементы которого являются побитовым `OR` операнда valarray по *правому*краю.

### <a name="remarks"></a>Remarks

Побитовую операцию можно использовать только для обработки битов данных **типа char** и **int** и вариантов, а не для типов **float**, **Double**, **longdouble**, **void**, **bool**или других, более сложных типов данных.

Побитовое `OR` имеет ту же таблицу истинности, что и логическое `OR`, но применяется к типам данных на уровне отдельных битов. В заданных битах *b*1 и *b*2, *b*1 `OR` *b*2 имеет **значение true** , если хотя бы один из битов имеет значение true; **значение false** , если оба бита имеют значение false.
...
Унарный оператор, который получает побитовое `NOT` значений каждого элемента в valarray.
...
Valarray логических значений, являющихся побитовыми `NOT`ми значений элементов операнда valarray.

### <a name="remarks"></a>Remarks

Побитовую операцию можно использовать только для обработки битов данных **типа char** и **int** и вариантов, а не для типов **float**, **Double**, **longdouble**, **void**, **bool** или других, более сложных типов данных.

Побитовое `NOT` имеет ту же таблицу истинности, что и логическое `NOT`, но применяется к типам данных на уровне отдельных битов. Если взять бит *b*, то ~*b* имеет значение true если бит *b* имеет значение false, и имеет значение false, если бит *b* имеет значение true. Логический оператор **НЕ**[operator!](#op_not) применяется на уровне элемента, интерпретируя все ненулевые значения как **true**. Результатом является объект valarray из логических значений. Побитовое `NOToperator~`, напротив, может привести к valarray значений, отличных от 0 или 1, в зависимости от результата побитовой операции.
...
Если длина больше единицы, функция-член добавляет значения к сумме, применяя `operator+=` между парами элементов класса `Type`. Этот оператор, следовательно, должен быть предоставлен для элементов типа `Type`.
