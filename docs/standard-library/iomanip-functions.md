...
*_Объем* извлеченное денежное значение.

*_Intl* Если **true**, используется международный формат. Значение по умолчанию — **false**.

### <a name="remarks"></a>Примечания

Манипулятор возвращает объект, который, при извлечении из потока `str`, ведет себя как `formatted input function` , вызывает функцию-член `get` для ограничения языкового стандарта `money_get` связанные с `str`, с использованием *_ Intl* для указания международного формата. Если в случае успешного выполнения вызов хранит в *_объем* извлеченное денежное значение. Затем манипулятор возвращает `str`.
...
*_Tptr* время в виде структуры времени.

*_Fmt* нужный формат следует использовать для получения значения времени.
...
*_Объем* Денежная сумма для вставки в поток.

*_Intl* присвоено **true** Если манипулятор должен использовать международный формат **false** противном случае.
...
Манипулятор возвращает объект, который при вставке в поток `str` ведет себя как форматированная выходная функция, вызывающая функцию-член `put` для ограничения языкового стандарта `money_put`, связанного с `str`. Если в случае успешного выполнения, вызов вставляет `amount` соответствующем формате, используя * _Intl` to indicate international format and `str.fill()`, as the fill element. The manipulator then returns `str ".
...
*_Tptr* значения времени для записи в указанный поток, в виде структуры времени.

*_Fmt* требуемый формат для записи значения времени.

### <a name="remarks"></a>Примечания

Манипулятор возвращает объект, который при вставке в поток `str` ведет себя как `formatted output function`. Функция вывода вызывает функцию-член `put` для ограничения языкового стандарта `time_put`, связанного с `str`. Функция вывода использует *_Tptr* для обозначения структуры времени и *_Fmt* для обозначения начала строки формата, завершающаяся символом null. В случае успешного выполнения вызов вставляет текст из строки формата и преобразованные значения из структуры времени. Затем манипулятор возвращает `str`.
...
*escape* объект, указанный пользователем символ или двухбайтовый символ для использования в качестве escape-символа для escape-последовательностей в строке.
...
*Маска* флаги для очистки.

### <a name="return-value"></a>Возвращаемое значение

Манипулятор возвращает объект, извлеченные из или вставить в поток `str`, вызовы **str**. [setf](../standard-library/ios-base-class.md#setf)( `ios_base::` [fmtflags](../standard-library/ios-base-class.md#fmtflags), _ *маска*), а затем возвращает `str`.
...
*_Базовому* основание числа.

### <a name="return-value"></a>Возвращаемое значение

Манипулятор возвращает объект, извлеченные из или вставить в поток `str`, вызовы **str**. `setf`( **маска**, [ios_base::basefield](../standard-library/ios-base-class.md#fmtflags)), а затем возвращает `str`. Здесь `mask` определяется следующим образом:

- Если _ *базы* имеет значение 8, затем `mask` — `ios_base::` [oct](../standard-library/ios-functions.md#oct).

- Если _ *Base* имеет значение 10, то mask будет иметь значение `ios_base::`[dec](../standard-library/ios-functions.md#dec).

- Если _ *базы* равно 16, затем `mask` — `ios_base::` [hex](../standard-library/ios-functions.md#hex).
...
*CH* символ, который будет использоваться для заполнения пробелов в правому краю.

### <a name="return-value"></a>Возвращаемое значение

Манипулятор шаблона возвращает объект, извлеченные из или вставить в поток `str`, вызовы **str**. [Заливка](../standard-library/basic-ios-class.md#fill)(`Ch`), а затем возвращает `str`. Тип `Elem` должен совпадать с типом элемента для потока `str`.
...
*Маска* флаги для установки.

### <a name="return-value"></a>Возвращаемое значение

Манипулятор возвращает объект, извлеченные из или вставить в поток `str`, вызовы **str**. [setf](../standard-library/ios-base-class.md#setf)(_ *маска*), а затем возвращает `str`.
...
Манипулятор возвращает объект, извлеченные из или вставить в поток `str`, вызовы **str**. [точность](../standard-library/ios-base-class.md#precision)(`Prec`), а затем возвращает `str`.
...
*Расширенный* ширину отображаемого поля.

### <a name="return-value"></a>Возвращаемое значение

Манипулятор возвращает объект, извлеченные из или вставить в поток `str`, вызовы **str**. [Ширина](../standard-library/ios-base-class.md#width)(_ *широкий*), затем возвращает `str`.
