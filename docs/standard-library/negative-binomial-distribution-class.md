---
title: Класс negative_binomial_distribution | Документы Майкрософт
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
...
Структура param_type {typedef negative_binomial_distribution`<`result_type > distribution_type; param_type (result_type k = 1, дважды p = 0,5); result_type k() const; дважды p() const;
