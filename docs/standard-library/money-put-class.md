...
`OutputIterator` Тип итератора, к которому денежных значений функции записывают свои выходные данные.
...
`_Intl` Логическое значение, указывающее тип обозначение денежной единицы, ожидаемого в последовательности: **true** Если международные, **false** Если внутренние.

`_Iosbase` Формат флаг, который при наборе указывает, что символ валюты является необязательным. в противном случае не требуется
