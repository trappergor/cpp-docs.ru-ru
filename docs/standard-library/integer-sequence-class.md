---
title: "Класс integer_sequence | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
...
---
# Класс integer_sequence
[!INCLUDE[vs2017banner](../assembler/inline/includes/vs2017banner.md)]

Представляет последовательность целых чисел. Может использоваться для определения и развертывания пакетов параметров в типах с переменным числом аргументов, например, std::tuple\< T.... >, которые передаются в функции в качестве аргументов.  
...
## <a name="example"></a>Пример  
 Следующий пример основан на исходное предложение [N3658](http://open-std.org/jtc1/sc22/wg21/docs/papers/2013/n3658.html). В нем продемонстрирован способ использования `integer_sequence` для создания `std::tuple` из `std::array<T,N>`, и способ использования `integer_sequence` для получения элементов кортежа.  
 ...
 Функция Применить принимает [std::tuple](../standard-library/tuple-class.md), и создает integer_sequence с использованием `tuple_size` вспомогательный класс. Обратите внимание, что [std::decay_t](../standard-library/decay-class.md)_is необходимо из-за [tuple_size](../standard-library/tuple-size-class-tuple.md) не работает со ссылочными типами. Функция `apply_` распаковывает элементы кортежа и пересылает их как отдельные аргументы при вызове функции. В этом примере функция представляет из себя простое лямбда-выражение, которое выводит значения.  
...
```  
  ...
  Чтобы сделать `index_sequence` для пакета параметров, используйте `index_sequence_for`\<T...>, который является псевдонимом для `make_index_sequence`\<sizeof...(T)>  
