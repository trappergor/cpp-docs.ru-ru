...
> [!NOTE]
> `#include <streambuf>` `#include <istream>`Библиотека > `#include <ios>`iostreamиспользуетинструкции,, и`#include <ostream>` . \<
...
Все объекты, объявленные в этом заголовке, совместно используют свойство довольно необычная — вы можете предположить, что они созданы до любых статических объектов, определяемых вами \<, в записи преобразования, которая включает в себя > iostream. Точно так же можно предположить, что эти объекты не уничтожаются до деструкторов для любых таких статических объектов, которые вы определяете. (При этом выходные потоки очищаются при завершении работы программы.) Поэтому можно безопасно читать из стандартных потоков или записывать в них до запуска программы и после ее завершения.
...
В этом примере устанавливает `cin` бит сбоя в потоке, если он поступает между нечисловыми символами. Программа очищает бит сбоя и удаляет из потока недопустимый символ, чтобы продолжить.
