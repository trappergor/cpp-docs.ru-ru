...
- Если *число* > 0, **n** = *число*. Указанная **d** = (*Высокая* - *низкая*)/ **n** больше нуля с использованием **d** равномерных поддиапазонов, каждый вес назначается следующим образом: `weight[k] = weightfunc(x)`, где **x** = *Low*  +  **k**    d d/2, для k = 0,..., n-1. +  * 
