...
*firstW* первый итератор в списке, на основе которого формируется распределение.

*lastW* последний итератор в списке, на основе которого формируется распределение (неинклюзивное, так как итераторы используют пустой элемент для элемента end).

*weightlist* [initializer_list](../cpp/initializers.md) на основе которого формируется распределение.

*число* число элементов в диапазоне распределения. Если `count==0`, то эквивалентно конструктору по умолчанию (всегда формируется 0).

*Низкий* наименьшее значение в диапазоне распределения.

*высокий уровень* наибольшее значение в диапазоне распределения.
...
- Если *число* > 0, **n** = *число*. Предоставленный **d** = (*высокий уровень* - *низкий*) и **n** больше, чем ноль, с помощью **d** универсальный диапазоны, каждый вес назначается следующим образом: `weight[k] = weightfunc(x)`, где **x** = *низкий* + **k**  *  **d** + **d** / 2, для **k** = 0,..., **n** - 1.
...
*число* число элементов в диапазоне распределения. Если *число* равно 0, это эквивалентно конструктору по умолчанию (всегда формируется 0).
