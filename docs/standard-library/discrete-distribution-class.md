...
*firstW* первый итератор в списке, из которого формируется распределение.

*lastW* последний итератор в списке, из которого формируется распределение (неинклюзивный так, как итераторы используют пустой элемент для конца).

*weightlist* [initializer_list](../cpp/initializers.md) из которого формируется распределение.
...
*Низкая* наименьшее значение в диапазоне распределения.

*высокий уровень* наибольшее значение в диапазоне распределения.

*weightfunc* объект, представляющий функцию вероятности распределения. Параметр и возвращаемое значение должно быть преобразуемым в **двойные**.
