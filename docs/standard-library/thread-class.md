...
`F` Определяемые приложением функции, выполняемый потоком.

`A` Список аргументов, передаваемых `F`.

`Other` Существующий `thread` объекта.
