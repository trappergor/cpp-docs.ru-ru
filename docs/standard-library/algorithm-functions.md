...
Возвращает **значение true** при наличии условия для каждого элемента в заданном диапазоне.
...
Возвращает **значение true** , если условие обнаружено для каждого элемента в указанном диапазоне или если диапазон пуст, и **false** в противном случае.

### <a name="remarks"></a>Примечания

Функция-шаблон возвращает **значение true** , только если для каждой `N` в диапазоне `[0, last - first)``pred(*(first + N))` предикат имеет **значение true**.
...
Возвращает **значение true** , если условие представлено хотя бы один раз в указанном диапазоне элементов.
...
Возвращает **значение true** , если условие обнаружено хотя бы один раз в указанном диапазоне, и **false** , если условие никогда не обнаруживается.

### <a name="remarks"></a>Примечания

Функция шаблона возвращает **значение true** , только если для некоторых `N` в диапазоне

`[0, last - first)``pred(*(first + N))` предикат имеет значение true.
...
**значение true** , если элемент находится в диапазоне, который равен указанному значению или эквивалентен ему. в противном случае — **значение false**.
...
*пред*\
Предикат, используемый для сравнения *значения* с *нижним* или *верхним*значением. Предикат сравнения принимает два аргумента и возвращает **значение true** , если первый имеет смысл меньше второго, и, в противном случае, **значение false**.

### <a name="return-value"></a>Возвращаемое значение

Возвращает ссылку на *нижнюю* , если `value < lower`, или ссылку на *верхний* , если `upper < value`. В противном случае возвращается ссылка на *value*.
...
Итератор вывода, обращающийся к позиции, которая находится за последним элементом в диапазоне назначения, то есть итератор обращается к `result` + (*последний* - *первым*).
...
Итератор вывода, обращающийся к позиции, которая находится за последним элементом в диапазоне назначения, то есть итератор обращается к *дестенд* -(*Last* - *First*).

### <a name="remarks"></a>Примечания

Диапазон источника должен быть допустимым, а в месте назначения должно быть достаточно свободного пространства для всех скопированных элементов.

Алгоритм `copy_backward` накладывает более строгие требования, чем алгоритм `copy`. Его итераторы ввода и вывода должны быть двунаправленными.
...
В диапазоне элементов копирует элементы, которые имеют **значение true** , для указанного условия.
...
Функция шаблона вычисляет `*(dest + N) = *(first + N))` один раз для каждого `N` в диапазоне `[0, count)`, чтобы строго увеличивать значения `N`, начиная с наименьшего значения. Затем оно возвращает значение `dest + N`. Если *dest* и *First* обозначают области хранилища, то *dest* не должен находиться в диапазоне `[first, last)`.
...
Тип разницы `InputIterator`, который подсчитывает количество элементов в диапазоне [*First*, *Last*), имеющие *значение*value.
...
*пред*\
Определяемый пользователем объект функции предиката, задающий условие, когда один элемент меньше другого. Предикат сравнения принимает два аргумента и возвращает **true** , если удовлетворены и **false** , если не удовлетворены.
...
Элементы в вероятном пустом диапазоне, определенном парой итераторов, возвращаемых `equal_range`, будут эквивалентны *значению* в смысле, определенном используемым предикатом.

Сложность алгоритма является логарифмической для итераторов произвольного доступа и линейной в противном случае, с количеством шагов, пропорциональное (*последний* - *первым*).
...
*пред*\
Определенный пользователем объект функции предиката или [лямбда-выражение](../cpp/lambda-expressions-in-cpp.md), определяющее условие, которому должен соответствовать искомый элемент. Унарный предикат принимает один аргумент и возвращает **true** , если это выполнено, или **значение false** , если не удовлетворено. Сигнатура " *пред* " должна быть фактически `bool pred(const T& arg);`, где `T` — это тип, в который `InputIterator` может быть неявно преобразован при разыменовании. Ключевое слово **const** отображается только для иллюстрации того, что объект функции или лямбда-выражение не должны изменять аргумент.

### <a name="return-value"></a>Возвращаемое значение

Входной итератор, который ссылается на первый элемент в диапазоне, удовлетворяющий условию, заданному предикатом (результат предиката равен **true**). Если ни один элемент не найден для удовлетворения предиката, возвращает *Last*.
...
*пред*\
Определенный пользователем объект функции предиката или [лямбда-выражение](../cpp/lambda-expressions-in-cpp.md), определяющее условие, которому не должен соответствовать искомый элемент. Унарный предикат принимает один аргумент и возвращает **true** , если это выполнено, или **значение false** , если не удовлетворено. Сигнатура " *пред* " должна быть фактически `bool pred(const T& arg);`, где `T` — это тип, в который `InputIterator` может быть неявно преобразован при разыменовании. Ключевое слово **const** отображается только для иллюстрации того, что объект функции или лямбда-выражение не должны изменять аргумент.

### <a name="return-value"></a>Возвращаемое значение

Входной итератор, ссылающийся на первый элемент в диапазоне, который не удовлетворяет условию, заданному предикатом (результатом предиката является **false**). Если все элементы соответствуют предикату (предикат приводит к **истине** для каждого элемента), возвращает значение *Last*.
...
Сложность линейная, но не более (*последняя* - *первой*) сравнений.
...
Сложность является линейной, с точной (`last` - `first`) вызовами требуемого генератора.
...
Сложность алгоритма линейная, но не более `2 * ((last1 - first1) - (last2 - first2)) - 1` сравнений для непустых исходных диапазонов.
...
*пред*\
Определяемый пользователем объект функции предиката, задающий условие, когда один элемент меньше другого. Предикат сравнения принимает два аргумента и должен возвращать **значение true** , если первый элемент меньше второго элемента, и **false** в противном случае.
...
Сложность зависит от объема доступной памяти, так как алгоритм выделяет память для временного буфера. Если доступен достаточный объем памяти, наилучшим вариантом является линейный с `(last - first) - 1`ными сравнениями; Если вспомогательная память недоступна, худший случай — `N log(N)`, где *N* = Last * - .*
...
Возвращает **значение true** , если элементы в указанном диапазоне образуют кучу, в противном случае **false** .
...
Возвращает итератор, расположенный на первом элементе в диапазоне [`first`, `last`), не удовлетворяющем условию упорядочивания кучи, или *End* , если диапазон образует кучу.
...
*пред*\
Двухместный предикат, который задает условие строгого слабого упорядочения, определяющее кучу. Предикат по умолчанию `std::less<>`, если значение " *пред* " не указано.
...
Первая функция-шаблон Возвращает последний итератор, `next` в `[first, last)`, где `[first, next)` — это куча, упорядоченная по объекту функции `std::less<>`. Если расстояние `last - first` меньше 2, функция возвращает *Last*.

Вторая функция шаблона ведет себя так же, как первая, за исключением того, что в качестве условия упорядочивания кучи вместо `std::less<>` используется предикат с именем « *пред* ».

## <a name="is_partitioned"></a>is_partitioned

Возвращает **значение true** , если все элементы в заданном диапазоне, которые проверяют **истинность** условия, предшествуют элементам, которые проверяют **значение false**.
...
Возвращает **значение true** , если все элементы в заданном диапазоне, которые проверяют **истинность** условия, предшествуют элементам, которые проверяют **false**, и в противном случае возвращает **значение false**.

### <a name="remarks"></a>Примечания

Функция шаблона возвращает **значение true** , только если все элементы в `[first, last)` секционированы по *пред*; то есть все элементы, `X` в `[first, last)`, для которых `pred (X)` истинно, выполняются перед всеми элементами `Y`, для которых `pred (Y)` имеет **значение false**.
...
*пред*\
Предикат, который проверяет равенство и возвращает **логическое**значение.

### <a name="return-value"></a>Возвращаемое значение

**значение true** , если диапазоны можно изменить, чтобы они совпадали в соответствии с предикатом сравнения. в противном случае — **значение false**.

### <a name="remarks"></a>Примечания

`is_permutation` в худшем случае имеет квадратичную сложность.

Первая функция шаблона предполагает, что в диапазоне, начиная с *first2* , имеется столько элементов, сколько находится в диапазоне, обозначенном `[first1, last1)`. Если во втором диапазоне элементов больше, они игнорируются. Если элементов меньше — результат будет непредсказуемым. Третья функция шаблона (C++ 14 и более поздние версии) не использует это допущение. Оба возвращают **значение true** только в том случае, если для каждого элемента X в диапазоне, обозначенном `[first1, last1)`, количество элементов y в том же диапазоне, для которого x = = Y совпадает с диапазоном, начиная с *first2* или `[first2, last2)`. Здесь `operator==` должны выполнить попарное сравнение своих операндов.
...
Возвращает **значение true** , если элементы в указанном диапазоне находятся в отсортированном порядке.
...
Первая функция шаблона возвращает [is_sorted_until](#is_sorted_until)`( first, last ) == last`. Функция `operator<` выполняет сравнение порядка.
...
Вторая версия позволяет предоставить объект функции сравнения, возвращающий **значение true** , если два заданных элемента находятся в отсортированном порядке, и **false** в противном случае.
...
Первая функция-шаблон Возвращает последний итератор, `next` в `[first, last]`, поэтому `[first, next)` является упорядоченной последовательностью, упорядоченной по `operator<`. Если `distance()` меньше 2, функция возвращает *Last*.
...
`swap` следует использовать в качестве предпочтений для **iter_swap**, который был добавлен в C++ стандарт для обеспечения обратной совместимости. Если `Fit1` и `Fit2` являются прямыми итераторами, то `iter_swap( Fit1, Fit2 )`эквивалентна `swap( *Fit1, *Fit2 )`.
...
- Несовпадений не найдено, и последовательности имеют одинаковое число элементов, поэтому последовательности равны, и результат сравнения равен **false**.
...
Сложность является линейной, требует `3 * (last - first)` сравнений.
...
Сложность является линейной: для непустого диапазона требуются `(last - first) - 1` сравнения.
...
Сложность алгоритма линейная, но не более `(last1 - first1) - (last2 - first2) - 1` сравнений.
...
*вывод*\
`initializer_list`, содержащий сравниваемые элементы.

### <a name="return-value"></a>Возвращаемое значение

Меньший из двух объектов; если ни один из них не меньше, то возвращается первый из двух объектов. В случае `initializer_list`он возвращает наименьшее из объектов в списке.

### <a name="remarks"></a>Примечания

При использовании алгоритма `min` очень редко объекты передаются как параметры. Большинство алгоритмов библиотеки стандартных программ C++ работают в диапазоне элементов, позиция которых задается итераторами, передаваемыми в качестве параметров. Если требуется функция, которая работает в диапазоне элементов, используйте [min_element](../standard-library/algorithm-functions.md#min_element). на перегрузках `initializer_list` в Visual Studio 2017 включено [constexpr](../cpp/constexpr-cpp.md) .
...
*пред*\
Определяемый пользователем объект функции предиката, определяющий, в котором один элемент меньше другого. Предикат сравнения принимает два аргумента и должен возвращать **значение true** , если первый из них меньше второго, и **false** в противном случае.
...
Если последовательность не пуста, функция выполняет не более `3 * (last - first - 1) / 2` сравнений.
...
Первая функция-шаблон возвращает `pair<const Type&, const Type&>( right, left )`, если *право* меньше *левого*. В противном случае возвращается значение `pair<const Type&, const Type&>( left, right )`.
...
## <a name="alg_move"></a>Перемещение&gt; &lt;ALG
...
Функция шаблона вычисляет `*(dest + N) = move(*(first + N))` один раз для каждого `N` в диапазоне `[0, last - first)`, чтобы строго увеличивать значения `N`, начиная с наименьшего значения. Затем оно возвращает значение `dest + N`. Если `dest` и *сначала* обозначить области хранилища, то *dest* не должен находиться в диапазоне `[first, last)`.
...
*пред*\
Определяемый пользователем объект функции предиката, задающий критерий сравнения, который должен соблюдаться идущими подряд элементами при упорядочении. Предикат сравнения принимает два аргумента и возвращает **true** , если удовлетворены и **false** , если не удовлетворены.

### <a name="remarks"></a>Примечания

Указанный диапазон должен быть допустимым. Все указатели должны поддерживать удаление ссылок, а последняя позиция в последовательности должна быть доступна из первой позиции за счет увеличения на один.

Алгоритм `nth_element` не гарантирует, что элементы во вложенных диапазонах не будут отсортированы. Таким образом, он дает меньшие гарантии, чем `partial_sort`, который упорядочивает элементы в диапазоне ниже некоторого выбранного элемента, и может использоваться в качестве более быстрой альтернативы `partial_sort`, когда упорядочивание нижнего диапазона не требуется.
...
Возвращает **значение true** , если условие никогда не представлено между элементами в заданном диапазоне.
...
Функция-шаблон возвращает **значение true** , только если для некоторых `N` в диапазоне `[0, last - first)``pred(*(first + N))` предикат всегда имеет **значение false**.
...
Элементы *a* и *b* эквивалентны, но не обязательно равны, если оба `pred( a, b )` имеют значение false, а `pred( b, a )` имеет значение false, где " *пред* " является предикатом, заданным параметром. Алгоритм `partition` не является стабильным и не гарантирует, что относительный порядок эквивалентных элементов будет сохранен. Алгоритм `stable_partition` сохраняет этот исходный порядок.

Сложность линейная: существуют `(last - first)` приложения, которые являются *допустимыми и не* `(last - first)/2` перестановок.
...
Копирует элементы, для которых условие имеет **значение true** , в одно назначение, для которого условие имеет **значение false** . Эти элементы должны поступать из указанного диапазона.
...
Функция шаблона копирует каждый элемент `X` в `[first,last)`, чтобы `*dest1++`, если `pred(X)` имеет значение true, или `*dest2++` в противном случае. Он возвращает `pair<OutputIterator1, OutputIterator2>(dest1, dest2)`.
...
Возвращает `ForwardIterator`, ссылающийся на первый элемент, который не удовлетворяет условию, проверенному на *допустимость, или*возвращает *Last* , если он не найден.

### <a name="remarks"></a>Примечания

Функция шаблона находит первый итератор, `it` в `[first, last)`, для которого `pred(*it)` имеет **значение false**. Последовательность должна быть упорядочена по *пред*.
...
Сложность является логарифмической, что требует не более `log (last - first)` сравнений.
...
**значение true** , если предыдущая перестановка лексикографически существует и заменила исходный порядок диапазона. в противном случае — **значение false**, в этом случае упорядочение преобразуется в максимальную перестановку лексикографически.
...
Сложность линейная, с максимумом (`last` - `first`)/2 подкачки.
...
Функция std:: random_shuffle () является устаревшей, она заменена [std::](../standard-library/algorithm-functions.md#shuffle)Replace. Пример кода и дополнительные сведения см. в разделе [\<random >](../standard-library/random.md) и Stack overflow POST, [почему методы std:: random_shuffle являются устаревшими в c++ 14?](https://go.microsoft.com/fwlink/p/?linkid=397954).
...
Сложность линейная; Существуют сравнения (`last` - `first`) на равенство.

[Класс List](../standard-library/list-class.md) имеет более эффективную версию функции-члена `remove`, которая также повторно связывает указатели.
...
Сложность линейная; Существуют сравнения (`last` - `first`) на равенство и не более (`last` - `first`).
...
Сложность линейная; Существуют сравнения (`last` - `first`) на равенство и максимум (`last` - `first`) назначений новых значений.
...
Алгоритм `replace_if` является обобщением алгоритма `replace`, что позволяет указать любой предикат, а не равенство заданному значению константы.
...
Сложность линейная, но не более (`last` - `first`).
...
*пред*\
Определяемый пользователем объект функции предиката, задающий условие, когда один элемент меньше другого. Бинарный предикат принимает два аргумента и должен возвращать значение **true** , когда первый элемент меньше второго элемента, и значение **false** в противном случае.
...
Сложность алгоритма линейная, но не более `2 * ((last1 - first1) - (last2 - first2)) - 1` сравнений.
...
*пред*\
Определяемый пользователем объект функции предиката, задающий критерий сравнения, который должен соблюдаться идущими подряд элементами при упорядочении. Этот бинарный предикат принимает два аргумента и возвращает **значение true** , если два аргумента указаны в порядке, и **false** в противном случае. Эта функция средства сравнения должна задать строгое слабое упорядочение пар элементов последовательности. Дополнительные сведения см. в разделе [Алгоритмы](../standard-library/algorithms.md).
...
Среднее значение сложности сортировки — `O( N log N )`, где *N* = *Last* * - .*
...
Сложность наиболее `N log N`, где *N* = *последний* * - .*
...
*пред*\
Определенный пользователем объект функции предиката, задающий условие, которое должно удовлетворяться, чтобы элемент был классифицирован. Унарный предикат принимает один аргумент и возвращает **true** , если это выполнено, или **значение false** , если не удовлетворено.
...
Элементы *a* и *b* эквивалентны, но не обязательно равны, если оба `pred( a, b )` имеют значение false, а `pred( b, a )` имеет значение false, где " *пред* " является предикатом, заданным параметром. Алгоритм `stable_partition` является стабильным и гарантирует, что относительный порядок эквивалентных элементов будет сохранен. Алгоритм `partition` не обязательно сохраняет этот исходный порядок.
...
Если ни один из элементов не меньше другого, то эти элементы эквивалентны, но не обязательно равны. Алгоритм `sort` является стабильным и гарантирует, что относительный порядок эквивалентных элементов будет сохранен.

Сложность среды выполнения `stable_sort` зависит от объема доступной памяти, но в лучшем случае (при наличии достаточного объема памяти) `O(N log N)`, а в худшем случае — `O(N (log N)^2)`, где *N* = *последний* - *первым*. Обычно алгоритм `sort` значительно быстрее, чем `stable_sort`.
...
Сложность линейна с помощью *last1* - *first1* . Если переставляются элементы из контейнеров одного и того же типа, то должна использоваться функция-член `swap` из этого контейнера, так как функция-член обычно имеет постоянную сложность.
...
## <a name="transform"></a>Преобразует
...
Сложность линейная, но не более (`last1` - `first1`).
...
Сложность является линейной, требует `(last - first) - 1` сравнений.
...
Сложность является линейной, требует сравнения (`last` - `first`).
...
*пред*\
Определяемый пользователем объект функции предиката сравнения, определяющий, в котором один элемент меньше другого. Предикат сравнения принимает два аргумента и возвращает **true** , если удовлетворены и **false** , если не удовлетворены.
...
Отсортированный диапазон — это предварительное условие использования `upper_bound`, где критерий упорядочивания совпадает с указанным предикатом сравнения.
