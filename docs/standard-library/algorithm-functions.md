...
Функция-шаблон возвращает **значение true** , только если для каждой `N` в диапазоне `[0, last - first)` предикат `pred(*(first + N))` имеет **значение true**.
...
*Возможен*\
Определенный пользователем объект функции предиката или [лямбда-выражение](../cpp/lambda-expressions-in-cpp.md), определяющее условие, которому не должен соответствовать искомый элемент. Унарный предикат принимает один аргумент и возвращает **true** , если это выполнено, или **значение false** , если не удовлетворено. Сигнатура " *пред* " должна быть эффективной `bool pred(const T& arg);` , где `T` — это тип, который `InputIterator` может быть неявно преобразован при разыменовании. Ключевое слово **const** отображается только для иллюстрации того, что объект функции или лямбда-выражение не должны изменять аргумент.
...
Функция-шаблон возвращает **значение true** только в том случае, если все элементы в `[first, last)` секционированы по *пред*; то есть все элементы `X` в, `[first, last)` для которых `pred (X)` имеет значение true, выполняются до всех элементов, `Y` для которых значение `pred (Y)` равно **false**.
...
Первая функция шаблона предполагает, что в диапазоне, начиная с *first2* , имеется столько элементов, сколько находится в диапазоне, заданном параметром `[first1, last1)` . Если во втором диапазоне элементов больше, они игнорируются. Если элементов меньше — результат будет непредсказуемым. Третья функция шаблона (C++ 14 и более поздние версии) не использует это допущение. Оба значения возвращают **значение true** , только если для каждого элемента X в диапазоне, обозначенном параметром, `[first1, last1)` задано количество элементов y в одном диапазоне, для которого x = = Y, в диапазоне, начиная с *first2* или `[first2, last2)` . Здесь `operator==` необходимо выполнить попарное сравнение своих операндов.
...
Первая функция-шаблон возвращает значение `pair<const Type&, const Type&>( right, left )` , если значение *right* меньше *левого*. В противном случае она возвращает `pair<const Type&, const Type&>( left, right )`.
...
Функция-шаблон возвращает **значение true** , только если для некоторых `N` значений в диапазоне `[0, last - first)` предикат `pred(*(first + N))` всегда имеет **значение false**.
