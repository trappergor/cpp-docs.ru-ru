...
`first` Прямой итератор, обращающийся к положению первого элемента в диапазоне для поиска.

`last` Прямой итератор, указывающий на позицию, следующую за последним элементом в диапазоне для поиска.

`comp` Двоичный предикат, задающий условие, которому должны удовлетворять значения соседних элементов в диапазоне, в которой выполняется поиск.
...
`first` Входной итератор, который указывает проверяемое условие. Итератор отмечает, где начинается диапазон элементов.

`last` Входной итератор, который указывает конец диапазона элементов для проверки условия.
...
`first` Входной итератор, который указывает, где следует запустить проверку диапазона элементов для условия.
...
`value` Требуется значение для сравнения, значение элемента или необходимо удовлетворяют условию двоичного предиката значение элемента.

`comp` Объект определяемой пользователем функции предиката, определяющий условие, в котором один элемент меньше другого. Бинарный предикат принимает два аргумента и возвращает `true` в случае соответствия и `false` в случае несоответствия.
...
`first` Входной итератор, обращающийся к положению первого элемента в исходном диапазоне.

`last` Входной итератор, указывающий на позицию, следующую за последним элементом в исходном диапазоне.

*destBeg* выходной итератор, обращающийся к положению первого элемента в диапазоне назначения.
...
`first` Двунаправленный итератор, обращающийся к положению первого элемента в исходном диапазоне.

`last` Двунаправленный итератор, указывающий на позицию, следующую за последним элементом в исходном диапазоне.

`destEnd` Двунаправленный итератор, указывающий положение, следующее за последним элементом в диапазоне назначения.
...
`last` Входной итератор, который указывает конец диапазона.

`dest` Выходной итератор, указывающий назначение для скопированных элементов.

`_Pred` Условие, для которого проверяется каждый элемент в диапазоне. Это условие предоставляется определенным пользователем объектом функции предиката. Предикат принимает один аргумент и возвращает значение `true` или `false`.
...
один раз для каждого `N` в диапазоне `[0, last - first)` строго на увеличение значений `N`, начиная с наименьшего значения. Если `dest` и `first` обозначают области хранилища, `dest` не должно быть в диапазоне `[ first, last )`.
...
`count` Целое число со знаком или без знака тип, указывающий количество элементов для копирования.
...
`first` Входной итератор, обращающийся к положению первого элемента в диапазоне к обходу.

`last` Входной итератор, указывающий на позицию, следующую за последним элементом в диапазоне к обходу.

`val` Значения элементов для подсчета.
...
`first` Входной итератор, обращающийся к положению первого элемента в диапазоне для поиска.

`last` Входной итератор, указывающий на позицию, следующую за последним элементом в диапазоне для поиска.

`_Pred` Объект определяемой пользователем функции предиката, определяющий условие, которое должно выполняться, если элемент является для подсчета. Предикат принимает один аргумент и возвращает значение **true** или **false**.
...
`First1` Входной итератор, указывающий позицию первого элемента в первом диапазоне для тестирования.

`Last1` Входной итератор, указывающий на позицию, следующую за последним элементом в первом диапазоне для тестирования.

`First2` Входной итератор, указывающий позицию первого элемента во втором диапазоне для тестирования.

`First2` Входной итератор, указывающий положение, следующее за последним элементом во втором диапазоне для тестирования.

`Comp` Объект определяемой пользователем функции предиката, определяющий условие, которое должно выполняться, чтобы два элемента считались эквивалентными друг другу. Двоичный предикат принимает два аргумента и возвращает **true** , если условие удовлетворено, или **false** , если условие не удовлетворено.
...
`val` Значение, поиск в упорядоченном диапазоне.

`comp` Объект определяемой пользователем функции предиката, определяющий условие, в котором один элемент меньше другого.
...
`first` Прямой итератор, обращающийся к положению первого элемента в диапазоне к обходу.

`last` Прямой итератор, указывающий на позицию, следующую за последним элементом в диапазоне к обходу.

`val` Значение, присваиваемое элементов в диапазоне [ `first`, `last`).
...
`First` Выходной итератор, указывающий позицию первого элемента в диапазоне для будет присвоено значение `Val`.

`Count` Целое число со знаком или без знака тип, указывающий число элементов, которое будет присвоено значение.

`Val` Значение, присваиваемое элементов в диапазоне [ `First`, *первый + Count*).
...
`first` Входной итератор, указывающий позицию первого элемента в диапазоне для поиска заданного значения.

`last` Входной итератор, указывающий на позицию, следующую за последним элементом в диапазоне для поиска заданного значения.
...
`First1` Прямой итератор, обращающийся к положению первого элемента в диапазоне для поиска.

`Last1` Прямой итератор, указывающий на позицию, следующую за последним элементом в диапазоне для поиска.

`First2` Прямой итератор, обращающийся к положению первого элемента в диапазоне для поиска.

`Last2` Прямой итератор, указывающий на позицию, следующую за последним элементом в диапазоне для поиска.
...
`first2` Прямой итератор, обращающийся к положению первого элемента в диапазоне для сравнения.

`last2` Прямой итератор, указывающий на позицию, следующую за последним элементом в диапазоне для сравнения.
...
`pred` Заданный пользователем объект функции предиката или [лямбда-выражение](../cpp/lambda-expressions-in-cpp.md) , определяющий условие, которое не должно удовлетворяться искомым элементом. Предикат берет один аргумент и возвращает `true` (условие удовлетворено) или `false` (условие не удовлетворено). Подпись `pred` должна быть `bool pred(const T& arg);`, где `T` — тип, к которому можно явным образом привести `InputIterator` при сбросе ссылки. Ключевое слово `const` показано только для демонстрации того, что объект функции или лямбда не должен изменять аргумент.
...
`first` Входной итератор, обращающийся к положению первого элемента в обрабатываемом диапазоне.

`last` Входной итератор, указывающий на позицию, следующую за последним элементом в диапазоне для выполнения операций.
...
`first` Прямой итератор, обращающийся к положению первого элемента в диапазоне, к которому должны быть назначены значения.

`last` Прямой итератор, указывающий на позицию, следующую за последним элементом в диапазоне, к которому должны быть назначены значения.

`_Gen` Объект функции, который вызывается без аргументов, используемый для формирования значений для каждого элемента в диапазоне назначения.
...
`First` Выходной итератор, обращающийся к положению первого элемента в диапазоне, к которому должны быть назначены значения.

`Count` Целое число со знаком или без знака тип, указывающий количество элементов присвоить значение с помощью функции генератора.

`Gen` Объект функции, который вызывается без аргументов, используемый для формирования значений для каждого элемента в диапазоне назначения.
...
`first1` Входной итератор, обращающийся к положению первого элемента в первом из двух упорядоченных исходных диапазонов, которые необходимо проверить для ли все элементы второго содержатся в первом.

`last1` Входной итератор, указывающий на позицию, следующую за последним элементом в первом из двух упорядоченных исходных диапазонов для ли все элементы второго содержатся в первом проверки.

`first2` Входной итератор, указывающий на позицию первого элемента во втором из двух последовательных упорядоченных исходных диапазонов для ли все элементы второго содержатся в первом проверки.

`last2` Входной итератор, указывающий на позицию, следующую за последним элементом во втором из двух последовательных упорядоченных исходных диапазонов для ли все элементы второго содержатся в первом проверки.

`comp` Объект определяемой пользователем функции предиката, определяющий условие, в котором один элемент меньше другого. Двоичный предикат принимает два аргумента и возвращает **true** , если условие удовлетворено, или **false** , если условие не удовлетворено.
...
`first` Двунаправленный итератор, обращающийся к положению первого элемента в первом из двух последовательных упорядоченных диапазонов, которые следует объединить и упорядочить в один диапазон.

`middle` Двунаправленный итератор, обращающийся к положению первого элемента во втором из двух последовательных упорядоченных диапазонов, которые следует объединить и упорядочить в один диапазон.

`last` Двунаправленный итератор, указывающий на позицию, следующую за последним элементом во втором из двух последовательных упорядоченных диапазонов, которые следует объединить и упорядочить в один диапазон.

`comp` Объект определяемой пользователем функции предиката, определяющий условие, в котором один элемент больше другого. Бинарный предикат принимает два аргумента и должен возвращать значение **true** , когда первый элемент меньше второго элемента, и значение **false** в противном случае.
...
`first` Итератор произвольного доступа, указывающий начало диапазона для проверки кучи.

`last` Итератор произвольного доступа, который указывает конец диапазона.

`comp` Условие, чтобы проверить, порядок элементов. Двоичный предикат берет один аргумент и возвращает `true` или `false`.
...
`begin` Итератор произвольного доступа, указывающий первый элемент диапазона для проверки кучи.

`end` Итератор произвольного доступа, который указывает конец диапазона для проверки кучи.

`compare` Бинарный предикат, который задает условие строгого слабого упорядочения, определяющее кучу. Предикат по умолчанию, если параметр `compare` не указан, — `std::less<>`.
...
Первая функция-шаблон возвращает последний итератор `next` в `[ begin , end ]` где `[ begin , next)` является кучей, упорядоченные по объекту функции `std::less<>`. Если расстояние `end - begin < 2`, функция возвращает `end`.
...
`first` Входной итератор, который указывает, где начинается диапазон для проверки условия.
...
`Last1` Прямой итератор, который ссылается на элемент, следующий за последним элементом диапазона.

`First2` Прямой итератор, ссылающийся на первый элемент второго диапазона, используемый для сравнения.

`Last2` Прямой итератор, который ссылается на элемент, следующий за последним элементом второго диапазона, используемый для сравнения.

`Pred` Предикат, который проверяет равенство и возвращает `bool`.
...
`first` Прямой итератор, который указывает, где начинается диапазона для проверки.

`last` Прямой итератор, который указывает конец диапазона.

`comp` Проверяемое условие для определения порядка между двумя элементами. Предикат принимает один аргумент и возвращает значение `true` или `false`. Он выполняет ту же задачу, что и `operator<`.

### <a name="remarks"></a>Примечания

Первая функция-шаблон возвращает [is_sorted_until](http://msdn.microsoft.com/en-us/bbad99d0-deaa-4fe6-ae58-eb5b3e4dded0)`( first, last ) == last`. `operator<` Функция выполняет сравнение заказа.

Вторая функция-шаблон возвращает `is_sorted_until( first, last , comp ) == last`. Функция предиката `comp` выполняет сравнение порядка.
...
`first` Прямой итератор, который указывает начало диапазона для проверки.

`last` Прямой итератор, который указывает конец диапазона.

`comp` Проверяемое условие для определения порядка между двумя элементами. Предикат принимает один аргумент и возвращает значение `true` или `false`.
...
`left` Одно из прямые итераторы, значение которого является обмена.

`right` Второе прямые итераторы, значение которого является обмена.
...
`first2` Входной итератор, обращающийся к положению первого элемента во втором диапазоне для сравнения.
...
`value` Значение, позиции первого или возможные первую позицию поиске в упорядоченном диапазоне.
...
`first` Итератор произвольного доступа, обращающийся к позиции первого элемента в диапазоне должны преобразовываться в куче.

`last` Итератор произвольного доступа, указывающий позицию, следующую за последним элементом в диапазоне должны преобразовываться в куче.
...
`left` Первый из сравниваемых объектов.

`right` Второй из сравниваемых объектов.
...
`first` Прямой итератор, обращающийся к положению первого элемента в диапазоне для поиска самого большого элемента.

`last` Прямой итератор, указывающий на позицию, следующую за последним элементом в диапазоне для поиска самого большого элемента.
...
`first1` Входной итератор, обращающийся к положению первого элемента в первом из двух упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон.

`last1` Входной итератор, указывающий на позицию, следующую за последним элементом в первом из двух упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон.

`first2` Входной итератор, указывающий на позицию первого элемента во втором из двух последовательных упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон.

`last2` Входной итератор, указывающий на позицию, следующую за последним элементом во втором из двух последовательных упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон.

`result` Выходной итератор, обращающийся к положению первого элемента в диапазоне назначения, куда должны быть объединены в один упорядоченный диапазон двух исходных диапазонов.
...
`_IList` Объект initializer_list, содержащий элементы для сравнения.
...
`first` Прямой итератор, обращающийся к положению первого элемента в диапазоне для поиска наименьший элемент.

`last` Прямой итератор, указывающий на позицию, следующую за последним элементом в диапазоне для поиска наименьший элемент.
...
`first` Прямой итератор, который указывает на начало диапазона.

`last` Прямой итератор, который указывает конец диапазона.

`comp` Дополнительный тест используется для элементов заказа.
...
Если последовательность является пустым, то функция будет выполнена не более `3 * (last - first - 1) / 2` сравнения.
...
Первая функция-шаблон возвращает `pair<const Type&, const Type&>( right , left )` Если `right` — меньше, чем `left`. В противном случае возвращает значение `pair<const Type&, const Type&>( left , right )`.
...
`Last2` Входной итератор, указывающий положение, следующее за последним элементом во втором диапазоне для тестирования.

`Comp` Объект определяемой пользователем функции предиката, который сравнивает текущие элементы в каждом диапазоне и определяет, эквивалентны ли они. Он возвращает значение **true** при выполнении условия и значение **false** — в противном случае.
...
`first` Входной итератор, указывающий, откуда начинается диапазон элементов для перемещения.

`last` Входной итератор, который указывает конец диапазона элементов для перемещения.
...
`first` Итератор, который указывает на начало диапазона для перемещения элементов.

`last` Итератор, который указывает конец диапазона для перемещения элементов. Этот элемент не перемещается.
...
`first` Двунаправленный итератор, указывающий на позицию первого элемента в диапазоне, чтобы быть перестановка.

`last` Двунаправленный итератор, указывающий на позицию, следующую за последним элементом в диапазоне, чтобы быть перестановка.

`comp` Объект определяемой пользователем функции предиката, который определяет критерии сравнения, которые должны соблюдаться идущими подряд элементами при упорядочении. Двоичный предикат принимает два аргумента и возвращает **true** , если условие удовлетворено, или **false** , если условие не удовлетворено.
...
`first` Произвольный доступ итератор, обращающийся к положению первого элемента в диапазоне секционирование.

*_Nth* итератор произвольного доступа, обращающийся к положению элемента верным границу секции.

`last` Произвольный доступ итератор, указывающий на позицию, следующую за последним элементом в диапазоне секционирование.
...
`first` Входной итератор, указывающий место запуска для проверки диапазона элементов для условия.

`last` Входной итератор, который указывает конец диапазона элементов.
...
`first` Произвольный доступ итератор, обращающийся к положению первого элемента в диапазоне сортировки.

`sortEnd` Произвольный доступ итератор, указывающий на позицию, следующую за последним элементом в диапазоны должны быть отсортированы.

`last` Произвольный доступ итератор, указывающий на позицию, следующую за последним элементом в диапазоне сортировки частично.
...
`first1` Входной итератор, обращающийся к положению первого элемента в исходном диапазоне.

`last1` Входной итератор, указывающий на позицию, следующую за последним элементом в исходном диапазоне.

`first2` Итератор произвольного доступа, обращающийся к положению первого элемента в упорядоченном диапазоне назначения.

`last2` Итератор произвольного доступа, указывающий на позицию, следующую за последним элементом в упорядоченном диапазоне назначения.

`comp` Объект определяемой пользователем функции предиката, определяющий условие, которое должно выполняться, чтобы два элемента считались эквивалентными друг другу. Бинарный предикат принимает два аргумента и возвращает `true` в случае соответствия и `false` в случае несоответствия.
...
`first` Двунаправленный итератор, обращающийся к положению первого элемента в диапазоне секционирование.

`last` Двунаправленный итератор, указывающий на позицию, следующую за последним элементом в диапазоне секционирование.

`comp` Объект определяемой пользователем функции предиката, определяющий условие, которое должно выполняться, если элемент является классифицировать. Предикат принимает один аргумент и возвращает значение **true** или **false**.
...
`first` Входной итератор, который указывает на начало диапазона для проверки на соответствие условию.

`last` Входной итератор, который указывает конец диапазона.

`dest1` Выходной итератор, используемый для копирования элементов, которые возвращают значение true, если условие тестировать с помощью `_Pred`.

`dest2` Выходной итератор, используемый для копирования элементов, которые возвращают значение false, если условие тестировать с помощью `_Pred`.
...
`first` Объект `ForwardIterator` указывает начало диапазона для проверки условия.

`last` Объект `ForwardIterator` указывает конец диапазона.
...
`first` Итератор произвольного доступа, обращающийся к положению первого элемента в куче.

`last` Итератор произвольного доступа, указывающий на позицию, следующую за последним элементом в куче.
...
`comp` Объект определяемой пользователем функции предиката, который определяет критерии сравнения, которые должны соблюдаться идущими подряд элементами при упорядочении. Бинарный предикат принимает два аргумента и возвращает `true` в случае соответствия и `false` в случае несоответствия.
...
`first` Прямой итератор, обращающийся к положению первого элемента в диапазоне, из которого удаляются элементы.

`last` Прямой итератор, указывающий на позицию, следующую за последним элементом в диапазоне, из которого удаляются элементы.

`val` Значение, чтобы удалить из диапазона.
...
`first` Входной итератор, обращающийся к положению первого элемента в диапазоне, из которого удаляются элементы.

`last` Входной итератор, указывающий на позицию, следующую за последним элементом в диапазоне, из которого удаляются элементы.

`result` Выходной итератор, обращающийся к положению первого элемента в диапазоне назначения, в который удаляются элементы.
...
`_Pred` Унарный предикат, который должен быть удовлетворен является значение элемента для замены.
...
`first` Прямой итератор, указывающий на позицию первого элемента в диапазоне, из которого удаляются элементы.
...
`first` Прямой итератор, указывающий на позицию первого элемента в диапазоне, в котором заменяются элементы.

`last` Прямой итератор, указывающий на позицию, следующую за последним элементом в диапазоне, в котором заменяются элементы.
...
`first` Входной итератор, указывающий на позицию первого элемента в диапазоне, в котором заменяются элементы.

`last` Входной итератор, указывающий на позицию, следующую за последним элементом в диапазоне, в котором заменяются элементы.

`result` Выходной итератор, указывающий на первый элемент в диапазоне назначения, в который копируется измененная последовательность элементов.
...
`result` Выходной итератор, указывающий на позицию первого элемента в диапазоне назначения, в который копируются элементы.
...
`last` Итератор, указывающий на позицию, следующую за последним элементом в диапазоне, в котором заменяются элементы.
...
`first` Двунаправленный итератор, указывающий на позицию первого элемента в диапазоне, в котором переставляются элементы.

`last` Двунаправленный итератор, указывающий на позицию, следующую за последним элементом в диапазоне, в котором переставляются элементы.
...
`first` Двунаправленный итератор, указывающий на позицию первого элемента в исходном диапазоне, в котором переставляются элементы.

`last` Двунаправленный итератор, указывающий на позицию, следующую за последним элементом в исходном диапазоне, в котором переставляются элементы.
...
`first` Прямой итератор, обращающийся к положению первого элемента в диапазоне для изменения места.

`middle` Прямой итератор, определяющий границу в диапазоне и указывающий на позицию первого элемента во второй части диапазона, элементы которого должны поменяться местами с элементами в первой части диапазона.

`Last` Прямой итератор, указывающий на позицию, следующую за последним элементом в диапазоне для изменения места.
...
_ `Last` Прямой итератор, указывающий на позицию, следующую за последним элементом в диапазоне для изменения места.

`result` Выходной итератор, обращающийся к положению первого элемента в диапазоне назначения.
...
`count` Размер последовательности, поиск которого выполняется.

`val` Значение элементов в последовательности, поиск которого выполняется.
...
`first1` Входной итератор, обращающийся к положению первого элемента в первом из двух упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон, представляющий разность двух исходных диапазонов.

`last1` Входной итератор, указывающий на позицию, следующую за последним элементом в первом из двух упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон, представляющий разность двух исходных диапазонов.

`first2` Входной итератор, обращающийся к положению первого элемента во втором из двух последовательных упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон, представляющий разность двух исходных диапазонов.

`last2` Входной итератор, указывающий на позицию, следующую за последним элементом во втором из двух последовательных упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон, представляющий разность двух исходных диапазонов.

`result` Выходной итератор, обращающийся к положению первого элемента в диапазоне назначения, где два исходных диапазона следует объединить в один упорядоченный диапазон, представляющий разность двух исходных диапазонов.
...
`first1` Входной итератор, обращающийся к положению первого элемента в первом из двух упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон, представляющий пересечение двух исходных диапазонов.

`last1` Входной итератор, указывающий на позицию, следующую за последним элементом в первом из двух упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон, представляющий пересечение двух исходных диапазонов.

`first2` Входной итератор, обращающийся к положению первого элемента во втором из двух последовательных упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон, представляющий пересечение двух исходных диапазонов.

`last2` Входной итератор, указывающий на позицию, следующую за последним элементом во втором из двух последовательных упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон, представляющий пересечение двух исходных диапазонов.

**_** *Результат* выходной итератор, обращающийся к положению первого элемента в диапазоне назначения, где два источника диапазоны, следует объединить в один упорядоченный диапазон, представляющий пересечение двух исходных диапазоны.
...
`first1` Входной итератор, обращающийся к положению первого элемента в первом из двух упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон, представляющий симметрическую разность двух исходных диапазонов.

`last1` Входной итератор, указывающий на позицию, следующую за последним элементом в первом из двух упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон, представляющий симметрическую разность двух исходных диапазонов.

`first2` Входной итератор, обращающийся к положению первого элемента во втором из двух последовательных упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон, представляющий симметрическую разность двух исходных диапазонов.

`last2` Входной итератор, указывающий на позицию, следующую за последним элементом во втором из двух последовательных упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон, представляющий симметрическую разность двух исходных диапазонов.

**_** *Результат* выходной итератор, обращающийся к положению первого элемента в диапазоне назначения, где два источника диапазоны, следует объединить в один упорядоченный диапазон, представляющий симметрическую разность двух исходных диапазонов.
...
`first1` Входной итератор, обращающийся к положению первого элемента в первом из двух упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон, представляющий объединение двух исходных диапазонов.

`last1` Входной итератор, указывающий на позицию, следующую за последним элементом в первом из двух упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон, представляющий объединение двух исходных диапазонов.

`first2` Входной итератор, обращающийся к положению первого элемента во втором из двух последовательных упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон, представляющий объединение двух исходных диапазонов.

`last2` Входной итератор, указывающий на позицию, следующую за последним элементом во втором из двух последовательных упорядоченных исходных диапазонов, которые следует объединить и упорядочить в один диапазон, представляющий объединение двух исходных диапазонов.

**_** *Результат* выходной итератор, обращающийся к положению первого элемента в диапазоне назначения, где два источника диапазоны, следует объединить в один упорядоченный диапазон, представляющий объединение двух исходных диапазонов.
...
`first` Итератор на первый элемент в диапазоне, который необходимо перемешать, включительно. Должен соответствовать требованиям `RandomAccessIterator` и `ValueSwappable`.

`last` Итератор на последний элемент в диапазоне, который необходимо перемешать, за исключением. Должен соответствовать требованиям `RandomAccessIterator` и `ValueSwappable`.

`gen` Генератор случайных чисел, `shuffle()` функция будет использоваться для операции. Должен соответствовать требованиям `UniformRandomNumberGenerator`.
...
`last` Произвольный доступ итератор, указывающий на позицию, следующую за последним элементом в диапазоне сортировки.

`comp` Объект определяемой пользователем функции предиката, который определяет критерии сравнения, которые должны соблюдаться идущими подряд элементами при упорядочении. Этот бинарный предикат принимает два аргумента и возвращает значение `true`, если эти два аргумента упорядочены, или значение `false` в противном случае. Эта функция средства сравнения должна задать строгое слабое упорядочение пар элементов последовательности. Дополнительные сведения см. в разделе [Алгоритмы](../standard-library/algorithms.md).
...
`first` Итератор произвольного доступа, обращающийся к положению первого элемента в куче целевой.

`last` Итератор произвольного доступа, указывающий позицию, следующую за последним элементом в куче целевой.
...
`_Pred` Объект определяемой пользователем функции предиката, определяющий условие, которое должно выполняться, если элемент является классифицировать. Предикат принимает один аргумент и возвращает значение **true** или **false**.
...
`first` Двунаправленный итератор, обращающийся к положению первого элемента в диапазоне сортировки.

`last` Двунаправленный итератор, указывающий на позицию, следующую за последним элементом в диапазоне сортировки.
...
Во время выполнения сложности `stable_sort` зависит от объема доступной памяти, однако лучшим вариантом (при наличии достаточного объема памяти) *O*( *N* журнала *N*) и в худшем случае — *O*( *N* (журнала *N* ) 2), где *N* =  *фамилия - имя.* Как правило, алгоритм **sort** работает значительно быстрее, чем `stable_sort`.
...
`left` Для первого переопределения первый объект для его содержимого обмена. Для второго переопределения — первый массив для обмена его содержимого.

`right` Для переопределения первый, второй объект для его содержимого обмена. Для второго переопределения — второй массив для обмена его содержимого.
...
`first1` Прямой итератор, указывающий на позицию первого первого диапазона, элементы которого являются обмена.

`last1` Прямой итератор, указывающий на элемент, следующий за последняя позиция первого диапазона, элементы которого являются обмена.

`first2` Прямой итератор, указывающий на позицию первой второго диапазона, элементы которого являются обмена.
...
`first1` Входной итератор, обращающийся к положению первого элемента в первом исходном диапазоне должна быть выполнена.

`last1` Входной итератор, указывающий на позицию, следующую за последним элементом в первом исходном диапазоне для выполнения операций.

`first2` Входной итератор, обращающийся к положению первого элемента во втором исходном диапазоне должна быть выполнена.

`result` Выходной итератор, обращающийся к положению первого элемента в диапазоне назначения.

`_Func` Унарный определяемой пользователем функции, объект, используемый в первой версии алгоритма, который применяется к каждому элементу в первом исходном диапазоне или определяемых пользователем (UD) объект бинарной функции используется во второй версии алгоритма, который применяется в прямом порядке парам элементов , для двух исходных диапазонов.
...
`first` Прямой итератор, обращающийся к положению первого элемента в диапазоне сканирования для удаления дубликатов.

`last` Прямой итератор, указывающий на позицию, следующую за последним элементом в диапазоне сканирования для удаления дубликатов.
...
`first` Прямой итератор, обращающийся к положению первого элемента в исходном диапазоне для копирования.

`last` Прямой итератор, указывающий на позицию, следующую за последним элементом в исходном диапазоне для копирования.

`result` Выходной итератор, обращающийся к положению первого элемента в диапазоне назначения, получающем копию с последовательными дубликаты удаляются.
...
`first` Положение первого элемента в диапазоне для поиска.

`last` На позицию, следующую за последним элементом в диапазоне для поиска.

`value` Значение в упорядоченном диапазоне, который должен быть превышен по значению, подразумевающие итератора, возвращаемого элемента.
