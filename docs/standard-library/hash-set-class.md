...
`Key` Тип данных элемента для сохранения в hash_set.
...
`Allocator` Тип, представляющий сохраненный объект распределителя, инкапсулирующий сведения о выделении и освобождении памяти объекта hash_set. Этот аргумент является необязательным, и значение по умолчанию — **распределителя***\<ключа>.*
...
`key` Ключ элементов для сопоставления из объекта hash_set.
...
`key` Аргумент ключ для сравнения с ключом сортировки элемента из объекта hash_set, в которой выполняется поиск.
...
`first` Положение первого элемента, удаляемого из объекта hash_set.

`last` Положение после последнего элемента, удаляемого из объекта hash_set.

`key` Ключ элементов, удаляемого из объекта hash_set.
...
`key` Ключ аргумента, к которым сравнивается ключ сортировки элемента из объекта hash_set, в которой выполняется поиск.
...
`right` Аргумент hash_set, предоставляющий элементы, которые будут заменены целевой hash_set.
