...
*Ключ* тип данных элемента для сохранения в hash_set.

*Признаки* тип, который включает два объекта функций: один из класса, то есть сравнения бинарным предикатом, способным сравнить два значения элементов как ключа сортировки для определения их относительного порядка и хэш-функцией, унарного предиката сопоставления ключевых значений элементы для целых чисел без знака типа `size_t`. Этот аргумент является необязательным и `hash_compare`*<Key,* **менее***\<ключ> >* значение по умолчанию.

*Распределитель* тип, представляющий сохраненный объект распределителя, инкапсулирующий сведения о выделении и освобождении памяти hash_set. Этот аргумент является необязательным, и значение по умолчанию — **распределителя***\<ключа>.*
...
Hash_set Упорядочивает управляемую им последовательность путем вызова сохраненного `Traits` объект типа [value_compare](#value_compare). Доступ к этому хранимому объекту можно получить через вызов функции-члена [key_comp](#key_comp). Такой объект-функция должен вести себя так же, как объект класса *hash_compare<Key, less\<Key> >.* В частности, для всех значений `key` типа Key вызов Trait (`key`) создает распределение значений типа size_t.
...
|[const_pointer](#const_pointer)|Тип, предоставляющий указатель на **const** элемент `hash_set`.|
|[const_reference](#const_reference)|Тип, предоставляющий ссылку на **const** элемент хранится в `hash_set` для чтения и выполнения **const** операций.|
|[const_reverse_iterator](#const_reverse_iterator)|Тип, предоставляющий двунаправленный итератор, который может читать любой **const** элемент `hash_set`.|
...
`allocator_type` является синонимом для параметра-шаблона *распределителя*.

Дополнительные сведения о *распределителя*, см. в разделе "Примечания" [класс hash_set](../standard-library/hash-set-class.md) раздела.
...
Если возвращаемое значение `begin` назначается `const_iterator`, невозможно изменить элементы в объекте hash_set. Если возвращаемое значение `begin` назначается `iterator`, элементы в объекте hash_set object можно изменить.
...
*ключ* ключ элементов для сопоставления из объекта hash_set.
...
`emplace` Функция-член возвращает пару, **bool** компонент возвращает **true** Если вставка была выполнена и **false** Если `hash_set` уже содержит элемент, ключ которого имел эквивалентное значение порядка, и компонент итератора которой возвращает адрес, где был вставлен новый элемент, или где уже находился элемент.
...
`end` используется для проверки, достиг ли итератор конца своего объекта hash_set. Значение, возвращаемое `end`, не должно быть подвергнуто удалению ссылки.
...
*ключ* ключ-аргумент для сравнения с ключом сортировки элемента из hash_set в котором выполняется поиск.
...
*_Where* положение элемента, удаляемого из объекта hash_set.

*Первый* положение первого элемента, удаляемого из объекта hash_set.

*последний* положение сразу после последнего элемента, удаляемого из объекта hash_set.

*ключ* ключ элементов, удаляемый из объекта hash_set.
...
*ключ* ключ-аргумент для сопоставления с ключом сортировки элемента из hash_set в котором выполняется поиск.

### <a name="return-value"></a>Возвращаемое значение

`iterator` Или `const_iterator` , адресующий положение элемента, эквивалентного указанному ключу или адресует положение после последнего элемента в hash_set, если совпадений не найдено для ключа.

### <a name="remarks"></a>Примечания

Функция-член возвращает итератор, который адресует элемент в hash_set, ключ сортировки которого является `equivalent` аргумент ключу согласно двоичному предикату, применяющему упорядочение на основе менее-чем отношения.

Если возвращаемое значение `find` назначается `const_iterator`, объект hash_set изменить нельзя. Если возвращаемое значение `find` назначается `iterator`, то объект hash_set можно изменить.
...
Распределитель, использующийся hash_set для управления памятью, который является параметром-шаблоном *распределителя*.
...
Первый `insert` функция-член возвращает пару, **bool** компонент возвращает **true** Если вставка была выполнена и **false** Если `hash_set` уже содержит элемент, ключ которого имел эквивалентное значение порядка, и компонент итератора которой возвращает адрес, где был вставлен новый элемент, или где уже находился элемент.

Для доступа к компоненту-итератору пары `pr`, возвращаемой этой функцией-членом, используйте `pr.first`, а для его разыменования — `*(pr.first)`. Чтобы получить доступ к **bool** компонент пары `pr` возвращенной этой функцией-членом, используйте `pr.second`, а для его разыменования используйте `*(pr.second)`.
...
Возвращает объект-функцию, объект hash_set использует для упорядочивания своих элементов, который является параметром шаблона *Traits*.

Дополнительные сведения о *Traits* см. в разделе [класс hash_set](../standard-library/hash-set-class.md) раздела.
...
Дополнительные сведения о *ключ*, см. в разделе "Примечания" [класс hash_set](../standard-library/hash-set-class.md) раздела.
...
`iterator` Или `const_iterator` , расположение элемента в hash_set с ключом, равным или больше, чем ключ аргумента либо адресует положение после последнего элемента в hash_set, если соответствие найдено для ключа.
...
После удаления всех существующих элементов в `hash_set`, `operator=` копирует или перемещает содержимое *правой* в `hash_set`.
...
*правом* аргумент hash_set, предоставляющий элементы для обмена местами с целевой объект hash_set.
...
`iterator` Или `const_iterator` , расположение элемента в hash_set с ключом, равным или больше, чем ключ аргумента, или адресует положение после последнего элемента в hash_set, если соответствие найдено для ключа.
...
Возвращает объект-функцию, объект hash_set использует для упорядочивания своих элементов, который является параметром шаблона *сравнения*.

Дополнительные сведения о *сравнения*, см. в разделе "Примечания" [класс hash_set](../standard-library/hash-set-class.md) раздела.
...
Обратите внимание, что оба [value_compare](../standard-library/set-class.md#value_compare) и [key_compare](../standard-library/set-class.md#key_compare) являются синонимами для параметра-шаблона *сравнения*. Оба типа предоставляются для классов hash_set и hash_multiset, где они идентичны, для совместимости с классами hash_map и hash_multimap, где они различаются.
...
Обратите внимание, что оба [key_compare](#key_compare) и `value_compare` являются синонимами для параметра-шаблона *Traits*. Оба типа предоставляются для классов hash_set и hash_multiset, где они идентичны, для совместимости с классами hash_map и hash_multimap, где они различаются.
