...
*Признаки*\
Тип, который содержит два объекта-функции, один из классов Compare, который является бинарным предикатом, который может сравнивать два значения элементов как ключи сортировки для определения их относительного порядка, и хэш-функция, которая является унарным предикатом, сопоставленным ключевым значениям элементов, которые не подписаны целые числа типа `size_t`. Этот аргумент является необязательным, и в качестве значения по умолчанию используется `hash_compare<Key, less<Key> >`.
...
Hash_set упорядочивает последовательность, которую он управляет, вызывая хранимый хэш- `Traits` объект типа [value_compare](#value_compare). Доступ к этому хранимому объекту можно получить через вызов функции-члена [key_comp](#key_comp). Такой объект-функция должен вести себя так же, как объект класса *hash_compare<Key, less\<Key> >.* В частности, для всех `key` значений типа Key признак вызова (`key`) дает распределение значений типа size_t.
...
|[const_pointer](#const_pointer)|Тип, предоставляющий указатель на элемент **const** в `hash_set`.|
|[const_reference](#const_reference)|Тип, предоставляющий ссылку на элемент **const** , хранящийся в, `hash_set` для чтения и выполнения операций **const** .|
|[const_reverse_iterator](#const_reverse_iterator)|Тип, предоставляющий двунаправленный итератор, который может читать любой элемент **const** в `hash_set`.|
...
Дополнительные сведения о *распределителя*см. в разделе "Примечания" статьи о [классе hash_set](../standard-library/hash-set-class.md) .
...
Если возвращаемое значение `begin` присваивается `const_iterator`, то элементы в объекте hash_set не могут быть изменены. Если возвращаемое значение `begin` присваивается `iterator`, то элементы в объекте hash_set можно изменить.
...
`hash_set`   Функция- членвозвращаетпару,компонентboolкоторойвозвращаетзначениеtrue,еслибылавыполненавставка,изначениеfalse,еслиобъектужесодержалэлемент,ключкоторогоимеетэквивалентноезначениевупорядочении,асвойство`emplace` компонент итератора возвращает адрес, по которому был вставлен новый элемент или где уже был найден элемент.
...
|-|-|
|*Val*|Значение элемента для вставки в [hash_set](../standard-library/hash-set-class.md), если `hash_set` уже не содержит этот элемент, или, в более общем случае, элемент, ключ которого эквивалентно упорядочен.|
|*_Where*|Место начала поиска правильной точки вставки. (Вставка может происходить в нерегулярное время, а не на логарифмическое время, если точка вставки сразу следует за *_Where*.)|
...
`end`используется для проверки того, достиг ли итератор конца его hash_set. Значение, возвращаемое `end`, не должно быть подвергнуто удалению ссылки.
...
Функция-член возвращает итератор, который обращается к элементу в hash_set, ключ сортировки `equivalent` которого является ключом аргумента в бинарном предикате, который вызывает упорядочивание на основе отношения сравнения "меньше чем".

Если возвращаемое значение `find` присваивается `const_iterator`, то объект hash_set изменить нельзя. Если возвращаемое значение `find` присваивается `iterator`, то объект hash_set можно изменить.
...
Распределитель, используемый hash_set для управления памятью, который является распределителем параметров шаблона .
...
|*Соответствовал*|Функция сравнения типа `const Traits`, используемая для упорядочивания элементов в `hash_set`. Значение по умолчанию — `hash_compare`.|
...
Четвертый и пятый конструкторы указывают копию объекта `hash_set`. `Right`
...
|*Началь*|Позиция первого элемента для копирования из `hash_set`.|
|*Последняя*|Позиция сразу после последнего элемента для копирования из `hash_set`.|
|*Интерфейс*|Объект initializer_list, из которого копируются элементы.|

### <a name="return-value"></a>Возвращаемое значение

Первая `insert` функция-член возвращает пару, компонент **bool** которой возвращает **значение true** , если была выполнена вставка, и **значение false** , `hash_set` если объект уже содержал элемент, ключ которого имеет эквивалентное значение в упорядочении. компонент итератора, который возвращает адрес, куда был вставлен новый элемент или где уже был найден элемент.

Для доступа к компоненту-итератору пары `pr`, возвращаемой этой функцией-членом, используйте `pr.first`, а для его разыменования — `*(pr.first)`. Чтобы получить доступ  к логическому компоненту `pr` пары, возвращаемой этой функцией- `pr.second`членом, используйте, и для разыменования этой функции используйте `*(pr.second)`.
...
Дополнительные сведения о *характеристиках* см. в разделе [класс hash_set](../standard-library/hash-set-class.md) .
...
Дополнительные сведения о *ключе*см. в разделе "Примечания" статьи о [классе hash_set](../standard-library/hash-set-class.md) .
...
Объект `iterator` или`const_iterator` , который обращается к расположению элемента в hash_set с ключом, который равен ключу аргумента или больше него, либо обращается к расположению, следующему за последним элементом в hash_set, если совпадение для ключа не найдено.
...
После стирания любых существующих элементов `hash_set`в, `operator=` копирует или перемещает `hash_set`содержимое *непосредственно* в.
...
Дополнительные сведения о *сравнении*см. в подразделе "Примечания" статьи о [классе hash_set](../standard-library/hash-set-class.md) .
...
Обратите внимание [](#key_compare) , что `value_compare` оба key_compare и являются синонимами для признаков параметра *шаблона.* Оба типа предоставляются для классов hash_set и hash_multiset, где они идентичны, для совместимости с классами hash_map и hash_multimap, где они различаются.
