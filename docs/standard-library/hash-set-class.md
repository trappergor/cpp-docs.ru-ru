...
*Признаки*<br/>
 Тип, который включает в себя два объекта-функции, один для сравнения классов то есть бинарным предикатом, способным сравнить два значения элементов как ключи сортировки для определения их относительного порядка и хэш-функцией, — это ключевые значения унарного предиката сопоставления элементов в числа без знака целых чисел типа `size_t`. Этот аргумент является необязательным, и в качестве значения по умолчанию используется `hash_compare<Key, less<Key> >`.
