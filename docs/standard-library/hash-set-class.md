...
|[begin](#cbegin)|Возвращает итератор const, обращающийся к первому элементу в `hash_set`.|
...
Функция-член `emplace` возвращает пару, компонент **bool** которой возвращает **значение true** , если была выполнена вставка, и **значение false** , если `hash_set` уже содержала элемент, ключ которого имеет эквивалентное значение в упорядочении, а его итератор Компонент возвращает адрес, по которому был вставлен новый элемент или где уже был найден элемент.
...
@No__t_0 или `const_iterator`, который обращается к расположению элемента, эквивалентному указанному ключу, или, если соответствие для ключа не найдено, в hash_set.

### <a name="remarks"></a>Заметки

Функция-член возвращает итератор, который обращается к элементу в hash_set, чей ключ сортировки `equivalent` ключу аргумента в двоичном предикате, который вызывает упорядочивание на основе отношения сравнения "меньше чем".
...
Четвертый и пятый конструкторы указывают копию `Right` `hash_set`.
...
Первая функция-член `insert` возвращает пару, компонент **bool** которой возвращает **значение true** , если была выполнена вставка, и **значение false** , если `hash_set` уже содержала элемент, ключ которого имеет эквивалентное значение в упорядочении, а итератор Компонент возвращает адрес, по которому был вставлен новый элемент или где уже был найден элемент.

Для доступа к компоненту-итератору пары `pr`, возвращаемой этой функцией-членом, используйте `pr.first`, а для его разыменования — `*(pr.first)`. Чтобы получить доступ к **логическому** компоненту пары `pr`, возвращаемых этой функцией-членом, используйте `pr.second` и для разыменования, используйте `*(pr.second)`.
...
@No__t_0 или `const_iterator`, который обращается к расположению элемента в hash_set с ключом, равным или больше, чем ключ аргумента или который обращается к расположению после последнего элемента в hash_set, если совпадение для ключа не найдено.
...
После удаления существующих элементов в `hash_set` `operator=` копирует или перемещает содержимое *непосредственно* в `hash_set`.
...
@No__t_0 или `const_iterator`, который обращается к расположению элемента в hash_set с ключом, который равен ключу аргумента или больше него, либо обращается к расположению, следующему за последним элементом в hash_set, если совпадение для ключа не найдено.
