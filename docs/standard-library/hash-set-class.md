...
Hash_set упорядочивает последовательность, которую он контролирует, вызывая хранимый хэш-`Traits` объекта типа [value_compare](#value_compare). Доступ к этому хранимому объекту можно получить через вызов функции-члена [key_comp](#key_comp). Такой объект-функция должен вести себя так же, как объект класса *hash_compare<Key, less\<Key> >.* В частности, для всех значений `key` типа Key признак вызова (`key`) дает распределение значений типа size_t.
...
Если возвращаемое значение `begin` присваивается `const_iterator`, то элементы в объекте hash_set изменить нельзя. Если возвращаемое значение `begin` присваивается `iterator`, то элементы объекта hash_set могут быть изменены.
...
Доя получения доступа к первому итератору в паре, возвращаемой функцией-членом, используйте `pr`. **first**, а для сброса ссылки на итератор нижней границы — \*( `pr`. **first**). Для доступа к второму итератору пары `pr`, возвращаемой функцией-членом, нужно использовать `pr`. **second**, а для сброса ссылки на итератор верхней границы — \*( `pr`. **second**).
...
`iterator` или `const_iterator`, который обращается к расположению элемента, эквивалентному указанному ключу, или к расположению, следующему за последним элементом в hash_set, если совпадение для ключа не найдено.

### <a name="remarks"></a>Remarks

Функция-член возвращает итератор, который обращается к элементу в hash_set, ключ сортировки которого `equivalent` в ключ аргумента в двоичном предикате, который вызывает упорядочивание на основе отношения сравнения "меньше чем".

Если возвращаемое значение `find` назначается `const_iterator`, то объект hash_set изменить нельзя. Если возвращаемое значение `find` назначается `iterator`, то объект hash_set можно изменить.
...
|*Первая*|Позиция первого элемента для копирования из `hash_set`.|
...
Первая функция-член `insert` возвращает пару, компонент **bool** которой возвращает **значение true** , если была выполнена вставка, и **значение false** , если `hash_set` уже содержал элемент, ключ которого имеет эквивалентное значение в упорядочении, а компонент итератора возвращает адрес, куда был вставлен новый элемент, или место, где элемент уже был найден.
...
`iterator` или `const_iterator`, который обращается к расположению элемента в hash_set с ключом, который равен или больше, чем ключ аргумента, или к месту последнего элемента в hash_set, если совпадение для ключа не найдено.
...
`iterator` или `const_iterator`, который обращается к расположению элемента в hash_set с ключом, который равен ключу аргумента или больше него, либо обращается к расположению, следующему за последним элементом в hash_set, если совпадение для ключа не найдено.
...
Обратите внимание, что и [key_compare](#key_compare) , и `value_compare` являются синонимами для признаков *параметра шаблона*. Оба типа предоставляются для классов hash_set и hash_multiset, где они идентичны, для совместимости с классами hash_map и hash_multimap, где они различаются.
