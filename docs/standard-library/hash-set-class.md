...
*Черты*\
Тип, который включает в себя два функциональных объекта, один из класса сравнить, что является двоичным предикат омичет два значения элемента, как сортировать ключи для определения `size_t`их относительного порядка и хэш-функции, которая является неамодельным предикатом отображение ключевых значений элементов для неподписанных integers типа. Этот аргумент является необязательным, и в качестве значения по умолчанию используется `hash_compare<Key, less<Key> >`.
...
|[const_pointer](#const_pointer)|Тип, который обеспечивает указатель на элемент `hash_set` **const** в .|
|[const_reference](#const_reference)|Тип, который предоставляет ссылку на **конст** элемент, хранящийся в `hash_set` для чтения и выполнения **конст-операций.**|
|[const_reverse_iterator](#const_reverse_iterator)|Тип, который обеспечивает двунаправленный итератор, который может `hash_set`читать любой **конст элемент** в .|
...
|[Итератор](#iterator)|Тип, предоставляющий двунаправленный итератор, который может считывать или изменять любой элемент в `hash_set`.|
...
|[указатель](#pointer)|Тип, предоставляющий указатель на элемент в `hash_set`.|
|[Ссылки](#reference)|Тип, предоставляющий ссылку на элемент, хранящийся в контейнере `hash_set`.|
...
|-|-|
|[Начать](#begin)|Возвращает итератор, обращающийся к первому элементу в `hash_set`.|
...
|[Ясно](#clear)|Стирает все элементы в `hash_set`.|
...
|[Пустой](#empty)|Проверяет, пуст ли `hash_set`.|
...
|[Найти](#find)|Возвращает итератор, адресующий расположение элемента в наборе `hash_set` с ключом, эквивалентным указанному ключу.|
|[get_allocator](#get_allocator)|Возвращает копию объекта `allocator`, который используется для создания контейнера `hash_set`.|
|[Вставить](#insert)|Вставляет элемент или диапазон элементов в `hash_set`.|
...
|[Размер](#size)|Возвращает количество элементов в контейнере `hash_set`.|
|[Своп](#swap)|Выполняет обмен элементами между двумя объектами `hash_set`.|
...
|Оператор|Описание|
|-|-|
|[hash_set::оператор](#op_eq)|Заменяет элементы `hash_set` копией другого `hash_set`.|
...
Для получения дополнительной информации о *Allocator*, см [hash_set.](../standard-library/hash-set-class.md)

### <a name="example"></a>Пример

См. пример для [get_allocator](#get_allocator) в качестве примера использования `allocator_type`.

## <a name="hash_setbegin"></a><a name="begin"></a>hash_set::начало
...
Если значение `begin` возврата назначено элементу `const_iterator`hash_set объекте, не может быть изменено. Если значение `begin` возврата назначено объекту, `iterator`элементы hash_set объектмогут быть изменены.
...
## <a name="hash_setclear"></a><a name="clear"></a>hash_set::ясно
...
## <a name="hash_setdifference_type"></a><a name="difference_type"></a>hash_set::difference
...
## <a name="hash_setemplace"></a><a name="emplace"></a>hash_set::место
...
|Параметр|Описание|
|-|-|
|*Валь*|Значение элемента для вставки в [hash_set](../standard-library/hash-set-class.md), если `hash_set` уже не содержит этот элемент, или, в более общем случае, элемент, ключ которого эквивалентно упорядочен.|

### <a name="return-value"></a>Возвращаемое значение

Функция `emplace` члена возвращает пару, компонент **bool** **возвращается верно,** если вставка была сделать и **ложной,** если `hash_set` уже содержал элемент, ключ которого имел эквивалентное значение в заказе, и чей компонент итератора возвращает адрес, где новый элемент был вставлен или где элемент уже был расположен.
...
|-|-|
|*Валь*|Значение элемента для вставки в [hash_set](../standard-library/hash-set-class.md), если `hash_set` уже не содержит этот элемент, или, в более общем случае, элемент, ключ которого эквивалентно упорядочен.|
|*_where*|Место начала поиска правильной точки вставки. (Вставка может произойти в амортизированное постоянное время, вместо логарифмического времени, если точка вставки сразу же следует *_Where.)*|
...
## <a name="hash_setempty"></a><a name="empty"></a>hash_set::пустой
...
## <a name="hash_setend"></a><a name="end"></a>hash_set::конец
...
`end`используется для проверки того, достиг ли итерататор конца своего hash_set. Значение, возвращаемое `end`, не должно быть подвергнуто удалению ссылки.
...
Доя получения доступа к первому итератору в паре, возвращаемой функцией-членом, используйте `pr`. **во-первых,** и к dereference более низкий связанный итератор, польза \*( `pr`. **первый**). Для доступа ко второму итератору пары `pr`, возвращаемой функцией-членом, нужно использовать `pr`. **второй**, и dereference верхней границы \*итератора, использование ( `pr`. **второй**).
...
## <a name="hash_setfind"></a><a name="find"></a>hash_set::найти
...
Или `iterator` `const_iterator` то, что адреса расположение элемента, эквивалентного указанному ключу или который адреса местоположения, преуспевающего последний элемент в hash_set, если не найдено совпадение для ключа.

### <a name="remarks"></a>Remarks

Функция члена возвращает итератор, который обращается к элементу `equivalent` в hash_set ключом сортировки которого является ключом аргумента под двоичным предикатом, который индуцирует упорядочение, основанное на меньшем, чем отношение сопоставимости.

Если значение `find` возврата назначено объекту, `const_iterator`hash_set объект не может быть изменен. Если значение `find` возврата назначено объекту, `iterator`hash_set объект может быть изменен.
...
Разлесть, используемая hash_set для управления памятью, которая является параметром шаблона *Allocator.*
...
|Параметр|Описание|
|-|-|
|*Аль*|Класс распределителя памяти для использования с этим объектом `hash_set`. Значение по умолчанию — `Allocator`.|
|*Комп*|Функция сравнения типа `const Traits`, используемая для упорядочивания элементов в `hash_set`. Значение по умолчанию — `hash_compare`.|
|*Правильно*|`hash_set`, копией которого будет создаваемый `hash_set`.|
...
## <a name="hash_setinsert"></a><a name="insert"></a>hash_set::вставка
...
|*Где*|Место начала поиска правильной точки вставки. (Вставка может происходить в амортизированном константном времени вместо логарифмического времени, если точка вставки следует сразу за `_Where`.)|
|*Первый*|Позиция первого элемента для копирования из `hash_set`.|
|*Последний*|Позиция сразу после последнего элемента для копирования из `hash_set`.|
|*IList*|Объект initializer_list, из которого копируются элементы.|

### <a name="return-value"></a>Возвращаемое значение

Функция `insert` первого члена возвращает пару, компонент **bool** возвращается **верным,** если `hash_set` вставка была внесена и **ложна,** если уже содержал элемент, ключ которого имел эквивалентное значение в заказе, и чей компонент итератора возвращает адрес, по которому был вставлен новый элемент или где элемент уже расположен.

Для доступа к компоненту-итератору пары `pr`, возвращаемой этой функцией-членом, используйте `pr.first`, а для его разыменования — `*(pr.first)`. Чтобы получить доступ к компоненту **bool** `pr` пары, `pr.second`возвращенной этой функцией `*(pr.second)`члена, и отойди его, используйте.
...
Возвращает объект функции, который hash_set использует для заказа своих элементов, то есть параметр шаблона *Traits.*

Для получения дополнительной информации о *Traits* смотрите тему [hash_set класса.](../standard-library/hash-set-class.md)
...
которая возвращается `_xVal` **верно,** если предшествует и не равна `_yVal` в порядке сортировки.
...
Обратите внимание, что и `key_compare` [value_compare](#value_compare) являются синонимами параметра *шаблона Traits.* Оба типа предоставляются для классов set и multiset, где они идентичны, для совместимости с классами map и multimap, где они различаются.
...
Для получения дополнительной информации о *ключе*, смотрите в разделе Замечания [hash_set тема класса.](../standard-library/hash-set-class.md)
...
Или `iterator` `const_iterator` то, что адреса расположение элемента в hash_set, что с ключом, который равен или больше, чем ключ аргумента или что адреса местоположения, преуспевающего последний элемент в hash_set если не найдено совпадение для ключа.
...
## <a name="hash_setoperator"></a><a name="op_eq"></a>hash_set::оператор
...
|Параметр|Описание|
|-|-|
|*Правильно*|[Hash_set](../standard-library/hash-set-class.md), который будет копироваться в `hash_set`.|

### <a name="remarks"></a>Remarks

После стирания любых `hash_set`существующих `operator=` элементов в , либо копии или перемещает содержимое *право* в `hash_set`.
...
## <a name="hash_setpointer"></a><a name="pointer"></a>hash_set::pоинтер
...
## <a name="hash_setreference"></a><a name="reference"></a>hash_set::ссылка
...
## <a name="hash_setsize"></a><a name="size"></a>hash_set:размер
...
Возвращает объект функции, который hash_set использует для заказа своих элементов, который является параметром шаблона *Сравнить.*

Для получения дополнительной информации о *Сравнении*, см [hash_set.](../standard-library/hash-set-class.md)
...
Обратите внимание, что [value_compare](../standard-library/set-class.md#value_compare) и [key_compare](../standard-library/set-class.md#key_compare) являются синонимами параметра *ШаблонА Сравнить.* Оба типа предоставляются для классов hash_set и hash_multiset, где они идентичны, для совместимости с классами hash_map и hash_multimap, где они различаются.
...
Обратите внимание, `value_compare` что как [key_compare, так](#key_compare) и являются синонимами параметра *шаблона Traits.* Оба типа предоставляются для классов hash_set и hash_multiset, где они идентичны, для совместимости с классами hash_map и hash_multimap, где они различаются.
