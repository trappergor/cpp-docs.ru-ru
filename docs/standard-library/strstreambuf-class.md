 ...
 *_Allocfunc*  
 ...
 `count`  
 Определяет длину буфера, на которую указывает `_Getptr`. Если `_Getptr` не является аргументом (первая форма конструктора), указан рекомендуемый размер выделения памяти для буферов.  
