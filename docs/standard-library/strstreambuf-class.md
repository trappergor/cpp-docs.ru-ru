...
ms.locfileid: "68459079"
---
# <a name="strstreambuf-class"></a>Класс strstreambuf

Описывает буфер потока, который управляет передачей элементов в последовательность элементов, хранящихся в объекте массива **char** , и из нее.
...
Если *_Freezeit* имеет значение true, функция изменяет сохраненный `strstreambuf` режим, чтобы сделать управляемую последовательность замороженной. В противном случае управляемая последовательность зафиксирована не будет.
...
Если функции не удалось выполниться успешно, возвращается значение `EOF`. В противном случае, если  *\_meta* == `EOF`возвращает другое значение, `EOF`отличное от. В противном случае он возвращает  *\_meta*.

### <a name="remarks"></a>Примечания

`EOF`Если  *\_meta* ! =, Защищенная виртуальная функция с членом пытается вставить `(char)_Meta` элемент в выходной буфер. Для этого существует несколько способов.
...
*Если\_объект meta* == ,элементдляобратнойпередачифактическиявляетсяуженаходящегосявпотокедотекущегоэлемента.`EOF` В противном случае этот элемент заменяется на `ch = (char)_Meta`. Функция может передать элемент обратно различными способами.
...
В частности, если [pptr](../standard-library/basic-streambuf-class.md#pptr) является пустым указателем, функция возвращает нуль. В противном случае `pptr`возвращается  -  [PBase](../standard-library/basic-streambuf-class.md#pbase).
...
- Если `_Way == ios_base::beg`значение равно, то Новая координата является началом потока и *_Off*.

- Если `_Way == ios_base::cur`значение равно, то Новая единица является текущей позицией в потоке плюс *_Off*.

- Если `_Way == ios_base::end`значение равно, то новая точка является концом потока и *_Off*.
...
В противном `_Which & ios_base::out` случае, если имеет значение, отличное от нуля, и существует выходной буфер, функция изменяет следующую точку для записи в выходной буфер. В противном случае операция размещения завершается сбоем. Для успешного выполнения операции размещения итоговая позиция потока должна находиться в управляемой последовательности.
...
Защищенная виртуальная функция-член пытается изменить текущие положения для управляемых потоков. Для объекта класса strstreambuf позиция потока состоит исключительно из смещения потока. Нулевое смещение обозначает первый элемент управляемой последовательности. Новая позицией определяется с помощью *_Sp*.
...
*расчета*\
Определяет длину буфера, на который указывает *_Getptr*. Если *_Getptr* не является аргументом (первой формой конструктора), рекомендуемый размер выделения буферов.
...
Второй конструктор ведет себя так же, как первый, за исключением того, что он сохраняет  *\_аллокфунк* как указатель на функцию для вызова, чтобы выделить память и  *\_фрифунк* в качестве указателя на функцию для вызова для освобождения хранилища.
...
- Если (`count` > 0), то *N* имеет `count`значение.

- Если (`count` = = 0), то *N* имеет `strlen`значение (( **const** `char` * `_Getptr` )).
...
Защищенная виртуальная функция-член пытается извлечь текущий элемент `ch` из входного буфера, затем переместить текущее расположение потока и вернуть элемент в виде (`int`) (`unsigned char`) **CH**. Это можно сделать только одним способом: Если доступная позиции чтения доступна, она принимает `ch` в качестве элемента, хранящегося в позиции чтения, и перемещает следующий указатель для входного буфера.
