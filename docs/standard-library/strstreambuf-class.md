...
Описывает буфер потока, который управляет передачей элементов в и из последовательности элементов, сохраненную в **char** объект массива.
...
Если *_Freezeit* имеет значение true, функция меняет сохраненный `strstreambuf` режим, чтобы сделать управляемую последовательность. В противном случае управляемая последовательность зафиксирована не будет.
...
- Если позиция возврата доступна, и если режим strstreambuf указывает, управляемой последовательности или нет, функция может сохранить `ch` в позиции возврата и уменьшить следующий указатель для входного буфера.
...
- Если (`count` > 0), затем *N* является `count`.
...
Защищенная функция-член пытается извлечь текущий элемент `ch` из входного буфера, затем переместить текущую позицию в потоке и возвратить элемент как (`int`) (`unsigned char`) **ch**. Это можно сделать только одним способом: если позиция чтения доступна, функция принимает `ch` как элемента, хранящегося в этой позиции чтения и перемещает следующий указатель для входного буфера.
