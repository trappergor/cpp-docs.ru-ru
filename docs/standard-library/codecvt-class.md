...
*Байтов* тип, используемый для кодирования символов вне программы.
...
*_State* состояние преобразования, которое поддерживается между вызовами функции-члену.

*first1* указатель на начало преобразуемой последовательности.

*last1* указатель в конец преобразуемой последовательности.

*next1* указатель за пределами преобразованной последовательности для первого непреобразованного символа.

*first2* указатель на начало преобразованной последовательности.

*last2* указатель на конец преобразованной последовательности.

*next2* указатель на `CharType` , происходящая после преобразования последнего `CharType`, первый неизмененный символ в целевой последовательности.
...
*first1* указатель на начало внешней последовательности.

*last1* указатель на конец внешней последовательности.

*_Len2* максимальное число `Byte`s, которое может возвращаться функцией-членом.
...
*next1* ссылка на указатель на первый непреобразованный `CharType`, после последнего `CharType` преобразован.

*first2* указатель на начало преобразованной последовательности.

*last2* указатель на конец преобразованной последовательности.

*next2* ссылка на указатель на первый непреобразованный `Byte`, после последнего `Byte` преобразован.
...
*first2* указатель на первую позицию в диапазоне назначения.

*last2* указатель в последнюю позицию в диапазоне назначения.

*next2* указатель на первый неизмененный элемент в целевой последовательности.
...
*_Len2* максимальное число байтов, которое может возвращаться функцией-членом.
...
*next2* ссылка на указатель на первый непреобразованный `Byte` после преобразования последнего `Byte`.
