...
|[IsReference](#reference)|Тип ссылки на элемент.|
...
|-|-|
|[inArray](#array)|Создает объект массива.|
...
|[пустых](#empty)|Проверяет наличие элементов.|
...
Конструктор `array(const array& right)` копий инициализирует управляемую последовательность с помощью последовательности [*right*`.begin()`, *right*`.end()`). Он используется для указания начальной управляемой последовательности, которая является копией последовательности, управляемой объектом массива *right*.
