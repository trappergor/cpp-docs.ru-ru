...
Функция-член заменяет последовательность, управляемую `*this` на повторение `N` элементов со значением *val*.
...
Функции-члены возвращают ссылку на элемент управляемой последовательности в положении *off*. Если эта позиция недопустима, функция выдает объект класса `out_of_range`.
...
Функции-члены возвращают ссылку на элемент управляемой последовательности в положении *off*. Если эта позиция недопустима, поведение станет неопределенным.

Имеется также не являющаяся членом [получить](array-functions.md#get) функции, доступные для получения ссылки на элемент **массива**.
...
Оператор-член присваивает каждому элементу *правой* соответствующий элемент управляемой последовательности, затем возвращает `*this`. Оно позволяет заменить управляемую последовательность копией управляемой последовательности в *правой*.
...
Функция-член меняет местами управляемые последовательности между `*this` и *правой*. Он выполняет присваивания элементов и вызовы конструктора у количестве, пропорционально пропорциональном `N`.

Имеется также не являющаяся членом [замены](array-functions.md#swap) функция может переставлять местами два **массива** экземпляров.
