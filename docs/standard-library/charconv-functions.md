...
Эти функции преобразования настроены для повышения производительности, а также поддерживают кратчайшие круговые пути. Кратчайшее-циклическое поведение означает, что при преобразовании числа в тип char записывается достаточно точности, чтобы обеспечить восстановление исходного числа при преобразовании этих символов обратно в число с плавающей запятой.

- При преобразовании символов в число числовое значение не обязательно должно завершаться нулем. Аналогично, при преобразовании числа в char результат не завершается нулем.
...
- Функции преобразования не чувствительны к режиму округления во время выполнения.
...
В следующей таблице описывается поведение преобразования с учетом различных сочетаний `fmt` `precision` параметров и. Термин «кратчайший круговой путь» означает запись минимального необходимого количества цифр, чтобы синтаксический анализ этого представления с помощью соответствующей функции восстановит `from_chars` значение в точности.
...
| `fmt` | Самое короткое поведение приема-передачи для указанного формата, например кратчайший экспоненциальный формат. |
