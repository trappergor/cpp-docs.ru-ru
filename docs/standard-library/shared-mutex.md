...
Заголовок &lt;shared_mutex > определяет классы `shared_mutex` и `shared_timed_mutex`, класс `shared_lock`шаблона и функцию `swap` шаблона для поддержки общих мьютексов.
