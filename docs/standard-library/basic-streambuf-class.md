...
Если *\_значение meta* не равно **traits_type:: EOF**, Защищенная виртуальная функция-член пытается вставить в выходные данные элемент **traits_type::** [to_char_type](../standard-library/char-traits-struct.md#to_char_type) ( *\_Meta*). вышестоящий. Для этого существует несколько способов.
