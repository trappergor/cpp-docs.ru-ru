...
Пара сигнатур функций, помеченных `(1)` как извлеченные символы, *выполняется* до тех пор, пока не будет найден *delim* , сохраняя их в *str*.

Пара подписей функций `(2)` , помеченная как разделитель строк по умолчанию, используется символом новой строки и ведет себя в виде строки ( `is`, `str`, `is`. `widen`(' `\n`')).
...
- После того как функция извлекает `str.`элементы [max_size](../standard-library/basic-string-class.md#max_size) , в этом случае флаг внутреннего состояния *имеет* значение `ios_base::failbit`.
...
Если функция не извлекает ни одного элемента, флаг внутреннего состояния *имеет* значение `ios_base::failbit`. В любом случае `getline` возвращается *значение*.
...
Функция преобразует последовательность элементов в *str* в значение `val` типа **Double** , как при вызове метода `strtod( str.c_str(), _Eptr)`, где `_Eptr` является объектом, внутренним по отношению к функции. Если ` str.c_str() == *_Eptr`, то создается объект типа `invalid_argument`. Если такой вызов задает `errno`, то создается объект типа `out_of_range`. В противном случае, если *idx* не является пустым указателем, `*_Eptr -  str.c_str()` функция `*idx` сохраняет в `val`и возвращает.
...
Функция преобразует последовательность элементов в *str* в значение `val` типа **float** , как при вызове метода `strtof( str.c_str(), _Eptr)`, где `_Eptr` является объектом, внутренним по отношению к функции. Если ` str.c_str() == *_Eptr`, то создается объект типа `invalid_argument`. Если такой вызов задает `errno`, то создается объект типа `out_of_range`. В противном случае, если *idx* не является пустым указателем, `*_Eptr -  str.c_str()` функция `*idx` сохраняет в `val`и возвращает.
...
Если `str.c_str() == *_Eptr`значение `stoi` равно, вызывается объект `invalid_argument`типа. Если такой вызов будет установлен `errno`или если возвращаемое значение не может быть представлено как объект типа **int**, то создается объект типа. `out_of_range` В противном случае, если *idx* не является пустым указателем, `*_Eptr - str.c_str()` функция `*idx`сохраняет в.
...
Функция преобразует последовательность элементов в *str* в значение `val` типа **Long** , если при вызове метода `strtol( str.c_str(), _Eptr, idx)`, где `_Eptr` является объектом, внутренним по отношению к функции. Если ` str.c_str() == *_Eptr`, то создается объект типа `invalid_argument`. Если такой вызов задает `errno`, то создается объект типа `out_of_range`. В противном случае, если *idx* не является пустым указателем, `*_Eptr -  str.c_str()` функция `*idx` сохраняет в `val`и возвращает.
...
Функция преобразует последовательность элементов в *str* в значение `val` типа **long double** , как при вызове метода `strtold( str.c_str(), _Eptr)`, где `_Eptr` — это объект, внутренний для функции. Если ` str.c_str() == *_Eptr`, то создается объект типа `invalid_argument`. Если такой вызов задает `errno`, то создается объект типа `out_of_range`. В противном случае, если *idx* не является пустым указателем, `*_Eptr -  str.c_str()` функция `*idx` сохраняет в `val`и возвращает.
...
Функция преобразует последовательность элементов в *str* в значение `val` типа **Long** `strtoll( str.c_str(), _Eptr, idx)`, равное, при условии, `_Eptr` что является объектом, внутренним с функцией. Если ` str.c_str() == *_Eptr`, то создается объект типа `invalid_argument`. Если такой вызов задает `errno`, то создается объект типа `out_of_range`. В противном случае, если *idx* не является пустым указателем, `*_Eptr -  str.c_str()` функция `*idx` сохраняет в `val`и возвращает.
...
Функция преобразует последовательность элементов в *str* в значение `val` типа **без знака Long** , равное, как при вызове метода `strtoul( str.c_str(), _Eptr, idx)`, где `_Eptr` — это объект, внутренний для функции. Если ` str.c_str() == *_Eptr`, то создается объект типа `invalid_argument`. Если такой вызов задает `errno`, то создается объект типа `out_of_range`. В противном случае, если *idx* не является пустым указателем, `*_Eptr -  str.c_str()` функция `*idx` сохраняет в `val`и возвращает.
...
Функция преобразует последовательность элементов в *str* в значение `val` типа **без знака Long** , равное, как при вызове метода `strtoull( str.c_str(), _Eptr, idx)`, где `_Eptr` является объектом, внутренним по отношению к функции. Если ` str.c_str() == *_Eptr`, то создается объект типа `invalid_argument`. Если такой вызов задает `errno`, то создается объект типа `out_of_range`. В противном случае, если *idx* не является пустым указателем, `*_Eptr -  str.c_str()` функция `*idx` сохраняет в `val`и возвращает.
...
Функция шаблона выполняет специализированную функцию члена *Left*. [поменять местами](../standard-library/basic-string-class.md#swap) (*right*) для строк, что гарантирует постоянную сложность.
...
Функция преобразует значение *Val* в последовательность элементов, хранящихся в объекте `Buf` массива, который является внутренним по отношению к функции, как при вызове метода `sprintf(Buf, Fmt, Val)`, где `Fmt`
