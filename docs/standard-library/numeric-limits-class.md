...
`long double` — то же самое, что **double** для компилятора C++.

Функция возвращает минимальное значение для типа, что есть то же самое, что [min](#min), если [has_denorm](#has_denorm) не равен **denorm_present**.
...
Член хранит **denorm_present** для типа с плавающей запятой, который имеет денормализованные значения, по сути переменное число бит экспоненты.
...
Возвращает наибольшее отрицательное конечное значение для типа (которой обычно является `min()` для целочисленных типов и `-max()` для типов с плавающей запятой). Возвращаемое значение является содержательным, если `is_bounded` имеет значение `true`.
...
Для типа `int` максимальное конечное значение — INT_MAX, а для типа **float** — FLT_MAX. Возвращаемое значение имеет смысл, если [is_bounded](#is_bounded) имеет значение **true**.
...
Минимальное нормализованное значение для типа `int` — INT_MIN, а для типа `float` — FLT_MIN. Возвращаемое значение имеет смысл, если [is_bounded](#is_bounded) имеет значение `true` или [is_signed](#is_signed) имеет значение `false`.
