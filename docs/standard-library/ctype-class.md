...
C++ Стандартная библиотека определяет две явные специализации этого шаблона класса:

- `ctype<char>`, явная специализация, различия которой описаны отдельно. Дополнительные сведения см. в разделе [ctype&lt;char&gt; Class](../standard-library/ctype-char-class.md).
...
Все остальные операции выполняются с **символьными** значениями так же, как и для явной специализации `ctype<char>`.
...
|[do_narrow](#do_narrow)|Виртуальная функция, вызываемая для преобразования символа типа `CharType` используемый локальным языком, в соответствующий символ типа **char** в собственной кодировке.|
...
|[do_widen](#do_widen)|Виртуальная функция, вызываемая для преобразования символа типа **char** в собственную кодировку в соответствующий символ типа `CharType` используемый языковым стандартом.|
...
Пример, в котором [ используется в качестве возвращаемого значения, см. в описании функции-члена ](#widen)widen`char_type`.
...
Виртуальная функция, вызываемая для преобразования символа типа `CharType` используемый локальным языком, в соответствующий символ типа **char** в собственной кодировке.
...
\ *по умолчанию*
Значение по умолчанию, присваиваемое функцией члена символам типа `CharType`, у которых нет символов аналога типа **char**.
...
Первая Защищенная функция-член возвращает собственный символ типа char, соответствующий символу параметра типа `CharType` или *Default* , если аналог не определен.
...
Защищенная функция-член возвращает самый маленький указатель `ptr` в диапазоне [`first`, `last`), для которого [do_is](#do_is)(`maskVal`, \* `ptr`) имеет значение true.
...
Защищенная функция-член возвращает самый маленький указатель `ptr` в диапазоне [`first`, `last`), для которого [do_is](#do_is)(`maskVal`, \* `ptr`) имеет значение false.
...
Вторая Защищенная функция-шаблон элемента заменяет каждый элемент `first` [`I`], для `I` в интервале [0, `last` - `first`) с `do_tolower`(`first` [`I`]).
...
Вторая Защищенная функция-шаблон элемента заменяет каждый элемент `first` [`I`], для `I` в интервале [0, `last` - `first`) с `do_toupper`(`first` [`I`]).
...
Виртуальная функция, вызываемая для преобразования символа типа **char** в собственную кодировку в соответствующий символ типа `CharType` используемый языковым стандартом.
...
Вторая Защищенная функция-член возвращает указатель на целевой диапазон символов типа `CharType` используемый языковой стандарт, преобразованный из собственных символов типа **char**.
...
Первая функция – член возвращает **значение true** , если проверяемый символ имеет атрибут, описанный значением маски. **значение false** , если не удается получить атрибут.
...
Первая функция-член возвращает собственный символ типа **char** , соответствующий символу параметра типа `CharType default`, если не определен аналог.
...
Вторая функция-член возвращает указатель на целевой диапазон символов типа `CharType` используемый языковым стандартом, преобразованных из машинных символов типа **char**.
