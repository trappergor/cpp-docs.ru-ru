...
ms.locfileid: "68453179"
---
# <a name="complex-class"></a>Класс complex

Класс шаблона описывает объект, в котором хранятся два объекта типа `Type`, один из которых представляет реальную часть комплексного числа, а другой — мнимую часть.
...
Явные специализации класса complex шаблона существуют для трех типов с плавающей запятой. В этой реализации значение любого другого типа `Type` **является результатом** приведения к `Type` **double** для фактических вычислений, при этом результат возвращается к сохраненному объекту типа.
...
Первый конструктор инициализирует хранимую реальную часть  *\_реалвал* , а хранимую  *\_* мнимую часть — имагвал. Второй конструктор инициализирует хранимую реальную часть `complexNum.real()` , а хранимую мнимую часть — на. `complexNum.imag()`
