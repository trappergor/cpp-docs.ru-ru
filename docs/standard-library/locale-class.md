...
Можно представить произвольную группу категорий с помощью `OR` с этими константами, как в. &#124; `time` `monetary`
...
Функция-член возвращает объект языкового стандарта, который заменяет или добавляет к  **\*этому** аспекту `Facet` , указанному в *Loc*.
...
Класс-член описывает статический объект-член, необходимый для каждого уникального аспекта языкового стандарта. Обратите внимание, что объект класса `id`нельзя скопировать или назначить.
...
Первый конструктор инициализирует объект для соответствия глобальному языковому стандарту. Второй и третий конструкторы инициализируют все категории языкового стандарта, чтобы их поведение соответствовало имени локали *LocName*. Оставшиеся конструкторы копируют *Loc*с упомянутыми исключениями:

`locale(const locale& Loc, const locale& Other, category Cat);`

заменяет *другие* аспекты, соответствующие категории c, для которой c & *Cat* не равно нулю.

`locale(const locale& Loc, const char* Locname, category Cat);`

`locale(const locale& Loc, const string& Locname, category Cat);`

заменяет эти аспекты, соответствующие категории c, для которой c & Cat — ненулевое значение.  `locale(Locname, _All)`

`template<class Facet> locale(const locale& Loc, Facet* Fac);`

заменяет в (или добавляет к) *Loc* аспект *fac*, если *fac* не является пустым указателем.

Если имя локали *LocName* является пустым указателем или недопустимым, функция создает исключение [runtime_error](../standard-library/runtime-error-class.md).
