...
`ptr` Указатель, значение которого отрисовывается неправильно при удалении.
...
`count` Число байтов памяти, которые нужно выделить.

`ptr` Указатель должен быть возвращен.
...
`count` Число байтов памяти, выделяемого для объекта массива.
