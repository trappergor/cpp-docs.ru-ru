...
Каждый из первых трех конструкторов задает пустую начальную priority_queue, а вторая также задает тип функции сравнения (`comp`), используемой при установлении порядка элементов, а третий явно указывает `container_type`(`_Cont`) для использования. Ключевое слово **explicit** подавляет некоторые виды автоматического преобразования типов.

Четвертый конструктор указывает копию *правого*priority_queue.

Последние три \[конструктора копируют диапазон " *первый*, *последний*)" контейнера и используют значения для инициализации priority_queue с повышением точности при указании типа функции сравнения класса и `Traits` `container_type`.
