...
*Контейнер* тип базового контейнера, используемый для реализации priority_queue.

*Сравнение* тип, предоставляющий объект функции, который может сравнить два значения элементов как ключи сортировки, чтобы определить их относительный порядок в priority_queue. Этот аргумент является необязательным и бинарный предикат **меньше***\<*** typename** *контейнера ***::value_type***>* значение по умолчанию.

## <a name="remarks"></a>Примечания

Элементы класса `Type` оговорено в шаблоне первый параметр объекта очереди, являются синонимами [value_type](#value_type) и должен соответствовать типу элемента в классе базового контейнера `Container` установленное по вторым параметром шаблона. `Type` Должен быть назначаемым, так как это можно было скопировать объекты этого типа и присвоить значения переменным этого типа.

Priority_queue Упорядочивает управляемую им последовательность путем вызова сохраненного объекта функции класса `Traits`. В целом, упорядочиваемые элементы должны лишь подлежать сравнению "меньше чем" для установления такого порядка, чтобы, имея любые два элемента, можно было определить, что они равны (ни один не меньше другого) или что один меньше другого. Это приводит к упорядочению неравнозначных элементов. С более технической точки зрения, функция сравнения является бинарным предикатом, который вызывает строгого слабое упорядочение в стандартном математически смысле.
...
*_ comp* функция сравнения типа **constTraits** используется для упорядочивания элементов в priority_queue, по умолчанию используется для сравнения функция базового контейнера.

*_Cont* базовый контейнер, из которых копироваться создаваемая priority_queue.

*правом* priority_queue, из которых создаваемый набор станет копией.

*Первый* положение первого элемента в диапазоне копируемых элементов.

*последний* позицию первого элемента за пределами диапазона копируемых элементов.

### <a name="remarks"></a>Примечания

Каждый из первых трех конструкторов определяет пустую исходную priority_queue; второй также указывает тип функции сравнения (`comp`) для использования при установлении порядка элементов, а третий явно указав `container_type`(`_Cont`) для использования. Ключевое слово **explicit** подавляет некоторые виды автоматического преобразования типов.
...
Ссылка на наибольший элемент, что определяется `Traits` , объектом priority_queue.
