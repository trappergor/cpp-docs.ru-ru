...
*Ключ* тип данных ключа для сохранения в hash_map.

*Тип* тип данных элемента для сохранения в hash_map.

*Признаки* тип, который включает два объекта функций: один из классов compare, который может сравнить два значения элементов как ключи сортировки для определения их относительного порядка и хэш-функции, которая является унарным предикатом и сопоставляет значения ключей элементов с без знака целых чисел типа `size_t`. Этот аргумент является необязательным, а hash_compare<`Key`, less<`Key`> > является значением по умолчанию.

*Распределитель* тип, представляющий сохраненный объект распределителя, инкапсулирующий сведения о выделении и освобождении памяти hash_map. Этот аргумент является необязательным, а значение по умолчанию — allocator<pair <const `Key`, `Type`>>.
...
*ключ* значение ключа элементов для сравнения с hash_map.
...
*ключ* значение ключа аргумента для сравнения с ключом сортировки элемента из hash_map в котором выполняется поиск.
...
*Первый* положение первого элемента, удаляемого из объекта hash_map.

*последний* положение сразу после последнего элемента, удаляемого из объекта hash_map.

*ключ* значение ключа элементов, удаляемый из объекта hash_map.
...
*ключ* значение ключа для сопоставления с ключом сортировки элемента из hash_map в котором выполняется поиск.
...
*правом* hash_map аргументов, предоставляющий элементы для обмена местами с целевой hash_map.
...
*ключ* значение ключа аргумента для сравнения с значением ключа сортировки элемента из hash_map в котором выполняется поиск.
