...
Объект `const_iterator` , определенный параметром hash_map, указывает на элементы, являющиеся объектами [value_type](#value_type), то есть `pair< const Key, Type >`тип, первый элемент которого является ключом элемента, а второй элемент — сопоставленная база данных, удерживаемая элементом.

Для разыменования объекта `const_iterator` `cIter` , указывающего на элемент в hash_map, используйте `->` оператор.

Чтобы получить доступ к значению ключа для элемента, используйте `cIter->first`, что `(*cIter).first`эквивалентно. Чтобы получить доступ к значению сопоставленной версии элемента, используйте `cIter->second`, что `(*cIter).second`эквивалентно.
...
Для разыменования объекта `const_reverse_iterator` `crIter` , указывающего на элемент в hash_map, используйте **->** оператор.
...
Для доступа к компоненту итератора пары `pr`, возвращенной этой функцией-членом, используйте `pr.first` и разыменуйте ее с помощью `*(pr.first)`. Чтобы получить доступ  к логическому компоненту `pr` пары, возвращаемой этой функцией- `pr.second`членом, используйте, и для разыменования этой функции используйте `*(pr.second)`.
...
Если возвращаемое значение `find` присваивается [const_iterator](#const_iterator), то объект hash_map изменить нельзя. Если возвращаемое значение `find` назначается итератору, [](#iterator)то объект hash_map можно изменить.
...
|Параметр|Описание|
|-|-|
|*Al*|Класс распределителя хранилища, используемый для этого объекта hash_map, который по умолчанию имеет `Allocator`значение.|
...
Первая `insert` функция-член возвращает пару, компонент bool которой возвращает значение true, если была произведена вставка, и значение false, если hash_map уже содержит элемент, ключ которого имеет эквивалентное значение в упорядочении, а компонент итератора возвращает адрес, по которому был вставлен новый элемент или где уже был размещен элемент.

Для доступа к компоненту итератора пары `pr`, возвращенной этой функцией-членом, используйте `pr`. **first**, а для его разыменования используйте \*(`pr`. **first**). Чтобы получить доступ  к логическому компоненту `pr` пары, возвращаемой этой функцией- `pr`членом, используйте. **second**, а для его разыменования используйте \*(`pr`. **second**).
...
Объект `iterator` , определенный параметром hash_map, указывает на элементы, являющиеся объектами [value_type](#value_type), которые имеют **ключ\<константы типа "пара", тип >,** первый элемент которого является ключом элемента, а второй — сопоставленной частью, содержащейся в дерев.

Для разыменования **итератора** `Iter` , указывающего на элемент в `->` multimap, используйте оператор.
...
См `iterator`. пример для [Begin](#begin) в качестве примера объявления и использования.
...
где значение `mapped_type` типа данных — это значение элемента с ключевым значением *Key*.
...
После стирания любых существующих элементов `hash_map`в, `operator=` копирует или перемещает `hash_map`содержимое *непосредственно* в.
...
Для разыменования `reverse_iterator` `rIter` указателя на элемент в hash_map используйте оператор->.
...
`value_type`объявляется `pair<const key_type, mapped_type>` как, а не `pair<key_type, mapped_type>` из-за того, что ключи ассоциативного контейнера не могут быть изменены с помощью неконстантного итератора или ссылки.
