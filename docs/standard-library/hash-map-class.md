...
*Черты*\
Тип, который включает два объекта-функции, один из классов compare, который может сравнить значения двух элементов как ключей сортировки, чтобы определить их относительный порядок, и хэш-функцию, которая является унарным предикатом, который сопоставляет значения ключей элементов с целыми числами без знака типа `size_t`. Этот аргумент является необязательным, и hash_compare<, `Key`менее<`Key`> > является значение по умолчанию.

*Распределителя*\
Тип, представляющий сохраненный объект распределителя, который инкапсулирует сведения о выделении и освобождении памяти для hash_map. Этот аргумент является необязательным, и значение\<по умолчанию `Key` `Type` является парой <const,>>.
...
Hash_map заказывает последовательность, которую он контролирует, вызывая сохраненный объект хэша *Traits* [класса value_compare.](../standard-library/value-compare-class.md) Доступ к этому сохраненному объекту можно получить путем вызова функции-члена [key_comp](#key_comp). Такой объект-функция должен функционировать так же, как и объект класса [hash_compare](../standard-library/hash-compare-class.md)<Key, less\<Key>>. В частности, для всех значений `Traits` *Ключ* типа *Key,* вызов () `size_t` `Key` дает распределение значений типа.

В целом, упорядочиваемые элементы должны лишь подлежать сравнению "меньше чем" для установления такого порядка, чтобы, имея любые два элемента, можно было определить, что они равны (ни один не меньше другого) или что один меньше другого. Это приводит к упорядочению неравнозначных элементов. С более технической точки зрения, функция сравнения является бинарным предикатом, который вызывает строгого слабое упорядочение в стандартном математически смысле. Двоичный предикат f(x y) — это `x` объект `y` функции, который имеет два объекта аргумента и и значение возврата **истинного** или **ложного.** Порядок, заданный для hash_map, является строгим слабым порядком, если бинарный предикат является нерефлексивным, антисимметричным и переходящим и если эквивалентность является переходящей, где два объекта x и y определяются как эквивалентные, когда оба параметра f(x,y) и f(y,x) имеют значение false. Если более строгое условие равенства между ключами заменяет условие эквивалентности, порядок становится общим (т.е. все элементы упорядочиваются относительно друг друга), и сопоставленные ключи будут неотличимы друг от друга.
...
|[const_pointer](#const_pointer)|Тип, который обеспечивает указатель на элемент `hash_map` **const** в .|
|[const_reference](#const_reference)|Тип, который предоставляет ссылку на **конст** элемент, хранящийся в `hash_map` для чтения и выполнения **конст-операций.**|
|[const_reverse_iterator](#const_reverse_iterator)|Тип, который обеспечивает двунаправленный итератор, который может `hash_map`читать любой **конст элемент** в .|
...
|[Итератор](#iterator)|Тип, предоставляющий двунаправленный итератор, который может считывать или изменять любой элемент в `hash_map`.|
...
|[указатель](#pointer)|Тип, предоставляющий указатель на элемент в `hash_map`.|
|[Ссылки](#reference)|Тип, предоставляющий ссылку на элемент, хранящийся в контейнере `hash_map`.|
...
|-|-|
|[В](#at)|Находит элемент в `hash_map` с указанным значением ключа.|
|[Начать](#begin)|Возвращает итератор, обращающийся к первый элемент в контейнере `hash_map`.|
...
|[Ясно](#clear)|Стирает все элементы в `hash_map`.|
...
|[Пустой](#empty)|Проверяет, пуст ли `hash_map`.|
...
|[Найти](#find)|Возвращает итератор, адресующий расположение элемента в наборе `hash_map` с ключом, эквивалентным указанному ключу.|
|[get_allocator](#get_allocator)|Возвращает копию объекта `allocator`, который используется для создания контейнера `hash_map`.|
|[Вставить](#insert)|Вставляет элемент или диапазон элементов в `hash_map`.|
...
|[Размер](#size)|Возвращает количество элементов в контейнере `hash_map`.|
|[Своп](#swap)|Выполняет обмен элементами между двумя объектами `hash_map`.|
...
|[hash_map::оператор](#op_eq)|Заменяет элементы `hash_map` копией другого `hash_map`.|
...
|Параметр|Описание|
|-|-|
|*Ключ*|Значение ключа элемента, который требуется найти.|
...
## <a name="hash_mapbegin"></a><a name="begin"></a>hash_map::начало
...
## <a name="hash_mapclear"></a><a name="clear"></a>hash_map::ясно
...
Определяется `const_iterator` hash_map указывает на элементы, которые являются `pair< const Key, Type >`объектами [value_type,](#value_type)то есть типа , чей первый член является ключом к элементу и чей второй член отображенный datum, удерживаемый элементом.

Чтобы указать `const_iterator` `cIter` на элемент в hash_map, `->` используйте оператора.
...
Чтобы получить значение ключа элемента, используйте `crIter` -> **first**, который эквивалентен (\* `crIter`) **.first**. Для получения доступа к значению сопоставленных данных элемента используйте `crIter` -> **second**, что эквивалентно (\* `crIter`). **первый**.

### <a name="example"></a>Пример

См. пример для [rend](#rend) в качестве примера объявления и использования `const_reverse_iterator`.

## <a name="hash_mapcount"></a><a name="count"></a>hash_map::счет
...
## <a name="hash_mapdifference_type"></a><a name="difference_type"></a>hash_map::difference
...
## <a name="hash_mapemplace"></a><a name="emplace"></a>hash_map::место
...
|Параметр|Описание|
|-|-|
|*Валь*|Значение, используемое для перемещения, создает элемент для вставки в [hash_map](../standard-library/hash-map-class.md), кроме случаев, когда `hash_map` уже содержит этот элемент (или, в более общем смысле, элемент, ключ которого эквивалентно упорядочен).|
...
Для доступа к компоненту итератора пары `pr`, возвращенной этой функцией-членом, используйте `pr.first` и разыменуйте ее с помощью `*(pr.first)`. Чтобы получить доступ к компоненту **bool** `pr` пары, `pr.second`возвращенной этой функцией `*(pr.second)`члена, и отойди его, используйте.
...
Вставка может происходить в амортизированное постоянное время, а не логарифмическое время, если точка вставки сразу же следует *_Where.*
...
## <a name="hash_mapempty"></a><a name="empty"></a>hash_map::пустой
...
`end`используется для проверки того, достиг ли итерататор конца своего hash_map.
...
## <a name="hash_mapfind"></a><a name="find"></a>hash_map::найти
...
`find`возвращает итератор, который обращается к элементу в hash_map ключ сортировки которого эквивалентен ключу аргумента под двоичным предикатом, который индуцирует упорядочение, основанное на меньшем, чем отношение сопоставимости.

Если значение `find` возврата назначено [const_iterator,](#const_iterator)hash_map объект не может быть изменен. Если значение `find` возврата назначено [итератору,](#iterator)hash_map объект может быть изменен
...
|Параметр|Описание|
|-|-|
|*Аль*|Класс разлестьа хранилища, который будет использоваться `Allocator`для этого hash_map объект, который по умолчанию.|
|*Комп*|Функция сравнения типа const `Traits` используется для упорядочения элементов в hash_map, чье значение по умолчанию — `hash_compare`.|
...
Первые три конструктора указывают пустой первоначальный hash_map, кроме того, второй определяет тип функции сравнения *(Comp*), которые будут использоваться при установлении порядка элементов, а третий явно определяет тип разлляции (*Al*), который будет использоваться. Ключевое слово **explicit** подавляет некоторые виды автоматического преобразования типов.
...
Последний конструктор перемещает hash_map *право*.

## <a name="hash_mapinsert"></a><a name="insert"></a>hash_map::вставка
...
|Параметр|Описание|
|-|-|
|*Валь*|Значение элемента для вставки в hash_map, кроме случаев, когда hash_map уже содержит этот элемент (или, в более общем смысле, элемент, ключ которого эквивалентно упорядочен).|
|*_where*|Подсказка о месте начала поиска правильной точки вставки.|
...
Функция `insert` первого члена возвращает пару, компонент bool возвращается верно, если вставка была сделана и ложна, если hash_map уже содержал элемент, ключ которого имел эквивалентное значение в заказе, и чей компонент итератора возвращает адрес, где был вставлен новый элемент или где элемент уже находится.

Для доступа к компоненту итератора пары `pr`, возвращенной этой функцией-членом, используйте `pr`. **во-первых,** и dereference \* `pr`его, использование ( . **первый**). Чтобы получить доступ к компоненту **bool** `pr` пары, `pr`возвращенной этой функцией члена, используйте . **во-вторых,** и присваивая его, используйте \*(. `pr` **второй**).

Функция `insert` второго элемента, версия подсказки, возвращает итератор, который указывает на положение, в котором новый элемент был вставлен в hash_map.

Последние `insert` две функции члена ведут себя так же, как и первые две, за исключением того, что они перемещают построение вставленного значения.
...
Вставка может произойти в амортизированное постоянное время для подсказки версии вставки, вместо логарифмического времени, если точка вставки сразу же следует *_Where.*
...
Определяется `iterator` hash_map указывает на элементы, которые являются объектами [value_type,](#value_type)то есть **типа пары\<const Key, Type>,** чей первый член является ключом к элементу и чей второй член является отображенный datum, удерживаемый элементом.

Чтобы указать на **итератор,** `Iter` указывающий на элемент `->` в мультикарте, используйте оператора.

Для получения доступа к значению ключа для элемента используйте `Iter` -> **first**, что эквивалентно (\* `Iter`). **первый**. Для получения доступа к значению сопоставленных данных элемента используйте `Iter` -> **second**, что эквивалентно (\* `Iter`). **второй**.

Тип `iterator` может быть использован для изменения значения элемента.

### <a name="example"></a>Пример

Например, [для примера](#begin) того, как `iterator`объявить и использовать .
...
Если возвращаемое значение `lower_bound` назначено `const_iterator`, объект hash_map изменить нельзя. Если значение `lower_bound` возврата назначено объекту, `iterator`hash_map объект может быть изменен.
...
## <a name="hash_mapoperator"></a><a name="op_at"></a>hash_map:оператор
...
|Параметр|Описание|
|-|-|
|*Ключ*|Ключевое значение элемента для вставки.|
...
где DataValue является значением `mapped_type` элемента с ключевой значением *ключа.*
...
## <a name="hash_mapoperator"></a><a name="op_eq"></a>hash_map::оператор
...
После стирания любых `hash_map`существующих `operator=` элементов в , либо копии или перемещает содержимое *право* в `hash_map`.
...
## <a name="hash_mapreference"></a><a name="reference"></a>hash_map::ссылка
...
Для получения доступа к значению ключа для элемента используйте `rIter` -> **first**, что эквивалентно (\* `rIter`). **первый**. Для получения доступа к значению сопоставленных данных элемента используйте `rIter` -> **second**, что эквивалентно (\* `rIter`). **первый**.

### <a name="example"></a>Пример

См. пример для [rbegin](#rbegin) в качестве примера объявления и использования `reverse_iterator`.

## <a name="hash_mapsize"></a><a name="size"></a>hash_map:размер
...
Если для `const_iterator` назначено возвращаемое значение, объект hash_map изменить нельзя. Если значение возврата назначено `iterator`объекту hash_map может быть изменено.
...
Для hash_map *м,* если два элемента *e1* *(k1,* *d1)* и *e2* *(k2,* *d2*) являются объектами типа [value_type,](#value_type)где *k1* и *k2* являются `m.key_comp()(k1, k2)`их ключами типа [key_type](#key_type) и *d1* и *d2* являются их данными типа [mapped_type,](#mapped_type)то `m.value_comp()(e1, e2)` эквивалентно . Хранимый объект определяет функцию-член
...
`value_type``pair<const key_type, mapped_type>` объявляется, а `pair<key_type, mapped_type>` не потому, что ключи ассоциативного контейнера не могут быть изменены с помощью непостоянного итератора или ссылки.
