...
`Key` Тип данных ключа для сохранения в hash_map.

`Type` Тип данных элемента для сохранения в hash_map.

`Traits` Тип, который включает в себя два объекта функции, один из классов compare, который может сравнить два значения элементов как ключи сортировки для определения их относительного порядка и хэш-функцию, которая является унарным предикатом и сопоставляет значения ключей элементов с целыми числами без знака типа `size_t`. Этот аргумент является необязательным, а hash_compare< `Key`, less< `Key`> > является значением по умолчанию.

`Allocator` Тип, представляющий сохраненный объект распределителя, инкапсулирующий сведения о выделении и освобождении памяти hash_map. Этот аргумент является необязательным, а значение по умолчанию — allocator<pair <const `Key`, `Type`>>.
...
|[const_pointer](#const_pointer)|Тип, предоставляющий указатель на элемент `const` в контейнере `hash_map`.|
|[const_reference](#const_reference)|Тип, предоставляющий ссылку на элемент `const`, сохраненный в контейнере `hash_map` для чтения и выполнения операций `const`.|
|[const_reverse_iterator](#const_reverse_iterator)|Тип, предоставляющий двунаправленный итератор, который может считывать любой элемент `const` в контейнере `hash_map`.|
...
Для разыменования `const_reverse_iterator` `crIter` указывает на элемент в объекте hash_map, используйте **->** оператор.
...
`key` Значение ключа элементов для сопоставления из объекта hash_map.
...
`key` Значение ключа аргумента для сравнения с ключом сортировки элемента из объекта hash_map, в которой выполняется поиск.
...
`_Where` Положение элемента, удаляемого из объекта hash_map.

`first` Положение первого элемента, удаляемого из объекта hash_map.

`last` Положение сразу после последнего элемента, удаляемого из объекта hash_map.

`key` Значение ключа элементов, удаляемого из объекта hash_map.
...
`key` Значение ключа, к которым сравнивается ключ сортировки элемента из объекта hash_map, в которой выполняется поиск.
...
Если возвращаемое значение **find** назначается [const_iterator](#const_iterator), то объект hash_map невозможно изменить. Если возвращаемое значение **find** назначается [iterator](#iterator), то объект hash_map можно изменить.
...
**итератор** определяется hash_map, указывающим на элементы, являющиеся объектами [value_type](#value_type) типа **pair\<const Key, Type>,** первый член которых является ключом для элемента, а второй член — сопоставленной единицей данных, хранящейся в элементе.
...
Для разыменования `reverse_iterator` `rIter` указывает на элемент в объекте hash_map, используйте оператор "->".
...
`right` Аргумент hash_map, предоставляющий элементы, которые будут заменены целевого объекта hash_map.
...
`key` Значение ключа аргумента для сравнения со значением ключа сортировки элемента из объекта hash_map, в которой выполняется поиск.
