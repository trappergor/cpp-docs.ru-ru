...
Главным преимуществом хэширования по сравнению с сортировкой является большая эффективность: успешное хэширование выполняет вставки, удаления и поиск с постоянным средним временем, в то время как время для методик сортировки пропорционально логарифму числа элементов в контейнере. Значение элемента в hash_map, но не связанное с ним значение ключа можно изменить напрямую. Значения ключей, связанные со старыми элементами, необходимо удалить и вставить новые значения ключей, связанные с новыми элементами.
...
`const_iterator`, определяемые hash_map, указывают на элементы, являющиеся объектами [value_type](#value_type), которые относятся к типу `pair< const Key, Type >`, первый элемент которого является ключом к элементу, а второй — сопоставленной частью, содержащейся в элементе.
...
`const_reverse_iterator`, определяемые hash_map, указывают на элементы, являющиеся объектами [value_type](#value_type), которые имеют тип `pair`\< **const key, Type**>, чей первый элемент является ключом к элементу, а второй — сопоставленной частью, содержащейся в элементе.

Для разыменования `const_reverse_iterator` `crIter`, указывающего на элемент в hash_map, используйте оператор **->** .

Чтобы получить доступ к значению ключа для элемента, **сначала**используйте `crIter` -> , что эквивалентно (\* `crIter`) **. First**. Чтобы получить доступ к значению сопоставленной части элемента, используйте `crIter` -> **Second**, который эквивалентен (\* `crIter`). **first**.
...
Если возвращаемое значение `find` назначается [const_iterator](#const_iterator), то объект hash_map изменить нельзя. Если возвращаемое значение `find` назначено [итератору](#iterator), то объект hash_map можно изменить.
...
|*Первая*|Положение первого элемента в диапазоне копируемых элементов.|
...
Первая функция-член `insert` возвращает пару, компонент bool которой возвращает значение true, если была произведена вставка, и значение false, если hash_map уже содержала элемент, ключ которого имеет эквивалентное значение в упорядочении, а компонент итератора возвращает адрес, куда был вставлен новый элемент, или место, где элемент уже был найден.
...
`iterator`, определяемые hash_map, указывают на элементы, являющиеся объектами [value_type](#value_type), которые имеют пару типа **\<const key, Type >,** первый элемент которых является ключом элемента, а второй — сопоставленной с ним частью.

Для разыменования **итератора**`Iter`, указывающий на элемент в multimap, используйте оператор `->`.

Чтобы получить значение ключа для элемента, **сначала**используйте `Iter` -> , что эквивалентно (\* `Iter`). **first**. Чтобы получить доступ к значению сопоставленной части элемента, используйте `Iter` -> **Second**, который эквивалентен (\* `Iter`). **second**.
...
**логический оператор**(`left`**ключа const &** **, &** `right`);
