...
*Ключ* тип данных ключа для сохранения в hash_multimap.

*Тип* тип данных элемента для сохранения в hash_multimap.

*Признаки* тип, который включает два объекта функций: один из класса *Traits* , которое способно сравнить значения двух элементов как ключи сортировки для определения их относительного порядка и хэш-функцией, является ключом унарного предиката сопоставления значения элементов беззнаковым целым числам типа `size_t`. Этот аргумент является необязательным, и в качестве значения по умолчанию используется `hash_compare<Key, less<Key>>`.

*Распределитель* тип, представляющий сохраненный объект распределителя, инкапсулирующий сведения о выделении и освобождении памяти для hash_multimap. Этот аргумент является необязательным, и в качестве значения по умолчанию используется `allocator<pair <const Key, Type>>`.
...
*ключ* ключ элементов для сравнения с hash_multimap.
...
*ключ* ключ-аргумент для сравнения с ключом сортировки элемента из hash_multimap в котором выполняется поиск.
...
*Первый* положение первого элемента, удаляемого из объекта hash_multimap.

*последний* положение сразу после последнего элемента, удаляемого из объекта hash_multimap.

*ключ* ключ элементов, удаляемый из объекта hash_multimap.
...
*ключ* ключ для сопоставления с ключом сортировки элемента из hash_multimap в котором выполняется поиск.
...
*правом* hash_multimap, предоставляющий элементы для обмена местами, или hash_multimap, элементы которого должны быть заменены объекта hash_multimap.
...
`value_type` объявляется как пары\<const [key_type](#key_type), [mapped_type](#mapped_type)> и не сопряжение\<key_type, mapped_type > так как ключи ассоциативного контейнера не может быть изменен через неконстантный итератор или ссылку.
