...
*Черты*\
Тип, который включает в себя два функциональных объекта, один из значений *класса,* который способен сравнить два значения элементов в качестве сортированных ключей для определения их `size_t`относительного порядка и функции хэша, которая является неарым предикатным отображением ключевых значений элементов для неподписанных рядов типа. Этот аргумент является необязательным, и в качестве значения по умолчанию используется `hash_compare<Key, less<Key>>`.
...
В целом, упорядочиваемые элементы должны лишь подлежать сравнению "меньше чем" для установления такого порядка, чтобы, имея любые два элемента, можно было определить, что они равны (ни один не меньше другого) или что один меньше другого. Это приводит к упорядочению неравнозначных элементов. С более технической точки зрения, функция сравнения является бинарным предикатом, который вызывает строгого слабое упорядочение в стандартном математически смысле. Двоичный предикат f(x, y) — это `x` объект `y` функции, который имеет два объекта аргумента и и значение возврата **истинного** или **ложного.** Заказ, наложенный на hash_multimap, является строгим слабым упорядочением, если двоичный предикат является необратимым, антисимметричным `x` и `y` переходным, и если эквивалентность является транзитной, где два объекта и определяются как эквивалентные, когда оба f(x, y) и f(y, x) **являются ложными.** Если более строгое условие равенства между ключами заменяет условие эквивалентности, порядок становится общим (т.е. все элементы упорядочиваются относительно друг друга), и сопоставленные ключи будут неотличимы друг от друга.
...
|[const_pointer](#const_pointer)|Тип, который обеспечивает указатель на элемент `hash_multimap` **const** в .|
|[const_reference](#const_reference)|Тип, который предоставляет ссылку на **конст** элемент, хранящийся в `hash_multimap` для чтения и выполнения **конст-операций.**|
|[const_reverse_iterator](#const_reverse_iterator)|Тип, который обеспечивает двунаправленный итератор, который может `hash_multimap`читать любой **конст элемент** в .|
...
|[Итератор](#iterator)|Тип, предоставляющий двунаправленный итератор, который может считывать или изменять любой элемент в `hash_multimap`.|
...
|[указатель](#pointer)|Тип, предоставляющий указатель на элемент в `hash_multimap`.|
|[Ссылки](#reference)|Тип, предоставляющий ссылку на элемент, хранящийся в контейнере `hash_multimap`.|
...
|-|-|
|[Начать](#begin)|Возвращает итератор, обращающийся к первый элемент в контейнере `hash_multimap`.|
...
|[Ясно](#clear)|Стирает все элементы в `hash_multimap`.|
...
|[Пустой](#empty)|Проверяет, пуст ли `hash_multimap`.|
...
|[Найти](#find)|Возвращает итератор, адресующий расположение элемента в наборе `hash_multimap` с ключом, эквивалентным указанному ключу.|
...
|[Вставить](#insert)|Вставляет элемент или диапазон элементов в `hash_multimap` в заданной позиции.|
...
|[Размер](#size)|Указывает новый размер `hash_multimap`.|
|[Своп](#swap)|Выполняет обмен элементами между двумя объектами `hash_multimap`.|
...
## <a name="hash_multimapbegin"></a><a name="begin"></a>hash_multimap::начало
...
Если значение `begin` возврата назначено элементу `const_iterator`hash_multimap объекте, не может быть изменено. Если значение `begin` возврата назначено объекту, `iterator`элементы hash_multimap объектмогут быть изменены.
...
## <a name="hash_multimapclear"></a><a name="clear"></a>hash_multimap:::ясно
...
Определяемые `const_iterator` hash_multimap указывают на объекты [value_type,](#value_type)которые имеют тип. `pair<const Key, Type>` Значение ключа доступно через первый элемент пары, а значение сопоставленного элемента — через второй элемент пары.

Чтобы указать `const_iterator` `cIter` на элемент в hash_multimap, `->` используйте оператора.
...
Определенный `const_reverse_iterator` hash_multimap указывает на объекты [value_type,](#value_type)которые имеют тип, `pair<const Key, Type>`первый член которого является ключом к элементу и вторым членом которого является картированный datum, удерживаемый элементом.

Чтобы указать `const_reverse_iterator` `crIter` на элемент в hash_multimap, `->` используйте оператора.

Чтобы получить доступ к значению ключа `crIter->first`для элемента, используйте, что `(*crIter).first`эквивалентно. Чтобы получить доступ к значению отображеного `crIter->second`datum `(*crIter).second`для элемента, используйте, что эквивалентно .

### <a name="example"></a>Пример

См. пример для [rend](#rend) в качестве примера объявления и использования `const_reverse_iterator`.

## <a name="hash_multimapcount"></a><a name="count"></a>hash_multimap::счет
...
## <a name="hash_multimapdifference_type"></a><a name="difference_type"></a>hash_multimap::difference
...
## <a name="hash_multimapemplace"></a><a name="emplace"></a>hash_multimap::место
...
|Параметр|Описание|
|-|-|
|*Валь*|Значение, используемое для перемещения, формирует элемент для вставки в [hash_multimap](../standard-library/hash-multimap-class.md).|
...
|Параметр|Описание|
|-|-|
|*Валь*|Значение, используемое для перемещения, создает элемент для вставки в [hash_multimap](../standard-library/hash-multimap-class.md), кроме случаев, когда `hash_multimap` уже содержит этот элемент (или, в более общем смысле, элемент, ключ которого эквивалентно упорядочен).|
...
## <a name="hash_multimapempty"></a><a name="empty"></a>hash_multimap::пустой
...
## <a name="hash_multimapfind"></a><a name="find"></a>hash_multimap::найти
...
Функция члена возвращает итератор, который обращается к элементу `equivalent` в hash_multimap ключом сортировки которого является ключом аргумента под двоичным предикатом, который индуцирует упорядочение, основанное на меньшем, чем отношение сопоставимости.

Если возвращаемое значение `find` назначено `const_iterator`, объект hash_multimap изменить нельзя. Если значение `find` возврата назначено объекту `iterator`hash_multimap может быть изменено.
...
|Параметр|Описание|
|-|-|
|*Аль*|Класс распределителя хранилища для данного объекта hash_multimap, значение по умолчанию `Allocator`.|
|*Комп*|Функция сравнения типа `const Traits` для использования при упорядочивании элементов в сопоставлении. Значение по умолчанию — `Traits`.|
...
Первые три конструктора указывают пустой первоначальный hash_multimap; второй определяет тип функции сравнения *(Comp),* которая будет использоваться при установлении порядка элементов, а третий явно`_Al`определяет тип аллигатора (), который будет использоваться. Ключевое слово `explicit` подавляет определенные виды автоматического преобразования типов.
...
## <a name="hash_multimapinsert"></a><a name="insert"></a>hash_multimap::вставка
...
|*Где*|Подсказка, где начинать поиск правильной точки вставки.|
...
Вставка может произойти в амортизированное `insert`постоянное время для подсказки версии , вместо логарифмического времени, если точка вставки сразу же следует *Где*.
...
`iterator` Определяется hash_multimap указывает на объекты [value_type](#value_type) `pair` \< , которые имеют тип **const Key, Type**>, чей первый член является ключом к элементу и чей второй член является отображенный datum, удерживаемый элементом.

Чтобы указать на **итератор,** `Iter` указывающий на элемент `->` в hash_multimap, используйте оператора.

Для получения доступа к значению ключа для элемента используйте `Iter` -> **first**, что эквивалентно (\* `Iter`). **первый**. Для получения доступа к значению сопоставленных данных элемента используйте `Iter` -> **second**, что эквивалентно (\* `Iter`). **первый**.
...
которая возвращается `left` **верно,** если предшествует и не равна `right` в порядке сортировки.
...
Для получения дополнительной информации о *Traits* смотрите тему [hash_multimap класса.](../standard-library/hash-multimap-class.md)
...
Для получения дополнительной информации о *ключе*, смотрите в разделе Замечания [темы hash_multimap класса.](../standard-library/hash-multimap-class.md)
...
Если возвращаемое значение `lower_bound` назначено `const_iterator`, объект hash_multimap изменить нельзя. Если значение `lower_bound` возврата назначено объекту `iterator`hash_multimap может быть изменено.
...
Для получения дополнительной информаци [hash_multimapи](../standard-library/hash-multimap-class.md) о *типе см.*
...
## <a name="hash_multimapoperator"></a><a name="op_eq"></a>hash_multimap:оператор
...
|Параметр|Описание|
|-|-|
|*Правильно*|[Hash_multimap](../standard-library/hash-multimap-class.md), который копируется в `hash_multimap`.|

### <a name="remarks"></a>Remarks

После стирания любых `hash_multimap`существующих `operator=` элементов в , либо копии или перемещает содержимое *право* в `hash_multimap`.
...
## <a name="hash_multimapsize"></a><a name="size"></a>hash_multimap:размер
...
Если возвращаемое значение `upper_bound` назначено `const_iterator`, объект hash_multimap изменить нельзя. Если значение `upper_bound` возврата назначено объекту, `iterator`hash_multimap объект может быть изменен.
...
`value_type`объявляется парной\<конст-key_type, [mapped_type](#mapped_type)>, а не парной\<key_type, mapped_type>, поскольку ключи ассоциативного контейнера не могут быть изменены с помощью непостоянного итератора или ссылки. [key_type](#key_type)
