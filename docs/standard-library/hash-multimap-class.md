...
Если возвращаемое значение `begin` присваивается `const_iterator`, то элементы в объекте hash_multimap изменить нельзя. Если возвращаемое значение `begin` присваивается `iterator`, то элементы объекта hash_multimap могут быть изменены.
...
`const_iterator`, определяемые hash_multimap, указывают на объекты [value_type](#value_type), которые имеют тип `pair<const Key, Type>`. Значение ключа доступно через первый элемент пары, а значение сопоставленного элемента — через второй элемент пары.
...
`const_reverse_iterator`, определяемые hash_multimap, указывают на объекты [value_type](#value_type), которые относятся к типу `pair<const Key, Type>`, первый элемент которого является ключом элемента, а второй элемент — сопоставленной частью, которая удерживается элементом.
...
Функция-член возвращает итератор, который обращается к элементу в hash_multimap, ключ сортировки которого `equivalent` в ключ аргумента в бинарном предикате, который вызывает упорядочивание на основе отношения сравнения "меньше".
...
|*Первая*|Позиция первого элемента, который следует скопировать из карты.|
...
`iterator`, определяемые hash_multimap, указывают на объекты [value_type](#value_type), которые имеют тип `pair`\< **const key, Type**>, первый элемент которого является ключом элемента, а второй — сопоставленным элементом.

Для разыменования **итератора**`Iter`, указывающий на элемент в hash_multimap, используйте оператор `->`.

Чтобы получить значение ключа для элемента, **сначала**используйте `Iter` -> , что эквивалентно (\* `Iter`). **first**. Чтобы получить доступ к значению сопоставленной части элемента, используйте `Iter` -> **Second**, который эквивалентен (\* `Iter`). **first**.
...
`reverse_iterator`, определяемые hash_multimap, указывают на объекты [value_type](#value_type), которые имеют тип `pair`\< **const Key, Type**>. Значение ключа доступно через первый элемент пары, а значение сопоставленного элемента доступно через второй элемент пары.
...
`value_type` объявляется как пара\<const [key_type](#key_type), [mapped_type](#mapped_type)> и не являются парными\<key_type, mapped_type >, так как ключи ассоциативного контейнера могут не изменяться с помощью неконстантного итератора или ссылки.
