...
|[итераци](#iterator)|Тип, предоставляющий двунаправленный итератор, который может считывать или изменять любой элемент в `hash_multimap`.|
...
|[открытым](#clear)|Стирает все элементы в `hash_multimap`.|
...
|[конце](#end)|Возвращает итератор, который обращается к месту, следующему за последним элементом в контейнере `hash_multimap`.|
...
Вставка может производиться в течение постоянного времени для версии подсказок `insert` , а не логарифмического времени, если точка вставки выполняется непосредственно после. *Where*
...
Для получения доступа к значению ключа для элемента используйте `Iter` -> **first**, что эквивалентно (\* `Iter`). **сначала**. Для получения доступа к значению сопоставленных данных элемента используйте `Iter` -> **second**, что эквивалентно (\* `Iter`). **сначала**.
