...
|[const_pointer](#const_pointer)|Тип, предоставляющий указатель на элемент **const** в `hash_multimap`.|
|[const_reference](#const_reference)|Тип, предоставляющий ссылку на элемент **const** , хранящийся в `hash_multimap` для чтения и выполнения операций **const** .|
|[const_reverse_iterator](#const_reverse_iterator)|Тип, предоставляющий двунаправленный итератор, который может читать любой элемент **const** в `hash_multimap`.|
...
Если возвращаемое значение `begin` присваивается `const_iterator`, то элементы в объекте hash_multimap не могут быть изменены. Если возвращаемое значение `begin` присваивается `iterator`, то элементы в объекте hash_multimap можно изменить.
...
@No__t_0, определенные hash_multimap, указывают на объекты [value_type](#value_type), которые имеют тип `pair<const Key, Type>`. Значение ключа доступно через первый элемент пары, а значение сопоставленного элемента — через второй элемент пары.
...
@No__t_0, определяемый hash_multimap, указывает на объекты [value_type](#value_type), которые имеют тип `pair<const Key, Type>`, первый элемент которого является ключом элемента, а второй — сопоставленной частью, содержащейся в элементе.

Для разыменования `const_reverse_iterator` `crIter` указывающего на элемент в hash_multimap, используйте оператор `->`.

Чтобы получить значение ключа для элемента, используйте `crIter->first`, что эквивалентно `(*crIter).first`. Чтобы получить доступ к значению сопоставленной части элемента, используйте `crIter->second`, что эквивалентно `(*crIter).second`.
...
Функция-член возвращает итератор, который обращается к элементу в hash_multimap, чей ключ сортировки `equivalent` ключу аргумента в бинарном предикате, который вызывает упорядочивание на основе отношения сравнения "меньше".
...
|*Соответствовал*|Функция сравнения типа `const Traits` используется для упорядочения элементов в схеме, которая по умолчанию `Traits`.|
...
|*Интерфейс*|Копируемый initializer_list.|
...
|*Началь*|Позиция первого элемента, который следует скопировать из карты.|
|*Последняя*|Позиция непосредственно перед последним элементом, который следует скопировать из карты.|
...
Вставка может выполняться в периодической константе для версии подсказок `insert`, а не логарифмического времени, если точка вставки непосредственно следует за точкой *.*
...
@No__t_0, определенные hash_multimap, указывают на объекты [value_type](#value_type), которые имеют тип `pair` \< **const key, Type**>, первый элемент которого является ключом элемента, а второй элемент — сопоставленная база данных, удерживаемая элементом.

Для разыменования **итератора** `Iter`, указывающий на элемент в hash_multimap, используйте оператор `->`.

Для получения доступа к значению ключа для элемента используйте `Iter` -> **first**, что эквивалентно (\* `Iter`). **first**. Для доступа к значению сопоставленной единицы информации для элемента используйте `Iter` -> **second**, что эквивалентно (\* `Iter`). **first**.
...
Дополнительные сведения о *характеристиках* см. в разделе [класс hash_multimap](../standard-library/hash-multimap-class.md) .
...
Дополнительные сведения о *ключе*см. в разделе "Примечания" статьи о [классе hash_multimap](../standard-library/hash-multimap-class.md) .
...
Дополнительные сведения о *типе* см. в разделе о [классе hash_multimap](../standard-library/hash-multimap-class.md) .
...
После удаления существующих элементов в `hash_multimap` `operator=` копирует или перемещает содержимое *непосредственно* в `hash_multimap`.
...
`reverse_iterator`, определяемый hash_multimap, указывает на объекты [value_type](#value_type) типа `pair`\< **const Key, Type**>. Значение ключа доступно через первый элемент пары, а значение сопоставленного элемента доступно через второй элемент пары.
...
`value_type` объявляется как парный \<const [key_type](#key_type), [mapped_type](#mapped_type)> и не являются парными \<key_type, mapped_type >, поскольку ключи ассоциативного контейнера могут не изменяться с помощью неконстантного итератора или ссылки.
