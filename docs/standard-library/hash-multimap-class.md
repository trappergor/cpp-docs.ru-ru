...
Объект `const_iterator` , определенный параметром hash_multimap, указывает на объекты [value_type](#value_type), имеющие тип `pair<const Key, Type>`. Значение ключа доступно через первый элемент пары, а значение сопоставленного элемента — через второй элемент пары.
...
Объект `const_reverse_iterator` , определенный параметром hash_multimap, указывает на объекты [value_type](#value_type), имеющие тип `pair<const Key, Type>`, первый элемент которого является ключом элемента, а второй элемент — сопоставленная база данных, удерживаемая элементом.

Для разыменования объекта `const_reverse_iterator` `crIter` , указывающего на элемент в hash_multimap, используйте `->` оператор.

Чтобы получить доступ к значению ключа для элемента, используйте `crIter->first`, что `(*crIter).first`эквивалентно. Чтобы получить доступ к значению сопоставленной версии элемента, используйте `crIter->second`, что `(*crIter).second`эквивалентно.
...
Функция-член возвращает итератор, который обращается к элементу в hash_multimap, чей ключ `equivalent` сортировки является ключом аргумента в бинарном предикате, который вызывает упорядочивание на основе отношения сравнения "меньше".
...
|*Соответствовал*|Функция сравнения типа `const Traits` для использования при упорядочивании элементов в сопоставлении. Значение по умолчанию — `Traits`.|
...
Вставка может производиться в течение постоянного времени для версии `insert`подсказок, а не логарифмического времени, если точка вставки выполняется непосредственно после.
...
Объект `iterator` , определенный параметром hash_multimap, указывает на объекты [value_type](#value_type), которые относятся `pair` к типу \< **const Key, Type**>, первый элемент которого является ключом элемента, а второй — сопоставленной частью, которой владеет элемент.

Для разыменования **итератора** `Iter` , указывающего на элемент в `->` hash_multimap, используйте оператор.
...
После стирания любых существующих элементов `hash_multimap`в, `operator=` копирует или перемещает `hash_multimap`содержимое *непосредственно* в.
...
`value_type`объявляется как парная\<константа [key_type](#key_type), [mapped_type](#mapped_type)> и не\<являются парными key_type, mapped_type >, так как ключи ассоциативного контейнера могут не изменяться с помощью неконстантного итератора или ссылки.
