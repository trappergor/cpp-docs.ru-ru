...
> [!NOTE]
> Функция сравнения — это бинарный предикат, который вызывает строгое слабое упорядочение в стандартном математически смысле. Бинарный предикат f (x, y) — это объект функции, имеющий два объекта-аргумента x и y, а также возвращаемое значение **true** или **false**. Порядок, накладываемый на набор, является строгим слабым порядком, если бинарный предикат является нерефлексивным, антисимметричным и транзитивным, и если эквивалентность является транзитивным, то два объекта x и y определяются как эквивалентные, если значения f (x, y) и f (y, x) равны **false**. Если более строгое условие равенства между ключами заменяет условие эквивалентности, порядок становится общим (т. е. все элементы упорядочиваются в соответствии друг с другом) и сопоставленные ключи будут неотличимы друг от друга.
>
...
|[const_iterator](#const_iterator)|Typedef для двунаправленного итератора, который может читать элемент **const** в сопоставлении.|
...
|[пусто](#clear)|Стирает все элементы в сопоставлении.|
...
|[empty](#empty)|Возвращает **значение true** , если значение Map пусто.|
...
**Константный** двунаправленный итератор, обращающийся к первому элементу в диапазоне, или расположение непосредственно за концом пустого диапазона (для пустого диапазона `cbegin() == cend()`).
...
**Константный** итератор с двунаправленным доступом, указывающий на место сразу за концом диапазона.
...
`const_iterator`, определяемые картой, указывают на элементы, являющиеся объектами [value_type](#value_type), которые относятся к типу `pair`\< **constKey**, **Type**>, чей первый элемент является ключом элемента, а второй — сопоставленным элементом.

Для разыменования `const_iterator` `cIter` указывающего на элемент в сопоставлении, используйте оператор `->`.

Чтобы получить значение ключа для элемента, **сначала**используйте `cIter` -> , что эквивалентно (\* `cIter`). **first**.

Чтобы получить доступ к значению сопоставленной части элемента, используйте `cIter` -> **Second**, который эквивалентен (\* `cIter`). **second**.
...
`const_reverse_iterator`, определяемые картой, указывают на элементы, являющиеся объектами [value_type](#value_type), которые относятся к типу `pair<const Key, Type>`, первый элемент которого является ключом элемента, а второй — сопоставленной частью, содержащейся в элементе.

Для разыменования `const_reverse_iterator crIter`, указывающего на элемент в сопоставлении, используйте оператор `->`.

Чтобы получить значение ключа для элемента, **сначала**используйте `crIter` -> , что эквивалентно (\* `crIter`). **сначала**.
...
*args*\
Аргументы, передаваемые для создания элемента для вставки в сопоставление, кроме случаев, когда сопоставление уже содержит элемент, значение которого правильным образом упорядочено.

### <a name="return-value"></a>Возвращаемое значение

[Пара](../standard-library/pair-structure.md) , компонент **bool** которой имеет значение true, если была сделана вставка, и значение false, если на карте уже содержится элемент эквивалентного значения в упорядочении. Компонент итератора пары "возвращаемое значение" указывает на вновь вставленный элемент, если компонент **bool** имеет значение true, или на существующий элемент, если **логический** компонент имеет значение false.

Чтобы получить доступ к компоненту итератора `pair` `pr`, используйте `pr.first`. для его разыменования используйте `*pr.first`. Чтобы получить доступ к **логическому** компоненту, используйте `pr.second`. См. пример кода далее в этой статье.
...
*args*\
Аргументы, передаваемые для создания элемента, который будет вставлен в сопоставление, кроме ситуации, когда сопоставление уже содержит этот элемента или, в более общем случае, кроме ситуации, когда сопоставление уже содержит элемент, ключ которого правильно упорядочен.
...
Для доступа к первому итератору пары `pr`, возвращаемому функцией-членом, используйте `pr`. **first**, а для сброса ссылки на итератор нижней границы — \*( `pr`. **first**). Для доступа к второму итератору пары `pr`, возвращаемой функцией-членом, нужно использовать `pr`. **second**, а для сброса ссылки на итератор верхней границы — \*( `pr`. **second**).
...
Если возвращаемое значение `find` присвоено `const_iterator`, то объект сопоставления изменить нельзя. Если возвращаемое значение `find` назначается `iterator`, то объект-карту можно изменить
...
*IList*\
[initializer_list](../standard-library/initializer-list.md), из которого нужно скопировать элементы.

### <a name="return-value"></a>Возвращаемое значение

Одноэлементные функции-члены (1) и (2) возвращают [пару](../standard-library/pair-structure.md) , компонент **bool** которой имеет значение true, если была сделана вставка, и значение false, если в карте уже содержится элемент, ключ которого имеет эквивалентное значение в упорядочении. Компонент итератора пары "возвращаемое значение" указывает на вновь вставленный элемент, если компонент **bool** имеет значение true, или на существующий элемент, если **логический** компонент имеет значение false.
...
Чтобы получить доступ к компоненту итератора `pair` `pr`, возвращаемого одноэлементными функциями-членами, используйте `pr.first`. для разыменования итератора в возвращенной паре используйте `*pr.first`, предоставляя элемент. Чтобы получить доступ к **логическому** компоненту, используйте `pr.second`. См. пример кода далее в этой статье.
...
Итератор, определенный в Map, указывает на элементы, являющиеся объектами [value_type](#value_type), которые имеют тип `pair<const Key, Type>`, первый элемент которого является ключом элемента, а второй — сопоставленной частью, содержащейся в элементе.

Для разыменования итератора *iter* , указывающего на элемент в сопоставлении, используйте оператор `->`.

Чтобы получить значение ключа для элемента, используйте `Iter->first`, что эквивалентно `(*Iter).first`. Чтобы получить доступ к значению сопоставленной части элемента, используйте `Iter->second`, что эквивалентно `(*Iter).second`.
...
`iterator` или `const_iterator`, который обращается к расположению элемента в сопоставлении с ключом, который равен ключу аргумента или больше него, либо обращается к расположению, следующему за последним элементом в сопоставлении, если совпадение для ключа не найдено.

Если возвращаемое значение `lower_bound` присвоено `const_iterator`, то объект сопоставления изменить нельзя. Если возвращаемое значение `lower_bound` назначается `iterator`, то объект Map можно изменить.
...
\ *comp*
Функция сравнения типа `const Traits` используется для упорядочения элементов в схеме, которая по умолчанию `hash_compare`.
...
`operator[]` можно использовать для вставки элементов в карту `m` с помощью `m[key] = DataValue;`, где `DataValue` — это значение `mapped_type` элемента со *значением ключа Key.*
...
После удаления существующих элементов в `map``operator=` копирует или перемещает содержимое *прямо* в карту.
...
`reverse_iterator`, определяемые картой, указывают на элементы, являющиеся объектами [value_type](#value_type), которые относятся к типу `pair<const Key, Type>`, первый элемент которого является ключом элемента, а второй — сопоставленной частью, содержащейся в элементе.

Для разыменования `reverse_iterator`ного *спрайта* , указывающего на элемент в сопоставлении, используйте оператор `->`.

Чтобы получить значение ключа для элемента, **сначала**используйте `rIter` -> , что эквивалентно (\* `rIter`). **first**. Чтобы получить доступ к значению сопоставленной части элемента, используйте `rIter` -> **Second**, который эквивалентен (\* `rIter`). **first**.
...
`iterator` или `const_iterator`, который обращается к расположению элемента в сопоставлении с ключом, который больше, чем ключ аргумента, или в случае, если соответствие для ключа не найдено.

Если возвращенное значение назначается `const_iterator`, объект-сопоставление изменить нельзя. Если возвращаемое значение назначается `iterator`, то объект-карту можно изменить.
...
Для Map *m*, если два элемента *E1*(*K1*, *D1*) и *e2*(*априорной оценкой K2*, *D2*) являются объектами типа `value_type`, где *K1* и *k1* являются их ключами типа `key_type` а *D1* и *D2* являются данными типа `mapped_type`, то `m.value_comp(e1, e2)` эквивалентно `m.key_comp(k1, k2)`. Сохраненный объект определяет функцию-член
