...
#### <a name="parameters"></a>Параметры  
 ...
 В C++&14; вы можете включить разнородный поиск, указав предикат std::less<>, не имеющий параметров типа. Более подробные сведения см. в разделе [Разнородный поиск в ассоциативных контейнерах](../standard-library/stl-containers.md#sequence_containers)  
  
 `Allocator`  
 Тип, представляющий сохраненный объект распределителя, который инкапсулирует сведения о выделении и освобождении памяти для сопоставления. Этот аргумент является необязательным, и в качестве значения по умолчанию используется `allocator<pair``<const``Key`*,* `Type``> >`.  
...
-   Класс шаблона, поскольку функции, которые он предоставляет, являются универсальными и не зависят от типа элемента или ключа. Типы данных, используемые для элементов и ключей, определяются как параметры в шаблоне класса вместе с функцией и распределителем сравнения.  
 ...
 Сопоставление упорядочивает контролируемые им элементы путем вызова сохраненного объекта-функции типа [key_compare](#map__key_compare). Этот сохраненный объект является функцией сравнения, для доступа к которой нужно обратиться к методу [key_comp](#map__key_comp). Как правило, любые два заданных элемента сравниваются, чтобы определить, что один из них меньше другого или они равны. После сравнения всех элементов создается упорядоченная последовательность неэквивалентных элементов.  
  
> [!NOTE]
>  Функция сравнения — это бинарный предикат, который вызывает строгое слабое упорядочение в стандартном математически смысле. Бинарный предикат                      f(x,y) является объектом-функцией с двумя объектами-аргументами                     x и                     y и возвращаемым значением `true` или `false`. Порядок, применяемый в наборе, — это строгое слабое упорядочивание, если бинарный предикат является нерефлексивным, антисимметричным и транзитивным и если эквивалентность является транзитивной, где два объекта                      x и                     y определены как эквивалентные, когда и                     f(x,y), и                      f(y,x) равны `false`. Если более строгое условие равенства между ключами заменяет условие эквивалентности, порядок становится общим (т. е. все элементы упорядочиваются в соответствии друг с другом) и сопоставленные ключи будут неотличимы друг от друга.  
>   
...
|||  
|-|-|  
|[map](#map__map)|Создание списка определенного размера или с элементами, обладающими указанным значением или указанным `allocator`, либо в качестве копии другого сопоставления.|  
  
### <a name="typedefs"></a>Typedefs  
  
|||  
|-|-|  
|[allocator_type](#map__allocator_type)|Typedef для класса `allocator` для объекта сопоставления.|  
|[const_iterator](#map__const_iterator)|Typedef для двунаправленного итератора, который может считывать элемент `const` в сопоставлении.|  
|[const_pointer](#map__const_pointer)|Typedef для указателя на элемент `const` в сопоставлении.|  
|[const_reference](#map__const_reference)|Typedef для ссылки на элемент `const`, хранящийся в сопоставлении для чтения и выполнения операций `const`.|  
|[const_reverse_iterator](#map__const_reverse_iterator)|Тип, предоставляющий двунаправленный итератор, который может считывать любой элемент `const` в сопоставлении.|  
|[difference_type](#map__difference_type)|Цельночисленный Typedef со знаком для числа элементов в сопоставлении, в диапазоне между элементами, на которые указывают итераторы.|  
|[iterator](#map__iterator)|Typedef для двунаправленного итератора, который может считывать или изменять любой элемент в сопоставлении.|  
|[key_compare](#map__key_compare)|Typedef для объекта функции, который может сравнить два ключа сортировки для определения относительного порядка двух элементов в сопоставлении.|  
|[key_type](#map__key_type)|Typedef для ключа сортировки, хранящегося в каждом элементе сопоставления.|  
|[mapped_type](#map__mapped_type)|Typedef для данных, хранящихся в каждом элементе сопоставления.|  
|[pointer](#map__pointer)|Typedef для указателя на элемент `const` в сопоставлении.|  
|[reference](#map__reference)|Typedef для ссылки на элемент, сохраненный в сопоставлении.|  
|[reverse_iterator](#map__reverse_iterator)|Typedef для двунаправленного итератора, который может считывать или изменять элемент в обратном сопоставлении.|  
|[size_type](#map__size_type)|Целочисленный Typedef без знака для числа элементов в сопоставлении|  
|[value_type](#map__value_type)|Typedef для типа объекта, хранящейся в виде элемента в сопоставлении.|  
  
### <a name="member-functions"></a>Функции-члены  
  
|||  
|-|-|  
|[at](#map__at)|Поиск элемента с заданным значением ключа.|  
|[begin](#map__begin)|Возврат итератора, указывающего на первый элемент в сопоставлении.|  
|[begin](#map__cbegin)|Возвращает итератор const, указывающего на первый элемент в сопоставлении.|  
|[cend](#map__cend)|Возврат итератора const после конца.|  
|[clear](#map__clear)|Стирает все элементы в сопоставлении.|  
|[count](#map__count)|Возврат числа элементов в сопоставлении, ключ которого соответствует ключу, заданному в параметре.|  
|[crbegin](#map__crbegin)|Возврат итератора const, указывающего на первый элемент в обратном сопоставлении.|  
|[crend](#map__crend)|Возврат итератора const, указывающего на местоположение после завершающего элемента в обратном сопоставлении.|  
|[emplace](#map__emplace)|Вставка в сопоставление элемента, созданного в качестве замены.|  
|[emplace_hint](#map__emplace_hint)|Вставка в сопоставление элемента, созданного в качестве замены, с подсказкой о размещении.|  
|[empty](#map__empty)|Возврат `true`, если сопоставление является пустым.|  
|[end](#map__end)|Возврат итератора после конца.|  
|[equal_range](#map__equal_range)|Возвращает пару итераторов. Первый итератор в паре указывает на первый элемент в `map` с ключом, который больше указанного ключа. Второй итератор в паре указывает на первый элемент в `map` с ключом, который больше или равен данному ключу.|  
|[erase](#map__erase)|Удаление элемента или диапазона элементов в сопоставлении с заданных позиций.|  
|[find](#map__find)|Возврат итератора, указывающего на расположение элемента в сопоставлении с ключом, равным указанному ключу.|  
|[get_allocator](#map__get_allocator)|Возврат копии объекта `allocator`, который используется для формирования сопоставления.|  
|[insert](#map__insert)|Вставка элемента или диапазона элементов в сопоставление в заданной позиции.|  
|[key_comp](#map__key_comp)|Возврат копии объекта сравнения, который используется для упорядочивания ключей в сопоставлении.|  
|[lower_bound](#map__lower_bound)|Возврат итератора, указывающего на первый элемент в сопоставлении с ключом, который больше или равен указанному ключу.|  
|[max_size](#map__max_size)|Возврат максимальной длины сопоставления.|  
|[rbegin](#map__rbegin)|Возврат итератора, указывающего на первый элемент в обратном сопоставлении.|  
|[rend](#map__rend)|Возврат итератора, указывающего на местоположение после завершающего элемента в обратном сопоставлении.|  
|[size](#map__size)|Возврат количества элементов в сопоставлении.|  
|[swap](#map__swap)|Обмен элементами между двумя сопоставлениями.|  
|[upper_bound](#map__upper_bound)|Возврат итератора, указывающего на первый элемент в сопоставлении со значением ключа, которое больше указанного ключа.|  
|[value_comp](#map__value_comp)|Извлечение копии объекта сравнения, который используется для упорядочивания значений элементов в сопоставлении.|  
  
### <a name="operators"></a>Операторы  
  
|||  
|-|-|  
|[operator&#91;&#93;](#map__operator_at)|Вставка элемента в сопоставление с заданным значением ключа.|  
|[operator=](#map__operator_eq)|Замена элементов сопоставления копией другого сопоставления.|  
...
### <a name="example"></a>Пример  
  См. пример для [get_allocator](#map__get_allocator) в качестве примера использования `allocator_type`.  
...
### <a name="remarks"></a>Примечания  
 ...
 `const_iterator`, определенный сопоставлением, указывает на элементы, которые являются объектами [value_type](#map__value_type) типа `pair`\< **constKey**, **Type**>, первый член которых — ключ элемента, а второй — сопоставленные данные, хранящиеся в элементе.  
...
### <a name="example"></a>Пример  
  См. пример для [begin](#map__begin) в качестве примера использования `const_iterator`.  
...
### <a name="remarks"></a>Примечания  
 ...
 В большинстве случаев для доступа к элементам в объекте-сопоставлении следует использовать [итератор](#map__iterator).  
...
### <a name="remarks"></a>Примечания  
 ...
 `const_reverse_iterator`, определенный сопоставлением, указывает на элементы, которые являются объектами [value_type](#map__value_type) типа `pair`*\<***constKey***,* **Type***>*, первый член которых — это ключ элемента, а второй — сопоставленные данные, хранящиеся в элементе.  
  
 Для разыменования `const_reverse_iterator``crIter`, указывающего на элемент в сопоставлении, используйте оператор **->**.  
  
 Для получения доступа к значению ключа для элемента используйте `crIter` -> **first**, что эквивалентно (\* `crIter`). **first**.  
  
 Для получения доступа к значению сопоставленных данных элемента используйте `crIter` -> **second**, что эквивалентно (\* `crIter`). **first**.  
  
### <a name="example"></a>Пример  
  См. пример для [rend](#map__rend) в качестве примера объявления и использования `const_reverse_iterator`.  
...
### <a name="remarks"></a>Примечания  
 `crbegin` используется с обратным `map` так же, как [begin](#map__begin) используется с обычным `map`.  
...
### <a name="remarks"></a>Примечания  
 `crend` используется с обратным сопоставлением так же, как [end](#map__end) используется с обычным `map`.  
...
|-|-|  
|Параметр|Описание|  
|`args`|Аргументы, передаваемые для создания элемента для вставки в сопоставление, кроме случаев, когда сопоставление уже содержит элемент, значение которого правильным образом упорядочено.|  
  
### <a name="return-value"></a>Возвращаемое значение  
 [Пара](../standard-library/pair-structure.md), чей компонент `bool` имеет значение true, если вставка была выполнена, и false, если сопоставление уже содержало элемент с эквивалентным значением в порядке. Компонент итератора пары возвращаемых значений указывает на вставленный элемент, если значение компонента `bool` равно true, или на существующий элемент, если значение компонента `bool` равно false.  
...
### <a name="remarks"></a>Примечания  
 ...
 [value_type](#map__value_type) элемента — это pair, поэтому значением элемента будет упорядоченная пара, первый компонент которой равен значению ключа, а второй — значению данных элемента.  
...
|-|-|  
|Параметр|Описание|  
|`args`|Аргументы, передаваемые для создания элемента, который будет вставлен в сопоставление, кроме ситуации, когда сопоставление уже содержит этот элемента или, в более общем случае, кроме ситуации, когда сопоставление уже содержит элемент, ключ которого правильно упорядочен.|  
...
### <a name="remarks"></a>Примечания  
 **end** используется для проверки, прошел ли итератор конец своего сопоставления.  
  
 Значение, возвращаемое **end**, не должно разыменовываться.  
  
 Пример кода см. в [map::find](#map__find).  
  
##  <a name="a-namemapequalrangea--mapequalrange"></a><a name="map__equal_range"></a>  map::equal_range  
 Возвращает пару итераторов, представляющих [lower_bound](#map__lower_bound) ключа и [upper_bound](#map__upper_bound) ключа.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Итератор, ссылающийся на положение элемента с указанным ключом, или положение после последнего элемента в сопоставлении ( `map::end()`), если соответствие для ключа не найдено.  
...
### <a name="remarks"></a>Примечания  
 ...
 Если возвращаемое значение **find** назначается **const_iterator**, объект-сопоставление изменить нельзя. Если возвращаемое значение **find** назначается **iterator**, объект-сопоставление можно изменить.  
...
|`Where`|Место начала поиска правильной точки вставки. (Если эта точка находится непосредственно перед `Where`, вставка может быть выполнена в постоянном времени с поправкой на амортизацию, а не в логарифмическом времени.)|  
|`ValTy`|Параметр-шаблон, определяющий тип аргумента, с помощью которого сопоставление формирует элемент типа [value_type`Val` и точно пересылает ](#map__value_type) в качестве аргумента.|  
...
|`InputIterator`|Аргумент функции-шаблона, который соответствует требованиям [итератора ввода](../standard-library/input-iterator-tag-struct.md), указывающего на элементы типа, которые можно использовать для создания объектов [value_type](#map__value_type).|  
|`IList`|[initializer_list](../standard-library/initializer-list.md), из которого нужно скопировать элементы.|  
  
### <a name="return-value"></a>Возвращаемое значение  
 Одноэлементные функции-члены (1) и (2) возвращают [пару](../standard-library/pair-structure.md), компонент `bool` которой имеет значение true, если была осуществлена вставка, и значение false, если сопоставление уже содержало элемент с эквивалентным порядковым значением ключа. Компонент итератора пары возвращаемых значений указывает на вставленный элемент, если значение компонента `bool` равно true, или на существующий элемент, если значение компонента `bool` равно false.  
...
### <a name="remarks"></a>Примечания  
 ...
 [value_type](#map__value_type) контейнера — это определение типа, который принадлежит контейнеру. Для сопоставления `map<K, V>::value_type` — это `pair<const K, V>`. Значение элемента — это упорядоченная пара, в которой первый компонент эквивалентен значению ключа, а второй компонент — значению данных элемента.  
 ...
 Для вставки элемента, созданного на месте (то есть без операций копирования или перемещения) см. разделы [map::emplace](#map__emplace) и [map::emplace_hint](#map__emplace_hint).  
...
### <a name="remarks"></a>Примечания  
 **Итератор**, определенный сопоставлением, указывает на элементы, которые являются объектами [value_type](#map__value_type) типа `pair`*\<***constKey**, **Type***>*, первый член которых — ключ элемента, а второй — сопоставленные данные, хранящиеся в элементе.  
  
 Для разыменования **итератора**`Iter`, указывающего на элемент сопоставления, используйте оператор **->**.  
...
### <a name="example"></a>Пример  
  См. пример для [begin](#map__begin) в качестве примера объявления и использования **iterator**.  
...
### <a name="remarks"></a>Примечания  
 ...
 Дополнительные сведения о `Traits` см. в разделе [Класс map](../standard-library/map-class.md).  
  
### <a name="example"></a>Пример  
  См. пример для [key_comp](#map__key_comp) в качестве примера объявления и использования `key_compare`.  
...
### <a name="remarks"></a>Примечания  
 ...
 Дополнительные сведения по `Key` см. в подразделе "Примечания" раздела [Класс map](../standard-library/map-class.md).  
  
### <a name="example"></a>Пример  
  См. пример для [value_type](#map__value_type) в качестве примера объявления и использования `key_type`.  
...
### <a name="return-value"></a>Возвращаемое значение  
 **iterator** или `const_iterator`, который адресует положение элемента в сопоставлении с ключом, который равен ключу-аргументу или больше него, либо адресует положение после последнего элемента в сопоставлении, если соответствие для ключа не найдено.  
  
 Если возвращаемое значение `lower_bound` присвоено `const_iterator`, то объект сопоставления изменить нельзя. Если возвращаемое значение `lower_bound` присвоено **iterator**, то объект сопоставления можно изменить.  
...
|`Comp`|Функция сравнения типа `const Traits` для использования при упорядочивании элементов в сопоставлении. Значение по умолчанию — `hash_compare`.|  
...
### <a name="remarks"></a>Примечания  
 Все конструкторы сохраняют тип объекта-распределителя, управляющего памятью для сопоставления. Затем его можно получить путем вызова [get_allocator](#map__get_allocator). Параметр-распределитель часто не указывается в объявлениях класса и в макросах предварительной обработки, используемых для замены альтернативных распределителей.  
  
 Все конструкторы инициализируют свои сопоставления.  
  
 Все конструкторы хранят объект-функцию типа Traits, которая используется для упорядочивания ключей сопоставления. Затем функцию можно получить путем вызова [key_comp](#map__key_comp).  
  
 Первые три конструктора указывают пустое начальное сопоставление, второй указывает тип функции сравнения ( `Comp`) для использовании при установлении порядка элементов, а третий явно указывает тип распределителя ( `Al`) для использования. Ключевое слово `explicit` подавляет некоторые виды автоматического преобразования типов.  
...
### <a name="remarks"></a>Примечания  
 Тип `mapped_type` является синонимом для параметра-шаблона класса `Type`.  
  
 Дополнительные сведения о `Type` см. в разделе [Класс map](../standard-library/map-class.md).  
  
### <a name="example"></a>Пример  
  См. пример для [value_type](#map__value_type) в качестве примера объявления и использования `mapped_type`.  
...
### <a name="remarks"></a>Примечания  
 ...
 При использовании `operator[]` для вставки элементов возвращаемая ссылка не отображает, меняет ли вставка уже существующий элемент или создает новый. Функции-члены [find](#map__find) и [insert](#map__insert) можно использовать для определения перед вставкой того, существует ли уже элемент с указанным ключом.  
...
### <a name="remarks"></a>Примечания  
 `rbegin` используется с обратным сопоставлением так же, как [begin](#map__begin) используется с обычным сопоставлением.  
...
### <a name="remarks"></a>Примечания  
 `rend` используется с обратным сопоставлением так же, как [end](#map__end) используется с обычным сопоставлением.  
...
### <a name="remarks"></a>Примечания  
 ...
 `reverse_iterator`, определенный сопоставлением, указывает на элементы, которые являются объектами [value_type](#map__value_type) типа `pair`*\<***constKey**, **Type***>*, первый член которых — ключ элемента, а второй — сопоставленные данные, хранящиеся в элементе.  
...
### <a name="example"></a>Пример  
  См. пример для [rbegin](#map__rbegin) в качестве примера объявления и использования `reverse_iterator`.  
...
### <a name="example"></a>Пример  
  См. пример для [size](#map__size) в качестве примера объявления и использования `size_type`.  
...
### <a name="return-value"></a>Возвращаемое значение  
 **Итератор** или `const_iterator`, адресующий положение элемента в сопоставлении с ключом, значение которого больше ключа-аргумента, или адресующий положение после последнего элемента в сопоставлении, если соответствие для ключа не найдено.  
  
 Если возвращенное значение назначается `const_iterator`, объект-сопоставление изменить нельзя. Если возвращенное значение назначается **iterator**, объект-сопоставление можно изменить.  
