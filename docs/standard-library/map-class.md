...
|[begin](#cbegin)|Возвращает итератор const, указывающего на первый элемент в сопоставлении.|
...
Для доступа к значению сопоставленной единицы информации для элемента используйте `cIter` -> **second**, что эквивалентно (\* `cIter`). **second**.
...
@No__t_0, определяемые картой, указывают на элементы, являющиеся объектами [value_type](#value_type), которые имеют тип `pair<const Key, Type>`, первый элемент которого является ключом элемента, а второй — сопоставленной частью, содержащейся в элементе.
...
Для доступа к первому итератору пары `pr`, возвращаемой функцией-членом, нужно использовать `pr`. **first**, а для разыменования итератора нижней границы — \*( `pr`. **first**). Для доступа ко второму итератору пары `pr`, возвращаемой функцией-членом, нужно использовать `pr`. **second**, а для сброса ссылки на итератор верхней границы — \*( `pr`. **second**).
...
Для доступа к компоненту итератора `pair` `pr`, возвращаемого одноэлементными функциями-членами, используйте `pr.first`. Для разыменования итератора в возвращенной паре используйте `*pr.first`, чтобы получить элемент. Чтобы получить доступ к **логическому** компоненту, используйте `pr.second`. См. пример кода далее в этой статье.
...
Итератор, определенный в Map, указывает на элементы, являющиеся объектами [value_type](#value_type), которые относятся к типу `pair<const Key, Type>`, первый элемент которого является ключом элемента, а второй — сопоставленной частью, содержащейся в элементе.
...
@No__t_0 или `const_iterator`, который обращается к расположению элемента в сопоставлении с ключом, который равен ключу аргумента или больше него, либо обращается к расположению, следующему за последним элементом в сопоставлении, если совпадение для ключа не найдено.
...
Для получения доступа к значению ключа для элемента используйте `rIter` -> **first**, что эквивалентно (\* `rIter`). **first**. Для доступа к значению сопоставленной единицы информации для элемента используйте `rIter` -> **second**, что эквивалентно (\* `rIter`). **first**.
...
@No__t_0 или `const_iterator`, который обращается к расположению элемента в сопоставлении с ключом, который больше, чем ключ аргумента, или в случае, если соответствие для ключа не найдено.
...
Для Map *m*, если два элемента *E1*(*K1*, *D1*) и *e2*(*априорной оценкой K2*, *D2*) являются объектами типа `value_type`, где *K1* и *K1* — это ключи типа 0, а *D1* и *D2* — их данные типа 3, а затем 4 эквивалентны 5. Хранимый объект определяет функцию-член
