...
`Key` Тип данных ключа для сохранения в сопоставлении.

`Type` Тип данных элемента для сохранения в сопоставлении.

`Traits` Тип, предоставляющий объект функции, который может сравнить два значения элементов как ключи сортировки для определения их относительного порядка в схеме. Этот аргумент является необязательным, и в качестве значения по умолчанию используется бинарный предикат `less<Key>`.
...
`Allocator` Тип, представляющий сохраненный объект распределителя, инкапсулирующий сведения о выделении и освобождении памяти карты. Этот аргумент является необязательным, и значением по умолчанию является `allocator<pair<const Key, Type> >`.
...
|[const_iterator](#const_iterator)|Typedef для двунаправленного итератора, который может считывать элемент `const` в сопоставлении.|
|[const_pointer](#const_pointer)|Typedef для указателя на элемент `const` в сопоставлении.|
|[const_reference](#const_reference)|Typedef для ссылки на элемент `const`, хранящийся в сопоставлении для чтения и выполнения операций `const`.|
|[const_reverse_iterator](#const_reverse_iterator)|Тип, предоставляющий двунаправленный итератор, который может считывать любой элемент `const` в сопоставлении.|
...
|[pointer](#pointer)|Typedef для указателя на элемент `const` в сопоставлении.|
...
|[empty](#empty)|Возврат `true`, если сопоставление является пустым.|
...
Для разыменования `const_iterator` `cIter` указывает на элемент в сопоставлении, используйте **->** оператор.
...
`const_reverse_iterator` Определяется точек карты для элементов, которые являются объектами [value_type](#value_type), то есть типа `pair<const Key, Type>`, которого первый элемент — это ключ к элементу и которого второй член является сопоставленных datum, удерживаемые элемента.
...
Для доступа к значению сопоставленных datum для элемента, используйте `crIter`  ->  **второй**, что эквивалентно (\* `crIter`). **Первый**.
...
`key` Значение ключа элементов для сравнения с карты.
...
Для доступа к компоненту итератора `pair` `pr`, используйте `pr.first`; для ее разыменовании используйте `*pr.first`. Для доступа к компоненту `bool` используйте `pr.second`. См. пример кода далее в этой статье.
...
`key` Значение ключа аргумента, который будет сравниваться с ключом сортировки элемента из карты, в которой выполняется поиск.
...
`key` Значение ключа, к которым сравнивается ключ сортировки элемента из карты, в которой выполняется поиск.
...
|`Where`|Место начала поиска правильной точки вставки. (Если эта точка находится непосредственно перед `Where`, вставка может быть выполнена в постоянном времени с поправкой на амортизацию, а не в логарифмическом времени.)|
|`ValTy`|Параметр-шаблон, определяющий тип аргумента, с помощью которого сопоставление формирует элемент типа [value_type`Val` и точно пересылает ](#value_type) в качестве аргумента.|
...
**Итератор** определяется точек карты для элементов, которые являются объектами [value_type](#value_type), то есть типа `pair`*\<***constKey**, **Тип***>*, которого первый элемент — это ключ к элементу и которого второй член является сопоставленных datum, удерживаемые элемента.
...
`reverse_iterator` Определяется точек карты для элементов, которые являются объектами [value_type](#value_type), то есть типа `pair`*\<***constKey**, **Тип***>*, которого первый элемент — это ключ к элементу и которого второй член является сопоставленных datum, удерживаемые элемента.

Для разыменования `reverse_iterator` `rIter` указывает на элемент в сопоставлении, используйте **->** оператор.
...
`right` Аргумент карты, предоставляющий элементы, которые будут заменены целевой карты.
...
`key` Значение ключа аргумента для сравнения со значением ключа сортировки элемента из карты, в которой выполняется поиск.
...
[\<карта > членов](http://msdn.microsoft.com/en-us/7e8f0bc2-6034-40f6-9d14-76d4cef86308)<br/>
