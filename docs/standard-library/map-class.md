...
*Ключ* тип данных ключа для сохранения в сопоставлении.

*Тип* тип данных элемента для сохранения в сопоставлении.

*Признаки* тип, предоставляющий объект функции, который может сравнить два значения элементов как ключи сортировки для определения их относительного порядка в сопоставлении. Этот аргумент является необязательным, и в качестве значения по умолчанию используется бинарный предикат `less<Key>`.
...
*ключ* значение ключа элементов для сравнения с карты.
...
*ключ* значение ключа аргумента для сравнения с ключом сортировки элемента из сопоставления в котором выполняется поиск.
...
*ключ* значение ключа для сравнения с ключом сортировки элемента из сопоставления в котором выполняется поиск.
...
`iterator` Определяется точек карты для элементов, которые являются объектами [value_type](#value_type), то есть типа `pair`*\<***constKey**, **Тип***>*, которого первый элемент — это ключ к элементу и которого второй член является сопоставленных datum, удерживаемые элемента.

Для разыменования **итератор** `Iter` указывающего на элемент в сопоставлении, используйте `->` оператор.
...
*правом* сопоставление-аргумент предоставляет элементы для обмена с целевым сопоставлением.
...
*ключ* значение ключа аргумента для сравнения с значением ключа сортировки элемента из сопоставления в котором выполняется поиск.
