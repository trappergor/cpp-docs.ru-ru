...
Эту функцию-член можно использовать вместо функции-члена `begin()`, чтобы гарантировать, что возвращаемое значение будет `const_iterator`. Обычно используется вместе с ключевым словом вывода типа [auto](../cpp/auto-cpp.md), как показано в следующем примере. В этом примере `Container` рекомендуется использовать изменяемый (не **const**) контейнер любого типа, который поддерживает `begin()` и. `cbegin()`
...
Объект `const_reverse_iterator` , определенный сопоставлением, указывает на элементы, являющиеся объектами [value_type](#value_type), которые имеют тип `pair<const Key, Type>`, первый элемент которого является ключом элемента, а второй — сопоставленной частью, содержащейся в элементе.

Для разыменования объекта `const_reverse_iterator crIter` , указывающего на элемент в сопоставлении, `->` используйте оператор.

Чтобы получить доступ к значению ключа для `crIter`элемента, используйте\*  ->  **First**, который эквивалентен ( `crIter`). **Сначала**.
...
Чтобы получить доступ к компоненту `pair` `pr`итератора, `pr.first`используйте оператор; для разыменования его `*pr.first`используйте. Чтобы получить доступ  к логическому компоненту, используйте `pr.second`. См. пример кода далее в этой статье.
...
Если возвращаемое значение `find` присвоено `const_iterator`, то объект сопоставления изменить нельзя. Если возвращаемое значение `find` присваивается `iterator`, то объект Map можно изменить
...
Чтобы получить доступ к значению ключа для элемента, используйте `Iter->first`, что `(*Iter).first`эквивалентно. Чтобы получить доступ к значению сопоставленной версии элемента, используйте `Iter->second`, что `(*Iter).second`эквивалентно.
...
Объект `iterator` или`const_iterator` , который обращается к расположению элемента в сопоставлении с ключом, который равен ключу аргумента или больше него, либо обращается к расположению, следующему за последним элементом в сопоставлении, если совпадение для ключа не найдено.

Если возвращаемое значение `lower_bound` присвоено `const_iterator`, то объект сопоставления изменить нельзя. Если возвращаемое значение `lower_bound` присваивается `iterator`, то объект Map можно изменить.
...
*Соответствовал*\
Функция сравнения типа `const Traits` для использования при упорядочивании элементов в сопоставлении. Значение по умолчанию — `hash_compare`.
...
`operator[]`может использоваться для вставки `m` элементов в карту с помощью `m[key] = DataValue;` WHERE, `DataValue` где — это значение `mapped_type` элемента с ключевым значением *Key*.
...
Объект `reverse_iterator` , определенный сопоставлением, указывает на элементы, являющиеся объектами [value_type](#value_type), которые имеют тип `pair<const Key, Type>`, первый элемент которого является ключом элемента, а второй — сопоставленной частью, содержащейся в элементе.
...
Объект `iterator` или`const_iterator` , который обращается к расположению элемента в сопоставлении с ключом, который больше, чем ключ аргумента, или в случае, если соответствие для ключа не найдено.

Если возвращенное значение назначается `const_iterator`, объект-сопоставление изменить нельзя. Если возвращаемое значение присвоено `iterator`, объект Map можно изменить.
...
Для Map *m*, если два элемента *E1*(*K1*, *D1*) и *E2*(*априорной оценкой K2*, *D2*) являются `value_type`объектами типа, где *K1* и *K1* являются ключами типа `key_type` и *D1* и *D2* — это данные типа `mapped_type`, `m.key_comp(k1, k2)`а `m.value_comp(e1, e2)` — эквивалентно. Сохраненный объект определяет функцию-член
