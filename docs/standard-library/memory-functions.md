...
`Val` Объект или функцию, для которого необходимо получить истинный адрес.
...
`Alignment` Граница выравнивания попыток.

`Size` Размер в байтах для выровненного хранилища.

`Ptr` Начальный адрес доступного смежного пула хранилища для использования. Этот параметр также является выходным параметром и задать должен содержать новый начальный адрес, если выравнивание выполняется успешно. Если `align()` выполняется неудачно, этот параметр не изменяется.

`Space` Общее пространство, доступное для `align()` для использования при создании выровненного хранилища. Этот параметр также является параметром вывода и содержит откорректированное пространство, оставшееся в хранилище после вычитания выровненного хранилища и всей связанной с ним дополнительной нагрузки.
...
`Alloc` Распределитель, используемый для создания объектов.

`Args` Ноль или более аргументов, которые будут объектами.
...
`Ty` Тип, управляемый возвращаемым общим указателем.

`Other` Тип, управляемый общим указателем на аргумент.

`Other` Общий указатель на аргумент.

### <a name="remarks"></a>Примечания

Функция шаблона возвращает пустой объект shared_ptr, если `const_cast<Ty*>(sp.get())` возвращает пустой указатель; в противном случае возвращается [класс shared_ptr](../standard-library/shared-ptr-class.md)\<за этот год > объекта, которому принадлежит ресурс, владельцем которого является `sp`. Выражение `const_cast<Ty*>(sp.get())` должно быть допустимым.
...
Функция сообщает любой сборщик мусора, диапазон адресов `[ ptr, ptr + _Size)` больше не будут содержать трассируемые указатели. (Все указатели на хранилище, выделенное должен не быть разыменован, кроме предоставления доступа.)
...
`ptr` Указатель на доступность, выделить области хранения.
...
`Ptr` Указатель на объект для удаления.

Другой тип элементов в массиве для удаления.
...
`sp` Общий указатель на аргумент.

### <a name="remarks"></a>Примечания

Функция шаблона возвращает пустой объект shared_ptr, если `dynamic_cast<Ty*>(sp.get())` возвращает пустой указатель; в противном случае возвращается [класс shared_ptr](../standard-library/shared-ptr-class.md)\<за этот год > объекта, которому принадлежит ресурс, владельцем которого является `sp`. Выражение `dynamic_cast<Ty*>(sp.get())` должно быть допустимым.
...
`count` Максимальное количество элементов в запросе, для которого требуется выделить память.
...
`T` Тип объекта, `unique_ptr` будет указывать значение.

`Types` Типы аргументов конструктора, определяемое `Args`.

`Args` Аргументы для передачи в конструктор объекта типа `T`.

`Elem` Массив элементов типа `T`.

`Size` Число элементов, чтобы выделить место для нового массива.
...
`_left` Общий или слабый указатель.

`right` Общий или слабый указатель.
...
*_Pbuf* указатель, чтобы освободить память.
...
Функция шаблона возвращает пустой объект shared_ptr, если `sp` представляет собой пустой `shared_ptr` объекта; в противном случае возвращается [класс shared_ptr](../standard-library/shared-ptr-class.md)\<за этот год > объекта, которому принадлежит ресурс, владельцем которого является `sp`. Выражение `static_cast<Ty*>(sp.get())` должно быть допустимым.
...
`Ty` Тип, управляемый левый общий или слабый указатель.

`Other` Тип, управляемый правый общий или слабый указатель.

`left` Левый общий или слабый указатель.

`right` Правый общий или слабый указатель.
...
Функция сообщает любой сборщик мусора, диапазон адресов `[ptr, ptr + _Size)` теперь могут содержать трассируемые указатели.

## <a name="undeclare_reachable"></a>  undeclare_reachable

Отменяет объявление возможности доступа указанная область памяти.
...
Если `ptr` не `nullptr`, функция сообщает любой сборщик мусора, `ptr` больше не доступна. Возвращает указатель на производный безопасно, сравнивает равно `ptr`.
...
`first` Входной итератор, обращающийся к первому элементу в исходном диапазоне.

`last` Входной итератор, обращающийся к последнему элементу в исходном диапазоне.

`dest` Прямой итератор, обращающийся к первому элементу в диапазоне назначения.

### <a name="return-value"></a>Возвращаемое значение

Прямой итератор, обращающийся к первой позиции после диапазон назначения, если исходный диапазон был пуст.
...
`first` Входной итератор, который ссылается на объект, подлежащий копированию.

`count` Целое число со знаком или без знака тип, указывающий количество раз для копирования объекта.

`dest` Прямой итератор, ссылающийся на куда новых копий.
...
`first` Прямой итератор, обращающийся к первому элементу в диапазоне назначения, который должен инициироваться.

`last` Прямой итератор, обращающийся к последнему элементу в диапазоне назначения, который должен инициироваться.

`val` Значение, используемое для инициализации диапазона назначения.
...
`first` Прямой итератор, обращающийся к первому элементу в диапазоне назначения, чтобы инициировать.

`count` Число подлежащих инициализации элементов.
