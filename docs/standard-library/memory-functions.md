...
### <a name="parameters"></a>Параметры  
 ...
 `Ptr`  
 Начальный адрес доступного смежного пула хранилища для использования. Этот параметр также является выходным параметром и задать должен содержать новый начальный адрес, если выравнивание выполняется успешно. Если `align()` выполняется неудачно, этот параметр не изменяется.  
