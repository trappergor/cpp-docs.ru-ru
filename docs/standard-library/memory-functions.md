...
*Val* объекта или функции, для которого необходимо получить истинный адрес.
...
*Выравнивание* граница выравнивания для выполнения попытки.

*Размер* размер в байтах для выровненного хранилища.

*PTR* начальный адрес доступного смежного пула хранилища для использования. Этот параметр также является параметром output и присваивается содержать новый начальный адрес в том случае, если выравнивание выполняется успешно. Если `align()` выполняется неудачно, этот параметр не изменяется.

*Пространство* общее пространство, доступное для `align()` для использования при создании выровненного хранилища. Этот параметр также является параметром вывода и содержит откорректированное пространство, оставшееся в хранилище после вычитания выровненного хранилища и всей связанной с ним дополнительной нагрузки.
...
*Alloc* распределителя, используемого для создания объектов.
...
*Ty* тип, управляемый возвращенного общий указатель.

*Другие* тип, управляемый общим указателем аргумента.

*Другие* общий указатель на аргумент.
...
*PTR* указатель в области хранения доступен, выделенный, допустимым.
...
*число* — максимальное количество элементов, запрошенный для какая область памяти для выделения.
...
*T* тип объекта, `unique_ptr` будет указывать.

*Типы* типы аргументов конструктора, заданных параметром *Args*.

*Args* аргументы, передаваемые конструктору объекта типа *T*.

*Elem* массив элементов типа *T*.

*Размер* количество элементов, которые нужно выделить место в новом массиве.
...
*_слева* общий или слабый указатель.

*правом* общий или слабый указатель.
...
*_Pbuf* указатель на область памяти будет отменено позже.
...
*Другие* тип, управляемый правый общий или слабый указатель.

*слева* левый общий или слабый указатель.

*правом* правый общий или слабый указатель.
...
*Первый* входной итератор, указывающий на первый элемент в исходном диапазоне.

*последний* итератор ввода последнего элемента в исходном диапазоне.

*dest* прямой итератор, указывающий на первый элемент в диапазоне назначения.
...
*Первый* итератор ввода, который ссылается на объект для копирования.

*число* объект подписанные или не подписанные целочисленный тип, указывающий, сколько раз копируемый объект.
...
*Первый* прямой итератор, указывающий на первый элемент в диапазоне назначения, подлежащем инициализации.

*последний* прямой итератор, адресующий последний элемент в диапазоне назначения, подлежащем инициализации.
...
*Первый* прямой итератор, адресующий первый элемент в диапазоне назначения, подлежащем инициализации.

*число* число подлежащих инициализации элементов.
