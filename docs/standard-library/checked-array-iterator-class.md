---
title: Класс checked_array_iterator | Документы Майкрософт
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
...
`index` (Необязательно) Элемент в массиве, чтобы инициализировать итератор.  По умолчанию итератор инициализируется первым элементом в массиве.
...
`right` `checked_array_iterator` По которому проверяется на равенство.
...
`right` `checked_array_iterator` По которому проверяется на неравенство.
...
`right` `checked_array_iterator` Для сравнения.
...
`_Off` Смещение, на которую увеличивается итератор.
...
`_Off` Смещение для добавления `checked_array_iterator`.
...
`_Off` Смещение для уменьшения из `checked_array_iterator`.
