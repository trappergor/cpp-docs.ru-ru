...
Элементы класса `Type`, оговоренные в первом параметре шаблона объекта Queue, являются синонимами [value_type](#value_type) и должны соответствовать типу элемента в базовом классе контейнера `Container` заданном вторым параметром-шаблоном. `Type` должны быть назначены, чтобы можно было копировать объекты этого типа и присваивать значения переменным этого типа.

Подходящие базовые классы контейнеров для очереди включают [deque](../standard-library/deque-class.md) и [List](../standard-library/list-class.md), а также любой другой контейнер последовательности, который поддерживает операции `front`, `back`, `push_back`и `pop_front`. Класс базового контейнера инкапсулирован в адаптер контейнера, который предоставляет только ограниченный набор функций-членов контейнера последовательностей в виде открытого интерфейса.

Объекты очереди сравнимы по равенству только в том случае, если элементы класса `Type` сравнимы по равенству и меньше, чем сравнимы, только если элементы класса `Type` менее сравнимы.
...
Если возвращаемое значение `front` присвоено `const_reference`, то объект очереди изменить нельзя. Если возвращаемое значение `front` назначается `reference`, то объект очереди можно изменить.

Функция-член возвращает `reference` первому элементу управляемой последовательности, который должен быть непустым.
