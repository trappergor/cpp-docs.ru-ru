...
- Он может считывать один или несколько элементов типа **char**, как при последовательных вызовах формы `fgetc(fp)` , и преобразовывать их в элемент `ch` типа `Char_T` , используя аспект преобразования файла `fac` для вызова `fac.in` по мере необходимости. В случае сбоя какого-либо преобразования или операции чтения эта функция завершается неудачно.
