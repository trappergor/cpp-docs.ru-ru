...
*Ключ* тип данных элемента для сохранения в объекте hash_multiset.
...
*Распределитель* тип, представляющий сохраненный объект распределителя, инкапсулирующий сведения о выделении и освобождении памяти объекта hash_multiset. Этот аргумент является необязательным, и значение по умолчанию — **распределителя***\<ключа>.*
...
Объект hash_multiset упорядочивает управляемую им последовательность путем вызова хранимого объекта хэш-признаков типа [value_compare](#value_compare). Доступ к этому хранимому объекту можно получить через вызов функции-члена [key_comp](#key_comp). Такой объект функции должен вести себя так же, как объект класса `hash_compare`*<Key,* **менее***\<ключ> >.* В частности, для всех значений *ключ* типа `Key`, вызов **признака**( *ключ*) создает распределение значений типа `size_t`.
...
|[const_iterator](#const_iterator)|Тип, предоставляющий двунаправленный итератор, который может читать **const** элемент `hash_multiset`.|
|[const_pointer](#const_pointer)|Тип, предоставляющий указатель на **const** элемент `hash_multiset`.|
|[const_reference](#const_reference)|Тип, предоставляющий ссылку на **const** элемент хранится в `hash_multiset` для чтения и выполнения **const** операций.|
|[const_reverse_iterator](#const_reverse_iterator)|Тип, предоставляющий двунаправленный итератор, который может читать любой **const** элемент `hash_multiset`.|
...
Если возвращаемое значение `begin` назначается `const_iterator`, невозможно изменить элементы в объекте hash_multiset. Если возвращаемое значение `begin` назначается `iterator`, элементы в объекте hash_multiset можно изменить.
...
*ключ* ключ элементов для сопоставления из объекта hash_multiset.
...
`end` используется для проверки, достиг ли итератор конца своего объекта hash_multiset. Значение, возвращаемое `end`, не должно быть подвергнуто удалению ссылки.
...
*ключ* ключ-аргумент для сравнения с ключом сортировки элемента из объекта hash_multiset в котором выполняется поиск.
...
*_Where* положение элемента, удаляемого из объекта hash_multiset.

*Первый* положение первого элемента, удаляемого из объекта hash_multiset.

*последний* положение сразу после последнего элемента, удаляемого из объекта hash_multiset.

*ключ* ключ элементов, удаляемый из объекта hash_multiset.
...
Функция-член возвращает итератор, который адресует элемент в hash_multiset, ключ сортировки которого является `equivalent` аргумент ключу согласно двоичному предикату, применяющему упорядочение на основе менее-чем отношения.

Если возвращаемое значение `find` назначается `const_iterator`, объект hash_multiset изменить нельзя. Если возвращаемое значение `find` назначается `iterator`, то объект hash_multiset можно изменить.
...
|*Зап.*|Функция сравнения типа `const Traits`, используемая для упорядочивания элементов в `hash_multiset`. Значение по умолчанию — `hash_compare`.|
|*Справа*|`hash_multiset`, копией которого будет создаваемый `hash_multiset`.|
...
|*Первый*|Положение первого элемента для копирования из объекта hash_multiset.|
|*последний*|Положение сразу после последнего элемента для копирования из объекта hash_multiset.|
...
Вставка может происходить в амортизированном константном времени для указания версии insert, а не в логарифмическом времени, если точка вставки следует сразу за *где*.
...
Возвращает параметр-шаблон hash_multiset *Traits*, который содержит объекты-функции, используемые для хэширования и упорядочивания элементов контейнера.

Дополнительные сведения о *Traits* см. в разделе [класс hash_multiset](../standard-library/hash-multiset-class.md) раздела.
...
Дополнительные сведения о *ключ*, см. в разделе "Примечания" [класс hash_multiset](../standard-library/hash-multiset-class.md) раздела.
...
После удаления всех существующих элементов в `hash_multiset`, `operator=` копирует или перемещает содержимое *правой* в `hash_multiset`.
...
*правом* аргумент hash_multiset, предоставляющий элементы для обмена с целевого объектаhash_multiset.
