...
`Traits` Тип, который включает в себя два объекта функции, один для сравнения классов, бинарным предикатом, способным сравнить два значения элементов как ключи сортировки для определения их относительного порядка и хэш-функции, которая является ключевые значения унарного предиката сопоставления элементов в числа без знака целые значения типа **size_t**. Этот аргумент является необязательным и `hash_compare`*<Key,* **менее***\<ключ> >* значение по умолчанию.

`Allocator` Тип, представляющий сохраненный объект распределителя, инкапсулирующий сведения о выделении и освобождении памяти объекта hash_multiset. Этот аргумент является необязательным, и значение по умолчанию — **распределителя***\<ключа>.*
...
`key` Ключ элементов для сопоставления из объекта hash_multiset.
...
`key` Аргумент ключ для сравнения с ключом сортировки элемента из объекта hash_multiset, в которой выполняется поиск.
...
`first` Положение первого элемента, удаляемого из объекта hash_multiset.

`last` Положение после последнего элемента, удаляемого из объекта hash_multiset.

`key` Ключ элементов, удаляемого из объекта hash_multiset.
...
`key` Ключ аргумента, к которым сравнивается ключ сортировки элемента из объекта hash_multiset, в которой выполняется поиск.
...
`right` Аргумент hash_multiset, предоставляющий элементы, которые будут заменены целевой hash_multiset.
