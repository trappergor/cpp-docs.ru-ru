...
|[const_iterator](#const_iterator)|Тип, предоставляющий двунаправленный итератор, который может читать элемент **const** в `hash_multiset`.|
|[const_pointer](#const_pointer)|Тип, предоставляющий указатель на элемент **const** в `hash_multiset`.|
|[const_reference](#const_reference)|Тип, предоставляющий ссылку на элемент **const** , хранящийся в `hash_multiset` для чтения и выполнения операций **const** .|
|[const_reverse_iterator](#const_reverse_iterator)|Тип, предоставляющий двунаправленный итератор, который может читать любой элемент **const** в `hash_multiset`.|
...
|[begin](#cbegin)|Возвращает итератор const, обращающийся к первому элементу в `hash_multiset`.|
...
Если возвращаемое значение `begin` присваивается `const_iterator`, то элементы в объекте hash_multiset не могут быть изменены. Если возвращаемое значение `begin` присваивается `iterator`, то элементы в объекте hash_multiset можно изменить.
...
`end` используется для проверки того, достиг ли итератор конца его hash_multiset. Значение, возвращаемое `end`, не должно быть подвергнуто удалению ссылки.
...
Функция-член возвращает итератор, который обращается к элементу в hash_multiset, чей ключ сортировки `equivalent` ключу аргумента в двоичном предикате, который вызывает упорядочивание на основе отношения сравнения "меньше чем".

Если возвращаемое значение `find` присваивается `const_iterator`, то объект hash_multiset изменить нельзя. Если возвращаемое значение `find` присваивается `iterator`, то объект hash_multiset можно изменить.
...
|*Соответствовал*|Функция сравнения типа `const Traits`, используемая для упорядочивания элементов в `hash_multiset`. Значение по умолчанию — `hash_compare`.|
...
Первые три конструктора указывают пустой начальный `hash_multiset`, второй задает тип функции сравнения (*comp*), используемой при установлении порядка элементов, а третий явно указывает тип распределителя (*Al*) используется. Ключевое слово **explicit** подавляет некоторые виды автоматического преобразования типов.
...
|*Началь*|Положение первого элемента для копирования из объекта hash_multiset.|
|*Последняя*|Положение сразу после последнего элемента для копирования из объекта hash_multiset.|
|*Интерфейс*|Объект initializer_list, содержащий элементы для копирования.|
...
Возвращает *признаки*параметра шаблона hash_multiset, которые содержат объекты-функции, используемые для хэширования и упорядочивания элементов контейнера.
...
Дополнительные сведения о *ключе*см. в разделе "Примечания" статьи о [классе hash_multiset](../standard-library/hash-multiset-class.md) .
...
После удаления существующих элементов в `hash_multiset` `operator=` копирует или перемещает содержимое *непосредственно* в `hash_multiset`.
