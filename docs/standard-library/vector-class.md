...
Ссылка на элемент, индекс которого указан в аргументе. Если `_Off` значение больше размера вектора, `at` вызывает исключение.
...
После стирания любых существующих элементов `vector`в, `operator=` копирует или перемещает `vector`содержимое *непосредственно* в.
