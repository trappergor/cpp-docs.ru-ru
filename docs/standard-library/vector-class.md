...
[ \<bool> Класс Vector](../standard-library/vector-bool-class.md) является полной специализацией вектора шаблона класса для элементов типа **`bool`** . Он имеет распределитель для базового типа, используемого специализацией.

[ \<bool> Класс ссылки Vector](../standard-library/vector-bool-class.md#reference_class) является вложенным классом, объекты которого могут предоставлять ссылки на элементы (одиночные биты) в \<bool> объекте Vector.
...
|||
|-|-|
|[уязвимо](#vector)|Создает вектор определенного размера, вектор с элементами определенного значения, вектор с определенным `allocator`, или вектор как копию какого-либо другого вектора.|
...
|[итераци](#iterator)|Тип, предоставляющий итератор произвольного доступа, который может читать или изменять любой элемент в векторе.|
|[вид](#pointer)|Тип, предоставляющий указатель на элемент в векторе.|
...
|[Назад](#back)|Возвращает ссылку на последний элемент вектора.|
|[начале](#begin)|Возвращает итератор произвольного доступа, указывающий на первый элемент в векторе.|
|[ресурсов](#capacity)|Возвращает число элементов, которое вектор может содержать без выделения дополнительного пространства.|
...
|[открытым](#clear)|Очищает элементы вектора.|
...
|[конце](#end)|Возвращает итератор произвольного доступа, который указывает на конец вектора.|
|[erase](#erase)|Удаляет элемент или диапазон элементов в векторе из заданных позиций.|
|[крышку](#front)|Возвращает ссылку на первый элемент в векторе.|
...
|[предназначен](#reserve)|Резервирует минимальную длину хранилища для объекта вектора.|
|[изменить размер](#resize)|Определяет новый размер вектора.|
|[shrink_to_fit](#shrink_to_fit)|Удаляет лишнюю емкость.|
|[size](#size)|Возвращает количество элементов в векторе.|
|[позиции](#swap)|Меняет местами элементы двух векторов.|
...
Ссылка на элемент, индекс которого указан в аргументе. Если значение параметра *Disposition* больше размера вектора, `at` вызывает исключение.
...
Если возвращаемое значение `begin` присваивается [vector:: const_iterator](#const_iterator), то `vector` объект нельзя изменить. Если возвращаемое значение `begin` назначается [vector::iterator](#iterator), то объект `vector` можно изменить.
...
*разместить*\
Место в [векторе](../standard-library/vector-class.md) , куда вставляется первый элемент.
...
Любая операция вставки может быть дорогостоящей, см. раздел [класс Vector](../standard-library/vector-class.md) для обсуждения `vector` производительности.
...
Итератор после конца для вектора. Если Vector является пустым, то `vector::end() == vector::begin()` .
...
Как предусловие, *First* и *Last* не должны быть итераторами в вектор или поведение не определено. Любая операция вставки может быть дорогостоящей, см. раздел [класс Vector](../standard-library/vector-class.md) для обсуждения `vector` производительности.
...
После стирания любых существующих элементов в `vector` , `operator=` копирует или перемещает содержимое *непосредственно* в `vector` .
...
## <a name="vector"></a><a name="vector"></a>уязвимо
