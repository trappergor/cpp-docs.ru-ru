...
*Тип* тип данных элемента для сохранения в объекте vector

`Allocator` Тип, представляющий сохраненный объект распределителя, инкапсулирующий сведения о выделении и освобождении памяти вектора. Этот аргумент является необязательным, и значение по умолчанию — **распределителя***\<тип>.*
...
`Count` Количество копий элемента, вставляемого в вектор.
...
`_Pos` Номер индекса или позиции элемента, который требуется ссылка в векторе.
...
`Val` Значение, присваиваемое элемент, добавляемый в конец вектора.
...
`count` Минимальная длина хранилища, выделяемого для вектора.
...
`Val` Значение инициализации новых элементов, добавленных в вектор, если новый размер больше, исходный размер. Если значение опущено, новые объекты используют конструктор по умолчанию.
