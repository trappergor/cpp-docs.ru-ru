...
Ссылка на элемент, индекс которого указан в аргументе. Если `_Off` больше, чем размер вектора, `at` возникло исключение.
...
Если возвращаемое значение `end` присваивается переменной типа `const_iterator`, то объект вектора нельзя изменить. Если возвращаемое значение `end` присваивается переменной типа `iterator`, то объект вектора можно изменить.
...
Как предусловие *первый* и *последнего* не должны быть итераторами в векторе, или поведение не определено. Любая операция вставки может быть ресурсоемкой. Факторы производительности при работе с объектом `vector` рассматриваются в разделе, посвященном [классу vector](../standard-library/vector-class.md).
...
После удаления всех существующих элементов в `vector`, `operator=` копирует или перемещает содержимое *правой* в `vector`.
...
|*Справа*|Вектор, для которого создаваемый вектор станет копией.|
|*Первый*|Положение первого элемента в диапазоне копируемых элементов.|
...
Шестой конструктор задает копию вектора *справа*.

Седьмой конструктор перемещает вектор *справа*.
