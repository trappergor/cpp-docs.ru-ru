...
`_Fnref` Справочник по оболочку, которая содержит вызываемый объект.
...
`right` Копируемый объект функции.

`Fx` Тип вызываемый объект.
...
`right` Объект функции для замены.
...
`Fty2` Тип целевого объекта, вызываемые для проверки.
