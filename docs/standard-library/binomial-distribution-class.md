...
Класс шаблона описывает распределение, которое создает значения указанного пользователем целочисленного типа или тип **int** , если он не указан, распределяется в соответствии с дискретным распределением по модели вероятности. В следующей таблице представлены ссылки на статьи об отдельных членах.
...
Члены `t()` свойств и `p()` возвращают текущие сохраненные значения параметров распределения *t* и *p* соответственно.
