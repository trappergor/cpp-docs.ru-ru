...
Класс шаблона описывает распределение, которое формирует значения указанного пользователем целочисленного типа или типа **int** Если тип не указан, распределенные в соответствии с биномиальное распределение дискретной функции вероятности. В следующей таблице представлены ссылки на статьи об отдельных членах.
...
Члены свойств `t()` и `p()` возвращают текущие хранимые распределение значений параметров *t* и *p* соответственно.
...
Первый конструктор создает объект, хранимые *p* значение *p* и которого хранимые *t* значение *t*.
