...
*Ключ* тип данных элемента для сохранения в наборе.

*Признаки* тип, предоставляющий объект функции, который может сравнить два значения элементов как ключи сортировки для определения их относительного порядка в наборе. Этот аргумент является необязательным, и в качестве значения по умолчанию используется бинарный предикат **less** *\<Key>*.
...
*Распределитель* тип, представляющий сохраненный объект распределителя, инкапсулирующий сведения о выделении и освобождении памяти. Этот аргумент является необязательным, и значение по умолчанию — **распределителя***\<ключа>.*
...
`allocator_type` является синонимом для параметра-шаблона [распределителя](../standard-library/set-class.md).
...
Если возвращаемое значение `begin` назначается `const_iterator`, элементы в объекте набора невозможно изменить. Если возвращаемое значение `begin` назначается `iterator`, элементы в объекте набора можно изменить.
...
*ключ* ключ элементов для сопоставления из набора.
...
*ключ* ключ-аргумент для сравнения с ключом сортировки элемента из набора в котором выполняется поиск.
...
*Ключ* значение ключа удаляемых элементов.
...
*ключ* значение ключа для сравнения с ключом сортировки элемента из набора в котором выполняется поиск.
...
Функция-член возвращает итератор, который ссылается на элемент в наборе, ключ которого эквивалентно значению аргумента *ключ* согласно двоичному предикату, применяющему упорядочение на основе на отношения «меньше».
...
|*Where*|Место начала поиска правильной точки вставки. (Если этой точки непосредственно предшествует *где*, вставка может происходить в амортизированном константном времени вместо логарифмического времени.)|
|*ValTy*|Параметр шаблона, определяющий тип аргумента, набор можно использовать для создания элемента из [value_type](../standard-library/map-class.md#value_type)и точно пересылает *Val* как аргумент.|
...
Функции-члены одного элемента, (1) и (2) возвращают [пары](../standard-library/pair-structure.md) которого **bool** компонент является значение true, если вставка была выполнена и значение false, если набор уже содержал элемент с эквивалентным значением упорядочение. Компонент итератора пары возвращаемых значений указывает на вставленный элемент, если **bool** компонент равно true, или на существующий элемент Если **bool** компонента имеет значение false.
...
Обратите внимание, что оба `key_compare` и [value_compare](#value_compare) являются синонимами для параметра-шаблона `Traits`. Оба типа предоставляются для классов set и multiset, где они идентичны, для совместимости с классами map и multimap, где они различаются.
...
Обратите внимание, что оба `key_type` и [value_type](#value_type) являются синонимами для параметра-шаблона `Key`. Оба типа предоставляются для классов set и multiset, где они идентичны, для совместимости с классами map и multimap, где они различаются.
...
Первая версия `operator=` использует [ссылки lvalue](../cpp/lvalue-reference-declarator-amp.md) для *правой*, из которого копируются элементы *правой* к этому `set`.

Вторая версия использует [ссылку rvalue](../cpp/rvalue-reference-declarator-amp-amp.md) для right. Она перемещает элементы из *правой* к этому `set`.
...
Все конструкторы хранят объект функции типа `Traits` , используемый для установления порядка ключей набора и его можно получить, вызвав [key_comp](#key_comp).

Первые три конструктора указывают пустой начальный набор, второй указывает тип функции сравнения (`comp`) для использования при установлении порядка элементов, а третий явно указывает тип распределителя (`al`) быть использовать. Ключевое слово **explicit** подавляет некоторые виды автоматического преобразования типов.
...
*правом* аргумент задан, предоставляющий элементы для обмена местами с набором целей.
...
`iterator` Или `const_iterator` , расположение элемента в наборе с ключом больше ключа-аргумента, или адресует положение после последнего элемента в наборе, если соответствие найдено для ключа.
