...
*Где*\
Место начала поиска правильной точки вставки. (Если этой точки непосредственно предшествует *где*, вставка может происходить в амортизированном константном времени вместо логарифмического времени.)
...
*ValTy*\
Параметр шаблона, определяющий тип аргумента, набор можно использовать для создания элемента из [value_type](../standard-library/map-class.md#value_type)и точно пересылает *Val* как аргумент.
...
*Al*\
Класс распределителя памяти для данного объекта-набора, по умолчанию — `Allocator`.
...
**bool-оператор**(**const Key &** `_xVal`, **const Key &** `_yVal`);
