...
`Right` ОБЪЕКТ `unique_ptr`.

`Nptr` `rvalue` Типа `std::nullptr_t`.

`Ptr` ОБЪЕКТ `pointer`.

`Deleter` Объект `deleter` функции, привязанной к `unique_ptr`.
...
Ресурс освобождается путем обращения к хранимому объекту `deleter` типа `Del`, которому известны принципы выделения ресурсов для конкретного объекта `unique_ptr`. Значение по умолчанию `deleter` `default_delete<T>` предполагается, что ресурс указывает `ptr` выделяется с помощью `new`, и его можно освободить путем вызова `delete _Ptr`. (Частичная специализация `unique_ptr<T[]>`управляет объектами массива, выделенными при помощи `new[]`, и обладает объектом `deleter` `default_delete<T[]>` по умолчанию для обращения к функции delete[] `ptr`.)
...
`right` Объект `unique_ptr` для замены указателей.
