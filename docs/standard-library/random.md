...
Вот несколько советов, которые \<нужно иметь в виду при использовании случайных>:
...
Есть много вариантов на \<выбор в случайном заголовке>, и любой из `rand()`них предпочтительнее устаревшей функции C Runtime. Для получения информации о `rand()` том, что случилось с и как \<случайные> устраняет эти недостатки, смотрите это [видео](https://go.microsoft.com/fwlink/p/?linkid=397615).
...
Шаблоны[двигателей](#eng)

### <a name="random-number-distributions"></a><a name="distributions"></a> Распределения случайных чисел

В следующих разделах перечисляется \<дистрибутивы, представленные в зазаголовке случайных>. Распределения — это механизмы постобработки, которые обычно используют результаты РГСЧ в качестве входа и распределяют выходные данные с помощью заданной функции плотности статистической вероятности. Дополнительные сведения см. в разделе [Механизмы и распределения](#engdist).
...
Распределение[случайных чиней](#distributions)

#### <a name="bernoulli-distributions"></a>Распределения Бернулли

|||
|-|-|
|[класс bernoulli_distribution](../standard-library/bernoulli-distribution-class.md)|Производит Бернулли распределения **bool** значения.|
...
|[класс fisher_f_distribution](../standard-library/fisher-f-distribution-class.md)|Производит F-дистрибутив (также известный как F-распределение Snedecor или распределение Fisher-Snedecor) реальных (плавающих точек) значений.|
...
|[normal_distribution класс](../standard-library/normal-distribution-class.md)|Формирует нормальное (Гауссово) распределение вещественных значений (с плавающей запятой).|
...
|[extreme_value_distribution класс](../standard-library/extreme-value-distribution-class.md)|Формирует распределение экстремальных вещественных значений (с плавающей запятой).|
...
|||
|-|-|
|[discrete_distribution класс](../standard-library/discrete-distribution-class.md)|Формирует дискретное распределение целых чисел.|
...
В этом разделе перечислены общие \<функции утилиты, предоставляемые в зазаголовке случайных>.
...
В этом разделе перечислены \<операторы, представленные в случайном заголовке>.
...
## <a name="engines-and-distributions"></a><a name="engdist"></a>Двигатели и дистрибутивы

Ссылайтесь на следующие разделы для получения \<информации о каждой из этих категорий шаблонов классов, определенных в случайных>. Обе категории шаблонов классов используют тип в качестве аргумента и используют общие имена параметров шаблона для описания свойств типа, которые разрешены в качестве фактического типа аргумента, следующим образом:

- `IntType`указывает на **короткий,** **Int**, **длинный,** **длинный,** **неподписанный короткий,** **неподписанный int,** **неподписанный длинный,** или **неподписанный длинный.**

- `UIntType`указывает на **неподписанный короткий,** **неподписанный int,** **неподписанный длинный,** или **неподписанный длинный.**

- `RealType`указывает на **поплавок,** **двойной,** или **длинный двойной**.
...
*Двигатель* — это шаблон класса или класса, экземпляры которого (генераторы) выступают в качестве источника случайных чисел, равномерно распределенных между минимальным и максимальным значением. *Адаптер механизма* предоставляет последовательность значений с различной стохастичностью, беря значения, сформированные другим механизмом случайных чисел, и применяя к ним определенный алгоритм.

Каждый механизм и адаптер механизма содержат следующие члены.

- `typedef`— тип, который возвращается `operator()`генератором. `numeric-type` `result_type` `numeric-type` передается как параметр шаблона при создании экземпляра.
...
- `template <class Seq> void seed(Seq& q)`семена генератора с помощью [seed_seq](../standard-library/seed-seq-class.md)`Seq`.
...
- `void discard(unsigned long long count)`эффективно `operator()` `count` вызывает время и отбрасывает каждое значение.
...
[Распределение случайных чисел](#distributions) — это шаблон класса или класса, экземпляры которого превращают поток равномерно распределенных случайных чисел, полученных из движка, в поток случайных чисел, которые имеют определенное распределение. У каждого распределения есть следующие члены.

- `typedef`— это тип, который возвращается дистрибутивом. `operator()` `numeric-type` `result_type` `numeric-type` передается как параметр шаблона при создании экземпляра.
...
- `typedef`пакет параметров, опционально `operator()` передаваемых и используемых вместо сохраненных параметров для генерации его значения возврата. `unspecified-type` `param_type`
...
- `typedef``distribution-type` , который является типом его `distribution_type`распределения.
...
Хотя согласно стандарту ISO для C++ функция `random_device` не обязана быть криптографически безопасной, в Visual Studio она реализована как криптобезопасная. (Термин "криптографически безопасный" не подразумевает каких-то гарантий, но обозначает минимальный уровень энтропии, а значит, и уровень предсказуемости данного алгоритма рандомизации. Для получения дополнительной информации, см. статью [Википедии Cryptographically безопасный псевдослучайный генератор чисел](https://go.microsoft.com/fwlink/p/?linkid=398017).) Поскольку стандарт ISO C' не требует этого, `random_device` другие платформы могут реализовываться как простой псевдослучайный генератор чисел (не криптографически безопасный) и могут быть пригодны только в качестве источника семян для другого генератора. При использовании `random_device` в коде для разных платформ изучите документацию по этим платформам.
