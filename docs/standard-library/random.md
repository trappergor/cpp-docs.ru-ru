...
> [!NOTE]
> В \<библиотеке случайных > используется инструкция "#include < initializer_list >".
...
Ниже приведены некоторые рекомендации, которые следует учитывать при использовании \<случайных >.
...
Существует множество вариантов выбора из \<заголовка Random >, и любой из них предпочтительнее устаревшей функции `rand()`времени выполнения C. Сведения о том, что не так `rand()` , а \<также о том, как случайная > устраняет эти недостатки, см. в [этом видео](https://go.microsoft.com/fwlink/p/?linkid=397615).
...
1. **Длина периода**: Количество итераций, необходимое для повторения последовательности создаваемых чисел. Чем период длиннее, тем лучше.

2. **Производительность**: Как быстро создаются числа и сколько памяти занимает. Чем меньше, тем лучше.

3. **Качество**: Как близко к истинным случайным числам создается последовательность. Часто это называется "*стохастичностью*".

В следующих разделах перечислены однородные генераторы случайных чисел (ргсч), \<предоставляемые в заголовке Random >.
...
В следующих разделах перечислены дистрибутивы, \<предоставленные в заголовке Random >. Распределения — это механизмы постобработки, которые обычно используют результаты РГСЧ в качестве входа и распределяют выходные данные с помощью заданной функции плотности статистической вероятности. Дополнительные сведения см. в разделе [Механизмы и распределения](#engdist).
...
|||
|-|-|
|[Класс bernoulli_distribution](../standard-library/bernoulli-distribution-class.md)|Создает распределение Бернулли значений типа **bool** .|
...
В этом разделе перечислены общие служебные функции, предоставляемые в \<заголовке Random >.
...
В этом разделе перечислены операторы, \<указанные в заголовке Random >.
...
Сведения о каждой из категорий класса шаблона, определенных в \<случайных >, см. в следующих разделах. Обе категории принимают тип в качестве аргумента и используют общие имена параметров шаблона для описания свойств типа, которые можно использовать как тип фактического аргумента, как показано далее.

- `IntType`Указывает **короткий**, **целочисленный**, **длинный**, длинное целое число **без знака**, беззнаковое целое число без **знака или**длинное целое число без знака.

- `UIntType`Указывает недлинное **короткое**, беззнаковое **целое**число без **знака или**длинное целое число без знака.

- `RealType`Указывает тип **float**, **Double**или **long double**.
