...
Ниже приведены некоторые рекомендации, которые следует учитывать при использовании > \<random.
...
Существует множество вариантов выбора из заголовка \<random >, и любой из них предпочтительнее устаревшей функции времени выполнения C `rand()`. Сведения о неправильной работе с `rand()` и о том, как \<random > устраняет эти недостатки, см. в [этом видео](https://go.microsoft.com/fwlink/p/?linkid=397615).
...
В следующих разделах перечислены однородные генераторы случайных чисел (Ргсч), предоставленные в заголовке > \<random.
...
Сведения о каждой из этих категорий шаблонов классов, определенных в \<random >, см. в следующих разделах. Обе эти категории шаблонов классов принимают тип в качестве аргумента и используют имена общих параметров шаблона для описания свойств типа, которые разрешены в качестве фактического типа аргумента, следующим образом:

- `IntType` указывает на **короткие**, **целочисленные** **, длинные, длинные** **длинные** **,** беззнаковые, беззнаковые **int**, **неподписанные**длинные и **беззнаковые**длинные.

- `UIntType` указывает **недлинное** **короткое**, неподписанное **целое**число **без знака или**длинное целое число без знака.

- `RealType` обозначает тип **float**, **Double**или **long double**.
