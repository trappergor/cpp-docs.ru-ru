...
|**Функции**||
|[compare_exchange_strong](#compare_exchange_strong)|Выполняет операцию *atomic_compare_and_exchange* для **этого** и возвращает результат.|
|[compare_exchange_weak](#compare_exchange_weak)|Выполняет операцию *weak_atomic_compare_and_exchange* для **этого** и возвращает результат.|
...
|[is_lock_free](#is_lock_free)|Указывает, являются ли атомарные операции с **этой** *блокировкой свободными*. Атомарный тип является *неблокирующим*, если никакие атомарные операции с этим типом не используют блокировки.|
...
Тип *Ty* должен быть *простым копированием*. То есть при использовании [memcpy](../c-runtime-library/reference/memcpy-wmemcpy.md) для копирования байтов необходимо создать допустимый объект *Ty* , который сравнивается с исходным объектом. Функции члена [compare_exchange_weak](#compare_exchange_weak) и [compare_exchange_strong](#compare_exchange_strong) используют [memcmp](../c-runtime-library/reference/memcmp-wmemcmp.md) , чтобы определить, равны ли два значения *Ty* . Эти функции не будут использовать определенную `operator==` *Ty*. Функции `atomic` элементов, используемые `memcpy` для копирования значений типа *Ty*.
...
Для каждого целочисленного типа, кроме **bool**, существует специализация. Каждая специализация предоставляет широкий набор методов для атомарных арифметических и логических операций.

||||
|-|-|-|
|**Атомарный\<> char**|**Атомарный\<знак > char**|**>\<Atomic неподписанный char**|
|**Атомарный\<char16_t >**|**Атомарный\<char32_t >**|**Атомарный\<> wchar_t**|
|**Атомарный\<короткий >**|**неатомарные\<короткие > без знака**|**атомарное\<> int**|
|**атомарное\<> int без знака**|**Атомарная\<длинная >**|**Атомарная\<неподписанная длинная >**|
|**атомарные\<длительные >**|**атомарные\<беззнаковые > Long**|

Специализации для целочисленных типов являются производными от соответствующих типов `atomic_integral`. Например, **атомарное\<> int без знака** является производным от `atomic_uint`.
...
## <a name="atomic"></a>Атомарная:: Atomic
...
Объекты, являющиеся экземплярами атомарных\<*Ty*>, могут быть инициализированы только конструктором, который принимает аргумент типа *Ty* , а не с помощью агрегатной инициализации. Однако объекты atomic_integral можно инициализировать только с помощью агрегатной инициализации.
...
## <a name="op_ty"></a>Атомарный:: operator *Ty*

Оператор для типа, указанного в шаблоне, атомарный\<*Ty*>. Извлекает сохраненное значение в  **\*этом**.
...
## <a name="op_eq"></a>Атомарный:: operator =
...
## <a name="op_inc"></a>атомарные:: operator + +
...
## <a name="op_add_eq"></a>атомарные:: operator + =
...
## <a name="op_dec"></a>Атомарный:: operator--
...
## <a name="op_sub_eq"></a>Атомарный:: operator-=
...
## <a name="op_and_eq"></a>Атомарный:: operator & =
...
## <a name="op_or_eq"></a>Атомарный::&#124;operator =
...
Этот оператор выполняет операцию чтения и изменения записи, чтобы заменить сохраненное значение  **\*этого** параметра на побитовое или для *значения* и текущее значение, хранящееся в  **\*этом**объекте, в пределах ограничений `memory_order_seq_cst`ограничения [memory_order](atomic-enums.md) .

## <a name="op_xor_eq"></a>Атомарный:: operator ^ =
...
Этот оператор выполняет операцию чтения-изменения-записи, чтобы заменить сохраненное значение  **\*этого** параметра на побитовое исключающее или для *значения* , а также текущее значение, хранящееся в  **\*этом**объекте, в пределах ограничений `memory_order_seq_cst` ограничения [memory_order](atomic-enums.md) .
...
**Логическое** значение, указывающее результат сравнения значений.
...
Перегрузки, не имеющие второй `memory_order` , используют неявный *Order2* , основанный на значении *Order1*. Если *Order1* имеет `memory_order_acq_rel`значение  , Order2 `memory_order_acquire`имеет значение. Если *Order1* имеет `memory_order_release`значение  , Order2 `memory_order_relaxed`имеет значение. Во всех остальных случаях *Order2* равно *Order1*.

Для перегрузок, принимающих `memory_order` два параметра, значение *Order2* не должно быть `memory_order_release` или `memory_order_acq_rel`, а также не должно быть надежным, чем значение *Order1*.
...
Слабая атомарная операция сравнения и Exchange выполняет обмен, если сравниваемые значения равны. Если значения не равны, операция не гарантирует выполнение Exchange.

Перегрузки, не имеющие второй `memory_order` , используют неявный *Order2* , основанный на значении *Order1*. Если *Order1* имеет `memory_order_acq_rel`значение  , Order2 `memory_order_acquire`имеет значение. Если *Order1* имеет `memory_order_release`значение  , Order2 `memory_order_relaxed`имеет значение. Во всех остальных случаях *Order2* равно *Order1*.

Для перегрузок, принимающих `memory_order` два параметра, значение *Order2* не должно быть `memory_order_release` или `memory_order_acq_rel`, а также не должно быть надежным, чем значение *Order1*.

## <a name="exchange"></a>атомарные:: Exchange
...
Метод выполняет операцию чтения-изменения и записи для атомарного добавления *значения* к сохраненному значению в  **\*этом**и применяет ограничения памяти, заданные по порядку.  `fetch_add`
...
Метод выполняет операцию чтения-изменения-записи, чтобы заменить сохраненное  **\*значение этого** параметра на побитовое и для *значения* , а текущее значение, хранящееся в  **\*этом**объекте, в памяти. `fetch_and` ограничения, заданные по *порядку*.
...
Метод выполняет операцию чтения-изменения-записи, чтобы заменить сохраненное  **\*значение этого** параметра на побитовое или для *значения* , а текущее значение, хранящееся в  **\*этом**объекте, в памяти. `fetch_or` ограничения, заданные по *порядку*.
...
Метод выполняет операцию чтения-изменения и записи для атомарного вычитания *значения* из сохраненного  **\*значения в рамках**ограничений памяти, заданных по порядку.  `fetch_sub`
...
Метод выполняет операцию чтения-изменения-записи, чтобы заменить сохраненное  **\*значение этого** параметра на побитовое исключающее или для *значения* , а также текущее значение, хранящееся в  **\*этом**элементе, и применяет свойство `fetch_xor` ограничения памяти, заданные по *порядку*.
...
Атомарный тип — Блокировка без блокировки, если ни одна атомарная операция с этим типом не использует блокировки.

## <a name="load"></a>Атомарный:: Load
