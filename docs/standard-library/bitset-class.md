...
*N*\
Указывает количество битов в объекте битовом массиве с ненулевым целым числом типа `size_t`, которые должны быть известны во время компиляции.
...
|||
|-|-|
|[all](#all)|Проверяет все биты в этом `bitset`, чтобы определить, заданы ли для них **значение true**.|
|[любой](#any)|Функция-член проверяет, равен ли какой-либо бит в последовательности 1.|
...
|[to_ullong](#to_ullong)|Возвращает сумму значений битов в `bitset` как **беззнаковое**длинное.|
|[to_ulong](#to_ulong)|Преобразует объект `bitset` в **беззнаковое значение типа Long** , которое будет формировать последовательность битов, содержащихся при использовании для инициализации `bitset`.|
...
|[operator&#91;&#93;](#op_at)|Возвращает ссылку на бит в указанной позиции в `bitset`, если `bitset` может быть изменен; в противном случае возвращается значение бита в этой позиции.|
...
### <a name="bitset"></a>битовом массиве
...
- Второй конструктор создает объект класса `bitset\<N>` и инициализирует биты, используя однозначный **длинный параметр Long без знака** .
...
Вторая функция-член создает исключение [out_of_range](../standard-library/out-of-range-class.md) , если указанная в качестве параметра координата превышает размер *n* для **битовом массиве\<** *n* **>** , бит которого был инвертирован.
...
Измененная Целевая битовом массиве, полученная в результате побитовой операции `AND` с битовом массиве, указанным в качестве параметра.

#### <a name="remarks"></a>Remarks

Два бита, объединенные оператором `AND`, возвращают **значение true** , если каждый бит имеет значение true; в противном случае их сочетание возвращает **значение false**.

Битовые массивы должен иметь одинаковый размер для комбинированного побитового оператора с оператором `AND` в функции оператора члена.
...
### <a name="op_lshift_eq"></a>&lt;&lt;оператора =
...
### <a name="op_rshift_eq"></a>&gt;&gt;оператора =
...
Класс `reference` существует только как вспомогательный класс для битового массива `operator[]`. Класс member описывает объект, который может получать доступ к отдельному биту в битовом массиве. Пусть *b* — это объект типа **bool**, *x* и *y* объектов типа **битовом массиве\<** *N* **>** , а в таком объекте допустимы позиции *i* и *j* . Обозначение *x [i]* ссылается на бит в позиции *i* в битовом массиве *x*. Функции — члены класса `reference` предоставляют по порядку следующие операции.

|Операция|Определение|
|---------------|----------------|
|*x*[*i*] = *b*|Сохраняет **логическое** значение *b* в битовой позиции *i* в битовом массиве *x*.|
...
Строковый объект класса `basic_string`, где каждый бит в битовом массиве имеет соответствующий символ 1, и символ 0, если бит не задан.
...
Возвращает **Неподписанное длинное** длинное значение, которое содержит те же биты, что и содержимое объекта битовом массиве.
...
Возвращает сумму битовых значений, которые находятся в битовой последовательности, в виде **беззнакового длинного числа**. Если это значение не **подписано длинным длинным** значением, то те же самые биты будут созданы повторно, если он используется для инициализации битовом массиве.

#### <a name="exceptions"></a>Исключения

Создает объект [overflow_error](overflow-error-class.md) , если любой бит в битовой последовательности имеет битовое значение, которое не может быть представлено в виде значения типа **без знака Long**.

#### <a name="remarks"></a>Remarks

Возвращает сумму битовых значений, которые находятся в битовой последовательности, в виде **беззнакового длинного числа**.
...
Функция-член создает объект [overflow_error](overflow-error-class.md) , если какой-либо бит в битовой последовательности имеет битовое значение, которое не может быть представлено как значение типа **без знака Long**.
