...
Шаблон класса описывает объект, хранящий N объектов типов `T1`, `T2`,... `TN`, соответственно, где `0 <= N <= Nmax`. Экстент экземпляра кортежа `tuple<T1, T2, ..., TN>` является числом `N` его аргументов шаблона. Индекс аргумента шаблона `Ti` и соответствующего сохраненного значения этого типа `i - 1`. Таким образом, при нумерации типов от 1 до N в этой документации соответствующие значения индекса находятся в диапазоне от 0 до N – 1.
...
Первые два оператора члена применяют элементы *right* к соответствующим элементам `*this`. Третий оператор-член назначает `right.first` элементу с индексом 0 в `*this` и `right.second` — элементу с индексом 1. Все три оператора-члена возвращают значение `*this`.
