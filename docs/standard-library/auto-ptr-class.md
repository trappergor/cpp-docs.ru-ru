...
Класс шаблона описывает смарт-указатель, называемый `auto_ptr`, для выделенного объекта. Указатель должен быть либо null, либо обозначать объект, выделенный **новым**. `auto_ptr` передает право владения, если сохраненное значение присваивается другому объекту (сохраненное значение заменяется после перемещения с пустым указателем). Деструктор `auto_ptr<Type>` удаляет выделенный объект. `auto_ptr<Type>` гарантирует, что выделенный объект автоматически удаляется при выходе точки управления за пределы блока даже с использованием созданного исключения. Не следует создавать два объекта `auto_ptr<Type>`, владеющих одним объектом.
...
Третий конструктор ведет себя так же, как и второй, за исключением того, `right`что он хранит. `ref`. `release`в `myptr`, где `ref` — это ссылка, хранящаяся в `right`.
...
Оператор выбора возвращает [get](#get)`( )`, поэтому выражение *ap*-> **member** ведет себя так же, как ( *ap*. **get**( ) )-> **member**, где *ap* — это объект класса `auto_ptr`\< **Type**>. Следовательно, сохраненный указатель не должен иметь значение NULL и `Type` должен быть классом, структурой или типом объединения `member` с элементом.

#### <a name="example"></a>Пример

Пример использования функции члена см. в разделе [auto_ptr](#auto_ptr).

### <a name="op_auto_ptr_lt_other_gt"></a>Оператор auto_ptr&lt;other&gt;
...
### <a name="op_auto_ptr_ref_lt_other_gt"></a>Оператор auto_ptr_ref&lt;other&gt;
...
Функция-член вычисляет выражение `delete myptr`, но только в том случае, если значение `myptr` сохраненного указателя изменяется в результате вызова функции. Затем она заменяет сохраненный указатель на `ptr`.
...
*указатель*\
Указатель, заданный для замены сохраненного `myptr`указателя.
