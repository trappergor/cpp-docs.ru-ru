...
Можно явно использовать итераторы с помощью членов и глобальных функций, таких `begin()` как `end()` **++** **--** и, и для перемещения вперед или назад. Можно также использовать итераторы неявно с циклом Range-for или (для некоторых типов итераторов) оператором  **\[индекса]** .
...
- **Вывод**. *Итератор* `X` вывода может выполнять итерацию по последовательности с помощью оператора **++** и может записывать элемент только один раз с помощью __\*__ оператора.

- **Ввод**. *Входной итератор* `X` может перебирать последовательность с помощью оператора + + и может считывать элемент любое количество **&ast;** раз с помощью оператора. Вы можете сравнить итераторы ввода с помощью **++** операторов и **! =** . После выполнения приращения любой копии итератора ввода ни одну из других копий нельзя будет безопасно сравнивать, разыменовывать и выполнять приращение.

- **Прямой**. *Прямой итератор* `X` может перебрать последовательность с помощью оператора + + и может считывать любой элемент или записывать неконстантные элементы любое количество **&ast;** раз с помощью оператора. Доступ к элементам элементов можно получить с помощью **->** оператора и сравнить прямые итераторы с **==** помощью операторов и **! =** . Вы можете сделать несколько копий однонаправленного итератора, каждая из которых может быть разыменована и для нее может быть выполнено независимое приращение. Прямой итератор, инициализируемый без ссылки на какой-либо контейнер, называется *прямым итератором NULL*. Пустые однонаправленные итераторы всегда равны.
...
Теперь C++ Корпорация Майкрософт предлагает проверенные итераторы и отладочные итераторы, чтобы не перезаписывать границы контейнера. Дополнительные сведения см. в разделах [Проверяемые итераторы](../standard-library/checked-iterators.md) и [Поддержка отладочных итераторов](../standard-library/debug-iterator-support.md).
