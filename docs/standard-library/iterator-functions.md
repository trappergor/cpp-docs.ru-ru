...
`InIt` Итератор, который должен увеличен, должен удовлетворять требованиям для итератора ввода.

`Off` Целочисленный тип, который можно преобразовать в тип отличия итератора, указывающий число приращений позиции итератора — дополнительными.
...
`_Cont` Контейнер, в которой будет выполняться назад Вставка.
...
`cont` Контейнер или initializer_list.
...
`first` Первый итератор — определить, расстояние которых от второй.

`last` Второй итератор — определить, расстояние от первого.
...
`_Cont` Объект-контейнер которого передней испытывает элемент вставки.
...
`_Cont` Контейнер, к которому должны быть добавлены новые элементы.

`_Where` Итератор, расположение точки вставки.
...
`Ptr` Указатель на массив назначения.

`Size` Размер массива назначения.

`Index` Необязательный индекс в массиве.
...
`_It` Итератор, хранящихся в новый итератор перемещения.
...
`first` Текущее положение.

`_Off` Количество раз для перечисления.
