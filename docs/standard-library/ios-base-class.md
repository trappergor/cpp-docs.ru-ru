...
Функция члена сохраняет *_Loc* в объекте языкового стандарта, а затем сообщает о событии `imbue_event`обратного вызова и. Она возвращает предыдущее сохраненное значение.
...
Функция-член возвращает ссылку на элемент _ *idx* расширяемого массива с элементами типа pointer **void** . Все элементы эффективно присутствуют и изначально хранят нулевой указатель. Возвращаемая ссылка является недействительной после следующего вызова `pword` для объекта, после изменения объекта с помощью вызова **basic_ios::** [copyfmt](../standard-library/basic-ios-class.md#copyfmt) или после уничтожения объекта.
...
Функция-член помещает пару `{pfn, idx}` в сохраненный [стек обратного вызова](../standard-library/ios-base-class.md)стека обратного вызова. Когда сообщается о событии обратного вызова **EV** , функции вызываются в обратном порядке в реестре по `(*pfn)(ev, *this, idx)`выражению.
...
Тип является перечислимым типом, описывающим объект, который может хранить режим поиска, используемый в качестве аргумента для функций-членов нескольких классов iostream. Ниже перечислены значения различных флагов.
...
Первая функция-член фактически вызывает [Флаги](#flags)( &#124;  *\_*  *\_* Флаги Маски) (устанавливает выбранные биты), а затем возвращает предыдущие флаги формата. Вторая функция-член фактически вызывает `flags(_Mask & fmtfl, flags & ~_Mask)` (заменяя выбранные биты в маске), а затем возвращает предыдущие флаги формата.
...
Статическая функция-член сохраняет `stdio` флаг синхронизации, который изначально имеет **значение true**. При **значении true**этот флаг обеспечивает правильную синхронизацию операций в одном файле между функциями [iostream](../standard-library/iostreams-conventions.md) и определенными в C++ стандартной библиотеке. В противном случае синхронизация может быть гарантирована или не гарантируется, но производительность может быть улучшена. Функция сохраняет *_Sync* в `stdio` флаге синхронизации и возвращает свое предыдущее сохраненное значение. Его можно вызвать надежно только перед выполнением любых операций со стандартными потоками.
...
Функция-член фактически вызывает [Флаги](#flags)(`~` *_Mask* **& флаги**) (Очистить выбранные биты).
...
Возвращаемое значение можно использовать как аргумент уникального индекса при вызове функций-членов [иворд](#iword) или [пворд](#pword).
