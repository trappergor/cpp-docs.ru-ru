...
Первая функция-член фактически вызывает [](#flags)флаги  *\_* (Флаги Маски &#124;  *\_* ) (устанавливает выбранные биты), а затем возвращает предыдущие флаги формата. Вторая функция-член фактически вызывает `flags(_Mask & fmtfl, flags & ~_Mask)` (заменяя выбранные биты в маске), а затем возвращает предыдущие флаги формата.
...
Функция-член фактически вызывает [](#flags)флаги`~`( *_Mask* **& флаги**) (Очистить выбранные биты).
