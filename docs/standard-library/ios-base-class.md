...
description: Справочник по API для класса Microsoft Visual C++ стандартной библиотеки `ios_base`
ms.date: 10/23/2020
f1_keywords:
...
|Имя|Описание|
|-|-|
|[`event_callback`](#event_callback)|Описывает функцию, переданную в [`register_call`](#register_callback) .|
...
|[`Init`](#init)|Создает стандартные `iostream` объекты при создании.|
...
|[`sync_with_stdio`](#sync_with_stdio)|Гарантирует, что `iostream` и операции с библиотекой времени выполнения C выполняются в том порядке, в котором они отображаются в исходном коде.|
...
|Имя|Описание|
|-|-|
|[`operator=`](#op_eq)|Оператор присваивания для объектов `ios_base`.|
...
Тип является перечислимым типом, описывающим объект, который может хранить событие обратного вызова, используемое в качестве аргумента функции, зарегистрированной в [`register_callback`](#register_callback) . Различные значения события:

- `copyfmt_event`, чтобы найти обратный вызов, который выполняется около конца вызова метода [`copyfmt`](../standard-library/basic-ios-class.md#copyfmt) , непосредственно перед копированием [маски исключения](../standard-library/ios-base-class.md) .

- `erase_event`, чтобы найти обратный вызов, который выполняется в начале вызова метода [`copyfmt`](../standard-library/basic-ios-class.md#copyfmt) или в начале вызова деструктора для `*this` .

- `imbue_event`, чтобы найти обратный вызов, который выполняется в конце вызова метода [`imbue`](#imbue) , непосредственно перед возвратом функции.

### <a name="example"></a>Пример

[`register_callback`](#register_callback)Пример см. в разделе.

## <a name="event_callback"></a><a name="event_callback"></a> `event_callback`

Описывает функцию, переданную в [`register_call`](#register_callback) .
...
Тип описывает указатель на функцию, которая может быть зарегистрирована в [`register_callback`](#register_callback) . Функции этого типа не должны вызывать исключение.

### <a name="example"></a>Пример

См [`register_call`](#register_callback) . пример, в котором используется `event_callback` .
...
[`ios_base::fmtflags`](#fmtflags)Список флагов см. в разделе.

Первая функция-член возвращает сохраненные флажки формата. Вторая функция – член сохраняет *`fmtfl`* в флагах формата и возвращает свое предыдущее сохраненное значение.
...
[`basic_ios::imbue`](../standard-library/basic-ios-class.md#imbue)Пример см. в разделе.
...
Функция-член возвращает ссылку на элемент *idx* расширяемого массива с элементами типа `long` . Все элементы эффективно присутствуют и изначально хранят нулевое значение. Возвращенная ссылка является недопустимой после следующего вызова для `iword` объекта, после того, как объект изменен вызовом метода `basic_ios::` [`copyfmt`](../standard-library/basic-ios-class.md#copyfmt) или после уничтожения объекта.

Если *`idx`* является отрицательным или если для элемента недоступно уникальное хранилище, функция вызывает [`setstate`](../standard-library/basic-ios-class.md#setstate) `(badbit)` и возвращает ссылку, которая может быть неуникальной.
...
| `binary`| Открыть в двоичном режиме. ( [`fopen`](../c-runtime-library/reference/fopen-wfopen.md) Описание двоичного режима см. в разделе).|
...
Оператор копирует сохраненные сведения о форматировании, создавая новую копию для любых расширяемых массивов. Затем оно возвращает значение `*this`. Обратите внимание, что стек обратных вызовов не копируется.
...
Функция-член помещает пару в `{pfn, idx}` сохраненный [стек обратного вызова](../standard-library/ios-base-class.md)стека обратного вызова. Когда сообщается о событии обратного вызова **EV** , функции вызываются в обратном порядке в реестре по выражению `(*pfn)(ev, *this, idx)` .
...
- `beg`, чтобы найти (изменить текущее положение чтения или записи) относительно начала последовательности (массива, потока или файла).
...
Первая функция-член фактически вызывает [`flags(_Mask | _Flags)`](#flags) (устанавливает выбранные биты), а затем возвращает предыдущие флаги формата. Вторая функция-член фактически вызывает `flags(_Mask & fmtfl, flags & ~_Mask)` (заменяя выбранные биты в маске), а затем возвращает предыдущие флаги формата.
...
Функция-член фактически вызывает [`flags(~_Mask & flags)`](#flags) (очищает выбранные биты).

### <a name="example"></a>Пример

[`ios_base::setf`](#setf)Пример использования см. в разделе `unsetf` .
