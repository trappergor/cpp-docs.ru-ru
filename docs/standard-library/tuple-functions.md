...
`Types` Последовательность типов, заявленных в кортеж, в порядке объявления.

`T` Тип возвращаемого элемента.

`Tuple` Кортеж std::tuple содержит произвольное количество элементов.
...
`TN` Тип параметра n-й функции.

`tN` Значение параметра n-й функции.
...
`TN` Базовый тип элемента n-й кортежа.
