...
*Chartype*\
Тип данных одиночного символа, сохраняемого в строке. Стандартная библиотека СЗ предоставляет специализации этого шаблона класса, со [строкой](../standard-library/string-typedefs.md#string) определения типа для элементов типа `char16_t` **char,** [wstring,](../standard-library/string-typedefs.md#wstring)для **wchar_t,** [u16string](../standard-library/string-typedefs.md#u16string) для , и [u32string](../standard-library/string-typedefs.md#u32string) для `char32_t`.

*Черты*\
Различные важные `CharType` свойства элементов в basic_string специализации описаны классом. `Traits` Значение по умолчанию — `char_traits`< `CharType`>.

*Распределителя*\
Тип, представляющий сохраненный объект распределителя, содержащий сведения о распределении строки и отмене распределения памяти. Значение по умолчанию является> **разносчика.** <  `CharType`
...
|[Итератор](#iterator)|Тип, предоставляющий итератор произвольного доступа, который может читать или изменять любой элемент в строке.|
|[npos](#npos)|Неподписанное интегральное значение, инициированное до -1, которое указывает либо "не найдено" или "все оставшиеся символы", когда функция поиска выходит из строя.|
|[указатель](#pointer)|Тип, предоставляющий указатель на символьный элемент в строке или символьном массиве.|
|[Ссылки](#reference)|Тип, предоставляющий ссылку на элемент в строке.|
...
|[Назначить](#assign)|Назначает значения новых символов содержимому строки.|
|[В](#at)|Возвращает ссылку на  элемент в заданном положении в строке.|
|[Назад](#back)||
|[Начать](#begin)|Возвращает итератор, адресующий первый элемент в строке.|
|[C_str](#c_str)|Преобразует содержимое строки в С-строку, оканчивающуюся нулевым символом.|
|[Емкость](#capacity)|Возвращает наибольшее количество элементов, которые можно хранить в строке без увеличения выделения памяти этой строки.|
...
|[Ясно](#clear)|Удаляет все элементы строки.|
|[Сравнить](#compare)|Сравнивает строку с указанной строкой, чтобы определить их равенство или установить, что одна из строк лексикографически меньше второй.|
|[Копировать](#copy)|Копирует не более указанного количества символов из индексированного положения в исходной строке в массив символов назначения. Не рекомендуется. Используйте вместо нее функцию [basic_string::_Copy_s](#copy_s).|
...
|[данные](#data)|Преобразует содержимое строки в массив символов.|
|[Пустой](#empty)|Проверяет, содержит ли строка символы.|
...
|[Найти](#find)|Поиск по строке в прямом направлении до первого вхождения подстроки, совпадающей с заданной последовательностью символов.|
|[find_first_not_of](#find_first_not_of)|Поиск по строке для первого символа, который не является элементом указанной строки.|
|[find_first_of](#find_first_of)|Выполняет поиск первого символа в строке, совпадающего с любым элементом заданной строки.|
|[find_last_not_of](#find_last_not_of)|Поиск по строке для последнего символа, который не является элементом указанной строки.|
|[find_last_of](#find_last_of)|Выполняет поиск последнего символа в строке, совпадающего с любым элементом заданной строки.|
|[Перед](#front)|Возвращает ссылку на первый элемент в строке.|
|[get_allocator](#get_allocator)|Возвращает копию объекта объекта `allocator`, использованного для создания строки.|
|[Вставить](#insert)|Вставляет элемент или количество элементов или диапазон элементов в указанное положение в строке.|
|[длина](#length)|Возвращает текущее количество элементов в строке.|
...
|[Заменить](#replace)|Заменяет элементы в указанном положении в строке на указанные символы или символы, скопированные из других диапазонов, обычных строк или С-строк.|
|[Заповедник](#reserve)|Устанавливает вместимость строки не меньше указанного числа.|
|[Изменения размера](#resize)|Указывает новый размер строки, добавляя или удаляя элементы по мере необходимости.|
...
|[Размер](#size)|Возвращает текущее количество элементов в строке.|
|[substr](#substr)|Копирует из указанного положения в строке подстроку, содержащую по крайней мере несколько символов.|
|[Своп](#swap)|Меняет местами содержимое двух строк.|

### <a name="operators"></a>Операторы

|Оператор|Описание|
|-|-|
|[оператора](#op_add_eq)|Присоединяет символы к строке.|
|[оператора](#op_eq)|Назначает значения новых символов содержимому строки.|
...
Персонажи могут быть придещены к строке, `append` используя [функции оператора](#op_add_eq) или функции участника или [push_back](#push_back). `operator+=`прикладивает значения одного аргумента, `append` в то время как функция члена нескольких аргументов позволяет указывать определенную часть строки для добавления.
...
## <a name="basic_stringassign"></a><a name="assign"></a>basic_string::назначить
...
*Рассчитывать*\
Количество символов, которые будут назначены, из строки исходного кода.
...
Строкам можно присваивать новые символьные значения. Новое значение может быть строкой, C-строкой или одним символом. [Оператор](#op_eq) может быть использован, если новое значение может быть описано одним параметром; в противном `assign`случае функция члена, которая имеет несколько параметров, может быть использована для определения того, какая часть строки должна быть назначена целевой строке.
...
Первый элемент строки имеет индекс нуля, а следующие элементы индексируются последовательно положительными целыми числами, так что строка длины *n* имеет *n-й*элемент, индексированный числом *n -* 1.

[Оператор-член&#91;&#93;](#op_at) быстрее, чем `at` функция члена для предоставления чтения и записи доступа к элементам строки.

Участник `operator[]` не проверяет, является ли индекс, пройденое `at` в качестве параметра, допустимом, но функция участника выполняет свою функцию, и поэтому ее следует использовать, если достоверность не определена. Недействительный индекс, который представляет собой индекс, менее нулевой или превышающей или `at` равный размеру строки, передаваемый функции элемента, выбрасывает исключение [out_of_range класса.](../standard-library/out-of-range-class.md) Недопустимый индекс, переданный в `operator[]`, приводит к непредсказуемому поведению, однако индекс, равный длине строки, допустим для константных строк, и при передаче этого индекса оператор возвращает символ NULL.
...
Все конструкторы сохраняют [basic_string::allocator_type](#allocator_type) и инициализируют управляемую последовательность. Объектом распределителя является аргумент `al`, если он существует. Для конструктора копий, `right.`это [basic_string::get_allocator](#get_allocator)`()`. В противном случае, `Alloc()`allocator .

Управляемая последовательность инициализируется для копии последовательности операндов, указанной оставшимися операндами. Конструктор без последовательности операндов указывает пустую начальную управляемую последовательность. Если `InputIterator` в конструкторе шаблонов есть целый тип, `first,  last` последовательность операнд `(size_type) first, (value_type) last`ведет себя так же, как и в .
...
## <a name="basic_stringbegin"></a><a name="begin"></a>basic_string::начало
...
Указатель на версию вызываемой С-строки.  Значение указателя не действителен после вызова неконсистной функции, включая деструктор, в basic_string класс на объекте.

### <a name="remarks"></a>Remarks

Объекты строки типа, относящиеся к шаблону класса basic_string\<> символа, не обязательно являются недействительными. Символ NULL '\0' используется как специальный символ в C-строке для пометки ее конца, но не имеет специального значения в объекте типа String и может быть частью строки так же, как любой другой символ. Существует автоматическое преобразование из **const char** <strong>\*</strong> в строки, но класс строки не предусматривает автоматического преобразования из строк C-стиля в объекты типа **\<basic_string>. **

Верная строка C-стиля не должна быть изменена, так как это может привести к аннулированию указателя на строку или удалить, так как строка имеет ограниченный срок службы и принадлежит строке класса.
...
## <a name="basic_stringcapacity"></a><a name="capacity"></a>basic_string::Емкость
...
Возвращается значение `cend` не должно быть отнесено к отнесению.

## <a name="basic_stringclear"></a><a name="clear"></a>basic_string::ясно
...
Функции `compare` участника сравнивают все или часть параметра и строки operand в зависимости от того, какие из них используются.
...
Для `string`типа, это эквивалентно `char*`.
...
Тип является синонимом `allocator_type::const_reference`. Для `type`строки, это эквивалентно `char&`const .
...
*рассчитывать* Количество символов, которые будут скопированы, в лучшем случае, из строки исходного кода.
...
Нулевой символ не приложен к концу копии.
...
## <a name="basic_stringdata"></a><a name="data"></a>basic_string::dата

Преобразует содержимое строки в нулевой массив символов.
...
Указатель на первый элемент нулевого массива, содержащего содержимое строки. Для пустой строки указатель указывает на один `value_type()`нулевой символ, равный.

### <a name="remarks"></a>Remarks

Указатель возвращается `data` по очкам `[data(), data() + size()]`в допустимом диапазоне. Каждый элемент в диапазоне соответствует текущим данным в строке. То есть, для *n* каждого действительного `data() + n == addressof(operator[](n))`смещения n в диапазоне, .

Если вы измените содержимое строки, возвращенной `data` **конст-перегрузкой,** поведение не определено. Вы также получаете неопределенное поведение, если символ нулевой терминала изменен на любое другое значение. Ответный указатель может быть признан недействительным, если нессылка на строку передается стандартной функции библиотеки. Он также может быть признан недействительным при вызове к функции нексонст-члена. Звонки к `at` `back`членам, `end` `front`, `rbegin` `begin` `rend`, `operator[]` , , , и не недействительным указателем.

До си-11 `data` не гарантировала, что возвращенная строка была нутера. Начиная с C-11, `data` и `c_str` оба возвращаютнули строку с нулевым завершением, и фактически являются одинаковыми.

Неконст перегрузка является новой в СЗ17. Чтобы использовать его, укажите опцию **/std:c'17** или **/std:c'latest** компилятор.
...
## <a name="basic_stringdifference_type"></a><a name="difference_type"></a>basic_string::difference
...
Для `string`типа, это эквивалентно `ptrdiff_t`.
...
## <a name="basic_stringempty"></a><a name="empty"></a>basic_string::пустой
...
`end`часто используется для проверки того, достиг ли итерататор конца своей строки. Возвращается значение `end` не должно быть отнесено к отнесению.

Если возвращаемое значение `end` присваивается `const_iterator`, то объект string не может быть изменен. Если значение `end` возврата назначено объекту `iterator`строки, может быть изменено.
...
*Рассчитывать*\
Количество элементов, которые будут удалены, если есть столько в диапазоне строки, начиная с *смещения.*
...
Функция третьего члена возвращает ** \*это.**
...
## <a name="basic_stringfind"></a><a name="find"></a>basic_string::найти
...
Поиск строки для первого символа, который не является элементом указанной строки.
...
Поиск по строке для последнего символа, который не является элементом указанной строки.
...
## <a name="basic_stringfront"></a><a name="front"></a>basic_string::фронт
...
## <a name="basic_stringinsert"></a><a name="insert"></a>basic_string::вставка
...
Тип `iterator` может использоваться для изменения значения символа и используется для итерировать через строку в переднем направлении.

### <a name="example"></a>Пример

См. пример объявления и использования `iterator` см. в разделе [begin](#begin).

## <a name="basic_stringlength"></a><a name="length"></a>basic_string::длина
...
Неподписанное интегральное значение, инициированное до -1, которое указывает либо "не найдено" или "все оставшиеся символы", когда функция поиска выходит из строя.
...
При `npos` проверке значения возврата оно может не сработать, если значение возврата не имеет [типа size_type](#size_type) а не не является ни **int,** ни **unsigned.**

### <a name="example"></a>Пример

См. пример объявления и использования `npos`[find](#find).

## <a name="basic_stringoperator"></a><a name="op_add_eq"></a>basic_string::оператор
...
## <a name="basic_stringoperator"></a><a name="op_eq"></a>basic_string:оператор
...
## <a name="basic_stringoperator"></a><a name="op_at"></a>basic_string:оператор
...
`operator[]`не проверяет, является ли индекс, пройденое `at` в качестве параметра, действительным, но функция участника делает и поэтому должна быть использована в действительности не уверен. Недействительный индекс (индекс меньше нуля или больше или равен размеру строки), передаваемый функции `at` участника, выбрасывает исключение out_of_range [класса.](../standard-library/out-of-range-class.md) Недопустимый индекс, переданный в `operator[]`, приводит к непредсказуемому поведению, однако индекс, равный длине строки, допустим для константных строк, и при передаче этого индекса оператор возвращает символ NULL.

Возвращенная ссылка может стать недействительной в результате перераспределений или модификаций строк, не являющихся **константными**.

При компиляции с [ \_помощью\_ITERATOR\_DEBUG LEVEL,](../standard-library/iterator-debug-level.md) установленного на 1 или 2, произойдет ошибка выполнения при попытке получить доступ к элементу за пределами строки. Дополнительные сведения см. в разделе [Проверенные итераторы](../standard-library/checked-iterators.md).
...
Для `string`типа, это эквивалентно **char**<strong>\*</strong>.
...
## <a name="basic_stringpush_back"></a><a name="push_back"></a>basic_string::pуш-обратно
...
Функция члена эффективно вызывает [вставку](#insert) [(конец,](#end) *char_value).*
...
## <a name="basic_stringreference"></a><a name="reference"></a>basic_string:ссылка
...
Тип `reference` может быть использован для изменения значения элемента.

Тип является синонимом `allocator_type::reference`.

Для `string`типа, это эквивалентно `chr&`.
...
Возвращается значение `rend` не должно быть отнесено к отнесению.
...
## <a name="basic_stringreplace"></a><a name="replace"></a>basic_string::замена
...
*Рассчитывать*\
Количество *раз, когда char_value* скопированв в строку операнд.
...
## <a name="basic_stringreserve"></a><a name="reserve"></a>basic_string::резерв
...
Концепция емкости для объектов типа string такая же, как и для объектов типа vector. В отличие от `reserve` вектор, функция члена может быть вызвана для сокращения емкости объекта. Это запрос без привязки и может как произойти, так и не произойти. Поскольку значение по умолчанию для параметра равно нулю, вызов `reserve` является необязательным запросом на уменьшение емкости строки, чтобы соответствовать числу символов, находящихся в настоящее время в строке. Емкость никогда не уменьшается ниже текущего количества символов.
...
## <a name="basic_stringsize"></a><a name="size"></a>basic_string:размер
...
Для `string`типа, это эквивалентно `size_t`.
...
Тип является синонимом второго параметра `Traits`шаблона.

Для `string`типа, это эквивалентно **char_traits\<>char. **
...
Это эквивалентно `traits_type::char_type` и эквивалентно **символ** для `string`объектов типа .
