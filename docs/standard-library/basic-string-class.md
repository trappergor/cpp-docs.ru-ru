...
*Признаки* \
Различные важные свойства элементов `CharType` в специализации basic_string описаны классом `Traits`. Значение по умолчанию — `char_traits`< `CharType`>.
...
|[begin](#cbegin)|Возвращает константный итератор, адресующий первый элемент в строке.|
...
Если возвращаемое значение должно быть проверено на `npos` значение, оно может не работать, если возвращаемое значение не имеет тип [size_type](#size_type) и не является ни **int** , ни **неподписанным**.
