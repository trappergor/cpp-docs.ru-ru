...
Тип `iterator` может использоваться для изменения значения символа и используется для итерации по строке в прямом направлении.
...
Целочисленное значение без знака равным -1, что означает «не найден» или «все остальные символы» при сбое функции поиска.
...
Если возвращаемое значение будет проверяться для `npos` значение, он может не работать, если возвращаемое значение имеет тип [size_type](#size_type) и является либо **int** или **без знака**.
...
`operator[]` не проверяет допустимость индекса, переданного в качестве параметра, а функция-член `at` и поэтому должен использоваться в действительности не обязательно. Недопустимый индекс (индекс меньше нуля либо больше или равен размеру строки) передается в функцию-член `at` вызывает [класс out_of_range](../standard-library/out-of-range-class.md) исключение. Недопустимый индекс, переданный в `operator[]`, приводит к непредсказуемому поведению, однако индекс, равный длине строки, допустим для константных строк, и при передаче этого индекса оператор возвращает символ NULL.
...
Тип `reference` может использоваться для изменения значения элемента.
...
Концепция емкости для объектов типа string такая же, как и для объектов типа vector. В отличие от объекта vector, функция-член `reserve` может вызываться для уменьшения емкости объекта. Это запрос без привязки и может как произойти, так и не произойти. По умолчанию значение параметра равно нулю, вызов `reserve` — это запрос без привязки для уменьшения емкости строки, чтобы она соответствовала числу символов в настоящее время в строке. Емкость никогда не уменьшается ниже текущего количества символов.
