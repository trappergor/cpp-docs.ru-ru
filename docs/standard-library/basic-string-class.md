...
|[copy](#copy)|Копирует не более указанного количества символов из индексированного положения в исходной строке в массив символов назначения. Устаревшее. Используйте вместо нее функцию [basic_string::_Copy_s](#copy_s).|
...
Символы могут быть добавлены к строке с помощью [оператора + =](#op_add_eq) или функций-членов `append` или [push_back](#push_back). `operator+=` добавляет значения с одним аргументом, а функция-член `append` с несколькими аргументами позволяет указать определенную часть строки для добавления.
...
[Оператор&#91; ](#op_at) Member работает быстрее, чем функция-член, `at` для предоставления доступа на чтение и запись к элементам строки.

`operator[]` члена не проверяет, является ли индекс, переданный как параметр, допустимым, но функция-член `at` имеет значение, поэтому следует использовать, если действительность не является обязательной. Недопустимый индекс, который является индексом меньше нуля или больше или равен размеру строки, переданному функции-члену `at` создает исключение [класса out_of_range](../standard-library/out-of-range-class.md) . Недопустимый индекс, переданный в `operator[]`, приводит к непредсказуемому поведению, однако индекс, равный длине строки, допустим для константных строк, и при передаче этого индекса оператор возвращает символ NULL.
...
Функции члена `compare` сравнивают либо все, либо часть строк параметров и операндов в зависимости от используемого объекта.
...
Если изменить содержимое строки, возвращенной **константной** перегрузкой `data`, поведение не определено. Вы также получаете неопределенное поведение, если символ терминала NULL меняется на любое другое значение. Возвращаемый указатель может быть недействительным, если неконстантная ссылка на строку передается в стандартную функцию библиотеки. Его также можно сделать недействительным при вызове функции-члена, не являющейся константой. Вызовы членов `at`, `back`, `begin`, `end`, `front`, `rbegin`, `rend`и `operator[]` не делают указатель недействительным. 

До C++ 11 `data` не гарантировала, что возвращаемая строка была прервана нулем. Начиная с C++ 11, `data` и `c_str` возвращают строку, завершающуюся нулем, и фактически одинаковы.
...
Тип `iterator` может использоваться для изменения значения символа и используется для прохода по строке в прямом направлении.
...
Если возвращаемое значение должно быть проверено на `npos` значение, оно может не работать, если только возвращаемое значение не имеет тип [size_type](#size_type) и не является ни **int** , ни **неподписанным**.
...
`operator[]` не проверяет допустимость индекса, переданного в качестве параметра, но функция-член `at` имеет значение, поэтому ее следует использовать в действительности не обязательно. Недопустимый индекс (индекс меньше нуля или больше или равен размеру строки), переданный функции-члену `at` выдает исключение [класса out_of_range](../standard-library/out-of-range-class.md) . Недопустимый индекс, переданный в `operator[]`, приводит к непредсказуемому поведению, однако индекс, равный длине строки, допустим для константных строк, и при передаче этого индекса оператор возвращает символ NULL.
...
Тип является синонимом для второго параметра шаблона `Traits`.
