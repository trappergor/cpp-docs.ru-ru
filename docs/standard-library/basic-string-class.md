...
Символы могут быть добавлены к строке с помощью [оператора + =](#op_add_eq) или функций `append` -членов или [push_back](#push_back). `operator+=`Добавляет значения с одним аргументом, а функция-член `append` с несколькими аргументами позволяет указать определенную часть строки для добавления.
...
Строкам можно присваивать новые символьные значения. Новое значение может быть строкой, C-строкой или одним символом. [Оператор =](#op_eq) может использоваться, если новое значение можно описать с помощью одного параметра; в противном случае `assign`можно использовать функцию-член с несколькими параметрами, чтобы указать, какая часть строки должна быть назначена целевой строке.
...
[Оператор&#91; ](#op_at) -член работает быстрее, чем функция `at` -член, предоставляющая доступ на чтение и запись к элементам строки.

Член `operator[]` не проверяет, является ли индекс, переданный в качестве параметра, допустимым, но `at` функция-член имеет значение, поэтому следует использовать, если действительность не является обязательной. Недопустимый индекс, который является индексом меньше нуля или больше или равен размеру строки, переданный функции `at` -члену, вызывает исключение [класса out_of_range](../standard-library/out-of-range-class.md) . Недопустимый индекс, переданный в `operator[]`, приводит к непредсказуемому поведению, однако индекс, равный длине строки, допустим для константных строк, и при передаче этого индекса оператор возвращает символ NULL.
...
Функции `compare` элементов сравнивают либо все, либо часть строк параметров и операндов в зависимости от используемого объекта.
...
Для типа `string`он `char*`эквивалентен.
...
Объекты типа string, принадлежащие классу basic_string \<char> шаблона C++, не обязательно завершаются символом NULL. Возвращаемый тип для `data` не является допустимой строкой C-String, так как не добавляется символ null. Символ NULL '\0' используется как специальный символ в C-строке для пометки ее конца, но не имеет специального значения в объекте типа String и может быть частью объекта string так же, как любой другой символ.

Существует автоматическое преобразование из **const char** <strong>\*</strong> в строки, но класс String не обеспечивает автоматическое преобразование из строк в стиле C в объекты типа  **\<basic_string char >** .
...
Для типа `string`он `ptrdiff_t`эквивалентен.
...
Если возвращаемое значение `npos` должно быть проверено на значение, оно может не работать, если возвращаемое значение не имеет тип [size_type](#size_type) и не является ни **int** , ни неподписанным.
...
Для типа `string`он `chr&`эквивалентен.
...
*расчета*\
Количество раз, когда *_Ch* копируется в строку операнда.
...
Для типа `string`он `size_t`эквивалентен.
