...
Если возвращаемое значение `find` присваивается `const_iterator`, то объект мультинабора нельзя изменить. Если возвращаемое значение `find` назначается `iterator`, то объект мультинабора можно изменить
...
Обратите внимание [](#key_compare) , что `value_compare` оба key_compare и являются синонимами для `Compare`параметра шаблона. Оба типа предоставляются для классов set и multiset, где они идентичны, для совместимости с классами map и multimap, где они различаются.
...
Обратите внимание [](#key_type) , что `value_type` оба key_type и являются синонимами для `Key`параметра шаблона. Оба типа предоставляются для классов set и multiset, где они идентичны, для совместимости с классами map и multimap, где они различаются.
