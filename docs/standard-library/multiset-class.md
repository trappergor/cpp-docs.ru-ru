...
|[const_iterator](#const_iterator)|Typedef для двунаправленного итератора, который может читать элемент **const** в `multiset`.|
...
|[const_reverse_iterator](#const_reverse_iterator)|Typedef для двунаправленного итератора, который может читать любой элемент **const** в `multiset`.|
...
Если возвращаемое значение `find` назначено `const_iterator`, то объект мультинабора изменить нельзя. Если возвращаемое значение `find` назначается `iterator`, то объект мультинабора можно изменить.
...
[value_type контейнера](../standard-library/map-class.md#value_type) — это определение типа, которое принадлежит контейнеру, а для набора `multiset<V>::value_type` — это `const V`.
...
**bool-оператор**( **ключ const &** *x*, **ключ const &** *y*);
...
`iterator` или `const_iterator`, который обращается к расположению элемента в Мультинаборе с ключом, который равен ключу аргумента или больше него, либо обращается к расположению, следующему за последним элементом в Мультинаборе, если совпадение для ключа не найдено.
...
`operator=` копирует или перемещает элементы *непосредственно* в этот `multiset`в зависимости от используемого ссылочного типа (lvalue или rvalue). Элементы, которые находились в этом `multiset` перед выполнением `operator=`, игнорируются.
