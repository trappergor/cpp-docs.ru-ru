...
*Ключ* тип данных элемента для сохранения в мультинабор.
...
`Allocator` Тип, представляющий сохраненный объект распределителя, инкапсулирующий сведения о выделении и освобождении памяти мультинабора. Значение по умолчанию — **распределителя***\<ключа >.*
...
`key` Ключ элементов для сопоставления из multiset.
...
`key` Аргумент ключ для сравнения с ключом сортировки элемента из мультинабора, по которому выполняется поиск.
...
`key` Значение ключа, к которым сравнивается ключ сортировки элемента из мультинабора, по которому выполняется поиск.
...
`right` Аргумент мультинабора, предоставляющий элементы, которые будут заменены целевой мультинабора.
...
[\<задать > членов](http://msdn.microsoft.com/en-us/0c2d57c0-173f-4204-b579-c5f06aad8b95)<br/>
