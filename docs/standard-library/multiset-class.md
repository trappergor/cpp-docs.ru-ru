...
*Сравнить*\
Тип, предоставляющий объект функции, который может сравнивать два значения элемента как ключи сортировки, чтобы определить их относительный порядок в мультинаборе. Двоичный предикат **меньше**\<ключевой> является значением по умолчанию.
...
- Шаблон класса, поскольку предоставляемая им функциональность является универсальной и независит от конкретного типа данных, содержащихся в качестве элементов. Тип данных, подлежащий использованию, вместо этого определяется как параметры в шаблоне класса вместе с функцией и распределителем сравнения.
...
|-|-|
|[Мультинабора](#multiset)|Создает `multiset`, который является пустым или копией части или целого заданного `multiset`.|
...
|[const_iterator](#const_iterator)|Typedef для двунаправленного итератора, который может `multiset`прочитать **конст элемент** в .|
|[const_pointer](#const_pointer)|Typedef для указателя на **конст** `multiset`элемент в .|
|[const_reference](#const_reference)|Typedef для ссылки на **конст** `multiset` элемент, хранящийся в для чтения и выполнения **конст-операций.**|
|[const_reverse_iterator](#const_reverse_iterator)|Typedef для двунаправленного итератора, который может `multiset`прочитать любой **конст элемент** в .|
...
|[Итератор](#iterator)|Typedef для двунаправленного итератора, который может считывать или изменять любой элемент в `multiset`.|
...
|[указатель](#pointer)|Typedef для указателя на элемент в `multiset`.|
|[Ссылки](#reference)|Typedef для ссылки на элемент, сохраненный в `multiset`.|
...
|-|-|
|[Начать](#begin)|Возвращает итератор, указывающий на первый элемент в `multiset`.|
...
|[Ясно](#clear)|Стирает все элементы в `multiset`.|
...
|[Пустой](#empty)|Проверяет, пуст ли `multiset`.|
...
|[Найти](#find)|Возвращает итератор, указывающий на первое расположение элемента в `multiset`, имеющего ключ, равный указанному ключу.|
|[get_allocator](#get_allocator)|Возвращает копию объекта `allocator`, который используется для создания `multiset`.|
|[Вставить](#insert)|Вставляет элемент или диапазон элементов в `multiset`.|
...
|[Размер](#size)|Возвращает количество элементов в `multiset`.|
|[Своп](#swap)|Выполняет обмен элементами между двумя объектами `multiset`.|
...
|Оператор|Описание|
|-|-|
|[оператора](#op_eq)|Заменяет элементы `multiset` копией другого `multiset`.|
...
## <a name="multisetallocator_type"></a><a name="allocator_type"></a>мультисет::allocator_type
...
## <a name="multisetbegin"></a><a name="begin"></a>мультисет::начало
...
## <a name="multisetcbegin"></a><a name="cbegin"></a>мультисет::cbegin
...
## <a name="multisetcend"></a><a name="cend"></a>мультисет::cend
...
## <a name="multisetclear"></a><a name="clear"></a>multiset::ясно
...
## <a name="multisetconst_iterator"></a><a name="const_iterator"></a>мультисет::const_iterator
...
## <a name="multisetconst_pointer"></a><a name="const_pointer"></a>мультисет::const_pointer
...
## <a name="multisetconst_reference"></a><a name="const_reference"></a>мультисет::const_reference
...
## <a name="multisetconst_reverse_iterator"></a><a name="const_reverse_iterator"></a>мультисет::const_reverse_iterator
...
## <a name="multisetcount"></a><a name="count"></a>мультисет:количество
...
## <a name="multisetcrbegin"></a><a name="crbegin"></a>мультисет:crbegin
...
## <a name="multisetcrend"></a><a name="crend"></a>мультисет:crend
...
## <a name="multisetdifference_type"></a><a name="difference_type"></a>мультисет::difference-type
...
## <a name="multisetemplace"></a><a name="emplace"></a>мультисет:место
...
## <a name="multisetemplace_hint"></a><a name="emplace_hint"></a>мультисет::emplace_hint
...
## <a name="multisetempty"></a><a name="empty"></a>мультисет:пустой
...
## <a name="multisetend"></a><a name="end"></a>мультисет::конец
...
## <a name="multisetequal_range"></a><a name="equal_range"></a>мультисет::equal_range
...
## <a name="multiseterase"></a><a name="erase"></a>мультисет::стереть
...
## <a name="multisetfind"></a><a name="find"></a>multiset::найти
...
Функция члена возвращает итератор, который относится к элементу в мультисете, ключ которого эквивалентен *ключу* аргумента под двоичным предикатом, который индуцирует упорядочение, основанное на менее сопоставимой связи.

Если значение `find` возврата назначено объекту `const_iterator`с несколькими наборами, не может быть изменено. Если значение `find` возврата назначено объекту `iterator`мультисета, может быть изменено
...
## <a name="multisetget_allocator"></a><a name="get_allocator"></a>мультисет::get_allocator
...
## <a name="multisetinsert"></a><a name="insert"></a>мультисет:вставить
...
|*Где*|Место начала поиска правильной точки вставки. (Если эта точка сразу же предшествует *Где,* вставка может произойти в амортизированное постоянное время вместо логарифмического времени.)|
|*ValTy*|Параметры шаблона, который определяет тип аргумента, который мультисет может использовать для построения элемента [value_type,](../standard-library/map-class.md#value_type)и идеально перенаправляет *Val* в качестве аргумента.|
...
## <a name="multisetiterator"></a><a name="iterator"></a>мультисет:Итератор
...
См пример [для начала](#begin) для примера того, как объявить и использовать . `iterator`

## <a name="multisetkey_comp"></a><a name="key_comp"></a>мультисет::key_comp
...
## <a name="multisetkey_compare"></a><a name="key_compare"></a>мультисет::key_compare
...
## <a name="multisetkey_type"></a><a name="key_type"></a>мультисет::key_type
...
## <a name="multisetlower_bound"></a><a name="lower_bound"></a>мультисет::lower_bound
...
Или `iterator` `const_iterator` то, что адреса расположение элемента в мультисете, что с ключом, который равен или больше, чем ключ аргумента, или что адреса местоположения, преуспевающего последний элемент в мультисете, если не найдено совпадение для ключа.
...
## <a name="multisetmax_size"></a><a name="max_size"></a>мультисет::max_size
...
## <a name="multisetmultiset"></a><a name="multiset"></a>мультисет:мультисет
...
|Параметр|Описание|
|-|-|
|*Аль*|Класс распределителя памяти, который будет использоваться для этого объекта-мультинабора. Значение по умолчанию — `Allocator`.|
|*Комп*|Функция сравнения типа `const Compare`, которая используется для упорядочивания элементов в мультинаборе. Значение по умолчанию — `Compare`.|
...
Первые три конструктора указывают пустой первоначальный мультисет, второй - тип функции сравнения *(Comp),* который будет использоваться при определении порядка элементов, а третий явно указывает тип разлляции (*Al*) для использования. Ключевое слово **explicit** подавляет некоторые виды автоматического преобразования типов.

Четвертый конструктор указывает копию многосетового *права.*

Пятый конструктор определяет копию мультисета, двигаясь *вправо.*
...
## <a name="multisetoperator"></a><a name="op_eq"></a>мультисет:оператор
...
`operator=`копирует или перемещает *right* элементы `multiset`в право в это, в зависимости от типов ссылки (lvalue или rvalue) используется. Элементы, которые находились в этом `multiset` перед выполнением `operator=`, игнорируются.
...
## <a name="multisetpointer"></a><a name="pointer"></a>мультисет::pоинтер
...
## <a name="multisetrbegin"></a><a name="rbegin"></a>мультисет::rbegin
...
## <a name="multisetrend"></a><a name="rend"></a>мультисет::ренд
...
## <a name="multisetreverse_iterator"></a><a name="reverse_iterator"></a>мультисет::reverse_iterator
...
## <a name="multisetsize"></a><a name="size"></a>мультисет:размер
...
## <a name="multisetsize_type"></a><a name="size_type"></a>мультисет::size_type
...
## <a name="multisetswap"></a><a name="swap"></a>мультисет::своп
...
## <a name="multisetupper_bound"></a><a name="upper_bound"></a>мультисет::upper_bound
...
## <a name="multisetvalue_comp"></a><a name="value_comp"></a>мультисет::value_comp
...
**bool оператор** **(конст-ключ&** `_xVal`, **Const Key&); ** `_yVal`
...
## <a name="multisetvalue_compare"></a><a name="value_compare"></a>мультисет::value_compare
...
Обратите внимание, `value_compare` что как [key_compare, так](#key_compare) `Compare`и являются синонимами параметра шаблона. Оба типа предоставляются для классов set и multiset, где они идентичны, для совместимости с классами map и multimap, где они различаются.
...
## <a name="multisetvalue_type"></a><a name="value_type"></a>мультисет::value_type
...
Обратите внимание, что [оба key_type](#key_type) и `value_type` являются `Key`синонимами параметра шаблона. Оба типа предоставляются для классов set и multiset, где они идентичны, для совместимости с классами map и multimap, где они различаются.
