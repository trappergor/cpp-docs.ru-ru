...
|[const_pointer](#const_pointer)|Тип, предоставляющий указатель на **const** элемент `multimap`.|
|[const_reference](#const_reference)|Тип, предоставляющий ссылку на **const** элемент хранится в `multimap` для чтения и выполнения **const** операций.|
|[const_reverse_iterator](#const_reverse_iterator)|Тип, предоставляющий двунаправленный итератор, который может читать любой **const** элемент `multimap`.|
...
|[pointer](#pointer)|Тип, предоставляющий указатель на **const** элемент `multimap`.|
...
|[size_type](#size_type)|Беззнаковый целочисленный тип, предоставляющий указатель на **const** элемент `multimap`.|
...
`const_iterator` Определенный мультиотображением, указывает на объекты [value_type](#value_type), которые имеют тип `pair<const Key, Type>`. Значение ключа доступно через первый член пары, а значение сопоставленного элемента доступно через второй член пары.
...
`const_reverse_iterator` Определенный мультиотображением, указывает на объекты [value_type](#value_type), которые имеют тип `pair<const Key, Type>`. Значение ключа доступно через первый член пары, а значение сопоставленного элемента доступно через второй член пары.

Для разыменования `const_reverse_iterator` *crIter* указывающего на элемент в мультиотображении, используйте **->** оператор.

Чтобы получить значение ключа для элемента, используйте `crIter->first`, что эквивалентно `(*crIter).first`. Для доступа к значению сопоставленных данных для элемента, используйте `crIter->second`, что эквивалентно `(*crIter).first`.
...
`difference_type` — тип, возвращаемый при вычитании или приращении через итераторы контейнера. `difference_type` Обычно используется для представления количества элементов в диапазоне [*первый*, *последнего*) между итераторами `first` и `last`, включая элемент, на который указывает по `first` и диапазон элементов до, но не включая элемент указывает `last`.
...
Если возвращенное значение `find` назначается `const_iterator`, то объект-мультиотображение изменить нельзя. Если возвращаемое значение `find` назначается `iterator`, объект-мультиотображение можно изменить.
...
|*Where*|Место начала поиска правильной точки вставки. (Если этой точки непосредственно предшествует *где*, вставка может происходить в амортизированном константном времени вместо логарифмического времени.)|
|*ValTy*|Параметр шаблона, определяющий тип аргумента, с карты можно использовать для создания элемента из [value_type](../standard-library/map-class.md#value_type)и точно пересылает *Val* как аргумент.|
...
Функция-член диапазоном (5) вставляет последовательность значений элементов в множественное сопоставление, соответствующее каждому элементу, адресованному итератором в диапазоне `[First, Last)`; таким образом, *последнего* не вставляется. Контейнер функции-члена `end()` ссылается на позицию сразу после последнего элемента в контейнере. Например, оператор `m.insert(v.begin(), v.end());` пытается вставить все элементы `v` в `m`.
...
`iterator` Определенный мультиотображением, указывает на объекты [value_type](#value_type), которые имеют тип `pair<const Key, Type>`. Значение ключа доступно через первый член пары, а значение сопоставленного элемента доступно через второй член пары.

Для разыменования `iterator` *Iter* указывающего на элемент в мультиотображении, используйте **->** оператор.
...
Четвертый конструктор указывает копию мультиотображения *справа*.

Пятый конструктор указывает копию мультиотображения путем перемещения *справа*.
...
После удаления всех существующих элементов в `multimap`, `operator=` копирует или перемещает содержимое *правой* в `multimap`.
...
`reverse_iterator` Определенный мультиотображением, указывает на объекты [value_type](#value_type), которые имеют тип `pair<const Key, Type>`. Значение ключа доступно через первый член пары, а значение сопоставленного элемента доступно через второй член пары.

Для разыменования `reverse_iterator` *rIter* указывающего на элемент в мультиотображении, используйте **->** оператор.

Чтобы получить значение ключа для элемента, используйте `rIter->first`, что эквивалентно `(*rIter).first`. Для доступа к значению сопоставленных данных для элемента, используйте `rIter->second`, что эквивалентно `(*rIter).second`.
...
Если возвращенное значение назначается `const_iterator`, то объект-мультиотображение изменить нельзя. Если возвращаемое значение присваивается `iterator`, объект-мультиотображение можно изменить.
...
[\<MAP > члены](https://msdn.microsoft.com/7e8f0bc2-6034-40f6-9d14-76d4cef86308)<br/>
