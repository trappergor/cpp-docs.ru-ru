...
`Key` Тип данных ключа для сохранения в контейнере multimap.

`Type` Тип данных элемента для сохранения в контейнере multimap.

`Traits` Тип, предоставляющий объект функции, который может сравнить два значения элементов как ключи сортировки для определения их относительного порядка в контейнере multimap. Бинарный предикат `less<Key>` является значением по умолчанию.
...
`key` Ключ элементов для сравнения с мультиотображения.
...
`key` Аргумент ключ для сравнения с ключом сортировки элемента из мультикарты, по которому выполняется поиск.
...
`Key` Ключ элементов для удаления.
...
`key` Значение ключа, к которым сравнивается ключ сортировки элемента из мультикарты, по которому выполняется поиск.
