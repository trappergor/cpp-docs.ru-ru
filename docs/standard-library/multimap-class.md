...
Чтобы получить доступ к значению ключа для элемента, используйте `crIter->first`, что `(*crIter).first`эквивалентно. Чтобы получить доступ к значению сопоставленной версии элемента, используйте `crIter->second`, что `(*crIter).first`эквивалентно.
...
`difference_type` — тип, возвращаемый при вычитании или приращении через итераторы контейнера. `first` `last` `first` Обычно используется для представления количества элементов в диапазоне [первый, последний) между итераторами и, включает элемент, на который указывает, и диапазон элементов вплоть до, но `difference_type` не включая элемент, на который указывает `last`.
...
После стирания любых существующих элементов `multimap`в, `operator=` копирует или перемещает `multimap`содержимое *непосредственно* в.
...
Чтобы получить доступ к значению ключа для элемента, используйте `rIter->first`, что `(*rIter).first`эквивалентно. Чтобы получить доступ к значению сопоставленной версии элемента, используйте `rIter->second`, что `(*rIter).second`эквивалентно.
...
Для multimap *m*, если два элемента *E1*(*K1*, *D1*) и *E2*(*априорной оценкой K2*, *D2*) являются `value_type`объектами типа, где *K1* и *априорной оценкой K2* являются ключами типа `key_type` и *D1* и *D2* являются данными типа `mapped_type`, `m.key_comp(k1, k2)`а затем `m.value_comp(e1, e2)` эквивалентны.
