...
В C++ 14 вы можете включить разнородный поиск, указав предикат `std::less<>` или `std::greater<>`, не имеющий параметров типа. Дополнительные сведения см. в статье [Разнородный поиск в ассоциативных контейнерах](../standard-library/stl-containers.md#heterogeneous-lookup-in-associative-containers-c14)
...
|[const_iterator](#const_iterator)|Тип, предоставляющий двунаправленный итератор, который может читать элемент **const** в `multimap`.|
|[const_pointer](#const_pointer)|Тип, предоставляющий указатель на элемент **const** в `multimap`.|
|[const_reference](#const_reference)|Тип, предоставляющий ссылку на элемент **const** , хранящийся в `multimap` для чтения и выполнения операций **const** .|
|[const_reverse_iterator](#const_reverse_iterator)|Тип, предоставляющий двунаправленный итератор, который может читать любой элемент **const** в `multimap`.|
...
|[pointer](#pointer)|Тип, предоставляющий указатель на элемент **const** в `multimap`.|
...
|[size_type](#size_type)|Целочисленный тип без знака, который предоставляет указатель на элемент **const** в `multimap`.|
...
**Константный** итератор с двунаправленным доступом, указывающий на первый элемент диапазона, или расположение непосредственно за концом пустого диапазона (для пустого диапазона, `cbegin() == cend()`).
...
`const_iterator`, определенные multimap, указывают на объекты [value_type](#value_type), которые имеют тип `pair<const Key, Type>`. Значение ключа доступно через первый элемент пары, а значение сопоставленного элемента доступно через второй элемент пары.

Для разыменования *`const_iterator`,* указывающего на элемент в multimap, используйте оператор **->** .
...
`const_reverse_iterator`, определенные multimap, указывают на объекты [value_type](#value_type), которые имеют тип `pair<const Key, Type>`. Значение ключа доступно через первый элемент пары, а значение сопоставленного элемента доступно через второй элемент пары.

Для разыменования `const_reverse_iterator` *критер* , указывающего на элемент в multimap, используйте оператор **->** .

Чтобы получить значение ключа для элемента, используйте `crIter->first`, что эквивалентно `(*crIter).first`. Чтобы получить доступ к значению сопоставленной части элемента, используйте `crIter->second`, что эквивалентно `(*crIter).first`.
...
`iterator`, определенные multimap, указывают на объекты [value_type](#value_type), которые имеют тип `pair<const Key, Type>`. Значение ключа доступно через первый элемент пары, а значение сопоставленного элемента доступно через второй элемент пары.
...
См. пример объявления и использования [ см. в разделе ](#begin)begin`iterator`.
...
|*Соответствовал*|Функция сравнения типа `constTraits` используется для упорядочения элементов в схеме, которая по умолчанию `Traits`.|
...
После удаления существующих элементов в `multimap``operator=` копирует или перемещает содержимое *непосредственно* в `multimap`.
...
`reverse_iterator`, определенные multimap, указывают на объекты [value_type](#value_type), которые имеют тип `pair<const Key, Type>`. Значение ключа доступно через первый элемент пары, а значение сопоставленного элемента доступно через второй элемент пары.

Для разыменования `reverse_iterator`ного *спрайта* , указывающего на элемент в multimap, используйте оператор **->** .

Чтобы получить значение ключа для элемента, используйте `rIter->first`, что эквивалентно `(*rIter).first`. Чтобы получить доступ к значению сопоставленной части элемента, используйте `rIter->second`, что эквивалентно `(*rIter).second`.

### <a name="example"></a>Пример

См. пример объявления и использования [ в примере для ](#rbegin)rbegin`reverse_iterator`.
...
Если возвращенное значение назначается `const_iterator`, то объект-мультиотображение изменить нельзя. Если возвращаемое значение назначается `iterator`, то объект multimap можно изменить.
...
Для multimap *m*, если два элемента *E1*(*K1*, *D1*) и *e2*(*априорной оценкой K2*, *D2*) являются объектами типа `value_type`, где *k1* и *априорной оценкой K2* являются ключами типа `key_type` а *D1* и *D2* являются данными типа `mapped_type`, то `m.value_comp(e1, e2)` эквивалентно `m.key_comp(k1, k2)`.
