...
*Ключ* тип данных ключа для сохранения в мультиотображении.

*Тип* тип данных элемента для сохранения в мультиотображении.

*Признаки* тип, предоставляющий объект функции, который может сравнить два значения элементов как ключи сортировки для определения их относительного порядка в мультиотображении. Бинарный предикат `less<Key>` является значением по умолчанию.
...
*Распределитель* тип, представляющий сохраненный объект распределителя, инкапсулирующий сведения о выделении и освобождении памяти карты. Этот аргумент является необязательным, и значением по умолчанию является `allocator<pair <const Key, Type> >`.
...
*ключ* ключ элементов для сравнения с мультиотображения.
...
*ключ* ключ-аргумент для сравнения с ключом сортировки элемента из мультиотображения в котором выполняется поиск.
...
*Где* положение элемента для удаления.

*Первый* положение первого элемента для удаления.

*Последний* положение после последнего элемента для удаления.

*Ключ* ключа удаляемых элементов.
...
*ключ* значение ключа для сравнения с ключом сортировки элемента из мультиотображения в котором выполняется поиск.
...
`iterator` Определяемый multimap точками объектам [value_type](#value_type), которые имеют тип `pair`*\<***const ключ**, **Тип***>*. Значение ключа доступно через первый член пары, а значение сопоставленного элемента доступно через второй член пары.
...
*правом* мультиотображение, предоставляющее элементы для обмена местами, или мультиотображение, элементы которого должны быть заменены мультиотображения `left`.
...
[\<MAP > члены](http://msdn.microsoft.com/7e8f0bc2-6034-40f6-9d14-76d4cef86308)<br/>
