...
ms.locfileid: "72687628"
---
# <a name="num_put-class"></a>Класс num_put

Шаблон класса, описывающий объект, который может служить в качестве аспекта языкового стандарта для управления преобразованием числовых значений в последовательности типа `CharType`.
...
Целочисленное поле вывода формируется теми же правилами, которые используются функциями печати для создания последовательности элементов **char** в файл. Предполагается, что каждый такой элемент char сопоставлен с эквивалентным элементом типа `CharType` простым сопоставлением «один к одному». Если функция печати дополняет поле пробелами или цифрой 0, то `do_put` вместо этого использует `fill`. Эквивалентная спецификация преобразования печати определяется следующим образом:
...
Заполнение происходит, только если минимальное количество элементов *N*, требуемое для указания поля вывода, меньше, чем **iosbase**. [width](../standard-library/ios-base-class.md#width). Такое заполнение состоит из последовательности из *N*  - **ширины** копий **заливки**. Заполнение выполняется следующим образом:
...
- В противном случае спецификация преобразования будет иметь значение **lg**. Если **iosbase**. **flags**  & **ios_base:: прописные буквы** не равны нулю, `g` заменяется на `G`.
...
ведет себя аналогично третьему, за исключением того, что квалификатор `l` в спецификации преобразования заменяется `L`.
