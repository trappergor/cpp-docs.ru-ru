...
<strong>(C ++ 98/03 только)</strong>  При наследовании класса распределителя вы должны предоставить [повторно привязать](#rebind) структуры, которого `_Other` typedef ссылки на созданный производный класс.
...
### <a name="allocator"></a> Распределитель
...
Первая функция-член эквивалентно **новый** ((`void` \*) `ptr`) **тип** (`val`).
...
### <a name="rebind"></a> повторную привязку
