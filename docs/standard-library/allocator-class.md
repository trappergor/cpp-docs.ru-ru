...
*Тип* тип объекта, для которого выполняется хранения выделенных и освобожденных.
...
*_Hint* указатель const, который может помочь объект распределителя удовлетворить потребности запроса хранилища, найдя адрес объекта выделенной до запроса.
...
`right` Копируемый объект распределителя.
...
`val` Значение, с которым инициализироваться, создаваемый объект.
...
`ptr` Указатель на первый объект освободить из хранилища.

`count` Количество объектов освободить из хранилища.
...
`ptr` Указатель, обозначающий адрес объекта будут уничтожены.
...
`right` Объект распределителя для назначения другого такого объекта.
...
*другие* тип элемента, для которого выделяется память.
