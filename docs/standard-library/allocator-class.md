...
ms.locfileid: "72690086"
---
# <a name="allocator-class"></a>Класс allocator

Шаблон класса описывает объект, который управляет выделением и освобождением памяти для массивов объектов типа `Type`. Объект класса `allocator` является объектом распределителя по умолчанию, указанным в конструкторах для нескольких шаблонов классов контейнеров в C++ стандартной библиотеке.
...
Все контейнеры C++ стандартной библиотеки имеют параметр шаблона, который по умолчанию имеет значение `allocator`. Создание контейнера с пользовательским распределителем дает возможность управлять выделением и освобождением памяти для элементов контейнера.
...
<strong>(Только C++ 98/03)</strong> При наследовании от класса распределителя необходимо предоставить структуру [повторной привязки](#rebind) , в которой `_Other` typedef ссылается на вновь производный класс.
...
Эти `Type`s указать форму, которую должны принимать указатели и ссылки для выделенных элементов. ( [распределитель::p оинтер](#pointer) не обязательно такой же, как `Type*` для всех объектов распределителя, хотя он имеет это очевидное определение для класса `allocator`.)
...
|[size_type](#size_type)|Целочисленный тип без знака, который может представлять длину любой последовательности, которую может выделить объект типа `allocator`.|
...
Тип указателя описывает объект `ptr`, который может обозначать с помощью выражения `*ptr` любой объект const, который может быть выделен объектом типа `allocator`.
...
Первая функция-член эквивалентна **новому** **типу** ((`void` \*) `ptr`) (`val`).
...
Функция-член освобождает хранилище для массива объектов Count типа `Type` начиная с *ptr*, вызывая `operator delete(ptr)`. Указатель *ptr* должен быть возвращен ранее путем вызова метода [allocate](#allocate) для объекта распределителя, который сравнивается с **\*this**, выделяя объект массива такого же размера и типа. `deallocate` никогда не создает исключений.
...
Функция-член уничтожает объект, обозначенный *ptr*, путем вызова деструктора `ptr->`**Тип**:: **~ Type**.
...
Тип целого числа со знаком описывает объект, который может представлять разницу между адресами любых двух элементов последовательности, которые может выделить объект типа `allocator`.
...
Тип указателя описывает объект `ptr`, который может обозначать, через **\*ptr**выражения любой объект, который может быть выделен объектом типа `allocator`.
...
### <a name="rebind"></a>повторно привязать
...
Шаблон класса Member определяет тип other. Его единственная цель — предоставить имя типа **allocator**\<_ **Other**>, если имеется имя типа **allocator**\< **Type**>.
...
Целочисленный тип без знака, который может представлять длину любой последовательности, которую может выделить объект типа `allocator`.
