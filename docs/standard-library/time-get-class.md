...
`InputIterator` Итератор, из которого считываются значения времени.
...
`first` Входной итератор, который указывает на начало последовательности для преобразования.

`last` Входной итератор, который указывает на конец последовательности.

`iosbase` Объект потока.

`state` Поле в iosbase установки соответствующие элементы битовой маски для указания ошибок.

`ptm` Указатель на структуру времени, где будет храниться время.

`fmt` Символ описателя преобразования.

`mod` Необязательный символ модификатора.
...
`first` Входной итератор, указывающий начало последовательности для преобразования.

`last` Входной итератор, указывающий конец последовательности для преобразования.

`iosbase` Формат флаг, который при наборе указывает, что символ валюты является необязательным. в противном случае оно является обязательным.

`state` Устанавливает соответствующие элементы битовой маски для состояния потока, в зависимости от того, успешно ли операции.

`ptm` Указатель, где будет храниться сведения о дате.
...
`iosbase` Не используется.

`state` Выходной параметр, устанавливающий соответствующие элементы битовой маски для состояния потока, в зависимости от того, успешно ли операции.

`ptm` Указатель, где будет храниться сведения месяца.
...
`ptm` Указатель, где будет храниться сведения дня недели.
...
`ptm` Указатель, где будет храниться сведения года.
...
`first` Входной итератор, который указывает, где начинается последовательность для преобразования.

`last` Входной итератор, который указывает на конец последовательности для преобразования.

`iosbase` Поток.

`state` Для состояния потока для указания ошибок задаются соответствующие элементы битовой маски.
...
`fmt_first` Указывает, где запустить директив формата.

`fmt_last` Указывает на конец директив формата.
...
`refs` Целочисленное значение используется для указания типа управление памятью для объекта.
