...
Знак процента (%) в формате, с последующим необязательным модификатором *mod* в наборе [EOQ #], затем следует описатель преобразования *fmt*, заменяет *первый* с значение, возвращенное `do_get(first, last, iosbase, state, ptm, fmt, mod)`. Ошибка преобразования устанавливает `ios_base::failbit` в *состояние* и возвращает.
