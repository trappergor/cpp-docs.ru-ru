...
Побитовое `AND` имеет ту же таблицу истинности, что и логическое `AND`, но применяется к типам данных на уровне отдельных битов. Оператор [operator&&](#op_amp_amp) применяется на уровне элемента, интерпретируя все ненулевые значения как true. Результатом является объект valarray из логических значений. Побитовое `AND` [оператор &](#op_amp), напротив, может привести в объекте valarray значений, отличных от 0 или 1, в зависимости от результата побитовой операции.
...
Логический `ANDoperator&&` применяется на уровне элемента, интерпретируя все ненулевые значения как true и результатом является объект valarray из логических значений. Побитового `AND`, [оператор &,](#op_amp), напротив, может привести в объекте valarray значений, отличных от 0 или 1, в зависимости от результата побитовой операции.
...
Побитовое исключающее `OR` ( **XOR**) имеет следующую семантику: Взять биты *b*1 и *b*2, *b*1 **XOR** *b*2 — **true** Если ровно один из битов имеет значение true; **false** Если оба бита имеют значение false, или если оба бита имеют значение true.
