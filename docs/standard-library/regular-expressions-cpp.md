...
ms.locfileid: "68451381"
---
# <a name="regular-expressions-c"></a>Регулярные выражения (C++)

C++ Стандартная библиотека поддерживает несколько грамматик регулярных выражений. В этом разделе обсуждаются варианты грамматики, доступные при использовании регулярных выражений.
...
- `ECMAScript`: Это наиболее близко к грамматике, используемой в языках JavaScript и .NET.
- `basic`: Основные регулярные выражения POSIX или Бре.
- `extended`: Расширенные регулярные выражения POSIX или ERE.
- `awk`: `extended`Это, но у него есть дополнительные escape-последовательности для непечатаемых символов.
- `grep`: `basic`Это, но также позволяет символам новой строки ("\n") разделять чередования.
- `egrep`: `extended`Это, но также позволяет символам новой строки разделять чередование.
...
- `optimize`: Повышение согласованности за счет возможного увеличения времени создания.
...
- "\*" имеет особое значение во всех случаях, за исключением случаев, когда это первый символ в регулярном выражении или первый символ, следующий за начальным "^" в регулярном выражении, или когда это первый символ в группе захвата или первый символ, следует за начальным значением "^" в группе захвата.
...
|"$$"||"$"|
||"\\&"|"&"|
|"$\`" (знак доллара, после которого стоит Обратная кавычка)||Последовательность символов, предшествующая части последовательности, которая соответствует регулярному выражению (`[match.prefix().first, match.prefix().second)`)|
