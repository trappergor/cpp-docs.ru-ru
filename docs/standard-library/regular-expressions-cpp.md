---
title: "Регулярные выражения (C++) | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "регулярные выражения [TR1]"
  - "регулярные выражения, Visual C++"
  - "регулярные выражения TR1"
  - "Visual C++, регулярные выражения"
...
В этом разделе рассматриваются грамматики подсистем различных регулярных выражений.  
  
## Содержание раздела  
  
-   [Грамматика регулярных выражений](#regexgrammar)  
  
-   [Сводка по грамматике](#grammarsummary)  
  
-   [Семантические сведения](#semanticdetails)  
  
-   [Подбор и поиск](#matchingandsearching)  
  
-   [Флаги формата](#formatflags)  
...
-   *Символ\-шаблон* ".", который соответствует любому символу в целевой последовательности, кроме новой строки.  
 ...
 *Группа захвата* в формате "\( *часть выражения* \)" или "\\\( *часть выражения* \\\)" в `BRE` и `grep`, соответствующая последовательности символов в целевой последовательности, которой соответствует шаблон между разделителями.  
...
-   "\(a\)" соответствует целевой последовательности "a" и связывает группу захвата 1 с частью последовательности "a", но не соответствует целевым последовательностям "B", "b" или "c".  
 ...
 В грамматиках `ECMAScript`, `BRE` и `grep` элемент также может являться *обратной ссылкой* в формате "\\`dd`", где `dd` представляет десятичное значение N, которое соответствует последовательности символов в целевой последовательности, совпадающей с последовательностью символов, соответствующих *группе захвата* N.  Например, "\(a\)\\1" соответствует целевой последовательности "aa", поскольку первая \(и единственная\) группа захвата соответствует первоначальной последовательности "a", а затем \\1 соответствует завершающей последовательности "a".  
...
### Повторение  
 За любым элементом, кроме *положительного утверждения*, *негативного утверждения* или *привязки*, может следовать число повторений.  Наиболее распространенный тип числа повторений обладает форматом "{`min`,`max`}" или "\\{`min`,`max`\\}" в `BRE` и `grep`.  Элемент, за которым следует такой формат числа повторений, соответствует числу успешных вхождений последовательности, соответствующей элементу, не менее `min` и не более `max`.  Например, "a{2,3}" соответствует целевой последовательности "aa" и целевой последовательности "aaa", но не целевой последовательности "a" или "aaaa".  
  
 Число повторений также может иметь один из перечисленных ниже форматов:  
  
-   "{`min`}" или "\\{`min`\\}" в `BRE` и `grep`.  Аналогично "{`min`,`min`}".  
  
-   "{`min`,}" или "\\{`min`,\\}" в `BRE` и `grep`.  Аналогично "{`min`,unbounded}".  
...
-   "a\*" соответствует целевой последовательности "", целевой последовательности "a", целевой последовательности "aa" и т.д.  
 ...
 Для всех грамматик, кроме `BRE` и `grep`, число повторений также может обладать одним из следующих форматов:  
...
-   "a\+" соответствует целевой последовательности "a", целевой последовательности "aa" и т.д., но не целевой последовательности "".  
 ...
 В грамматике `ECMAScript` за числом повторений в любом формате может следовать символ "?", указывающий *"нежадное" повторение*.  
...
### Чередование  
 Во всех грамматиках регулярных выражений, кроме `BRE` и `grep`, за соединенным регулярным выражением может следовать символ "&#124;" и другое соединенное регулярное выражение.  Таким образом можно объединить любое количество регулярных выражений.  Результирующее выражение соответствует любой целевой последовательности, которая соответствует одному или нескольким соединенным регулярным выражением.  
  
 Если целевой последовательности соответствует несколько соединенных регулярных выражений, грамматика `ECMAScript` выбирает первое из соединенных регулярных выражений, соответствующих последовательности \(*первое соответствие*\); грамматики других регулярных выражений выбирают регулярное выражение, которое обеспечивает *наиболее длинное соответствие*.  Например, "ab&#124;cd" соответствует целевой последовательности "ab" и целевой последовательности "cd", но не соответствует целевой последовательности "abd" или целевой последовательности "acd".  
  
 В `grep` и `egrep` символ новой строки \("\\n"\) используется для разделения чередований.  
  
### Часть выражения  
 В `BRE` и `grep` часть выражения является объединением.  В других грамматиках регулярных выражений часть выражения является чередованием.  
  
##  <a name="grammarsummary"></a> Сводка по грамматике  
 В следующей таблице приведены функции, доступные в различных грамматиках регулярных выражений:  
  
|Элемент|BRE|ERE|ECMA|grep|egrep|awk|  
|-------------|---------|---------|----------|----------|-----------|---------|  
...
##  <a name="semanticdetails"></a> Семантические сведения  
...
### Обратная ссылка  
 Обратная ссылка — это обратная косая черта, которая сопровождается десятичным значением N.  Она соответствует содержимому n\-ной *группы захвата*.  Значение N не должно превышать количество групп захвата, предшествующих обратной ссылке.  В `BRE` и `grep` значение N определяется десятичным разрядом, который следует за обратной косой чертой.  В грамматике `ECMAScript` значение N определяется всеми десятичными разрядами, которые следуют за обратной косой чертой.  Поэтому в `BRE` и `grep` значение N никогда не будет больше 9, даже если регулярное выражение содержит более 9 групп захвата.  В грамматике `ECMAScript` значение N не ограничено.  
...
### Символ упорядочивания  
 Символ упорядочивания в выражении в квадратных скобках добавляет *элемент упорядочивания* в набор, заданный выражением в квадратных скобках.  Чтобы создать символ упорядочивания, используйте символ "\[.", после которого необходимо указать символ упорядочивания, а затем — символ ".\]".  
  
### Управляющая escape\-последовательность  
 Управляющая escape\-последовательность — это обратная косая черта, за которой следует буква "c", а затем — одна из букв в диапазоне от "a" до "z" или от "A" до "Z".  Она соответствует управляющему символу ASCII, который именуется данной буквой.  Например, "\\ci" соответствует целевой последовательности "\\x09", поскольку параметр \<ctrl\-i\> имеет значение 0x09.  
...
### Escape\-выражение формата файлов  
 Escape\-выражения формата файлов состоит из стандартных последовательностей escape\-символов языка C, "\\\\", "\\a", "\\b", "\\f", "\\n", "\\r", "\\t", "\\v". Они обладают стандартными значениями: обратная косая черта, оповещение, стирание назад, перевод строки, возврат каретки, горизонтальная табуляция и вертикальная табуляция, соответственно.  В грамматике `ECMAScript` использование элементов "\\a" и "\\b" не допускается. \(Использование элемента "\\\\", однако он является escape\-выражением удостоверения, а не формата файлов\).  
...
### Escape\-последовательность удостоверения  
 Escape\-последовательность удостоверения — это обратная косая черта, за которой следует один символ.  Она соответствует данному символу.  Данная последовательность необходима, если символ имеет особое значение; при использовании escape\-последовательности удостоверения специальное значение удаляется.  Например:  
...
|`ERE`, `egre`|{ '\(', '\)', '{', '.', '\[', '\\', '\*', '^', '$', '\+', '?', '&#124;' }|  
|`awk`|`ERE` плюс { '"', '\/' }|  
|`ECMAScript`|Все символы, кроме символов, которые могут быть частью идентификатора.  Как правило, это буквы, цифры, "$", "\_" и escape\-последовательности Юникод.  Дополнительные сведения см в спецификации языка ECMAScript.|  
...
-   "\[^\]abc\]" соответствует целевой последовательности "d", но не целевым последовательностям "a", "b", "c" или "\]".  
 ...
 В грамматике `ECMAScript` используйте элемент "\\\]" для представления символа "\]" в выражении в квадратных скобках.  
...
### Отрицательное утверждение границы слова  
 Отрицательное утверждение границы слова соответствует целевой строке, если текущая позиция в данной строке не следует за *границей слова*.  
  
### Группа незахвата  
 Группа незахвата помечает свое содержимое в качестве единого элемента в грамматике регулярного выражения, но не снабжает меткой целевой текст.  Например, "\(a\)\(?:b\)\*\(c\) соответствует целевому тексту "abbc" и связывает группу захвата 1 с частью последовательности "a", а группу захвата 2 с частью последовательности "c".  
...
-   ^  $  \\  .  \*  \+  ?  \(  \)  \[  \]  {  }  &#124;  
 ...
 В грамматиках `BRE` и `grep` специальное значение имеют следующие символы:  
  
-   .   \[   \\  
  
 Также в `BRE` и `grep`, следующие символы имеют специальное значение при использовании в указанном контексте.  
  
-   Символ "\*" обладает специальным значением во всех случаях, кроме случаев, когда он является первым символом в регулярном выражении либо первым символом, следующим за первым символом "^" в регулярном выражении; а также если данный символ является первым символом в группе захвата или первым символом, который следует за первым символом "^" в группе захвата.  
...
-   Символ "$'" обладает специальным значением, если является последним символом регулярного выражения.  
 ...
 В грамматиках `ERE`, `egrep` и `awk` специальное значение имеют следующие символы:  
  
-   .   \[   \\   \(   \*   \+   ?   {   &#124;  
  
 Кроме того, в грамматиках `ERE`, `egrep` и `awk` следующие символы имеют специальное значение при использовании в указанном контексте.  
...
-   "\(aa\)\(a\*\)" соответствует целевой последовательности "aaaa" и связывает группу захвата 1 с частью целевой последовательности "aa" в начале данной последовательности, а группу захвата 2 — с частью целевой последовательности "aa" в конце данной последовательности.  
  
-   "\(?\=aa\)\(a\)&#124;\(a\)" соответствует целевой последовательности "a" и связывает группу захвата 1 с пустой последовательностью \(поскольку произошел сбой положительного утверждения\), а группу захвата 2 — с частью последовательности "a".  Данное выражение также соответствует целевой последовательности "aa" и связывает группу захвата 1 с частью последовательности "aa", а группу захвата 2 — с пустой последовательностью.  
  
### Escape\-последовательность Юникод  
...
##  <a name="matchingandsearching"></a> Подбор и поиск  
...
-   Поиск регулярного выражения "bcd" в целевой последовательности "bcdbcd" выполняется успешно, и результат соответствует первым трем символам.  
 ...
 При наличии нескольких соответствующих частей целевой последовательности существует два способа выбрать выбрать соответствующий шаблон.  *Первое соответствие* выбирает часть последовательности, найденную первой при установке соответствия с регулярным выражением.  *Наиболее длинное соответствие* выбирает наиболее длинную часть последовательности из соответствующих в данном местоположении.  Если максимальную длину имеет несколько частей последовательности, наиболее длинное соответствие выбирает часть последовательности, найденную первой.  Например, если используется первое соответствие, при поиске регулярного выражения "b&#124;bc" в целевой последовательности "abcd" результатом будет часть последовательности "b", поскольку крайнее левое условие чередования соответствует данной части последовательности; поэтому первое соответствие не предпринимает попытки обратиться к правому крайнему условию чередования.  Если используется наиболее длинное соответствие, такие же условия поиска дают результат "bc", поскольку "bc" длиннее, чем "b".  
...
|"$$"||"$"|  
||"\\&"|"&"|  
|"$\`" \(знак доллара и символ обратного апострофа\)||Последовательность символов, предшествующая части последовательности, которая соответствует регулярному выражению \(`[match.prefix().first, match.prefix().second)`\)|  
...
|"$n"|"\\n"|Последовательность символов, которая соответствует группе захвата в позиции `n`,где `n` — число в диапазоне от 0 до 9 \(`[match[n].first, match[n].second)`|  
||"\\\\n"|"\\n"|  
|"$nn"||Последовательность символов, которая соответствует группе захвата в позиции `nn`,где `nn` — число в диапазоне от 10 до 99 \(`[match[nn].first, match[nn].second)`|  
