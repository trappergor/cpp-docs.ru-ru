...
`Container` Тип базового контейнера, используемый для реализации стека. Значение по умолчанию — класс `deque`*\<Type>*.
...
`val` Элемент, добавляемый в верхней части стека.
...
`right` Контейнер, в которой сконструированный стека является копией.
