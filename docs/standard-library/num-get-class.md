...
`InputIterator` Тип итератора, из которого функции получения числовых значений считывают своих входные данные.
...
`first` Начало диапазона символов, из которого считывается значение.

`last` Конец диапазона символов, из которого считывается значение.

`_Iosbase` [Ios_base](../standard-library/ios-base-class.md) которого флаги используются при преобразовании.

`_State` Состояние какая failbit (см. [ios_base::iostate](../standard-library/ios-base-class.md#iostate)) будет добавлен после сбоя.

`val` Прочитанное значение.
