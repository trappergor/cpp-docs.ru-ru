...
*InputIterator* тип итератора, из которого функции получения числовых считывают своих входные данные.
...
*Первый* начало диапазона символов для чтения числа.

*последний* конец диапазона символов для чтения числа.

*_Iosbase* [ios_base](../standard-library/ios-base-class.md) , флаги которого используются при преобразовании.

*_State* состояние какие failbit (см. в разделе [ios_base::iostate](../standard-library/ios-base-class.md#iostate)) добавляется в случае сбоя.

*Val* значение, которое было считано.
