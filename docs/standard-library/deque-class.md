...
*Выделен*\
Тип, представляющий сохраненный объект распределителя, содержащий сведения о распределении и отмене распределения памяти для deque. Этот аргумент является необязательным, и значением по умолчанию является **\<тип распределителя >** .
...
|||
|-|-|
|[deque](#deque)|Создает документ `deque`. Предусмотрено несколько конструкторов для настройки содержимого нового `deque` типа различными способами: пусто; загружено с указанным числом пустых элементов; содержимое перемещено или скопировано из другого `deque`объекта; содержимое скопировано или перемещено с помощью итератора; один элемент, `deque` `count` скопированный в время. Некоторые конструкторы позволяют использовать настраиваемый `allocator` для создания элементов.|
...
|[const_reverse_iterator](#const_reverse_iterator)|Тип, предоставляющий итератор произвольного доступа, который может получать доступ к элементам в `deque` константе типа AS и считывать их. Объект deque просматривается в обратном порядке. Дополнительные сведения см. в статье [Класс reverse_iterator](../standard-library/reverse-iterator-class.md).|
...
|[cend](#cend)|Возвращает **Константный** итератор произвольного доступа, указывающий на сразу за концом `deque`.|
...
|[empty](#empty)|Возвращает **значение true** , `deque` если содержит нулевые элементы, и **false** , если содержит один или несколько элементов.|
...
Предоставляет указатель на элемент **const** в deque.
