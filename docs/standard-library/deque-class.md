...
*Распределитель* тип, представляющий сохраненный объект распределителя, инкапсулирующий сведения о выделении и освобождении памяти очереди. Этот аргумент является необязательным, и значение по умолчанию — **распределителя\<типа>***.*
...
|-|-|
|[deque](#deque)|Создает документ `deque`. Несколько конструкторов, позволяющих настроить содержимое нового `deque` по-разному: пустым, указанное число пустых элементов, переместить или скопировать из другого содержимое `deque`; содержимое копируется или перемещается с помощью итератора; и один элемент, скопированный в `deque` `count` раз. Некоторые конструкторы позволяют использовать настраиваемый `allocator` для создания элементов.|
...
|[const_reverse_iterator](#const_reverse_iterator)|Тип, предоставляющий итератор произвольного доступа, который можно получить доступ к и считывать элементы в `deque` как **const**. Объект deque просматривается в обратном порядке. Дополнительные сведения см. в статье [Класс reverse_iterator](../standard-library/reverse-iterator-class.md).|
...
|[cend](#cend)|Возвращает произвольного доступа **const** итератор, который указывает конец `deque`.|
...
|[empty](#empty)|Возвращает **true** Если `deque` содержит ноль элементов и **false** если он содержит один или несколько элементов.|
...
*Первый* положение первого элемента в диапазоне элементов списка для копирования из очереди аргументов.

*Последний* положение первого элемента после диапазона элементов, копируемых из очереди аргументов.

*Число* количество копий элемента, вставляемых в очередь.

*Val* значение элемента, вставляемого в очередь.

*IList* initializer_list, вставляемый в очередь.
...
*POS* индекса (или номер позиции) элемента для ссылки в очереди.

### <a name="return-value"></a>Возвращаемое значение

Если *pos* больше размера очереди, `at` возникло исключение.
...
Если возвращаемое значение `begin` присвоено `const_iterator`, то объект очереди нельзя изменить. Если возвращаемое значение `begin` назначается `iterator`, то объект очереди можно изменить.
...
Предоставляет указатель на **const** элемент в очереди.
...
Первые два конструктора указывают пустую начальную очередь; Кроме того, второй указывает тип распределителя (`_Al`) для использования.
...
Шестой конструктор задает копию очереди *справа*.

Седьмой и восьмой конструкторы копируют диапазон `[First, Last)` очереди.

Седьмой конструктор перемещает очередь *справа*.
...
`end` используется для проверки, достиг ли итератор конца очереди.
...
*_Where* положение элемента, который необходимо удалить из очереди.

*Первый* положение первого элемента, удаленного из очереди.

*последний* положение после последнего элемента, удаленного из очереди.
...
|*Val*|Значение элемента, вставляемого в очередь.|
|*Количество*|Количество элементов, вставляемых в очередь.|
|*Первый*|Положение первого элемента в диапазоне элементов в копируемой очереди аргументов.|
|*последний*|Положение первого элемента за пределами диапазона элементов в копируемой очереди аргументов.|
...
*POS* положение элемента очереди, на который должна быть ссылка.
...
Первое переопределение копирует элементы в эту очередь из *правой*, источнике назначения. Второе переопределение перемещает элементы в эту очередь из *правой*.
...
*_Newsize* новый размер очереди.

*Val* значение новых элементов, которое необходимо добавить в очередь, если новый размер больше, исходный размер. Если значение не задано, новым элементам назначается значение по умолчанию для класса.

### <a name="remarks"></a>Примечания

Если размер очереди меньше запрошенного размера, *_Newsize*, элементы добавляются в очередь, пока не достигнет требуемого размера.

Если размер очереди больше запрошенного размера, ближайшие к конец очереди элементы удаляются, пока не достигнет размера deque *_Newsize*.
...
*правом* очередь, предоставляющая элементы для обмена местами, или очередь, элементы которого должны быть заменены очереди `left`.

*слева* deque, элементы которого должны быть заменены очереди *правой*.
