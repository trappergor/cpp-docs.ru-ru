...
|||
|-|-|
|[deque](#deque)|Создает документ `deque`. Есть несколько конструкторов для настройки содержимого новых `deque` различными способами: Empty; Загружено с указанным числом пустых элементов; содержимое перемещено или скопировано из другого `deque`; содержимое, скопированное или перемещенное с помощью итератора; и один элемент, скопированный в `deque` `count` раз. Некоторые конструкторы позволяют использовать настраиваемый `allocator` для создания элементов.|
...
|[const_reverse_iterator](#const_reverse_iterator)|Тип, предоставляющий итератор произвольного доступа, который может получать доступ к элементам в `deque` и считывать их в виде **константы**. Объект deque просматривается в обратном порядке. Дополнительные сведения см. в статье [Класс reverse_iterator](../standard-library/reverse-iterator-class.md).|
...
|[empty](#empty)|Возвращает **значение true** , если `deque` содержит нулевые элементы, и **false** , если содержит один или несколько элементов.|
...
Если *POS* больше размера deque, `at` создает исключение.
...
Первые два конструктора указывают пустую начальную deque; второй также указывает тип распределителя (`_Al`), который будет использоваться.

Третий конструктор задает повторение указанного числа (`count`) элементов со значением по умолчанию для класса `Type`.

Четвертый и пятый конструкторы указывают повторение элементов (*Count*) значений `val`.
