...
Шаблон класса описывает объект, управляющий неизменяемой последовательностью элементов типа `sub_match<BidIt>`, созданных поиском регулярных выражений. Каждый элемент указывает на часть последовательности, которая соответствует группе захвата, соответствующей этому элементу.
...
Оператор-член заменяет последовательность, управляемую `*this`, на копию последовательности, управляемой *вправо*.
...
Функция-член возвращает ссылку на элемент *n* управляемой последовательности или ссылку на пустой объект `sub_match`, если `size() <= n` или если группа захвата *n* не была частью соответствия.
...
Функция – член меняет местами содержимое `*this` и *right* в постоянное время и не создает исключения.
