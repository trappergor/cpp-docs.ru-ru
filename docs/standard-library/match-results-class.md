...
Каждая функция – член создает форматированный текст под элементом управления формата *FMT*. Первая функция – член записывает форматированный текст в последовательность, определенную его аргументом *out* , *и возвращает.* Вторая функция-член возвращает объект строки, содержащий копию форматированного текста.
