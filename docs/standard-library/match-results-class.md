...
`Alloc` Тип распределителя для управления хранилищем.
...
`sub` Индекс подстроки совпадения.
...
`right` Копируемый объект match_results.
...
`right` Объект match_results для замены.
