...
*dest_size* \
Размер *dest*. Если `char_type` имеет **тип char**, то этот размер задается в байтах. Если `char_type` является **wchar_t**, то этот размер задается в виде слов.
...
Необходимо иметь возможность вводить значение типа `CharType`, чтобы `int_type` затем возвращаться к `CharType` без изменения исходного значения.
...
*dest_size* \
Размер *dest*. Если `char_type` имеет **тип char**, то он находится в байтах. Если `char_type` является **wchar_t**, то это в словах.
...
@No__t_0 представление проверяемого символа, если `int_type` символа не равно значению символа EOF.
