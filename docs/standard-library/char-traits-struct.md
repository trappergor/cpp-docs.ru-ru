...
Вторая функция-член возвращает указатель на строку, для первой *_Num* элементам которой были назначены значения *_CharFrom*.
...
Стандарт C++ гласит, что это значение не должно соответствовать допустимому значению `char_type`. Компилятор Майкрософт C++ применяет это ограничение для типа **char**, но не для типа **wchar_t**. Это демонстрируется в приведенном ниже примере.
...
Необходимо иметь возможность присвоить значение типа `CharType` `int_type` CAST, а затем вернуться `CharType` , не изменяя исходное значение.
...
*dest_size*\
Размер *dest*. Если `char_type` значение равно **char**, то оно находится в байтах. Если `char_type` является **wchar_t**, то это в словах.
...
Представление проверяемого символа, `int_type` если символ не равен значению символа EOF. `int_type`
