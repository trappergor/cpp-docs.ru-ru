...
**_** *CharFrom* , значение которого должно быть присвоено — символ.

*_CharTo* элемент, должно быть присвоено значение символа.

* strTo * строке или символьном массиве, элементы которого начальной назначаются значения символов.

`_Num` Число элементов, которые должны быть присвоены значения.

### <a name="return-value"></a>Возвращаемое значение

Вторая функция-член возвращает указатель на строку, первым `_Num` элементам которой присвоены значения *_CharFrom*.
...
* str1 * первый из двух строк для сравнения с друг с другом.

* str2 * второе из двух строк сравниваться друг с другом.

`_Num` Число элементов в сравнение строк.
...
`_To` Элемент в начале строке или символьном массиве, предназначенный для получения скопированного последовательность символов.

`_From` Элемент в начале исходного массива строку или символ для копирования.

`_Num` Число копируемых элементов.
...
`dest` Строке или символьном массиве, предназначенный для получения скопированного последовательность символов.

`dest_size` Размер `dest`. Если `char_type` — `char`, то это размер в байтах. Если `char_type` — `wchar_t`, то это размер в словах.

`_From` Источник строковом или символьном массив для копирования.
...
Стандарт C++ гласит, что это значение не должно соответствовать допустимому значению `char_type`. Компилятор Visual C++ принудительно вводит это ограничение для типа `char`, но не для типа `wchar_t`. Это демонстрируется в приведенном ниже примере.
...
`_Ch1` Первое из двух символов, для проверки на равенство.

`_Ch2` Второе из двух символов, для проверки на равенство.
...
`_Ch1` Первым из двух знаков для проверки на равенство как **int_type**s.

`_Ch2` Второе из двух символов проверяемые на равенство как `int_type`s.
...
`str` Первый символ в строке поиска.

`_Num` Количество позиций, начиная с первой, в диапазоне для поиска.

`_Ch` Символ для поиска в диапазоне.
...
Должно быть возможно приведение типов для преобразования значения типа **CharType** в `int_type`, а затем обратно в **CharType** без изменения исходного значения.
...
`str` C-строка, длина которого является универсальной.
...
`_Ch1` Первое из двух знаков для проверки меньше.

`_Ch2` Второе из двух знаков для проверки меньше.
...
`_Num` Число элементов, копируемых из исходной строки.

### <a name="return-value"></a>Возвращаемое значение

Первый элемент `_To`, скопированный в строку или массив символов, предназначенный для получения скопированной последовательности символов.
...
`dest` Элемент в начале строке или символьном массиве, предназначенный для получения скопированного последовательность символов.

`dest_size` Размер `dest`. Если `char_type` — `char`, то это размер в байтах. Если `char_type` — `wchar_t`, то это размер в словах.

`_From` Элемент в начале исходного массива строку или символ для копирования.

`count` Число элементов, копируемых из исходной строки.

### <a name="return-value"></a>Возвращаемое значение

Первый элемент `dest`, скопированный в строку или массив символов, предназначенный для получения скопированной последовательности символов.
...
`_Ch` Символ, представленный как `int_type` проверяемое для, является ли символ EOF или не.

### <a name="return-value"></a>Возвращаемое значение

Представление `int_type` символа проверяется, не равно ли значение **int_type** символа значению символа конца файла.
...
`_Ch` `int_type` Символ в виде `char_type`.
...
`_Ch` `char_type` Символ в виде `int_type`.
