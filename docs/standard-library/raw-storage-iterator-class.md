...
`val` Значение объекта типа **тип** должны быть вставлены в памяти.
...
`first` Прямой итератор, который должен лежат в основе `raw_storage_iterator` объекта, создаваемого.
