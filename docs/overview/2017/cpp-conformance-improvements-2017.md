...
Закрытые деструкторы могут сделать тип неконструируемым. `std::is_constructible<T, Args...>` вычисляется, как если бы было написано следующее объявление:
...
`std::is_convertible` требует, чтобы целевой тип был допустимым возвращаемым типом. В более ранних версиях Visual Studio компилятор неправильно разрешал использование абстрактных типов, что могло приводить к неверному разрешению перегрузки и непредусмотренному поведению среды выполнения.  Следующий код теперь правильно выводит предупреждение C2338:
...
**Примечание.** В Python-библиотеке Boost в течение долгого времени существовало относящееся к MSVC обходное решение для опережающего объявления шаблона в [unwind_type.hpp](https://github.com/boostorg/python/blame/develop/include/boost/python/detail/unwind_type.hpp). В режиме [/permissive-](../../build/reference/permissive-standards-conformance.md), начиная с Visual Studio 2017 версии 15.8 (_MSC_VER=1915), компилятор MSVC корректно выполняет поиск в зависимости от аргументов (ADL) и согласуется с другими компиляторами, делая условие обходного решения ненужным. Чтобы избежать ошибки *C3861: 'unwind_type': идентификатор не найден*, см. [PR 229](https://github.com/boostorg/python/pull/229) в репозитории Boostorg для обновления файла заголовка. Пакет Boost [vcpkg](../../build/vcpkg.md) уже исправлен, поэтому при получении или обновлении источника Boost из vcpkg не нужно применять отдельное исправление.
...
В Visual Studio 2017 версии 15.9 код выдает эту ошибку: *ошибка C3615: функция constexpr 'S<int>:: f' не может результировать в константное выражение. Примечание: сбой был вызван неопределенной функцией или функцией, не объявленной как "constexpr" Примечание: см. использование g*. Чтобы избежать этой ошибки, удалите квалификатор `constexpr` из явного создания экземпляра функции f(). 
