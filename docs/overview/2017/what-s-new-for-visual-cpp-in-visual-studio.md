...
Полный список стандартных библиотек в см. в статье [Исправления стандартной библиотеки в Visual Studio 2017 RTM](https://blogs.msdn.microsoft.com/vcblog/2017/02/06/stl-fixes-in-vs-2017-rtm/).
...
- `normal_distribution<float>` больше не выдает предупреждений в стандартной библиотеке о сужающем преобразовании из типа double во float.
...
- `future::get()` теперь, в соответствии со стандартом, не допускает `future`.
- `iterator_traits<void *>` являлся критической ошибкой, так как пытался сформировать `void&`. Теперь он становится пустой структурой, позволяющей использовать `iterator_traits` в условиях SFINAE "is iterator".
...
- `conjunction` `disjunction` теперь создает экземпляры типов N + 1 вместо 2N + 2.
...
- `std::equal` `std::partition` была передача оптимизации потоков перехода, сохраняющая сравнения итераторов.
...
- `__declspec(allocator)` теперь защищен только для C1XX, чтобы предотвращать вывод предупреждений из Clang, который не понимает declspec.
