...
|name|Описание|
|----------|-----------------|
|[Крект:: Крект](#crect)|Создает объект `CRect`.|
...
|[Крект:: Интерсектрект](#intersectrect)|Задает `CRect` значение, равное пересечению двух прямоугольников.|
|[Крект:: Исректемпти](#isrectempty)|Определяет, `CRect` является ли пустой. `CRect`пуст, если ширина и (или) Высота равны 0.|
|[Крект:: Исректнулл](#isrectnull)|Определяет, равны `top`ли `bottom`значения переменных члена `right` ,, `left`и 0.|
...
|[Крект::P Тинрект](#ptinrect)|Определяет, находится ли указанная точка `CRect`в.|
...
|[Крект:: Унионрект](#unionrect)|Задает `CRect` значение, равное объединению двух прямоугольников.|
...
|name|Описание|
|----------|-----------------|
|[Крект:: operator —](#operator_-)|Вычитает заданные смещения из `CRect` или `CRect` Deflate и возвращает результирующий `CRect`объект.|
...
|[Крект:: operator&amp;](#operator_amp)|Создает пересечение `CRect` и прямоугольник и возвращает результирующий `CRect`объект.|
|[Крект:: operator&amp;=](#operator_amp_eq)|Задает `CRect` значение, равное `CRect` пересечению и прямоугольнику.|
|[Крект:: operator&#124;](#operator_or)|Создает объединение `CRect` и прямоугольник и возвращает результирующий `CRect`объект.|
|[Крект:: operator &#124;=](#operator_or_eq)|Задает `CRect` значение, равное `CRect` объединению и прямоугольнику.|
|[Крект:: operator +](#operator_add)|Добавляет заданное смещение в `CRect` или `CRect` Deflate и возвращает результирующий `CRect`объект.|
|[Крект:: operator + =](#operator_add_eq)|Добавляет указанные смещения в `CRect` или `CRect`Deflate.|
|[Крект:: operator =](#operator_eq)|Копирует размеры прямоугольника в `CRect`.|
|[Крект:: operator-=](#operator_-_eq)|Вычитает заданные смещения из `CRect` или `CRect`Deflate.|
|[Крект:: operator = =](#operator_eq_eq)|Определяет, `CRect` равен ли прямоугольник.|

## <a name="remarks"></a>Примечания

`CRect`также включает функции элементов для работы `CRect` с объектами и `RECT` структурами Windows.

Объект может передаваться как параметр функции `RECT` везде, где может быть `LPCRECT`передана структура, или `LPRECT`. `CRect`

> [!NOTE]
> Этот класс является производным от `tagRECT` структуры. (Имя `tagRECT` `RECT` для структуры является редко используемым именем.) Это означает, что элементы данных (`left` `right`, `top`, и `bottom`) `RECT` структуры являются доступными элементами `CRect`данных.

Содержит `CRect` переменные-члены, определяющие точки прямоугольника, расположенные в верхней левой и нижней правой части.
...
Будьте внимательны при управлении с `CRect` помощью функций-членов [CDC::D птолп](../../mfc/reference/cdc-class.md#dptolp) и [CDC:: лптодп](../../mfc/reference/cdc-class.md#lptodp) . Если в качестве режима сопоставления контекст отображения имеет отрицательное значение, то, как в `MM_LOENGLISH` `CDC::DPtoLP` , преобразуется `CRect` , чтобы его верх больше нижней. Функции, такие `Height` как `Size` и, затем возвращают отрицательные значения для высоты преобразованной `CRect`области, а прямоугольник — без нормализации.

При `CRect` использовании перегруженных операторов первый операнд должен быть `CRect`a; второй может быть либо структурой [Rect](/windows/win32/api/windef/ns-windef-rect) , либо `CRect` объектом.
...
`CPoint` Объект, `CRect`центерпоинт.
...
Копирует прямоугольник в `CRect`. `lpSrcRect`
...
Если аргументы не заданы, `left` `top` `right`то члены,, `bottom` и не инициализируются.

Конструкторы `CRect`(`const RECT&`) и `CRect`(`LPCRECT`) выполняют [CopyRect](#copyrect). Другие конструкторы напрямую инициализируют переменные членов объекта.
...
*y*<br/>
Указывает число единиц, которые должны быть сведены к верхнему и нижнему `CRect`краям.

*size*<br/>
[Размер](/windows/win32/api/windef/ns-windef-size) или [ксизе](csize-class.md) , указывающий количество единиц, которые нужно деувеличить `CRect`. Значение указывает количество единиц, которые должны быть сведены к левой и правой сторонам `cy` , а значение указывает количество единиц, которое будет разворотировать сверху и снизу. `cx`
...
*r*<br/>
Задает число единиц, которые должны быть сведены к правой `CRect`части.
...
Для этого `DeflateRect` добавляет единицы измерения слева и сверху и вычитает единицы из правой и нижней части. Параметры — это `DeflateRect` значения со знаком; положительные значения преобразуются `CRect` в неструктурированные и отрицательные.

Первые две `CRect` перегрузки разменяют обе пары противоположных сторон так, что ее общая ширина уменьшается на два раза *x* (или `cx`), а ее общая высота уменьшается на два раза по *оси y* (или `cy`). Две другие перегрузки размещаются на каждой стороне `CRect` независимо от других.
...
*лпрект*<br/>
Указывает на структуру [RECT](/windows/win32/api/windef/ns-windef-rect) или `CRect` объект, который содержит координаты прямоугольника в верхнем левом и нижнем правом углу.
...
*x*<br/>
Указывает число единиц, которое будет разворотировать `CRect`левую и правую части.

*y*<br/>
Указывает число единиц, которое будет вычислено сверху и снизу от `CRect`.

*size*<br/>
[Размер](/windows/win32/api/windef/ns-windef-size) или [ксизе](csize-class.md) , указывающий количество единиц, которое необходимо увеличить `CRect`. Значение указывает количество единиц, которое будет разворотировать левую и правую части `cy` , а значение указывает количество единиц, которое будет вычислено сверху и снизу. `cx`

*лпрект*<br/>
Указывает на структуру [Rect](/windows/win32/api/windef/ns-windef-rect) или `CRect` на число единиц, которые нужно увеличить на каждой стороне.

*l*<br/>
Задает число единиц, которое необходимо увеличить с левой стороны `CRect`.

*t*<br/>
Указывает число единиц, которое будет вычислено в верхней `CRect`части.
...
Для этого `InflateRect` вычитает единицы слева и сверху и добавляет единицы вправо и вниз. Параметры имеют значения `InflateRect` со знаком; положительные значения преобразуются в `CRect` неровные, а отрицательные — в неизменном виде.

Первые две `CRect` перегрузки разменяют обе пары противоположных сторон так, что ее общая ширина увеличивается на два раза *x* (или `cx`), а ее общая высота увеличивается на два раза *y* (или `cy`). Две другие перегрузки размещаются на каждой стороне `CRect` независимо от других.
...
*lpRect1*<br/>
Указывает на структуру [RECT](/windows/win32/api/windef/ns-windef-rect) или `CRect` объект, который содержит исходный прямоугольник.
...
Определяет, равны ли значения `CRect` "Top", "Left", "Bottom" и "Right" равными 0.
...
`CRect` Нормализует таким образом, чтобы высота и ширина были положительными.
...
> [!NOTE]
> Для правильной `CRect` работы следующих функций элементов требуются нормализованные прямоугольники: [Высота](#height), [Ширина](#width), [Размер](#size), [исректемпти](#isrectempty), [PtInRect](#ptinrect), [екуалрект](#equalrect), [унионрект](#unionrect), [интерсектрект](#intersectrect), [субтрактрект](#subtractrect), [operator = =](#operator_eq_eq), [оператор! =](#operator_neq), [оператор &#124; ](#operator_or), [operator &#124;=](#operator_or_eq), [оператор &](#operator_amp)и [оператор & =](#operator_amp_eq).
...
Перемещает `CRect` *x* единиц вдоль осей x и *y* вдоль оси y. Параметры *x* и *y* — это значения со знаком, `CRect` поэтому их можно перемещать влево или вправо, а также вверх или вниз.
...
Преобразует `CRect` в [LPRECT](../../mfc/reference/data-types-mfc.md).
...
Определяет, `rect` `CRect` равно ли равенство, путем сравнения координат верхнего левого и нижнего правого угла.
...
Определяет `CRect` , является ли значение *Rect* неравным, сравнивая координаты их верхнего левого угла.
...
*лпрект*<br/>
Указывает на структуру [RECT](/windows/win32/api/windef/ns-windef-rect) или `CRect` объект, который содержит число единиц, которые нужно увеличить с каждой стороны `CRect`.

### <a name="remarks"></a>Примечания

Значения *x* и *y* (или `cx` и `cy`) для параметра добавляются в `CRect`.

Третья перегрузка `CRect` уменьшается на число единиц, указанное в каждом элементе параметра.
...
*лпрект*<br/>
Указывает на структуру [RECT](/windows/win32/api/windef/ns-windef-rect) или `CRect` объект Rect, который содержит число единиц, которые должны быть сведены каждый из `CRect` сторон.

### <a name="remarks"></a>Примечания

Значения *x* и *y* (или `cx` и `cy`) для параметра вычитаются из `CRect`.

Третья перегрузка `CRect` уменьшается на число единиц, указанное в каждом элементе параметра. Обратите внимание, что эта перегрузка функция, например [дефлатерект](#deflaterect).
...
Задает `CRect` значение, равное `CRect` пересечению `rect`и.
...
Задает `CRect` значение, равное `CRect` объединению `rect`и.
...
*rect*<br/>
`CRect` Содержит [прямоугольник](/windows/win32/api/windef/ns-windef-rect)или.
...
Первые две перегрузки возвращают `CRect` объект, который `CRect` равен заданным смещениям.
...
Результат `CRect` , полученный `CRect` при перемещении или переходе к количеству единиц, указанных в параметре.

### <a name="remarks"></a>Примечания

Значения параметров *x* и *y* (или `cx` и `cy`) добавляются в `CRect`расположение.

Третья перегрузка возвращает новый `CRect` , который `CRect` равен количеству единиц, заданному в каждом элементе параметра.
...
Объект `CRect` , полученный в результате перемещения или `CRect` деспрямления по количеству единиц, указанных в параметре.

### <a name="remarks"></a>Примечания

Значения параметров *x* и *y* (или `cx` и `cy`) вычитаются из `CRect`расположения.

Третья перегрузка возвращает новый `CRect` , который `CRect` равен количеству единиц, указанных в каждом элементе параметра. Обратите внимание, что эта перегрузка функции, например [дефлатерект](#deflaterect), не [субтрактрект](#subtractrect).
...
Возвращает объект `CRect` , который является `CRect` пересечением и *rect2*.
...
Объект `CRect` , представляющий собой `CRect` пересечение и *rect2*.
...
Возвращает объект `CRect` , который является `CRect` объединением и *rect2*.
...
Объект `CRect` , который является `CRect` объединением и *rect2*.
...
Определяет, находится ли указанная точка `CRect`в.
...
Ненулевое значение, если `CRect`точка находится внутри; в противном случае — 0.
...
Элементы `cx` `CRect`и `cy` возвращаемого значения содержат высоту и ширину.
...
Делает измерения `CRect` равными для `lpRectSrc2` вычитания из `lpRectSrc1`.
...
`SubtractRect`не совпадает с [оператором operator-](#operator_-) and [-=](#operator_-_eq). Ни один из этих операторов никогда `SubtractRect`не вызывается.
