...
|[CImage::CreateEx](#createex)|Создает растровое изображение раздела DIB (с дополнительными параметрами) и прикрепляет его к ранее `CImage` созданному объекту.|
...
При использовании `CImage` в проекте MFC Обратите внимание на то, какие функции-члены в проекте предполагают указатель на объект [CBitmap](../../mfc/reference/cbitmap-class.md) . Если вы хотите `CImage` использовать с такой функцией, как [кмену:: аппендмену](../../mfc/reference/cmenu-class.md#appendmenu), используйте [CBitmap::](../../mfc/reference/cbitmap-class.md#fromhandle) `CImage` FromHandle, передайте ей хбитмап и используйте возвращенное `CBitmap*`значение.
...
> [!NOTE]
> При создании проекта с помощью `CImage`необходимо определить `CString` перед включением *атлимаже. h*. Если в проекте используется ATL без MFC, включите *atlstr. h* , прежде чем включать *атлимаже. h*. Если в проекте используется MFC (или проект ATL с поддержкой MFC), включите *афксстр. h* перед включением *атлимаже. h*.<br/>
> <br/>
...
*бблендоп*<br/>
Альфа-смешение для исходного и конечного точечных рисунков, глобальное значение Alpha, применяемое ко всему исходному точечному рисунку, и сведения о форматировании исходного растрового изображения. Исходная и Целевая функции смешения в настоящее время ограничены AC_SRC_OVER.

*поинтдест*<br/>
Ссылка на структуру [точек](/previous-versions/dd162805\(v=vs.85\)) , определяющую левый верхний угол прямоугольника назначения в логических единицах.
...
*ректдест*<br/>
Ссылка на структуру [Rect](/previous-versions/dd162897\(v=vs.85\)) , определяющую назначение.
...
*еориентатион*<br/>
Задает ориентацию точечного рисунка. Ниже указаны доступные значения.

- DIBOR_DEFAULT. ориентация точечного рисунка определяется операционной системой.

- DIBOR_BOTTOMUP строки точечного рисунка в обратный порядок. Это приводит к тому, что [CImage::-разряд](#getbits) возвращает указатель около конца буфера Bitmap и [CImage::](#getpitch) noreturn для возврата отрицательного числа.
...
*поинтдест*<br/>
Структура [точек](/previous-versions/dd162805\(v=vs.85\)) , указывающая верхний левый угол прямоугольника назначения.
...
*ректдест*<br/>
Структура [Rect](/previous-versions/dd162897\(v=vs.85\)) , указывающая конечный прямоугольник.
...
**Примечание** . В Visual Studio этот класс хранит `CImage` количество созданных объектов. Всякий раз, когда счетчик переходит к 0 `GdiplusShutdown` , функция автоматически вызывается для освобождения ресурсов, используемых GDI+. Это гарантирует, что `CImage` все объекты, созданные непосредственно или опосредованно библиотеками DLL, всегда `GdiplusShutdown` уничтожаются должным образом и не вызываются из DllMain.

Использование глобальных `CImage` объектов в библиотеке DLL не рекомендуется. Если необходимо использовать глобальный `CImage` объект в библиотеке DLL, вызовите метод [CImage:: релеасегдиплус](#releasegdiplus) , чтобы явно освободить ресурсы, используемые GDI+.

##  <a name="create"></a>CImage:: Create

Создает точечный рисунок и присоединяет его к ранее `CImage` созданному объекту. `CImage`
...
*екомпрессион*<br/>
Указывает тип сжатия для сжатого нижнего точечного рисунка (файлы DIB, расположенные сверху вниз, не могут быть сжаты). Может принимать одно из следующих значений:
...
*пдвбитфиелдс*<br/>
Используется, только если *екомпрессион* имеет значение BI_BITFIELDS, в противном случае оно должно быть равно null. Указатель на массив из трех битовых масок DWORD, указывающий, какие биты каждого пикселя используются для красного, зеленого и синего компонентов цвета соответственно. Сведения об ограничениях для битовых полей см. в разделе [битмапинфохеадер](/previous-versions//dd183376\(v=vs.85\)) в Windows SDK.
...
##  <a name="destroy"></a>CImage::D естрой
...
Биты на пиксель обычно имеют значение 1, 4, 8, 16, 24 или 32. Дополнительные сведения `biBitCount` о [BITMAPINFOHEADER](/previous-versions//dd183376\(v=vs.85\)) значении см. в описании членабитмапинфохеадерв Windows SDK.
...
##  <a name="getexporterfilterstring"></a>CImage:: Жетекспортерфилтерстринг
...
*агуидфилетипес*<br/>
Массив идентификаторов GUID, каждый элемент которого соответствует одному из типов файлов в строке. В примере, приведенном ниже *псзаллфилесдескриптион* , *агуидфилетипес*[0] является GUID_NULL, а остальные значения массива — форматами файлов изображений, поддерживаемыми текущей операционной системой.
...
где "&#124;" — это символ-разделитель, `chSeparator`заданный параметром. Например:
...
где "&#124;" — это символ-разделитель, заданный параметром *чсепаратор*. Например:
...
##  <a name="getmaxcolortableentries"></a>CImage:: Жетмаксколортаблинтриес
...
Значение красного, зеленого, синего (RGB) пикселя. Если пиксель находится за пределами текущей вырезанной области, возвращается значение CLR_INVALID.
...
##  <a name="getwidth"></a>CImage:: Полуширинный
...
##  <a name="isdibsection"></a>CImage:: Исдибсектион
...
##  <a name="isindexed"></a>CImage:: проиндексирован
...
##  <a name="istransparencysupported"></a>CImage:: Истранспаренцисуппортед
...
##  <a name="loadfromresource"></a>CImage:: Лоадфромресаурце
...
##  <a name="maskblt"></a>CImage:: Маскблт
...
##  <a name="operator_hbitmap"></a>CImage:: operator ХБИТМАП

Этот оператор используется для получения подключенного маркера `CImage` GDI Windows объекта. Этот оператор является оператором приведения, который поддерживает прямое использование объекта ХБИТМАП.
...
*ректсрк*<br/>
Ссылка на структуру [Rect](/previous-versions/dd162897\(v=vs.85\)) , указывающую координаты исходного прямоугольника.

*поинтмаск*<br/>
Структура [точек](/previous-versions/dd162805\(v=vs.85\)) , указывающая верхний левый угол битовой карты маски.
...
##  <a name="releasedc"></a>CImage:: Релеаседк
...
##  <a name="setcolortable"></a>CImage:: Сетколортабле
...
##  <a name="setpixelindexed"></a>CImage:: Сетпикселиндексед
...
##  <a name="transparentblt"></a>CImage:: Транспарентблт
...
*кртранспарент*<br/>
Цвет исходного растрового изображения, который должен считаться прозрачным. По умолчанию CLR_INVALID указывает, что используется цвет, заданный в качестве прозрачного цвета изображения.
