...
Ограничение не может быть типом значения, включая такие встроенные типы, как **int** и **double**. Поскольку типы значений не могут иметь производные классы, ограничению может удовлетворять только один класс. В этом случае универсальный шаблон можно переписать с заменой параметра типа определенным типом значения.
...
- Ограничения сами не могут быть параметрами типа, но могут содержать параметры типа в открытом сконструированном типе. Например:
