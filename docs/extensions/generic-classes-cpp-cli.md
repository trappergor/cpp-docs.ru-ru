...
Можно также объявить универсальный класс с [ограничениями для параметров универсального типа (C++/CLI)](constraints-on-generic-type-parameters-cpp-cli.md) для типов, которые могут использоваться для параметра типа. В следующем примере любой тип, используемый для `ItemType`, должен реализовывать интерфейс `IItem`. Например, попытка использовать тип **int**, который не реализует `IItem`, приведет к ошибке времени компиляции, так как аргумент типа не удовлетворяет ограничение.
...
В следующем примере три различных экземпляра универсального класса MyClass\<ItemType> создаются с помощью аргументов соответствующих типов (**int**, **double**и **string**).
...
В следующем примере объявляется универсальная структура `MyGenStruct` с одним полем (`myField`) и присваивает значения разных типов (**int**, **double**, `String^`) этому полю.
