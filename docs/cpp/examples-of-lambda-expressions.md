...
В данной статье приводится описание методов использования лямбда-выражений в программах. Общие сведения о лямбда-выражения, см. в разделе [лямбда-выражения](../cpp/lambda-expressions-in-cpp.md). Дополнительные сведения о структуре лямбда-выражения, см. в разделе [синтаксис лямбда-выражений](../cpp/lambda-expression-syntax.md).

##  <a name="declaringLambdaExpressions"></a> Объявление лямбда-выражений

### <a name="example-1"></a>Пример 1

Поскольку лямбда-выражение тип, можно назначить его **автоматически** переменной или [функция](../standard-library/function-class.md) объекта, как показано ниже:
...
Дополнительные сведения см. в разделе [автоматически](../cpp/auto-cpp.md), [класс function](../standard-library/function-class.md), и [вызовов функций](../cpp/function-call-cpp.md).
...
В этом примере лямбда-выражение передается в качестве аргумента функции `find_if`. Лямбда-выражение возвращает **true** если его параметром является четным числом.
...
Дополнительные сведения о `find_if` функции, см. в разделе [find_if](../standard-library/algorithm-functions.md#find_if). Дополнительные сведения о функции стандартной библиотеки C++, реализующих типичные алгоритмы, см. в разделе [ \<алгоритм >](../standard-library/algorithm.md).
...
##  <a name="methodLambdaExpressions"></a> Использование лямбда-выражение в функции

### <a name="example"></a>Пример

Лямбда-выражения можно использовать в теле функции. Лямбда-выражение может получать доступ к любой функции или данным-членам, которые способна использовать включающая функция. Можно явно или неявно захватывать **это** указатель для предоставления доступа к функциям и данным-членам включающего класса.
**Visual Studio 2017 версии 15.3 и более поздние версии** (состав [/std: c ++ 17](../build/reference/std-specify-language-standard-version.md)): Записать **это** по значению (`[*this]`) при лямбда-выражение используется в операциях асинхронные и параллельные где код может выполняться после за пределы области исходного объекта.

Можно использовать **это** указатель явным образом в функции, как показано ниже:
...
Вы можете захватывать **это** указатель неявно:
...
Функция `ApplyScale` использует лямбда-выражение для выведения произведения масштаба на каждый элемент объекта `vector`. Лямбда-выражение неявно захватывает **это** таким образом, он может обращаться к `_scale` член.

[[Содержание этой статьи](#top)]

##  <a name="templateLambdaExpressions"></a> С помощью лямбда-выражения с шаблонами

### <a name="example"></a>Пример

Поскольку лямбда-выражения имеют тип, их можно использовать с шаблонами C++. В следующем примере показаны функции `negate_all` и `print_all`. `negate_all` Функция применяет унарный **оператор -** к каждому элементу в `vector` объекта. Функция `print_all` печатает каждый элемент в объекте `vector` в консоли.
...
Тело лямбда-выражения выполняет правила как для структурированной обработки исключений (SEH), так и для обработки исключений C++. Можно обработать возникшее исключение в теле лямбда-выражения или перенести обработку исключения во включающий фрагмент. В следующем примере используется **for_each** функции и лямбда-выражение для заполнения `vector` со значениями другого. Она использует **попробуйте**/**catch** блок для обработки недопустимого доступа к первому вектору.
...
##  <a name="managedLambdaExpressions"></a> Использование лямбда-выражения с управляемыми типами (C++выполняет)
...
> [!IMPORTANT]
>  Лямбда-выражения не поддерживаются в эти общие сущности среды выполнения (CLR) управляемый язык: **ссылочного класса**, **структура ссылки**, **класса значений**, и **структура значения**.
