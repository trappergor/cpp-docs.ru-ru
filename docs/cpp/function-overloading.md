...
## <a name="argument-type-differences"></a>Различия типов аргументов  
 ...
 По этой же причине аргументы типа изменен с помощью функций **const** или `volatile` , интерпретируются как базовый тип для перегрузки.  
  
 Однако механизм перегрузки функций может различать ссылки, с указанием **const** и `volatile` и ссылки на базовый тип. Это позволяет создать код, аналогичный приведенному ниже.  
...
-   последовательности преобразований, которые могут быть сокращены путем удаления промежуточных преобразований, не учитываются.  
 ...
 Получающаяся последовательность преобразований (если таковые имеются), называется наилучшей последовательностью сопоставления. Существует несколько способов для преобразования объекта типа `int` ввода `unsigned long` с использованием стандартных преобразований (описанных в [стандартные преобразования](../cpp/standard-conversions.md)):  
...
1.  Точное соответствие. Точное соответствие между типами, с которыми функция вызывается, и типами, объявленными в прототипе функции, всегда является наилучшим соответствием. Последовательности тривиальных преобразований классифицируются как точные соответствия. Однако последовательности, не выполняющие никакие из этих преобразований, считаются лучше, чем последовательности, осуществляющие следующие преобразования:  
    ...
    -   Из указателя в ссылку на **const** (`type`  **&**  для **const** `type`  **&** ).  
  
    -   Из указателя в ссылку на `volatile` (`type`  **&**  для `volatile` `type`  **&** ).  
...
Граф, иллюстрирующий предпочтительные преобразования  
 ...
 ![Multi&#45;наследование, показывающее предпочтительные преобразования](../cpp/media/vc391t2.gif "vc391T2")  
...
```  
 ...
 Доступные преобразования определяемого пользователем класса `UDC` : из типа `int` и тип **длинные**. Поэтому компилятор проверяет преобразования для типа сопоставляемого объекта: `UDC`. Преобразование в `int` существует и оно выбирается.  
...
```  
 ...
 В предыдущем примере, определенное пользователем преобразование **long-оператор**, вызывается для преобразования `udc` ввода **длинные**. Если нет определенное пользователем преобразование в тип **длинные** было определено, преобразование продолжилось бы следующим образом: тип `UDC` был бы преобразован в тип `int` с использованием определенного пользователем преобразования. Затем стандартное преобразование из типа `int` ввода **длинные** было бы выполнено в соответствии с аргументом в объявлении.  
...
-   Преобразование из типа `int` ввода **длинные**; затем преобразование в тип `UDC2` (двухступенчатое преобразование).  
...
## <a name="argument-matching-and-the-this-pointer"></a>Сопоставление аргументов и указатель this  
 ...
 Когда `->` оператора выбора члена используется для доступа к функции-члена класса `class_name`, `this` аргумент-указатель имеет тип `class_name * const`. Если элементы объявлены как `const` или `volatile`, типы, `const class_name * const` и `volatile class_name * const`соответственно.  
