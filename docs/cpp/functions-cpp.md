...
Если функция изменяет аргумент, передаваемый по ссылке, изменяется исходный объект, а не его локальная копия. Чтобы предотвратить изменение такого аргумента функции, следует определить как const &:

```cpp
void DoSomething(const std::string& input){...}
```

**C++ 11:**  Для явной обработки аргументов, которые передаются по ссылке rvalue или ссылку lvalue, используйте двойной амперсанд в параметре, чтобы указать универсальную ссылку:
...
"Обычные" возвращаемые типы расположены слева от сигнатуры функции. Объект *завершающего возвращаемого типа* находится в правой части подписи и предшествует оператор ->. Завершающие возвращаемые типы особенно полезны в шаблонах функций, когда тип возвращаемого значения зависит от параметров шаблона.
