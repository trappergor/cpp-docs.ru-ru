...
**Visual Studio 2017 версии 15.3 и более поздние версии** (состав [/std: c ++ 17](../build/reference/std-specify-language-standard-version.md)): В шаблонах функций, можно использовать `constexpr if` инструкцию, чтобы принимать решения ветвления во время компиляции, не приходится прибегать к нескольким перегрузок функций. Например можно написать одну функцию параметра, дескрипторы, распаковки, (ни одной перегрузке не нулевой параметр является обязательным): 
