...
-   **/CLR** или **/CLR: pure** параметр компилятора и `catch` инструкции могут передавать исключения C++. **/CLR** параметры компилятора определяют спецификацию **/EHa** параметр. Обратите внимание, что компилятор не поддерживает передачу управляемых исключений. Это, поскольку управляемые исключения, которые являются производными от [класса System.Exception](../standard-library/exception-class.md), уже являются объектами, которые можно перемещать между потоками с помощью средства languange среды выполнения.  
