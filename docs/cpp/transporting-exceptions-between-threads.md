...
Функция `make_exception_ptr` возвращает объект `exception_ptr`, который ссылается на исключение, заданное параметром *e* .
...
Вызовите функцию `current_exception` в блоке **catch** . Если исключение находится в полете, а блок **catch** может перехватить исключение, функция `current_exception` возвращает объект `exception_ptr`, который ссылается на исключение. В противном случае функция возвращает объект `exception_ptr` со значением null.
...
При использовании параметра компилятора **/EHa** можно перехватывать исключение SEH в C++ блоке **catch** . Функция `current_exception` возвращает объект `exception_ptr`, который ссылается на исключение SEH. Функция `rethrow_exception` создает исключение SEH, если он вызывается с объектом сетранспортед `exception_ptr` в качестве аргумента.

Функция `current_exception` возвращает значение NULL `exception_ptr` при вызове в обработчике завершения **__finally** SEH, обработчике исключений **__except** или в выражении фильтра **__except** .
...
Вызов функции `make_exception_ptr` эквивалентен созданию C++ исключения, его перехвату в блоке **catch** и последующему вызову функции `current_exception` для возврата объекта `exception_ptr`, который ссылается на исключение. Реализация Майкрософт для функции `make_exception_ptr` является более эффективной, чем создание и последующий перехват исключения.
