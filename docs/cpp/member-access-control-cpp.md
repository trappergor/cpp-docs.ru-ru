...
ms.locfileid: "81369864"
---
# <a name="member-access-control-c"></a>Управление доступом к членам (C++)

Элементы управления доступом позволяют отделить [общедоступный](../cpp/public-cpp.md) интерфейс класса от [частных](../cpp/private-cpp.md) деталей реализации и [защищенных](../cpp/protected-cpp.md) членов, которые используются только производными классами. Спецификатор доступа действует для всех членов, объявленных после него, пока не будет объявлен следующий спецификатор доступа.
...
Доступ по умолчанию является **частным** в классе и **общедоступным** в структуре или союзе. Спецификаторы доступа класса могут использоваться любое количество раз и в любом порядке. Выделение хранилища для объектов типов классов зависит от реализации, но членам гарантировано присваиваются старшие адреса памяти, расположенные подряд между описателями доступа.

## <a name="member-access-control"></a>Управление доступом к членам

|Тип доступа|Значение|
|--------------------|-------------|
|[Частная](../cpp/private-cpp.md)|Члены класса, объявленные **частными,** могут использоваться только функциями членов и друзьями (классами или функциями) класса.|
|[protected](../cpp/protected-cpp.md)|Члены класса, объявленные **защищенными,** могут использоваться функциями членов и друзьями (классами или функциями) класса. Кроме того, они могут использоваться производными классами данного класса.|
|[public](../cpp/public-cpp.md)|Члены класса, объявленные **публичными,** могут использоваться любой функцией.|
...
- Объявляет ли полученный класс базовый класс с помощью **спецификации общедоступного** доступа.
...
Производный класс можно объявить без спецификатора доступа базового класса. В таком случае вывод считается частным, если в полученном классовом заявлении используется ключевое слово **класса.** Вывод считается общедоступным, если в полученном классовом декларировании используется ключевое слово **struct.** Например, приведенный ниже код
...
Обратите внимание, что участники, объявленные частным доступом, недоступны для функций или производных классов, если эти функции или классы не анонсированы с использованием декларации **друзей** в базовом классе.

Тип **соединения** не может иметь базовый класс.

> [!NOTE]
> При указании частного базового класса рекомендуется явно использовать **частное** ключевое слово, чтобы пользователи производного класса понимали доступ к члену.

## <a name="access-control-and-static-members"></a>Управление доступом и статические члены

Когда вы указываете базовый класс как **частный,** он затрагивает только нестатических членов. Открытые статические члены по-прежнему доступны в производных классах. Однако доступ к членам базового класса с помощью указателей, ссылок и объектов может потребовать преобразования, а во время выполнения управление доступом применяется снова. Рассмотрим следующий пример:
...
В предыдущем примере кода управление доступом запрещает преобразование указателя на `Derived2` к указателю на `Base`. **Этот** указатель неявно типа `Derived2 *`. Чтобы выбрать `CountOf` функцию, **она** должна `Base *`быть преобразована в тип. Такое преобразование не поддерживается, поскольку `Base` — это закрытый косвенный базовый класс в `Derived2`. Преобразование к типу закрытого базового класса применимо только для указателей на непосредственные производные классы. Поэтому указатели типа `Derived1 *` можно преобразовать к типу `Base *`.
...
Контроль доступа, применяемый к [виртуальным](../cpp/virtual-cpp.md) функциям, определяется типом, используемым для вызова функции. Переопределение объявлений функции не влияет на управление доступом для данного типа. Пример:
