...
Простейшая форма псевдонима эквивалентна механизму **typedef** из c++ 03:
...
Ограничением механизма **typedef** является то, что оно не работает с шаблонами. Напротив, синтаксис псевдонима типа в C ++11 позволяет создавать шаблоны псевдонимов:
...
В следующем примере показано, как использовать шаблон псевдонима с пользовательским механизмом выделения памяти — в данном случае целочисленным векторным типом. Можно заменить любой тип на **int** , чтобы создать удобный псевдоним для скрытия сложных списков параметров в основном функциональном коде. Применяя по всему коду пользовательские механизмы выделения памяти, можно повысить удобочитаемость и уменьшить риск возникновения ошибок, связанных с опечатками.
...
Объявление **typedef** вводит имя, которое в пределах его области видимости преобразуется в синоним для типа, заданного в объявлении *типа* .
...
В отличие от объявлений **классов**, **структур**, **объединений**и **перечислений** , объявления **typedef** не вводит новые типы — они представляют новые имена для существующих типов.
...
Одно из использования объявлений **typedef** заключается в том, чтобы сделать объявления более однородными и компактными. Пример:
...
После приведенного выше оператора **typedef** объявление
...
**typedef** часто сочетается с **struct** для объявления и именования определяемых пользователем типов:
...
Объявление **typedef** можно использовать для повторного объявления того же имени для ссылки на один и тот же тип. Пример:
...
Программа *Prog. CPP* включает два файла заголовков, которые содержат объявления **typedef** для имени `CHAR`. Если в обеих объявлениях указывается один и тот же тип, такое повторное объявление допустимо.

**Typedef** не может переопределить имя, которое ранее было объявлено как другой тип. Таким образом, если *file2. H* содержит
...
### <a name="typedefs-in-c-vs-c"></a>определения типов в C++ vs. C

Использование описателя **typedef** с типами классов поддерживается в основном из-за объявления неименованных структур в объявлениях **typedef** в ANSI C. Например, многие программисты C используют следующий код.
...
В C++различия между именами **typedef** и реальными типами (объявленными с ключевыми словами **Class**, **struct**, **Union**и **enum** ) более различны. Хотя методика C объявления структуры без имени в операторе **typedef** по-прежнему работает, она не предоставляет никаких преимуществ для нотаций, как это делается на языке c.
...
- Имя (синоним) не может использоваться после префикса **класса**, **структуры**или **объединения** .
