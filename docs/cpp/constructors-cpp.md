---
title: "Конструкторы (C++) | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
...
-   [Списки членов](../cpp/constructors-cpp.md#member_lists)  
  
-   [Явные конструкторы](../cpp/constructors-cpp.md#explicit_constructors)  
  
-   [Конструкторы по умолчанию](../cpp/constructors-cpp.md#default_constructors)  
  
-   [Конструкторы копии и перемещения](../cpp/constructors-cpp.md#copy_and_move_constructors)  
  
-   [Явно заданные и удаленные конструкторы](../cpp/constructors-cpp.md#explicitly_defaulted_and_deleted_constructors)  
  
-   [Конструкторы в производных классах](../cpp/constructors-cpp.md#constructors_in_derived_classes)  
  
-   [Конструкторы классов с множественным наследованием](../cpp/constructors-cpp.md#constructors_for_classes_that_have_multiple_inheritance)  
  
-   [Виртуальные функции в конструкторах](../cpp/constructors-cpp.md#virtual_functions_in_constructors)  
  
-   [Конструкторы и составные классы](../cpp/constructors-cpp.md#constructors_in_composite_classes)  
  
-   [Делегирующие конструкторы](../cpp/constructors-cpp.md#delegating_constructors)  
  
-   [Наследование конструкторов (C++11)](../cpp/constructors-cpp.md#inheriting_constructors)  
  
-   [Правила объявления конструкторов](../cpp/constructors-cpp.md#rules_for_declaring_constructors)  
  
-   Конструктор по умолчанию и конструктор копии  
  
-   [Явно вызываемые конструкторы](../cpp/constructors-cpp.md#explicitly_invoking_constructors)  
...
```  
 ...
 Когда конструктор является явным, эта строка вызывает ошибку компилятора: `ShippingOrder so(42, 10.8);`.  Дополнительные сведения см. в разделе [Преобразования](../cpp/user-defined-type-conversions-cpp.md).  
  
##  <a name="default_constructors"></a> Конструкторы по умолчанию  
 *Конструкторы по умолчанию* не имеют параметров и подчиняются несколько иным правилам:  
  
 Конструкторы по умолчанию являются *специальными функциями\-членами*. Если в классе не объявлены конструкторы, компилятор создает конструктор по умолчанию:  
...
```  
 ...
 Это пример проблемы Most Vexing Parse \(наиболее неоднозначного анализа\).  Поскольку выражение примера можно интерпретировать как объявление функции или как вызов конструктора по умолчанию и в связи с тем, что средства синтаксического анализа C\+\+ отдают предпочтение объявлениям перед другими действиями, данное выражение обрабатывается как объявление функции.  Дополнительные сведения см. в статье Википедии [Most Vexing Parse \(Наиболее неоднозначный анализ\)](http://en.wikipedia.org/wiki/Most_vexing_parse).  
 ...
 *Конструктор копии* — это специальная функция\-член, принимающая в качестве входных данных ссылку на объект того же типа и создающая его копию.  Дополнительные сведения см. в разделе [Конструкторы копий и операторы присвоения копий \(C\+\+\)](../cpp/copy-constructors-and-copy-assignment-operators-cpp.md).  Конструктор перемещений — это тоже специальная функция\-член, передающая право на владение текущим объектом новой переменной без копирования исходных данных.  Дополнительные сведения см. в разделах [Конструкторы перемещений и операторы присваивания перемещения \(C\+\+\)](http://msdn.microsoft.com/ru-ru/1442de5f-37a5-42a1-83a6-ec9cfe0414db) и [Конструкторы move и операторы присваивания move \(C\+\+\)](../Topic/Move%20Constructors%20and%20Move%20Assignment%20Operators%20\(C++\).md).  
  
##  <a name="explicitly_defaulted_and_deleted_constructors"></a> Явно заданные и удаленные конструкторы  
 Можно явно создавать конструкторы копии по умолчанию, конструкторы по умолчанию, конструкторы перемещения, операторы присваивания копий, операторы присваивания перемещения и деструкторы.  Вы можете явно удалить все специальные функции\-члены.  Дополнительные сведения см. в разделе [Явно используемые по умолчанию и удаленные функции](../Topic/Explicitly%20Defaulted%20and%20Deleted%20Functions.md).  
...
##  <a name="rules_for_declaring_constructors"></a> Правила объявления конструкторов  
 Конструктор имеет то же имя, что и класс.  Можно объявить любое количество конструкторов в соответствии с правилами перегруженных функций.  \(Дополнительные сведения см. в разделе [Перегрузка](../misc/overloading-cpp.md).\)  
...
|Тип конструктора|Аргументы|Назначение|  
|----------------------|---------------|----------------|  
...
|Конструктор копии|Может принимать один аргумент ссылки на тот же тип класса|Копирование объектов типа класса|  
 ...
 Если все базовые классы и классы\-члены класса `type` имеют конструкторы копий, принимающие аргумент **const**, созданный компилятором конструктор копий принимает один аргумент типа **const** `type`**&**.  В противном случае созданный компилятором конструктор копии принимает один аргумент типа `type`**&**.  
 ...
 Соглашение о вызовах stdcall используется для статических функций\-членов и глобальных функций, объявленных с ключевым словом **\_\_stdcall**, в которых не используется переменное число аргументов.  Если ключевое слово **\_\_stdcall** используется в нестатической функции\-члене, например в конструкторе, компилятор использует соглашение о вызовах thiscall.  
  
 Конструкторы базовых классов не наследуются производными классами.  Объект типа производного класса создается, начиная с компонентов базового класса, а затем перемещается в компоненты производного класса.  Компилятор использует конструктор каждого базового класса при инициализации этой части полного объекта \(за исключением случаев виртуального наследования, описанных в разделе [Инициализация базовых классов](../misc/initializing-base-classes.md)\).  
...
```  
 ...
 Объекты, созданные путем явного вызова конструкторов, как показано в этих двух примерах, не имеют имени и существуют столько же времени, сколько и выражение, в котором они были созданы.  Дополнительные сведения о них см. в разделе [Временные объекты](../cpp/temporary-objects.md).  
