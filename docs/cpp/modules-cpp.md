---
title: Общие сведения о модулях вC++
ms.date: 07/23/2019
helpviewer_keywords:
- modules [C++]
- modules [C++], overview
...
ms.locfileid: "68537810"
---
# <a name="overview-of-modules-in-c"></a>Общие сведения о модулях вC++
...
В Visual Studio 2019 версии 16,2 модули не полностью реализованы в компиляторе Майкрософт C++ . С помощью функции "модули" можно создавать модули с одной секцией и импортировать модули стандартной библиотеки, предоставляемые корпорацией Майкрософт. Чтобы включить поддержку модулей, скомпилируйте их с `/experimental:modules` помощью `/std:c++latest`и. В проекте Visual Studio щелкните правой кнопкой мыши узел проекта в **Обозреватель решений** и выберите пункт **свойства**. Задайте в раскрывающемся списке **Конфигурация** значение **все конфигурации**, а затем выберите **Свойства** > конфигурации**CC++/**  > **Language** >  **C++ включить модули ( экспериментальный)** .
...
- STD. Regex предоставляет содержимое регулярных выражений \<заголовков >
- STD. FileSystem предоставляет содержимое файловой системы заголовков \<>
- STD. Memory предоставляет содержимое > памяти заголовка \<
- STD. Threading предоставляет содержимое заголовков \<атомарных > \<, CONDITION_VARIABLE > \<, будущих > \<, мьютексов \<>, shared_mutex > \<и Thread >
- STD. Core предоставляет все остальное в C++ стандартной библиотеке

Чтобы использовать эти модули, просто добавьте оператор import в начало файла исходного кода. Например:
...
Чтобы использовать модуль стандартной библиотеки Майкрософт, необходимо скомпилировать программу с параметрами [/EHsc](../build/reference/eh-exception-handling-model.md) и [/MD](../build/reference/md-mt-ld-use-run-time-library.md) .

## <a name="basic-example"></a>Простой пример

В следующем примере показано простое определение модуля в исходном файле с именем **foo. икскс**. Расширение **. икскс** требуется для файлов интерфейса модуля в Visual Studio. В этом примере файл интерфейса содержит определение функции, а также объявление. Однако определения можно также поместить в один или несколько отдельных файлов (как показано в следующем примере). Инструкция **Export Module foo** указывает, что этот файл является основным интерфейсом для модуля с именем `Foo`. `Foo` Модификатор **экспорта** указывает, что эта функция будет видима при импорте другой программой или модулем. `f()` Обратите внимание, что модуль ссылается `Bar`на пространство имен.
...
Файл **MyProgram. cpp** использует инструкцию **Import** для доступа к имени, экспортируемому `Foo`. Обратите внимание, `Bar` что имя отображается здесь, но не все его члены. Также обратите внимание, `ANSWER` что макрос не отображается.
