...
||`long` `long`|Больше, чем unsigned `long`.<br /><br /> Объекты типа `long long` могут объявляться как объекты типа `signed` `long long` или `unsigned` `long long`. `signed` `long long` является синонимом для `long long`.|  
