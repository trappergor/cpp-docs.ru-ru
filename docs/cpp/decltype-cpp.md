...
В C++ 11 можно использовать описатель **decltype** на завершающем типе возвращаемого значения вместе с ключевым словом **auto** для объявления функции-шаблона, тип возвращаемого значения которой зависит от типов ее шаблонных аргументов. Рассмотрим это в следующем примере кода, где заполнитель *UNKNOWN* показывает, что указать тип возвращаемого значения невозможно.
