---
title: Инициализаторы | Документация Майкрософт
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-language
...
### <a name="default_initialization"></a> Инициализация по умолчанию  
 Инициализация по умолчанию для классов, структур и объединений — это инициализация с помощью конструктора по умолчанию. Конструктор по умолчанию может вызываться без выражения инициализации или с **новый** ключевое слово:  
...
-   объект инициализируется с **новый** ключевое слово и пустых скобок или фигурных скобок  
...
-   Класс, структура и члены объединения инициализируются с применением инициализации путем копирования во время агрегатной инициализации. См. в разделе [агрегатной инициализации](#agginit) примеры.  
...
-   переменная инициализируется с **новый** ключевое слово, а также непустой круглых или фигурных скобок  
  
-   переменная инициализируется с **static_cast**  
...
-   Класс инициализируется с помощью **новый** ключевое слово  
...
> [!NOTE]
>  <!--conformance note-->В Visual Studio 2015 и более ранних версиях статистическое выражение не должна содержать инициализаторы фигурной скобкой или равенства для нестатических членов. Это ограничение был удален в стандарте C ++ 14 и реализации в Visual Studio 2017. 
...
> [!IMPORTANT]
>  Члены массива, объявленные, но не инициализированные явно во время агрегатной инициализации инициализируются с нуля, как показано на `myArr3` выше.  
...
-   Объявление переменной, явно указан как **extern**. Пример:  
...
Граф принятия решений для инициализации ссылочных типов  
 ...
 Ссылки на **volatile** типы (объявленные как **volatile** *typename *** &** *идентификатор*) могут быть инициализированы с помощью **volatile** объектов одного типа или с объектами, которые не были объявлены как **volatile**. Они не может, тем не менее, инициализировать с помощью **const** объектов этого типа. Аналогично, ссылки на **const** типы (объявленные как **const** *typename *** &** *идентификатор*) может быть инициализируется **const** объекты одного типа (или все, что имеется преобразование к этому типу или с объектами, которые не были объявлены как **const**). Они не может, тем не менее, инициализировать с помощью **volatile** объектов этого типа.  
