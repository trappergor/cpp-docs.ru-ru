---
title: "Инициализаторы | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "инициализаторы элемента массива"
  - "массивы [C++], инициализаторы элемента массива"
  - "деклараторы, как инициализаторы"
  - "инициализаторы, элемент массива"
  - "инициализация массивов, инициализаторы"
...
-   Члены массивов, классов [POD](../standard-library/is-pod-class.md), структур и объединений инициализируются нулевым значением.  
...
```  
 ...
 Дополнительные сведения об инициализации глобальных статических объектов см. в статье [Дополнительные сведения о запуске](../cpp/additional-startup-considerations.md).  
...
-   Класс, структура и члены объединения инициализируются с применением инициализации путем копирования во время агрегатной инициализации.  Примеры см. в разделе [Агрегатная инициализация](../cpp/initializers.md#INIT_AggInit).  
...
```  
 ...
 В некоторых случаях, если конструктор копии класса удален или недоступен, копируемая инициализация вызывает ошибку компилятора.  Дополнительные сведения см. в статье [Явная инициализация](http://msdn.microsoft.com/ru-ru/c89724f8-ddd3-4d77-b86d-77fcd8bd8595).  
...
> [!IMPORTANT]
>  Члены массива, объявленные, но не инициализированные явно во время агрегатной инициализации, инициализируются нулевым значением, как в `myArr3` \(см. выше\).  
...
-   Объявление переменной, явно указанной как `extern`.  Пример:  
 ...
 ![Графы решений для инициализации типов ссылок](../cpp/media/vc38s71.png "vc38S71")  
Граф принятия решений для инициализации ссылочных типов  
  
 Ссылки на типы `volatile` \(объявленные как `volatile` *имя\_типа* **&** *идентификатор*\) можно инициализировать с помощью объектов `volatile` того же типа или с помощью объектов, которые не были объявлены как `volatile`.  Однако их невозможно инициировать с помощью объектов **const** этого типа.  Аналогично, ссылки на типы **const** \(объявленные как **const** *имя\_типа* **&** *идентификатор*\) можно инициализировать с помощью объектов **const** того же типа \(а также с помощью любых объектов, которые могут быть преобразованы в этот тип, или с помощью объектов, которые не были объявлены как **const**\).  Однако их невозможно инициировать с помощью объектов `volatile` этого типа.  
...
### Инициализация внешних переменных  
 Объявления автоматических, регистровых, статических и внешних переменных могут содержать инициализаторы.  Однако объявления внешних переменных могут содержать инициализаторы, только если эти переменные не объявлены как `extern`.  Дополнительные сведения см. в статье [Внешнее](../misc/external.md).  
  
## См. также  
 [\(NOTINBUILD\)Declarators](http://msdn.microsoft.com/ru-ru/8a7b9b51-92bd-4ac0-b3fe-0c4abe771838)