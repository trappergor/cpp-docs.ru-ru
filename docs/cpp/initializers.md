...
Дополнительные сведения об инициализации глобальных статических объектов см. в разделе [Дополнительные рекомендации](../cpp/additional-startup-considerations.md)по запуску.
...
> [!IMPORTANT]
> Элементы массива, объявляемые, но не инициализированные во время инициализации агрегата, инициализируются нулем, как `myArr3` показано выше.
...
Ссылки на **временные** типы (объявленные как **переменные** идентификаторы *TypeName* <strong>&</strong> ) можно инициализировать с помощью **переменных с постоянными** объектами того же типа или с объектами, которые не были объявлены как **volatile** . . Однако они не могут быть инициализированы с помощью **константных** объектов этого типа. Аналогичным образом, ссылки на типы **const** ( объявленные как константные *идентификаторы* *TypeName* <strong>&</strong> ) можно инициализировать с помощью объектов **const** одного типа (или любого объекта, который имеет преобразование в этот тип или с объектами , которые не объявлены как **const**. Однако они не могут быть инициализированы с помощью **переменных** с постоянными объектами этого типа.
