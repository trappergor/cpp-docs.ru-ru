...
ms.date: 07/29/2019
description: Инициализация классов, структур, массивов и фундаментальных типов в C++.
helpviewer_keywords:
...
### <a name="default-initialization"></a><a name="default_initialization"></a>Инициализация по умолчанию
...
### <a name="list-initialization"></a>Инициализация списка
...
### <a name="aggregate-initialization"></a><a name="agginit"></a>Агрегатная инициализация
...
Ссылки на **`volatile`** типы (объявленные как **`volatile`** *typename* <strong>&</strong> *идентификатор*TypeName) могут быть инициализированы **`volatile`** объектами того же типа или с объектами, которые не были объявлены как **`volatile`** . Однако они не могут быть инициализированы **`const`** объектами этого типа. Аналогичным образом, ссылки на **`const`** типы (объявленные как **`const`** *typename* <strong>&</strong> *идентификатор*TypeName) можно инициализировать с помощью **`const`** объектов одного типа (или любого объекта, который имеет преобразование в этот тип или с объектами, которые не были объявлены как **`const`** ). Однако они не могут быть инициализированы **`volatile`** объектами этого типа.
