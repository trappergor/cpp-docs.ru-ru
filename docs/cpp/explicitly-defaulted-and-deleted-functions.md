...
Возможна утечка этих правил в иерархии объектов. Например, если по какой-либо причине базовый класс не имеет конструктора по умолчанию, который можно назвать из производного класса, то есть **общедоступный** или **защищенный** конструктор, который не принимает никаких параметров, то класс, который вытекает из него, не может автоматически генерировать свой собственный конструктор по умолчанию.
...
- Конструктор копии должен быть объявлен в частном порядке, чтобы скрыть его, но, поскольку он объявлен на всех, автоматическое поколение конструктора по умолчанию предотвращается. Если конструктор по умолчанию необходим, он должен быть определен явным образом, даже если он не выполняет никаких действий.
...
Обратите внимание, что вы можете по умолчанию специальную функцию члена вне тела класса до тех пор, пока это inlinable.
...
Для того чтобы специальные функции-члены, а также обычные функции-члены и функции, не являющиеся членами, не могли определяться или вызываться, их можно удалить. Удаляние специальных функций члена обеспечивает более чистый способ предотвращения создания компилятора специальными функциями-членами, которые вы не хотите. Функция должна удаляться сразу после объявления, а не позже (в отличие от объявления по умолчанию, которое можно выполнять позже объявления).
...
Обратите внимание в предыдущем `call_with_true_double_only` примере, что вызов с помощью `call_with_true_double_only` аргумента **поплавка** вызовет ошибку компилятора, но вызов с помощью аргумента **int** не будет; в **int** случае, аргумент будет повышен от **Int** к **двойному** и успешно вызвать **двойную** версию функции, даже если это не может быть то, что предназначено. Чтобы убедиться, что любой вызов этой функции с помощью недвойного аргумента вызывает ошибку компилятора, можно объявить шаблонную версию удаленной функции.
