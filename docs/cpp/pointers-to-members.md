...
1. Спецификатор объявления:
   ...
   - Необязательные описатели **const** и/или **volatile** .
...
В предыдущем примере `pwCaption` является указателем на любой член класса `Window`, имеющий тип `char*`. `pwCaption` имеет тип `char * Window::* `. В следующем фрагменте кода объявляются указатели на функции-члены `SetCaption` и `GetCaption`.
...
Разница между **.** операторы <strong>\*</strong> и **->** <strong>\*</strong> (операторы указателей на члены) — это то, что **.** <strong>\*</strong> оператор выбирает элементы по ссылке на объект или объект, а оператор **->** <strong>\*</strong> выбирает элементы с помощью указателя. (Дополнительные сведения об этих операторах см. [в разделе выражения с операторами указателей на члены](../cpp/pointer-to-member-operators-dot-star-and-star.md).)

Результатом операторов указателя на член является тип элемента, в данном случае `char *`.
...
Адрес статического члена не является указателем на член. Это обычный указатель на один экземпляр статического члена. Поскольку для всех объектов данного класса существует только один экземпляр статического элемента, можно использовать обычные операторы address-of ( **&** ) и разыменование (<strong>\*</strong>).
