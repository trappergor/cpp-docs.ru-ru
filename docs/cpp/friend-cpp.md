...
Функции, объявленные в дружественном объявлении, обрабатываются так, как если бы они были объявлены с помощью ключевого слова **extern** . Дополнительные сведения см. в разделе [extern](extern-cpp.md).
...
В предыдущем примере в области действия вводится имя класса `ForwardDeclared`, но полное объявление — в частности, часть, в которой объявляется функция `IsAFriend`, — отсутствует. Поэтому при объявлении **дружественного** класса в классе `HasFriends` возникает ошибка.
...
Используйте `class friend F`, если ссылочный тип еще не объявлен:
...
В следующем примере `friend F` ссылается на класс `F`, который объявлен вне области видимости NS.
...
Используйте `friend F`, чтобы объявить параметр шаблона дружественным:
...
**Дружественная** функция — это функция, которая не является членом класса, но имеет доступ к закрытым и защищенным членам класса. Дружественные функции не считаются членами класса; это обычные внешние функции с особыми правами доступа. Друзья не находятся в области класса и не вызываются с помощью операторов выбора членов ( **.** и- **>** ), если они не являются членами другого класса. **Дружественная** функция объявляется классом, который предоставляет доступ. **Дружественное** объявление можно поместить в любое место объявления класса. На него не влияют ключевые слова управления доступом.

В следующем примере показан класс `Point` и дружественная функция `ChangePrivate`. **Дружественная** функция имеет доступ к закрытым членам данных объекта `Point`, который он получает в качестве параметра.
...
В предыдущем примере дружественный доступ к классу `A::Func1( B& )` предоставляется только функции `B`. Таким образом, доступ к закрытому члену `_b` правильный в `Func1` класса `A`, но не в `Func2`.
...
![Влияние дружественных отношений](../cpp/media/vc38v41.gif "Последствия дружественных отношений") <br/>
