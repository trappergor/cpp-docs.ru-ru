---
title: Объявление using | Документация Майкрософт
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-language
...
ms.locfileid: "46053990"
---
# <a name="using-declaration"></a>Объявление using

Объявление using вводит имя в ту область объявления, в котором отображается объявление using.
...
*вложенные name-specifier* последовательность пространства имен, класса или перечисления имена и операторы разрешения области действия (:), заканчиваться точкой оператора разрешения области действия. Один оператор разрешения области действия может использоваться для вводят имя из глобального пространства имен. Ключевое слово **typename** является необязательным и может использоваться для разрешения имен зависимых наличие в шаблоне класса от базового класса.

*неполное идентификатор* неполное идентификатор выражение, которое может иметь идентификатор, имя перегруженного оператора, определяемые пользователем литерал оператор или преобразование имени функции, имени деструктор класса или шаблона имя и список аргументов.

*Список деклараторов* список с разделителями запятыми [**typename**] *вложенные name-specifier* *неполное идентификатор* деклараторов, а затем при необходимости по кнопку с многоточием.

## <a name="remarks"></a>Примечания

Объект объявление using вводит неполное имя как синоним для объекта, объявленные в другом месте. Она позволяет одно имя из определенного пространства имен, использоваться без явной квалификации в области объявления, в котором он находится. Это отличается от [директива using](../cpp/namespaces-cpp.md#using_directives), что позволяет *все* имена в пространство имен для использования без уточнения. **С помощью** ключевое слово также используется для [псевдонимы типов](../cpp/aliases-and-typedefs-cpp.md).
...
Члены, объявленные с помощью объявления можно ссылаться с помощью явной квалификации. Префикс `::` указывает на глобальное пространство имен.
...
Имя, определенное при **с помощью** объявление является псевдонимом для исходного имени. Оно не влияет на тип, компоновку и другие атрибуты исходного объявления.
