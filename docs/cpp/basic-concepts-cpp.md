---
title: Основные понятия (C++) | Документация Майкрософт
ms.custom: index-page
ms.date: 11/04/2016
ms.technology:
- cpp-language
ms.topic: conceptual
dev_langs:
- C++
helpviewer_keywords:
- C++, basic language concepts
ms.assetid: 961801e6-2ffd-4bf1-bb71-7f55e48d9c79
author: mikeblome
ms.author: mblome
ms.workload:
- cplusplus
ms.openlocfilehash: ac218913864944c4a0d2cbd973a796cf38edc3a7
ms.sourcegitcommit: 913c3bf23937b64b90ac05181fdff3df947d9f1c
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 09/18/2018
ms.locfileid: "46056590"
---
# <a name="basic-concepts--c"></a>Основные понятия (C++)

В этом разделе рассматриваются понятия, которые абсолютно необходимы для понимания C++. Программистам, работающим с языком C, большинство из них уже знакомы, однако ряд незначительных отличий в них могут привести к тому, что программы будут порождать непрогнозируемый результат. В раздел входят следующие темы:

- [Объявления и определения](../cpp/declarations-and-definitions-cpp.md)

- [Область объекта C++ или функции](../cpp/scope-visual-cpp.md)

- [Правила определения и компоновки программы](../cpp/program-and-linkage-cpp.md)

- [Запуск и завершение](../cpp/startup-and-termination-cpp.md)

- [L-значений и r значения](../cpp/lvalues-and-rvalues-visual-cpp.md)

- [Временные объекты](../cpp/temporary-objects.md)

- [Выравнивание](../cpp/alignment-cpp-declarations.md)

- [alignof и alignas](../cpp/alignof-and-alignas-cpp.md)

- [Тривиальные, стандартная раскладка и типы POD](../cpp/trivial-standard-layout-and-pod-types.md)

## <a name="see-also"></a>См. также

[Справочник по языку C++](../cpp/cpp-language-reference.md)