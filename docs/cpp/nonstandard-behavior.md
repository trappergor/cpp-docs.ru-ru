...
ms.locfileid: "65222026"
---
# <a name="nonstandard-behavior"></a>Нестандартное поведение

В следующих разделах перечислены некоторые ситуации, где в реализации корпорацией Майкрософт C++ не соответствует C++ standard. Приведенные ниже номера разделов соответствуют номерам разделов в стандарте C++ 11 (ISO/IEC 14882:2011(E)).
...
Microsoft C++ компилятора сейчас не поддерживает привязку независимых имен при первоначальном синтаксическом анализе шаблона. Это не соответствует разделу 14.6.3 спецификации C++ ISO. Это может привести к тому, что перегруженные версии, объявленные после шаблона (но до создания его экземпляра), будут видны.
...
Стандарт языка C++ (раздел 1.8, пункт 6) требует, чтобы полные объекты C++ имели уникальные расположения хранения. Тем не менее с помощью Microsoft C++, бывают случаи, где типы без членов данных будут совместно использовать хранилище, доступном с другими типами в течение времени существования объекта.