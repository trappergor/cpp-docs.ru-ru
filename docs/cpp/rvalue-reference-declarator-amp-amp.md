...
Можно также перегрузить обычные функции и операторы, чтобы воспользоваться преимуществами семантики перемещения. Visual Studio 2010 была введена семантика перемещения в C++ стандартной библиотеки. Например, класс `string` реализует операции, использующие семантику перемещения. Рассмотрим следующий пример, в котором объединяются несколько строк и выводится результат:
...
Дополнительные сведения о введении семантики перемещения в C++ стандартной библиотеки в Visual Studio 2010, см. в разделе [ C++ Стандартная библиотека](../standard-library/cpp-standard-library-reference.md).
