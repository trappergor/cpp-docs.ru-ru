...
Дополнительные сведения о поведении семантики перемещения в C++ стандартную библиотеку в Visual Studio 2010 см. в разделе [ C++ Стандартная библиотека](../standard-library/cpp-standard-library-reference.md).
...
Функция C++ стандартной библиотеки [std:: Move](../standard-library/utility-functions.md#move) позволяет преобразовать объект в ссылку rvalue на этот объект. Кроме того, можно использовать ключевое слово **static_cast** для приведения левостороннего значения к ссылке rvalue, как показано в следующем примере:
