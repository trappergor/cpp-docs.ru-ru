...
> [!NOTE]
> В C++ 17 и более поздних версиях класс **std:: Variant** является строго типизированной альтернативой для объединений.

**Объединение** — это определяемый пользователем тип, в котором все члены совместно используют одно и то же расположение в памяти. Это означает, что в любой момент времени объединение не может содержать больше одного объекта из списка своих членов. Независимо от количества членов объединения, оно использует лишь количество памяти, необходимое для хранения своего крупнейшего члена.
...
Начните объявление объединения с помощью ключевого слова **Union** и заключите список членов в фигурные скобки:
...
В дополнение к ограничениям именованных объединений анонимное объединение подчиняется следующим дополнительным ограничениям:

- Они также должны быть объявлены как **статические** , если они объявлены в области файла или пространства имен.

- Они могут иметь только **открытые** члены; **закрытые** и **защищенные** члены в анонимных объединениях создают ошибки.
