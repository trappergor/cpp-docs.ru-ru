...
```  
 ...
 Имя поиске зависимых имен проверяются имена из контекста определения шаблона, в следующем примере этот контекст осуществляет поиск `myFunction(char)`— и контекст экземпляра шаблона. В следующем примере экземпляр шаблона создается в основной части кода; Таким образом `MyNamespace::myFunction` видна с момента создания экземпляра и выбирается как более подходящее сопоставление. В случае переименования функции `MyNamespace::myFunction` вместо нее будет вызываться функция `myFunction(char)`.  
