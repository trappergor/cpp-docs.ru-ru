 ...
 **Оператор delete** функция является статической, поэтому не может быть виртуальной. `operator delete` Функция подчиняется контроля доступа, как описано в разделе [управления доступом к членам](../cpp/member-access-control-cpp.md).  
