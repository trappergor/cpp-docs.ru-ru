...
```  
 ...
 Обратите внимание на использование [GetExceptionCode](http://msdn.microsoft.com/library/windows/desktop/ms679356) для определения исключения. Эту функцию необходимо вызывать внутри фильтра. `Eval_Exception` не удается вызвать `GetExceptionCode`, но он должен иметь код исключения, переданного ему.  
