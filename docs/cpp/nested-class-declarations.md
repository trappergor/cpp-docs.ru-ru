...
`BufferedIO::BufferedInput`и `BufferedIO::BufferedOutput` объявляются `BufferedIO`в пределах . Эти имена классов не видимы за пределами области класса `BufferedIO`. Однако объект типа `BufferedIO` не содержит объекты типа `BufferedInput` или `BufferedOutput`.
...
В предыдущем примере для декларации имени функции используется синтаксис *с имени квалифицированного типа.* Объявление:

```cpp
BufferedIO::BufferedInput::read()
```

означает "функция `read`, которая является членом класса `BufferedInput`, который находится в области класса `BufferedIO`". Поскольку в этой декларации используется синтаксис *с имени квалифицированного типа,* возможны конструкции следующей формы:
...
Предыдущая декларация эквивалентна предыдущей, но вместо имен класса используется имя **typedef.**
