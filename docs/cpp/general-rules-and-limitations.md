...
- Если один модуль в программе содержит объявления **DllImport** и **dllexport** для одной и той же функции или объекта, атрибут **dllexport** имеет приоритет над атрибутом **DllImport** . Однако компилятор создает предупреждение. Пример:
