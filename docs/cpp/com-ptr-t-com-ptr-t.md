...
#### Параметры  
 ...
 `fAddRef`  
 Если имеет значение **true**, то вызывается метод `AddRef`, чтобы увеличить счетчик ссылок инкапсулированного указателя на интерфейс.  
 ...
 *lpcStr*  
 Многобайтовая строка, которая содержит либо **CLSID** \(начинается с символа "**{**"\), либо **ProgID**.  
  
 `pOuter`  
 Внешняя неизвестная строка для [агрегирования](http://msdn.microsoft.com/library/windows/desktop/ms686558).  
