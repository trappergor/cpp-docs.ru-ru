...
```  
 ...
 Максимальное значение может быть представлен по типу **float** — 3.402823466E38 — гораздо меньше, чем 1E300. Таким образом число преобразуется в бесконечность, и результатом является «inf».  
...
### <a name="pointer-to-classes"></a>Указатель на классы  
 ...
 ![Граф наследования, показывающий базовый&#45;класса специальных](../cpp/media/vc38xa1.gif "vc38XA1")  
