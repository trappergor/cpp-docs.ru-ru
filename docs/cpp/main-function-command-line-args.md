---
title: функции main и аргументы командной строки (C++)
description: Функция main является точкой входа для C++ программы.
ms.date: 01/15/2019
...
ms.locfileid: "76123907"
---
# <a name="opno-locmain-function-and-command-line-arguments"></a>функции main и аргументы командной строки

Все C++ программы должны иметь функцию `main`. При попытке скомпилировать C++ *exe* -проект без функции main компилятор вызовет ошибку. (Библиотеки динамической компоновки и библиотеки static не имеют функции `main`.) Функция `main`, где начинается выполнение исходного кода, но до того, как программа войдет в функцию `main`, все члены класса static без явных инициализаторов устанавливаются в ноль. В корпорации C++майкрософт глобальные staticные объекты также инициализируются перед записью `main`. К функции `main`, которая не применяется к другим C++ функциям, применяются некоторые ограничения. Функция `main`:
...
Если возвращаемое значение не указано, компилятор предоставляет возвращаемое значение, равное нулю. Кроме того, функции `main` и `wmain` могут быть объявлены как возвращаемые **void** (без возвращаемого значения). Если вы объявили `main` или `wmain` как возвращаемые **void** , то нельзя вернуть код exit в родительский процесс или операционную систему с помощью оператора [return](../cpp/return-statement-in-program-termination-cpp.md) . Чтобы возвратить код exit, если `main` или `wmain` объявлены как **void** , необходимо использовать функцию [exit](../cpp/exit-function.md) .
...
> [!NOTE]
> По соглашению `argv[0]` — это команда, с помощью которой вызывается программа. Однако можно порождать процесс с помощью [CreateProcess](/windows/win32/api/libloaderapi/nf-libloaderapi-getmodulefilenamew) и при одновременном использовании обоих аргументов (*лпаппликатионнаме* и *лпкоммандлине*) `argv[0]` может не быть именем исполняемого файла; Используйте [GetModuleFileName](/windows/win32/api/libloaderapi/nf-libloaderapi-getmodulefilenamew) , чтобы получить имя исполняемого файла и полный путь к нему.
...
Программа может вызывать `spawn` или `exec` семейства подпрограмм в библиотеке времени выполнения C. Если это так, не следует подавлять подпрограммы обработки среды, так как эта процедура используется для передачи среды из родительского процесса в дочерний процесс.
