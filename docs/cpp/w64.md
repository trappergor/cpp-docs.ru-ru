...
> [!IMPORTANT]
>  [/Wp64](../build/reference/wp64-detect-64-bit-portability-issues.md) параметр компилятора и **__w64** ключевое слово, являются устаревшими в Visual Studio 2010 и Visual Studio 2013 и удалена, начиная с Visual Studio 2013. Если вы используете `/Wp64` параметр компилятора в командной строке, то компилятор выводит [Предупреждение командной строки D9002](https://msdn.microsoft.com/c58b405b-0f26-434e-b57f-4f05e1ca81e6). **__W64** ключевое слово, автоматически игнорируется. Обнаруживать конфликты переносимости на 64-разрядные платформы рекомендуется не при помощи этого параметра и ключевого слова, а при помощи компилятора Visual C++, предназначенного для 64-разрядной платформы. Дополнительные сведения см. в разделе [Настройка Visual C++ для 64-разрядная версия, x64 целевых объектов](../build/configuring-programs-for-64-bit-visual-cpp.md).  
