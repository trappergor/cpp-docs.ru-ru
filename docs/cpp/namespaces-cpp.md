...
Члены именованного пространства имен могут быть определены за пределами пространства имен, в котором они объявляются явной квалификацией определяемого имени. Однако определение должно располагаться после точки объявления в пространстве имен, окружающем то пространство имен, где находится объявление. Например:
...
Все C++ типы и функции стандартных библиотек объявляются в `std` пространстве имен или `std`пространствах имен, вложенных в.
...
Имена пространств имен должны быть уникальными, из-за чего зачастую они получаются не слишком короткими. Если длинное имя оказывается трудночитаемым или его сложно вводить в файле заголовка, где нельзя использовать директиву using, можно создать псевдоним пространства имен, который будет служить в качестве сокращения фактического имени. Например:
