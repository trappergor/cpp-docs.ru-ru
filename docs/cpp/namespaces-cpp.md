...
## <a name="using-directives"></a><a id="using_directives"></a>с помощью директив

**Директива об использовании** позволяет использовать все имена в **пространстве имен** без *имени-имя* в качестве явного квалификатора. Используйте директиву использования в файле реализации (т.е. q.cpp), если вы используете несколько различных идентификаторов в пространстве имен; если вы используете только один или два идентификатора, то рассмотрите использование декларации, чтобы привести эти идентификаторы в область, а не все идентификаторы в пространстве имен. Если локальная переменная имеет такое же имя, как и переменная пространства имен, то переменная пространства имен будет скрытой. Создавать переменную пространства имен с те же именем, что и у глобальной переменной, является ошибкой.
...
Реализация функций в contosodata.cpp должна использовать полностью квалифицированное имя, даже если вы размещаете **директиву использования** в верхней части файла:
...
Если идентификатор не объявлен явно в пространстве имен, он неявно считается входящим в глобальное пространство имен. В общем, старайтесь избегать принятия деклараций в глобальном масштабе, когда это возможно, за исключением [основной функции](../c-language/main-function-and-program-execution.md)точки входа, которая должна быть в глобальном пространстве имен. Чтобы явно указать глобальный идентификатор, используйте оператор разрешения области видимости без имени, как сделано в `::SomeFunction(x);`. Это позволит отличать данный идентификатор от любого другого элемента с таким же именем, находящегося в другом пространстве имен. Кроме того, это облегчит понимание кода.

## <a name="the-std-namespace"></a>Пространство имен std

Все стандартные типы и функции `std` библиотеки C' объявляются `std`в пространстве имен или пространствах имен, вложенных внутри.

## <a name="nested-namespaces"></a>Вложенные пространства имен

Пространства имен могут быть вложенными. Обычное вложенное пространство имен имеет неквалифицированный доступ к членам родительского элемента, но родительские члены не имеют безоговорочного доступа к вложенным пространства имен (если оно не объявлено в качестве вонго), как показано в следующем примере:
...
## <a name="namespace-aliases"></a><a id="namespace_aliases"></a>Псевдонимы именного пространства

Имена пространств имен должны быть уникальными, из-за чего зачастую они получаются не слишком короткими. Если длина имени затрудняет чтение кода или утомительно вводить в файл заголовка, где не могут быть использованы директивы, то можно сделать псевдоним пространства имен, который служит аббревиацией для фактического имени. Пример:
...
Это называется неназванным или анонимным пространством имен, и это полезно, когда вы хотите сделать переменные декларации невидимыми для кода в других файлах (т.е. дать им внутреннюю связь) без создания именного пространства. Весь код, находящийся в том же файле, может видеть идентификаторы в безымянном пространстве имен, но эти идентификаторы, а также само пространство имен, будет невидимым за пределами этого файла или, точнее, вне блока перевода.

## <a name="see-also"></a>См. также раздел

[Декларации и определения](declarations-and-definitions-cpp.md)
