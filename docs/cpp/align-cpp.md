...
Теперь этот тип содержит 32-разрядный атрибут выравнивания. Это означает, что все статические и автоматические экземпляры начинаются с 32-разрядной границы. Дополнительная структура типов, объявленных с этим типом в качестве члена сохранить атрибут выравнивания этого типа, т. е. любая структура с `Str1` , что элемент имеет атрибут выравнивания по крайней мере 32.
...
Теперь `aType` и `bType` одинаковый размер (8 байт), но переменные типа `bType` выравниваются по 32 байтам.
