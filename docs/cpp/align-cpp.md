...
Выравнивание при выделении памяти в куче зависит от того, какая функция выделения вызвана.  Например, если вы используете **malloc**, результат зависит от размера операнда. Если *arg* > = 8, возвращаемая память выравнивается по 8 байтам. Если *arg* < 8, то память выравнивается представляет собой первую степень 2 меньше, чем *arg*. Например, если использовать malloc(7), выравнивание выполняется по 4 байтам.
...
**/Zp** параметр компилятора и **пакет** pragma иметь влияние на упаковку данных для членов структуры и объединения. В этом примере показано, как **/Zp** и `__declspec(align(#))` совместной работы:
...
В следующей таблице перечислены смещение каждого члена в различных **/Zp** (или #pragma **пакет**) значения, показывающий их взаимодействие.
