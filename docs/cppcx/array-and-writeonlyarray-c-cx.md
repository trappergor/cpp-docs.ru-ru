...
ms.locfileid: "70740693"
---
# <a name="array-and-writeonlyarray-ccx"></a>Классы Array и WriteOnlyArray (C++/CX)

Вы можете свободно использовать обычные массивы в стиле C или [std:: Array](../standard-library/array-class-stl.md) в C++программе/CX (хотя обычно используется [std:: Vector](../standard-library/vector-class.md) ), но в любом API, опубликованном в метаданных, необходимо преобразовать массив или вектор в стиле C в [массив Platform:: Array ](../cppcx/platform-array-class.md)или тип [Platform:: WriteOnlyArray](../cppcx/platform-writeonlyarray-class.md) в зависимости от того, как он используется. Тип [Platform::Array](../cppcx/platform-array-class.md) не сравним по эффективности и широте возможностей с типом [std::vector](../standard-library/vector-class.md), поэтому в общем случае не рекомендуется использовать его во внутреннем коде, который выполняет множество операций над элементами массива.
...
Пассаррай используется, когда вызывающий объект передает массив в метод. Тип C++ входного параметра — `const` [Platform:: Array](../cppcx/platform-array-class.md)\<T >.

FillArray используется, когда вызывающий объект передает массив для заполнения методом. Тип C++ входного параметра — [Platform:: WriteOnlyArray](../cppcx/platform-writeonlyarray-class.md)\<T >.

Рецеивеаррай используется, когда вызывающий объект получает массив, выделенный методом. В C++/CX массив можно вернуть в возвращаемом значении в виде Array^ или в качестве выходного параметра типа Array^*.
...
В шаблоне ReceiveArray клиентский код объявляет массив и передает его методу, который выделяет память для массива и инициализирует его. Тип C++ входного параметра — указатель на шляпу: `Array<T>^*`. В следующем примере показано, как объявить объект массива в JavaScript и передать его функции C++, которая выделяет память, инициализирует элементы и возвращает массив в JavaScript. JavaScript обрабатывает выделенный массив в качестве возвращаемого значения, но функция C++ обрабатывает его как выходной параметр.
