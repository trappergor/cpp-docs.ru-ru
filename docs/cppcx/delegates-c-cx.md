...
Объявляя делегатов, которые будут подвергаться воздействию клиентов через двоичный интерфейс приложения Windows Runtime, используйте [Windows::Foundation::TypedEventHandler\<TSender, TResult>](/uwp/api/windows.foundation.typedeventhandler-2). Этот делегат имеет предопределенные двоичные прокси и заглушки, которые позволяют его использовать клиентами Javascript.

## <a name="consuming-delegates"></a>Использование делегатов

При создании универсального приложения платформы Windows вы часто работаете с делегатом как с типом события, которое предоставляет класс Runtime Windows. Чтобы подписаться на событие, создайте экземпляр типа делегата, указав функцию (или лямбда-выражение), соответствующее сигнатуре делегата. Затем воспользуйтесь оператором `+=` , чтобы передать объект делегата члену события в классе. Это называется "подписаться на событие". Когда экземпляр класса инициирует событие, вызывается ваша функция, а также другие обработчики, которые были добавлены данным объектом или другими объектами.
...
Вы можете определить своих делегатов, определить обработчики событий или дать потребителям возможность передавать пользовательские функции компоненту Windows Runtime. Как и любой другой тип Windows Runtime, публичный делегат не может быть объявлен общим.
...
> [!NOTE]
> Вы используете символ «Я», когда ссылаетесь на тип делегата, так же, как и в любом типе ссылки Windows Runtime.

Объявление события всегда имеет тип делегата. В этом примере показана типная подпись делегата в Windows Runtime:
...
В следующем примере клиентское приложение передает пользовательскому делегату общедоступный метод в компоненте Runtime Windows, который выполняет делегата против каждого элемента в: `Vector`
...
Если клиентское приложение загружает компонент Runtime Windows, который работает в квартире с потоком, и предоставляет делегата этому компоненту, то по умолчанию делегат вызывается непосредственно на поток ероточного потока. Большинство компонентов Windows Runtime могут работать либо в STA, либо в MTA.
