...
> [!WARNING]
>  Если используется лямбда-выражение, которое перехватывает указатель "this", необходимо с помощью оператора `-=` явно отменить регистрацию в событии до выхода из лямбда-выражения. Для получения дополнительной информации см. [Events](../cppcx/events-c-cx.md).  
