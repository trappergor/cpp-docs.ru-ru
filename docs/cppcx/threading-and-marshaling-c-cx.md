...
По различным причинам некоторые классы не могут быть гибкими. Если вы обращаетесь к экземплярам негибких классов и из потока пользовательского интерфейса, и из фонового потока, необходимо уделить особое внимание тому, чтобы обеспечить правильное поведение во время выполнения. Microsoft C++ компилятор выдает предупреждение, если создать экземпляр негибкого класса во время выполнения в приложении в глобальной области или объявления типа, отличный от agile, как член класса в классе ссылки, который сам помечен как гибкий.
