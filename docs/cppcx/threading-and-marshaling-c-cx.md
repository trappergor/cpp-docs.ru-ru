---
title: "Работа с потоками и маршалинг (C++/CX) | Microsoft Docs"
ms.custom: ""
ms.date: "12/30/2016"
...
f1_keywords: 
  - "C4451"
helpviewer_keywords: 
  - "вопросы, связанные с потоками, C++/CX"
  - "гибкость, C++/CX"
  - "C++/CX, вопросы, связанные с потоками"
...
В большинстве случаев обращение к экземплярам классов [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)], как и к стандартным объектам C\+\+, возможно из любого потока. Такие классы называются "гибкими". Однако некоторые классы [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)], входящие в Windows, не являются гибкими и должны использоваться скорее как COM\-объекты, а не как стандартные объекты C\+\+. Для работы с негибкими классами не нужно быть специалистом по COM, однако нужно учитывать модель потоков классов и их механизмы маршалинга. В этой статье приведены общие сведения и инструкции по реализации редких сценариев, в которых приходится использовать экземпляры негибких классов.  
  
## Модель потоков и механизмы маршалинга  
 Класс [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)] может поддерживать одновременный доступ к потокам различными способами, что указывается двумя атрибутами, которые к нему применяются.  
...
## Использование компонентов [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]  
 При создании приложения [!INCLUDE[win8_appname_long](../cppcx/includes/win8-appname-long-md.md)] вы можете работать как с гибкими, так и негибкими компонентами. При работе с негибкими компонентами может возникнуть следующее предупреждение.  
  
### Предупреждение компилятора при использовании негибких классов \(C4451\)  
...
-   Негибкая переменная объявляется в области класса, и существует возможность того, что код использует указатель в другом подразделении без надлежащего маршалинга.  
 ...
 Если ни одно из этих условий не выполняется, можно пометить содержащий класс как негибкий. Иными словами, необходимо напрямую использовать негибкие объекты только в негибких классов, а в гибких классах использовать их с помощью Platform::Agile\<T\>.  
...
```  
 ...
 В Visual C\+\+ при создании ссылки на внутрипроцессный класс [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)] с поведением маршалинга None компилятор выдает предупреждение C4451, но не предлагает использовать тип `Platform::Agile<T>`.  Компилятор не может ничем помочь помимо этого предупреждения, поэтому вам необходимо самостоятельно обеспечить надлежащую работу класса и убедиться, что код вызывает компоненты однопотокового подразделения \(STA\) только из потока пользовательского интерфейса, а компоненты многопотокового подразделения \(MTA\) — только из фонового потока.  
  
## Разработка гибких компонентов [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]  
 Если определить класс ссылки в [!INCLUDE[cppwrt_short](../cppcx/includes/cppwrt-short-md.md)], он является гибким по умолчанию, т. е. имеет значения атрибутов `ThreadingModel`\=Both и `MarshallingType`\=Agile.  Если используется [!INCLUDE[cppwrl](../cppcx/includes/cppwrl-md.md)], можно сделать класс гибким, унаследовав его класса `FtmBase`, использующего `FreeThreadedMarshaller`.  Создавая класс с атрибутами `ThreadingModel`\=Both или `ThreadingModel`\=MTA, убедитесь, что он является потокобезопасным. Дополнительные сведения см. в разделе [Создание и использование объектов \(WRL\)](http://msdn.microsoft.com/ru-ru/d5e42216-e888-4f1f-865a-b5ccd0def73e).  
 ...
 В следующем примере показано, как применить атрибуты `MarshalingBehavior` и `ThreadingModel` к классу среды выполнения из библиотеки классов [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]. Если приложение использует библиотеку DLL и в нем активируется объект класса `ref new` с помощью ключевого слова `MySTAClass`, этот объект активируется в однопотоковом подразделении и не поддерживает маршалинг.  
...
-   Значения атрибутов `ThreadingModel` и `MarshallingBehavior` в производном классе не соответствуют аналогичным атрибутам базового класса.  
 ...
 Сведения о потоковой модели и маршалинге, которые требуются для компонента [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)] от сторонних разработчиков, указываются в регистрационных данных манифеста приложения для компонента. Рекомендуется делать все создаваемые компоненты [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)] гибкими. Это гарантирует, что клиентский код сможет вызывать компонент из любого потока приложения, и улучшает производительность таких вызовов, поскольку они являются прямыми вызовами без маршалирования. Если создать класс таким образом, клиентскому коду не придется применять `Platform::Agile<T>`, чтобы использовать этот класс.  
