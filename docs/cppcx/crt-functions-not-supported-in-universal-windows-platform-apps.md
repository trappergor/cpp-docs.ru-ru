...
description: Справочное руководство по функциям CRT, не поддерживаемых в приложениях Universal Windows Platform.
ms.date: 04/16/2020
...
Многие функции C-времени выполнения (CRT) недоступны при создании универсальных приложений платформы Windows (UWP). Иногда доступны обходные пути, например, можно использовать Windows Runtime или AA Win32. В других случаях функции CRT были запрещены, поскольку соответствующие функции или поддерживающие AI не применимы к приложениям UWP. Чтобы найти альтернативный метод, который поддерживается для Windows Runtime, [см.](/uwp/win32-and-com/alternatives-to-windows-apis-uwp)

В следующей таблице перечислены функции CRT, которые недоступны при создании приложений UWP. Он указывает на любые обходные пути, которые применяются.
...
|_loaddll _getdllprocaddr _unloaddll|Эти функции устарели в предыдущих версиях CRT. Кроме того, пользователь не может загрузить DLLs, за исключением тех, которые в том же пакете приложений.|Для загрузки и использования упакованных DLL следует применять API Win32 `LoadPackagedLibrary`, `GetProcAddress`и `FreeLibrary` .|
...
|_heapwalk _heapadd _heapchk _heapset _heapused|Эти функции обычно используются для работы с кучей. Однако соответствующие API Win32 не поддерживаются в приложениях UWP. И приложения больше не могут создавать и использовать закрытые кучи.|Обходное решение отсутствует. Но функция `_heapwalk` доступна в DEBUG CRT для целей отладки. Эти функции не могут быть использованы в приложениях, загруженных в Microsoft Store.|

Следующие функции доступны в CRT для приложений UWP. Однако используйте их только тогда, когда вы не можете использовать соответствующие AIS Win32 или Windows Runtime, например, при переносе больших баз кода:

|||
|-|-|
|Однобайтовые строковые функции, например `strcat`, `strcpy`, `strlwr`и так далее.|Сделайте ваши приложения UWP строго Unicode, потому что все AI Win32 и Windows Runtime AI, которые подвергаются использовать только наборы символов Unicode.  Функции однобайного были оставлены для переноса больших баз кода, но в противном случае следует избегать. Соответствующие широкие функции char должны использоваться, когда это возможно.|
|Функции ввода-вывода потоков и ввода-вывода файлов нижнего уровня, например `fopen`, `open`и так далее.|Эти функции синхронны, что не рекомендуется для приложений UWP. В приложениях UWP для открытия, чтения и записи файлов используйте асинхронные API, чтобы избежать блокировки потока пользовательского интерфейса. Примеры таких API можно найти в классе `Windows::Storage::FileIO` .|
...
Как упомянутые ранее AI, так и следующие AI недоступны в приложениях магазина Windows 8.x и приложениях Windows Phone 8.x.
