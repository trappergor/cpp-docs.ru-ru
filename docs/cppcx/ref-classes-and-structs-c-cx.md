...
ms.locfileid: "70740887"
---
# <a name="ref-classes-and-structs-ccx"></a>Классы и структуры ссылки (C++/CX)

Класс C++/CX поддерживает определяемые пользователем *ссылочные классы* и *Ссылочные структуры*, а также *классы значений* и *структуры значений*, определяемые пользователем. Эти структуры данных являются основными контейнерами, по C++которым/CX поддерживает систему типов Среда выполнения Windows. Их содержимое передается в метаданные в соответствии с определенными правилами, и это позволяет передавать их между компонентами среда выполнения Windows и универсальная платформа Windows приложениями, написанными на или C++ на других языках.
...
- Он может содержать члены C++класса/CX, включая классы ссылок, классы значений, структуры ссылок, структуры значений или структуры значений, допускающие значения NULL. Он также может содержать скалярные типы, такие как float64, bool и т. д. Он также может содержать стандартные типы C++, например `std::vector` , или пользовательский класс при условии, что они не являются открытыми. C++Конструкции `public`/CX могут иметь специальные возможности, `internal` `protected`, `private`, или `protected private` . Все члены `public` или `protected` передаются в метаданные. Стандартные типы C++ должны иметь доступность `private`, `internal`или `protected private` , что не позволит им быть переданным в метаданные.
...
В следующем фрагменте кода объявляется класс ссылки `Person` . Обратите внимание, C++ `std::map` что стандартный тип используется в закрытых членах, а`IMapView` интерфейс Среда выполнения Windows используется в открытом интерфейсе. Также обратите внимание, что в конце объявлений ссылочных типов используется оператор "^".
...
`public` Ссылочный класс или структура ссылки создаются в метаданных, но их можно использовать из других универсальная платформа Windows приложений и среда выполнения Windows компонентов, которые должны иметь по крайней мере один открытый или защищенный конструктор. Класс ссылки, являющийся открытым и имеющий открытый конструктор, необходимо также объявлять как `sealed` , чтобы предотвратить его дальнейшее изменение через двоичный интерфейс приложений (ABI).
...
Класс ссылки, имеющий открытый или защищенный конструктор в C++языке/CX, должен быть объявлен как Sealed. Это ограничение означает, что классы, написанные на других языках, например C# или Visual Basic, не могут наследоваться от типов, объявляемых в компоненте Среда выполнения Windows, написанном в C++/CX.
