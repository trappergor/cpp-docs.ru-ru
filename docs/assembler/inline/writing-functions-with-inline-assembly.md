---
title: "Написание функций во встроенной сборке | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "__asm - ключевое слово [C++], в функциях"
  - "ассемблер [C++], запись функций"
  - "функции [C++], встроенная сборка"
  - "встроенная сборка [C++], запись функций"
...
manager: "ghogen"
---
# Написание функций во встроенной сборке
...
```  
 ...
 Поскольку встроенная версия функции `power2` не выполняет оператор `return` С, при компилировании на уровне предупреждений 2 или выше отображается безвредное предупреждение.  Функция возвращает значение, но компилятор не может определить его при отсутствии оператора `return`.  С помощью [предупреждения \#pragma](../../preprocessor/warning.md) можно отключить создание этого предупреждения.  
