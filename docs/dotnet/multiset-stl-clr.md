...
ms.locfileid: "80208498"
---
# <a name="multiset-stlclr"></a>multiset (STL/CLR)

Класс шаблона описывает объект, управляющий последовательностью элементов различной длины с двунаправленным доступом. `multiset` контейнера используется для управления последовательностью элементов как (почти) сбалансированным упорядоченным деревом узлов, каждый из которых хранит один элемент.
...
|[operator!= (multiset) (STL/CLR)](#op_neq)|Определяет, является ли объект `multiset` не равным другому объекту `multiset`.|
|[operator< (multiset) (STL/CLR)](#op_lt)|Определяет, является ли объект `multiset` меньше другого `multiset` объекта.|
|[operator<= (multiset) (STL/CLR)](#op_lteq)|Определяет, является ли объект `multiset` меньше или равным другому объекту `multiset`.|
|[operator== (multiset) (STL/CLR)](#op_eq)|Определяет, равен ли объект `multiset` другому объекту `multiset`.|
|[operator> (multiset) (STL/CLR)](#op_gt)|Определяет, является ли объект `multiset` больше другого `multiset` объекта.|
|[operator>= (multiset) (STL/CLR)](#op_gteq)|Определяет, является ли объект `multiset` больше другого `multiset` объекта или равен ему.|
...
Объект упорядочивает последовательность, которую он управляет, путем вызова сохраненного объекта делегата типа [мультинабор:: key_compare (STL/CLR)](../dotnet/multiset-key-compare-stl-clr.md). При создании мультинабора можно указать сохраненный объект делегата. Если объект делегата не указан, по умолчанию используется `operator<(key_type, key_type)`сравнения. Доступ к этому сохраненному объекту осуществляется путем вызова функции [-члена мультинабора:: key_comp (STL/CLR)](../dotnet/multiset-key-comp-stl-clr.md)`()`.
...
Функция-член фактически вызывает [мультинабор:: Erase (STL/CLR)](../dotnet/multiset-erase-stl-clr.md)`(` [мультинабор:: Begin (STL/CLR)](../dotnet/multiset-begin-stl-clr.md)`(),` [мультинабора:: end (STL/CLR)](../dotnet/multiset-end-stl-clr.md)`())`. Он используется, чтобы гарантировать, что управляемая последовательность пуста.
...
Функция-член возвращает пару итераторов `cliext::pair<iterator, iterator>(` [мультинабор:: lower_bound (STL/CLR)](../dotnet/multiset-lower-bound-stl-clr.md)`(key),` [мультинабор:: upper_bound (STL/CLR)](../dotnet/multiset-upper-bound-stl-clr.md)`(key))`. Он используется для определения диапазона элементов, находящихся в настоящий момент в управляемой последовательности, соответствующей указанному ключу.
...
## <a name="operatorlt-multiset-stlclr"></a><a name="op_lt"></a>Оператор&lt; (мультинабор) (STL/CLR)
...
Функция оператора возвращает значение true, если для наименьшей `i` позиций, для которого `!(right[i] < left[i])` также верно, что `left[i] < right[i]`. В противном случае он возвращает `left->size() < right->size()` вы используете его, чтобы проверить, упорядочивается ли *Left* перед *right* , когда два набора данных сравниваются по элементу.
...
## <a name="operatorlt-multiset-stlclr"></a><a name="op_lteq"></a>Оператор&lt;= (мультинабор) (STL/CLR)
...
Функция оператора возвращает значение true только в том случае, если последовательности, управляемые *левым* и *правым* , имеют одинаковую длину и для каждой `i`расположения `left[i] ==` `right[i]`. Он используется для проверки, упорядочивается ли *Left* *так, как если* бы два набора элементов сравнивались по элементу.
...
## <a name="operatorgt-multiset-stlclr"></a><a name="op_gt"></a>Оператор&gt; (мультинабор) (STL/CLR)
...
## <a name="operatorgt-multiset-stlclr"></a><a name="op_gteq"></a>Оператор&gt;= (мультинабор) (STL/CLR)
