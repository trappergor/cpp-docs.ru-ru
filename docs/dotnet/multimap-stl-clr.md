...
ms.locfileid: "80208528"
---
# <a name="multimap-stlclr"></a>multimap (STL/CLR)

Класс шаблона описывает объект, управляющий последовательностью элементов различной длины с двунаправленным доступом. `multimap` контейнера используется для управления последовательностью элементов как (почти) сбалансированным упорядоченным деревом узлов, каждый из которых хранит один элемент. Элемент состоит из ключа, для упорядочивания последовательности и сопоставленного значения, которое передается вместе с.
...
|[operator!= (multimap) (STL/CLR)](#op_neq)|Определяет, является ли объект `multimap` не равным другому объекту `multimap`.|
|[operator< (multimap) (STL/CLR)](#op_lt)|Определяет, является ли объект `multimap` меньше другого `multimap` объекта.|
|[operator<= (multimap) (STL/CLR)](#op_lteq)|Определяет, является ли объект `multimap` меньше или равным другому объекту `multimap`.|
|[operator== (multimap) (STL/CLR)](#op_eq)|Определяет, равен ли объект `multimap` другому объекту `multimap`.|
|[operator> (multimap) (STL/CLR)](#op_gt)|Определяет, является ли объект `multimap` больше другого `multimap` объекта.|
|[operator>= (multimap) (STL/CLR)](#op_gteq)|Определяет, является ли объект `multimap` больше другого `multimap` объекта или равен ему.|
...
Объект упорядочивает последовательность, которую он управляет, вызывая сохраненный объект делегата типа [multimap:: key_compare (STL/CLR)](../dotnet/multimap-key-compare-stl-clr.md). При создании multimap можно указать сохраненный объект делегата. Если объект делегата не указан, по умолчанию используется `operator<(key_type, key_type)`сравнения. Доступ к этому сохраненному объекту осуществляется путем вызова функции [-члена multimap:: key_comp (STL/CLR)](../dotnet/multimap-key-comp-stl-clr.md)`()`.
...
Первая функция-член удаляет элемент управляемой последовательности, *на которую указывает, и*возвращает итератор, который обозначает первый элемент, оставшийся после удаления элемента, или [multimap:: end (STL/CLR)](../dotnet/multimap-end-stl-clr.md)`()`, если такого элемента не существует. Он используется для удаления одного элемента.
...
Функция оператора возвращает значение true, если для наименьшей `i` позиций, для которого `!(right[i] < left[i])` также верно, что `left[i] < right[i]`. В противном случае он возвращает `left->size() < right->size()` вы используете его, чтобы проверить, упорядочивается ли *Left* перед *right* , если два сопоставления поддерживают сравнение элементов по элементу.
...
## <a name="operatorlt-multimap-stlclr"></a><a name="op_lteq"></a>Оператор&lt;= (multimap) (STL/CLR)
...
Функция оператора возвращает значение true только в том случае, если последовательности, управляемые *левым* и *правым* , имеют одинаковую длину и для каждой `i`расположения `left[i] ==` `right[i]`. Он используется для проверки, упорядочивается ли *Left* так же, как и *справа* , если два сопоставления поддерживают сравнение элементов по элементу.
...
## <a name="operatorgt-multimap-stlclr"></a><a name="op_gteq"></a>Оператор&gt;= (multimap) (STL/CLR)
