...
Очередь большая проведения сравнения.  
...
## Заметки  
 Функция возвращает оператора `right` `<` `left`.  Он используется для выполнения приказано ли `left` после `right` при сравнении 2 очереди элемент элементом.  
