...
Deque больше или равное сравнение.  
...
## Заметки  
 Функция возвращает оператора `!(``left` `<` `right``)`.  Он используется для выполнения не приказано ли `left` до 2 deques `right` при сравнении элемент элементом.  
