...
Объявление и поведение типов перечисления Enum изменилось с управляемых расширений для C++ к Visual C++.  
 ...
 В управляемых расширениях объявление типа enum предшествует `__value` ключевое слово. Том, чтобы различать собственное перечисление и перечисление CLR, который является производным от `System::ValueType`, при аналогичные функции. Например:  
