...
Рассмотрим следующий код.  Закрепление не требуется, поскольку `ppchar` является внутренним указателем, и если сборщик мусора перемещает строки, он указывает, будет также обновить `ppchar`. Без [pin_ptr (C + +/ CLI)](../extensions/pin-ptr-cpp-cli.md), код будет работать, и не имеет потенциальных снижение производительности, вызванные путем закрепления.
