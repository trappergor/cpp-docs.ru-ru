...
ms.locfileid: "80208551"
---
# <a name="map-stlclr"></a>map (STL/CLR)

Класс шаблона описывает объект, управляющий последовательностью элементов различной длины с двунаправленным доступом. `map` контейнера используется для управления последовательностью элементов как (почти) сбалансированным упорядоченным деревом узлов, каждый из которых хранит один элемент. Элемент состоит из ключа, для упорядочивания последовательности и сопоставленного значения, которое передается вместе с.

В описании ниже `GValue` совпадает с:

`Microsoft::VisualC::StlClr::GenericPair<GKey, GMapped>`

где:

`GKey` совпадает с *ключом* , если только второй не является ссылочным типом, в этом случае это `Key^`

`GMapped` совпадает с *сопоставлением* , если только второй не является ссылочным типом, в этом случае это `Mapped^`
...
|[operator!= (map) (STL/CLR)](#op_neq)|Определяет, является ли объект `map` не равным другому объекту `map`.|
|[operator< (map) (STL/CLR)](#op_lt)|Определяет, является ли объект `map` меньше другого `map` объекта.|
|[operator<= (map) (STL/CLR)](#op_lteq)|Определяет, является ли объект `map` меньше или равным другому объекту `map`.|
|[operator== (map) (STL/CLR)](#op_eq)|Определяет, равен ли объект `map` другому объекту `map`.|
|[operator> (map) (STL/CLR)](#op_gt)|Определяет, является ли объект `map` больше другого `map` объекта.|
|[operator>= (map) (STL/CLR)](#op_gteq)|Определяет, является ли объект `map` больше другого `map` объекта или равен ему.|
...
Объект упорядочивает последовательность, которую он управляет, вызывая сохраненный объект делегата типа [Map:: key_compare (STL/CLR)](../dotnet/map-key-compare-stl-clr.md). При создании схемы можно указать сохраненный объект делегата. Если объект делегата не указан, по умолчанию используется `operator<(key_type, key_type)`сравнения. Доступ к этому сохраненному объекту осуществляется путем вызова функции [-члена Map:: key_comp (STL/CLR)](../dotnet/map-key-comp-stl-clr.md)`()`.

Такой объект делегата должен накладывать строгое слабое упорядочение на ключи типа [Map:: key_type (STL/CLR)](../dotnet/map-key-type-stl-clr.md). Это означает, что для любых двух ключей `X` и `Y`:

`key_comp()(X, Y)` возвращает один и тот же логический результат при каждом вызове.

Если `key_comp()(X, Y)` имеет значение true, `key_comp()(Y, X)` должен иметь значение false.

Если `key_comp()(X, Y)` имеет значение true, то `X` считается упорядоченным до `Y`.

Если `!key_comp()(X, Y) && !key_comp()(Y, X)` имеет значение true, говорят, что `X` и `Y` имеют эквивалентное упорядочение.

Для любого элемента `X`, который предшествует `Y` в управляемой последовательности, `key_comp()(Y, X)` имеет значение false. (Для объекта делегата по умолчанию ключи никогда не уменьшаются в значении.) В отличие от [схемы](../dotnet/map-stl-clr.md)класса шаблона, объект класса шаблона `map` не требует уникальности ключей для всех элементов. (Два или более ключа могут иметь эквивалентное упорядочение.)
...
Функция-член вызывает метод [Map:: стирания (STL/CLR)](../dotnet/map-erase-stl-clr.md)`(` [Map:: Begin (STL/CLR)](../dotnet/map-begin-stl-clr.md)`(),` [карте:: end (STL/CLR)](../dotnet/map-end-stl-clr.md)`())`. Он используется, чтобы гарантировать, что управляемая последовательность пуста.
...
Эта функция-член возвращает значение true для пустой управляемой последовательности. Он эквивалентен [Map:: size (STL/CLR)](../dotnet/map-size-stl-clr.md)`() == 0`. Он используется для проверки, пуста ли схема.
...
Функция-член возвращает пару итераторов `cliext::pair<iterator, iterator>(` [Map:: lower_bound (STL/CLR)](../dotnet/map-lower-bound-stl-clr.md)`(key),` [map:: upper_bound (STL/CLR)](../dotnet/map-upper-bound-stl-clr.md)`(key))`. Он используется для определения диапазона элементов, находящихся в настоящий момент в управляемой последовательности, соответствующей указанному ключу.
...
Первая функция-член удаляет элемент управляемой последовательности, *на которую указывает, и*возвращает итератор, который обозначает первый элемент, оставшийся после удаления элемента, или [сопоставьте:: end (STL/CLR)](../dotnet/map-end-stl-clr.md)`()`, если такого элемента не существует. Он используется для удаления одного элемента.
...
Первая функция-член пытается вставить элемент со значением *Val*и возвращает пару значений `X`. Если `X.second` имеет значение true, `X.first` назначает новый вставленный элемент; в противном случае `X.first` обозначает элемент с таким же порядковым порядком, который уже существует и не вставляет новый элемент. Он используется для вставки одного элемента.
...
Функция-член возвращает `value_type` объект, ключ которого является *ключом* , а сопоставленное значение *сопоставляется*. Он используется для составления объекта, подходящего для использования с несколькими другими функциями элементов.
...
инициализирует управляемую последовательность с помощью последовательности [`first`, `last`) с предикатом упорядочения " *пред*". Он используется для того, чтобы управляемая последовательность была копией другой последовательности с указанным предикатом упорядочения.
...
Функция оператора возвращает значение true, если для наименьшей `i` позиций, для которого `!(right[i] < left[i])` также верно, что `left[i] < right[i]`. В противном случае он возвращает `left->size() < right->size()` вы используете его, чтобы проверить, упорядочивается ли *Left* до *right* , когда две карты сравниваются по элементу.
...
Функция оператора возвращает значение true только в том случае, если последовательности, управляемые *левым* и *правым* , имеют одинаковую длину и для каждой `i`расположения `left[i] ==` `right[i]`. Он используется для проверки, упорядочивается ли *Left* так же, как и *справа* , если две карты сравниваются по элементу.
