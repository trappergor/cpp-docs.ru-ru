...
Пары больше или равное сравнение.  
...
## Заметки  
 Функция возвращает оператора `!(``left` `<` `right``)`.  Он используется для выполнения не приказано ли `left` до 2 пары `right` при сравнении элемент элементом.  
...
**\[x, 4\]**  
**\[x3\] \>\= \[x3 наоборот\]**  
**\[x3\] \>\= \[\] x4 принимает значение " false "**   
## Требования  
