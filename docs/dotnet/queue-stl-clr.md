...
|[queue::generic_value (STL/CLR)](#generic_value)|Тип элемента для универсальный интерфейс для адаптера контейнера.|  
...
|[operator!= (queue) (STL/CLR)](#op_neq)|Определяет, если `queue` объект не равен другому `queue` объекта.|  
...
|[operator> (queue) (STL/CLR)](#op_gt)|Определяет, если `queue` объект больше другого `queue` объекта.|  
...
### <a name="remarks"></a>Примечания  
 Эта функция-член возвращает значение true для пустой управляемой последовательности. Это эквивалентно [queue::size (STL/CLR)](../dotnet/queue-size-stl-clr.md)`() == 0`. Используется для проверки, является ли очередь пуста.  
...
### <a name="remarks"></a>Примечания  
 Функция-член возвращает ссылку на первый элемент управляемой последовательности, который должен быть пустым. Используется для доступа к первый элемент, если известно, что он существует.  
...
### <a name="remarks"></a>Примечания  
 Функция оператор возвращает `!(left == right)`. Можно использовать для тестирования ли `left` не упорядочен таким же, как `right` при двух очереди являются сравниваемых элемент элемент.  
