...
ms.locfileid: "79546161"
---
# <a name="queue-stlclr"></a>queue (STL/CLR)

Класс шаблона описывает объект, управляющий последовательностью элементов различной длины, которые имеют первый поочередный доступ. Для управления базовым контейнером в качестве очереди используется `queue` адаптер контейнера.
...
|[operator!= (queue) (STL/CLR)](#op_neq)|Определяет, является ли объект `queue` не равным другому объекту `queue`.|
|[operator< (queue) (STL/CLR)](#op_lt)|Определяет, является ли объект `queue` меньше другого `queue` объекта.|
|[operator<= (queue) (STL/CLR)](#op_lteq)|Определяет, является ли объект `queue` меньше или равным другому объекту `queue`.|
|[operator== (queue) (STL/CLR)](#op_eq)|Определяет, равен ли объект `queue` другому объекту `queue`.|
|[operator> (queue) (STL/CLR)](#op_gt)|Определяет, является ли объект `queue` больше другого `queue` объекта.|
|[operator>= (queue) (STL/CLR)](#op_gteq)|Определяет, является ли объект `queue` больше другого `queue` объекта или равен ему.|
...
Функция оператора возвращает значение true, если для наименьшей `i` позиций, для которого `!(right[i] < left[i])` также верно, что `left[i] < right[i]`. В противном случае он возвращает `left->`[очередь:: size (STL/CLR)](../dotnet/queue-size-stl-clr.md)`() <` `right->size()` вы используете его, чтобы проверить, упорядочивается ли *Left* до *право* , когда две очереди сравниваются по элементу.
...
## <a name="operatorlt-queue-stlclr"></a><a name="op_lteq"></a>Оператор&lt;= (Queue) (STL/CLR)
...
Функция оператора возвращает значение true только в том случае, если последовательности, управляемые *левым* и *правым* , имеют одинаковую длину и для каждой `i`расположения `left[i] ==` `right[i]`. Он используется для проверки, упорядочивается ли *Left* *так, как если* бы две очереди сравнивались по элементу.
...
## <a name="operatorgt-queue-stlclr"></a><a name="op_gteq"></a>Оператор&gt;= (Queue) (STL/CLR)
