...
В следующем примере кода показано, что `MyClass` класс не может быть создан, так как функция `MyClass::func2` не реализована. Чтобы включить для примера компиляцию, раскомментируйте `MyClass::func2` .
...
|private|Член недоступен, ни внутри, ни вне сборки.  Дополнительные сведения см. в разделе [Private](../cpp/private-cpp.md) .|
...
При использовании Visual C++ для определения типов **`this`** указатель в ссылочном типе имеет тип "Handle". **`this`** Указатель в типе значения имеет тип "внутренний указатель".
...
- [Оператор Handle to Object (^)](../extensions/handle-to-object-operator-hat-cpp-component-extensions.md)
...
Поведение деструкторов в управляемом классе Visual C++ отличается от управляемые расширения для C++. Дополнительные сведения об этом изменении см. [в разделе изменения семантики деструктора](../dotnet/changes-in-destructor-semantics.md).
...
Код, написанный на Visual C++ и скомпилированный с помощью **/CLR** , запускает деструктор типа, если:
