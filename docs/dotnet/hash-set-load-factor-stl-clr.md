...
## Заметки  
 Функция\-член возвращает `(float)`[hash\_set::size](../dotnet/hash-set-size-stl-clr.md)`() /` [hash\_set::bucket\_count](../dotnet/hash-set-bucket-count-stl-clr.md)`()`.  Он используется, чтобы определить средний размер блока.  
