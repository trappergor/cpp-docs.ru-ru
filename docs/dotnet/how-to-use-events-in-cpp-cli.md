---
title: "Практическое руководство. Использование событий в C++/CLI | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "события [C++], обращение в интерфейсах"
...
manager: "ghogen"
---
# Практическое руководство. Использование событий в C++/CLI
[!INCLUDE[vs2017banner](../assembler/inline/includes/vs2017banner.md)]

В данной статье показано, как использовать интерфейс, который объявляет событие и функцию, чтобы вызывать это событие, класс и обработчик событий, реализующего интерфейс.  
  
## События интерфейса  
 Следующий пример кода добавляет обработчик событий, вызывает события, которое вызывает обработчик событий записи его имя в консоль\- и затем удаляет обработчик событий.  
...
## Пользовательские методы доступа  
 В следующем примере показано, как задать расширения функциональности события при добавлении и удалении обработчики и при возникновении события.  
 ...
 **Output**  
  ...
  **В обработчике событий H1**  
**В обработчике событий H2 с аргументами 1 и 2.2**   
## Доступ по умолчанию переопределения, добавлять, удалять и вызывает методы доступа  
 В этом примере показано, как переопределить доступ по умолчанию для добавления, удаления и вызвать методы событий:  
...
## Несколько обработчиков событий  
 Приемник событий или любой другой клиентский код, можно добавить один или несколько обработчики к событиям.  
 ...
 **Output**  
  ...
  **Нажмите кнопку \(x\=7, y\=3.14159\)**  
**DblClick s\=System.Char \(\[\]\)**  
**DblClickAgain s\=System.Char \(\[\]\)**   
## Статические события  
 В следующем примере показано, как определять и использовать статические события.  
 ...
 **Output**  
  ...
  **В обработчике событий H1**  
**В обработчике событий H2 с аргументами 11 и 11.11**  
**В обработчике событий H1**  
**В обработчике событий H2 с аргументами 22 и 22.22**   
## Виртуальные события  
 В этом примере реализовать виртуальный, управляемые события в интерфейсе и класс:  
 ...
 **Output**  
  ...
  **В обработчике H1**  
**В обработчике H2 с аргументами 1 и 2.2** Простое событие нельзя указать, что переопределило или скрывало событие базового класса.  Необходимо указать функции доступа всего события, а затем укажите ключевое слово `new` или `override` для каждой функции доступа.  
 ...
 **Output**  
  ...
  **hi**  
**hello из Event2**   
## Создание событий, определенных в другой сборке  
 Событие и обработчик событий можно указать в одной сборке, и использовать другой сборке.  
