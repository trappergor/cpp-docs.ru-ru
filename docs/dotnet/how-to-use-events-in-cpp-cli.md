---
title: Практическое руководство. Использование событий в C++выполняет
ms.date: 11/04/2016
helpviewer_keywords:
...
ms.locfileid: "62387179"
---
# <a name="how-to-use-events-in-ccli"></a>Практическое руководство. Использование событий в C++выполняет

В этой статье показано, как использовать интерфейс, объявляющий событие и функции для вызова этого события, а класс и обработчиком событий, реализующих этот интерфейс.

## <a name="interface-events"></a>События интерфейса

В следующем примере кода добавляется обработчик событий, вызывающий событие, что вызывает обработчик событий для записи его имени в консоли и затем удаляет обработчик событий.
...
## <a name="custom-accessor-methods"></a>Методы настраиваемого метода доступа

Следующий пример показано, как определить поведение события в том случае, если обработчики добавляются или удаляются, и при возникновении события.
...
В этом примере показано, как переопределить по умолчанию доступ на добавление, удаление, методов и порождения события:
...
Приемник событий, или любой другой код клиента, можно добавить один или несколько обработчиков события.
...
Следующий пример показывает способы определения и использование статических событий.
...
В этом примере реализован виртуальных управляемых событий в интерфейсе и класс:
...
Простое событие нельзя переопределить или скрыть событие базового класса.  Необходимо определить все функции метода доступа события, а затем укажите `new` или `override` ключевое слово для каждой функции метода доступа.
...
Следующий пример кода демонстрируется реализация абстрактных событий.
...
## <a name="raising-events-that-are-defined-in-a-different-assembly"></a>Вызов событий, которые определены в другой сборке

События и обработчик событий можно быть определены в одной сборке и на другую сборку.
