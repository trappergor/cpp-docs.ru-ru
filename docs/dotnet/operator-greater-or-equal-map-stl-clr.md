...
## Заметки  
 Функция возвращает оператора `!(``left` `<` `right``)`.  Он используется для выполнения не приказано ли `left` перед `right` при сравнении 2 сопоставления элемент элементом.  
