...
При компиляции с параметром **/CLR** можно ОБРАБАТЫВАТЬ исключения CLR, а также стандартный класс <xref:System.Exception> предоставляет множество полезных методов для обработки исключений CLR и рекомендуется в качестве базового класса для определяемых пользователем классов исключений.
...
Дополнительные сведения о различиях в обработке исключений в управляемых и неуправляемых приложениях см. в разделе [различия в поведении обработки исключений в разделе C++управляемые расширения для ](../dotnet/differences-in-exception-handling-behavior-under-clr.md).
...
Выражение C++ Throw расширяется для создания обработчика для типа CLR. В следующем примере создается пользовательский тип исключения, а затем создается экземпляр этого типа:
...
Для перехвата исключений CLR и машинного кода можно использовать ту же структуру блока **try**/**catch** :
...
### <a name="order-of-unwinding-for-c-objects"></a>Порядок очистки C++ объектов

Очистка происходит для всех C++ объектов с деструкторами, которые могут находиться в стеке времени выполнения между функцией выдачи и функцией обработки. Поскольку типы CLR выделяются в куче, для них не применяется очистка.
...
При вызове типа неуправляемого объекта он упаковывается с исключением типа <xref:System.Runtime.InteropServices.SEHException>. При поиске соответствующего предложения **catch** возможны два варианта.

- Если обнаружен собственный C++ тип, исключение разносится в оболочку и сравнивается с обнаруженным типом. Это сравнение позволяет перехватывать C++ собственный тип обычным способом.

- Однако если сначала просматривается предложение **catch** типа **SEHException** или любого из его базовых классов, оно перехватит исключение. Поэтому перед любыми предложениями catch типов CLR следует поместить все C++ предложения catch, которые перехватывают собственные типы.
