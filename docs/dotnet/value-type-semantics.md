...
```  
 ...
 В новом синтаксисе закрепление всего объекта возвращенных `new` выражение не поддерживается. Вместо этого адрес внутреннего члена должен быть администратором. Например:  
