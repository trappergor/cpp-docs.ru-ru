...
ms.locfileid: "80208774"
---
# <a name="deque-stlclr"></a>deque (STL/CLR)

Класс шаблона описывает объект, управляющий последовательностью элементов различной длины с произвольным доступом. `deque` контейнера используется для управления последовательностью элементов, которая выглядит как непрерывный блок хранилища, но может быть увеличена или уменьшена в любой момент без необходимости копировать оставшиеся элементы. Таким способом можно эффективно реализовать `double-ended queue`. (Следовательно, имя.)

В описании ниже `GValue` совпадает с `Value`, если только второй не является ссылочным типом, в этом случае он `Value^`.
...
|[operator< (deque) (STL/CLR)](#op_lt)|Определяет, является ли объект `deque` меньше другого `deque` объекта.|
|[operator<= (deque) (STL/CLR)](#op_lteq)|Определяет, является ли объект `deque` меньше или равным другому объекту `deque`.|
|[operator= (deque) (STL/CLR)](#op_as)|Заменяет управляемую последовательность.|
|[operator== (deque) (STL/CLR)](#op_eq)|Определяет, равен ли объект `deque` другому объекту `deque`.|
|[operator> (deque) (STL/CLR)](#op_gt)|Определяет, является ли объект `deque` больше другого `deque` объекта.|
|[operator>= (deque) (STL/CLR)](#op_gteq)|Определяет, является ли объект `deque` больше другого `deque` объекта или равен ему.|
...
Функция оператора возвращает значение true, если для наименьшей `i` позиций, для которого `!(right[i] < left[i])` также верно, что `left[i] < right[i]`. В противном случае он возвращает `left->size() < right->size()` вы используете его, чтобы проверить, упорядочивается ли *Left* до *право* , когда два декуес сравниваются по элементу.
...
## <a name="operatorlt-deque-stlclr"></a><a name="op_lteq"></a>Оператор&lt;= (deque) (STL/CLR)
...
Функция оператора возвращает значение true только в том случае, если последовательности, управляемые *левым* и *правым* , имеют одинаковую длину и для каждой `i`расположения `left[i] ==` `right[i]`. Он используется для проверки, упорядочивается ли *Left* так же, как и *справа* , если два декуес сравниваются по элементу.
...
## <a name="operatorgt-deque-stlclr"></a><a name="op_gteq"></a>Оператор&gt;= (deque) (STL/CLR)
