...
## Заметки  
 Член оператор копирует `right` к объекту, а затем возвращает `*this`.  Он используется, чтобы заменить, хранящихся пары итератора копией сохраненного пар итератора в `right`.  
