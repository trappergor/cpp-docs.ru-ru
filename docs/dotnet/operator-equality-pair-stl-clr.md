...
## Заметки  
 Функция возвращает оператора `left``.first ==` `right``.first &&` `left``.second ==` `right``.second`.  Он используется для выполнения приказано ли `left` таким же, как `right` при сравнении 2 пары элемент элементом.  
