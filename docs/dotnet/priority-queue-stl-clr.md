...
ms.locfileid: "80208472"
---
# <a name="priority_queue-stlclr"></a>priority_queue (STL/CLR)

Класс шаблона описывает объект, управляющий упорядоченной последовательностью элементов различной длины с ограниченным доступом. Для управления базовым контейнером в качестве очереди приоритетов используется `priority_queue` адаптера контейнера.
...
|Значение\<Иприоритикуеуе, контейнер >|Поддерживать универсальный адаптер контейнера.|

## <a name="remarks"></a>Remarks

Объект выделяет и освобождает хранилище для последовательности, которая управляется через базовый контейнер, типа `Container`, который хранит элементы `Value` и растет по запросу. Последовательность упорядочивается в виде кучи, при этом элемент с наивысшим приоритетом (верхний элемент) легко становится доступным и съемным. Объект предоставляет доступ к отправке новых элементов и выталкивания только элемента с самым высоким приоритетом, реализуя очередь приоритетов.

Объект упорядочивает последовательность, которую он управляет, вызывая сохраненный объект делегата типа [priority_queue:: value_compare (STL/CLR)](../dotnet/priority-queue-value-compare-stl-clr.md). При создании priority_queue можно указать сохраненный объект делегата; Если объект делегата не указан, по умолчанию используется `operator<(value_type, value_type)`сравнения. Доступ к этому сохраненному объекту осуществляется путем вызова функции [-члена priority_queue:: value_comp (STL/CLR)](../dotnet/priority-queue-value-comp-stl-clr.md)`()`.
...
Если `value_comp()(X, Y)` имеет значение true, `value_comp()(Y, X)` должен иметь значение false.

Если `value_comp()(X, Y)` имеет значение true, то `X` считается упорядоченным до `Y`.

Если `!value_comp()(X, Y) && !value_comp()(Y, X)` имеет значение true, говорят, что `X` и `Y` имеют эквивалентное упорядочение.
...
Функция члена вставляет элемент со значением `val` в управляемую последовательность и переупорядочивает управляемую последовательность для поддержки дисциплины кучи. Его можно использовать для добавления в очередь еще одного элемента.
