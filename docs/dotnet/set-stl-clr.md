...
ms.locfileid: "79546155"
---
# <a name="set-stlclr"></a>set (STL/CLR)

Класс шаблона описывает объект, управляющий последовательностью элементов различной длины с двунаправленным доступом. `set` контейнера используется для управления последовательностью элементов как (почти) сбалансированным упорядоченным деревом узлов, каждый из которых хранит один элемент.
...
|[operator!= (set) (STL/CLR)](#op_neq)|Определяет, является ли объект `set` не равным другому объекту `set`.|
|[operator< (set) (STL/CLR)](#op_lt)|Определяет, является ли объект `set` меньше другого `set` объекта.|
|[operator<= (set) (STL/CLR)](#op_lteq)|Определяет, является ли объект `set` меньше или равным другому объекту `set`.|
|[operator== (set) (STL/CLR)](#op_eq)|Определяет, равен ли объект `set` другому объекту `set`.|
|[operator> (set) (STL/CLR)](#op_gt)|Определяет, является ли объект `set` больше другого `set` объекта.|
|[operator>= (set) (STL/CLR)](#op_gteq)|Определяет, является ли объект `set` больше другого `set` объекта или равен ему.|
...
Объект упорядочивает последовательность, которую он управляет, вызывая сохраненный объект делегата типа [Set:: key_compare (STL/CLR)](../dotnet/set-key-compare-stl-clr.md). При создании набора можно указать сохраненный объект делегата. Если объект делегата не указан, по умолчанию используется `operator<(key_type, key_type)`сравнения. Доступ к этому сохраненному объекту осуществляется путем вызова функции [-члена Set:: key_comp (STL/CLR)](../dotnet/set-key-comp-stl-clr.md)`()`.
...
Функция-член эффективно вызывает [Set:: Erase (STL/CLR)](../dotnet/set-erase-stl-clr.md)`(` [Set:: Begin (STL/CLR)](../dotnet/set-begin-stl-clr.md)`(),` [Set:: end (STL/CLR)](../dotnet/set-end-stl-clr.md)`())`. Он используется, чтобы гарантировать, что управляемая последовательность пуста.
...
Первая функция-член удаляет элемент управляемой последовательности, *на которую указывает, и*возвращает итератор, который обозначает первый элемент, оставшийся после удаления элемента, или [Задайте:: end (STL/CLR)](../dotnet/set-end-stl-clr.md)`()`, если такого элемента не существует. Он используется для удаления одного элемента.
...
Функция оператора возвращает значение true, если для наименьшей `i` позиций, для которого `!(right[i] < left[i])` также верно, что `left[i] < right[i]`. В противном случае он возвращает `left->size() < right->size()` вы используете его, чтобы проверить, упорядочивается ли *Left* перед *right* , когда два набора сравниваются по элементу.
...
## <a name="operatorlt-set-stlclr"></a><a name="op_lteq"></a>Оператор&lt;= (Set) (STL/CLR)
...
Функция оператора возвращает значение true только в том случае, если последовательности, управляемые *левым* и *правым* , имеют одинаковую длину и для каждой `i`расположения `left[i] ==` `right[i]`. Он используется для проверки того, упорядочивается ли *Left* *так, как если* бы два набора сравнивались по элементу.
...
## <a name="operatorgt-set-stlclr"></a><a name="op_gteq"></a>Оператор&gt;= (Set) (STL/CLR)
