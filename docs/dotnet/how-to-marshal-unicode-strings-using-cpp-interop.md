...
Чтобы передать строку Юникода из управляемого в неуправляемую функцию, можно использовать функцию PtrToStringChars (объявлено в файле Vcclr.h) для доступа к памяти, где хранится управляемая строка. Так как этот адрес будет передан в неуправляемую функцию, очень важно, что память была закреплена с [pin_ptr (C++выполняет)](../extensions/pin-ptr-cpp-cli.md) чтобы предотвратить перемещение строковые данные, следует цикла сборки мусора выполняются во время выполняет неуправляемой функции.
