...
ms.locfileid: "79545783"
---
# <a name="vector-stlclr"></a>vector (STL/CLR)

Класс шаблона описывает объект, управляющий последовательностью элементов различной длины с произвольным доступом. `vector` контейнера используется для управления последовательностью элементов в виде непрерывного блока хранилища. Блок реализуется как массив, который растет по запросу.
...
|[operator!= (vector) (STL/CLR)](#op_neq)|Определяет, является ли объект `vector` не равным другому объекту `vector`.|
|[operator< (vector) (STL/CLR)](#op_lt)|Определяет, является ли объект `vector` меньше другого `vector` объекта.|
|[operator<= (vector) (STL/CLR)](#op_lteq)|Определяет, является ли объект `vector` меньше или равным другому объекту `vector`.|
|[operator== (vector) (STL/CLR)](#op_eq)|Определяет, равен ли объект `vector` другому объекту `vector`.|
|[operator> (vector) (STL/CLR)](#op_gt)|Определяет, является ли объект `vector` больше другого `vector` объекта.|
|[operator>= (vector) (STL/CLR)](#op_gteq)|Определяет, является ли объект `vector` больше другого `vector` объекта или равен ему.|
...
Функция-член возвращает хранилище, выделенное для хранения управляемой последовательности, значение по крайней мере в виде [вектора:: size (STL/CLR)](../dotnet/vector-size-stl-clr.md)`()`. Он используется для определения того, насколько может расти контейнер, прежде чем он должен будет перераспределить хранилище для управляемой последовательности.
...
Функция-член фактически вызывает метод [vector:: Erase (STL/CLR)](../dotnet/vector-erase-stl-clr.md)`(` [вектор:: Begin (STL/CLR)](../dotnet/vector-begin-stl-clr.md)`(),` [вектор:: end (STL/CLR)](../dotnet/vector-end-stl-clr.md)`())`. Он используется, чтобы гарантировать, что управляемая последовательность пуста.
...
Функция оператора возвращает значение true, если для наименьшей `i` позиций, для которого `!(right[i] < left[i])` также верно, что `left[i] < right[i]`. В противном случае он возвращает `left->size() < right->size()` вы используете его, чтобы проверить, упорядочивается ли *Left* перед *right* , когда два вектора сравниваются по элементу.
...
Функция оператора возвращает значение true только в том случае, если последовательности, управляемые *левым* и *правым* , имеют одинаковую длину и для каждой `i`расположения `left[i] ==` `right[i]`. Он используется для проверки, упорядочивается ли *Left* так же, как и *справа* , если два вектора сравниваются по элементу.
