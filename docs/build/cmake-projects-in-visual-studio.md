...
> [!NOTE]
> CMake становятся более интегрирована с Visual Studio за последние несколько выпусков. Чтобы просмотреть правильные сведения для версии, которую вы используете, убедитесь, что правильно настроены выбора версий в левом верхнем углу этой страницы. 

::: moniker range="vs-2019"

Введение в Visual Studio 2019 **редактор параметров CMake** и другие усовершенствования, внесенные через Visual Studio 2017. **C++ Инструменты CMake для Windows** компонент использует **открыть папку** функцию, чтобы включить интегрированную среду разработки для использования файлы проектов CMake (например, CMakeLists.txt) непосредственно в целях IntelliSense и Просмотр. Поддерживаются генераторы Visual Studio и Ninja. Если вы используете генератор Visual Studio, создается временный файл проекта, который передается в msbuild.exe, но никогда не загружается для IntelliSense или просмотра. Можно также импортировать существующего кэша CMake. 

## <a name="installation"></a>Установка

**C++Инструменты CMake для Windows** устанавливается по умолчанию как часть **разработка классических приложений на C++**  рабочей нагрузки и как часть **разработка приложений Linux на C++**  рабочей нагрузки. См. в разделе [проектов CMake кросс платформенные](../linux/cmake-linux-project.md) Дополнительные сведения.
...
- Visual Studio добавляет **CMake** элементы к **проекта** меню с командами для просмотра и редактирования скриптов CMake.
...
Конфигурации построения, переменные среды, аргументы командной строки и другие параметры можно настроить без изменения с файлом CMakeLists.txt с помощью **редактор параметров CMake**. Дополнительные сведения см. в статье [Настраиваемые параметры CMake](customize-cmake-settings.md).
...
Если вы установили [vcpkg](vcpkg.md), проекты CMake, открытые в Visual Studio автоматически интегрирует файл vcpkg цепочки инструментов. Это означает, что должен использовать vcpkg для проектов CMake без дополнительной настройки. Эта поддержка работает как для установки локального vcpkg и установки vcpkg на удаленных компьютерах, являющихся целевыми. Это поведение можно отключить автоматически при указании любой цепочки инструментов в конфигурации параметры CMake.

## <a name="just-my-code-for-cmake-projects"></a>Только мой код для проектов CMake

При построении для Windows с помощью компилятора MSVC свои проекты CMake теперь поддерживают Отладка только собственного кода в компиляторе и компоновщике если он включен в Visual Studio. Чтобы изменить этот параметр, перейдите к **средства** > **параметры** > **Отладка** > **Общие**.

## <a name="customize-configuration-feedback"></a>Настройка конфигурации отзыва

По умолчанию большинство сообщений конфигурации подавляются, если не возникает ошибка. Вы можете просмотреть все сообщения при включении этой функции в **средства** > **параметры** > **CMake**.

   ![Настройка параметров диагностики CMake](media/vs2019-cmake-configure-options.png "диагностические параметры CMake")
