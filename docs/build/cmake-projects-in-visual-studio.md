...
Visual Studio использует файл с именем `CMakeSettings.json` для хранения переменных среды или параметров командной строки для Cmake.exe. `CMakeSettings.json` также позволяет определять и хранить несколько конфигураций сборки CMake и с удобством переключаться между ними в IDE. 
...
Вы можете настраивать конфигурации сборки, переменные среды, аргументы командной строки и другие параметры без изменения файла CMakeLists.txt с помощью файла `CMakeSettings.json`. Дополнительные сведения см. в статье [Настраиваемые параметры CMake](customize-cmake-settings.md).
