...
По умолчанию Visual Studio предоставляет по одной конфигурации `x64-Debug`. Можно добавить дополнительные конфигурации, щелкнув зеленый знак «плюс». Параметры, отображаемые в редакторе, могут различаться в зависимости от выбранной конфигурации.
...
Соответствует **переменным**. Содержит пару "имя-значение" переменных CMAK, переданных **как** *_имя_=_значение_* в CMAK. Если в инструкциях по сборке проекта CMak указано Добавление переменных непосредственно в файл кэша CMak, рекомендуется добавить их здесь.
...
Технология JSON IntelliSense позволяет редактировать файл *CMakeSettings. JSON* :
   ...
   ![Технология CMak JSON](media/cmake-json-intellisense.png "Технология CMak JSON")
...
Чтобы добавить дополнительную конфигурацию, щелкните правой кнопкой мыши *CMakeSettings. JSON* и выберите **Добавить конфигурацию**.
   ...
   ![Добавление конфигурации в CMak](media/cmake-add-configuration.png "Добавление конфигурации в CMak")
