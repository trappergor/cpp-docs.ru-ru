...
При построении библиотеки DLL в Visual C++, точки входа по умолчанию `_DllMainCRTStartup` предоставляемые VCRuntime автоматически связана в последовательности. Необходимо указать функцию точки входа для библиотеки DLL с помощью [/Entry (символ точки входа)](reference/entry-entry-point-symbol.md) параметр компоновщика.
