...
Visual Studio предоставляет предопределенные конфигурации для отладки и выпуска x86 и x64. По умолчанию ваш проект имеет конфигурацию отладки x86-Debug в `CppProperties.json`. Чтобы добавить новую конфигурацию, щелкните правой кнопкой мыши `CppProperties.json` файл **обозревателе решений** и выберите **Добавление конфигурации**:

![Откройте папку — Добавление новой конфигурации](media/open-folder-add-config.png "открыть папку Добавление новой конфигурации")
...
|`intelliSenseMode`|Используемая подсистема IntelliSense. Можно указать варианты конкретной архитектуры для MSVC, Clang и gcc.<br/><br/>- windows-msvc-x86 (по умолчанию)<br/>- windows-msvc-x64<br/>- msvc-arm<br/>- windows-clang-x86<br/>- windows-clang-x64<br/>- windows-clang-arm<br/>- Linux-x64<br/>- Linux-x86<br/>- Linux-arm<br/>- gccarm|

Примечание. Значения `msvc-x86` и `msvc-x64` поддерживаются только для обеспечения обратной совместимости. Используйте `windows-msvc-*` варианты вместо этого.
...
Можно также определить **сред** свойство в конфигурации. Он применяется только к конфигурации и переопределяет все глобальные переменные, с тем же именем. В примере ниже конфигурация x64 определяет локальную переменную **INCLUDE**, переопределяющую глобальное значение.
