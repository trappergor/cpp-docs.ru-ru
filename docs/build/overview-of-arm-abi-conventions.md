...
---
# <a name="overview-of-arm32-abi-conventions"></a>Обзор конвенций ARM32 ABI

Двоичный интерфейс приложения (ABI) для кода, скомпилированного для Windows на процессорах ARM, основан на стандартном встроенном двоичном интерфейсе приложения ARM. В этой статье указаны различия между системой Windows на устройствах ARM и стандартной системой. Этот документ охватывает ARM32 ABI. Для получения информации о ARM64 ABI, [см.](arm64-windows-abi-conventions.md) Для получения дополнительной информации о стандартном ARM EABI [см.](http://infocenter.arm.com/help/index.jsp?topic=/com.arm.doc.subset.swdev.abi/index.html)
...
Windows на ARM выполняется в режиме с прямым порядком байтов. И компилятор MSVC, и время выполнения Windows ожидают малоконечных данных во все времена. Хотя инструкция SETEND в архитектуре набора инструкций (ISA) ARM позволяет даже коду пользовательского режима изменять текущий порядок байтов, поступать так не рекомендуется, так как это опасно для приложения. Если в режиме с обратным порядком байтов возникает исключение, работа становится непредсказуемой, что может привести к сбою приложения в пользовательском режиме или к возникновению критической ошибки в режиме ядра.
...
- Целевая инструкция должна относиться к одному из следующих типов.
   ...
   |---------------------|-----------|------------------|
   |MOV, MVN|Переместить|Rm != PC, Rd != PC|
...
## <a name="integer-registers"></a>Единые регистры

Процессор ARM поддерживает 16 целочисленных регистров.

|Зарегистрировать|Переменный?|Роль|
|--------------|---------------|----------|
...
## <a name="floating-point-exceptions"></a>Исключения из плавающей точки
...
Для функций без переменного числа аргументов двоичный интерфейс приложения Windows на ARM соответствует правилам ARM для передачи параметров, включая VFP и усовершенствованные расширения SIMD. Эти правила следуют [Стандарту процедурного вызова для архитектуры ARM,](http://infocenter.arm.com/help/topic/com.arm.doc.ihi0042c/IHI0042C_aapcs.pdf)консолидированному с расширениями VFP. По умолчанию в регистры передаются четыре первых целочисленных аргумента и до восьми аргументов с плавающей запятой или векторных аргументов; дополнительные аргументы передаются в стек. Аргументы назначаются регистрам или стеку с помощью следующей процедуры.
...
### <a name="stage-b-pre-padding-and-extension-of-arguments"></a>Этап B: Предварительная обивка и расширение аргументов
...
### <a name="stage-c-assignment-of-arguments-to-registers-and-stack"></a>Этап C: Назначение аргументов в регистры и стек
...
Функции, выделяющие 4 КБ или больше в стеке, должны проверять, что каждая страница перед последней обрабатывается по порядку. Это гарантирует, что ни один код не может «перепрыгнуть» страницы охраны, которые Windows использует для расширения стека. Обычно это осуществляется с помощью вспомогательной функции `__chkstk`, которая получает все выделение стека в байтах, поделенное на 4, из r4 и возвращает итоговое значение выделение стека в байтах обратно в r4.
...
## <a name="cc-specifics"></a>Специфика К/КЗ
...
## <a name="stack-walking"></a>Стек ходьба

Код Windows компилируется с включенными указателями кадра[(/Oy (Frame-Pointer Omission)](reference/oy-frame-pointer-omission.md)для обеспечения быстрой ходьбы стека. Обычно регистр r11 указывает на следующее звено в цепочке, которое представляет собой пару {r11, lr}, задающую указатель на предыдущий фрейм в стеке и адрес возврата. Мы рекомендуем также включить указатели фреймов в вашем коде для улучшения профилирования и трассировки.

## <a name="exception-unwinding"></a>Исключение раскручивание

Очистка стека во время обработки исключений реализуется посредством кодов очистки. Коды очистки представляют собой последовательность байт, хранимых в разделе .XDATA исполняемого образа. Они описывают работу пролога функции и кода эпилога абстрактно, так что эффекты пролога функции могут быть отменены в рамках подготовки к раскручиванию стека кадра вызывающего абонента.

ARM EABI задает модель очистки исключения, использующую коды очистки. Однако такой спецификации недостаточно для очистки в Windows, так как требуется обрабатывать случаи, когда процессор находится в середине пролога или эпилога функции. Для получения дополнительной информации о Windows по [ARM Exception Handling](arm-exception-handling.md)данным об исключениях ARM и раскручивании, см.
...
## <a name="cycle-counter"></a>Счетчик цикла
