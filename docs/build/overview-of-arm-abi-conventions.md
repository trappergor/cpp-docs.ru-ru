...
Windows на ARM выполняется в режиме с прямым порядком байтов. Компилятор MSVC и среды выполнения Windows ожидается, что данные с прямым порядком байтов все время. Хотя инструкция SETEND в архитектуре набора инструкций (ISA) ARM позволяет даже коду пользовательского режима изменять текущий порядок байтов, поступать так не рекомендуется, так как это опасно для приложения. Если в режиме с обратным порядком байтов возникает исключение, работа становится непредсказуемой, что может привести к сбою приложения в пользовательском режиме или к возникновению критической ошибки в режиме ядра.
...
### <a name="stage-a-initialization"></a>О рабочей области Инициализация
...
### <a name="stage-b-pre-padding-and-extension-of-arguments"></a>Этап б: Предварительно заполнение и расширение аргументов
