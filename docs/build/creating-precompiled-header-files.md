...
ms.locfileid: "65220821"
---
# <a name="precompiled-header-files"></a>Файлы предварительно скомпилированных заголовков

При создании нового проекта в Visual Studio *предкомпилированного файла заголовка* с именем «pch.h» добавляется в проект. (В более ранних версиях Visual Studio, файл был вызван «stdafx.h».) Файл предназначена для ускорения процесса сборки. Любой стабильной файлы заголовков, например заголовки стандартной библиотеки например `<vector>`, должны быть включены здесь. Предкомпилированный заголовок компилируется только в том случае, когда или все файлы, которые он содержит, изменяются. Если только внести изменения в исходный код проекта, сборка будет пропускать компиляции для предкомпилированного заголовка. 

Параметры компилятора для предкомпилированных заголовков, являются [/Y](reference/y-precompiled-headers.md). На страницах свойств проекта параметры находятся в разделе **свойства конфигурации > C/C++ > предварительно скомпилированные заголовки**. Вы можете не использовать предкомпилированные заголовки, и можно указать заголовок имя и имя файла и путь выходного файла. 

## <a name="custom-precompiled-code"></a>Пользовательские предварительно скомпилированный код.

Для больших проектов, которые принимают значительное время на создание может потребоваться рассмотреть возможность создания пользовательских предварительно скомпилированных файлов. Компиляторы Microsoft C и C++ содержат параметры для предварительной компиляции любого кода C или C++, включая встроенный код. Данная возможность позволяет скомпилировать стабильное тело кода, сохранять скомпилированное состояние кода в файле, а при последующих компиляциях объединять предварительно скомпилированный код с кодом, который еще находится в стадии разработки. Каждая последующая компиляция выполняется быстрее, так как стабильный код не нужно компилировать повторно.
...
Можно предварительно скомпилировать любой C или C++ кода; Вы не ограничены предварительную компиляцию только файлы заголовков.
...
Параметры предварительно скомпилированного заголовка [/Yc (создать предкомпилированный заголовочный файл)](reference/yc-create-precompiled-header-file.md) и [/Yu (использование файла предкомпилированного заголовка)](reference/yu-use-precompiled-header-file.md). Используйте **/Yc** для создания предкомпилированного заголовка. При использовании с необязательным [hdrstop](../preprocessor/hdrstop.md) директивы pragma **/Yc** позволяет предварительной компиляции обоих файлов заголовков и исходного кода. Выберите **/Yu** использовать существующий предкомпилированный заголовок в существующей компиляции. Можно также использовать **/FP** с **/Yc** и **/Yu** варианты, чтобы альтернативное имя для предкомпилированного заголовка.
...
[/Yu](reference/yu-use-precompiled-header-file.md) параметры компилятора позволяют указать, какой файл PCH, следует использовать.
...
![Структура файла makefile, использующий файл предкомпилированного заголовка](media/vc30ow1.gif "структура файла makefile, использующий файл предкомпилированного заголовка") <br/>
...
Дополнительные сведения о файлах makefile, см. в разделе [справочнике NMAKE](reference/nmake-reference.md). Также см. в разделе [параметры компилятора MSVC](reference/compiler-options.md) и [параметры компоновщика MSVC](reference/linker-options.md).
