...
Чтобы использовать библиотеку, скачанную с помощью диспетчера пакетов **vcpkg** , можно проигнорировать приведенные ниже инструкции. См. раздел [vcpkg: Диспетчер C++ пакетов для Windows, Linux и MacOS](vcpkg.md#integrate-with-visual-studio-windows) для получения дополнительных сведений.
