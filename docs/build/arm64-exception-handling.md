...
1. Регистр указателя фрейма выделенный: Если сохранен в другом регистре в прологе, которое зарегистрировать (x29) не затрагиваются вся функция, что в любой момент может восстановить исходный.
...
Для фрейма связанных функций пары «fp-lr» могут быть сохранены в любой позиции в локальной переменной в зависимости от рекомендации по оптимизации. Целью является максимальное количество локальных переменных, которые могут быть достигнута по одной одной инструкции, на основе указателя фрейма (x29) или указатель стека (sp). Тем не менее для `alloca` функции, должны быть соединены и x29 должен указывать на нижней части стека. Чтобы разрешить большее покрытие register пара addressing-mode, защищенный регистр сохранить области располагаются в верхней части стека локальной сети. Ниже приведены примеры, иллюстрирующие некоторые из наиболее эффективный последовательности пролога. Ради ясности и лучшее расположение кэша порядок хранения вызываемой сохранения регистров в все канонические Прологи — в порядке «растущей вверх». `#framesz` ниже представляет размер всего стека (за исключением области alloca). `#localsz` и `#outsz` обозначают размер локальной сети (включая сохранения область для \<x29, lr > пары) и исходящие размер параметра, соответственно.
...
1. Применение, не являющихся конечными функции (lr сохраняется в области сохранения Int)
   ...
   Все локальные осуществляется на основании SP. \<x29 > указывает на предыдущий кадр анимации.
...
1. Объединять в цепочки, размер пакета > 512 (необязательно для функций без alloca)
   ...
   В целях оптимизации x29 можно поместить в любом месте в локальной сети для обеспечения большее покрытие «reg пары» и до/после-indexed смещение режим адресации. "Локальные" ниже указатели на фреймы может осуществляться в зависимости от SP.
...
1. 1 или 2 слов Заголовок описания общий размер структуры и предоставляя ключевые функциональные данные. Второе слово присутствует, только если оба **число эпилога** и **слов кодов** поля задается значение 0. Ниже приведены битовые поля в заголовке.
   ...
   Д. **Число эпилога** представляет собой 5-битовое поле, имеющее два значения в зависимости от состояния **E** бит:
      ...
      1. Если **E** имеет значение 0: счетчик общее число областей эпилога, описанные в разделе 2. Если присутствует более 31 области существует в функции, а затем **слов кодов** поле должно быть присвоено значение 0 указывает на необходимость в слове расширения.

      2. Если **E** имеет значение 1, то это поле указывает индекс первого кода очистки, описывающий один и только эпилога.

   f. **Код слова** представляет собой 5-битовое поле, указывающее число 32-разрядных слов, необходимые для размещения всех кодов очистки в разделе 3. Если требуются более 31 слов (т. е. более чем 124 очистки байты кода), то это поле должен иметь значение 0, чтобы указать на потребность в слове расширения.
...
1. После заголовка и необязательно расширенный заголовок, описанных выше, если **число эпилога** не равно нулю, список сведений об областях эпилога одной слово упакованы и хранятся в порядке увеличения начального смещения. Каждая область содержит следующие биты:
...
1. После списка областей эпилога идет массив байтов, содержащий коды очистки, подробно описаны в следующем разделе. Этот массив дополняется в конец ближайшей границы полного слова. Очистки коды записываются в этот массив, начиная с наиболее тело функции, двигаясь края функции. Байты, предназначенные для каждого кода очистки хранятся в порядке с прямым порядком байтов, они позволяет извлекать напрямую, начиная с наиболее важный, во-первых, который определяет операцию и длину остальной части кода.

1. Наконец, после байтов кодов очистки Если **X** бит в заголовке было задано значение 1, включает сведения об обработчике исключений. Он состоит из одного **RVA обработчик исключений** адрес обработчика исключений, сопровождаемую переменной длины, объем данных, необходимый для обработчика исключений.
...
Следует отметить, несмотря на то, что пролог и каждый эпилог имеет собственный индекс в кодах очистки, таблица используется ими совместно, что это вполне возможно, (а не полностью редко), они могут совместно использовать те же коды (см. Пример 2 в разделе примеры метка ак). Создателям компиляторов следует оптимизировать для этого случая, в частности потому, что наибольший индекс, который можно задать, равно 255, таким образом уменьшая число кодов очистки для конкретной функции.
...
|`save_r19r20_x`|    001zzzzz: Сохранить \<x19, x20 > парой в [sp-#Z * 8]!, смещение предварительно индексированных > =-248 |
|`save_fplr`|        01zzzzzz: Сохранить \<x29, lr > связываться в [sp + #Z * 8], смещение \<= 504. |
|`save_fplr_x`|        10zzzzzz: Сохранить \<x29, lr > связываться в [sp-(#Z + 1) * 8]!, смещение предварительно индексированных > = -512 |
|`alloc_m`|        11000xxx "xxxxxxxx: выделить огромная стопка с размером \< 16 КБ (2 ^ 11 * 16). |
|`save_regp`|        110010xx "xxzzzzzz: сохранить x(19+#X) парой в [sp + #Z * 8], смещение \<= 504 |
|`save_regp_x`|        110011xx "xxzzzzzz: сохранить x(19+#X) пары в [sp-(#Z + 1) * 8]!, смещение предварительно индексированных > = -512 |
|`save_reg`|        110100xx "xxzzzzzz: сохраните reg x(19+#X) в [sp + #Z * 8], смещение \<= 504 |
|`save_reg_x`|        1101010 x "xxxzzzzz: сохраните reg x(19+#X) в [sp-(#Z + 1) * 8]!, смещение предварительно индексированных > = -256 |
|`save_lrpair`|         1101011 x "xxzzzzzz: сохранить пару \<x (19 + 2 *#X), lr > в [sp + #Z*8], смещение \<= 504 |
...
|`add_fp`|        11100010' xxxxxxxx: Настройка x29 с: Добавление x29, sp, #x * 8 |
...
- **Флаг** представляет собой 2-битовое поле, как описано выше, статусу:
  - 00 = упакованные данные очистки не используются; оставшиеся биты пункты в записи .xdata
...
- **CR** представляет собой 2-битовый флаг, указывающее, включаются ли функция Дополнительные инструкции по настройке цепочки кадров и возврата ссылку:
  - 00 = применение функции \<x29, lr > пары не сохраняется в стеке.
  - 01 = применение функции \<lr > сохраняется в стеке
  - 10 = зарезервировано;
  - 11 = цепочке функция инструкцию пары хранилища и нагрузки используется в пролога и эпилога \<x29, lr >
- **H** 1-битовый флаг, указывающее, является ли функция начальное расположение целочисленный параметр регистрирует (x0 x7) путем их сохранения в самом начале функции. (0 = не помещает в начальное, 1 = помещает в начальное расположение).
- **RegI** представляет собой 4-битовое поле, представляющее число неизменяемых INT регистров (x19 x28) сохраняется в расположении каноническую стека.
- **RegF** представляет собой 3-битовое поле, представляющее число неизменяемых FP регистров (d8 d15) сохраняется в расположении каноническую стека. (RegF = 0: не регистр FP сохраняется; RegF > 0: RegF + 1 FP регистры сохраняются). Упакованные очистки данных не может использоваться для функции, которая сохранить только один регистр FP.
...
Шаг 5. Выделить оставшиеся стек, включая локальной сети \<x29, lr > пары, а исходящие область параметров. 5а соответствует канонического типа 1. 5b и 5c — для канонического типа 2. 5d и 5e предназначены для оба типа 3 и введите 4.
...
5б|**CR** == 11 &AMP; &AMP;<br/>512 < #locsz < = 4080|3|`sub sp,sp,#locsz`<br/>`stp x29,lr,[sp,0]`<br/>`add x29,sp,0`|`alloc_m`<br/>`save_fplr`<br/>`set_fp`
...
\*\*\* Нет соответствующей инструкции для `mov x29,sp` присутствует в эпилога. Упакованные очистки данных нельзя использовать, если для функции требуется восстановление sp из x29.
