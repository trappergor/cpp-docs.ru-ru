...
1. Предоставьте достаточное описание, чтобы разрешить раскрутку без проверки кода во всех случаях.
   ...
   - Если очистка не может быть полностью описана с помощью кодов очистки, в некоторых случаях необходимо вернуться к декодированию инструкций. Это увеличивает общую сложность, и в идеале следует избегать.
...
1. Регистр выделенного указателя фрейма: если SP сохранен в другом регистре (X29) в прологе, Регистр остается нетронутым по всей функции. Это означает, что исходный пакет обновления может быть восстановлен в любое время.
...
1. Неконечные функции без привязке (LR сохраняются в сохраненной области int)
   ...
   \* выделение области хранения reg не делится на STP, поскольку предварительно индексированный STP-LR не может быть представлен с кодами очистки.
...
![Структура записи. pdata](media/arm64-exception-handling-pdata-record.png "Структура записи. pdata")
...
![формат записи. XData](media/arm64-exception-handling-xdata-record.png "формат записи. XData")
...
1. Один или два слова, описывающие общий размер структуры и предоставляющие данные о ключевых функциях. Второе слово имеется только в том случае, если в полях количество и **код** **эпилога** задано значение 0. Заголовок содержит следующие битовые поля:
   ...
   а. **Длина функции** — это 18-разрядное поле. Указывает общую длину функции в байтах, деленную на 4. Если функция больше 1 МБ, для описания функции необходимо использовать несколько записей. pdata и. XData. Дополнительные сведения см. в разделе [крупные функции](#large-functions) .

   б. « **Сторно** » — это 2-битовое поле. Он описывает версию оставшегося. XData. В настоящее время определена только версия 0, поэтому значения 1-3 не разрешены.

   в. **X** является 1-битовым полем. Он указывает на присутствие (1) или отсутствие (0) данных исключения.

   . **E** — это 1 битовое поле. Он указывает, что информация, описывающая один заключительный фрагмент, упаковывается в заголовок (1), а не требует дополнительных слов области позже (0).
   ...
   е) **Слова Code** — это 5-битовое поле, которое указывает число 32-разрядных слов, которые должны содержать все коды очистки в разделе 3. Если требуется более 31 слова (т. е. Если количество байтов кода очистки превышает 124), то это поле должно быть равно 0, чтобы указать, что требуется расширение слова.
...
1. Если **Счетчик эпилога** не равен нулю, список сведений об областях эпилога, упакованный в слово, поступает после заголовка и дополнительного расширенного заголовка. Они хранятся в порядке возрастания начального смещения. Каждая область содержит следующие биты:
   ...
   а. **Начальное смещение эпилога** — это 18-битовое поле, которое имеет смещение в байтах, деленное на 4, относительно начала функции.

   б. **RES** — это 4-битовое поле, зарезервированное для будущего расширения. Оно должно иметь значение 0.
...
|`save_fplr`|        01zzzzzz: Save \<X29, LR > парой на `[sp+#Z*8]`, offset \<= 504. |
...
|`save_regp`|        110010xx'xxzzzzzz: сохранение пары x (19 + #X) на `[sp+#Z*8]`, смещение \<= 504 |
|`save_regp_x`|        110011xx'xxzzzzzz: сохранить пару x (19 + #X) на `[sp-(#Z+1)*8]!`, предварительно индексированное смещение > =-512 |
|`save_reg`|        110100xx'xxzzzzzz: сохранение reg x (19 + #X) на `[sp+#Z*8]`, смещение \<= 504 |
|`save_reg_x`|        1101010x'xxxzzzzz: сохранение reg x (19 + #X) на `[sp-(#Z+1)*8]!`, предварительно индексированное смещение > =-256 |
|`save_lrpair`|         1101011x'xxzzzzzz: Save паре \<x (19 + 2 * #X), LR > на `[sp+#Z*8]`, смещение \<= 504 |
|`save_fregp`|        1101100x'xxzzzzzz: сохранить пару d (8 + #X) на `[sp+#Z*8]`, смещение \<= 504 |
|`save_fregp_x`|        1101101x'xxzzzzzz: сохранить пару d (8 + #X), с `[sp-(#Z+1)*8]!`, предварительно индексированное смещение > =-512 |
|`save_freg`|        1101110x'xxzzzzzz: сохранение reg d (8 + #X) на `[sp+#Z*8]`, смещение \<= 504 |
...
В прологе не допускаются одноадресные смещения. Все диапазоны смещения (#Z) соответствуют кодированию адресации STP/STR, за исключением `save_r19r20_x`, в которых 248 достаточно для всех областей сохранения (10 регистров int + 8 FP регистров + 8 входных регистров).

`save_next` должен следовать паре регистров для сохранения переменной int или FP: `save_regp`, `save_regp_x`, `save_fregp`, `save_fregp_x`, `save_r19r20_x`или другой `save_next`. Она сохраняет следующую пару регистров в следующем 16-байтовом слоте в возрастающем порядке. `save_next` ссылается на первую пару регистра FP, если она соответствует `save-next`, обозначающей пару последних целочисленных регистров.

Поскольку размер обычных инструкций возврата и перехода одинаковы, нет необходимости в разделенном `end` коде очистки для сценариев заключительного вызова.

`end_c` предназначен для управления несмежными фрагментами функций в целях оптимизации. После `end_c`, указывающего на конец кодов очистки в текущей области, должен следовать другой ряд кода очистки с реальным `end`. Коды очистки между `end_c` и `end` представляют операции пролога в родительском регионе (фиктивном прологе).  Дополнительные сведения и примеры описаны в разделе ниже.

### <a name="packed-unwind-data"></a>Упакованные данные очистки

Для функций, чьи журналы и эпилоги следуют канонической форме, описанной ниже, можно использовать Упакованные данные для раскрутки. Он полностью устраняет необходимость в записи. XData и значительно сокращает затраты на предоставление данных очистки. Канонические журналы и эпилоги предназначены для удовлетворения общих требований простой функции: одна, не требующая обработчика исключений и выполняющая операции установки и уничтожения в стандартном порядке.
...
- **CR** — это 2-разрядный флаг, указывающий, содержит ли функция дополнительные инструкции по настройке цепочки кадров и возвращаемой ссылки:
  ...
  - 10 = зарезервировано;
  - 11 = цепочка функций, инструкция Store/Load в прологе или эпилога \<X29, LR >
...
- **Регф** — это 3-разрядное поле, указывающее количество регистров FP (D8-D15), которые были сохранены в каноническом расположении стека. (Регф = 0: регистр FP не сохраняется; Регф > 0: регистры Регф + 1 FP сохранены). Упакованные данные выкрутки нельзя использовать для функций, которые сохраняют только один регистр FP.

Канонические журналы, которые попадают в категории 1, 2 (без исходящего параметра), 3 и 4 в разделе выше, могут быть представлены упакованным форматом очистки.  Эпилоги для канонических функций соответствует аналогичной форме, за исключением того, что **H** не действует, инструкция `set_fp` опущена и порядок шагов и инструкций в каждом шаге отменяется в заключительном порядке. Алгоритм для упакованного. XData следует этим действиям, описанным в следующей таблице.

Шаг 0. предварительное вычисление размера каждой области.

Шаг 1. Сохранение вызываемых объектов int — сохраненные регистры.

Шаг 2. Этот шаг относится к типу 4 в ранних разделах. LR сохраняется в конце области int.

Шаг 3. Сохранение вызываемых объектов FP — сохраненные регистры.

Шаг 4. Сохранение входных аргументов в домашней области параметров.
...
d|(**CR** = = 00 \|\| **CR**= = 01) & &<br/>#locsz < = 4080|1|`sub sp,sp,#locsz`|`alloc_s`/`alloc_m`
5e|(**CR** = = 00 \|\| **CR**= = 01) & &<br/>#locsz > 4080|2|`sub sp,sp,4080`<br/>`sub sp,sp,#(locsz-4080)`|`alloc_m`<br/>`alloc_s`/`alloc_m`

\* если **CR** = = 01 и **Рег** — нечетное число, шаг 2 и последняя save_rep на шаге 1 будут объединены в один save_regp.

\*\* если **рег** == **CR** = = 0, а **регф** ! = 0, то первый STP для операции с плавающей запятой выполняет предварительное уменьшение.

\*\*\* в эпилога отсутствует инструкция, соответствующая `mov x29,sp`. Упакованные данные очистки нельзя использовать, если функции требуется восстановление SP из X29.
...
1. Только эпилоги (регион 2: Пролог находится в регионе узла)
   ...
   Предполагается, что при переходе по времени в этот регион были выполнены все коды пролога. Частичная очистка может выполняться в эпилоги так же, как и в обычной функции. Этот тип региона не может быть представлен Compact. pData. В записи Full. XData ее можно закодировать с помощью "фантомного" пролога, заключенного в скобки с помощью `end_c` и `end` пары кода очистки.  В начале `end_c` указывает, что размер пролога равен нулю. Начальный индекс эпилога для одного пункта эпилога, в `set_fp`.
...
1. Без журналов или эпилоги (регион 3: журналы и все эпилоги находятся в других фрагментах):
   ...
   Формат Compact. pdata можно применить с помощью флага установки = 10. С полной записью XData, число эпилога = 1. Код очистки совпадает с кодом для региона 2 выше, но начальный индекс эпилога также указывает на `end_c`. Частичная очистка никогда не будет выполняться в этом регионе кода.
...
Регион 1. `set_fp`, `save_regp 0,240`, `save_fplr_x_256`, `end` с заключительным индексом "начало индекса" указывает на `set_fp` как обычно.

Регион 2: `save_regp 2, 224`, `end_c`, `set_fp`, `save_regp 0,240`, `save_fplr_x_256`, `end`. Начальный индекс эпилога указывает на первый `save_regp 2, 224`кода очистки.
...
### <a name="example-3-variadic-unchained-function"></a>Пример 3. Variadic. функция с несцеплением
