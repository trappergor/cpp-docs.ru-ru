...
**/E** параметр отменяет компиляцию. Отправьте предварительно обработанного файла для компиляции. **/E** также подавляются выходные файлы из **/FA**, **/Fa**, и **/Fm** параметры. Дополнительные сведения см. в разделе [/FA, /Fa (файл листинга)](fa-fa-listing-file.md) и [/Fm (имя файла сопоставления)](fm-name-mapfile.md).

Чтобы подавить `#line` использовать директивы, [/EP (Предварительная обработка в stdout без директив #line)](ep-preprocess-to-stdout-without-hash-line-directives.md) вместо него.

Для отправки предварительно обработанные выходные данные в файл вместо `stdout`, использовать [/P (Предварительная обработка в файл)](p-preprocess-to-a-file.md) вместо него.
