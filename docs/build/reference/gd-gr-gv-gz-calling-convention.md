...
**/Gv** определяет соглашение о вызовах `__vectorcall` для всех функций, кроме функций-членов C++, функций с именем main, функций с переменным списком аргументов `vararg` или функций, помеченных конфликтующим атрибутом `__cdecl`, `__stdcall` или `__fastcall`. Это соглашение о вызовах доступно только в 32- и 64-разрядных архитектурах, поддерживающих /arch:SSE2 и более поздние версии, и игнорируется компиляторами, предназначенными для архитектуры ARM.
