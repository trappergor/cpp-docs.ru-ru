...
**/GV** указывает `__vectorcall` соглашение о вызовах для всех функций за исключением C++ функции-члены, функции с именем `main`, функции с `vararg` переменное число аргументов или функции, помеченные атрибутом конфликтующее `__cdecl`, `__stdcall`, или `__fastcall` атрибута. Это соглашение о вызовах доступно только в 32- и 64-разрядных архитектурах, поддерживающих /arch:SSE2 и более поздние версии, и игнорируется компиляторами, предназначенными для архитектуры ARM.
...
## <a name="cdecl-specifics"></a>Особенности соглашения __cdecl
...
## <a name="fastcall-specifics"></a>Особенности соглашения __fastcall
...
## <a name="stdcall-specifics"></a>Особенности соглашения __stdcall
...
## <a name="vectorcall-specifics"></a>Особенности соглашения __vectorcall

Целочисленные аргументы функции `__vectorcall` передаются по значению с использованием до двух (32-разрядная архитектура) или четырех (64-разрядная архитектура) целочисленных регистров и до шести регистров XMM для значений с плавающей запятой или векторных значений. Дополнительные аргументы передаются в стеке справа налево. Вызываемая функция очищает стек до возврата управления. Возвращаемые векторные значения и значения с плавающей запятой возвращаются в XMM0.
