...
- Нет
   ...
   Без параметров **/CLR** создает метаданные для приложения. Создает метаданные для приложения, которые могут использовать другие CLR-приложения, а также позволяет приложению применять типы и данные в метаданных других CLR-компонентов. Дополнительные сведения см. в разделе [смешанные (собственные и управляемые) сборки](../../dotnet/mixed-native-and-managed-assemblies.md) и [как: Миграция в/CLR](../../dotnet/how-to-migrate-to-clr.md).

- **чистые**

   **/ CLR: pure является нерекомендуемым**. Будущие версии компилятора могут не поддерживать этот параметр. Мы рекомендуем перенести код, который должен быть чистым кодом MSIL, на C#.

- **safe**

   **/ CLR: safe является устаревшим**. Будущие версии компилятора могут не поддерживать этот параметр. Мы рекомендуем перенести код, который должен быть чистого MSIL для C#.
...
1. Изменить **поддержки Common Language Runtime** свойство.
