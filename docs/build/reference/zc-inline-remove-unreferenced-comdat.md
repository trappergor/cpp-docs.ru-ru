...
Если параметр **/Zc: Inline** включен, то тот же код вызывает ошибку [LNK2019](../../error-messages/tool-errors/linker-tools-error-lnk2019.md) , так как компилятор не создает текст невстроенного кода для `Example::inline_call` в примере. obj. Это вызывает невстроенный вызов в `main` для ссылки на неопределенный внешний символ.
