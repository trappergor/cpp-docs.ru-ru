---
title: Стандартные макросы для команд MSBuild и свойств
ms.date: 03/20/2019
f1_keywords:
...
В зависимости от параметров установки Visual Studio можно сделать сотни макросы доступны в проект Visual Studio (с учетом MSBuild). Они соответствуют свойствам MSBuild, заданным по умолчанию, указанным в файлах PROPS или TARGETS либо в параметрах проекта. Эти макросы можно использовать в любом месте диалогового окна **Страницы свойств** проекта, где допустимы строки. В этих макросах регистр не учитывается.

## <a name="view-the-current-properties-and-macros"></a>Просмотр текущих свойств и макросов

Для отображения всех доступных макросов в **страницы свойств** диалогового окна в разделе **каталоги VC ++**, нажмите кнопку со стрелкой раскрывающегося списка в конце строки свойства. Щелкните **изменить** и затем в диалоговом окне редактирования выберите **макросы** кнопки. Текущий набор свойств и макросов, видимых для Visual Studio, отображается вместе с текущим значением для каждого. Дополнительные сведения см. в разделе **specifying User-Defined значения** раздел [Справочник по страницам свойств проекта C++](property-pages-visual-cpp.md).

![Кнопка "макросы" VC ++](../media/vcppdir_libdir_macros.png "меню макросы")

## <a name="list-of-common-macros"></a>Список общих макросов

Эта таблица описывает часто используемые подмножество доступных макросов; реализовано множество более нет в списке. Перейдите к **макросы** диалогового окна, чтобы увидеть все свойства и их текущие значения в проекте. Дополнительные сведения о том, как определения свойств MSBuild создаются и используются как макросы в файлах VCXPROJ, PROPS и TARGETS, см. в разделе [Свойства MSBuild](/visualstudio/msbuild/msbuild-properties).
...
|**$(PlatformShortName)**|Короткое имя текущей архитектуре, например, «x86» или «x64".|
