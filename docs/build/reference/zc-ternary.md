...
**/Zc:ternary** параметр выключен по умолчанию. Используйте **/Zc:ternary** для включения поведения, соответствующие требованиям, или **/Zc:ternary-** для явного указания прежнее поведение компилятора несоответствующих. [/ Разрешительным-](permissive-standards-conformance.md) параметр неявно включает этот параметр, но его можно переопределить с помощью **/Zc:ternary-**.
