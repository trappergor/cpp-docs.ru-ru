...
Можно передать имя объединенный PGD-файла, компоновщик во время оптимизации сборки с помощью **PGD =**_filename_ аргумент [/useprofile](useprofile.md) параметр компоновщика, или с помощью с помощью устаревших **/PGD** параметр компоновщика. Если вы объединить PGC-файлы в файл с именем *base_name*.pgd, не нужно указать имя файла в командной строке, так как компоновщик выбирает имя файла по умолчанию.
