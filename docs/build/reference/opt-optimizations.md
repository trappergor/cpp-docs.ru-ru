...
По умолчанию **/OPT: ICF** включен компоновщиком, если не **/OPT: NOICF** или [/DEBUG](debug-generate-debug-info.md) указан. Чтобы переопределить поведение по умолчанию и предотвратить COMDAT свертывается в программе, укажите **/OPT: NOICF**.
...
Можно использовать [/VERBOSE](verbose-print-progress-messages.md) параметр компоновщика, чтобы просмотреть функции, которые будут удалены с **предотвратят** и функций, свертываемых при **/OPT: ICF**.
