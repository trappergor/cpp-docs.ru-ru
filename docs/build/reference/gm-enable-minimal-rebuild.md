...
**/GM** является устаревшим. Он не может запустить сборку для определенных видов изменений в файле заголовка. Этот параметр можно безопасно удалить из проектов. Позволяет сократить время сборки, рекомендуется использовать предкомпилированные заголовки и добавочное и параллельные сборки параметров. Список параметров компилятора, см. в разделе **нерекомендуемые и удаленные параметры компилятора** статьи [параметры компилятора, упорядоченные по категориям](../../build/reference/compiler-options-listed-by-category.md).
