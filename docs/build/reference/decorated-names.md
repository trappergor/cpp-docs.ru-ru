...
Декорированные соглашения об именовании изменились в различных версиях Visual Studio и могут отличаться в разных целевых архитектурах. Для правильной связи с исходными файлами, созданными с помощью Visual Studio C++ , C и библиотеки DLL и библиотеки должны быть скомпилированы с использованием того же набора инструментов компилятора, флагов и целевой архитектуры.
...
##  <a name="format-of-a-c-decorated-name"></a><a name="Format"></a>Формат C++ декорированного имени
...
|`__stdcall`|Начальный символ подчеркивания ( **_** ) и символ в конце ( **\@** ), за которым следует число байтов в списке параметров в десятичном формате|
...
|`__vectorcall`|Два знака в конце ( **\@\@** ), за которыми следует десятичное число байтов в списке параметров|
...
1. Создайте список, выполнив компиляцию исходного файла, содержащего данные, объект или определение функции или прототип с параметром компилятора [Тип файла листинга](fa-fa-listing-file.md) , установленным в Assembly с исходным кодом ( **/FAS**).
   ...
   Например, введите `cl /c /FAs example.cpp` в командной строке разработчика, чтобы создать файл листинга, например ASM.
