...
@No__t-0, содержащий узлы `ClInclude`, создается при первом запуске проекта. При создании собственных файлов VCXPROJ убедитесь, что все элементы проекта также имеют запись в файле фильтров. Значения в узле `ClInclude` переопределяют фильтрацию по умолчанию на основе расширений файлов. При добавлении нового элемента в проект с помощью Visual Studio интегрированная среда разработки добавит запись отдельного файла в файл фильтров. Если изменить расширение файла, фильтр не будет автоматически переназначен. 
