...
`PgoAutoSweep` Сохраняет текущий счетчик данные профиля в файле и затем сбрасывает счетчики. Используйте функцию во время профильной оптимизации обучение для записи всех данных профиля из выполняющейся программе для `.pgc` файл для последующего использования в оптимизации сборки.
...
`.pgc` Файлы, созданные `PgoAutoSweep` должны быть объединены в `.pgd` файл, используемый для создания оптимизированного исполняемый файл. Можно использовать [pgomgr](pgomgr.md) команду, чтобы выполнить слияние.

Можно передать имя объединенный `.pgd` файл во время оптимизации сборки с помощью компоновщика **PGD =** _filename_ аргумент [/useprofile](reference/useprofile.md) компоновщика параметр, или с помощью устаревших **/PGD** параметр компоновщика. При объединении `.pgc` файлы в файл с именем *base_name*.pgd, не нужно указать имя файла в командной строке, так как компоновщик выбирает имя файла по умолчанию.

`PgoAutoSweep` Функция поддерживает указанный параметр потокобезопасность при создании инструментированной сборки. Если вы используете параметр по умолчанию или указать **NOEXACT** аргумент [/genprofile или/fastgenprofile](reference/genprofile-fastgenprofile-generate-profiling-instrumented-build.md) вызовы параметр компоновщика `PgoAutoSweep` не являются поточно ориентированными. **EXACT** аргумент создает поточно ориентированными и повысить точность, но медленнее, инструментированный исполняемый файл.
...
В примере ниже используется `PgoAutoSweep` для создания двух `.pgc` файлов на разных этапах во время выполнения. Первый содержит данные, описывающие поведение среды выполнения до `count` равно 3, а второй содержит данные, собранные с этого момента до того, как завершение работы приложения.
