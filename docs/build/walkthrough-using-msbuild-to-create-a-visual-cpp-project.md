---
title: Пошаговое руководство. Создание проекта Visual C++ с помощью MSBuild
ms.date: 05/16/2019
helpviewer_keywords:
...
1. В элемент `<ItemGroup>` добавьте приведенные ниже дочерние элементы `<ProjectConfiguration>`. Они определяют конфигурации отладки и выпуска для 32-разрядной операционной системы Windows:
...
1. Добавьте в элемент `<ItemGroup>` приведенный ниже дочерний элемент `<ClCompile>`. Он указывает имя исходного файла C/C++, подлежащего компиляции:
...
1. Добавьте в элемент `<ItemGroup>` приведенный ниже дочерний элемент `<ClInclude>`. Он указывает имя файла заголовка для исходного файла C/C++:
