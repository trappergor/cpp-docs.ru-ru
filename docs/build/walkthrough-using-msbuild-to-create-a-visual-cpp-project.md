...
В этом пошаговом руководстве показано, как использовать MSBuild для сборки проекта Visual C++ из командной строки. Вы узнаете, как для создания исходных файлов C++ и XML файла проекта для консольного приложения Visual C++. После создания проекта, вы узнаете, как настроить процесс построения.
...
- Копия Visual Studio с **разработки настольных приложений с помощью C++** установлен рабочей нагрузки.

- Общее представление о системе MSBuild.

> [!NOTE]
> Не используйте этот подход, если вы собираетесь изменить файл проекта позже с помощью интегрированной среды разработки Visual Studio. При создании VCXPROJ-файл вручную, Интегрированной среде разработки Visual Studio может невозможно изменить или загрузить его, особенно в том случае, если в проекте используются подстановочные знаки в элементах проекта.

> [!NOTE]
> Большая часть инструкции низкого уровня сборки находятся в **.targets** и **с расширением PROPS** файлы, которые определены в каталоге VCTargets, хранящегося в свойстве `$(VCTargetsPath)`. Путь по умолчанию для этих файлов в Visual Studio 2017 г Enterprise Edition — C:\\Program Files (x86)\\Microsoft Visual Studio\\2017 г.\\Enterprise\\Common7\\IDE\\ VC\\VCTargets\\.

## <a name="creating-the-c-source-files"></a>Создание исходных файлов C++

В этом пошаговом руководстве вы создадите проект, имеющий исходный файл и файл заголовка. Исходный файл main.cpp содержит функцию main консольного приложения. Файл заголовка main.h содержит код, чтобы включить файл заголовка iostream. Эти файлы C++ можно создать с помощью Visual Studio или текстовый редактор, например кода Visual Studio.
...
В XML-файл, содержащий корневой элемент проекта является файл проекта MSBuild (\<проекта >). В следующем примере проекта \<проекта > элемент содержит семь дочерних элементов:
...
6. Добавьте следующие \<ClCompile > дочерний элемент в \<ItemGroup > элемент. Этот дочерний элемент задает имя исходного файла C/C++ для компиляции:
   ...
   > [!NOTE]
   > \<ClCompile > — *собран* и определяется в **VCTargets** каталога.
...
MSBuild создает каталог для выходных файлов и компилирует и связывает проект для создания программы Myproject.exe. После завершения процесса сборки, используйте следующую команду для запуска приложения:
...
MSBuild можно выполнения целевых объектов построения стандартных, применяются пользовательские свойства и использования настраиваемых инструментов, событий и этапов построения. В этом разделе рассмотрены следующие задачи:
...
MSBuild предоставляет различные способы для настройки процесса построения. Добавление к проекту MSBuild пользовательские шаги построения, инструменты и события в следующих темах:
