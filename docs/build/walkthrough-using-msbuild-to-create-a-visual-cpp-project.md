...
1. Создайте файл с именем main.cpp и добавьте в этот файл следующий код:
...
1. Создайте файл с именем main.h и добавьте в этот файл следующий код:
...
Файл проекта MSBuild — это XML-файл, содержащий корневой элемент проекта (`<Project>`). В следующем примере проекта `<Project>` элемент содержит семь дочерние элементы:

- Три тега группы элементов (`<ItemGroup>`), укажите конфигурацию проекта и платформу, имя исходного файла и имя файла заголовка.

- Три тега импорта (`<Import>`), укажите параметры размещения Microsoft Visual C++.
...
1. Добавьте следующие два `<ProjectConfiguration>` дочерних элементов в `<ItemGroup>` элемент. Этот дочерний элемент задает отладки и выпуска конфигураций для 32-разрядной операционной системы Windows:
...
1. Добавьте следующий `<Import>` элемент, задающий путь к параметрам C++ по умолчанию для этого проекта:
...
1. Добавьте следующий `<Import>` элемент, задающий путь к текущим параметрам C++ для этого проекта:

    ```xml
    <Import Project="$(VCTargetsPath)\Microsoft.Cpp.props" />
    ```

1. Добавьте следующий `<ClCompile>` дочерний элемент в `<ItemGroup>` элемент. Этот дочерний элемент задает имя исходного файла C/C++ для компиляции:
   ...
   > [!NOTE]
   > `<ClCompile>` — *разрабатывать* и определен в **VCTargets** каталога.

1. Добавьте следующий `<ClInclude>` дочерний элемент в `<ItemGroup>` элемент. Этот дочерний элемент задает имя файла заголовка для исходного файла C/C++:
...
1. Добавьте следующий `<Import>` элемент, задающий путь к файлу, который определяет целевой объект для этого проекта:
...
MSBuild создает каталог для выходных файлов и компилирует и связывает проект для создания программы Myproject.exe. После завершения процесса построения, используйте следующую команду, чтобы запустить приложение в папке отладки:
...
Объект *разрабатывать* — это именованный набор предварительно определенных или пользовательских команд, которые могут быть выполнены во время сборки. Используется параметр командной строки (`/t`) для указания целевого объекта построения. Для `myproject` пример проекта, предварительно определенных **чистой** целевой удаляет все файлы в папке debug и создает новый файл журнала.
...
Чтобы перестроить проект с набором инструментов Visual C++ 11.0 Windows XP, введите следующие команды:
