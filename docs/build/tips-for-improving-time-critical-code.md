...
- Отложить сортировку до времени, не критического для производительности.

- Сортировка данных в более раннем, не критическом для производительности времени.
...
Дополнительные сведения см. в разделе [Обработка бездействующего цикла](../mfc/idle-loop-processing.md) и [многопоточность](../parallel/multithreading-support-for-older-code-visual-cpp.md).
...
- Чтобы задать верхний и нижний пределы рабочего набора, используйте [сетпроцессворкингсетсизе](/windows/win32/api/winbase/nf-winbase-getprocessworkingsetsize).

- Чтобы получить верхний и нижний пределы рабочего набора, используйте [жетпроцессворкингсетсизе](/windows/win32/api/winbase/nf-winbase-setprocessworkingsetsize).
