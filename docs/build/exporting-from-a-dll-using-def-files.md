...
Если при экспорте функции в файле C++, необходимо либо поместить декорированные имена в DEF-файл, либо определить экспортируемые функции с помощью стандартных средств компоновки C с помощью extern «C». Если вам нужно поместить декорированные имена в DEF-файл, можно получить их с помощью [DUMPBIN](../build/reference/dumpbin-reference.md) средство или с помощью компоновщика [/MAP](../build/reference/map-generate-mapfile.md) параметр. Обратите внимание, что декорированные имена, созданные компилятором, зависят от компилятора. Если вы поместите декорированные имена, созданные корпорацией Майкрософт C++ компилятор (MSVC) в DEF-файл приложения, связанные с библиотекой DLL также должны быть построены с использованием той же версии MSVC, чтобы декорированные имена в вызывающем приложении соответствовали экспортируемым именам в DEF-файла библиотеки DLL. 

> [!NOTE]
> Библиотеки DLL, созданные с помощью Visual Studio 2015 могут использоваться приложениями, созданных с помощью Visual Studio 2017 или Visual Studio 2019.
