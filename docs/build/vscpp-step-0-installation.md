...
Добро пожаловать в Visual Studio 2019! В этой версии несложно для выбора и установки необходимых компонентов. И из-за его уменьшено минимальное занимаемое место, он устанавливает быстро и с меньшим влиянием на систему.
...
1. Проверьте [требования к системе](/visualstudio/releases/2019/system-requirements). Эти требования позволяют узнать, поддерживает ли ваш компьютер Visual Studio 2019.
...
Ознакомиться с вопросами о запуске предыдущих версий Visual Studio параллельно с Visual Studio 2019 [Visual Studio 2019 целевая платформа и совместимость](/visualstudio/releases/2019/compatibility/) страницы.
...
Запустите файл начального загрузчика, чтобы установить Visual Studio Installer. Этот новый упрощенный установщик включает в себя все необходимое для установки и настройки Visual Studio.
...
1. В папке **Загрузки** дважды щелкните файл начального загрузчика, имя которого совпадает с именем одного из следующих файлов или похоже на них:
   ...
   * **vs_enterprise.exe** для Visual Studio Enterprise;
   ...
   Если вы получили контроль учетных записей пользователей, выберите **Да**.

1. Мы попросим вас принять [условия лицензии](https://visualstudio.microsoft.com/license-terms/) и [заявление о конфиденциальности](https://privacy.microsoft.com/privacystatement) корпорации Майкрософт. Выберите **по-прежнему**.
...
> [!TIP]
> В любой момент после установки можно установить рабочие нагрузки или компоненты, которые не были установлены изначально. Если среда Visual Studio открыта, выберите пункт **Сервис** > **Получить средства и компоненты...**; откроется Visual Studio Installer. **Visual Studio Installer** можно также открыть из меню "Пуск". После этого можно выбрать рабочие нагрузки или компоненты, которые вы хотите установить. Выберите **изменить**.

## <a name="step-5---choose-individual-components-optional"></a>Шаг 5 — выбор отдельных компонентов (необязательно)

Если вы не хотите пользоваться функцией рабочих нагрузок для настройки установки Visual Studio, или вы хотите добавить дополнительные компоненты, не устанавливает рабочей нагрузки, это можно сделать путем установки или добавления отдельных компонентов **отдельные компоненты** вкладки. Выберите, а затем следуйте указаниям.

  ![Visual Studio 2019 г. — Установка отдельных компонентов](../get-started/media/vs-installer-individual-components.png "отдельные компоненты для установки Visual Studio")

## <a name="step-6---install-language-packs-optional"></a>Шаг 6. Установка языковых пакетов (необязательно)

По умолчанию при первом запуске установщик пытается использовать язык операционной системы. Чтобы установить Visual Studio на языке по своему выбору, выберите **языковые пакеты** вкладку с помощью установщика Visual Studio, а затем следуйте инструкциям на экране.

  ![Visual Studio 2019 г. — Установка языковых пакетов](../get-started/media/vs-installer-language-packs.png "языковые пакеты для установки Visual Studio")
...
На системном диске можно уменьшить объем установки Visual Studio. Вы можете переместить кэш загрузки, общие компоненты, пакеты SDK и средства на другие диски и оставить Visual Studio на самом быстром диске.

  ![Visual Studio 2019 г. — Изменение расположения установки](../get-started/media/vs-installer-installation-locations.png "изменение расположения установки")

> [!IMPORTANT]
> Можно выбрать другого диска только в том случае, при первой установке Visual Studio. Если вы уже установили его и изменить дисков, необходимо удалить Visual Studio, а затем переустановите его.

## <a name="step-8---start-developing"></a>Шаг 8. Начало разработки

1. После завершения установки Visual Studio, выберите **запуска** кнопку, чтобы приступить к разработке с помощью Visual Studio.

1. На начальном экране выберите **Создать проект**.

1. В поле поиска введите тип приложения, которые вы хотите создать, чтобы просмотреть список доступных шаблонов. Список шаблонов зависит от рабочих нагрузок, выбранный во время установки. Чтобы просмотреть различные шаблоны, выберите разных рабочих нагрузок.

   Можно также фильтровать поиск определенного языка программирования с помощью **языка** стрелку раскрывающегося списка. Можно отфильтровать с помощью **платформы** списка и **тип проекта** список, слишком.

1. Новый проект открывается в Visual Studio, и вы будете готовы кода!
...
В Visual Studio 2017 несложно для выбора и установки необходимых компонентов. И из-за его уменьшено минимальное занимаемое место, он устанавливает быстро и с меньшим влиянием на систему.
