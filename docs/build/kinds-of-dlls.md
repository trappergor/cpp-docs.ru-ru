...
С помощью Visual Studio, можно построить библиотеки DLL Win32 в C или C++ , не использующие библиотеки Microsoft Foundation Class (MFC). Можно создать проект библиотеки MFC DLL с помощью мастера приложений Win32.

Сама библиотека MFC доступна в библиотеках статической компоновки или в библиотеках DLL, с помощью мастера MFC DLL. Если библиотека DLL использует MFC, Visual Studio поддерживает три различных скрипта разработки библиотеки DLL:
...
Если библиотека DLL использует MFC, Visual Studio можно используйте для построения не - MFC библиотеки DLL Win32. Компоновка библиотеки DLL с MFC (статически или динамически) занимает значительное место на диске и памяти. Не нужно связывать с MFC, если библиотека DLL фактически не использует MFC.
