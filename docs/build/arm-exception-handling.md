...
Windows в ARM использует *коды очистки* для управления очисткой стека во время [структурированной обработки исключений](/windows/win32/debug/structured-exception-handling) (SEH). Коды очистки представляют собой последовательность байт, хранимых в разделе .XDATA исполняемого образа. Они описывают операции пролога функции и кода эпилога абстрактным способом, поэтому эффекты пролога функции могут быть отменены при подготовке к открутке в кадр стека вызывающего объекта.
...
- При возникновении исключения в теле функции не имеет значения, отменяются ли операции пролога, или операции эпилога выполняются в прямом направлении. Оба случая должны давать одинаковые результаты.
...
|Байт 1|Байт 2|Байт 3|Байт 4|Размер операции|Пояснение|
|------------|------------|------------|------------|------------|-----------------|
...
Коды очистки 0xFD и 0xFE эквивалентны обычному коду end 0xFF, однако имеют один дополнительный код операции nop в случае эпилога — 16- или 32-разрядный. Для прологов коды 0xFD, 0xFE и 0xFF полностью эквивалентны друг другу. Эти учетные записи для общих эпилога заканчивается `bx lr` или `b <tailcall-target>`, у которых нет эквивалентной инструкции пролога. Это повышает вероятность того, что последовательности очистки можно будет совместно использовать в прологе и эпилогах.
...
Рядом с каждым кодом операции находится подходящий код очистки для описания этой операции. Последовательность кодов очистки для пролога является зеркальным отображением кодов очистки для эпилога без учета последней инструкции. В этом случае часто предполагается, что коды очистки для пролога всегда хранятся в обратном порядке от порядка выполнения в прологе. Это дает нам общий набор кодов очистки:
...
Упакованные со сжатием функции чаще всего заранее выделяют место под сохранение дополнительных регистров в обычном прологе, а затем выполняют сохранения регистров с помощью `str` или `stm` вместо `push`. Это обеспечивает обработку всех указателей стека в исходном прологе функции.
