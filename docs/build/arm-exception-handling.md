---
title: Обработка исключений ARM | Документы Microsoft
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-tools
...
### <a name="assumptions"></a>Допущения  
 Исполняемые образы для Windows на ARM используют формат переносимого исполняемого файла (PE). Дополнительные сведения см. в разделе [Microsoft PE и COFF спецификации](http://go.microsoft.com/fwlink/p/?linkid=84140). Информация об обработке исключений хранится в разделах .PDATA и .XDATA образа.  
...
|-----------------|----------|-------------|  
|0|0—31|*Функция запуска RVA* является 32-разрядной относительный виртуальный адрес начала функции. Если функция содержит код бегунка, необходимо задать младший бит этого адреса.|  
|1|0—1|*Флаг* представляет собой 2-битовое поле, указывающее, как интерпретировать остальные 30 бит второго слова .pdata. Если *флаг* равно 0, то оставшиеся биты формируют *RVA сведения исключение* (с низким два бита неявно 0). Если *флаг* не равно нулю, то оставшиеся биты формируют *упакованные данные очистки* структуры.|  
|1|2—31|*Сведения об исключении RVA* или *упакованные данные очистки*.<br /><br /> *Исключение сведения RVA* адрес структуры сведений исключении переменной длины, хранимых в разделе .xdata. Эти данные должны быть выровнены по 4-байтовой границе.<br /><br /> *Упакованные данные очистки* представляет собой сжатое описание операций, необходимых для выполнения очистки из функции в канонической форме. В этом случае запись .XDATA не требуется.|  
...
|-----------------|----------|-------------|  
|0|0—31|*Функция запуска RVA* является 32-разрядной относительный виртуальный адрес начала функции. Если функция содержит код бегунка, необходимо задать младший бит этого адреса.|  
|1|0—1|*Флаг* представляет собой 2-битовое поле, имеющее следующие значения:<br /><br /> -00 = упакованные данные очистки не используются; остальные биты указывают на запись .xdata.<br />-01 = упакованные данные очистки.<br />-10 = упакованные данные, где функции предполагается отсутствие пролога очистки. Это удобно для описания фрагментов функции, которые разобщены с началом функции.<br />-11 = зарезервированным.|  
...
|1|13—14|*Возвращаемое значение* представляет собой 2-битовое поле, указывающее, каким образом функция возвращает:<br /><br /> -00 = возврат посредством извлечения {pc} ( *L* битовый флаг должно быть установлено в 1, в данном случае).<br />-01 = возвращение посредством 16-разрядного ветвления.<br />-10 = возвращение посредством 32-разрядного ветвления.<br />-11 = полное отсутствие эпилога вообще. Это удобно для описания разобщенного фрагмента функции, который может содержать только пролог, однако его эпилог находится в другом месте.|  
|1|15|*H* 1-битовый флаг, указывающий, является ли функция «регистры» целочисленный параметр регистрирует (r0 — r3) посредством их отправки в начале функции, и освобождает 16 байт стека перед возвратом. (0 = не помещает в начальное расположение, 1 = помещает в начальное расположение.)|  
|1|16—18|*REG* 3-битовое поле, которое указывает индекс последнего сохранения неизменяемого регистра. Если *R* бит равен 0, то сохраняются только целочисленные регистры и предполагается находятся в диапазоне от r4 — rN, где N равно 4 + *Reg*. Если *R* из битов равен 1, а затем только операций с плавающей запятой, сохраняются и предполагается находятся в диапазоне от d8 — dN, где N равно 8 + *Reg*. Особое сочетание *R* = 1 и *Reg* = 7 указывает, что никакие регистры не сохраняются.|  
|1|19|*R* представляет собой 1-битовый флаг, указывающее, являются ли сохраненные неизменяемые регистры целочисленных регистров (0) либо регистров с плавающей запятой (1). Если *R* равен 1 и *Reg* поле имеет значение 7, не неизменяемые регистры не отправлялись.|  
...
|1|21|*C* представляет 1-битовый флаг, который указывает, содержит ли функция Дополнительные инструкции по настройке цепочки кадров для ускорения проверки стека (1) или нет (0). Если этот бит задан, r11 неявно добавляется в список сохраняемых неизменяемых регистров для целочисленных операций. (См. описанные ниже ограничения при *C* используется флаг.)|  
|1|22—31|*Можно настроить стека* представляет собой 10-битовое поле, указывающее количество байт стека, выделенных для этой функции, поделенное на 4. Однако напрямую возможно кодирование лишь значений из диапазона 0x000—0x3F3. Функции, выделяющие более 4044 байт стека, должны использовать полную запись .XDATA. Если *настроить стек* поле имеет размер 0x3F4 и более, то 4 младших бита имеют особое значение:<br /><br /> -Биты 0 — 1 указывают число слов в подстройке стека (1-4) минус 1.<br />-Бит 2 имеет значение 1, если пролог объединил эту подстройку со своей операцией отправки.<br />-Бит 3 имеет значение 1, если эпилог объединил эту подстройку со своей операцией извлечения.|  
...
-   Если *C* флаг имеет значение 1:  
    ...
    -   *L* должен быть установлен флаг в значение 1, так как цепочка кадров требует как r11 и LR.  
  
    -   R11 не должно включаться в набор регистров, описываемый *Reg*. То есть, если r4 — r11 помещаются в стек, *Reg* должен описывать только r4-r10, так как *C* флага подразумевает r11.  
  
-   Если *Ret* поле имеет значение 0, *L* флаг должен иметь значение 1.  
  
 Нарушение этих ограничений ведет к неподдерживаемой последовательности операций.  
  
 В целях дальнейшего обсуждения двух псевдофлагов являются производными от *настроить стек*:  
  
-   *Общая папка* или «свертывание пролога» указывает, что *настроить стек* имеет размер 0x3F4 или больше и бит 2 имеет значение.  
...
|5|*Измените стек* ! = 0 и PF == 0|16/32|`sub sp,sp,#xx`|00—7F/E8—EB|  
  
 Инструкция 1 всегда будет доступен, если *H* бит равен 1.  
  
 Настройка цепочки кадров, присутствует инструкция 3а или 3б при *C* бита. Это 16-разрядная `mov`, если отправляются только регистры r11 и LR; в противном случае это 32-разрядная `add`.  
...
|-----------------|-----------------------------------|----------|------------|  
|6|*Измените стек*! = 0 и *EF*== 0|16/32|`add   sp,sp,#xx`|  
...
|10а|*Возвращаемое значение*== 1|16|`bx    reg`|  
|10б|*Возвращаемое значение*== 2|32|`b     address`|  
  
 Инструкция 6 является явной подстройкой стека, если указана подстройка без свертывания. Поскольку *PF* не зависит от *EF*, можно иметь инструкцию без 5 инструкции 6 или наоборот.  
  
 Инструкции 7 и 8 используют ту же логику как пролог, чтобы определить, какие регистры восстанавливаются из стека, но со следующими двух следующих изменений: во-первых, *EF* используется вместо *PF*; во-вторых, если *Ret*  = 0, то LR заменяется на PC в списке регистров и эпилог моментально завершается.  
  
 Если *H* имеет значение, то присутствует либо инструкция 9а, либо инструкция 9б. Инструкция 9а используется при *L* равно 0, чтобы указать, что LR не в стеке. В этом случае выполнена Ручная Подстройка стека и *Ret* должно равняться 1 или 2 для указания явного возврата. Инструкция 9б используется при *L* — 1, чтобы указать на раннее завершение эпилога, чтобы возвратить и подстроить стек, в то же время.  
  
 Если эпилог еще не закончен, затем либо инструкция 10А, присутствует 10б, указывая на 16- или 32-разрядное ветвление в зависимости от значения *Ret*.  
...
1.  Заголовок из 1 или 2 слов, описывающий общий размер структуры .XDATA и предоставляющий ключевые функциональные данные. Второе слово присутствует, только если *число эпилогов* и *слов кодов* поля равны 0. Эти поля описаны в следующей таблице.  
  
    |Слово|Биты|Назначение|  
    |----------|----------|-------------|  
    ...
    |0|18—19|*Йверы* представляет собой 2-битовое поле, описывающее версию оставшиеся xdata. Сейчас определена только версия 0; значения 1—3 зарезервированы.|  
    |0|20|*X* представляет собой 1-битовое поле, указывающее наличие (1) или отсутствие (0) данных исключения.|  
    |0|21|*E* является 1-битовое поле, которое указывает, что сведения, описывающие отдельного эпилога, упакована в заголовок (1) не потребует дополнительную область слова более поздней версии (0).|  
    |0|22|*F* представляет собой 1-битовое поле, указывающее, что данная запись описывает фрагмент функции (1) или всю функцию целиком (0). Если указан фрагмент, пролог отсутствует, поэтому его обработку можно проигнорировать.|  
    |0|23—27|*Число эпилогов* представляет собой 5-битовое поле, имеющее два значения в зависимости от состояния *E* бит:<br /><br /> -Если *E* равно 0, это поле представляет собой число общее число областей исключений, описанных в разделе 3. Если присутствует более 31 области в функции, то это поле и *слов кодов* поля должны быть равными 0 указывается потребность в слове расширения.<br />-Если *E* -1, это поле указывает индекс первого кода очистки, описывающего единственный эпилог.|  
    |0|28—31|*Код слова* — 4-битовое поле, которое указывает число 32-разрядных слов, требуемое для размещения всех кодов очистки в разделе 4. Если требуются более 15 слов для более чем 63 байт кодов очистки, это поле и *число эпилогов* поля должны быть равными 0 указывается потребность в слове расширения.|  
    |1|0—15|*Расширенные число эпилогов* представляет 16-битовое поле, которое предоставляет дополнительное место для кодирования необычайно большого числа эпилогов. Слово расширения, содержащее это поле присутствует, только если *число эпилогов* и *слов кодов* поля в первом слове заголовка равны 0.|  
    |1|16—23|*Расширенный код слова* представляет собой 8-битовое поле, которое предоставляет дополнительное место для кодирования необычайно большого числа слов кодов очистки. Слово расширения, содержащее это поле присутствует, только если *число эпилогов* и *слов кодов* поля в первом слове заголовка равны 0.|  
    |1|24—31|Зарезервированное|  
  
2.  После исключения (если *E* бит в заголовке было задано значение 0) находится список информации об областях эпилога, которые упаковываются в слова и хранятся в порядке увеличения начального смещения. Каждая область содержит следующие поля.  
    ...
    |20—23|*Условие* представляет собой 4-битовое поле, содержащее условие, под которым выполняется эпилога. Для безусловных эпилогов в нем следует задать значение 0xE, которое означает "всегда". (Эпилог должен быть полностью условным или полностью безусловным, а в режиме Thumb-2 эпилог начинается с первой инструкции после кода операции IT.)|  
    |24—31|*Начальный индекс эпилога* представляет собой 8-битовое поле, которое указывает индекс байта первого кода очистки, описывающего этот эпилог.|  
...
4.  Если *X* поля в заголовке равно 1, после байтов кодов очистки располагаются сведения об обработчике исключений. Они состоят из одного *RVA обработчик исключений* , содержащий адрес обработчика исключений, за которым следует по объем (переменный) данных, необходимый для обработчика исключений.  
...
|F6|00—FF|||32|`vpop  {dS-dE}`<br /><br /> Где S равен ((код & 0x00F0) >> 4 + 16 и E равен (код & 0x000F) + 16.|  
...
-   Только эпилоги; пролог и, возможно, дополнительные эпилоги в других фрагментах.  
 ...
 В третьем и четвертом сценарии наличие псевдопролога указывается путем установки *флаг* поле краткой записи .pdata 2, или установив *F* флаг в заголовке .xdata 1. В любом случае проверка потребности в частичной очистке пролога игнорируется, и все очистки, не относящиеся к эпилогу, считаются полными.  
...
-   Слово 0  
    ...
    -   *Функция начала RVA* = 0x000535F8 (= 0x004535F8 — 0x00400000).  
  
-   Слово 1  
  
    -   *Флаг* = 1, указывает канонические форматы пролога и эпилога.  
  
    -   *Функция длина* = 0x31 (= 0x62/2)  
  
    -   *Возвращаемое значение* = 1, указывает возвращаемое 16-разрядного ветвления  
  
    -   *H* = 0, указывает, что параметры не помещены  
    ...
    -   *Можно настроить стека* = 0, указывает на отсутствие подстройки стека.  
...
-   Слово 0  
    ...
    -   *Функция начала RVA* = 0x000533AC (= 0x004533AC — 0x00400000)  
...
-   Слово 1  
    ...
    -   *Возвращаемое значение* = 0, указывает на возвращение посредством извлечения {pc}.  
  
    -   *H* = 0, указывает, что параметры не помещены  
  
    -   *R*= 0 и *Reg* = 3, указывает на отправку/извлечение для r4 — r7  
  
    -   *L* = 1, указывает сохранения/восстановления LR  
  
    -   *C* = 0, указывает на отсутствие цепочки кадров.  
  
    -   *Измените стек* = 3 (= 0x0C/4)  
...
-   Слово 0  
    ...
    -   *Функция начала RVA* = 0x00053988 (= 0x00453988 — 0x00400000).  
...
-   Слово 1  
    ...
    -   *Возвращаемое значение* = 0, указывает на посредством извлечения {pc} — возвращение (в этом случае ПК ldr [sp] #0x14 возвратить)  
  
    -   *H* = 1, указывает, что параметры помещены  
...
-   Слово 0  
    ...
    -   *Функция начала RVA* = 0x000592F4 (= 0x004592F4 — 0x00400000).  
...
-   Слово 0  
    ...
    -   *Йверы* = 0, указывает первую версию .XData  
  
    -   *X* = 0, указывает на отсутствие данных об исключении  
  
    -   *E* = 0, указывает списка областей эпилога  
...
-   Слово 0  
    ...
    -   *Функция начала RVA* = 0x00085A20 (= 0x00485A20 — 0x00400000).  
...
-   Слово 0  
    ...
    -   *Число эпилогов* = 0x001, указывающее, 1 общее число областей  
...
-   Слово 0  
    ...
    -   *Функция начала RVA* = 0x00088C24 (= 0x00488C24 — 0x00400000).  
...
-   Слово 0  
    ...
    -   *X* = 1, указывает на наличие данных исключений  
  
    -   *E* = 1, указывает на отдельный эпилог.  
  
    -   *F* = 0, указывает на полное описание функции, включая пролог.  
  
    -   *Число эпилогов* = 0x00, указывает, что коды очистки эпилога начинаются с смещения 0x00.  
...
-   Слово 0  
    ...
    -   *Функция начала RVA* = 0x00088C72 (= 0x00488C72 — 0x00400000).  
...
-   Слово 1  
    ...
    -   *R*= 0 и *Reg* = 7, указывающее, никакие регистры были сохранения/восстановления  
  
    -   *L* = 1, указывает сохранения/восстановления LR  
  
    -   *C* = 0, указывает на отсутствие цепочки кадров.  
  
    -   *Можно настроить стека* = 1, указывает на 1 подстройки стека × 4 байта  
