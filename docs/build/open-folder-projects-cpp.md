...
> [!WARNING]
> В настоящее время существует известная ошибка, при которой `INCLUDE` значение, указанное в `environments`, неправильно передается свойству `includePath`. Эту ошибку можно обойти, добавив Полный литерал include в массив `includePath`.
...
Если вы не видите нужный IntelliSense, можно устранить неполадки, перейдя в **меню Сервис**  > **Параметры**  > **текстовый редактор**  > **C/C++**   > **Дополнительно** и **включив ведение журнала** . в **значение true**. Чтобы начать, установите для параметра **уровень ведения журнала** значение 5 и **фильтры ведения журнала** — до 8.
