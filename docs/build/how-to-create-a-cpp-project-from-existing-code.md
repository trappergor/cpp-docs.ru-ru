...
1. Укажите расположение проекта, каталог исходных файлов и типы файлов, которые мастер импортирует в новый проект. Для продолжения нажмите кнопку **Далее**.
    ...
    ![размещение проекта;](media/location.png)

1. Укажите настройки проекта, например, среду сборки и ее параметры, соответствующие конкретному типу нового проекта. Для продолжения нажмите кнопку **Далее**.

    | Параметр | Описание |
    | --- | --- |
    | **Использование Visual Studio** | Предписывает использовать средства сборки, включенные в Visual Studio, для сборки нового проекта. Этот параметр выбран по умолчанию.<br/><br/>Выберите **Тип проекта**, чтобы указать тип проекта, создаваемого мастером. Выберите **Проект приложения Windows**, **Проект консольного приложения**, **Проект библиотеки DLL** или **Проект статической библиотеки (LIB)** .<br/><br/>Установите флажок **Добавить поддержку для ATL**, чтобы добавить ее в новый проект.<br/><br/>Установите флажок **Добавить поддержку для MFC**, чтобы добавить ее в новый проект.<br/><br/>Установите флажок **Добавить поддержку для среды CLR**, чтобы добавить ее в новый проект. Выберите **поддержку общеязыковой среды выполнения** для типа соответствия, например, **общеязыковая среда выполнения (Старый синтаксис)** для обеспечения соответствия C++ с управляемыми расширениями для синтаксиса, синтаксис программирования CLR перед Visual Studio 2005. |
...
1. Задайте нужные параметры отладочной конфигурации. Для продолжения нажмите кнопку **Далее**.
