...
1. В Visual Studio 2019 г., примите значения по умолчанию и нажмите кнопку **ОК**. 
   В Visual Studio 2017, щелкните **ОК** открыть **проекта ATL** мастера. Нажмите кнопку **параметры приложения** Чтобы просмотреть доступные параметры. Поскольку этот проект создает элемент управления, а элемент управления должен быть сервером в процессе, оставьте **тип приложения** как библиотеку DLL. Нажмите кнопку **ОК**.

Visual Studio создаст проект, создав несколько файлов. Вы можете просматривать эти файлы в **обозревателе решений** , развернув `Polygon` объекта. Ниже перечислены файлы.
