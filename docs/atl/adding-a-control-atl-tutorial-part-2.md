...
ms.locfileid: "67344331"
---
# <a name="adding-a-control-atl-tutorial-part-2"></a>Добавление элемента управления (учебник ATL, часть 2)

На этом шаге добавьте элемент управления в проект, выполнить его сборку и протестировать его на веб-страницы.
...
1. Тип `PolyCtl` качестве короткого имени и обратите внимание, что другие поля заполняются автоматически. Не нажимайте кнопку **Готово** , но, поскольку необходимо внести некоторые дополнительные изменения.
...
|**ProgID**|Читаемое имя, которое может использоваться для поиска CLSID элемента управления.|

Вы найдете несколько дополнительных параметров необходимо изменить в **управления ATL** мастера.
...
1. Выберите **точек подключения** "флажок". Этот параметр создает поддержка исходящий интерфейс в IDL-файле.
...
Можно также сделать элемент управления *вставляемые*, то есть встраивается в приложения, которые поддерживают внедренные объекты, например Excel или Word.
...
Как мастер создает элемент управления, возникают некоторые изменения кода и добавление файлов. Создаются следующие файлы:
...
- Добавляет `#include` инструкцию, чтобы файлы stdafx.h и stdafx.cpp для включения ATL файлы, необходимые для поддержки элементов управления.

- Изменяет Polygon.idl для включения сведений о новом элементе управления.

- Добавляет новый элемент управления в сопоставление объекта в Polygon.cpp.
...
1. На **построения** меню, щелкните **собрать полигон**.
    ...
    После завершения создания элемента управления щелкните правой кнопкой мыши файл PolyCtl.htm в **обозревателе решений** и выберите **просмотреть в браузере**. Отображается HTML-страницы, содержащей элемент управления. Вы увидите страницу с помощью заголовка «Тестовая страница ATL 8.0 для объекта PolyCtl» и элемента управления, текст PolyCtl.
...
> [!NOTE]
> При прохождении этого руководства, если появится сообщение об ошибке, что DLL-файл создать невозможно, закройте файл PolyCtl.htm и тестовый элемент управления ActiveX контейнер и повторно выполните сборку решения. Если вы по-прежнему не удается создать библиотеку DLL, перезагрузите компьютер или выйти из системы, если вы используете службы терминалов.
