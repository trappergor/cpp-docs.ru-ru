...
### <a name="remarks"></a>Примечания  
 Установка `pdwAttributes` для NULL предоставляет способ подтверждения существования `CSid` без доступа к атрибуту. Обратите внимание, что этот метод не должен использоваться проверять права доступа, как в среде Windows 2000 могут возникнуть неверные результаты. Приложения должны вместо этого использовать [CAccessToken::CheckTokenMembership](../../atl/reference/caccesstoken-class.md#checktokenmembership) метод.  
