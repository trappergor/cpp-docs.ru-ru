...
- **Источник данных**
  ...
  - Вкладка **подключения**
      ...
      Содержимое этой вкладки зависит от выбранного поставщика. Хотя существует множество типов поставщиков, этот раздел охватывает соединения для двух наиболее распространенных: данных S'L и ODBC. Другие типы аналогичны вариантам полей, описанных здесь.

      Для данных SQL:

      1. **Выберите или введите имя сервера:** Нажмите меню списка выпадающих, чтобы отобразить все зарегистрированные серверы данных в сети и выбрать один из них.

      1. **Введите информацию для входа на сервер:** Введите имя пользователя и пароль для входа на сервер данных.
         ...
         >
         > - **Использование интегрированной безопасности Windows NT**
      ...
      1. **Выберите базу данных на сервере:** Нажмите меню списка выпадающих, чтобы отобразить все зарегистрированные базы данных на сервере данных и выбрать ее.

         \- или -

         **Прикрепите файл базы данных в качестве имени базы данных:** Указать файл, который будет использоваться в качестве базы данных; введите явное имя пути.

      Для данных ODBC:

      1. **Укажите источник данных:** Можно использовать имя источника данных или строку соединения.

         **Используйте имя источника данных:** Этот список отображения отображает источники данных, зарегистрированные в вашей машине. Вы можете заранее настроить источники данных, используя администратор источника данных ODBC

         \- или -

         **Используйте строку соединения:** Либо введите строку подключения, которая уже получена, либо нажмите кнопку **Сборки;** отображается диалоговый ящик **«Выберите источник данных».** Выберите файл или источник данных компьютера и щелкните **OК**.
  ...
  - **Расширенная** вкладка
      ...
      **Настройки сети:** Укажите **уровень олицетворения** (уровень олицетворения, который сервер может использовать при выдавая себя за клиента; непосредственно соответствует уровням олицетворения RPC) и **уровень защиты** (уровень защиты данных, отправляемых между клиентом и сервером; непосредственно соответствует уровням защиты RPC).

      **Прочие:** В **тайм-ауте Connect**укажите количество секунд простоя, допустимое до тайм-аута. В поле **Права доступа** укажите права доступа к подключению к базе данных.
