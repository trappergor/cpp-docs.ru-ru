---
title: Класс CAtlExeModuleT | Документация Майкрософт
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-atl
...
### <a name="parameters"></a>Параметры  
 *nShowCmd*  
 Определяет, как будет отображаться окно. Этот параметр может принимать одно из значений, рассматриваемые в [WinMain](http://msdn.microsoft.com/library/windows/desktop/ms633559) раздел. По умолчанию SW_HIDE.  
...
### <a name="remarks"></a>Примечания  
 По умолчанию этот метод просто вызывает [CoUninitialize](http://msdn.microsoft.com/library/windows/desktop/ms688715) и вызывается из деструктора. Переопределите этот метод, если переопределить [CAtlExeModuleT::InitializeCom](#initializecom).  
