...
|[COMMAND_CODE_HANDLER](#command_code_handler)|Сопоставляет сообщение WM_COMMAND с функцией обработчика на основе кода уведомления.|
|[COMMAND_HANDLER](#command_handler)|Сопоставляет сообщение WM_COMMAND с функцией обработчика на основе кода уведомления и идентификатора элемента меню, элемента управления или ускорителя.|
|[COMMAND_ID_HANDLER](#command_id_handler)|Сопоставляет сообщение WM_COMMAND с функцией обработчика на основе идентификатора элемента меню, элемента управления или ускорителя.|
|[COMMAND_RANGE_CODE_HANDLER](#command_range_code_handler)|Сопоставляет сообщение WM_COMMAND с функцией обработчика на основе кода уведомления и непрерывного диапазона идентификаторов элементов управления.|
|[COMMAND_RANGE_HANDLER](#command_range_handler)|Сопоставляет сообщение WM_COMMAND с функцией обработчика на основе непрерывного диапазона идентификаторов элементов управления.|
...
|[NOTIFY_CODE_HANDLER](#notify_code_handler)|Сопоставляет сообщение WM_NOTIFY с функцией обработчика на основе кода уведомления.|
|[NOTIFY_HANDLER](#notify_handler)|Сопоставляет сообщение WM_NOTIFY с функцией обработчика на основе кода уведомления и идентификатора элемента управления.|
|[NOTIFY_ID_HANDLER](#notify_id_handler)|Сопоставляет сообщение WM_NOTIFY с функцией обработчика на основе идентификатора элемента управления.|
|[NOTIFY_RANGE_CODE_HANDLER](#notify_range_code_handler)|Сопоставляет сообщение WM_NOTIFY с функцией обработчика на основе кода уведомления и непрерывного диапазона идентификаторов элементов управления.|
|[NOTIFY_RANGE_HANDLER](#notify_range_handler)|Сопоставляет сообщение WM_NOTIFY с функцией обработчика на основе непрерывного диапазона идентификаторов элементов управления.|
...
Всегда начинать схему сообщений с помощью BEGIN_MSG_MAP. Затем можно объявить последующие альтернативные схемы сообщений.
...
|[COMMAND_HANDLER](#command_handler)|Сопоставляет сообщение WM_COMMAND с функцией обработчика на основе кода уведомления и идентификатора элемента меню, элемента управления или ускорителя.|
|[COMMAND_ID_HANDLER](#command_id_handler)|Сопоставляет сообщение WM_COMMAND с функцией обработчика на основе идентификатора элемента меню, элемента управления или ускорителя.|
|[COMMAND_CODE_HANDLER](#command_handler)|Сопоставляет сообщение WM_COMMAND с функцией обработчика на основе кода уведомления.|
|[COMMAND_RANGE_HANDLER](#command_range_handler)|Сопоставляет непрерывный диапазон сообщений WM_COMMAND с функцией обработчика на основе идентификатора элемента меню, элемента управления или ускорителя.|
|[NOTIFY_HANDLER](#notify_handler)|Сопоставляет сообщение WM_NOTIFY с функцией обработчика на основе кода уведомления и идентификатора элемента управления.|
|[NOTIFY_ID_HANDLER](#notify_id_handler)|Сопоставляет сообщение WM_NOTIFY с функцией обработчика на основе идентификатора элемента управления.|
|[NOTIFY_CODE_HANDLER](#notify_code_handler)|Сопоставляет сообщение WM_NOTIFY с функцией обработчика на основе кода уведомления.|
|[NOTIFY_RANGE_HANDLER](#notify_range_handler)|Сопоставляет непрерывный диапазон сообщений WM_NOTIFY с функцией обработчика на основе идентификатора элемента управления.|
...
Когда объект получает сообщение WM_PAINT, сообщение направляется в `CMyExtWindow::OnPaint` для фактической обработки. `CMyExtWindow` Если `OnPaint` указывает, что сообщение требует дальнейшей обработки, сообщение будет направлено на схему сообщений по умолчанию в `CMyBaseWindow`.

Кроме схемы сообщений по умолчанию, можно определить альтернативную схему сообщений с помощью [ALT_MSG_MAP](#alt_msg_map). Всегда начинать схему сообщений с помощью BEGIN_MSG_MAP. Затем можно объявить последующие альтернативные схемы сообщений. В следующем примере показана схема сообщений по умолчанию и одна альтернативная схема сообщений, каждая из которых содержит одну функцию обработчика:
...
> [!NOTE]
>  Всегда начинать схему сообщений с помощью BEGIN_MSG_MAP. Затем можно объявить последующие альтернативные схемы сообщений с помощью ALT_MSG_MAP. Макрос [END_MSG_MAP](#end_msg_map) отмечает конец схемы сообщения. Каждая схема сообщений должна иметь ровно один экземпляр BEGIN_MSG_MAP и END_MSG_MAP.
...
CHAIN_MSG_MAP направляет сообщения в схему сообщений по умолчанию базового класса (объявленную с помощью [BEGIN_MSG_MAP](#begin_msg_map)). Чтобы направить сообщения на альтернативную карту сообщений базового класса (объявленную с помощью [ALT_MSG_MAP](#alt_msg_map)), используйте [CHAIN_MSG_MAP_ALT](#chain_msg_map_alt).
...
- Если процедура окна использует `CMyClass`вторую альтернативную схему сообщения и `OnChar` не обрабатывает сообщение, сообщение направляется на указанную альтернативную схему сообщения в `CMyBaseClass`. `CMyBaseClass`должна быть объявлена эта схема сообщений с помощью ALT_MSG_MAP (1).
...
CHAIN_MSG_MAP_DYNAMIC направляет сообщения во время выполнения в схему сообщений по умолчанию в другом объекте. Объект и его схема сообщений связаны с *диначаинид*, определяемым с помощью [Кдинамикчаин:: сетчаинентри](cdynamicchain-class.md#setchainentry). Для использования CHAIN_MSG_MAP_DYNAMIC необходимо создать производный `CDynamicChain` класс от. Пример см. в обзоре [кдинамикчаин](../../atl/reference/cdynamicchain-class.md) .

> [!NOTE]
>  Всегда начинать схему сообщений с помощью [BEGIN_MSG_MAP](#begin_msg_map). Затем можно объявить последующие альтернативные схемы сообщений с помощью ALT_MSG_MAP. Макрос [END_MSG_MAP](#end_msg_map) отмечает конец схемы сообщения. Каждая схема сообщений должна иметь ровно один экземпляр BEGIN_MSG_MAP и END_MSG_MAP.
...
- Если процедура окна использует `CMyClass`вторую альтернативную схему сообщения и `OnChar` не обрабатывает сообщение, сообщение направляется на `m_obj`указанную альтернативную схему сообщения. Класс `CMyContainedClass` должен объявлять эту схему сообщений с помощью ALT_MSG_MAP (1).
...
Аналогичен [COMMAND_HANDLER](#command_handler), но сопоставляет сообщение [WM_COMMAND](/windows/win32/menurc/wm-command) на основе только кода уведомления.
...
COMMAND_HANDLER сопоставляет сообщение [WM_COMMAND](/windows/win32/menurc/wm-command) с указанной функцией обработчика на основе кода уведомления и идентификатора элемента управления. Например:
...
`bHandled` Перед`CommandHandler` вызовом схема сообщения устанавливает значение true. Если `CommandHandler` не обрабатывает сообщение полностью, оно должно иметь `bHandled` значение false, чтобы указать, что сообщение нуждается в дальнейшей обработке.

> [!NOTE]
>  Всегда начинать схему сообщений с помощью [BEGIN_MSG_MAP](#begin_msg_map). Затем можно объявить последующие альтернативные схемы сообщений с помощью [ALT_MSG_MAP](#alt_msg_map). Макрос [END_MSG_MAP](#end_msg_map) отмечает конец схемы сообщения. Каждая схема сообщений должна иметь ровно один экземпляр BEGIN_MSG_MAP и END_MSG_MAP.

В дополнение к COMMAND_HANDLER можно использовать [MESSAGE_HANDLER](#message_handler) для отображения сообщения WM_COMMAND без учета идентификатора или кода. В этом случае `MESSAGE_HANDLER(WM_COMMAND, OnHandlerFunction)` будет направлять все сообщения WM_COMMAND в `OnHandlerFunction`.
...
Аналогичен [COMMAND_HANDLER](#command_handler), но сопоставляет сообщение [WM_COMMAND](/windows/win32/menurc/wm-command) на основе только идентификатора элемента меню, элемента управления или ускорителя.
...
Аналогичен [COMMAND_RANGE_HANDLER](#command_range_handler), но сопоставляет сообщения [WM_COMMAND](/windows/win32/menurc/wm-command) с конкретным кодом уведомления из диапазона элементов управления с одной функцией обработчика.
...
DECLARE_EMPTY_MSG_MAP — это удобный макрос, который вызывает макросы [BEGIN_MSG_MAP](#begin_msg_map) и [END_MSG_MAP](#end_msg_map) для создания пустой схемы сообщения:
...
`bHandled` Перед`MessageHandler` вызовом схема сообщения устанавливает значение true. Если `MessageHandler` не обрабатывает сообщение полностью, оно должно иметь `bHandled` значение false, чтобы указать, что сообщение нуждается в дальнейшей обработке.
...
Аналогичен [NOTIFY_HANDLER](#notify_handler), но сопоставляет сообщение [WM_NOTIFY](/windows/win32/controls/wm-notify) на основе только кода уведомления.
...
NOTIFY_HANDLER сопоставляет сообщение [WM_NOTIFY](/windows/win32/controls/wm-notify) с указанной функцией обработчика на основе кода уведомления и идентификатора элемента управления.

Любая функция, указанная в макросе NOTIFY_HANDLER, должна быть определена следующим образом:

`LRESULT NotifyHandler(int idCtrl, LPNMHDR pnmh, BOOL& bHandled);`

`bHandled` Перед`NotifyHandler` вызовом схема сообщения устанавливает значение true. Если `NotifyHandler` не обрабатывает сообщение полностью, оно должно иметь `bHandled` значение false, чтобы указать, что сообщение нуждается в дальнейшей обработке.

> [!NOTE]
>  Всегда начинать схему сообщений с помощью [BEGIN_MSG_MAP](#begin_msg_map). Затем можно объявить последующие альтернативные схемы сообщений с помощью [ALT_MSG_MAP](#alt_msg_map). Макрос [END_MSG_MAP](#end_msg_map) отмечает конец схемы сообщения. Каждая схема сообщений должна иметь ровно один экземпляр BEGIN_MSG_MAP и END_MSG_MAP.

В дополнение к NOTIFY_HANDLER можно использовать [MESSAGE_HANDLER](#message_handler) для отображения сообщения WM_NOTIFY без учета идентификатора или кода. В этом случае `MESSAGE_HANDLER(WM_NOTIFY, OnHandlerFunction)` будет направлять все сообщения WM_NOTIFY в `OnHandlerFunction`.
...
Аналогичен [NOTIFY_HANDLER](#notify_handler), но сопоставляет сообщение [WM_NOTIFY](/windows/win32/controls/wm-notify) на основе только идентификатора элемента управления.
...
Аналогичен [NOTIFY_RANGE_HANDLER](#notify_range_handler), но сопоставляет сообщения [WM_NOTIFY](/windows/win32/controls/wm-notify) с конкретным кодом уведомления из диапазона элементов управления с одной функцией обработчика.
...
Аналогичен [COMMAND_CODE_HANDLER](#command_code_handler), но отображает команды, отраженные в родительском окне.
...
Аналогичен [COMMAND_HANDLER](#command_handler), но отображает команды, отраженные в родительском окне.
...
Аналогичен [COMMAND_ID_HANDLER](#command_id_handler), но отображает команды, отраженные в родительском окне.
...
Аналогичен [COMMAND_RANGE_CODE_HANDLER](#command_range_code_handler), но отображает команды, отраженные в родительском окне.
...
Аналогичен [COMMAND_RANGE_HANDLER](#command_range_handler), но отображает команды, отраженные в родительском окне.
