...
Когда объект `CMyExtWindow` получает сообщение WM_PAINT, оно направляется `CMyExtWindow::OnPaint` для фактической обработки. Если `OnPaint` указывает, что сообщение требует дальнейшей обработки, сообщение будет направлено на схему сообщений по умолчанию в `CMyBaseWindow`.
...
- Если процедура окна использует схему сообщений по умолчанию `CMyClass`, а `OnPaint` не обрабатывает сообщение, то сообщение направляется в `CMyBaseClass`схему сообщений по умолчанию для обработки.

- Если процедура окна использует первую альтернативную карту сообщений в `CMyClass`, все сообщения направляются в схему сообщений `CMyBaseClass`по умолчанию.

- Если в процедуре окна используется вторая альтернативная схема сообщения `CMyClass`и `OnChar` не обрабатывает сообщение, сообщение направляется на указанную альтернативную схему сообщения в `CMyBaseClass`. `CMyBaseClass` должны объявлять эту схему сообщений ALT_MSG_MAP (1).
...
- Если процедура окна использует схему сообщений по умолчанию `CMyClass`, а `OnPaint` не обрабатывает сообщение, то сообщение направляется в `m_obj`схему сообщений по умолчанию для обработки.

- Если процедура окна использует первую альтернативную карту сообщений в `CMyClass`, все сообщения направляются в схему сообщений `m_obj`по умолчанию.

- Если в процедуре окна используется вторая альтернативная схема сообщения `CMyClass`и `OnChar` не обрабатывает сообщение, сообщение направляется на указанную альтернативную схему сообщения `m_obj`. Класс `CMyContainedClass` должен объявить эту схему сообщений ALT_MSG_MAP (1).
...
Схема сообщений устанавливает `bHandled` в значение TRUE перед вызовом `CommandHandler`. Если `CommandHandler` не обрабатывает сообщение полностью, оно должно установить `bHandled` в значение FALSE, чтобы указать, что сообщение нуждается в дальнейшей обработке.
...
В дополнение к COMMAND_HANDLER можно использовать [MESSAGE_HANDLER](#message_handler) для отображения WM_COMMAND сообщения без учета идентификатора или кода. В этом случае `MESSAGE_HANDLER(WM_COMMAND, OnHandlerFunction)` будет направлять все WM_COMMAND сообщения `OnHandlerFunction`.
...
Предоставляет обработчик по умолчанию для дочернего окна (элемента управления), который получит отраженные сообщения; обработчик будет правильно передавать необработанные сообщения `DefWindowProc`.
...
Схема сообщений устанавливает `bHandled` в значение TRUE перед вызовом `MessageHandler`. Если `MessageHandler` не обрабатывает сообщение полностью, оно должно установить `bHandled` в значение FALSE, чтобы указать, что сообщение нуждается в дальнейшей обработке.
...
Схема сообщений устанавливает `bHandled` в значение TRUE перед вызовом `NotifyHandler`. Если `NotifyHandler` не обрабатывает сообщение полностью, оно должно установить `bHandled` в значение FALSE, чтобы указать, что сообщение нуждается в дальнейшей обработке.

> [!NOTE]
>  Всегда начинать схему сообщений с [BEGIN_MSG_MAP](#begin_msg_map). Затем можно объявить последующие альтернативные схемы сообщений с помощью [ALT_MSG_MAP](#alt_msg_map). Макрос [END_MSG_MAP](#end_msg_map) отмечает конец схемы сообщения. Каждая схема сообщений должна содержать только один экземпляр BEGIN_MSG_MAP и END_MSG_MAP.

В дополнение к NOTIFY_HANDLER можно использовать [MESSAGE_HANDLER](#message_handler) для отображения WM_NOTIFY сообщения без учета идентификатора или кода. В этом случае `MESSAGE_HANDLER(WM_NOTIFY, OnHandlerFunction)` будет направлять все WM_NOTIFY сообщения `OnHandlerFunction`.
