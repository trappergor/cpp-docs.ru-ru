...
- **Поддержка регистрации компонента** Если ATL-проект включает в себя поддержку для компонентов COM + 1.0, можно задать этот параметр. Регистрации компонента позволяет объекту COM + 1.0 для получения списка компонентов, регистрация компонентов или отменяет регистрацию компонентов (по отдельности или все сразу).
