...
## <a name="c17"></a>C++17
**Сжатый static_assert** (доступен с /std:c++17). В C++17 параметр сообщения для static_assert является необязательным. Дополнительные сведения см. в разделе [Расширение static_assert, v2](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2014/n3928.pdf). 

**атрибут [[fallthrough]]** (доступен с /std:c++17) Атрибут [[fallthrough]] можно использовать в контексте операторов switch как подсказку для компилятора о том, что требуется поведение проваливания. Это позволит избежать вывода предупреждений компилятора. Дополнительные сведения см. в разделе [Формулировка для атрибута [[fallthrough]]](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0188r0.pdf). 

**Обобщенные циклы for на основе диапазона** (параметр компилятора не требуется). Для циклов for на основе диапазона больше не требуется, чтобы функции begin() и end() возвращали объекты одного типа. Это позволяет end() возвращать граничную метку согласно использованию в диапазонах [range-v3](https://github.com/ericniebler/range-v3) и технической спецификации по диапазонам (завершенной, но еще не опубликованной). Дополнительные сведения см. в разделе [Обобщение цикла For на основе диапазона](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0184r0.html). 
...
**Новый параметр компилятора для extern constexpr**. В более ранних версиях Visual Studio компилятор всегда обеспечивал внутреннюю компоновку переменной `constexpr` даже в том случае, когда переменная была помечена `extern`. В Visual Studio 2017 версии 15.5 новый параметр компилятора [/Zc:externConstexpr](build/reference/zc-externconstexpr.md) обеспечивает правильное поведение, соответствующее стандартам. Дополнительные сведения см. в разделе о [компоновке extern constexpr](#extern_linkage).

**Удаление динамических спецификаций исключений**: [P0003R5](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0003r5.html). Спецификации динамических исключений стали нерекомендуемыми в C++11. Функция удалена из C++17, но (по-прежнему) нерекомендуемая спецификация `throw()` сохраняется исключительно в качестве псевдонима для `noexcept(true)`. Дополнительные сведения см. в разделе [Удаление спецификации динамических исключений и noexcept](#noexcept_removal). 

**not_fn()** : [P0005R4](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0005r4.html) является заменой not1 и not2.

**Переформулировка enable_shared_from_this**: [P0033R1](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0033r1.html) `enable_shared_from_this` была добавлена в C++11. Стандарт C++17 обновляет спецификацию для улучшения обработки некоторых сложных случаев. [14]

**Соединение карт и наборов**: [P0083R3](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0083r3.pdf). Эта функция позволяет извлекать узлы из ассоциативных контейнеров (например, карты, набора, unordered_map, unordered_set), которые затем могут быть изменены и вставлены обратно в тот же или другой контейнер, использующий этот же тип узла. (Распространенным вариантом использования является извлечение узла из std::map, изменение ключа и повторная вставка.)

**Неподдерживаемые части библиотеки**: [P0174R2](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0174r2.html). В течение ряда лет несколько функций стандартной библиотеки C++ были заменены более новыми или признаны нецелесообразными или проблемными. Эти функции являются официально нерекомендуемыми в C++17. 

**Удаление поддержки распределителя в std::function**: [P0302R1](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0302r1.html). До C++17 у шаблона класса `std::function` было несколько конструкторов, принимающих аргумент распределителя. Однако использование распределителей в данном контексте было проблематичным, а семантика — неясной. Поэтому эти конструкторы были удалены.

**Исправления для not_fn()**: [P0358R1](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0358r1.html). Новая формулировка для `std::not_fn` поддерживает распространение категории значений в случае вызова программы-оболочки.

**shared_ptr<T[]>, shared_ptr<T[N]>**: [P0414R2](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0414r2.html). Объединение изменений shared_ptr из основ работы с библиотеками с C++17. [14]

**Исправление shared_ptr для массивов**: [P0497R0](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0497r0.html). Исправления shared_ptr для поддержки массивов. [14]

**Уточнение insert_return_type**: [P0508R0](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0508r0.html). Ассоциативные контейнеры с уникальными ключами и неупорядоченные контейнеры с уникальными ключами имеют функцию-член `insert`, возвращающую `insert_return_type` вложенного типа. Тип возвращаемого значения теперь определяется как специализация типа, который параметризуется по итератору и типу узла контейнера. 

**Встроенные переменные для STL**: [P0607R0](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0607r0.html). 

**Функции в приложении D признаны нерекомендуемыми**. В приложении D стандарта C++ содержатся все функции, которые являются устаревшими, включая `shared_ptr::unique()`, `<codecvt>` и `namespace std::tr1`. Если задан параметр компилятора /std:c++17, почти все функции стандартной библиотеки в приложении D помечаются как нерекомендуемые. Дополнительные сведения см. в разделе [Функции стандартной библиотеки в приложении D помечены как нерекомендуемые](#annex_d).

Пространство имен `std::tr2::sys` в `<experimental/filesystem>` теперь по умолчанию выдает предупреждение о нерекомендуемых функциях при установленном параметре /std:c++14 и по умолчанию удалено в /std:c++17.
...
Теперь компилятор выдает предупреждение, если во время компиляции у него есть информация о том, что собственный объект с удаленным конструктором копий передается по значению через границу собственных и управляемых функций. В тех случаях, когда компилятор во время компиляции не имеет такой информации, он внедряет проверку времени выполнения, чтобы программа немедленно вызвала std::terminate при проявлении некорректного маршалинга. В Visual Studio 2017 версии 15.3 для следующего кода будет создана ошибка C4606 "'A': для передачи аргумента по значению через границы собственного и управляемого требуется действительный конструктор копии. В противном случае поведение в среде выполнения будет неопределенным".
```cpp
class A 
{ 
public: 
...
```
Это предупреждение по умолчанию отключено (включено по умолчанию в версии 15.5) и влияет только на код, скомпилированный с `/Wall /WX`.
...
extern "C" __declspec(noinline) HRESULT __stdcall
```
Это новое предупреждение C4768 будет выводиться в некоторых заголовках Windows SDK, которые поставлялись с Visual Studio 2017 15.3 или более ранними версиями (например, с версией 10.0.15063.0, также известной как пакет SDK для RS2). Однако в более поздних версиях заголовков Windows SDK (в частности, ShlObj.h и ShlObj_core.h) ошибка была исправлена, поэтому это предупреждение не создается. При появлении этого предупреждения от заголовков Windows SDK можно выполнить следующие действия:
1)  Переключитесь на последнюю версию Windows SDK, поставляемую вместе с выпуском Visual Studio 2017 15.5.
...
В более ранних версиях Visual Studio компилятор всегда обеспечивал переменной `constexpr` внутреннюю компоновку даже в том случае, когда переменная была помечена как `extern`. В Visual Studio 2017 версии 15.5 новый параметр компилятора /Zc:externConstexpr обеспечивает правильное поведение, соответствующее стандартам. В конечном счете это будет поведением по умолчанию.
...
### <a name="typeid-cant-be-used-on-incomplete-class-type"></a>typeid нельзя использовать в типе неполного класса.
...
Следующий код приводит к возникновению ошибки C5043 и C5040 в Visual Studio 2017 версии 15.5 в случае применения параметра /std:c++17:
...
Статические члены данных constexpr теперь являются неявно встроенными. Это означает, что их объявления в пределах класса теперь являются определениями. Использование внешних определений для статических членов данных constexpr избыточно и не рекомендуется. Если в Visual Studio 2017 версии 15.5 применен параметр /std:c++17, следующий код выдает предупреждение C5041 *'size': внешнее определение для статического члена данных constexpr не требуется и не рекомендуется использовать в C++17*:
...
Следующий код успешно компилируется при указании параметра /std:c ++14, но при указании параметра /std:c++17 завершается ошибкой C2027:*использование неопределенного типа 'A<T>'*:
