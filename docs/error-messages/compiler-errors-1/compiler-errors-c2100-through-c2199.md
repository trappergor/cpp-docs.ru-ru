---
title: Ошибка Компилятораs C2100 through C2199 | Документация Майкрософт
ms.custom: ''
ms.date: 11/17/2017
ms.technology:
- cpp-diagnostics
...
|C2119 ошибки компилятора|"*идентификатор*": тип для "*тип*" не может быть выведен из пустого инициализатора|
...
|C2123 ошибки компилятора|"*идентификатор*": шаблоны псевдонима не может быть специализированы явно или частично|
...
|C2176 ошибки компилятора|Инструкция return не может находиться в обработчике блока функция try-связанном с конструктором|
...
|C2187 ошибки компилятора|Синтаксическая ошибка: "*маркера*" непредвиденный здесь элемент|
|[Ошибка компилятора C2188](compiler-error-c2188.md)|"*номер*": слишком большое значение для расширенных символов|
|C2189 ошибки компилятора|атрибут «alignas» не может применяться к битовому полю, параметру функции, объявлению исключения или переменная, объявленная с «зарегистрироваться» класс хранения|
