...
Перечислитель «identifier» в switch с перечислением «перечисление» не обрабатывается явно меткой выбора  
  
 Перечисление не имеет связанного обработчика `switch` инструкции.  
