...
|Предупреждение компилятора (уровень 1) C4602|#Директива #pragma pop_macro: «имя макроса» ранее не #pragma push_macro для этого идентификатора|  
...
|[Предупреждение компилятора (уровень 1) C4606](../../error-messages/compiler-warnings/compiler-warning-level-1-c4606.md)|#в директиве pragma warning: «номер предупреждения» игнорируется; Предупреждения анализа кода не связаны с уровнями предупреждений|  
...
|Предупреждение компилятора (уровень 3) C4609|«%$S» является производным от интерфейса по умолчанию «%$S» в типе «%$S». Используйте интерфейс по умолчанию для «%$S», или к разрыву связи базовый/производный.|  
...
|Предупреждение компилятора (уровень 1) C4613|%$S: класс сегмента не может быть изменен|  
|[Предупреждение компилятора (уровень 1) C4615](../../error-messages/compiler-warnings/compiler-warning-level-1-c4615.md)|#в директиве pragma warning: неизвестный тип пользовательского предупреждения|  
|[Предупреждение компилятора (уровень 1) C4616](../../error-messages/compiler-warnings/compiler-warning-level-1-c4616.md)|#в директиве pragma warning: номер предупреждения «число» не является действительным предупреждением компилятора|  
|[Предупреждение компилятора (уровень 1) C4618](../../error-messages/compiler-warnings/compiler-warning-level-1-c4618.md)|параметры директивы pragma включают пустую строку; пропуск pragma|  
|[Предупреждение компилятора (уровень 3) C4619](../../error-messages/compiler-warnings/compiler-warning-level-3-c4619.md)|#в директиве pragma warning: нет предупреждения с номером «number»|  
...
|[Предупреждение компилятора (уровень 3) C4641](../../error-messages/compiler-warnings/compiler-warning-level-3-c4641.md)|В комментарии XML-документа содержится неоднозначная перекрестная ссылка:|  
...
|[Предупреждение компилятора (уровень 1) C4650](../../error-messages/compiler-warnings/compiler-warning-level-1-c4650.md)|данные отладки не включены в предкомпилированный заголовок; будут доступны только глобальные символы из заголовка|  
...
|[Предупреждение компилятора (уровень 1) C4656](../../error-messages/compiler-warnings/compiler-warning-level-1-c4656.md)|«символ»: тип данных новых были изменены с момента последней сборки или имеются разные объявления|  
|Предупреждение компилятора (уровень 1) C4657|выражение включает тип данных, который обновился с момента последнего сборки|  
...
|[Предупреждение компилятора (уровень 1) C4659](../../error-messages/compiler-warnings/compiler-warning-level-1-c4659.md)|#Директива #pragma «pragma»: использование зарезервированного сегмента «сегмент» поведение не определено, используйте #pragma comment (linker,...)|  
...
|[Предупреждение компилятора C4687](../../error-messages/compiler-warnings/compiler-warning-c4687.md)|«класс»: Запечатанный абстрактный класс не может реализовывать интерфейс «интерфейс»|  
...
|Предупреждение компилятора (уровень 1) C4693|«класс»: Запечатанный абстрактный класс не может иметь любой экземпляр члены «член экземпляра»|  
|Предупреждение компилятора (уровень 1) C4694|«класс»: Запечатанный абстрактный класс не может иметь базового класса «базовый_класс»|  
|Предупреждение компилятора (уровень 1) C4695|#Директива #pragma кодировка_выполнения: «набор символов» не является поддерживаемой аргументом: поддерживается в настоящее время единственным «UTF-8"|  
...
|[Предупреждение компилятора (уровень 4) C4703](../../error-messages/compiler-warnings/compiler-warning-level-4-c4703.md)|используется потенциально неинициализированная локальная переменная-указатель "%s"|  
...
|[Предупреждение компилятора (уровень 4) C4709](../../error-messages/compiler-warnings/compiler-warning-level-4-c4709.md)|запятая-оператор в выражении индекса массива|  
...
|[Предупреждение компилятора (уровень 1) C4716](../../error-messages/compiler-warnings/compiler-warning-level-1-c4716.md)|«функция»: должен возвращать значение|  
...
|Предупреждение компилятора (уровень 1) C4719|Обнаружена константа с двойной точностью (double) при указанном параметре Qfast - используйте суффикс "f" для задания одинарной точности|  
...
|Предупреждение компилятора (уровень 1) C4728|параметр /Yl- игнорируется, поскольку требуется ссылка PCH|  
...
|[Предупреждение компилятора (уровень 1) C4733](../../error-messages/compiler-warnings/compiler-warning-level-1-c4733.md)|Присваивание для "FS:0" во встроенном ассемблерном коде: обработчик не зарегистрирован как безопасный|  
...
|[Предупреждение компилятора (уровень 4) C4740](../../error-messages/compiler-warnings/compiler-warning-level-4-c4740.md)|входящие или исходящие потоки для встроенного ассемблерного кода препятствуют глобальной оптимизации|  
...
|[Предупреждение компилятора C4746](../../error-messages/compiler-warnings/compiler-warning-c4746.md)|переменное обращение «%s» определяется параметром/volatile:\<iso &#124; ms >; попробуйте использовать встроенные функции __iso_volatile_load/store|  
...
|Предупреждение компилятора (уровень 4) C4752|найдены Intel(R) Advanced Vector Extensions; рассмотрите использование /arch:AVX|  
...
|Предупреждение компилятора (уровень 4) C4764|Для перехваченных объектов выравнивание не может превышать 16 байтов|  
|Предупреждение компилятора (уровень 4) C4767|длина имени раздела "%s" превышает 8 знаков, имя будет усечено компоновщиком|  
|Предупреждение C4770|частично проверенное перечисление "%s" используется в качестве индекса|  
|[Предупреждение компилятора (уровень 1) C4772](../../error-messages/compiler-warnings/compiler-warning-level-1-c4772.md)|#Импорт ссылается на тип из отсутствующей библиотеки типов; отсутствующий тип «тип» используется как заполнитель|  
...
|Предупреждение компилятора (уровень 2) C4792|на функцию "%s", объявленную с использованием sysimport, ссылается машинный код; для компоновки требуется импортировать библиотеку|  
...
|Предупреждение компилятора (уровень 1) C4794|сегмент переменной из локальной памяти потока "%s" изменен с "%s" на "%s"|  
