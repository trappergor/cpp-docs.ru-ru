...
«функция»: не представлен прототип функции: преобразование «()» в «(void)»  
 ...
 Компилятор не обнаружил явный список аргументов функции. Это предупреждение имеет только компилятор C.  
  
 Это предупреждение отключено по умолчанию. В разделе [компилятора предупреждений, которые Off по умолчанию](../../preprocessor/compiler-warnings-that-are-off-by-default.md) подробнее.  
