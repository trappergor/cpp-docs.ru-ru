...
**Имя POSIX для этого элемента является устаревшим. Вместо этого используйте имя, соответствующее стандарту ISO C и C++:** *New-Name*. **Дополнительные сведения см. в справке в Интернете.**
...
Чтобы отключить предупреждения об устаревании для этих функций, определите макрос препроцессора ** \_ CRT \_ нонстдк \_ без \_ предупреждений**. Этот макрос можно определить в командной строке, включив параметр `/D_CRT_NONSTDC_NO_WARNINGS` .

## <a name="unsafe-crt-library-functions"></a>Небезопасные функции библиотеки CRT

**Эта функция или переменная может быть ненадежной. Вместо этого рекомендуется использовать** *Сейф-Version* **. Чтобы отключить нерекомендуемое использование, используйте \_ CRT \_ Secure \_ без \_ предупреждений.  Дополнительные сведения см. в справке в Интернете.**
...
Чтобы отключить предупреждения об устаревании для этих функций в CRT, определите параметр ** \_ CRT \_ Secure \_ без \_ предупреждений**.

Чтобы отключить предупреждения о нерекомендуемых глобальных переменных, определите параметр ** \_ CRT \_ Secure \_ без \_ предупреждений \_ **.
...
__"std::__*function_name*__:: \_ unchecked \_ итераторы:: не \_ использовать устаревший" вызов std::__*function_name* **с параметрами, которые могут быть ненадежными. Этот вызов полагается на вызывающий объект, чтобы проверить правильность переданных значений. Чтобы отключить это предупреждение, используйте-D \_ SCL \_ \_ без \_ предупреждений. См. документацию по использованию Visual C++ "проверенные итераторы"** .

Это предупреждение появляется в отладочных сборках, так как некоторые функции шаблона стандартной библиотеки C++ не проверяют правильность параметров. Часто это обусловлено тем, что функции не хватает информации для проверки границ контейнера. Или, так как итераторы могут неправильно использоваться функцией. Это предупреждение помогает понять эти функции, так как они могут быть источником серьезных брешей в системе безопасности в программе. Дополнительные сведения см. в разделе [проверенные итераторы](../../standard-library/checked-iterators.md).
...
Если вы проверили, что в коде не может быть ошибки переполнения буфера, это предупреждение можно отключить. Чтобы отключить предупреждения для этих функций, определите параметр ** \_ SCL \_ \_ без \_ предупреждений**.
...
Чтобы отключить предупреждения об устаревании для этих элементов, определяйте ** \_ CRT \_ \_ без \_ предупреждений**. Дополнительные сведения см. в документации по устаревшей функции или переменной.
