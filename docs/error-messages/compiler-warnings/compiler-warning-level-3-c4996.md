...
- ["std::*function_name*::\_Unchecked\_итераторы::\_Deprecate" вызов std::*имя_функции*с параметрами, которые могут быть небезопасными - вызов зависит от вызывающего объекта, чтобы проверить правильность переданных значений. Чтобы отключить это предупреждение, используйте -D_SCL_SECURE_NO_WARNINGS. См. в документации по использованию «Проверяемых итераторов» Visual C++](#unsafe-standard-library-functions)
