...
- [Имя POSIX для этого элемента устарело. Вместо этого используйте ISO C и C++ имя, соответствующее стандарту: *новое_имя*. See online help for details.](#posix-function-names) (Функция или переменная может быть небезопасной. Используйте strncpy_s. Дополнительные сведения см. в справке.)
...
- Откройте **страницы свойств** диалоговое окно для вашего проекта. Сведения о том, как с помощью диалогового окна страницы свойств, см. в разделе [страницы свойств](../../build/reference/property-pages-visual-cpp.md).
...
**Имя POSIX для этого элемента устарело. Вместо этого используйте ISO C и C++ имя, соответствующее стандарту:** *новое_имя*. **См. в разделе сведений о справке в Интернете.**
...
## <a name="example-user-defined-deprecated-function"></a>Пример Определяемая пользователем функция не рекомендуется
