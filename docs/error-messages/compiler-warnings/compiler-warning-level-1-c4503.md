...
При использовании более старой версии Visual Studio это предупреждение может быть выдано, если код содержит шаблоны, специализированные для шаблонов. Например, карта карт (из C++ стандартной библиотеки). В этом случае можно сделать typedef типом (например, **структурой**), содержащим карту.
