...
\#в директиве pragma warning: предупреждение с номером «число» не является действительным предупреждением компилятора  
  
 Число предупреждений, указанное в [предупреждение](../../preprocessor/warning.md) pragma не может быть переназначен. Директива pragma обрабатывается.  
