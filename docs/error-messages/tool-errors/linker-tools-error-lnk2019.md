...
ms.locfileid: "87219811"
---
# <a name="linker-tools-error-lnk2019"></a>Ошибка средств компоновщика LNK2019

> неразрешенный :::no-loc(extern)::: символ Al "*symbol*", на который ссылается функция "*функция*"
...
Соглашения о вызовах ( [:::no-loc(__cdecl):::](../../cpp/cdecl.md) , [:::no-loc(__stdcall):::](../../cpp/stdcall.md) , [:::no-loc(__fastcall):::](../../cpp/fastcall.md) или [:::no-loc(__vectorcall):::](../../cpp/vectorcall.md) ) кодируются как часть декорированного имени. Убедитесь, что соглашение о вызовах одинаково.

### <a name="a-symbol-is-defined-in-a-c-file-but-declared-without-using-no-locextern-c-in-a-c-file"></a>Символ определен в файле C, но объявлен без использования :::no-loc(extern)::: "C" в файле C++

Символы, определенные в файле, который компилируется как C, имеют разные декорированные имена, чем символы, объявленные в файле C++, если не используется модификатор [ :::no-loc(extern)::: "C"](../../cpp/using-:::no-loc(extern):::-to-specify-linkage.md) . Убедитесь, что объявление соответствует компоновке компиляции для каждого символа. Аналогично, если символ определяется в файле C++, который будет использоваться программой C, в определении следует использовать `:::no-loc(extern)::: "C"` .

### <a name="a-symbol-is-defined-as-no-locstatic-and-then-later-referenced-outside-the-file"></a>Символ определяется как :::no-loc(static)::: , а затем на него указывает ссылка за пределами файла

В C++, в отличие от C, [Глобальная :::no-loc(const)::: Ants](../../error-messages/tool-errors/global-:::no-loc(const):::ants-in-cpp.md) имеет **`:::no-loc(static):::`** компоновку. Чтобы обойти это ограничение, можно включить **`:::no-loc(const):::`** инициализацию в файл заголовка и включить этот заголовок в cpp-файлы. Кроме того, можно сделать переменную :::no-loc(const)::: Ant и использовать :::no-loc(const)::: ссылку Ant для доступа к ней.

### <a name="a-no-locstatic-member-of-a-class-isnt-defined"></a>:::no-loc(static):::Член класса не определен

:::no-loc(static):::Член класса должен иметь уникальное определение или будет нарушать правило одного определения. :::no-loc(static):::Член класса, который не может быть определен как встроенный, должен быть определен в одном исходном файле с помощью его полного имени. Если он не определен вообще, компоновщик создает ошибку LNK2019.
...
Код приложения должен определять соответствующую точку входа: `:::no-loc(main):::` или `:::no-loc(wmain):::` для консольных приложений, `:::no-loc(WinMain):::` или `:::no-loc(wWinMain):::` для приложений Windows. Дополнительные сведения см. в разделе [ :::no-loc(main)::: функция и аргументы командной строки](../../cpp/:::no-loc(main):::-function-command-line-args.md) или [ :::no-loc(WinMain)::: функция](/windows/win32/api/winbase/nf-winbase-win:::no-loc(main):::). Чтобы использовать настраиваемую точку входа, укажите параметр компоновщика [/Entry (символ точки входа)](../../build/reference/entry-entry-point-symbol.md) .
...
Автоматические переменные (области видимости функции) могут использоваться только в области видимости данной функции. Эти переменные нельзя объявлять **`:::no-loc(extern):::`** и использовать в других исходных файлах. Пример см. в разделе [Automatic (Function Scope) Variables](../../error-messages/tool-errors/automatic-function-scope-variables.md).
...
Например, если вы используете :::no-loc(AVX2)::: встроенную функцию, но не указываете параметр компилятора [ / :::no-loc(ARCH)::: : :::no-loc(AVX2)::: ](../../build/reference/arch-x86.md) , компилятор предполагает, что встроенная функция является :::no-loc(extern)::: функцией Al. Вместо создания встроенной инструкции компилятор создает вызов :::no-loc(extern)::: символа Al с тем же именем, что и встроенная инструкция. Когда компоновщик пытается найти определение этой отсутствующей функции, он создает ошибку LNK2019. Убедитесь, что используются только встроенные функции и типы, поддерживаемые целевой архитектурой.

### <a name="you-mix-code-that-uses-native-no-locwchar_t-with-code-that-doesnt"></a>Вы занимаетесь смешением кода, который использует собственный :::no-loc(wchar_t)::: код с кодом, который не

Действия по согласованности языка C++, выполненные в Visual Studio 2005, сделали **`:::no-loc(wchar_t):::`** собственный тип по умолчанию. Если не все файлы были скомпилированы с использованием одних и тех же параметров **/Zc: :::no-loc(wchar_t)::: ** , ссылки типа могут не разрешаться в совместимые типы. Убедитесь **`:::no-loc(wchar_t):::`** , что типы во всех файлах библиотек и объектов совместимы. Либо обновите из **`:::no-loc(wchar_t):::`** определения типа, либо используйте соответствующие параметры **/Zc: :::no-loc(wchar_t)::: ** при компиляции.

## <a name="third-party-library-issues-and-vcpkg"></a>Проблемы с библиотекой сторонних производителей и vcpkg

Если вы видите эту ошибку при попытке настроить библиотеку стороннего производителя в рамках сборки, рассмотрите возможность использования [vcpkg](../../vcpkg.md), диспетчера пакетов C++ для установки и сборки библиотеки. vcpkg поддерживает большой и растущей [список библиотек сторонних производителей](https://github.com/Microsoft/vcpkg/tree/master/ports). Он задает все свойства конфигурации и зависимости, необходимые для успешной сборки в рамках проекта.

## <a name="diagnosis-tools"></a>Средства диагностики

Иногда трудно определить, почему компоновщику не удается найти определенное определение символа. Часто проблема заключается в том, что в сборку не включен код, содержащий определение. Или же параметры сборки создали разные декорированные имена для :::no-loc(extern)::: символов Al. Существует несколько средств и параметров, которые могут помочь в диагностике ошибок LNK2019.

- С помощью [/:::no-loc(VERBOSE):::](../../build/reference/verbose-print-progress-messages.md) параметра компоновщика можно определить, на какие файлы ссылается компоновщик. С помощью этого параметра можно проверить, включен ли в сборку файл, содержащий определение символа.

- [/:::no-loc(EXPORTS):::](../../build/reference/dash-exports.md)Параметры и [/:::no-loc(SYMBOLS):::](../../build/reference/symbols.md) **:::no-loc(DUMPBIN):::** программы позволяют определить, какие символы определены в библиотеке DLL, а также в файлах объектов или библиотек. Убедитесь, что экспортированные декорированные имена соответствуют декорированным именам, которые ищет компоновщик.

- **:::no-loc(UNDNAME):::** Программа может отобразить эквивалентный недекорированный :::no-loc(extern)::: символ Al для декорированного имени.
...
В этом примере :::no-loc(extern)::: переменная Al объявлена, но не определена:
...
Ниже приведен еще один пример, где переменная и функция объявляются как, **`:::no-loc(extern):::`** но определение не предоставляется:
...
### <a name="a-no-locstatic-data-member-is-declared-but-not-defined"></a>:::no-loc(static):::Элемент данных объявлен, но не определен

Ошибка LNK2019 также может возникать, если :::no-loc(static)::: элемент данных объявлен, но не определен. В следующем примере показано возникновение ошибки LNK2019 и приводятся сведения по ее устранению.
...
### <a name="inconsistent-no-locwchar_t-type-definitions"></a>Непротиворечивые :::no-loc(wchar_t)::: определения типов

В этом примере создается библиотека DLL с экспортом, который `WCHAR` разрешается в **`:::no-loc(wchar_t):::`** .
...
Чтобы устранить эту ошибку, измените **`unsigned short`** значение на **`:::no-loc(wchar_t):::`** или `WCHAR` или скомпилируйте LNK2019g. cpp с помощью **/Zc: :::no-loc(wchar_t)::: - **.

## <a name="additional-resources"></a>Дополнительные ресурсы

Дополнительные сведения о возможных причинах и решениях ошибки LNK2001 см. в Stack Overflow вопросе, [что является неопределенной ссылкой или неразрешенной :::no-loc(extern)::: ошибкой символа Al и как ее исправить?](https://stackoverflow.com/q/12573816/2002113).
