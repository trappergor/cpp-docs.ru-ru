---
title: "Сообщения векторизатора и параллелизатора | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
...
Параметры компилятора Visual C\+\+ [\/Qpar\-report](../../build/reference/qpar-report-auto-parallelizer-reporting-level.md) и [\/Qvec\-report](../../build/reference/qvec-report-auto-vectorizer-reporting-level.md) можно использовать для настройки [Автоматическая параллелизация и автоматическая векторизация](../../parallel/auto-parallelization-and-auto-vectorization.md) на вывод кодов причины и информационных сообщений о его действиях.  В этой статье описываются коды причины и сообщения.  
...
###  <a name="BKMK_ReasonCode50x"></a> 5xx  
 Коды причины 5*xx* применяются как к автоматическому параллелизатору, так и к автоматическому векторизатору.  
...
###  <a name="BKMK_ReasonCode100x"></a> 10xx  
 Коды причины 10*xx* относятся к автоматическому параллелизатору.  
...
|1005|Указана директива pragma **no\_parallel**.|  
|1006|Эта функция содержит **openmp**.  Для устранения проблемы удалите из этой функции все **openmp**.|  
...
###  <a name="BKMK_ReasonCode110x"></a> 11xx  
 Коды причины 11*xx* относятся к автоматическому векторизатору.  
...
###  <a name="BKMK_ReasonCode120x"></a> 12xx  
 Коды причины 12*xx* относятся к автоматическому векторизатору.  
...
###  <a name="BKMK_ReasonCode140x"></a> 14xx  
 Коды причины 14*xx* возникают, если некоторые заданные параметры несовместимы с автоматической векторизацией.  
  
|Код причины|Объяснение|  
|-----------------|----------------|  
|1400|Указана директива **\#pragma loop\(no\_vector\)**.|  
|1401|Для целевой архитектуры x86 или ARM указан ключ **\/kernel**.|  
|1402|Для целевой архитектуры x86 не указан ключ **\/arch:SSE2** или выше.|  
|1403|Указан ключ **\/arch:ATOM**, и цикл содержит операции с типом double.|  
|1404|Указан ключ **\/O1** или **\/Os**.|  
...
###  <a name="BKMK_ReasonCode150x"></a> 15xx  
 Блок кодов причины 15*xx* относится к совмещению имен \(псевдонимам\).  Совмещение имен возникает, когда одно и тоже расположение в памяти доступно под двумя разными именами.  
