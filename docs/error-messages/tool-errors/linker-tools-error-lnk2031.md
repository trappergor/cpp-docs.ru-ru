...
При попытке импортировать неуправляемую функцию в чистом образе, помните, что неявные соглашения о вызовах различаются между внутренней и чистой компиляции. Дополнительные сведения о чисто образах см. в разделе [чистый и проверяемый код (C++выполняет)](../../dotnet/pure-and-verifiable-code-cpp-cli.md).
