...
ms.locfileid: "80197008"
---
# <a name="c-runtime-error-r6035"></a>Ошибка времени выполнения C R6035

Библиотека среды C++ выполнения Microsoft Visual Runtime, ошибка R6035 — Модуль в этом приложении инициализирует глобальный файл cookie безопасности модуля, в то время как функция, которая полагается на этот файл cookie безопасности, активна.  Вызовите __security_init_cookie ранее.
...
Ошибка R6035 указывает, что был выполнен вызов `__security_init_cookie` после того, как была указана Защищенная функция. Если выполнение было продолжено, будет обнаружено ложное переполнение буфера, так как файл cookie в стеке больше не соответствует глобальному файлу cookie.
...
[Функции безопасности в КОМПИЛЯТОРОМ MSVC](https://blogs.msdn.microsoft.com/vcblog/2017/06/28/security-features-in-microsoft-visual-c/)
