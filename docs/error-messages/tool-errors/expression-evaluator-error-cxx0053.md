---
title: Ошибка вычислителя выражений CXX0053 | Документы Microsoft
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-diagnostics
ms.topic: error-reference
f1_keywords:
- CXX0053
dev_langs:
- C++
helpviewer_keywords:
- CAN0053
- CXX0053
ms.assetid: fe74ed9e-9241-4df1-881f-104348e296c6
author: corob-msft
ms.author: corob
ms.workload:
- cplusplus
ms.openlocfilehash: 91e2668f950e6b2553113e74061994d1855ce7bc
ms.sourcegitcommit: 76b7653ae443a2b8eb1186b789f8503609d6453e
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 05/04/2018
---
# <a name="expression-evaluator-error-cxx0053"></a>Ошибка вычислителя выражений CXX0053
обнаружен символ соответствует во время привязки точек останова  
  
 Символ, используемый как точка останова не является функцией. Эта ошибка может возникнуть, указав данные-член в качестве точки останова.  
  
 Эта ошибка идентична ошибке CAN0053.