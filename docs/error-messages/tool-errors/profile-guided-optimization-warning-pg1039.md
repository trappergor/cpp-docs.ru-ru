---
title: Предупреждение профильной оптимизации PG1039
ms.date: 11/04/2016
f1_keywords:
- PG1039
helpviewer_keywords:
- PG1039
ms.assetid: a67dfd66-b610-4d78-9658-2bcf7a506b9f
ms.openlocfilehash: 5b23d3fe48b80ad63aee1ea31c5d767b90dc85ff
ms.sourcegitcommit: c6f8e6c2daec40ff4effd8ca99a7014a3b41ef33
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 04/24/2019
ms.locfileid: "64344381"
---
# <a name="profile-guided-optimization-warning-pg1039"></a>Предупреждение профильной оптимизации PG1039

Не удалось дождаться момента, удобного для уборки.

Приложение не разрешает [pgosweep](../../build/pgosweep.md) для запуска.  По возможности запустите pgosweep, когда приложение находится в состоянии простоя.