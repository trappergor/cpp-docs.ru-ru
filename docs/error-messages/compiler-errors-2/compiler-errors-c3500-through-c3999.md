...
|C3512 ошибки компилятора|последовательность символов-разделителей для необработанного строкового литерала должны содержать не более 16 символов|
...
|C3546 ошибки компилятора|«...»: нет доступных для развертывания пакетов параметра|
...
|[Ошибка компилятора C3552](compiler-error-c3552.md)|Ожидаемый «decltype(auto)»|
...
|C3565 ошибки компилятора|Общий объем памяти tile_static (*номер* байт) превышает ограничение *номер* байт при компиляции графа вызовов concurrency::parallel_for_each|
...
|C3594 ошибки компилятора|исключение при обработке (try, catch, throw и т.д.) не поддерживается в коде, ограниченном amp|
|C3595 ошибки компилятора|Константа находится вне допустимого диапазона, в коде, ограниченном amp|
...
|C3613 ошибки компилятора|отсутствует тип возвращаемого значения, после «->» (требуется «int»)|
...
|[C3615 ошибки компилятора](compiler-error-c3615.md)|функция constexpr "*функция*" не может быть выражение константы|
...
|[Ошибка компилятора C3627](compiler-error-c3627.md)|Только типы значений могут быть упакованы|
...
|[Ошибка компилятора C3661](compiler-error-c3661.md)|в списке явных переопределений не найден любой из переопределяемых методов|
...
|C3677 ошибки компилятора|строковый литерал после «оператор» не может иметь кодирующий префикс|
...
|C3690 ошибки компилятора|ожидается строковый литерал, обнаружено определяемой пользователем строкой литерала вместо|
...
|[Ошибка компилятора C3739](compiler-error-c3739.md)|"*класса*": синтаксис является только поддерживаются, если "макет\_зависимых" параметр для event_receiver имеет значение true|
...
|[Ошибка компилятора C3743](compiler-error-c3743.md)|можно только обработчик или отсоединения весь интерфейс, когда параметр «layout_dependent» для event_receiver имеет значение true|
...
|[Ошибка компилятора C3768](compiler-error-c3768.md)|не удается получить адрес виртуальной функции vararg в чистом управляемом коде|
...
|[Ошибка компилятора C3799](compiler-error-c3799.md)|индексированные свойства не может быть пустой список параметров|
...
|[Ошибка компилятора C3828](compiler-error-c3828.md)|аргументы размещения невозможно указать для "*ключевое слово*«выражение для типа»*типа*"|
...
|[Ошибка компилятора C3851](compiler-error-c3851.md)|"*маркера*": универсальное имя символа не может обозначать символ из базового набора|
...
|[Ошибка компилятора C3857](compiler-error-c3857.md)|"*шаблона*": не допускаются несколько списков или универсальных параметров|
...
|[Ошибка компилятора C3869](compiler-error-c3869.md)|в ограничении gcnew отсутствует пустой параметр список «()»|
...
|C3875 ошибки компилятора|вызов функции нестатический член отсутствует список аргументов|
...
|C3934 ошибки компилятора|функция, которая находится в любой форме «main» не может иметь спецификаторы ограничений, отличных от restrict(cpu)|
...
|C3937 ошибки компилятора|не разрешен пустой спецификатор ограничений|
...
|C3949 ошибки компилятора|Раскрытие пакета, «...», не может находиться в скобки операторе объявления|
...
|C3963 ошибки компилятора|многомерные массивы не поддерживаются|
