...
|Ошибка компилятора C2737|"*идентификатор*": необходимо инициализировать объект «constexpr»|
...
|Ошибка компилятора C2740|значение операнда "*номер*«выходит за пределы диапазона»*lower_bound* - *upper_bound*"|
|Ошибка компилятора C2741|размер фрейма слишком велик|
...
|Ошибка компилятора C2744|"*оператор*" не является допустимым оператором CLR или WinRT|
...
|Ошибка компилятора C2759|в отчеты встроенного ассемблера: *error_message*|
...
|Ошибка компилятора C2763|"*шаблона*": Недопустимое использование строкового литерала в качестве аргумента шаблона для "*параметр*"|
...
|Ошибка компилятора C2769|Вам не удается инициализировать фигурную скобку массиве управляемых/WinRT в списке инициализаторов базового класса или члена|
...
|Ошибка компилятора C2789|"*идентификатор*": необходимо инициализировать объект типа, квалифицированный как const|
...
|Ошибка компилятора C2799|"*идентификатор*": необходимо инициализировать объект типа класса, квалифицированного как const без конструктора по умолчанию, предоставляемый пользователем|
