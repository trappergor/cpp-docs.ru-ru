---
title: Ошибки компилятора с C3000 по C3099 | Документация Майкрософт
ms.custom: ''
ms.date: 11/17/2017
ms.technology:
- cpp-diagnostics
...
|Error|Сообщение|
|-----------|-------------|
|С C3000 ошибки компилятора|Является устаревшей.|
...
|[Ошибка компилятора C3050](compiler-error-c3050.md)|"*класс*": класс ref не может наследовать от "*идентификатор*"|
...
|C3061 ошибки компилятора|оператор "*оператор*": запрещен при перечислении "*тип*«с базовым типом»*тип*"|
...
|C3067 ошибки компилятора|Список инициализаторов не может использоваться с встроенным operator]|
...
|C3078 ошибки компилятора|в новых выражениях необходимо указать размер массива|
...
|C3086 ошибки компилятора|не удается найти «std::initializer_list»: необходимо #include \<initializer_list >|
...
|C3089 ошибки компилятора|"*идентификатор*": имя параметра не соответствует имени любого члена данных|
...
|C3092 ошибки компилятора|"*класс*": член класса не может быть немного поле, «static» или «const»|
...
|C3098 ошибки компилятора|"*идентификатор*": атрибут не содержит пользовательских конструкторов|
