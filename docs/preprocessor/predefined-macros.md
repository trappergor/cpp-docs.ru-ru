...
- **&#95; Строка &#95; &#95;** Определяется как целочисленный номер строки в текущем исходном файле. **&#95; &#95;&#95;** Значение`#line` макроса линии можно изменить с помощью директивы. Этот макрос определяется всегда.
...
- **&#95;M&#95;IX86&#95;FP** определяется как целочисленное литеральное значение, указывающее параметр компилятора [/Arch](../build/reference/arch-arm.md) , который был задан, или значение по умолчанию. Этот макрос всегда определяется, если целью компиляции является процессор x86. В противном случае — не определяется. Если определено, значение равно:
  ...
  - 2 `/arch:SSE2`, если задан `/arch:AVX`параметр компилятора `/arch:AVX2` , или. Это значение используется по умолчанию, `/arch` если не указан параметр компилятора. Если `/arch:AVX` указан параметр, также определяется  **&#95; &#95;макрос&#95; AVX** . Если `/arch:AVX2` указан, **&#95; &#95;&#95;** также определяются и AVX, и **&#95; &#95;AVX2&#95;** .
...
- **&#95;&#95; Метка &#95;времени** Определяется как строковый литерал, который содержит дату и время последнего изменения текущего исходного файла, в форме сокращенной длины, которая возвращается функцией CRT [asctime](../c-runtime-library/reference/asctime-wasctime.md) , `Fri 19 Aug 13:32:58 2016`например. Этот макрос определяется всегда.
...
- **&#95;Библиотека&#95;ATL** , определенная в \<атлдеф. h > как целочисленный литерал, который кодирует номер версии ATL.

- **&#95;MFC&#95;ver** , определенный \<в afxver_. h > как целочисленный литерал, который кодирует номер версии MFC.
