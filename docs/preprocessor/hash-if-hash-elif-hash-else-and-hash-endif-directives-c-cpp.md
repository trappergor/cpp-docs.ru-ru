...
*условное* : \
&nbsp;&nbsp;&nbsp;&nbsp;*Если-Part elif — части* <sub>неявное согласие</sub> *else-Part* <sub>неявное согласие</sub> *endif-строка*
...
Все директивы условной компиляции, такие как **#if** и **#ifdef**, должны соответствовать закрывающей директиве **#endif** перед концом файла. В противном случае создается сообщение об ошибке. Если в включаемых файлах содержатся директивы условной компиляции, они должны соответствовать тем же условиям: В конце включаемого файла не должно быть несовпадающих директив условной компиляции.
...
- Преобразование представляет тип **int** так же, как и тип **Long**, и **Неподписанное целое** точно так же, как и **без знака Long**.
...
Вызов функции `credit` компилируется, если определен идентификатор `CREDIT`. Если определен идентификатор `DEBIT`, компилируется вызов функции `debit`. Если ни один из этих идентификаторов не определен, компилируется вызов функции `printerror`. И, и являются отдельными идентификаторами в C C++ , так как их варианты различны. `credit` `CREDIT`
