...
Компилятор включает в себя предупреждения, отключенные по умолчанию, так как многие разработчики не хотят видеть их. Однако такие предупреждения можно включить с помощью одного из следующих параметров.
