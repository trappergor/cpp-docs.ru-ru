...
description: Справочник по константам TRACING_SESSION_MSVC_EVENT_FLAGS пакета SDK для C++ Build Insights.
ms.date: 02/12/2020
helpviewer_keywords:
- C++ Build Insights
...
Константы `TRACING_SESSION_MSVC_EVENT_FLAGS` используются для описания событий MSVC, которые следует собирать при трассировке. С помощью констант вы можете инициализировать поле `MsvcEventFlags` структуры [TRACING_SESSION_OPTIONS](tracing-session-options-struct.md).
...
| `TRACING_SESSION_MSVC_EVENT_FLAGS_ALL` | Этот флаг позволяет включить все события. |
