---
title: Константы TRACING_SESSION_MSVC_EVENT_FLAGS
description: В C++ пакете SDK для аналитики сборки TRACING_SESSION_MSVC_EVENT_FLAGS ссылки на константы.
ms.date: 02/12/2020
helpviewer_keywords:
- C++ Build Insights
...
ms.locfileid: "78333878"
---
# <a name="tracing_session_msvc_event_flags-constants"></a>Константы TRACING_SESSION_MSVC_EVENT_FLAGS
...
Константы `TRACING_SESSION_MSVC_EVENT_FLAGS` используются для описания событий КОМПИЛЯТОРОМ MSVC, которые должны быть собраны во время трассировки. Используйте их для инициализации поля `MsvcEventFlags` структуры [TRACING_SESSION_OPTIONS](tracing-session-options-struct.md) .
