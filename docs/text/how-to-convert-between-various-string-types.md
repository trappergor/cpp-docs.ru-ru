...
ms.locfileid: "81375813"
---
# <a name="how-to-convert-between-various-string-types"></a>Практическое руководство. Преобразование различных типов строк

В этой теме показано, как преобразовывать различные типы строк Visual C'в другие строки. Типы строк, которые `char *`охватываются включают, `wchar_t*` [, _bstr_t](../cpp/bstr-t-class.md), [CComBSTR](../atl/reference/ccombstr-class.md), [CString](../atl-mfc-shared/using-cstring.md), [basic_string](../standard-library/basic-string-class.md), и <xref:System.String?displayProperty=fullName>. Во всех случаях копия строки производится при преобразовании в новый тип. Любые изменения, внесенные в новую строку, не повлияют на исходную строку, и наоборот.

## <a name="converting-from-char-"></a>Преобразование из символа\*

## <a name="example"></a>Пример

### <a name="description"></a>Описание

В этом примере показано, `char *` как преобразовывать из типа строк в другие перечисленные выше. Строка `char *` (также известная как строка стиля C) использует нулевой символ для обозначения конца строки. Строки стиля C обычно требуют одного байта на одного персонажа, но также могут использовать два байта. В приведенных `char *` ниже примерах строки иногда называются многобайтными строками символов из-за данных строк, которые являются результатом преобразования из строк Unicode. Функции одиночного байта`MBCS`и мультибайта () могут работать на `char *` струнах.
...
В этом примере показано, `wchar_t *` как преобразовывать из типа строк в другие перечисленные выше. Несколько типов `wchar_t *`строк, в том числе, реализуют широкие форматы символов. Чтобы преобразовать строку между мультибайтом и широким форматом символов, можно использовать один вызов функции `mbstowcs_s` типа или вызов конструктора для такого `CStringA`класса.
...
В этом примере показано, `_bstr_t` как преобразовывать из типа строк в другие перечисленные выше. Объект `_bstr_t` — это способ инкапсулировать `BSTR` широкие строки символов. Строка BSTR имеет значение длины и не использует нулевой символ для прекращения строки, но тип строки, в который вы преобразуете, может потребовать прекращения нуля.
...
В этом примере показано, `CComBSTR` как преобразовывать из типа строк в другие перечисленные выше. Как и `CComBSTR` _bstr_t, объект — это способ инкапсулировать широкие строки BSTR символов. Строка BSTR имеет значение длины и не использует нулевой символ для прекращения строки, но тип строки, в который вы преобразуете, может потребовать прекращения нуля.
...
В этом примере показано, `CString` как преобразовывать из типа строк в другие перечисленные выше. `CString`основано на типе данных TCHAR, который, `_UNICODE` в свою очередь, зависит от того, определен ли символ. Если `_UNICODE` не определено, `TCHAR` определяется `CString` как символ и содержит многобайт строки символа; если `_UNICODE` определяется, `TCHAR` определяется `wchar_t` `CString` как и содержит широкую строку символов.

`CStringA`является мультибайт строка `CString`всегда `CStringW` версия , является широкой строки символа только версия. `CStringA` Ни, `CStringW` `_UNICODE` ни использовать для определения того, как они должны компилировать. `CStringA`и `CStringW` используются в этом примере для уточнения незначительных различий в распределении размера буфера и обработке выходных данных.
...
В этом примере показано, `basic_string` как преобразовывать из типа строк в другие перечисленные выше.
...
## <a name="converting-from-systemstring"></a>Преобразование из системы::String

## <a name="example"></a>Пример

### <a name="description"></a>Описание

В этом примере показано, как преобразовывать из широкой системы символов (Unicode): [Строка](/dotnet/api/system.string) в другие типы строк, перечисленные выше.
...
[ATL и MFC String Преобразование Макрос](../atl/reference/string-conversion-macros.md)<br/>
[Операции CString, относящиеся к струнам C-Style](../atl-mfc-shared/cstring-operations-relating-to-c-style-strings.md)<br/>
...
[Как: Преобразование системы::String к wchar_t или символ\*](../dotnet/how-to-convert-system-string-to-wchar-t-star-or-char-star.md)<br/>
