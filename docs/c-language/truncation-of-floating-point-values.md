---
title: Усечение значений с плавающей запятой
ms.date: 11/04/2016
helpviewer_keywords:
- floating-point numbers, truncation
ms.assetid: 051a6e22-c636-4af8-9ac4-40160f4affca
ms.openlocfilehash: 3aeb6ab1063d0782a6c5d297e387f3810c2e136a
ms.sourcegitcommit: 9266fc76ac2e872e35a208b4249660dfdfc87cba
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 04/16/2020
ms.locfileid: "81480882"
---
# <a name="truncation-of-floating-point-values"></a>Усечение значений с плавающей запятой

**ANSI 3.2.1.4** Направление усечения или округления при преобразовании числа с плавающей запятой в формат с плавающей запятой меньшего размера

При возникновении недотека значение переменной плавающей точки округляется до нуля. Потеря значимости может вызвать ошибку времени выполнения или привести к созданию непредсказуемого значения, в зависимости от указанных оптимизаций.

## <a name="see-also"></a>См. также

[Плавающая точка Математика](../c-language/floating-point-math.md)
