---
title: "Окружение | Документация Майкрософт"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology:
- cpp-language
ms.tgt_pltfrm: 
ms.topic: article
dev_langs:
- C++
ms.assetid: 2cf3e7d4-efb9-4eef-93a1-69ae1a60d3c0
caps.latest.revision: 8
author: mikeblome
ms.author: mblome
manager: ghogen
ms.translationtype: Human Translation
ms.sourcegitcommit: d6eb43b2e77b11f4c85f6cf7e563fe743d2a7093
ms.openlocfilehash: be1890982e84b4de7df66efb22fa6c91a6a0e8e8
ms.contentlocale: ru-ru
ms.lasthandoff: 05/18/2017

---
# <a name="environment"></a>Среда
-   [Аргументы функции main](../c-language/arguments-to-main.md)  
  
-   [Интерактивные устройства](../c-language/interactive-devices.md)  
  
## <a name="see-also"></a>См. также  
 [Поведение, определяемое реализацией](../c-language/implementation-defined-behavior.md)
