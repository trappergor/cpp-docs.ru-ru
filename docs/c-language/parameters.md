...
Если в списке параметров встречается хотя бы один параметр, список может заканчиваться запятой, за которой следуют три точки ( **,...** ). Эта конструкция, называемая "многоточием", указывает переменное число аргументов для функции. (Дополнительные сведения см. в разделе [вызовы с переменным количеством аргументов](../c-language/calls-with-a-variable-number-of-arguments.md) .) Однако вызов функции должен иметь по крайней мере столько аргументов, сколько параметров перед последней запятой.
