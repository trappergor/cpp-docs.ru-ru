---
title: "Диапазон значений типа char | Документы Майкрософт"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology:
- devlang-cpp
ms.tgt_pltfrm: 
ms.topic: article
dev_langs:
- C++
ms.assetid: 15ae9781-ec21-4333-bba8-6d2383bbf7f1
caps.latest.revision: 6
author: mikeblome
ms.author: mblome
manager: ghogen
translation.priority.ht:
- cs-cz
- de-de
- es-es
- fr-fr
- it-it
- ja-jp
- ko-kr
- pl-pl
- pt-br
- ru-ru
- tr-tr
- zh-cn
- zh-tw
translationtype: Human Translation
ms.sourcegitcommit: 3f91eafaf3b5d5c1b8f96b010206d699f666e224
ms.openlocfilehash: fd9fb6e12af29bf32b4ed8ad20252bf21e2e9e3e
ms.lasthandoff: 04/01/2017

---
# <a name="range-of-char-values"></a>Диапазон значений типа char
**ANSI 3.2.1.1** Имеет ли "простой" тип **char** тот же диапазон значений, что и **signed char** или `unsigned char`  
  
 Все знаковые значения char находятся в диапазоне от –128 до 127. Все беззнаковые значения char находятся в диапазоне от 0 до 255.  
  
 Параметр компилятора /J меняет используемый по умолчанию вариант: вместо **signed** устанавливается `unsigned`.  
  
## <a name="see-also"></a>См. также  
 [Символы](../c-language/characters.md)
