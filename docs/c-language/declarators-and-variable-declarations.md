---
title: "Деклараторы и объявления переменных | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "деклараторы, определение"
  - "объявление переменных, операторы объявления"
  - "объявление переменных, деклараторы"
...
|[Массивы](../Topic/Array%20Declarations.md)|Переменные, состоящие из коллекции элементов того же типа|  
...
|[Объединения](../c-language/union-declarations.md)|Переменные, состоящие из нескольких значений разных типов, занимающих одинаковое пространство для хранения|  
 ...
 Декларатор — это часть объявления, задающая имя, которое нужно вставить в программу.  Он может включать модификаторы, например **\*** \(указатель на\), и любые ключевые слова соглашения о вызовах Microsoft.  
...
> [!NOTE]
>  См. соответствующий синтаксис для `declaration` в разделе [Обзор объявлений](../Topic/Overview%20of%20Declarations.md) или раздел [Сводные сведения о синтаксисе языка C](../c-language/c-language-syntax-summary.md) для получения сведений о синтаксисе, который ссылается на `declarator`.  
  
 Если декларатор состоит из неизмененного идентификатора, объявляемый элемент имеет базовый тип.  Если звездочка \(**\***\) отображается слева от идентификатора, тип изменяется на тип указателя.  Если после идентификатора следуют квадратные скобки \(**\[ \]**\), тип изменяется на тип массивов.  Если после идентификатора следуют скобки, тип меняется на тип функции.  Дополнительные сведения об интерпретации приоритетности в пределах объявлений см. раздел [Интерпретация более сложных деклараторов](../c-language/interpreting-more-complex-declarators.md).  
  
 Каждый декларатор объявляет по крайней мере один идентификатор.  Декларатор должен включать описатель типа, чтобы называться полным объявлением.  Описатель типа предоставляет тип элементов типа массива, тип объекта, к которому относится тип указателя, или возвращаемый тип функции.  
  
 Объявления [массивов](../Topic/Array%20Declarations.md) и [указателей](../c-language/pointer-declarations.md) более подробно обсуждаются далее в этом разделе.  В следующем примере проиллюстрировано несколько простых форм деклараторов.  
