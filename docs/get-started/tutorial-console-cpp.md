...
ms.locfileid: "57700652"
---
# <a name="create-a-c-console-app-project"></a>Создание проекта консольного приложения С++

Обычной отправной точкой для программиста на C++ является приложение "Hello World", выполняемое в командной строке. Именно его вы и создадите в Visual Studio в рамках этой статьи.

## <a name="prerequisites"></a>Предварительные требования

- Установите и запустите на своем компьютере Visual Studio с рабочей нагрузкой "Разработка классических приложений на C++". Если установка еще не выполнена, см. статью [Установка поддержки C++ в Visual Studio 2017](../build/vscpp-step-0-installation.md).
...
3. В поле ввода **Имя** внизу введите имя нового проекта *CalculatorTutorial*, а затем нажмите кнопку **ОК**.
   ...
   При этом создается пустое консольное приложение Windows на C++. Консольные приложения используют окно консоли Windows для отображения выходных данных и приема данных, вводимых пользователем. В Visual Studio открывается окно редактора со сформированным кодом, который должен выглядеть следующим образом:
...
1. Чтобы запустить приложение, нажмите клавиши **CTRL+F5** или перейдите в меню **Отладка** и выберите **Запуск без отладки**. Если отображается всплывающее окно **Следующий проект устарел**, можете выбрать **Больше не показывать это диалоговое окно** и затем выберите **Да**, чтобы создать приложение. Вы должны увидеть всплывающее окно консоли с текстом, указанным в коде.
...
[Возникла проблема.](#edit-the-code-issues)
...
1. В меню **Проект** выберите команду **Добавить класс**. В поле ввода **Имя класса** введите *Calculator*. Нажмите кнопку **ОК**. Это добавляет в проект два новых файла. Чтобы сохранить сразу все измененные файлы, нажмите клавиши **CTRL+SHIFT+S**. Это сочетание клавиш для команды **Файл** > **Сохранить все**. Кроме того, имеется кнопка **Сохранить все** на панели инструментов, имеющая вид значка с двумя гибкими дисками рядом с кнопкой **Сохранить**. В общем случае рекомендуется регулярно нажимать кнопку **Сохранить все**, чтобы не пропустить никакие файлы при сохранении.

   ![Создание класса Calculator](./media/calculator-create-class.gif "Создание класса Calculator")

   Класс подобен схеме для объекта, который выполняет какие-либо действия. В этом случае мы определяем калькулятор и особенности его работы. Использованный ранее мастер **Добавить класс** создал файлы H и CPP, имя которых совпадает с именем класса. Вы можете просмотреть полный список файлов проекта в окне **обозревателя решений**, отображаемом в боковой части интегрированной среды разработки. Если окно не отображается, его можно открыть из строки меню, выбрав **Вид** > **Обозреватель решений**.
...
Если на этом этапе снова выполнить сборку и запуск кода, он все равно будет отображать "Hello World" в консоли. Далее вам нужно изменить функцию `main` для выполнения вычислений.
...
1. Установите точку останова на строке `result = c.Calculate(x, oper, y);`, сразу после запроса входных данных у пользователя. Для этого щелкните серую вертикальную панель, идущую вдоль левой части окна редактора, рядом с указанной строкой, чтобы появилась красная точка.

   ![Задание точки останова](./media/calculator-set-breakpoint.gif "Задание точки останова")

   Теперь при отладке программа всегда приостанавливает выполнение на этой строке. Но у нас уже есть общее представление о работе программы в простых случаях и приостанавливать выполнение каждый раз не требуется, поэтому давайте сделаем эту точку останова условной.

1. Щелкните правой кнопкой мыши красную точку, обозначающую точку останова, и выберите команду **Условия**. В поле ввода для условия введите `(y == 0) && (oper == '/')`. По завершении нажмите кнопку **Закрыть**; условие сохраняется автоматически.
...
1. Чтобы отладить программу, нажмите клавишу **F5** или выберите **Локальный отладчик Windows** (кнопка на панели инструментов со значком зеленой стрелки). Если ввести в консольном приложении нечто вроде "5 - 0", программа работает как обычно и продолжает выполнение. Но если ввести "10 / 0", она приостанавливает выполнение в точке останова. Вы можете ввести любое число пробелов между оператором и числами; функция `cin` достаточно интеллектуальна для того, чтобы правильно проанализировать входные данные.
...
Каждый раз при отладке кода вы можете заметить, что появляется несколько новых окон. Они могут оказаться полезными в процессе отладки. Давайте рассмотрим окно **Видимые**. Окно **Видимые** содержит текущие значения переменных, используемых по меньшей мере за 3 строки перед текущей строкой и через 3 строки после нее.

   ![Окно "Видимые"](./media/calculator-autos.png "Окно \"Видимые\"")

Чтобы просмотреть все переменные из этой функции, переключитесь в окно **Локальные**. Вы можете оперативно изменить значения этих переменных во время отладки, чтобы узнать их влияние на программу. В данном случае мы оставим их без изменений.

   ![Окно "Локальные"](./media/calculator-locals.png "Окно \"Локальные\"")

Кроме того, вы можете просто навести указатель на переменные в самом коде, чтобы просмотреть их текущие значения на участке, где приостановлено выполнение. Прежде чем делать это, убедитесь, что окно редактора находится в фокусе, щелкнув его.
...
1. Желтая линия слева показывает текущую точку выполнения. Сейчас она находится на строке, вызывающей `Calculate`, поэтому нажмите клавишу **F11**, чтобы выполнить **Шаг с заходом** для этой функции. Вы переходите в тело функции `Calculate`. Возможность **Шаг с заходом** следует использовать осторожно, так как при чрезмерном применении вы можете потерять очень много времени, так как она заходит в весь код, используемый на текущей строке, включая функции стандартных библиотек.

1. Теперь, когда точка выполнения находится в начале функции `Calculate`, нажмите клавишу **F10** для перехода на следующую строку в рамках выполнения программы. Это также называется **Шаг с обходом**. **Шаг с обходом** можно использовать для перемещения по строкам, не вдаваясь в подробности того, что происходит в каждой части строки. В общем случае следует использовать **Шаг с обходом**, а не **Шаг с заходом**, если только вы не хотите подробно изучить код, вызываемый из другого расположения (как в случае с переходом к телу функции `Calculate`).
...
1. Похоже, программа работает правильно — принимает первое число и делит его на второе. В строке `cout` наведите указатель мыши на переменную `result` или взгляните на `result` в окне **Видимые**. Вы увидите, что значение указано как "inf". Такое поведение выглядит неправильным, поэтому давайте его исправим. Строка `cout` просто выводит любое значение, хранящееся в `result`, поэтому при продвижении еще на один шаг с помощью клавиши **F10** в окне консоли отображается следующее:

   ![Результат деления на ноль](./media/calculator-divide-by-zero-fail.png "Результат деления на ноль")

   Это происходит, так как деление на ноль является неопределенным, поэтому у программы нет числового ответа на запрошенную операцию.
...
![Консольное приложение калькулятора](./media/calculator-app.gif "Консольное приложение калькулятора")
