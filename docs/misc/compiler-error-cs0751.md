...
Разделяемый метод должен быть объявлен в разделяемом классе или разделяемой структуре  
 ...
 Невозможно объявить [разделяемый](../Topic/partial%20\(Method\)%20\(C%23%20Reference\).md) метод, если он не инкапсулирован в разделяемом классе или разделяемой структуре.  
