...
"объявление поля": в структуре не может содержаться инициализатор полей экземпляров  
 ...
 Инициализировать поле экземпляра [структуры](../Topic/struct%20\(C%23%20Reference\).md) невозможно. Поля типов значения инициализируются значениями по умолчанию, а поля ссылочного типа инициализируются значением `null`.  
