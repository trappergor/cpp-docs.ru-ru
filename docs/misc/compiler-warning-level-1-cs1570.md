...
XML\-комментарий для "конструкция" содержит некорректный XML — "причина"  
 ...
 Если используется параметр [\/doc](../Topic/-doc%20\(C%23%20Compiler%20Options\).md), все комментарии в исходном коде должны быть в формате XML. Любая ошибка разметки XML вызывает предупреждение CS1570. Пример:  
