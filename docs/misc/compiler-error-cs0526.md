...
Интерфейсы не могут содержать конструкторы.  
 ...
 Не удается определить конструкторы для [интерфейсов](../Topic/interface%20\(C%23%20Reference\).md). Метод считается конструктором, если он имеет то же имя, что и класс, и не имеет тип возврата.  
