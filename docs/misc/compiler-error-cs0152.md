...
Метка "метка" уже встречалась в этом операторе switch  
 ...
 Метка повторялась в операторе [переключения](../Topic/switch%20\(C%23%20Reference\).md). Для получения дополнительной информации см. [switch](../Topic/switch%20\(C%23%20Reference\).md).  
