...
Невозможно передать поля "переменная" как аргумент с ключевым словом ref или out, потому что это "тип\_переменной\_только\_для\_чтения"  
 ...
 Эта ошибка происходит при попытке передать в функцию в качестве аргумента ref или out член переменной [foreach](../Topic/foreach,%20in%20\(C%23%20Reference\).md), [using](../Topic/using%20Statement%20\(C%23%20Reference\).md) или [fixed](../Topic/fixed%20Statement%20\(C%23%20Reference\).md). Так как в данных контекстах эти переменные доступны только для чтения, такая передача не разрешается.  
