...
Класс "класс" не имеет подходящего статического метода Main  
 ...
 Параметр компилятора [\/Main](../Topic/-main%20\(C%23%20Compiler%20Options\).md) задает класс, в котором следует искать метод **Main**. Однако метод [Main](../Topic/Main\(\)%20and%20Command-Line%20Arguments%20\(C%23%20Programming%20Guide\).md) не был определен правильно.  
