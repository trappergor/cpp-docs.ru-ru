...
Не может определять перегруженные методы, которые различаются только параметрами ref и out  
 ...
 Не допускается применять методы, которые отличаются только использованием модификаторов [ref](../Topic/ref%20\(C%23%20Reference\).md) и [out](../Topic/out%20\(C%23%20Reference\).md) в параметре.  
