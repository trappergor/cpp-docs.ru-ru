...
До передачи управления из текущего метода выходному параметру "параметр" должно быть присвоено значение  
 ...
 Параметру, помеченному ключевым словом [out](../Topic/out%20\(C%23%20Reference\).md), не было присвоено значение в теле метода. Дополнительные сведения см. в разделе [Передача параметров](../Topic/Passing%20Parameters%20\(C%23%20Programming%20Guide\).md).  
