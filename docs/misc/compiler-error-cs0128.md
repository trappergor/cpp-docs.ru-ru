...
В этой области действия уже определена локальная переменная с именем "переменная"  
 ...
 Компилятор обнаружил объявления двух локальных переменных с одинаковыми именами. Для получения дополнительной информации см. [Методы](../Topic/Methods%20\(C%23%20Programming%20Guide\).md).  
