...
Невозможно передать "параметр" как аргумент с ключевым словом ref или out, потому что "причина"  
 ...
 Эта ошибка происходит при передаче переменной как аргумента [ref](../Topic/ref%20\(C%23%20Reference\).md) или [out](../Topic/out%20\(C%23%20Reference\).md) в контексте, когда переменная доступна только для чтения. Контексты только для чтения включают переменные итерации [foreach](../Topic/foreach,%20in%20\(C%23%20Reference\).md), переменные [using](../Topic/using%20Statement%20\(C%23%20Reference\).md) и переменные `fixed`. Чтобы устранить эту ошибку, не вызывайте функции, принимающие переменную `foreach`, `using` или `fixed` в качестве параметра `ref` или `out` в блоках `using`, операторах `foreach` и `fixed`.  
