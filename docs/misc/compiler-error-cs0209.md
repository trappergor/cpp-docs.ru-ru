...
Локальная переменная, объявленная в операторе fixed, должна иметь тип указателя  
 ...
 Переменная, объявленная в [операторе fixed](../Topic/fixed%20Statement%20\(C%23%20Reference\).md), должна быть указателем. Дополнительные сведения см. в разделе [Небезопасный код и указатели](../Topic/Unsafe%20Code%20and%20Pointers%20\(C%23%20Programming%20Guide\).md).  
