...
В операторе объявления используется предложение [Оператор New](../Topic/New%20Operator%20\(Visual%20Basic\).md) для задания параметра типа в качестве типа, который должен быть создан, и параметр типа объявляется без ограничения `New`.  
