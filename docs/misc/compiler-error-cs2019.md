...
Недопустимый тип результата для \/target: необходимо указать "exe", "winexe", "library" или "module"  
 ...
 Использовался параметр компилятора [\/target](../Topic/-target%20\(C%23%20Compiler%20Options\).md), но был передан недопустимый параметр. Чтобы устранить эту ошибку, перекомпилируйте программу, используя форму параметра **\/target**, подходящую для вашего выходного файла.  
