...
"метод" не может задавать для параметра ref только атрибут Out. Используйте атрибуты In и Out вместе или не используйте их.  
 ...
 Метод интерфейса имеет параметр, который использует [ref](../Topic/ref%20\(C%23%20Reference\).md) только с атрибутом [Out](frlrfSystemRuntimeInteropServicesOutAttributeClassTopic). Параметр `ref`, который использует атрибут **Out**, должен также использовать атрибут [In](frlrfSystemRuntimeInteropServicesInAttributeClassTopic).  
