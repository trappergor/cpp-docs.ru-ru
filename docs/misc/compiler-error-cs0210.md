...
Требуется указать инициализатор в объявлении оператора fixed или using.  
 ...
 Переменная должна быть объявлена и инициализирована в [операторе fixed](../Topic/fixed%20Statement%20\(C%23%20Reference\).md). Для получения дополнительной информации см. [Небезопасный код и указатели](../Topic/Unsafe%20Code%20and%20Pointers%20\(C%23%20Programming%20Guide\).md).  
...
```  
 ...
 В приведенном ниже примере также возникает ошибка CS0210, так как в [операторе using](../Topic/using%20Statement%20\(C%23%20Reference\).md) не содержится инициализатор.  
