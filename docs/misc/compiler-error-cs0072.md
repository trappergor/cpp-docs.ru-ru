...
"событие": невозможно переопределить; "метод" не является событием.  
 ...
 [Событие](../Topic/event%20\(C%23%20Reference\).md) может переопределяться только другим событием. Для получения дополнительной информации см. [События](../Topic/Events%20\(C%23%20Programming%20Guide\).md).  
