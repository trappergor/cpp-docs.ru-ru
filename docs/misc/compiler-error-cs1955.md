...
## Пример  
 В приведенном ниже примере кода возникает ошибка CS1955, так как происходит попытка вызвать поле и свойство с помощью оператора вызова метода [\(\)](../Topic/\(\)%20Operator%20\(C%23%20Reference\).md). Поле или свойство вызвать невозможно, но можно обратиться к хранящемуся в нем значению с помощью оператора доступа к члену \([.](../Topic/.%20Operator%20\(C%23%20Reference\).md)\).  
