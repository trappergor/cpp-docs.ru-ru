...
Объявление XML\-литерала используется в неоднозначном для компилятора месте [!INCLUDE[vbprvb](../dotnet/includes/vbprvb_md.md)]. То есть компилятор [!INCLUDE[vbprvb](../dotnet/includes/vbprvb_md.md)] не может определить, является ли символ "меньше чем" \(\<\) оператором сравнения или началом XML\-литерала. Примером является следующий код:  
