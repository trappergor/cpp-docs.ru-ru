...
Для получения адреса ранее зафиксированного выражения нельзя использовать оператор fixed  
 ...
 Локальная переменная в методе [unsafe](../Topic/unsafe%20\(C%23%20Reference\).md) или параметре уже зафиксирована \(в стеке\), поэтому невозможно получить адрес любой из этих двух переменных в выражении [fixed](../Topic/fixed%20Statement%20\(C%23%20Reference\).md). Для получения дополнительной информации см. [Небезопасный код и указатели](../Topic/Unsafe%20Code%20and%20Pointers%20\(C%23%20Programming%20Guide\).md).  
