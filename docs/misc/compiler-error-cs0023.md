...
Оператор "оператор" не может применяться к операнду типа "тип"  
 ...
 Предпринята попытка применить оператор к переменной, тип которой не предназначен для работы с этим оператором. Дополнительные сведения см. в разделах [Типы](../Topic/Types%20\(C%23%20Programming%20Guide\).md) и [Операторы C\#](../Topic/C%23%20Operators.md).  
