...
Спецификаторы параметров по умолчанию не разрешены  
 ...
 [Параметры метода](../Topic/Method%20Parameters%20\(C%23%20Reference\).md) не могут иметь значения по умолчанию. Если вы хотите получить тот же эффект, используйте перегрузки метода. Дополнительные сведения см. в разделе [Передача параметров](../Topic/Passing%20Parameters%20\(C%23%20Programming%20Guide\).md).  
