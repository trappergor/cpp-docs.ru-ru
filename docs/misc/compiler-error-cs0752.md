...
Разделяемый метод не может иметь параметры out.  
 ...
 Разделяемый метод не может иметь параметр [out](../Topic/out%20\(C%23%20Reference\).md). Параметры out запрещены, поскольку при удалении компилятором разделяемого метода нет никакой гарантии, что параметр out когда\-либо будет назначен.  
