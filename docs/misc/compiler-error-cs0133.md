...
Выражение, назначаемое переменной "переменная", должно быть константным  
 ...
 Переменная [const](../Topic/const%20\(C%23%20Reference\).md) не может принимать в качестве своего значения выражение, которое не является константой. Для получения дополнительной информации см. [Константы](../Topic/Constants%20\(C%23%20Programming%20Guide\).md).  
