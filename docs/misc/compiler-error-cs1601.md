...
Параметр метода или делегата не может иметь тип "тип"  
 ...
 Некоторые типы в библиотеке классов .NET Framework, например <xref:System.TypedReference>, <xref:System.RuntimeArgumentHandle> и <xref:System.ArgIterator>, нельзя использовать как параметры [ref](../Topic/ref%20\(C%23%20Reference\).md) или [out](../Topic/out%20\(C%23%20Reference\).md), поскольку они потенциально могут использоваться для выполнения небезопасных операций.  
