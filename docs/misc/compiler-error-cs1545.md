...
Свойство, индексатор или событие "свойство" не поддерживается в данном языке; попробуйте напрямую вызвать метод доступа set или get.  
 ...
 Код использует объект, имеющий [индексатор](../Topic/Indexers%20\(C%23%20Programming%20Guide\).md), не являющийся индексатором по умолчанию, и пытается использовать индексированный синтаксис. Чтобы устранить эту ошибку, вызовите метод доступа `get` или `set` свойства.  
