...
"событие": событие должно иметь тип delegate  
 ...
 Для ключевого слова event требуется тип [delegate](../Topic/delegate%20\(C%23%20Reference\).md). Дополнительные сведения см. в разделах [События](../Topic/Events%20\(C%23%20Programming%20Guide\).md) и [Делегаты](../Topic/Delegates%20\(C%23%20Programming%20Guide\).md).  
