...
"элемент" не может быть запечатанным, так как не является результатом переопределения  
 ...
 Значение [sealed](../Topic/sealed%20\(C%23%20Reference\).md) использовалось для "элемент", который не был помечен атрибутом [override](../Topic/override%20\(C%23%20Reference\).md). Для получения дополнительной информации см. [Наследование](../Topic/Inheritance%20\(C%23%20Programming%20Guide\).md).  
