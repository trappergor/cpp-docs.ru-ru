...
> [!IMPORTANT]
>  Не передавайте данные, вводимые пользователем, в функцию `_exec`, не выбрав это содержимое явно.  `_exec` в конечном итоге вызовет функцию [CreateProcess](http://msdn.microsoft.com/library/windows/desktop/ms682425.aspx), поэтому помните, что неопределенные пути могут стать причиной возникновения уязвимостей безопасности.  
 ...
 Вызовы `_exec` не сохраняют режимы преобразования открытых файлов.  Если новый процесс должен использовать файлы, наследуемые из вызывающего процесса, используйте подпрограмму [\_setmode](../c-runtime-library/reference/setmode.md), чтобы задать нужный режим преобразования для этих файлов.  Перед вызовом функции `_exec` необходимо явно сбросить \(используя ключевое слово `fflush` или `_flushall`\) или закрыть все потоки.  Сигнальные параметры не сохраняются в новых процессах, создаваемых вызовом подпрограмм `_exec`.  Сигнальные параметры сбрасываются до значений по умолчанию в новом процессе.  
...
```  
 ...
 Запустите следующую программу, чтобы выполнить Crt\_args.exe:  
