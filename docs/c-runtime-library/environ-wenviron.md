...
Переменная `_environ` является указателем на массив указателей на строки многобайтовых символов, которые составляют среду процесса.  Использование этой глобальной переменной не рекомендуется в силу наличия более безопасных функциональных версий [getenv\_s, \_wgetenv\_s](../c-runtime-library/reference/getenv-s-wgetenv-s.md) и [\_putenv\_s, \_wputenv\_s](../c-runtime-library/reference/putenv-s-wputenv-s.md), которые должны использоваться вместо глобальной переменной.  `_environ` объявлена в файле Stdlib.h.  
