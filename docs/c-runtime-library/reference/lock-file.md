---
title: "_lock_file | Документы Майкрософт"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
Блокирует объект `FILE`, чтобы обеспечить согласованность для потоков, параллельно обращающихся к объекту `FILE`.  
...
## <a name="remarks"></a>Примечания  
 Функция `_lock_file` блокирует объект `FILE`, заданный с помощью `file`. Базовый файл не заблокирован `_lock_file`. Используйте [_unlock_file](../../c-runtime-library/reference/unlock-file.md) для снятия блокировки с файла. Вызовы `_lock_file` и `_unlock_file` должны совпадать в потоке.  
