...
> [!IMPORTANT]
>  Функции `_mbsnicmp` и `_mbsnicmp_l` не могут использоваться в приложениях, запускаемых в среде выполнения Windows. Дополнительные сведения: [Функции CRT, которые не поддерживаются в приложениях универсальной платформы Windows](../../cppcx/crt-functions-not-supported-in-universal-windows-platform-apps.md).  
...
|> 0|Подстрока `string1` больше, чем подстрока `string2`.|  
 ...
 При ошибке проверки параметров эти функции возвращают значение `_NLSCMPERROR`, которое определено в \<string.h> и \<mbstring.h>.  
