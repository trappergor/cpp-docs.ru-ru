...
*dest_size*<br/>
Размер целевого буфера строки в единицах **char** для узких и многобайтовых функций, а также единиц **wchar_t** для расширенных функций. Это значение должно быть больше нуля и не больше **RSIZE_MAX**.
...
|Любое действие|Любое действие|**NULL**|**EINVAL**|*конечный адрес* [0] задано значение 0|
|Любое действие|0 или слишком мал|Любое действие|**ERANGE**|*конечный адрес* [0] задано значение 0|

## <a name="remarks"></a>Примечания

Функция **strcpy_s** копирует содержимое в адрес *src*, включая завершающий символ null, в расположение, указанное в параметре *dest*. Строка назначения должна быть достаточно велика для сохранения исходной строки и завершающего нуля. Поведение **strcpy_s** не определено, если исходная и Целевая строки перекрываются.

**wcscpy_s** — это версия **strcpy_s**для расширенных символов, а **_mbscpy_s** — многобайтовая версия. Аргументы **wcscpy_s** — это строки расширенных символов; **_mbscpy_s** и **_mbscpy_s_l** — это строки многобайтовых символов. В остальном эти функции ведут себя одинаково. **_mbscpy_s_l** идентичен **_mbscpy_s** , за исключением того, что он использует переданный параметр языкового стандарта вместо текущего языкового стандарта. Для получения дополнительной информации см. [Locale](../../c-runtime-library/locale.md).
...
При создании C++ кода версии шаблонов могут быть проще в использовании.
