...
Структура **_utimbuf** сохраняет доступ к файлам и время изменения, используемые **_utime** для изменения дат изменения файлов. Структура содержит следующие поля, которые имеют тип **time_t**:
...
Определенные версии структуры **_utimbuf** ( **_utimebuf32** и **__utimbuf64**) определяются с использованием 32-разрядных и 64-разрядных версий типа времени. Они используются в 32- и 64-разрядных версиях этой функции. **_utimbuf** сам по умолчанию использует 64-разрядный тип времени, если не определен **_USE_32BIT_TIME_T** .

**_utime** идентичен **_futime** , за исключением того, что аргумент *filename* для **_utime** является именем файла или путем к файлу, а не дескриптором открытого файла.
...
Эта программа использует **_utime** для установки времени изменения файла на текущее время.
