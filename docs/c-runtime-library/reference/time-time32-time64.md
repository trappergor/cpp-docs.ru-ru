...
Возвращает время, как секунд, истекших с полуночи 1 января 1970 г., или значение -1, в случае ошибки.

## <a name="remarks"></a>Примечания

**Время** функция возвращает число секунд, истекших после полуночи (00: 00:00), 1 января 1970 года по Гринвичу (UTC) по системным часам. Возвращаемое значение хранится в расположении, указанном *destTime*. Этот параметр может быть **NULL**, в этом случае возвращаемое значение не сохраняется.

**время** является оболочкой для **_time64** и **time_t** по умолчанию, эквивалентно **__time64_t**. Если вам нужно заставить компилятор интерпретировать **time_t** как старое 32-разрядное **time_t**, можно определить **_USE_32BIT_TIME_T**. Это не рекомендуется, так как в приложении может произойти сбой после 18-го января 2038 года; использование этого макроса не поддерживается на 64-разрядных платформах.
