...
**localeconv** возвращает указатель на заполненный объект типа [struct lconv](../../c-runtime-library/standard-types.md). Значения, содержащиеся в объекте копируются из параметров языкового стандарта в локальной памяти потока и может быть перезаписана при последующих вызовах **localeconv**. Изменения, внесенные в значения в этом объекте, не изменяйте параметры языкового стандарта. Вызовы [setlocale](setlocale-wsetlocale.md) с *категории* значения **LC_ALL**, **LC_MONETARY**, или **LC_NUMERIC** Перезаписать содержимое структуры.
...
|Поле|Значение|
|-|-|
decimal_point,<br/>_W_decimal_point|Указатель на символ для неденежных значений десятичной запятой.
thousands_sep,<br/>_W_thousands_sep|Указатель на символ, разделяющий группы разрядов слева от десятичной запятой для неденежных значений.
группирование|Указатель на **char**-размера целое число, которое содержит размер каждой группы разрядов для неденежных значений.
int_curr_symbol,<br/>_W_int_curr_symbol|Указатель международный символ валюты для текущего языкового стандарта. Первые три символа определяют международный алфавитный символ валюты, как определено в стандарте *ISO 4217. Коды для представления валют и фондов*. Четвертый символ (непосредственно перед нуль-символом) разделяет международный символ валюты и денежное значение.
...
mon_grouping|Указатель на **char**-размера целое число, которое содержит размер каждой группы разрядов в денежных значениях.
...
За исключением случаев указанные, члены **lconv** структуры, содержащих `char *` и `wchar_t *` версии являются указателями на строки. Любой из них, равные **» «** (или **L "»** для **wchar_t \*** ) либо нулевой длины, либо не поддерживается в текущем языковом стандарте. Обратите внимание, что **decimal_point** и **_W_decimal_point** всегда поддерживаются и имеют ненулевую длину.

**Char** члены структуры имеют небольшие неотрицательные числа, а не символы. Любые из них, равные **CHAR_MAX**, не поддерживаются в текущем языковом стандарте.

Значения **группирование** и **mon_grouping** интерпретируются в соответствии со следующими правилами:
