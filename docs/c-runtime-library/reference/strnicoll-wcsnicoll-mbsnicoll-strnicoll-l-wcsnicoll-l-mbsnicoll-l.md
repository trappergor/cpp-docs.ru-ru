...
> [!IMPORTANT]
> **_mbsnicoll** и **_mbsnicoll_l** не может использоваться в приложениях, выполняемых в среде выполнения Windows. Дополнительные сведения: [Функции CRT, которые не поддерживаются в приложениях универсальной платформы Windows](../../cppcx/crt-functions-not-supported-in-universal-windows-platform-apps.md).
...
Каждая из этих функций возвращает **_NLSCMPERROR**. Для использования **_NLSCMPERROR**, включают любой из СТРОК. H или MBSTRING. З. **_wcsnicoll** может завершиться ошибкой, если *string1* или *string2* содержит коды расширенных символов, за пределами домена упорядоченной последовательности. При возникновении ошибки **_wcsnicoll** может задать **errno** для **EINVAL**. Для проверки ошибки во время вызова **_wcsnicoll**, задайте **errno** 0 и проверьте **errno** после вызова **_wcsnicoll**.

## <a name="remarks"></a>Примечания

Каждая из этих функций выполняет сравнение без учета регистра первого *число* символы в *string1* и *string2* согласно кодовой странице. Эти функции следует использовать только в том случае, когда есть различие между порядком символов в наборе и лексикографическим порядком символов в кодовой странице, и это различие представляет интерес при сравнении строк. Версии этих функций без **_l** суффикс страница текущего языкового стандарта и кода. Версии с **_l** суффиксом идентичны, за исключением того, что они используют переданный языковой стандарт. Для получения дополнительной информации см. [Locale](../../c-runtime-library/locale.md).
