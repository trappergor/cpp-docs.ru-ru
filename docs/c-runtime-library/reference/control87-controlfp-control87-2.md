...
Функция **_control87** получает и задает управляющее слово с плавающей запятой. Управляющее слово с плавающей запятой позволяет программе изменять режимы точности, округления и бесконечности в зависимости от платформы. **_Control87** также можно использовать для маскирования или раскрытия исключений с плавающей запятой. Если значение параметра *Mask* равно 0, **_control87** получает управляющее слово с плавающей запятой. Если параметр *Mask* имеет ненулевое значение, то для управляющего слова задается новый параметр: Для любого бита (то есть равного 1) в *маске*соответствующий бит в *New* используется для обновления управляющего слова. Иными словами, **фпкнтрл** = ((**фпкнтрл** & ~*Mask*) &#124; (*Новая* & *Маска*)), где **фпкнтрл** — это управляющее слово с плавающей запятой.
