...
*Сравнение* функции сравнения. Первым аргументом является *контекста* указателя. Второй аргумент — указатель на *ключ* для поиска. Третий аргумент — указатель на элемент массива, который будет сравниваться со значением *ключ*.

*контекст* указатель на контекст, который может быть любой объект, *сравнения* требуется доступ к подпрограммы.

## <a name="remarks"></a>Примечания

**Qsort_s** функция реализует алгоритм быстрой сортировки для сортировки массива из *номер* элементов, каждый из *ширина* байт. Аргумент *базового* является указателем на базовый массив для сортировки. **qsort_s** перезаписывает этот массив отсортированными элементами. Аргумент *сравнения* — это указатель на предоставляемую пользователем подпрограмму, которая сравнивает два элемента массива и возвращает значение, указывающее, их связь. **qsort_s** вызовы *сравнения* сопоставление один или несколько раз во время сортировки, передавая указатели на два элемента массива при каждом вызове:
...
**qsort_s** ведет себя как **qsort** , но *контекста* параметр и наборы **errno**. Путем передачи *контекста* параметра позволяет функции сравнения использовать указатель на объект для доступа к функциональным возможностям объекта или другие сведения, которые не доступны через указатель элемента. Добавление *контекста* делает **qsort_s** более надежное, поскольку *контекста* может использоваться для предотвращения ошибок повторного входа, связанных с использованием статических переменных для предоставления Общие сведения, доступные для *сравнения* функции.
...
В следующем примере демонстрируется использование *контекста* параметр в **qsort_s** функции. *Контекста* параметр облегчает выполнение потокобезопасных сортировок. Вместо статических переменных, которые необходимо синхронизировать для обеспечения потокобезопасности, можно передавать разные *контекста* параметр в каждой операции сортировки. В этом примере используется объект языкового стандарта в качестве *контекста* параметра.
