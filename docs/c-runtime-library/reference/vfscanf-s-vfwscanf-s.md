...
**Vfscanf_s** функция считывает данные из текущей позиции *поток* в расположения, заданные *arglist* список аргументов (если таковые имеются). Каждый аргумент в списке должен быть указателем на переменную типа, соответствующий спецификатору типа в *формат*. *формат* определяет интерпретацию входных данных поля и имеет те же форму и функциональные возможности, что *формат* аргумент для **scanf_s**; в разделе [поля спецификации формата: функции scanf и wscanf](../../c-runtime-library/format-specification-fields-scanf-and-wscanf-functions.md) описание *формат*. **vfwscanf_s** — это двухбайтовая версия **vfscanf_s**; формат аргумент **vfwscanf_s** представляет собой строку расширенных символов. Эти функции ведут себя одинаково, если поток открыт в режиме ANSI. **vfscanf_s** сейчас не поддерживает ввод из потока ЮНИКОДА.
