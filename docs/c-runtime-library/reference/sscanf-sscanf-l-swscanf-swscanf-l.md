...
**Sscanf** функция считывает данные из *буфера* в расположении, указанном каждого *аргумент*. Каждый *аргумент* должен быть указателем на переменную с типом, который соответствует спецификатору типа в *формат*. *Формат* элементы управления аргумент интерпретацию входных данных, поля и имеет те же форму и функциональные возможности, что *формат* аргумент для **scanf** функции. Если копирование производится между перекрывающимися строками, поведение не определено.

> [!IMPORTANT]
> При чтении строки с **sscanf**, всегда указывайте ширину для **%s** формат (например, **«% 32s»** вместо **«%s»**); в противном случае , неправильно отформатированный ввод может легко привести к переполнению буфера.

**swscanf** — это двухбайтовая версия **sscanf**; аргументы для **swscanf** представляют собой строки расширенных символов. **sscanf** не обрабатывает многобайтовые шестнадцатеричных символов. **swscanf** не обрабатывает шестнадцатеричное число полных Юникода или символы «зону совместимости». В противном случае **swscanf** и **sscanf** ведут себя одинаково.
