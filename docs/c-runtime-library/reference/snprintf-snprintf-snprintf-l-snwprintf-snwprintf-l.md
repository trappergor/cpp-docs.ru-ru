...
Функция **snprintf** и cемейство функций **_snprintf** форматируют и сохраняют *число* или меньше символов в *буфера*. Функция **snprintf** всегда хранит завершающий символ null, усечение выходных данных, при необходимости. Семейство функций **_snprintf** только добавляет знак завершения null, если длина форматированной строки строго меньше, чем *число* символов. Каждый *аргумент* (если есть) преобразуется и выводится согласно соответствующей спецификацией формата в *формат*. Формат состоит из обычных символов и имеет те же форму и функциональные возможности, что *формат* аргумент для [printf](printf-printf-l-wprintf-wprintf-l.md). Если копирование производится между перекрывающимися строками, поведение не определено.

> [!IMPORTANT]
> Убедитесь, что *format* не является строкой, определяемой пользователем. Поскольку **_snprintf** функции не гарантируют нулем — в частности, если возвращаемое значение равно *число*— убедитесь, что Далее следует код, добавляющий знак завершения null. Дополнительные сведения см. в разделе [Как избежать переполнения буфера](http://msdn.microsoft.com/library/windows/desktop/ms717795).
