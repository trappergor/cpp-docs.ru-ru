...
Эти примитивы с плавающей запятой реализуют C++ версии функции CRT [fpclassify](fpclassify.md) для типов с плавающей запятой. Вычисляется аргумент *x* , и классификация возвращается как одна из этих констант, определенная в Math. h:
...
Эти примитивы с плавающей запятой возвращают натуральный журнал *x*, LN (*x*) или log<sub>*e*</sub>(*x*), когда *base_flag* имеет значение 0. Они возвращают журнал с основанием 10 из *x*или log<sub>10</sub>(*x*), если *base_flag* не равен нулю. Эти функции не используются внутри. Для переносимости предпочтительнее использовать [журналы функций, логф, логл, LOG10, log10f и log10l](log-logf-log10-log10f.md).
...
*квадрат*<br/>
Смещение квадранта, равное 0, 1, 2 или 3, чтобы `sin`использовать `cos`для создания результатов `-cos` ,, `-sin`и.
