...
Вызовите **_resetstkoflw** , чтобы восстановить страницу защиты, когда восстановление выполняется после исключения переполнения стека. Эту функцию можно вызывать из основной части блока **__except** или за пределами блока **__except** . Однако существуют некоторые ограничения на момент ее использования. **_resetstkoflw** никогда не следует вызывать из:
...
**Libraries** Все версии [функций библиотеки CRT](../../c-runtime-library/crt-library-features.md).
...
В следующем примере показано рекомендуемое использование **_resetstkoflw** в программе, в которой структурированные исключения преобразуются в C++ исключения.
