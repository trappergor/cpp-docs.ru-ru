...
Чтобы определить исключения, которые были вызваны операцией с плавающей запятой, используйте функцию fetestexcept. Используйте параметр *except* , чтобы указать, какие флаги состояния исключения следует проверить. Функция **фетестексцепт** использует эти макросы исключений, определенные \<в fenv. h, > in, *за исключением* и возвращаемым значением:
