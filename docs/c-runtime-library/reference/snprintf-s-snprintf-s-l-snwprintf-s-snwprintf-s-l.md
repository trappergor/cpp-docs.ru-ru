...
**_snprintf_s** возвращает число символов, сохраненных в *буфера*, не считая завершающий нуль-символ. **_snwprintf_s** возвращает число расширенных символов, сохраненных в *буфера*, без учета завершающего расширенного символа null.

Если это хранилище, необходимое для хранения данных и завершающий нуль-символ, превышает *sizeOfBuffer*, вызывается обработчик недопустимого параметра, как описано в [проверка параметров](../../c-runtime-library/parameter-validation.md). Если после обработчика недопустимых параметров выполнение приложения продолжается, эти функции устанавливают *буфера* равным пустой строке, задайте **errno** для **ERANGE**и возвращают -1.
...
**_Snprintf_s** функция форматирует и сохраняет *число* или меньше символов в *буфера* и добавляет завершающий нуль-символ. Каждый аргумент (если есть) преобразуется и выводится согласно соответствующей спецификацией формата в *формат*. Форматирование согласуется с **printf** семейство функций см. в разделе [синтаксис описания формата: функции printf и wprintf](../../c-runtime-library/format-specification-syntax-printf-and-wprintf-functions.md). Если копирование производится между перекрывающимися строками, поведение не определено.

Если *число* — [_TRUNCATE](../../c-runtime-library/truncate.md), затем **_snprintf_s** записывает все строки в виде будет помещаться в *буфера* и оставить место для Завершение значение null. Если вся строка (с учетом завершающего нуль) помещается в *буфера*, затем **_snprintf_s** возвращает число записанных символов (не включая конечное значение null); в противном случае **_snprintf_s**  возвращает значение -1, усечение произошла.

> [!IMPORTANT]
> Убедитесь, что *format* не является строкой, определяемой пользователем.

**_snwprintf_s** — это двухбайтовая версия **_snprintf_s**; аргументы указателя для **_snwprintf_s** представляют собой строки расширенных символов. Определение кодировки ошибок в **_snwprintf_s** могут отличаться от того, **_snprintf_s**. **_snwprintf_s**, таких как **swprintf_s**, записывает выходные данные в строку, а не для назначения типа **ФАЙЛ**.
