...
32-разрядных и 64-разрядных версий из **gmtime**, [mktime](mktime-mktime32-mktime64.md), [mkgmtime](mkgmtime-mkgmtime32-mkgmtime64.md), и [localtime](localtime-localtime32-localtime64.md) используют одну общую **tm**  структуры на поток для преобразования. Каждый вызов одной из этих функций уничтожает результат любого предыдущего вызова. Если *sourceTime* представляет дату до полуночи 1 января 1970 г **gmtime** возвращает **NULL**. Ошибка не возвращается.

**_gmtime64**, которая использует **__time64_t** структуры, включает даты вверх до 23:59:59 31 декабря 3000 года в формате UTC, тогда как **_gmtime32** представляет даты только до 23:59:59 18 января 2038 года в формате UTC. Полночь 1-ого января 1970 года — нижняя граница диапазона дат для обеих функций.

**gmtime** — это встроенная функция, результатом которого является **_gmtime64**, и **time_t** эквивалентен **__time64_t** Если **_USE_32BIT_TIME_ T** определен. Если необходимо принудительно компилятор интерпретировать **time_t** как старое 32-разрядное **time_t**, можно определить **_USE_32BIT_TIME_T**, но делаю это выводится **gmtime** к встраиванию для **_gmtime32** и **time_t** необходимо определить как **__time32_t**. Рекомендуется не делать этого, поскольку данная возможность не поддерживается на 64-разрядных платформах и, в любом случае, ваше приложение может завершиться с ошибкой после 18-го января 2038 года.
...
**_Gmtime32** функция разбивает *sourceTime* значение и сохраняет его в статически выделенной структуре типа **tm**, определенный во времени. З. Значение *sourceTime* обычно получается из вызова [время](time-time32-time64.md) функции.
