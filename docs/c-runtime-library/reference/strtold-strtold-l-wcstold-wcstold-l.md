...
## <a name="remarks"></a>Примечания  
 Каждая функция преобразует входную строку `nptr` в `long double`. Функция `strtold` преобразует `nptr` в значение long двойной точности. Функция `strtold` прекращает чтение строки `nptr` на первом знаке, который она не может распознать как часть числа. Это может быть завершающий нуль-символ. `wcstold` — это версия `strtold` с расширенными символами; ее аргумент `nptr` — строка расширенных символов. В остальном эти функции работают одинаково.  
...
|`_tcstold_l`|`_strtold_l`|`_strtold_l`|`_wcstold_l`|  
 ...
 Функция `strtold` ожидает, что `nptr` указывает на строку следующего вида:  
