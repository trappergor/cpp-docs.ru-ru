...
**_locking** возвращает 0 в случае успеха. Значение возврата -1 указывает на сбой, и в этом случае [errno](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md) устанавливается на одно из следующих значений.
...
| **EDEADLOCK** | Нарушение блокировки. Возвращается, когда указан **_LK_LOCK** или **_LK_RLCK** флаг и файл не может быть заблокирован после 10 попыток. |
| **EINVAL** | Недействительный аргумент был дан **_locking**. |
...
**Функция _locking** блокирует или разблокирует *байты nbytes* файла, указанного *fd.* Блокировка байтов в файле предотвращает доступ других процессов к этим байтам. Все блокировки или разблокировки начинаются с текущего положения указателя файла и распространяются на следующие *nbytes* байт. Возможна блокировка байтов за концом файла.
...
Можно заблокировать несколько разделов файла, которые не перекрываются. Разблокируемый раздел файла должен быть предварительно заблокирован. **_locking** не объединяет прилегающие регионы; если два заблокированных региона смываются, каждый регион должен быть разблокирован отдельно. Разделы следует блокировать только на короткое время. Их необходимо разблокировать перед закрытием файла или выходом из программы.
