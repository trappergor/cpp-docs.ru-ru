...
Находят следующее имя, в том случае, если таковое имеется, которое соответствует *filespec* аргумента в предыдущем вызове [_findfirst](findfirst-functions.md), а затем измените *fileinfo* содержимое структуры соответствующим образом.
...
В случае успеха возвращает 0. В противном случае возвращается значение -1 и задает **errno** значение, указывающее характер ошибки. Возможные коды ошибок приведены в следующей таблице.

|Значение errno|Условие|
|-|-|
**EINVAL**|Недопустимый параметр: *fileinfo* был **NULL**. Или операционная система возвратила непредвиденную ошибку.
...
Необходимо вызвать [_findclose](findclose.md) после завершения работы с помощью **_findfirst** или **_findnext** функцию (или любых их вариантов). Она освобождает ресурсы, используемые этими функциями в приложении.

Варианты этих функций с **w** префикс — версии для расширенных символов; в противном случае они идентичны соответствующим функциям однобайтовые.
...
Если у вас нет особых причин для использования версии, указывающие размер времени явно, используйте **_findnext** или **_wfindnext** или, если необходима поддержка размеров файлов больше 3 ГБ, используйте **_ findnexti64** или **_wfindnexti64**. Все эти функции используют 64-разрядный тип времени. В предыдущих версиях эти функции использовали 32-разрядный тип времени. Если это является критическим изменением для приложения, можно определить **_USE_32BIT_TIME_T** для получения старого поведения. Если **_USE_32BIT_TIME_T** определен, **_findnext**, **_finnexti64** и соответствующие версии Юникода используют 32-разрядное время.
