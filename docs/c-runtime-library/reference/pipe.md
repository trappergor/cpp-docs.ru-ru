...
Чтобы использовать функцию **_pipe** для обмена данными между родительским процессом и дочерним процессом, каждый процесс должен иметь только один дескриптор, Открытый в канале. Дескрипторы должны быть противоположными: если родительский процесс имеет доступ на чтение, то дочерний процесс должен иметь открытый дескриптор записи. Самый простой способ сделать это — побитовое или ( **|** ) флаг **_O_NOINHERIT** с *TextMode*. Затем используйте **_dup** или **_dup2** , чтобы создать наследуемую копию дескриптора канала, который необходимо передать дочернему элементу. Закройте исходный дескриптор, затем создайте дочерний процесс. После возвращения из вызова spawn закройте дублирующий дескриптор в родительском процессе. Дополнительные сведения смотрите в примере 2 ниже в данном разделе.
