...
**Функции _execvpe** проверяют свои параметры. Если *cmdname* является нулевой указатель, или если *argv* является нулевой указатель, указатель на пустой массив, или указатель на массив, который содержит пустую строку в качестве первого аргумента, эти функции вызывают недействительным обработчик параметра, как описано в [параметр проверки.](../../c-runtime-library/parameter-validation.md) Если выполнение разрешено продолжать, эти функции устанавливают **errno** к **EINVAL** и возвращают -1. Ни один процесс не запущен.
