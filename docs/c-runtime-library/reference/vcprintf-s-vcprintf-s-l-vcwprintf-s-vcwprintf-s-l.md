...
Менее безопасные версии этих функций, например если *формат* является пустым указателем, вызывается обработчик недопустимого параметра, как описано в [проверка параметров](../../c-runtime-library/parameter-validation.md). Кроме того, в отличие от менее безопасные версии этих функций Если *формат* не соответствует формату, создается исключение недопустимого параметра. Если выполнение может быть продолжено, эти функции возвращают код ошибки и набор **errno** к этому коду ошибки. Код ошибки по умолчанию — **EINVAL** Если более конкретное значение не применяется.

## <a name="remarks"></a>Примечания

Каждая из этих функций принимает указатель на список аргументов, а затем форматирует и записывает указанные данные в консоль. **_vcwprintf_s** — это двухбайтовая версия **_vcprintf_s**. Она принимает строку расширенных символов в качестве аргумента.

Версии этих функций, имеющих **_l** суффиксом идентичны, за исключением того, что они используют переданный параметр языкового стандарта вместо текущего языкового стандарта.
