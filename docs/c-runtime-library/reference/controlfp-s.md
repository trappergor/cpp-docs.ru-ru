...
Если *маска* — не равно нулю, устанавливается новое значение управляющего слова: для любого включенного бита, которое имеет значение (то есть равного 1) в *маска*, соответствующий бит в *новый* используется для обновления элемента управления Word. Другими словами *fpcntrl* = ((*fpcntrl* & ~*маска*) &#124; (*newControl* & *маски* )) где *fpcntrl* — это слово с плавающей запятой. В этом случае *currentControl* присваивается значение после завершения изменения; это не старое битовое значение управляющего слова.
