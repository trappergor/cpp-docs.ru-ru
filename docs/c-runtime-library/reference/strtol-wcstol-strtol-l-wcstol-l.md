...
**стртол**, **wcstol**, **_strtol_l**и **_wcstol_l** возвращают значение, представленное в виде *строки*. Они возвращают 0, если преобразование невозможно. Когда представление вызовет переполнение, оно возвращает **LONG_MAX** или **LONG_MIN**.

в случае переполнения или потери значимости **устанавливается значение** **ERANGE** . Он имеет значение **еинвал** , если *строка* имеет **значение NULL**. Или, если значение *base* не равно нулю и меньше 2 или больше 36. Дополнительные сведения о **ERANGE**, **еинвал**и других кодах возврата см. в разделе [_doserrno, не_sys_errlist и _sys_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md).

## <a name="remarks"></a>Remarks

Функции **стртол**, **wcstol**, **_strtol_l**и **_wcstol_l** преобразуют *строку* в **Long**. Они прекращают чтение *строки* с первого символа, который не распознается как часть числа. Это может быть завершающий символ null или первый буквенно-цифровой символ, который больше или равен *base*.

**wcstol** и **_wcstol_l** — это версии **стртол** и **_strtol_l**для расширенных символов. Их *строковый* аргумент является строкой расширенных символов. Эти функции ведут себя одинаково для **стртол** и **_strtol_l** в противном случае. Параметр категории **LC_NUMERIC** языковой стандарт определяет распознавание символа системы счисления (маркер дробной части или десятичной запятой) в *строке*. Функции **стртол** и **wcstol** используют текущий языковой стандарт. **_strtol_l** и **_wcstol_l** использовать переданный языковой стандарт. Дополнительные сведения см. в разделе [setlocale] и [языковой стандарт](../../c-runtime-library/locale.md).
...
**стртол** ждет, что *строка* указывает на строку следующего вида:
...
Функции **_strtol_l** и **_wcstol_l** являются специфичными для Microsoft, а не частью стандартной библиотеки C. Дополнительные сведения о совместимости см. в статье [Compatibility](../compatibility.md).
