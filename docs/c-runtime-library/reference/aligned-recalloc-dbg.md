...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "_aligned_recalloc_dbg - функция"
  - "aligned_recalloc_dbg - функция"
...
## Заметки  
 ...
 `_aligned_recalloc_dbg` задает для `errno` значение `ENOMEM` в случае сбоя выделения памяти; значение `EINVAL` возвращается, если необходимый объем памяти \(включая ранее упомянутую нагрузку\) превышает `_HEAP_MAXREQ`.  Сведения об этих и других кодах ошибок см. в разделе [errno, \_doserrno, \_sys\_errlist, and \_sys\_nerr](../Topic/errno,%20_doserrno,%20_sys_errlist,%20and%20_sys_nerr.md).  
