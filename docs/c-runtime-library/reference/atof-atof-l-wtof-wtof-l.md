...
Каждая функция возвращает **двойные** значение, порожденное интерпретации входных символов как числа. Возвращаемое значение равно 0,0, если аргумент невозможно преобразовать в значение этого типа.
...
Объект *пробелы* состоит из пробелы или символы табуляции, которые игнорируются; *входа* либо плюс (+) или минус (–); и *цифр* — один или несколько десятичных цифр. Если перед десятичной запятой никаких цифр нет, после нее должен отображаться хотя бы один символ. За десятичными цифрами может следовать показатель степени, который состоит из вводной буквы (**e**, или **E**) и при необходимости со знаком десятичное целое число.
...
Версии этих функций с **_l** суффиксом идентичны, за исключением того, что они используют *языкового стандарта* переданный параметр вместо текущего языкового стандарта.
...
Эта программа показывает, как числа, хранящиеся в виде строк можно преобразовать в числовые значения с помощью **atof** и **_atof_l** функции.
