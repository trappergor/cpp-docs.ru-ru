...
> [!IMPORTANT]
> **_mbslen**, **_mbslen_l**, **_mbstrlen**, и **_mbstrlen_l** не может использоваться в приложениях, выполняемых в среде выполнения Windows. Дополнительные сведения: [Функции CRT, которые не поддерживаются в приложениях универсальной платформы Windows](../../cppcx/crt-functions-not-supported-in-universal-windows-platform-apps.md).
...
Каждая из этих функций возвращает число символов в *str*, за исключением терминалов null. Возвращаемое значение не является зарезервированным для указания ошибки, за исключением **_mbstrlen** и **_mbstrlen_l**, которые возвращают `((size_t)(-1))` , если строка содержит недопустимый Многобайтовый символ.

## <a name="remarks"></a>Примечания

**strlen** интерпретирует строку как строку однобайтовых символов, поэтому его возвращаемое значение всегда равно числу байт, даже если строка содержит многобайтовые символы. **wcslen** — это двухбайтовая версия **strlen**; аргумент **wcslen** — строка расширенных символов, а число символов выражается в расширенных (двухбайтовых) символах. **wcslen** и **strlen** ведут себя идентично.
