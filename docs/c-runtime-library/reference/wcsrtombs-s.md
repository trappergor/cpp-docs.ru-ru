...
Если параметр *Count* является специальным значением [_TRUNCATE](../../c-runtime-library/truncate.md), **wcsrtombs_s** преобразует столько строк, сколько помещается в буфер назначения, в то время как при этом остается место для нулевого терминатора.
...
Если **wcsrtombs_s** встречает широкий символ, который он не может преобразовать в многобайтовый символ, он помещает-1 в  *\*претурнвалуе*, устанавливает для буфера назначения пустую строку **, устанавливает значение** очистки в **еилсек**и возвращает **еилсек** .

Если последовательности, на которые указывает *вкстр* и *мбстр* , перекрываются, поведение **wcsrtombs_s** не определено. на **wcsrtombs_s** влияет Категория LC_TYPE текущего языкового стандарта.
