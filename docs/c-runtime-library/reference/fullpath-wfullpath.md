...
Если определены и **_DEBUG** , и **_CRTDBG_MAP_ALLOC** , вызовы методов **_fullpath** и **_wfullpath** заменяются вызовами **_fullpath_dbg** и **_wfullpath_dbg** , чтобы можно было выполнять отладку выделения памяти. Дополнительные сведения см. в разделе [_fullpath_dbg, _wfullpath_dbg](fullpath-dbg-wfullpath-dbg.md).
