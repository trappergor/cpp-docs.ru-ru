...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "_aligned_offset_realloc_dbg - функция"
  - "aligned_offset_realloc_dbg - функция"
...
## Заметки  
 `_aligned_offset_realloc_dbg` — это отладочная версия функции [\_aligned\_offset\_realloc](../../c-runtime-library/reference/aligned-offset-realloc.md).  Если [\_DEBUG](../Topic/_DEBUG.md) не определен, каждый вызов функции `_aligned_offset_realloc_dbg` сокращается до вызова функции \_`aligned_offset_realloc`.  И \_`aligned_offset_realloc`, и `_aligned_offset_realloc_dbg` выполняют перераспределение блока памяти в основной куче, однако `_aligned_offset_realloc_dbg` включает различные функции отладки: буферы на обеих сторонах пользовательской части блока для тестирования утечек, параметр типа блока для отслеживания конкретных типов выделения, а также сведения о `filename`\/`linenumber` для определения источника запросов на выделение.  
  
 Как и [\_aligned\_offset\_malloc](../../c-runtime-library/reference/aligned-offset-malloc.md), `_aligned_offset_realloc_dbg` позволяет выполнить выравнивание структуры со смещением в пределах структуры.  
