...
**Scanf** функция считывает данные из стандартного входного потока **stdin** и записывает данные в расположении, указанном *аргумент*. Каждый *аргумент* должен быть указателем на переменную-тип, соответствующий спецификатору типа в *формат*. Если копирование производится между перекрывающимися строками, поведение не определено.

> [!IMPORTANT]
> При чтении строки с **scanf**, всегда следует указывать ширину для **%s** формат (например, **«% 32 с»** вместо **«%s»**); в противном случае — значение Ввод в неправильном формате может легко привести к переполнению буфера. Кроме того, рекомендуется использовать [scanf_s, _scanf_s_l, wscanf_s, _wscanf_s_l](scanf-s-scanf-s-l-wscanf-s-wscanf-s-l.md) или [fgets](fgets-fgetws.md).

**wscanf** — это двухбайтовая версия **scanf**; *формат* аргумент **wscanf** — строка расширенных символов. **wscanf** и **scanf** ведут себя одинаково, если поток открыт в режиме ANSI. **scanf** сейчас не поддерживает ввод из потока ЮНИКОДА.
