...
В функциях **strcmp** учитывается регистр. стрикмп, **вксикмп и\_** **мбсикмп сравниваютстроки,сначалапреобразуяих\_** в их формы в нижнем регистре.  **\_** Две строки, содержащие символы, расположенные между "Z" и "a" в таблице ASCII ("[", "\\", "]", "^", "_" и "\`"), сравниваются по-разному в зависимости от их регистра. Например, две строки "ABCD" и "ABCD ^" сравнивают один из них, если сравнение является строчным ("ABCDE" > "abcd ^"), а другой способ ("ABCD" < "ABCD ^"), если сравнение является прописным.
