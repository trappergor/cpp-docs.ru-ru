...
Каждая из этих функций загружает и выполняет новый процесс, передавая массив указателей аргументам командной строки и используя переменную среды **PATH** для поиска файла для выполнения.

**Функции _execvp** проверяют свои параметры. Если *cmdname* является нулевой указатель, или *argv* является нулевой указатель, указатель на пустой массив, или если массив содержит пустую строку в качестве первого аргумента, эти функции вызывают недействительным обработчик параметров, как описано в [параметрвалиции](../../c-runtime-library/parameter-validation.md). Если выполнение разрешено продолжать, эти функции устанавливают **errno** к **EINVAL** и возвращают -1. Ни один процесс не запущен.
