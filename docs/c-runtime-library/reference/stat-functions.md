...
Каждая из этих функций возвращает 0, если получена информация о состоянии файла. Возвращаемое значение-1 указывает на ошибку в этом случае **errno** равно **ENOENT**, указывающее, что имя файла или путь не найден. Возвращаемое значение **EINVAL** указывает недопустимый параметр; **errno** задается значение **EINVAL** в этом случае.

Дополнительные сведения об этих и других кодах возврата см. в разделе [_doserrno, errno, _sys_errlist и _sys_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md) .

Отметка даты в файле может быть представлена, если она позже полуночи 1 января 1970 года и раньше 23:59:59, 31 декабря 3000 года, время в формате UTC, если не используется **_stat32** или **_wstat32**, или определена директива **_ USE_32BIT_TIME_T**, в этом случае дата может быть представлена только до 23:59:59 18 января 2038 года, время UTC.

## <a name="remarks"></a>Примечания

**_Stat** функция получает сведения о файле или каталоге, заданном параметром *путь* и сохраняет его в структуре, на который указывает *буфера*. **_stat** автоматически требуемым образом обрабатывает аргументы строки многобайтовых символов соответствующим образом, распознавая последовательности многобайтовых символов согласно многобайтовой кодовой страницей в настоящий момент.

**_wstat** — это двухбайтовая версия **_stat**; *путь* аргумент **_wstat** представляет собой строку расширенных символов. **_wstat** и **_stat** ведут себя одинаково, за исключением того, что **_wstat** не обрабатывает многобайтовые строки.
...
**_stat** эквивалентно **_stat64i32**, и **структуры** **_stat** содержит 64-разрядное время. Это значение true, если не **_USE_32BIT_TIME_T** определен, в этом случае старое поведение действует; **_stat** использует 32-разрядное время, и **структуры** **_stat** содержит 32-разрядное время. То же самое справедливо для **_stati64**.

> [!NOTE]
> **_wstat** не работает с символьными ссылками Windows Vista. В этих случаях **_wstat** будет всегда отчета размер файла, равный 0. **_stat** правильно работает с символьными ссылками.

Эта функция проверяет свои параметры. Если параметр *путь* или *буфера* — **NULL**, вызывается обработчик недопустимого параметра, как описано в [проверка параметров](../../c-runtime-library/parameter-validation.md).
...
**_Stat** структура, определенная в SYS\STAT. Символ «H» включает следующие поля.
...
**st_ino**|Номер узла информации ( **inode**) для файла (UNIX). В файловых системах UNIX **inode** описывает дату создания файла и отметки времени, разрешения и содержимое. Если файлы связаны жесткими ссылками друг с другом, они имеют одинаковое **inode**. **Inode**и, следовательно, **st_ino**, не имеет смысла в файловых системах FAT, HPFS или NTFS.
**st_mode**|Битовая маска для информации о файловом режиме. **_S_IFDIR** бит устанавливается, если *путь* задает каталог; **_S_IFREG** бит устанавливается, если *путь* задает обычный файл или устройство. Пользовательские биты чтения/записи устанавливаются в соответствии с режимом разрешений файла; пользовательские биты выполнения устанавливаются согласно расширению имени файла.
...
**st_size**|Размер файла в байтах; 64-разрядное целое для вариантов с **i64** суффикс.
