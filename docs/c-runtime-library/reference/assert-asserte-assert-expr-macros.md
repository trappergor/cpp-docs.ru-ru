...
Несмотря на то, что **_ASSERT_EXPR**, **_ASSERT** и **_ASSERTE** являются макросами и доступны, включая \<Crtdbg. h >, приложение должно связываться с отладочной версией библиотеки времени выполнения языка C, если **_DEBUG** определена, так как эти макросы вызывают другие функции времени выполнения.
