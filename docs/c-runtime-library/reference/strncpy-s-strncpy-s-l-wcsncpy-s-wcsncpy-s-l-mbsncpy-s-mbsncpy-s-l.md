...
|Любое действие|Любое действие|**NULL**|**EINVAL**|*стрдест* [0] задано значение 0|
|Любое действие|0|Любое действие|**EINVAL**|не изменено|
|не **null**|слишком мало|Любое действие|**ERANGE**|*стрдест* [0] задано значение 0|
...
Для вышеприведенного абзаца есть исключения. Если параметр *Count* имеет значение **_TRUNCATE**, то *стрсаурце* , как помещается в *стрдест* , копируется, оставляя место для завершающего null, который всегда добавляется.
...
означает, что мы просим **strncpy_s** скопировать пять символов в буфер с пятью байтами. в этом случае не останется пробела для терминатора null, поэтому **strncpy_s** отменяет строку и вызывает обработчик недопустимых параметров.
...
Поведение **strncpy_s** не определено, если исходная и Целевая строки перекрываются.
...
**wcsncpy_s** и **_mbsncpy_s** — это версии **strncpy_s**для расширенных символов и многобайтовых символов. Аргументы и возвращаемые значения **wcsncpy_s** и **mbsncpy_s** различаются соответственно. В остальном эти шесть функций ведут себя идентично.
...
> [!NOTE]
> **_strncpy_s_l**, **_wcsncpy_s_l** и **_mbsncpy_s_l** не имеют зависимости от языковых стандартов и предоставляются только для **_tcsncpy_s_l** и не предназначены для непосредственного вызова.
