...
Если *размер* больше, чем **_ALLOCA_S_THRESHOLD**, затем **_malloca** пытается выделить в куче и возвращает указатель null, если не удается выделить пространство. Если *размер* меньше или равно **_ALLOCA_S_THRESHOLD**, затем **_malloca** пытается выделить в стеке и исключение переполнения стека создается в том случае, если невозможно пространство выделить. Исключение переполнения стека не исключения C++; это структурированное исключение. Вместо использования обработки исключений C++, необходимо использовать [структурированную обработку исключений](../../cpp/structured-exception-handling-c-cpp.md) (SEH), чтобы перехватить это исключение.
