...
В отличие от **vscanf** и **vwscanf**, **vscanf_s** и **vwscanf_s** требует указания размера буфера для всех входных параметров типов **c**, **c**, s, **или**наборов элементов управления строки, заключенных в **[]** . Размер буфера в символах передается как дополнительный параметр, после которого сразу следует указатель на буфер или переменную. Размер буфера в символах для строки **wchar_t** не совпадает с размером в байтах.
