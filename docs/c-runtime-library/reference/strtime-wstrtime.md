...
**_wstrtime** — это двухбайтовая версия **_strtime**; аргумент и возвращаемое значение **_wstrtime** представляют собой строки расширенных символов. В остальном эти функции ведут себя одинаково. Если *timestr* — **NULL** указателя или если *timestr* отформатирован неправильно, указан неправильный вызывается обработчик параметров, как описано в [параметр Проверка](../../c-runtime-library/parameter-validation.md). Если исключение может быть продолжено, эти функции возвращают **NULL** и задайте **errno** для **EINVAL** Если *timestr* было **NULL** или задать **errno** для **ERANGE** Если *timestr* имеет неправильный формат.
