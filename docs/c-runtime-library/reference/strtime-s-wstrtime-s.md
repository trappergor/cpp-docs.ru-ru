...
|*Буфера*|*numberOfElements*|Возвращает|Содержимое *буфера*|
|--------------|------------------------|------------|--------------------------|
|**Null**|(любые)|**EINVAL**|Без изменений|
|Не **NULL** (указывая на действительный буфер)|0|**EINVAL**|Без изменений|
|Не **NULL** (указывая на действительный буфер)|0 < size < 9|**EINVAL**|Пустая строка.|
|Не **NULL** (указывая на действительный буфер)|Size > 9|0|Текущая дата в формате, указанном в разделе "Примечания"|

## <a name="security-issues"></a>Проблемы с безопасностью

Прохождение в недействительное значение**недействительного** для буфера приведет к нарушению доступа, если параметр *numberOfElements* превышает 9.

Прохождение значения для *numberOfElements,* превышающее фактический размер буфера, приведет к перезапуску буфера.

## <a name="remarks"></a>Remarks

Эти функции обеспечивают более безопасные версии [_strtime](strtime-wstrtime.md) и [_wstrtime.](strtime-wstrtime.md) **Функция _strtime_s** копирует текущее местное время в буфер, на который указывает *timestr*. Время отформатировано как **hh:mm:ss,** где **hh** — это две цифры, представляющие час в 24-часовой нотации, **мм** – две цифры, представляющие минуты за часом, а **ss** — две цифры, представляющие секунды. Например, строка **18:23:44** представляет 23 минуты и 44 секунды после 18:00. Размер буфера должен составлять не менее 9 байт; фактический размер указывается в качестве второго параметра.

**_wstrtime** является широкохарактерным вариантом **_strtime;** аргументивация и значение возврата **_wstrtime** являются широкохарактерными строками. В остальном эти функции ведут себя одинаково.
