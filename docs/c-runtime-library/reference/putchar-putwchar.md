...
Возвращает записанный символ. Чтобы указать ошибку или состояние конца файла, **putc** и **putchar** возврат **EOF;** **putwc** и **putwchar** возвращение **WEOF**. Для всех четырех подпрограмм используйте [ferror](ferror.md) или [feof](feof.md) для проверки наличия ошибки или конца файла. Если переданная нулевая закателька для *потока,* эти функции генерируют недействительное исключение параметра, как описано в [проверке параметра.](../../c-runtime-library/parameter-validation.md) Если исполнение разрешено продолжать, они возвращают **EOF** или **WEOF** и устанавливают **errno** в **EINVAL.**
