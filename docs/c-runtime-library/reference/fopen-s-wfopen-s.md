...
При открытии файла с использованием типа доступа **"a"** или **"a +"** все операции записи выполняются в конце файла. Указатель файла может перемещаться с помощью [fseek](fseek-fseeki64.md) или [rewind](rewind.md), но он всегда перемещается обратно в конец файла перед выполнением любой операции записи, чтобы существующие данные не могли быть перезаписаны.
