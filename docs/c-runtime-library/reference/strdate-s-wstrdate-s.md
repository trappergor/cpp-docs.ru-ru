...
|*buffer*|*numberOfElements*|Назад|Содержание *буфера*|
|--------------|------------------------|------------|--------------------------|
...
Передача недействительного не **NULL** для буфера приведет к нарушению прав доступа, если значение *numberOfElements* параметра больше 9.

Передача значений для размера, который больше, чем фактический размер *буфера* приведет к переполнению буфера.

## <a name="remarks"></a>Примечания

Эти функции представляют собой более безопасные версии **_strdate** и **_wstrdate**. **_Strdate_s** функция копирует текущую системную дату в буфер, на которые указывают *буфера*форматированное **мм**/**дд** / **гг**, где **мм** состоит из двух цифр, представляющих месяц, **дд** состоит из двух цифр, представляющих день, и **гг**  имеет две последние цифры года. Например, строка **12/05/99** представляет 5 декабря 1999 г. Буфер должен содержать по крайней мере 9 символов.
