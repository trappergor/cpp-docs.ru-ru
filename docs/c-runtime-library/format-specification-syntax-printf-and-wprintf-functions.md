...
**%**[*Флаги*](#flags) [*Ширина*](#width) [. [*точность*](#precision) Размер -[*это*](#size) [*тип*](#type)
...
> [!NOTE]
> В Visual Studio 2015 `printf` и `scanf` семейство функций были объявлены **внеочередными** и перемещены в заголовки `<stdio.h>` и `<conio.h>` заголовки. При переносе старого кода вы можете увидеть *LNK2019* в связи с этими функциями. Для получения дополнительной информации, [см.](../porting/visual-cpp-change-history-2003-2015.md#stdio_and_conio)
...
Аргументы, которые следуют за строкой формата, интерпретируются согласно соответствующему символу *type* и необязательному префиксу [size](#size). Преобразования для символьных типов `char` и `wchar_t` определяются с помощью символов **c** или **C**; однобайтовые и многобайтовые строки или строки с расширенными символами указываются с помощью символа **s** или **S**, в зависимости от используемой функции форматирования. Аргументы характера и строки, которые определяются с `char` `char*` помощью **c** и `wchar_t` `wchar_t*` **s,** интерпретируются как `printf` семейные функции, или как и `wprintf` семейные функции. Символьные и строковые аргументы, указанные с использованием **C** и **S**, интерпретируется как `wchar_t` и `wchar_t*` функциями семейства `printf` или как `char` и `char*` функциями семейства `wprintf`. Такое поведение специфична для майкрософт.

Целые типы, `short` `int`такие `long` `long long`как , `unsigned` , , и их варианты, определяются с помощью **d,** **я**, **o**, **u**, **x**, и **X**. Типы плавающих `float`точек, такие как, `double`и `long double`, указаны с помощью **a,** **A,** **e,** **e, f,** **F,** **g**и **G**. **f** По умолчанию, если они не изменены приставкой *размера,* рядовые аргументы принуждаются `int` к `double`типу, а аргументы плавающей точки принуждаются к . В 64-разрядных системах тип `int` является 32-разрядным значением; поэтому 64-разрядные целые числа будут усекаться при форматировании для вывода, если не используется префикс *size* со значением **ll** или **I64**. Для типов указателей, которые указываются с помощью символа**p**, используется размер указателя по умолчанию для платформы.

> [!NOTE]
> **Microsoft-специфически:** Символ типа **«я»** и поведение символов **c,** **C,** **s**и S `printf` типа `wprintf` **S,** когда они используются с функциями и функциями, являются расширениями Майкрософт. C стандарта ISO постоянно использует **c** и **s** для узких символов и строк, и **C** и **S** — для расширенных символов и строк во всех функциях форматирования.
...
|**Я**|Целое число|Десятичное целое число со знаком.|
...
|**Г**|С плавающей запятой|Значения со знаком отображаются в формате **f** или **e** в зависимости от того, какой формат является более компактным для заданного значения и точности. Формат **e** используется только в том случае, когда порядок значения меньше –4 или больше или равен значению аргумента *precision*. Нули в конце отбрасываются, а десятичная запятая отображается только в том случае, если за ней следует хотя бы одна цифра.|
|**Г**|С плавающей запятой|Аналогичен формату **g**, за исключением того, что вместо**e** порядок величины представляет символ **E** (где это необходимо).|
|**a**|С плавающей запятой|Подписано гексадецмическое двухточное плавающее значение, которое имеет форму «0x*h.hhhh*__p»__*dd*, где *h.hhhhh* являются шестиугольными цифрами (с использованием нижних букв случая) мантиссы, и *dd* являются одной или более цифрами для экспонента. Точность определяет количество цифр после запятой.|
|**A**|С плавающей запятой|Подписано гексадецмальным двухточным плавающим значением, которое имеет форму «0X*h.hhhh*__P»__*dd*, где *h.hhhhh* являются шеститочными цифрами (с использованием заглавных букв) мантисы, а *dd* являются одной или более цифрами для экспонента. Точность определяет количество цифр после запятой.|
...
||Когда он используется с **e,** **E,** **f,** **F**, **a**, или **Формат А,** **#** флаг заставляет значение вывода содержать десятичную точку.|Десятичный разделитель появляется, только если после него есть цифры.|
...
В спецификации преобразования необязательное поле спецификации ширины отображается после любых символов *флага*. Аргумент *width* — неотрицательное целое десятичное число, управляющее минимальным количеством выводимых символов. Если число символов в выходном значении меньше указанной ширины, пробелы добавляются влево или вправо от значений, в зависимости от того, указан ли флаг левого выравнивания ()**-** до достижения минимальной ширины. Если параметр *width* имеет префикс 0, при преобразовании к целому числу или числу с плавающей запятой добавляются начальные нули до тех пор, пока не будет достигнута минимальная ширина, кроме случаев преобразования в бесконечность или NaN (не число).
...
В отличие от спецификации ширины, спецификация точности может вызывать либо усечение выходного значения, либо округление значения с плавающей запятой. Если *точность* указана как 0, а значение, подаваемые к преобразованию, 0, результатом является отсутствие вывода символов, как показано в этом примере:
...
|----------|-------------|-------------|
|**a**, **A**|Точность определяет количество цифр после запятой.|Точность по умолчанию — 13. Если точность 0, ни одна десятичная **#** точка не печатается, если флаг не используется.|
...
Спецификатор типа **hc** или **hC** аналогичен **c** в функциях `printf` и **C** в функциях `wprintf`. **LC**, **lC**, **wc**, или **wC** тип спецификатор является `printf` синонимом **C** `wprintf` в функциях и **с** в функциях. Спецификатор типа **hs** или **hS** аналогичен **s** в функциях `printf` и **S** в функциях `wprintf`. Спецификатор типа **ls**, **lS**, **ws** или **wS** аналогичен **S** в функциях `printf` и **s** в функциях `wprintf`.

> [!NOTE]
> **Microsoft-специфически:** **I** (uppercase i), **I32**, **I64**, и **W** аргумент размер модификатора префиксы Microsoft расширений и не ISO C-совместимы. Префикс **h** при использовании с данными типа `char` и префикс **l** (строчная L) при использовании с данными типа `double` являются расширениями Майкрософт.

## <a name="see-also"></a>См. также раздел

[printf, _printf_l, wprintf, _wprintf_l](../c-runtime-library/reference/printf-printf-l-wprintf-wprintf-l.md)<br/>
[printf_s, _printf_s_l, wprintf_s, _wprintf_s_l](../c-runtime-library/reference/printf-s-printf-s-l-wprintf-s-wprintf-s-l.md)<br/>
[printf_p позиционные параметры](../c-runtime-library/printf-p-positional-parameters.md)
