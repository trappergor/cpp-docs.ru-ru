---
title: "Стандартные типы | Документация Майкрософт"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
|----------|-------------------------------|  
|int8_t, uint8_t|char со знаком, char без знака|  
|int16_t, int16_t|short, short без знака|  
|int32_t, uint32_t|int, int без знака|  
|int64_t, int64_t|long long, long long без знака|  
...
|`_EXCEPTION_POINTERS`|Содержит запись исключения. Дополнительные сведения см. в разделе [EXCEPTION_POINTERS](http://msdn.microsoft.com/library/windows/desktop/ms679331).|FPIEEE.H|  
...
|`size_t` (__int64 без знака или целое число без знака, в зависимости от целевой платформы)|Результат выполнения оператора `sizeof`.|CRTDEFS.H и другое, включая файлы|  
...
|`uintptr_t` (длинное целое или `__int64`, в зависимости от целевой платформы)|Целое число без знака или версия без знака __int64 от `intptr_t`.|STDDEF.H и другое, включая файлы|  
