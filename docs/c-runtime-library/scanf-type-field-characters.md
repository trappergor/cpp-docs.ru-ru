...
|`x`|Шестнадцатеричное целое число.|Указатель на `int`.|Номер|  
|`e`, `E`, `f`, `F`, `g`, `G`|Число с плавающей запятой, состоящее из необязательного знака (+ или –), последовательности из одной или нескольких десятичных цифр, содержащей десятичную запятую, и необязательной экспоненты ("e" или "E"), за которой следует целое число со знаком (необязательным).|Указатель на `float`.|Номер|  
