...
|[time, _time32, _time64](../c-runtime-library/reference/time-time32-time64.md)|Получают текущее системное время как значение типа `time_t`, `__time32_t` или `__time64_t`.|  
|[_tzset](../c-runtime-library/reference/tzset.md)|Задает значения внешних переменных времени но основе значения переменной времени среды `TZ`.|  
