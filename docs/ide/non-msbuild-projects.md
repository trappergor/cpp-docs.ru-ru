...
|Launch.VS.JSON|Укажите аргументы командной строки. Доступ к файлу можно получить в **обозревателе решений**, воспользовавшись пунктом контекстного меню **Параметры отладки и запуска**.|
|Tasks.VS.JSON|Укажите настраиваемого построения команд и параметров компилятора. Доступ к файлу можно получить в **обозревателе решений**, воспользовавшись пунктом контекстного меню **Настройка задач**.|
...
Используйте `output` свойство, чтобы указать исполняемый объект, который будет запущен при нажатии клавиши **F5**. Например:
...
|||
|-|-|
|`${env.<VARIABLE>}`| Указывает любых переменных (например, ${env. ПУТЬ} ${env.COMSPEC} и т. д), заданным для командной строки разработчика. Дополнительные сведения см. в статье [Командная строка разработчика для Visual Studio](/dotnet/framework/tools/developer-command-prompt-for-vs).|
