---
title: Стандартные макросы для команд и свойств сборки | Документы Майкрософт
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-ide
...
В зависимости от параметров установки Visual Studio может предоставлять вам сотни макросов. Они соответствуют свойствам MSBuild, заданным по умолчанию, указанным в файлах PROPS или TARGETS либо в параметрах проекта. Эти макросы можно использовать в любом месте диалогового окна **Страницы свойств** проекта, где допустимы строки. В этих макросах регистр не учитывается.  
 ...
 В следующей таблице описано несколько распространенных доступных макросов. Этот список не является исчерпывающим. Дополнительные сведения о том, как определения свойств MSBuild создаются и используются как макросы в файлах VCXPROJ, PROPS и TARGETS, см. в разделе [Свойства MSBuild](/visualstudio/msbuild/msbuild-properties).  
