---
title: 'Обмен полями записей: Принцип работы RFX | Документация Майкрософт'
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-data
...
2.  `AddNew` или `Edit` подготавливает поля в буфере редактирования, чтобы RFX мог обнаружить элементы измененных данных полей.  
...
3.  Вы непосредственно задать значения элементов данных полей, значения которых требуется изменить или заполнить для новой записи. Это могут быть вызовом `SetFieldNull`.  
...
5.  Для `AddNew`, `Update` завершается путем восстановления ранее сохраненные значения записи, которая была текущей до `AddNew` вызова. Для `Edit`, новые, измененные значения остаются на месте.  
