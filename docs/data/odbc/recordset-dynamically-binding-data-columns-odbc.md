...
> [!NOTE]
>  Этот раздел относится к объектам, производным от `CRecordset`, в которых пакетное получение строк не реализовано. Если вы используете пакетное получение строк, применять описанные в ней методы, как правило, не рекомендуется. Дополнительные сведения о пакетном получении строк см. в статье [Набор записей. Пакетное получение строк (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md).

##  <a name="_core_when_you_might_bind_columns_dynamically"></a> Ситуации, в которых применяется динамическая привязка столбцов
...
###  <a name="_core_determining_which_table_columns_are_not_in_your_recordset"></a> Определение столбцов таблицы, отсутствующих в наборе записей
...
1. Вместе со списком "Столбцы для динамической привязки" создайте список "Значения из динамических столбцов", в котором будут содержаться значения из каждого столбца.
   ...
   Например, на рисунке показан список "Значения из динамических столбцов" (список 4) с одним элементом — объектом `CString`, содержащим номер телефона из текущей записи: 555-1212.
...
Результатом выполнения предыдущих процедур являются два основных списка: "Столбцы для динамической привязки", содержащий имена столбцов, и "Значения из динамических столбцов", содержащий значения столбцов для текущей записи.

> [!TIP]
> Если новые столбцы имеют разные типы данных, может потребоваться дополнительный список, каждый элемент которого определяет тип соответствующего элемента в списке столбцов. (Для него можно использовать значения AFX_RFX_BOOL, AFX_RFX_BYTE и т. д. Эти константы определены в файле AFXDB.H.) Выбор типа списка зависит от способа представления типов столбцов.

###  <a name="_core_adding_rfx_calls_to_bind_the_columns"></a> Добавление вызовов RFX для привязки столбцов
