...
Чтобы реализовать собственный метод FlushData, вам потребуется:
...
Это хороший реализация FlushData сохраняет только те строки и столбцы, которые необходимо обновить. Параметры HROW и HACCESSOR можно использовать для определения текущей строки и столбца, сохраняемые для оптимизации.
...
В следующем примере показан как `FlushData` реализуется в классе RUpdateRowset в поддерживающего (см. в разделе Rowset.h в образце кода):
...
Значения по умолчанию FlushData и Execute, будет возвращать S_OK. Таким образом, если эта функция не переопределяется, изменения отображаются для успешного выполнения (будет возвращено значение S_OK), но они не передаются в хранилище данных.

В образце UpdatePV (в файле Rowset.h) `SetDBStatus` метод обрабатывает значения по умолчанию следующим образом:
...
Например, в `CUpdateSessionColSchemaRowset` класс в этом примере (в Session.h), первый столбец заданы следующим образом:
