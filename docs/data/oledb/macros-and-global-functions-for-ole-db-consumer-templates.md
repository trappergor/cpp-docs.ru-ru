...
*бауто*<br/>
окне Указывает, является ли этот метод доступа автоматическим методом доступа или ручным методом доступа. Если **значение равно true**, метод доступа является автоматическим; Если **значение равно false**, то метод доступа является ручным. Автоматический метод доступа означает, что данные извлекаться для вас при операциях перемещения.
...
*pszName*<br/>
окне Указатель на имя столбца. Имя должно быть строкой в Юникоде. Это можно сделать, поместив "L" перед именем, например: `L"MyColumn"`.
...
*нпреЦисион*<br/>
окне Максимальная точность, используемая при получении данных, *wType* `DBTYPE_NUMERIC`. В противном случае этот параметр игнорируется.

*нскале*<br/>
окне Масштаб, используемый при получении данных и *wType* , `DBTYPE_NUMERIC` или `DBTYPE_DECIMAL`.
...
Поставщики поддерживают только входные и выходные типы параметров, поддерживаемые в базовом источнике данных. Тип является сочетанием одного или нескольких значений `DBPARAMIO` (см. раздел [структуры DBBINDING](/previous-versions/windows/desktop/ms716845(v=vs.85)) в *справочнике по программисту OLE DB*):

- `DBPARAMIO_NOTPARAM` метод доступа не имеет параметров. Как правило, `eParamIO` это значение задается в параметрах доступа к строкам, чтобы напомнить пользователю, что параметры игнорируются.
