...
Затем запустите `GetColumnInfo` функции в файле CustomRS.cpp следующим образом:
...
Укажите статический массив, содержащий сведения о столбце. Если потребитель не столбец закладки, теряется запись в массиве. Вы можете динамически выделить этот массив, но необходимо обязательно уничтожить его должным образом. В этом примере определяет и использует макросы ADD_COLUMN_ENTRY и ADD_COLUMN_ENTRY_EX используется для вставки данных в массив. Макросы можно добавить в файл CustomRS.H, как показано в следующем коде:
