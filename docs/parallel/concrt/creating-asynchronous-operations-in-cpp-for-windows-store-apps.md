...
Методы `getPrimesCancellation` и `cancelGetPrimes` работают вместе, позволяя пользователю отменить операцию. Когда пользователь выбирает **отменить** кнопки, `cancelGetPrimes` вызовы методов [IAsyncOperationWithProgress\<TResult, TProgress >:: Cancel](/uwp/api/windows.foundation.iasyncinfo.cancel) для отмены операции. Среда выполнения с параллелизмом, которая управляет базовой асинхронной операцией, создает тип внутреннего исключения, перехваченные средой выполнения Windows для связи, что Отмена завершена. Дополнительные сведения о модели отмены см. в разделе [отмены](../../parallel/concrt/cancellation-in-the-ppl.md).
