...
- [Не выпараллелизации небольшие тела циклов](#small-loops)

- [Экспресс параллелизма на максимально возможном уровне](#highest)

- [Использование parallel_invoke для решения проблем деления и властвуй](#divide-and-conquer)

- [Использование отмены или обработки исключений для прерывания из параллельного цикла](#breaking-loops)

- [Узнайте, как отмена и обработка исключений влияют на уничтожение объектов.](#object-destruction)

- [Не блокируйте несколько раз в параллельном цикле](#repeated-blocking)

- [Не выполнять блокирующие операции при отмене параллельной работы](#blocking)

- [Не записывать в общие данные в параллельном цикле](#shared-writes)

- [По возможности избегайте ложного совместного доступа](#false-sharing)

- [Убедитесь, что переменные допустимы в течение всего времени существования задачи.](#lifetime)

## <a name="small-loops"></a>Не выпараллелизации небольшие тела циклов
...
## <a name="highest"></a>Экспресс параллелизма на максимально возможном уровне
...
Чтобы уменьшить объем затрат на планирование в этом примере, можно распараллелить внешние циклы перед внутренними или использовать другие параллельные конструкции, например конвейер. В следующем примере функция `ProcessImages` изменяется, чтобы использовать алгоритм [параллелизма::p arallel_for_each](reference/concurrency-namespace-functions.md#parallel_for_each) для параллелизации внешнего цикла.
...
## <a name="divide-and-conquer"></a>Использование parallel_invoke для решения проблем деления и властвуй
...
Полную версию этого примера см. в разделе [руководство. использование parallel_invoke для написания параллельной подпрограммы сортировки](../../parallel/concrt/how-to-use-parallel-invoke-to-write-a-parallel-sort-routine.md). Дополнительные сведения о алгоритме `parallel_invoke` см. в разделе [Параллельные алгоритмы](../../parallel/concrt/parallel-algorithms.md).

[[В начало](#top)]

## <a name="breaking-loops"></a>Использование отмены или обработки исключений для прерывания из параллельного цикла
...
Хотя обработка исключений является менее эффективным способом отмены параллельной работы, чем механизм отмены, существуют случаи, в которых лучше применять обработку исключений. Например, следующий метод, `for_all`, рекурсивно выполняет рабочую функцию для каждого узла структуры `tree`. В этом примере элемент данных `_children` — это [std:: List](../../standard-library/list-class.md) , содержащий объекты `tree`.
...
## <a name="object-destruction"></a>Узнайте, как отмена и обработка исключений влияют на уничтожение объектов.
...
Несмотря на то что эта схема сама по себе не представляет никаких проблем, рассмотрим следующий код, выполняющий две задачи параллельно. Первая задача создает объект `Container`, а вторая задача отменяет общую задачу. Для иллюстрации пример использует два объекта [Concurrency:: Event](../../parallel/concrt/reference/event-class.md) , чтобы убедиться, что отмена происходит после создания объекта `Container` и что объект `Container` уничтожается после выполнения операции отмены.
...
## <a name="blocking"></a>Не выполнять блокирующие операции при отмене параллельной работы
...
Рассмотрим следующий пример, определяющий функцию `parallel_find_answer`, которая выполняет поиск элемента указанного массива, удовлетворяющего заданной предикативной функции. Если функция предиката возвращает **значение true**, то параллельная рабочая функция создает объект `Answer` и отменяет общую задачу.
...
## <a name="shared-writes"></a>Не записывать в общие данные в параллельном цикле
...
Рассмотрим следующий пример, использующий алгоритм [Concurrency::p arallel_for_each](reference/concurrency-namespace-functions.md#parallel_for_each) и объект `critical_section` для вычисления числа простых чисел в объекте [std:: Array](../../standard-library/array-class-stl.md) . Этот пример нельзя масштабировать, так как каждый поток должен ждать доступа к общей переменной `prime_sum`.
...
Полную версию этого примера см. в разделе [как использовать комбинирование для повышения производительности](../../parallel/concrt/how-to-use-combinable-to-improve-performance.md). Дополнительные сведения о классе `combinable` см. в разделе [Parallel Containers and Objects](../../parallel/concrt/parallel-containers-and-objects.md).
...
В следующем примере используется метод [Concurrency:: task_group:: wait](reference/task-group-class.md#wait) , чтобы убедиться, что задача завершается перед возвратом функции `perform_action`.
