...
1. Щелкните правой кнопкой мыши узел проекта в **Обозреватель решений**и выберите пункт **свойства**. В разделе **Свойства** > конфигурации**CC++/**  > предкомпилированные**заголовки** > предкомпилированный**заголовок** выберите **создать**.
...
1. В диалоговом окне **Новый проект** выберите визуальный **C++** узел на панели **типы проектов** , а затем в области **шаблоны** выберите **консольное приложение Win32** . Введите имя проекта, `BasicAgent`например, и нажмите кнопку **ОК** , чтобы открыть **Мастер консольного приложения Win32**.
...
[!code-cpp[concrt-basic-agent#3](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-an-agent-based-application_4.h)]
   ...
   `_file_name` Элемент — это имя файла, из которого агент считывает данные. Элемент является объектом [Concurrency:: ITarget](../../parallel/concrt/reference/itarget-class.md) , в который агент записывает содержимое файла. `_target` `_error` Элемент содержит все ошибки, происходящие во время существования агента.

1. Добавьте следующий код для `file_reader` конструкторов `public` в раздел `file_reader` класса.
...
1. Добавьте метод в общедоступный раздел `file_reader` класса. `get_error`
...
1. В функции создайте объект [Concurrency:: Event](../../parallel/concrt/reference/event-class.md) , который сигнализирует об окончании обработки. `_tmain`

[!code-cpp[concrt-basic-agent#10](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-an-agent-based-application_11.cpp)]

1. `call` Создайте объект, который обновляет контрольную сумму при получении данных.

[!code-cpp[concrt-basic-agent#11](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-an-agent-based-application_12.cpp)]

   Этот `call` объект также `event` задает объект при получении пустой строки для сигнализации об окончании обработки.

1. Создайте объект, который считывает данные из файла Test. txt и записывает содержимое этого файла `call` в объект. `file_reader`
...
## <a name="sample-input"></a>Пример входных данных
...
Чтобы предотвратить одновременный доступ к элементам данных, рекомендуется добавить методы, выполняющие работу `protected` , в раздел или `private` класса. Добавляйте только методы, которые отправляют или получают сообщения от агента в `public` раздел класса.
...
Еще один пример приложения на основе агента см. в разделе [пошаговое руководство. Использование функции Join для предотвращения](../../parallel/concrt/walkthrough-using-join-to-prevent-deadlock.md)взаимоблокировок.
