...
Модель программирования OpenMP определяется открытым стандартом и обладает четко определенными привязками к языкам программирования Fortran и C/C++. OpenMP версий 2.0 и 2.5, в котором поддерживаются корпорацией Майкрософт C++ компилятора, поддерживают, хорошо подходит для параллельных алгоритмов, которые являются итеративными; то есть они выполняют параллельные итерации по массиву данных. OpenMP 3.0 поддерживает не только итеративные задачи в дополнение к итеративной задачи.
