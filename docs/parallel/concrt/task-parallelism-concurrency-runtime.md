...
- [Сравнение продолжений на основе значений и задач](#value-versus-task)
...
- [Выполнение отложенных задач](#delayed-tasks)

- [Группы задач](#task-groups)

- [Сравнение task_group с structured_task_group](#comparing-groups)

- [Пример](#example)

- [Надежное программирование](#robust)

## <a name="lambdas"></a>Использование лямбда-выражений
...
При использовании функции [Concurrency:: create_task](reference/concurrency-namespace-functions.md#create_task) можно использовать ключевое слово `auto` вместо объявления типа. Например, рассмотрим следующий код, который создает и печатает матрицу тождественности.
...
- Определять подсказки, как должно планироваться продолжение. (Это относится только к приложениям универсальная платформа Windows (UWP). Дополнительные сведения см. [в статье Создание асинхронных C++ операций в для приложений UWP](../../parallel/concrt/creating-asynchronous-operations-in-cpp-for-windows-store-apps.md).
...
## <a name="value-versus-task"></a>Сравнение продолжений на основе значений и задач

Принимая во внимание объект `task`, который имеет возвращаемый тип `T`, вы можете предоставить значение типа `T` или `task<T>` в соответствующие задачи продолжения. Продолжение, принимающее тип `T`, называется *продолжением на основе значения*. Продолжение на основе значения планируется для выполнения, когда предшествующая задача завершается без ошибок и не отменяется. Продолжение, принимающее тип `task<T>` в качестве своего параметра, называется *продолжением задачи*. Продолжение на основе задачи всегда планируется для выполнения после завершения предшествующей задачи, даже если предшествующая задача отменяется или создает исключение. Затем можно вызвать `task::get`, чтобы получить результат предшествующей задачи. Если предшествующая задача была отменена, `task::get` выдает [Concurrency:: task_canceled](../../parallel/concrt/reference/task-canceled-class.md). Если предшествующая задача выдала исключение, `task::get` повторно выдает это исключение. Продолжение на основе задачи не отмечается как отмененное, когда отменяется предшествующая задача.
...
> [!TIP]
> Как и в случае с `when_all`, `when_any` не блокируется и может быть защищена от вызова в приложении UWP в потоке ASTA.

## <a name="delayed-tasks"></a>Выполнение отложенных задач
...
> [!IMPORTANT]
> PPL также определяет алгоритм [параллелизма::p arallel_invoke](reference/concurrency-namespace-functions.md#parallel_invoke) , который использует класс `structured_task_group` для параллельного выполнения набора задач. Поскольку алгоритм `parallel_invoke` имеет более лаконичный синтаксис, рекомендуется по возможности использовать его вместо класса `structured_task_group`. В разделе [Параллельные алгоритмы](../../parallel/concrt/parallel-algorithms.md) описываются `parallel_invoke` более подробно.
...
## <a name="comparing-groups"></a>Сравнение task_group с structured_task_group
...
Можно выполнить дополнительные задачи для `task_group` объекта после вызова метода [Concurrency:: task_group:: wait](reference/task-group-class.md#wait) или [concurrency:: task_group:: run_and_wait](reference/task-group-class.md#run_and_wait) . И наоборот, если при выполнении дополнительных задач для `structured_task_group` объекта после вызова методов [Concurrency:: structured_task_group:: wait](reference/structured-task-group-class.md#wait) или [concurrency:: structured_task_group:: run_and_wait](reference/structured-task-group-class.md#run_and_wait) поведение не определено.
...
Полные примеры, демонстрирующие использование алгоритма `parallel_invoke`, см. в разделе [как использовать parallel_invoke для написания параллельной подпрограммы сортировки](../../parallel/concrt/how-to-use-parallel-invoke-to-write-a-parallel-sort-routine.md) и [как использовать parallel_invoke для выполнения параллельных операций](../../parallel/concrt/how-to-use-parallel-invoke-to-execute-parallel-operations.md). Полный пример использования класса `task_group` для реализации асинхронных фьючерсов см. в разделе [Пошаговое руководство. Реализация фьючерсов](../../parallel/concrt/walkthrough-implementing-futures.md).
...
|[Практическое руководство. Создание задачи, выполняемой после задержки](../../parallel/concrt/how-to-create-a-task-that-completes-after-a-delay.md)|Показывает, как использовать классы `task`, `cancellation_token_source`, `cancellation_token`и `task_completion_event` для создания задачи, которая завершается после некоторой задержки.|
