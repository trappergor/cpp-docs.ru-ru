...
2.  Все изображения, красный базового цвета удалите зеленый и синий компоненты и затем затемнить его.  
  
3.  Для любого другого изображения примените тонирования выцветшей фотографии.  
...
2.  В `ProcessImages` функцию, создайте `countdown_event` переменной. `countdown_event` Класс показан далее в этом пошаговом руководстве.  
...
6.  Добавьте следующий код, чтобы отправить в начало сети полный путь к каждой JPEG-файл в каталоге.  
