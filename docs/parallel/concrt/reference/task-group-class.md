`task_group` Класс представляет коллекцию параллельной работы, для которой возможно ожидание или Отмена.  
...
|[Ожидание](#wait)|Ожидает, пока не работают на `task_group` объекта завершена или отменена.|  
