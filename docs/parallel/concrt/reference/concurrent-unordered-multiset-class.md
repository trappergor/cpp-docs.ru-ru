...
|Имя|Описание|  
|----------|-----------------|  
|[concurrent_unordered_multiset](#ctor)|Перегружен. Создает параллельного неупорядоченного мультинабора.|  
...
|[insert](#insert)|Перегружен. Добавляет элементы в `concurrent_unordered_multiset` объекта.|  
|[key_eq](#key_eq)|Объект функции сравнения хранимых равенства.|  
...
|Имя|Описание|  
|----------|-----------------|  
|[operator=](#operator_eq)|Перегружен. Назначает содержимое другого `concurrent_unordered_multiset` этого объекта. Этот метод не является безопасным в режиме параллелизма.|  
