...
---
# <a name="concurrency-namespace-functions"></a>пространство имен функции параллелизма
||||  
|-|-|-|  
|[Alloc](#alloc)|[CreateResourceManager](#createresourcemanager)|[DisableTracing](#disabletracing)|  
|[EnableTracing](#enabletracing)|[Бесплатная](#free)|[GetExecutionContextId](#getexecutioncontextid)|  
...
|[parallel_sort](#parallel_sort)|[parallel_transform](#parallel_transform)|[Получение](#receive)|  
|[run_with_cancellation_token](#run_with_cancellation_token)|[Отправить](#send)|[set_ambient_scheduler](#set_ambient_scheduler)|  
...
### <a name="remarks"></a>Примечания  
 Дополнительные сведения о том, какие сценарии в приложении выгодно использовать подраспределитель кэширования см. в разделе [планировщик](../../../parallel/concrt/task-scheduler-concurrency-runtime.md).  
...
### <a name="parameters"></a>Параметры  
 ...
 `_Trg`  
 Указатель или ссылка на целевой объект, на который отправляются данные.  
  
 `_Data`  
 Ссылка для отправки данных.  
  
### <a name="return-value"></a>Возвращаемое значение  
 `true`Если сообщение было принято до возврата методом, `false` в противном случае.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Асинхронная конструкция, представленная Asyncaction ^, IAsyncActionWithProgress\<TProgress настроек ^, IAsyncOperation\<TResult настроек ^, или IAsyncOperationWithProgress\<TResult, TProgress настроек ^. Возвращаемый интерфейс зависит от сигнатуры лямбда-выражения, переданного функции.  
...
### <a name="remarks"></a>Примечания  
 Несколько последующие вызовы этого метода будет возвращать один и тот же экземпляр диспетчера ресурсов. Каждый вызов метода увеличивает ссылку подсчета диспетчера ресурсов и должен совпадать с помощью вызова [IResourceManager::Release](http://msdn.microsoft.com/en-us/5d1356ec-fbd3-4284-a361-1e9e20bbb522) метод, когда планировщик завершает взаимодействует с диспетчером ресурсов.  
  
 [unsupported_os](unsupported-os-class.md) создается, если операционная система не поддерживается средой выполнения с параллелизмом.  
  
##  <a name="a-namecreatetaska--createtask"></a><a name="create_task"></a>create_task  
 Создает PPL [задачи](http://msdn.microsoft.com/en-us/5389e8a5-5038-40b6-844a-55e9b58ad35f) объекта. Функция `create_task` может использоваться в любой ситуации, где бы вы использовали конструктор задач. Она предоставлена главным образом для удобства, поскольку позволяет использовать ключевое слово `auto` при создании задач.  
...
### <a name="parameters"></a>Параметры  
 ...
 `_ReturnType`  
 `_Param`  
 Параметр, из которого будет создаваться задача. Это может быть лямбда-выражения или объекта функции, `task_completion_event` объект, другой `task` объект или интерфейс Windows::Foundation:: iasyncinfo, если вы используете задачи в приложении магазина Windows.  
...
### <a name="remarks"></a>Примечания  
 Первая перегрузка ведет себя как задача конструктор, который принимает один параметр.  
  
 Вторая перегрузка связывает токен отмены, в состав свежесозданной задаче. При использовании этой перегрузки недопустимы для передачи в другом `task` объект в качестве первого параметра.  
  
 Тип возвращаемой задачи выводится из первого параметра функции. Если `_Param` — `task_completion_event<T>`, `task<T>`, или функтор, который возвращает либо тип `T` или `task<T>`, тип созданной задачи — `task<T>`.  
  
 В приложении для магазина Windows Если `_Param` типа Windows::Foundation:: iasyncoperation\<T настроек ^ или Windows::Foundation:: iasyncoperationwithprogress\<T, P настроек ^, или функтор, который возвращает один из этих типов, созданная задача будет иметь тип `task<T>`. Если `_Param` типа Windows::Foundation:: iasyncaction ^ или Windows::Foundation:: iasyncactionwithprogress\<P настроек ^, или функтор, который возвращает один из этих типов, созданная задача будет иметь тип `task<void>`.  
...
##  <a name="a-namefreea--free"></a><a name="free"></a>Бесплатная  
...
### <a name="parameters"></a>Параметры  
 `_PAllocation`  
 Указатель на память, выделенную ранее `Alloc` метод, который должен освобождаться. Если параметр `_PAllocation` присвоено значение `NULL`, этот метод будет игнорировать ее и немедленный возврат.  
...
### <a name="remarks"></a>Примечания  
 Этот метод используется для получения идентификатора для контекста выполнения, прежде чем передать `IExecutionContext` интерфейс в качестве параметра во все методы, предлагаемые диспетчером ресурсов.  
...
### <a name="remarks"></a>Примечания  
 Этот метод используется для получения идентификатора для планировщика, прежде чем передать `IScheduler` интерфейс в качестве параметра во все методы, предлагаемые диспетчером ресурсов.  
...
### <a name="return-value"></a>Возвращаемое значение  
 `true`Если группа задач, которые в настоящее время выполняется, отменяется, `false` в противном случае.  
...
### <a name="remarks"></a>Примечания  
 Эта функция полезна, если необходимо создать `task_handle` объекта лямбда-выражением, поскольку он позволяет создать объект, не зная истинный тип лямбда-функтора.  
...
### <a name="parameters"></a>Параметры  
 ...
 `_Allocator`  
 Тип распределителя памяти совместимые стандартной библиотеки C++.  
 ...
 `_Alloc`  
 Экземпляр распределителя памяти совместимые стандартной библиотеки C++.  
...
### <a name="remarks"></a>Примечания  
 Все перегрузки требуют `n * sizeof(T)` дополнительное пространство, где `n` — количество элементов для сортировки и `T` является типом элемента. В большинстве случаев parallel_buffered_sort покажет повышение производительности по [parallel_sort](concurrency-namespace-functions.md), и при наличии доступной памяти следует использовать через parallel_sort.  
  
 Если не указать бинарное средство сравнения `std::less` используется по умолчанию и требует, чтобы предоставить оператор тип элемента `operator<()`.  
  
 Если не указать тип распределителя или экземпляра, распределитель памяти библиотеки Standard C++ `std::allocator<T>` используется для выделения буфера.  
...
### <a name="parameters"></a>Параметры  
 ...
 `_Partitioner`  
 Тип разделителя, который используется для разделения указанного диапазона.  
 ...
 `_Step`  
 Значение шага при переборе от `first` в `last`. Этап должно быть положительным. [invalid_argument](../../../standard-library/invalid-argument-class.md) возникает, если шаг меньше 1.  
  
 `_Func`  
 Функция, выполняемая в каждой итерации. Это может быть лямбда-выражение, указатель на функцию или другой объект, который поддерживает версию оператора вызова функции с сигнатурой `void operator()(``_Index_type``)`.  
  
 `_Part`  
 Ссылка на объект-разделитель. Аргумент может принимать одно из `const` [auto_partitioner](auto-partitioner-class.md)`&`, `const` [static_partitioner](static-partitioner-class.md)`&`, `const` [simple_partitioner](simple-partitioner-class.md) `&` или [affinity_partitioner](affinity-partitioner-class.md) `&` Если [affinity_partitioner](affinity-partitioner-class.md) используется объект, ссылка должен быть ссылкой неконстантной l значением, чтобы алгоритм мог сохранять состояние для повторного использования в последующих циклах.  
...
### <a name="parameters"></a>Параметры  
 ...
 `last`  
 Итератор, обращающийся к позиции после последнего элемента для включения в параллельных итерации.  
...
### <a name="remarks"></a>Примечания  
 [auto_partitioner](auto-partitioner-class.md) будет использоваться для перегрузки без явного разделитель.  
...
### <a name="remarks"></a>Примечания  
 Обратите внимание, что один или несколько объектов функции, предоставленных в качестве параметров может выполняться встроено в контекст вызова.  
  
 Если один или несколько объектов функции, передаваемые в качестве параметров эта функция создает исключение, среда выполнения будет выбрать одно такое исключение и распространить из вызова `parallel_invoke`.  
...
### <a name="parameters"></a>Параметры  
 ...
 `_Proj_func`  
 Объект проекции, определяемой пользователем функции, который преобразует элемент в целочисленное значение.  
  
 `_Chunk_size`  
 Минимальный размер блока, который будет разбит на два для параллельного выполнения.  
  
### <a name="remarks"></a>Примечания  
 Все перегрузки требуют `n * sizeof(T)` дополнительное пространство, где `n` — количество элементов для сортировки и `T` является типом элемента. Унарного функтора проекции с подписью `I _Proj_func(T)` должен возвращать ключ для заданного элемента, где `T` является типом элемента и `I` является тип как целое число без знака.  
  
 Если не указать функции проекции, функции проекции по умолчанию, который просто возвращает элемент используется для целочисленных типов. Функция не компилируются, если элемент не целочисленный тип в отсутствие функции проекции.  
...
##  <a name="a-namereceivea--receive"></a><a name="receive"></a>Получение  
...
### <a name="parameters"></a>Параметры  
 ...
 `_Filter_proc`  
 Функция фильтрации, которая определяет, должны ли приниматься сообщения.  
  
### <a name="return-value"></a>Возвращаемое значение  
 Значение из источника типа полезной нагрузки.  
  
### <a name="remarks"></a>Примечания  
 Если параметр `_Timeout` имеет значение, отличное от константы `COOPERATIVE_TIMEOUT_INFINITE`, исключение [operation_timed_out](operation-timed-out-class.md) создается, если истечения указанного периода времени до получения сообщения. Если необходимо, чтобы время ожидания нулевой длины, следует использовать [try_receive](concurrency-namespace-functions.md) функции, в отличие от вызова `receive` с временем ожидания `0` (нуль), как он является более эффективным и не создает исключения времени ожидания.  
...
### <a name="remarks"></a>Примечания  
 Метод создает исключение [invalid_operation](invalid-operation-class.md) исключение, если присутствует диспетчер ресурсов во время его вызова и [invalid_argument](../../../standard-library/invalid-argument-class.md) исключение, если соответствие вернет пустой набор ресурсов.  
  
 Версию метода, принимающую массив сходств группы в качестве параметра, следует использовать только в операционных системах Windows версии 7 и выше. В противном случае — [invalid_operation](invalid-operation-class.md) исключения.  
...
### <a name="parameters"></a>Параметры  
 ...
 `_A`  
 Параллельный вектор, элементы которого должны быть обмену с элементами параллельного вектора `_B`.  
...
### <a name="remarks"></a>Примечания  
 Функция шаблона является алгоритмом, который специализируется на классе контейнера `concurrent_vector` для выполнения функции-члена `_A`. [concurrent_vector::Swap](concurrent-vector-class.md#swap)( `_B`). Это экземпляры частичного упорядочивания шаблонов функций компилятором. Когда функции-шаблоны перегружаются таким образом, что соответствие шаблона и вызова функции не является уникальным, компилятор выберет наиболее специализированную версию функции-шаблона. Общие версии функции шаблона, `template <class T> void swap(T&, T&)`, в алгоритме класс работает путем назначения и медленной операцией. Специализированная версия в каждом контейнере работает гораздо быстрее, так как она может работать с внутренним представлением класса контейнера.  
  
 Этот метод не является безопасным в режиме параллелизма. Необходимо убедиться, что нет других потоков, выполняющих операции на любых параллельных векторах, при вызове этого метода.  
...
### <a name="parameters"></a>Параметры  
 ...
 `_value`  
 Ссылка на расположение, где будет помещен результат.  
  
 `_Filter_proc`  
 Функция фильтрации, которая определяет, должны ли приниматься сообщения.  
  
### <a name="return-value"></a>Возвращаемое значение  
 Объект `bool` значение, указывающее, будет ли полезная нагрузка в `_value`.  
...
### <a name="remarks"></a>Примечания  
 Если этот метод вызывается на контекст планировщика среды параллелизма, планировщик найдет другой контекст для выполнения на основном ресурсе. Поскольку планировщик предназначен для совместной работы по своей природе, этот контекст не может возобновиться точно после указанного числа миллисекунд. Если планировщик занят выполнением других задач, которые не уступают планировщику совместно, период ожидания может быть не ограничен.  
...
### <a name="remarks"></a>Примечания  
 Функция `when_all` является функцией без блокировки, в качестве результата создающей `task`. В отличие от [task::wait](task-class.md#wait), безопасно вызывать данную функцию [!INCLUDE[win8_appname_long](../../../build/includes/win8_appname_long_md.md)] приложения в потоке ASTA (STA приложения).  
  
 Если одна из задач отменяется или создает исключение, возвращенная задача завершится рано, в отмененном состоянии, и исключение, если таковое встречено, возникнет при вызове [task::get](task-class.md#get) или `task::wait` для этой задачи.  
...
### <a name="remarks"></a>Примечания  
 Функция `when_any` является функцией без блокировки, в качестве результата создающей `task`. В отличие от [task::wait](task-class.md#wait), безопасно вызывать данную функцию [!INCLUDE[win8_appname_long](../../../build/includes/win8_appname_long_md.md)] приложения в потоке ASTA (STA приложения).  
