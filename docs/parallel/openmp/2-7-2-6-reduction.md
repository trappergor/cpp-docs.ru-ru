...
Несмотря на то что правый операнд &#124; &#124; оператор не имеет побочных эффектов в этом примере, они допускаются, но следует использовать с осторожностью. В этом контексте побочный эффект, который гарантированно не происходит во время последовательного выполнения цикла может возникать при параллельном выполнении. Это различие может происходить, поскольку порядок выполнения итерации не определен.
