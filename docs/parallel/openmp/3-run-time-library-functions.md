...
Текущая реализация поддержки из `omp_get_dynamic` и `omp_set_dynamic` выглядит следующим образом: 

Входной параметр, чтобы `omp_set_dynamic` не влияет на политику потоков и не изменяет количество потоков. `omp_get_num_threads` всегда возвращает число определяемых пользователем, если, для которой включено, или номер потока по умолчанию. В текущей реализации Майкрософт `omp_set_dynamic(0)` приводит к отключению динамического работа с потоками, чтобы для следующих параллельной области можно повторно использовать существующий набор потоков. `omp_set_dynamic(1)` Включение динамического работа с потоками, удаляя существующий набор потоков и создание нового набора для предстоящих область параллельной обработки. Число потоков в новом наборе совпадает со значением старого набора и основан на возвращаемое значение `omp_get_num_threads`. Таким образом, для наилучшей производительности используйте `omp_set_dynamic(0)` повторно использовать существующие потоки.
...
Если реализация не реализует динамическую настройку число потоков, эта функция всегда возвращает 0. Дополнительные сведения см. в разделе [omp_set_dynamic](#317-omp_set_dynamic-function).
