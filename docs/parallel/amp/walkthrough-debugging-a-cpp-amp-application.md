---
title: Пошаговое руководство. Отладка C++ приложения amp
ms.date: 04/23/2019
helpviewer_keywords:
...
ms.locfileid: "69631586"
---
# <a name="walkthrough-debugging-a-c-amp-application"></a>Пошаговое руководство. Отладка C++ приложения amp

В этом разделе показано, как выполнить отладку приложения C++ , которое использует ускоренный параллелизмC++ (amp) для использования преимуществ графического процессора. Он использует программу параллельного сокращения, которая суммирует большой массив целых чисел. В данном пошаговом руководстве рассмотрены следующие задачи:
...
- Убедитесь, что номера строк отображаются в текстовом редакторе. Дополнительные сведения см. в разделе [Практическое руководство. Отображение номеров строк в редакторе](/visualstudio/ide/reference/how-to-display-line-numbers-in-the-editor).
...
1. Выберите **консольное приложение Win32**, `AMPMapReduce` введите в поле **имя** , а затем нажмите кнопку **ОК** .
...
8. Откройте Ампмапредуце. cpp и замените его содержимое следующим кодом.
...
10. В **Обозреватель решений**откройте контекстное меню для **ампмапредуце**и выберите пункт **Свойства**.

11. В диалоговом окне **страницы свойств** в разделе **Свойства конфигурации** > выберите **CC++/** предкомпилированные**заголовки**.

12. Для свойства **предкомпилированного заголовка** установите флажок **не использовать предварительно скомпилированные заголовки**, а затем нажмите кнопку **ОК** .

13. В строке меню последовательно выберите **Сборка**  >  **Собрать решение**.

## <a name="debugging-the-cpu-code"></a>Отладка кода ЦП

В этой процедуре вы будете использовать локальный отладчик Windows, чтобы убедиться в правильности кода ЦП в этом приложении. Сегмент кода ЦП в этом приложении, который особенно интересно, является `for` циклом `reduction_sum_gpu_kernel` в функции. Он управляет параллельным снижением на основе дерева, которое выполняется на GPU.
...
2. Закрепите окно **потоков GPU** в нижней части Visual Studio. Нажмите кнопку **развернуть потоковый переключатель** , чтобы отобразить текстовые поля плитки и потока. В окне **потоков GPU** отображается общее число активных и заблокированных потоков GPU, как показано на следующем рисунке.
   ...
   ![Окно потоков GPU с 4 активными потоками](../../parallel/amp/media/campc.png "Окно потоков GPU с 4 активными потоками") <br/>
   Окно "Потоки GPU"
   ...
   В окне **потоков GPU** имеется четыре активных потока GPU, и 28 потоков GPU заблокированы в [tile_barrier:: wait](reference/tile-barrier-class.md#wait) , определенном в строке 21 (`t_idx.barrier.wait();`). Все потоки GPU 32 принадлежат к первой плитке, `tile[0]`. Стрелка указывает на строку, содержащую текущий поток. Чтобы переключиться на другой поток, используйте один из следующих методов.
...
3. Убедитесь, что в списке в левом верхнем углу выбраны **потоки** . На следующем рисунке в окне **Параллельные стеки** показано представление потоков GPU, которые вы видели в окне **потоков GPU** , в стеке вызовов.
   ...
   ![Окно параллельных стеков с 4 активными потоками](../../parallel/amp/media/campd.png "Окно параллельных стеков с 4 активными потоками") <br/>
   Окно "Параллельные стеки"

   32. потоки перешли `_kernel_stub` в лямбда-инструкцию `parallel_for_each` в `sum_kernel_tiled` вызове функции, а затем на функцию, где происходит снижение параллелизма. 28 из 32 потоков продвигаемся к инструкции [tile_barrier:: wait](reference/tile-barrier-class.md#wait) и остается заблокированной в строке 22, тогда как остальные 4 потока остаются активными в `sum_kernel_tiled` функции в строке 30.
   ...
   ![Подсказка для окна параллельных стеков](../../parallel/amp/media/campe.png "Подсказка для окна параллельных стеков") <br/>
...
3. Щелкните заголовок столбца **добавить контрольные значения** , `localIdx`введите, а затем нажмите клавишу **Ввод** .

4. Снова щелкните заголовок столбца **добавить контрольные значения** , `globalIdx`введите, а затем нажмите клавишу **Ввод** .

5. Снова щелкните заголовок столбца **добавить контрольные значения** , `localA[localIdx[0]]`введите, а затем нажмите клавишу **Ввод** .
   ...
   ![Параллельные окно контрольных значений с отсортированными результатами](../../parallel/amp/media/campf.png "Параллельные окно контрольных значений с отсортированными результатами") <br/>
...
3. Выберите символ флага в левой части строки, содержащий четыре активных потока.
   ...
   ![Окно потоков GPU с помеченными потоками](../../parallel/amp/media/campg.png "Окно потоков GPU с помеченными потоками") <br/>
...
4. Если вы хотите сосредоточиться на четырех отмеченных Вами потоках, можно выбрать отображение в окнах **потоки GPU**, параллельное наблюдениеи **Параллельные стеки** только отмеченные потоки.
   ...
   ![Панель инструментов "расположение отладки" с значком "Показывать только помеченные] " (../../parallel/amp/media/camph.png "Панель инструментов \"расположение отладки\" с значком \"Показывать только помеченные") " <br/>
...
Вы можете заморозить (приостановить) и разморозить (возобновить) потоки GPU в окне **потоков GPU** или в окне **контроль параллельных значений** . Вы можете заморозить и разморозить потоки ЦП таким же образом. Дополнительные сведения см. [в разделе как Используйте окно](/visualstudio/debugger/how-to-use-the-threads-window)потоки.
...
3. Откройте контекстное меню активной строки и выберите пункт заморозить.
   ...
   ![Окна потоков GPU] , показывающие замороженные потоки (../../parallel/amp/media/campk.png "Окна потоков GPU") , показывающие замороженные потоки <br/>
...
1. В контекстном меню для одного из потоков в окне **потоки GPU** выберите **Группировать по**, **адрес**.
   ...
   ![Окно потоков GPU с потоками, сгруппированными по адресу](../../parallel/amp/media/campl.png "Окно потоков GPU с потоками, сгруппированными по адресу") <br/>
