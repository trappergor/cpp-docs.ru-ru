...
ms.locfileid: "77126335"
---
# <a name="walkthrough-debugging-a-c-amp-application"></a>Пошаговое руководство. Отладка приложения C++ AMP

В этом разделе показано, как выполнить отладку приложения C++ , которое использует ускоренный параллелизмC++ (amp) для использования преимуществ графического процессора. Он использует программу параллельного сокращения, которая суммирует большой массив целых чисел. В этом пошаговом руководстве описаны следующие задачи:
...
1. В диалоговом окне **страницы свойств** в разделе **Свойства конфигурации**выберите **C/C++**  > **Предкомпилированные заголовки**.
...
В этой процедуре вы будете использовать локальный отладчик Windows, чтобы убедиться в правильности кода ЦП в этом приложении. Сегмент кода ЦП в этом приложении, который особенно интересно, является циклом `for` в функции `reduction_sum_gpu_kernel`. Он управляет параллельным снижением на основе дерева, которое выполняется на GPU.
...
В этом разделе показано, как выполнить отладку кода GPU, который является кодом, содержащимся в функции `sum_kernel_tiled`. Код GPU вычисляет сумму целых чисел для каждого "блока" в параллельном режиме.
...
1. Чтобы открыть окно **потоков GPU** , в строке меню выберите **Отладка** > **Windows** > **потоков GPU**.
...
2. Закрепите окно **потоков GPU** в нижней части Visual Studio. Нажмите кнопку **развернуть потоковый переключатель** , чтобы отобразить текстовые поля плитки и потока. В окне **потоков GPU** отображается общее число активных и заблокированных потоков GPU, как показано на следующем рисунке.
   ...
   В окне **потоков GPU** имеется четыре активных потока GPU и 28 потоков GPU, заблокированных на [tile_barrier:: wait](reference/tile-barrier-class.md#wait) , определенный в примерно строке 21 (`t_idx.barrier.wait();`). Все потоки GPU 32 принадлежат к первой плитке `tile[0]`. Стрелка указывает на строку, содержащую текущий поток. Чтобы переключиться на другой поток, используйте один из следующих методов.
...
1. Чтобы открыть окно **Параллельные стеки** , в строке меню выберите **Отладка** > **Windows** > **Параллельные стеки**.
...
3. Убедитесь, что в списке в левом верхнем углу выбраны **потоки** . На следующем рисунке в окне **Параллельные стеки** показано представление потоков GPU, которые вы видели в окне **потоков GPU** , в стеке вызовов.
   ...
   32. потоки походили от `_kernel_stub` лямбда-оператора в `parallel_for_each` вызове функции, а затем к функции `sum_kernel_tiled`, где происходит снижение параллельности. 28 из 32 потоков продвигаемся к инструкции [tile_barrier:: wait](reference/tile-barrier-class.md#wait) и остается заблокированной в строке 22, тогда как остальные 4 потока остаются активными в функции `sum_kernel_tiled` в строке 30.
...
1. Чтобы открыть окно **"контроль параллельных значений** ", в строке меню выберите **Отладка** > **Windows** > **параллельное наблюдение** > **параллельное наблюдение 1**.
...
5. Снова щелкните заголовок столбца **добавить контрольные значения** , введите `localA[localIdx[0]]`, а затем нажмите клавишу **Ввод** .
   ...
   ![Параллельные окно контрольных значений с отсортированными результатами](../../parallel/amp/media/campf.png "Окно "Контроль параллельных данных" с отсортированными результатами") <br/>
...
6. В правом верхнем углу окна " **контроль параллельных** данных" есть элемент управления фильтра, который можно использовать для фильтрации содержимого с помощью логических выражений. Введите `localA[localIdx[0]] > 20000` в текстовом поле элемент управления фильтр, а затем нажмите клавишу **Ввод** .
   ...
   Теперь окно содержит только потоки, в которых значение `localA[localIdx[0]]` больше 20000. Содержимое по-прежнему сортируется по столбцу `localA[localIdx[0]]`, что является действием сортировки, выполненным ранее.
...
4. Если вы хотите сосредоточиться на четырех отмеченных Вами потоках, можно выбрать отображение в окнах **потоки GPU**, **параллельное наблюдение**и **Параллельные стеки** только отмеченные потоки.
   ...
   ![Панель инструментов "расположение отладки" с значком "Показывать только помеченные"](../../parallel/amp/media/camph.png "Панель инструментов "Место отладки" со значком "Показать только помеченные"") <br/>
...
3. Откройте контекстное меню активной строки и выберите пункт **заморозить**.
   ...
   ![Окна потоков GPU, показывающие замороженные потоки](../../parallel/amp/media/campk.png "Окно "Потоки GPU", в котором показаны замороженные потоки") <br/>
...
4. В строке меню выберите **отладка** > **продолжить** , чтобы следующие четыре потока GPU пропускались после барьера 22, и чтобы достичь точки останова в строке 30. Окно **потоков GPU** показывает, что четыре ранее замороженных потока остаются замороженными и находятся в активном состоянии.
