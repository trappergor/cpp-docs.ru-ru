...
|Реляционные операторы|Допустимы для всех типов: == и !=|
...
- **Int**, `uint` , **float**, **Double**, `norm` или `unorm` scalar.
...
- T не является **двойным**, `norm` или `unorm` .
