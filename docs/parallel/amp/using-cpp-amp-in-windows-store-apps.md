...
Если вы используете Visual C++ расширения компонентов C++/CX, чтобы создать приложение универсальной платформы Windows (UWP), рекомендуется использовать типы plain-old-data (POD) вместе со смежным хранилищем — например, `std::vector` или массивы в стиле C — для данные, которые будут использоваться с C++ AMP. Это может помочь добиться более высокой производительности, чем с помощью отличных от POD типов или контейнеров Windows RT, так как не маршалинга не требуются.
