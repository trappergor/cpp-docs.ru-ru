...
ms.locfileid: "56676491"
---
# <a name="how-to-create-resources-c"></a>Практическое руководство. Создание ресурсов (C++)

Можно создавать ресурсы в своем проекте:

- С помощью файла описания ресурсов. Этот шаг является обязательным, прежде чем добавлять ресурсы.

- Добавление ресурсов в проект. Этот шаг показывает, как использовать **представление ресурсов**.

- С помощью шаблона ресурсов для создания настраиваемых ресурсов.

> [!NOTE]
> **Редактора ресурсов** и **представление ресурсов** недоступны в выпусках Express.
...
> [!NOTE]
> Сценарий ресурса можно добавить только в существующий проект загружается в интегрированной среде разработки Visual Studio. Невозможно создать автономного сценария ресурсов за пределами проекта, то, что шаблоны ресурсов (RCT-файлы) можно создать в любой момент.
...
> [!NOTE]
> Некоторые команды доступны только в том случае, если файл открыт отдельно, то есть за пределами проекта или без загрузки проекта. Например, чтобы использовать **Сохранить как** команду, чтобы сохранить файл с другим именем формат или файл, файл должен быть открыт отдельно.

- Открытие файла описания ресурсов за пределами проекта, в меню, перейдите к **файл** > **откройте**и выберите **файл**. Затем найдите файл описания ресурсов, выделите файл и выберите **откройте**.

  - Открытие файла описания ресурсов в текстовом формате, используйте стрелку раскрывающегося списка в правой части **откройте** кнопку на предыдущем шаге и выберите **открыть с помощью**. Затем выберите **редактор исходного кода (текста)** и из **открыть как** стрелку раскрывающегося списка выберите **текст**, и ресурс откроется в **исходный код**редактора.
...
- Чтобы открыть несколько ресурсов сценарии выполните приведенные выше действия для каждого файла, который требуется открыть, например, *Source1.rc* и *Source2.rc*. Затем, когда оба RC-файлов будут открыты в отдельных окнах документа, либо использовать **окно** меню или щелкните правой кнопкой мыши один из RC-файлов и выберите **создать группу горизонтальных вкладок** или **новый вертикальной табуляции Группа**. Windows теперь располагаются, чтобы одновременно можно было их просматривать.

> [!TIP]
> Файлы описания ресурсов также можно открыть, щелкнув правой кнопкой мыши RC-файл в **обозревателе решений**, нажимая кнопку **открыть с помощью...**  и выбрав **редактор исходного кода (текста)**.

### <a name="to-edit-a-resource-script-file"></a>Для изменения файла описания ресурсов

- Чтобы преобразовать существующий файл ресурсов в шаблон с помощью файла описания ресурсов, откройте в меню, перейдите в раздел **файл** > **Сохранить \< *filename*> как**. Затем укажите расположение и выберите **ОК**.
...
- Добавление поддержки MFC с открытым, файл описания ресурсов в **представление ресурсов**, выделите папку ресурсов (например, *MFC.rc*). Затем в [окно "Свойства"](/visualstudio/ide/reference/properties-window), задайте **режим MFC** для **True**.
  ...
  > [!NOTE]
  > Дополнение к параметру **режим MFC**, RC-файл должен быть частью проекта MFC. Просто установив **режим MFC** для **True** на .rc файл в проекте Win32, вы не получите функций MFC.
...
Щелкните правой кнопкой мыши можно использовать на **представление ресурсов** окно, чтобы открыть контекстное меню команд, или дважды щелкните строку заголовка, чтобы закрепить или открепить окно. Щелкните правой кнопкой мыши строку заголовка дополнительные команды, управляющие поведением окна. Дополнительные сведения см. в разделе [управления Windows](/visualstudio/ide/customizing-window-layouts-in-visual-studio).
...
| **Пользовательский** | Откройте **Создание настраиваемого ресурса** диалоговое окно, чтобы создать настраиваемый ресурс.<br/><br/>Также включает в себя **тип ресурса** свойства, которое предоставляет текстовое поле для ввода имени типа настраиваемого ресурса. Visual C++ автоматически преобразует в прописную имя, при выходе.<br/><br/>Настраиваемые ресурсы только редактирования в двоичном редакторе. |
...
Для создания определенного ресурса, воспользуйтесь одним из следующих:
...
> [!NOTE]
> Поместите файлы шаблонов языковой специфики в подкаталогах основного каталога шаблонов. Например файлы шаблонов перейдите... \\< каталог_шаблонов_ресурсов\>\1033. Visual Studio выполняет поиск нового RCT-файлы в Visual Studio \Program Files\Microsoft \<версии\>\VC\VCResourceTemplates \Program Files\Microsoft Visual Studio \<версии > \VC\VCResourceTemplates\\< LCID\> (например, LCID 1033 для английского языка), или в любом месте [путь включаемых файлов](../windows/how-to-specify-include-directories-for-resources.md). Если вы предпочитаете хранить ваш RCT-файлы в другом месте, необходимо добавить расположение в путь включаемых файлов.
