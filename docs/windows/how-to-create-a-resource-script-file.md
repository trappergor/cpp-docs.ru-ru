...
>
> Эта информация не относится к ресурсам в классических приложений Windows или приложениях универсальной платформы Windows. В проектах на языках .NET не используются файлы описания ресурсов. Сведения о добавлении ресурсов в управляемые проекты см. в разделе [файлы ресурсов](../windows/resource-files-visual-studio.md), [ресурсов приложения и система управления ресурсами](/windows/uwp/app-resources/), и [ресурсов в настольных приложениях](/dotnet/framework/resources/index) в *руководства разработчика .NET Framework*.

## <a name="create-a-resource-script"></a>Создайте скрипт ресурсов
...
1. Укажите имя для скрипта ресурсов в **имя** текстовое поле, а затем выберите **откройте**.

### <a name="to-open-a-resource-script"></a>Чтобы открыть сценарий ресурсов

Ресурсы можно просмотреть в сценарий ресурсов без необходимости открытия проекта. Файл скрипта будет открыт в окне документа в **представление ресурсов** окна (как в случае, когда файл открыт в рамках проекта).

> [!NOTE]
> Некоторые команды доступны только в том случае, если файл имеет открыт отдельно (за пределами проекта). Открытие автономного файла означает, что открывается без загрузки проекта.
>
...
Чтобы открыть сценарий ресурсов за пределами проекта:

1. Из **файл** меню, выберите **откройте**, затем выберите **файл**.

1. В **открыть файл** диалоговом окне найдите файл описания ресурсов, выделите файл и выберите **откройте**.

Чтобы открыть несколько скриптов ресурсов за пределами проекта:
...
1. Когда оба RC-файлы открыты, в **окно** меню (или щелкните правой кнопкой мыши один из RC-файлов) выберите **создать группу горизонтальных вкладок** или **создать группу вертикальных вкладок**.
...
> [!TIP]
> Ярлык — щелкнуть правой кнопкой мыши RC-файл в **обозревателе решений**, выберите **открыть с помощью...**  и выберите **редактор исходного кода (текста)**.

### <a name="to-add-mfc-support"></a>Добавление поддержки MFC

Как правило если при создании приложения Microsoft Foundation Class (MFC) для Windows с помощью [мастер приложений MFC](../mfc/reference/mfc-application-wizard.md), мастер создает базовый набор файлов (включая RC-файл ресурсов), содержит основные функции MFC. Тем не менее при редактировании RC-файла для приложения Windows, не зависит от MFC, недоступны следующие функции MFC:

Некоторые функции, относящиеся к MFC включают мастеры кода MFC, командные строки меню, список содержимого элементов управления поля со списком и размещение элементов управления ActiveX.

Добавление поддержки MFC в файл описания ресурсов:
...
Используйте **Создание настраиваемого ресурса** диалоговое окно, чтобы создать новый настраиваемый ресурс в проекте C++ со следующим свойством:
...
Можно создать новый ресурс, следующим образом:

- В **представление ресурсов**, выберите RC-файл, а затем **изменить** меню и выберите **добавить ресурс**. В **добавить ресурс** диалоговое окно, выберите тип ресурса, вы хотите добавить в проект.

   > [!TIP]
   > Щелкните правой кнопкой мыши RC-файл в **представление ресурсов** и выберите **добавить ресурс** в контекстном меню.

- В **обозревателе решений**, щелкните правой кнопкой мыши папку проекта и выберите **добавить**, затем выберите **добавить ресурс** в контекстном меню. В **добавить ресурс** диалоговое окно, выберите тип ресурса, вы хотите добавить в проект.
...
- В [представление классов](/visualstudio/ide/viewing-the-structure-of-code), щелкните класс правой кнопкой мыши и выберите пункт **добавить**. Выберите **добавить ресурс** в контекстном меню и выберите тип ресурса, вы хотите добавить в проект.

- В **проекта** меню, выберите **добавить ресурс**.

## <a name="create-a-resource-template"></a>Создать шаблон ресурсов
...
1. В **Добавление нового элемента** отображаемое в диалоговом окне **шаблоны:** области выберите **файл шаблона ресурсов (.rct)**.
...
1. В **добавить ресурс** диалоговом окне выберите знак «плюс» (**+**) рядом с ресурсом, чтобы развернуть узел ресурса и просмотреть все шаблоны, доступные для этого ресурса.
