---
title: Добавление или удаление строки | Документация Майкрософт
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-windows
...
ms.locfileid: "39464153"
---
# <a name="adding-or-deleting-a-string"></a>Добавление или удаление строки
Можно быстро вставлять новые записи в таблицу строк с помощью редактора. Новые строки помещаются в конец таблицы и получают следующий доступный идентификатор. Затем можно изменить свойства идентификатора, значения или заголовка в [окно "Свойства"](/visualstudio/ide/reference/properties-window) при необходимости.  
  
 Редактор строк гарантирует, что вы не используете идентификатор, который уже используется. Если выбрать идентификатор уже используется, редактор строк будет получать уведомления, а затем назначьте универсальный уникальный идентификатор, например IDS_STRING58113.  
