...
1. Для настольных программ Windows требуется > &lt;windows. h. &lt;файле Tchar. h > определяет макрос `TCHAR`, который, в конечном итоге, разрешается в **wchar_t** , если символ Юникода определен в проекте, в противном случае он разрешается в **char**.  Если вы всегда создаете Юникод с включенным поддержкой Юникода, то не требуется TCHAR и можете просто использовать **wchar_t** напрямую.
