...
1. В диалоговом окне **Добавление нового элемента** выберите **Файл C++ (.cpp)**. В поле **имя** введите имя файла, например *хелловиндовсдесктоп. cpp*. Нажмите кнопку **Добавить**.
...
1. Для настольных программ Windows требуется &lt;> Windows. h. &lt;в файле Tchar. h> определен `TCHAR` макрос, который в конечном итоге разрешается в **wchar_t** , если в проекте определен символ Юникода, в противном случае он разрешается в **char**.  Если вы всегда создаете Юникод с включенным поддержкой Юникода, то не требуется TCHAR и можете просто использовать **wchar_t** напрямую.
