...
- Язык ресурса определяет язык, используемый [FindResource](/windows/win32/api/winbase/nf-winbase-findresourcew) для выявления искомого ресурса. Ресурсы могут иметь различия для каждого языка, не связанного с текстом, например ускорители, которые могут работать только на японской клавиатуре или точечном рисунке, который подходит только для китайских локализованных сборок.
