 ...
 *предложений ограничений*  
 Список (не разделенный запятыми) **где** предложения определяют ограничения параметров типов. Имеет следующий вид:  
 ...
 *модификаторы доступа*  
 Модификаторы доступа для универсального класса. Для среды выполнения Windows, — только разрешенные модификатор `private`. Для среды CLR разрешенные модификаторы — `private` и `public`.  
  
 *identifier*  
 Имя универсального класса — любой допустимый идентификатор C++.  
  
 *модификаторы* (необязательно)  
 Допустимые модификаторы — `sealed` и **абстрактный**.  
...
```  
 ...
 Типы значений (встроенные типы, такие как `int` или `double`, либо определяемые пользователем типы значений) и ссылочные типы могут использоваться в качестве аргумента универсального типа. Синтаксис в рамках универсального определения одинаково независим. Синтаксически неизвестный тип обрабатывается так, как если бы он был ссылочным типом. Однако среда выполнения может определить, является ли фактически используемый тип типом значения, и подставить соответствующий созданный код для прямого доступа к членам. Типы значений, используемые в качестве аргументов универсального типа, не упаковываются и поэтому снижение производительности, связанное с упаковкой-преобразованием, не оказывает на них негативного влияния. Синтаксис, используемый в теле универсального должно быть **T ^** и "**->**«вместо of»**.**". При использовании [ref new gcnew](../windows/ref-new-gcnew-cpp-component-extensions.md) для типа параметра соответствующим образом рассматриваются средой выполнения как создание простой тип значения, если аргумент типа является типом значения.  
  
 Можно также объявить универсальный класс с [ограничений для параметров универсального типа (C + +/ CLI)](../windows/constraints-on-generic-type-parameters-cpp-cli.md) для типов, которые можно использовать для параметра типа. В следующем примере любой тип, используемый для `ItemType`, должен реализовывать интерфейс `IItem`. Попытка использовать, например, тип `int`, который не реализует `IItem`, приведет к ошибке компиляции, поскольку аргумент типа не удовлетворяет ограничению.  
...
## <a name="example"></a>Пример  
 В следующем примере три различных экземпляра универсального класса MyClass\<ItemType >, созданных с помощью аргументов соответствующих типов (`int`, **двойные**, и **строка**).  
...
```  
 ...
 После именования сконструированных вложенных универсальных типов параметр внешнего типа не включается в список параметров внутреннего типа, даже если внутренний тип неявно параметризуется параметром внешнего типа. В представленном выше случае имя сконструированного типа будет Outer\<int >:: внутреннего\<строки >.  
...
## <a name="example"></a>Пример  
 В следующем примере объявляется универсальная структура `MyGenStruct`, с одним полем `myField`и присваивает значения разных типов (`int`, **двойные**, **строка ^**) для этого поля.  
