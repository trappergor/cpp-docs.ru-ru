...
*precision*<br/>
Используемых Точность, используемая для записи столбца. Дополнительные сведения см. в описании элемента `bPrecision` [структуры DBBINDING](/previous-versions/windows/desktop/ms716845(v=vs.85))

*масштаб*<br/>
Используемых Масштаб, используемый для записи столбца. Дополнительные сведения см. в описании элемента `bScale` [структуры DBBINDING](/previous-versions/windows/desktop/ms716845(v=vs.85))
...
Этот пример привязывает столбец таблицы к **длинному** элементу данных и задает поля состояния и длины.
...
Этот пример привязывает четыре столбца к **длинному**, символьной строке, метке времени и `DB_NUMERIC` целому числу в указанном порядке.
...
|||
|-|-|
|**Применение**|**класс**, **Структура**, член, метод|
|**Повторяемый**|нет|
