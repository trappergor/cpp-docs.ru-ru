...
ms.locfileid: "80167515"
---
# <a name="attributes-alphabetical-reference"></a>Алфавитный указатель атрибутов

В компиляторе Майкрософт C++ доступны следующие атрибуты:
...
|[attribute](attribute.md)|Позволяет создать настраиваемый атрибут.|
...
|[entry](entry.md)|Указывает экспортированную функцию или константу в модуле, определяя точку входа в библиотеке DLL.|
...
|[licensed](licensed.md)|Указывает, что компонент, к которому он применяется, имеет лицензию и должен быть создан с помощью `IClassFactory2`.|
...
|[progid](progid.md)|Указывает ProgID для COM-объекта.|
...
|[restricted](restricted.md)|Указывает, что библиотека, или член модуля, интерфейса или DISP, не может вызываться произвольным образом.|
...
|[источник](source-cpp.md)|Указывает, что член класса, свойства или метода является источником событий.|
|[строка](string-cpp.md)|Указывает, что одномерный **символ**, **wchar_t**, `byte`или эквивалентный массив или указатель на такой массив должен рассматриваться как строка.|
...
|[usesgetlasterror](usesgetlasterror.md)|Сообщает вызывающему, что при возникновении ошибки при вызове этой функции вызывающий объект может вызвать `GetLastError`, чтобы получить код ошибки.|
