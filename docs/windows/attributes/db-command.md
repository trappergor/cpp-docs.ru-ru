---
title: db_command (атрибут COM C++) | Документация Майкрософт
ms.custom: ''
ms.date: 07/10/2018
ms.technology:
- cpp-windows
...
> блок параметров привязки 1 &nbsp; &nbsp;блок параметров привязки команды OLE DB 2 &nbsp; &nbsp;продолжение блок параметров привязки команды OLE DB 3...
...
Если строка команды содержит один или несколько параметров, таких как \[db_column] или \[bindto], **db_command** создает набор строк и карту метода доступа для обслуживания этих связанных переменных. См. в разделе [db_accessor](db-accessor.md) Дополнительные сведения.
...
*name*<br/>
(Необязательно) Имя дескриптора, который можно использовать для работы с набором строк. При указании *name* **db_command** создает класс с указанным *именем*, который можно использовать для просмотра набора строк или для выполнения нескольких запросов на изменение. Если не указать *name*, пользователю нельзя будет вернуть больше одной строки результатов.

*source_name*<br/>
(Необязательно) `CSession` Переменной или экземпляр класса, который имеет `db_source` атрибут, примененный к нему, на котором выполняется команда. См. в разделе [db_source](db-source.md).
...
*Привязки*<br/>
(Необязательно) Позволяет отделить параметры привязки от команды OLE DB.
...
*bulk_fetch*<br/>
(Необязательно) Целочисленное значение, указывающее количество извлекаемых строк.
...
Значение больше 1 обозначает массовое получение строк. Массовое получение строк относится к возможности большого набора строк получить несколько дескрипторов строк (набор строк будет иметь тип [CBulkRowset](../../data/oledb/cbulkrowset-class.md) и вызовет `SetRows` с указанным числом строк).

Если *bulk_fetch* меньше единицы, `SetRows` возвращает нуль.

## <a name="remarks"></a>Примечания

**db_command** создает [CCommand](../../data/oledb/ccommand-class.md) объект, который используется потребителем OLE DB для выполнения команды.

Можно использовать **db_command** с областью класса или функции; основное различие заключается в области объекта `CCommand` . В области видимости функции такие данные, как привязки, завершаются в конце функции. Данные об использовании области класса и функции включают в себя класс шаблона потребителя OLE DB `CCommand<>`, но другие аргументы шаблона для случаев, функций и классов. В случае функции привязки выполняются для `Accessor` , содержит локальные переменные, а использование класса `CAccessor`-производным классом в качестве аргумента. При использовании в качестве атрибута класса **db_command** работает в сочетании с **db_column**.
