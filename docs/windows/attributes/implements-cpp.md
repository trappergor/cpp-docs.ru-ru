...
ms.locfileid: "59033687"
---
# <a name="implements-c"></a>implements (C++)

Указывает диспетчерские интерфейсы, которые нужно принудительно в качестве членов компонентного класса IDL.
...
По умолчанию только COM-интерфейсы, которые являются базовые классы из `coclass` добавляются в компонентного класса IDL. **реализует** позволяет принудительно выполнять другие интерфейсы, чтобы быть IDL `coclass` членов.
...
Следующий пример состоит из трех частей: IDL-файл и его связанные h-файл и файл C++.

Предполагается следующий IDL-файла, которое будет доступно для компилятора.
...
И следующий файл .h, который также должен быть доступным для компилятора.
...
В следующей программе, не реализует интерфейс `IBar1`, `IBar2`, и `ISna` будет отсутствовать в `coclass` в созданном IDL.
