...
-   `__has_finalizer(` `type` `)`  
     ...
     (В [!INCLUDE[cppwrt](../build/reference/includes/cppwrt_md.md)] не поддерживается.) Возвращает **true** Если тип среды CLR имеет метод завершения. См. в разделе [деструкторы и методы завершения в разделе: определение и использование классов и структур (C + +/ CLI)](../dotnet/how-to-define-and-consume-classes-and-structs-cpp-cli.md#BKMK_Destructors_and_finalizers) Дополнительные сведения.  
