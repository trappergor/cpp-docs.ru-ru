...
- Все [символы](../windows/symbols-resource-identifiers.md) должно быть уникальным в пределах приложения, чтобы предотвратить конфликты определений символов в файлах заголовков.
...
- Имена символов не учитывается, но сохраняется регистр первого определения символа.
...
В именах символов используются описательные префиксы, указывающие на тип ресурса или объект, которые они представляют. Эти описательные префиксы начинаются с сочетания знаков "ID". Библиотека Microsoft Foundation Class (MFC) использует символ, показанный в следующей таблице соглашения об именовании:
...
Значения символов для ресурсов, таких как ускорители, растровые изображения, курсоры, диалоговые окна, значки, меню, таблицы строк и версии, сведения должны быть десятичными числами в диапазоне от 0 до 32 767, но не может быть шестнадцатеричным. Значения символов для частей ресурсов (элементов управления диалоговых окон или отдельных строк в таблице строк) могут быть в диапазоне от 0 до 65 534 или в диапазоне от –32 768 до 32 767. Дополнительные сведения о диапазонах чисел см. в разделе [TN023: Стандартные ресурсы MFC](../mfc/tn023-standard-mfc-resources.md).

Символы ресурсов являются 16-разрядных чисел. Их можно вводить как со знаком или без знака, однако они используются внутренне как целых чисел без знака, поэтому отрицательные числа будут приведены к соответствующим положительным значениям.
...
В **символы ресурсов** диалоговом окне выберите символ, который требуется удалить и выберите пункт **удалить**.
