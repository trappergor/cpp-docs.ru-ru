...
Диалоговое окно **новое &lt;устройство&gt; тип образа** позволяет создать новый образ устройства указанного типа. Чтобы открыть диалоговое окно **Создание образа \<устройства >** , последовательно выберите пункты **изображение** меню > **новый тип изображения**. Включены следующие свойства: **тип целевого образа** и **Пользовательский**.
...
Используйте диалоговое окно **Открытие изображения &lt;устройство&gt;** для открытия образов устройств в C++ проектах. В нем перечислены существующие образы устройств в текущем ресурсе (изображения, которые являются частью текущего ресурса). Включается следующее свойство:
...
1. Последовательно выберите пункты **изображение** меню > **создать образ устройства**или щелкните правой кнопкой мыши в области **редактора изображений** и выберите пункт **создать изображение устройства**.
...
1. Перейдите в меню **изображение** > **Открыть изображение устройства** и выберите изображение из списка текущие изображения. Например, выберите версию значка 32 × 32, 16 цветов.
...
Пока изображение значка отображается в **редакторе изображений**, перейдите в меню **изображение** > **Удалить образ устройства**. При удалении последнего изображения значка в ресурсе также удаляется ресурс.
...
1. Перейдите в меню **image** > **new device image (создать изображение устройства** ) и выберите нужный стиль изображения 256.
...
1. Выберите цвет из цвета 256, отображаемых в палитре **цвета** в окне **цвета** .
   ...
   > [!NOTE]
   > Начальная палитра, используемая для изображений 256-цветов, соответствует палитре, возвращаемой `CreateHalftonePalette` API Windows. Все значки, предназначенные для оболочки Windows, должны использовать эту палитру для предотвращения мерцания во время реализации палитры.
...
1. В **диалоговом окне новый файл**выберите **визуальную C++**  папку, выберите **файл точечного рисунка (. bmp)** в поле **шаблоны** и нажмите кнопку **Открыть**.

   Точечный рисунок откроется в **редакторе изображений**.

1. Внесите необходимые изменения в новый точечный рисунок.

1. Если точечный рисунок по-прежнему открыт в **редакторе изображений**, перейдите в меню **файл** > **Сохранить *имя_файла*. bmp как**.
...
1. Перейдите в **файл** меню > **Сохранить *имя файла* как**.

1. В диалоговом окне **сохранить файл как** в поле **имя файла** введите имя файла и расширение, которое обозначает нужный формат.

1. Щелкните **Сохранить**.

### <a name="to-add-a-new-image-resource-to-an-unmanaged-c-project"></a>Добавление нового ресурса изображения в неуправляемый C++ проект
