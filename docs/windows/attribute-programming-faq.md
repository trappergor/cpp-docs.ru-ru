...
##  <a name="vcconattributeprogrammmingfaqanchor1"></a> Что такое значение HRESULT  
 `HRESULT` Является простой тип данных, часто используется в качестве возвращаемого значения атрибутов и ATL в целом. В следующей таблице описаны различные значения. Дополнительные значения содержатся в файле winerror.h файл заголовка.  
...
##  <a name="vcconattributeprogrammmingfaqanchor2"></a> Если необходимо указать имя параметра для атрибута?  
 В большинстве случаев Если атрибут имеет один параметр, этот параметр называется. Это имя не требуется при вставке атрибутов в коде. Например, следующее использование [статистическую обработку](../windows/aggregatable.md) атрибут:  
...
##  <a name="vcconattributeprogrammmingfaqanchor5"></a> Как использовать атрибуты в проекте ATL без атрибутов?  
 Возможно, проект ATL без атрибутов, который IDL-файле, и можно приступить к добавлению помеченных атрибутами объектов. В этом случае используйте мастер добавления класса для предоставления кода.  
  
##  <a name="vcconattributeprogrammmingfaqanchor6"></a> Как использовать IDL-файл в проекте с атрибутами?  
 Возможно IDL-файла, который вы хотите использовать в проекте ATL с атрибутами. В этом случае используется [importidl](../windows/importidl.md) атрибут, для компиляции в IDL-файл h-файл (см. в разделе [страницы свойств MIDL](../ide/midl-property-pages.md) в диалоговом окне страницы свойств проекта) и включите h-файл в проект .  
...
##  <a name="vcconattributeprogrammmingfaqhowcaniforwarddeclareanattributedinterface"></a> Как пересылать объявить интерфейсе с атрибутами?  
 Если вы собираетесь сделать у прямого объявления интерфейсе с атрибутами, необходимо применить те же атрибуты для опережающего объявления, относящиеся к объявлению фактический интерфейс. Необходимо также применить [Экспорт](../windows/export.md) для вашей опережающего объявления атрибута.  
