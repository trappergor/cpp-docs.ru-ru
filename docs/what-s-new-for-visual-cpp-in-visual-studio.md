...
- [!INCLUDE[vs_dev15_md](misc/includes/vs_dev15_md.md)] позволяет использовать [/sdl](build/reference/sdl-enable-additional-security-checks.md) с [/await](build/reference/await-enable-coroutine-support.md). Мы удалили ограничение [/RTC](build/reference/rtc-run-time-error-checks.md) для соподпрограмм.

   **Visual Studio 2017 версия 15.3**:  
- [/std:c++14 и /std:c++latest](build/reference/std-specify-language-standard-version.md): эти параметры компилятора позволяют выбирать в проекте определенные версии языка C++, соответствующие стандарту ISO. Большинство новых стандартных функций защищены с помощью параметра **/std:c++latest**.
...
- Повышенная защищенность кода: улучшенное создание диагностики компилятора при переполнении буфера. Параметр [/guard:cf](build/reference/guard-enable-control-flow-guard.md) теперь обеспечивает защиту операторов switch, которые создают таблицы переходов.
- Управление версиями. Значение встроенного препроцессора **\_MSC\_VER** теперь монотонно обновляется при каждом обновлении набора инструментов Visual C++. Дополнительные сведения см. в статье [Visual C++ Compiler Version](https://blogs.msdn.microsoft.com/vcblog/2016/10/05/visual-c-compiler-version/) (Версия компилятора Visual C++).
...
**Visual Studio 2017 версии 15.3**: добавлена поддержка генератора CMake Ninja. 

**Visual Studio 2017 версии 15.5**: добавлена поддержка для импорта существующих кэшей CMake. 

**Visual Studio 2017 версии 15.7**: добавлены поддержка CMake 3.11, анализ кода в проектах CMake, просмотр целевых объектов в обозревателе решений, параметры для создания кэша и возможность компиляции одного файла. Дополнительные сведения см. в статьях [CMake support in Visual Studio](https://blogs.msdn.microsoft.com/vcblog/2018/04/09/cmake-support-in-visual-studio-targets-view-single-file-compilation-and-cache-generation-settings/) (Поддержка CMake в Visual Studio) [CMake projects in Visual C++](ide/cmake-tools-for-visual-cpp.md) (Проекты CMake в Visual C++).
...
Теперь доступны новые возможности для записи и упаковки приложений C++ для универсальной платформы Windows и Microsoft Store. С помощью преобразователя классических приложений можно упаковывать существующие классические приложения для развертывания с помощью Microsoft Store. Дополнительные сведения см. в статьях [Использование среды выполнения Visual C++ в проекте Centennial](https://blogs.msdn.microsoft.com/vcblog/2016/07/07/using-visual-c-runtime-in-centennial-project/) и [Перенос классического приложения на универсальную платформу Windows (UWP) с помощью Desktop Bridge](/windows/uwp/porting/desktop-to-uwp-root).

**Visual Studio 2017 версии 15.5**:

Добавлен шаблон проекта **Проект упаковки приложения Windows**, который поддерживает упаковку классических приложений с помощью моста для классических приложений Desktop Bridge. Чтобы открыть его, выберите **Файл > Создать > Проект**, **Установленные > Visual C++ > Универсальная платформа Windows** при установке рабочей нагрузки универсальных приложений Windows.
...
Дополнительные сведения см. в записи блога [Boost.Test Unit Testing: Dynamic Library support and New Item Template](https://blogs.msdn.microsoft.com/vcblog/2018/01/10/boost-test-unit-testing-dynamic-library-support-and-new-item-template/) (Модульное тестирование Boost.Test. Поддержка динамической библиотеки и новый шаблон элемента). 

**Visual Studio 2017 версии 15.7**:

Добавлена поддержка [CodeLens](https://docs.microsoft.com/en-us/visualstudio/ide/find-code-changes-and-other-history-with-codelens) для проектов модульного тестирования C++. Дополнительные сведения см. в записи блога [Announcing CodeLens for C++ Unit Testing](https://blogs.msdn.microsoft.com/vcblog/2018/04/09/announcing-codelens-for-c-unit-testing/) (Объявлена поддержка CodeLens для модульного тестирования C++).
