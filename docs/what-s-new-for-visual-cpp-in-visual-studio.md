...
Номер версии компилятора и инструментов в [!INCLUDE[vs_dev15_md](misc/includes/vs_dev15_md.md)] — 14.10.24629. 
...
* _**Новые возможности в версии-кандидате**_ Функция IntelliSense замены точки на стрелку была перенесена из экспериментальных в расширенные и теперь включена по умолчанию. Функции расширения областей и приоритета расширения в редакторе также были перенесены из экспериментальных в расширенные.

* _**Новые возможности в версии-кандидате**_ Экспериментальные функции рефакторинга "Изменить сигнатуру" и "Извлечь функцию" теперь доступны по умолчанию.

* _**Новые возможности в версии-кандидате**_ Включена новая экспериментальная функция для проектов C++ — "Ускорение загрузки проекта". Когда вы в следующий раз откроете проект C++, он загрузится быстрее, а в последующий раз — очень быстро.
...
* _**Новые возможности в версии-кандидате**_ Улучшения в процедуре открытия папки.     
Можно выполнить настройку с помощью следующих файлов JSON:
...
В Visual Studio 2017 добавлена поддержка использования проектов CMake без преобразования в файлы проекта MSBuild (с расширением VCXPROJ). Дополнительные сведения см. в статье [Поддержка CMake в Visual Studio](https://blogs.msdn.microsoft.com/vcblog/2016/10/05/cmake-support-in-visual-studio/). При открытии проектов CMake с помощью функции "Открыть папку" автоматически настраивается среда для редактирования, сборки и отладки C++.

* _**Новые возможности в версии-кандидате**_ Для работы C++ IntelliSense не требуется создавать файл CppProperties.json в корневой папке. Кроме того, мы добавили новый раскрывающийся список, позволяющий пользователям легко переключаться между конфигурациями, предоставляемыми файлами CMake и CppProperties.json.

* _**Новые возможности в версии-кандидате**_ Дальнейшая настройка поддерживается посредством файла CMakeSettings.json, который находится в одной папке с файлом CMakeLists.txt.
...
C++ поставляется как дополнительный компонент для рабочей нагрузки универсального приложения для Windows.  
