...
> [!NOTE]
>  Если забыт определять конструктор без аргументов в класс, который использует макросы DECLARE_SERIAL и IMPLEMENT_SERIAL, вы получите предупреждение компилятора «нет конструктора по умолчанию» в строке, где используется IMPLEMENT_SERIAL-макрос.  
