...
Иногда требуется экспортировать только отдельные необходимые члены класса. Например, если вы экспортируете `CDialog`-производного класса может потребоваться только для экспорта в конструктор и `DoModal` вызова. Вы можете экспортировать эти элементы с помощью библиотеки DLL. DEF-файл, но можно также использовать `AFX_EXT_CLASS` в таким же образом для отдельных членов, которые требуется экспортировать.
...
Например DECLARE_DYNAMIC-макрос определяется следующим образом при создании библиотеки DLL расширения MFC:
...
Как было сказано выше, `AFX_EXT_CLASS` уже определен таким образом. Необходимо повторно определить `AFX_DATA` же `AFX_EXT_CLASS` вокруг его определение.
...
Тот же метод можно использовать для автоматического экспорта `CArchive` оператор извлечения для классов, используйте макрос DECLARE_SERIAL и IMPLEMENT_SERIAL. Экспортируйте оператор архива, скобок объявления класса (расположенный в. H-файл) с помощью следующего кода:
...
Экспорт ресурсов осуществляется по списку ресурсов. В каждое приложение — однонаправленный список `CDynLinkLibrary` объектов. При поиске ресурса, большая часть стандартных реализаций MFC, которые загружают ресурсы быть первой в текущем модуле ресурсов (`AfxGetResourceHandle`) и если он не найден обход списка `CDynLinkLibrary` объектов при попытке загрузить запрошенный ресурс.
...
- **/ MD** приложение должно использовать версия DLL библиотеки времени выполнения C
