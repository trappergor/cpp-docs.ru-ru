...
-   Строка с именем *strText* , получатель отображает его пользователю.  
 ...
 В этой модели взаимодействия word «Получено» отправить сообщение *strText* переменная является отображает их в другой конец связи, поэтому его для получателя указывает, что определенное число пакетов данных были Получено. Получатель отвечает с аналогичной строкой с надписью «Отправленные», для отображения на экране исходного отправителя. Обе строки указывает произошедшем успешной связи.  
