...
Когда изменение метки отменено или завершено, элемент управления "дерево" отправляет сообщение уведомления [TVN_ENDLABELEDIT](/windows/win32/Controls/tvn-endlabeledit) . Параметр *lParam* является адресом структуры [нмтвдиспинфо](/windows/win32/api/commctrl/ns-commctrl-tvdispinfow) . Элемент **Item** является структурой [твитем](/windows/win32/api/commctrl/ns-commctrl-tvitemw) , определяющей элемент и включающей редактируемый текст. Вы несете ответственность за обновление метки элемента (при необходимости), возможно, после проверки измененной строки. Если редактирование отменено `TV_ITEM` , член псзтекст имеет значение 0.
