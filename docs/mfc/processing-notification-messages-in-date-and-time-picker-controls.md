...
При взаимодействии пользователей с элементом управления "Выбор даты и времени" элемент`CDateTimeCtrl`управления () отправляет сообщения уведомления своему родительскому окну (обычно это представление или объект диалогового окна). Обрабатывайте эти сообщения, если требуется сделать что-нибудь в ответе. Например, когда пользователь открывает средство выбора даты и времени для вывода элемента управления "Календарь на встроенный месяц", отправляется уведомление DTN_DROPDOWN.
...
- DTN_DROPDOWN уведомляет родительский элемент управления "Календарь внедренного месяца". Это уведомление отправляется, только если не задан стиль DTS_UPDOWN. Дополнительные сведения об этом уведомлении см. [в разделе доступ к элементу управления "месячный календарь"](../mfc/accessing-the-embedded-month-calendar-control.md).

- DTN_CLOSEUP уведомляет родительский элемент управления "Календарь внедренного месяца", который будет закрыт. Это уведомление отправляется, только если не задан стиль DTS_UPDOWN.

- DTN_DATETIMECHANGE уведомляет родительский элемент о том, что в элементе управления произошло изменение.

- DTN_FORMAT уведомляет родительский элемент, что текст требуется отобразить в поле обратного вызова. Дополнительные сведения об этом уведомлении и полях обратного вызова см. [в разделе Использование полей обратного вызова в элементе управления средства выбора даты и времени](../mfc/using-callback-fields-in-a-date-and-time-picker-control.md).
