...
### <a name="remarks"></a>Примечания  
 Параметр `pbParamInfo` определяет типы параметров, передаваемых в метод или свойство. Дополнительные параметры для метода или свойства, для которого вызван метод могут быть переданы с помощью *va_list* параметра.  
