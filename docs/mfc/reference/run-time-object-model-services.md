...
Дополнительные сведения об этих службах объектов времени выполнения и сериализации см. в статье [класс CObject: Доступ к сведениям о](../../mfc/accessing-run-time-class-information.md)классе времени выполнения.
...
|[RUNTIME_CLASS](#runtime_class)|`CRuntimeClass` Возвращает структуру, соответствующую именованному классу.|
...
|[DECLARE_OLECTLTYPE](#declare_olectltype)|Объявляет функции члена `GetMiscStatus`икласса элемента управления. `GetUserTypeNameID`|
...
|[IMPLEMENT_OLECTLTYPE](#implement_olectltype)|Реализует функции элементов `GetMiscStatus`икласса элемента управления. `GetUserTypeNameID`|
|[IMPLEMENT_OLECREATE_FLAGS](#implement_olecreate_flags)|Этот макрос или [IMPLEMENT_OLECREATE](#implement_olecreate) должен присутствовать в файле реализации для любого класса, использующего `DECLARE_OLECREATE`. |
...
Определяет, реализует ли библиотека общих элементов управления указанный API (это версия [AFX_COMCTL32_IF_EXISTS](#afx_comctl32_if_exists)в Юникоде).
...
Добавьте макрос DECLARE_DYNAMIC в модуль header (. h) для класса, а затем включите этот модуль во все cpp-модули, которым требуется доступ к объектам этого класса.

Если вы используете макросы Dynamic и IMPLEMENT_DYNAMIC DECLARE_, как описано, можно использовать макрос RUNTIME_CLASS и `CObject::IsKindOf` функцию для определения класса объектов во время выполнения.

Если DECLARE_DYNAMIC включен в объявление класса, то IMPLEMENT_DYNAMIC должен быть включен в реализацию класса.
...
Позволяет динамически создавать `CObject`объекты производных классов во время выполнения.
...
Если DECLARE_DYNCREATE включен в объявление класса, то IMPLEMENT_DYNCREATE должен быть включен в реализацию класса.
...
Объявляет функции члена `GetMiscStatus`икласса элемента управления. `GetUserTypeNameID`
...
`DECLARE_PROPPAGEIDS` Используйте макрос в конце объявления класса. Затем в cpp-файле, который определяет функции-члены для класса, используйте `BEGIN_PROPPAGEIDS` макросы, записи макросов для каждой страницы свойств элемента управления `END_PROPPAGEIDS` и макрос для объявления конца списка страниц свойств.

Дополнительные сведения о страницах свойств см. в статье [элементы ActiveX: Страницы](../mfc-activex-controls-property-pages.md)свойств.
...
Создает код C++ заголовка, необходимый для `CObject`класса, производного от которого можно выполнить сериализацию.
...
Если DECLARE_SERIAL включен в объявление класса, то IMPLEMENT_SERIAL должен быть включен в реализацию класса.

Макрос DECLARE_SERIAL включает все функции DECLARE_DYNAMIC и DECLARE_DYNCREATE.

С помощью макроса AFX_API можно автоматически экспортировать `CArchive` оператор извлечения для классов, использующих макросы DECLARE_SERIAL и IMPLEMENT_SERIAL. Скобки объявления класса (расположенные в h-файле) со следующим кодом:
...
Создает C++ код, необходимый для динамического `CObject`производного класса с доступом во время выполнения к имени класса и его положению в иерархии.
...
Позволяет динамически создавать `CObject`объекты производных классов во время выполнения при использовании с макросом DECLARE_DYNCREATE.
...
Платформа использует эту возможность для динамического создания новых объектов, например при считывании объекта с диска во время сериализации. Добавьте макрос IMPLEMENT_DYNCREATE в файл реализации класса. Дополнительные сведения см. в [разделах о классах CObject](../../mfc/using-cobject.md).

При использовании макросов DECLARE_DYNCREATE и IMPLEMENT_DYNCREATE можно использовать макрос RUNTIME_CLASS и `CObject::IsKindOf` функцию Member для определения класса объектов во время выполнения.
...
Этот макрос или [IMPLEMENT_OLECREATE](#implement_olecreate) должен присутствовать в файле реализации для любого класса, использующего DECLARE_OLECREATE.
...
Реализует функции элементов `GetMiscStatus`икласса элемента управления. `GetUserTypeNameID`
...
RUNTIME_CLASS возвращает указатель на структуру [крунтимекласс](../../mfc/reference/cruntimeclass-structure.md) для класса, заданного параметром *class_name*. Только `CObject`производные классы, объявленные с помощью DECLARE_DYNAMIC, DECLARE_DYNCREATE или DECLARE_SERIAL, будут возвращать указатели `CRuntimeClass` на структуру.
...
Если DECLARE_OLECREATE включен в объявление класса, то IMPLEMENT_OLECREATE должен быть включен в реализацию класса. Объявление класса, использующее DECLARE_OLECREATE, должно также использовать DECLARE_DYNCREATE или DECLARE_SERIAL.
...
Этот макрос или [IMPLEMENT_OLECREATE_FLAGS](#implement_olecreate_flags) должен присутствовать в файле реализации для любого класса, использующего `DECLARE_OLECREATE`.
