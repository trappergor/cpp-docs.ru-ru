...
|[Коккманажер:: Креатедлгконтролс](#createdlgcontrols)|Создает элементы управления ActiveX, размещенные связанным объектом `COleContainer`.|
...
Базовый класс, `CNoTrackObject`, является недокументированным базовым классом (расположенным в АФКСТЛС. H). Классы, производные от класса `CNoTrackObject`, предназначенные для использования в платформе MFC, освобождаются от обнаружения утечек памяти. Не рекомендуется наследовать непосредственно от `CNoTrackObject`.
...
*лпмсг*<br/>
Указатель на структуру `MSG`, содержащую сообщение для проверки.
...
Поведением `IsDialogMessage` по умолчанию является проверка сообщений клавиатуры и их преобразование в выделенный фрагмент для соответствующего диалогового окна. Например, при нажатии клавиши TAB выбирается следующий элемент управления или группа элементов управления.
...
*фандлеринфо*<br/>
Если значение не равно NULL, `OnEvent` заполняет `pTarget` и `pmf` элементы структуры `AFX_CMDHANDLERINFO` вместо диспетчеризации команды. Как правило, этот параметр должен иметь значение NULL.
...
*поккдиалогинфо*<br/>
Структура `_AFX_OCC_DIALOG_INFO`, содержащая сведения о шаблоне диалогового окна и всех элементах управления ActiveX, размещенных в диалоговом окне.
...
Поведение по умолчанию вызывает `SplitDialogTemplate`, определяя, имеются ли какие-либо элементы управления ActiveX, а затем возвращает результирующий шаблон диалогового окна.

Переопределите эту функцию, чтобы настроить процесс создания диалогового окна, размещающего элементы управления ActiveX.

##  <a name="postcreatedialog"></a>Коккманажер::P Осткреатедиалог
...
Эта память была выделена вызовом `SplitDialogTemplate`и была использована для всех размещенных элементов управления ActiveX в диалоговом окне.
...
> [!NOTE]
>  Память, выделенная для нового шаблона, освобождается в функции `PostCreateDialog`.
