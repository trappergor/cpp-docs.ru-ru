...
|name|Описание|
|----------|-----------------|
|[CMDIFrameWnd:: Креатеклиент](#createclient)|Создает окно МДИКЛИЕНТ Windows для этого `CMDIFrameWnd`. Вызывается функцией `CWnd`члена класса. `OnCreate`|
...
Чтобы создать полезное окно фрейма MDI для приложения, создайте класс, производный `CMDIFrameWnd`от. Добавьте переменные члена в производный класс для хранения данных, относящихся к вашему приложению. Реализуйте в производном классе функции-члены обработчика сообщений и схему сообщений, чтобы указать, что происходит, когда сообщения направляются в окно.

Можно создать окно фрейма MDI, вызвав функцию- `CFrameWnd`член [CREATE](../../mfc/reference/cframewnd-class.md#create) или [лоадфраме](../../mfc/reference/cframewnd-class.md#loadframe) .
...
`LoadFrame`требует меньше аргументов, `Create`чем, и вместо этого извлекает большинство значений по умолчанию из ресурсов, включая заголовок фрейма, значок, таблицу сочетаний клавиш и меню. Для доступа `LoadFrame`ко всем этим ресурсам должны быть назначены одинаковые идентификаторы ресурсов (например, IDR_MAINFRAME).

Несмотря `MDIFrameWnd` на то, `CFrameWnd`что является производным от, класс `CMDIFrameWnd` окна фрейма, производный от, не должен объявляться вместе с `DECLARE_DYNCREATE`.

Класс наследует большую часть своей реализации по умолчанию `CFrameWnd`из. `CMDIFrameWnd` Подробный список этих функций см. в описании класса [CFrameWnd](../../mfc/reference/cframewnd-class.md) . `CMDIFrameWnd` Класс имеет следующие дополнительные возможности:

- Окно фрейма MDI управляет окном МДИКЛИЕНТ, перемещает его в сочетании с панелями управления. Окно клиента MDI — это прямой родительский элемент окон дочернего фрейма MDI. Стили окон WS_HSCROLL и WS_VSCROLL, заданные в `CMDIFrameWnd` , применяются к окну клиента MDI, а не к главному окну фрейма, чтобы пользователь мог прокручивать клиентскую область MDI (например, как в диспетчере программ Windows).
...
Дополнительные сведения о см `CMDIFrameWnd`. в разделе [окна](../../mfc/frame-windows.md)с рамками.
...
Вызовите функцию `LoadFrame` -член или,чтобысоздатьвидимоеокнофреймаMDI.`Create`
...
Реализация по умолчанию выполняет поиск всплывающего меню, содержащего стандартные команды меню окон, такие как ID_WINDOW_NEW и ID_WINDOW_TILE_HORZ.
...
Эта функция члена отправляет сообщение [WM_MDIACTIVATE](../../mfc/reference/cwnd-class.md#onmdiactivate) в активируемое дочернее окно и отключается дочернее окно.
...
> [!NOTE]
>  Дочернее окно MDI активируется независимо от окна фрейма MDI. Когда кадр становится активным, дочернее окно, которое было активировано последним, отправляет сообщение [WM_NCACTIVATE](../../mfc/reference/cwnd-class.md#onncactivate) для рисования кадра активного окна и строки заголовка, но не получает другое сообщение WM_MDIACTIVATE.
...
- MDITILE_HORIZONTAL плитки дочерние окна MDI, чтобы одно окно появлялось над другим.

- MDITILE_SKIPDISABLED запрещает мозаичное заполнение отключенных дочерних окон MDI.

- MDITILE_VERTICAL плитки дочерние окна MDI, чтобы одно окно появился рядом с другим.
