...
|`LM_COMMIT`|Сбрасывает `LM_MRUWIDTH` к ширине плавающей панели элементов управления.|  
...
### <a name="parameters"></a>Параметры  
 *контроллер домена*  
 Указатель на контекст устройства для использования для подготовки к просмотру границу панели управления.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Объект `CRect` объект, содержащий текущий ширину (в пикселях) каждой стороны объекта панели элементов управления. Например, значение *левой* элемент, из [CRect](../../atl-mfc-shared/reference/crect-class.md) , является ширину левой границы.  
