...
*lpSecurityAttrs*<br/>
Указывает на [SECURITY_ATTRIBUTES](/previous-versions/windows/desktop/legacy/aa379560\(v=vs.85\)) структура, задающая атрибуты безопасности для потока.
...
Значение `m_bAutoDelete` не влияет на способ закрытия базовый дескриптор потока, но влияет на время закрытием дескриптора. Дескриптор потока всегда закрывается, когда `CWinThread` уничтожении объекта.
...
`m_hThread` Данные-член — это открытая переменная типа ДЕСКРИПТОРА. Он допустим только если базовый объект ядра потока в настоящий момент, и еще не закрыт дескриптор.

Деструктор CWinThread вызывает CloseHandle на `m_hThread`. Если [m_bAutoDelete](#m_bautodelete) является значение TRUE, если поток завершается, CWinThread объект уничтожается, который не делает недействительными все указатели на объекта CWinThread и его переменных-членов. Может потребоваться `m_hThread` элементу для проверки значения выхода потока или ожидания сигнала. Объект CWinThread и его `m_hThread` набор элементов во время выполнения потоков и после его завершения, `m_bAutoDelete` значение false, прежде чем продолжить выполнение потока. В противном случае поток может завершить уничтожения объекта CWinThread и закрыть дескриптор, прежде чем пытаться использовать его. Если вы используете этот способ, вы несете ответственность для удаления объекта CWinThread.
...
`m_nThreadID` Данные-член — это открытая переменная типа DWORD. Он допустим только если базовый объект ядра потока в настоящий момент.
Также см. Примечания [m_hThread](#m_hthread) времени существования.
