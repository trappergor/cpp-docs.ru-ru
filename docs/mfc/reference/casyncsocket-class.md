...
|[CAsyncSocket:: Attach](#attach)|Присоединяет к `CAsyncSocket` объекту маркер сокета.|
...
|[CAsyncSocket::D етач](#detach)|Отсоединяет от `CAsyncSocket` объекта маркер сокета.|
...
|name|Описание|
|----------|-----------------|
|[CAsyncSocket:: OnAccept](#onaccept)|Уведомляет сокет прослушивания о том, что он может принимать запросы на `Accept`подключение, ожидающие подключения, путем вызова.|
...
|[CAsyncSocket:: OnReceive](#onreceive)|Уведомляет сокет прослушивания о том, что данные для извлечения `Receive`вызываются.|
|[CAsyncSocket:: OnSend](#onsend)|Уведомляет сокет о том, что он может отправить данные `Send`путем вызова.|
...
|[СОКЕТ CAsyncSocket:: operator](#operator_socket)|Этот оператор используется для получения маркера `CAsyncSocket` сокета объекта.|

### <a name="public-data-members"></a>Открытые члены данных

|name|Описание|
|----------|-----------------|
|[CAsyncSocket:: m_hSocket](#m_hsocket)|Указывает маркер сокета, присоединенный `CAsyncSocket` к этому объекту.|
...
Чтобы использовать `CAsyncSocket` объект, вызовите его конструктор, а затем вызовите функцию [CREATE](#create) для создания базового маркера сокета `SOCKET`(Type), за исключением принятых сокетов. Для сокета сервера вызывается функция-член [Listen](#listen) , а для сокета клиента вызывается функция-член [Connect](#connect) . Сокет сервера должен вызвать функцию [Accept](#accept) при получении запроса на соединение. Используйте оставшиеся `CAsyncSocket` функции для выполнения обмена данными между сокетами. После завершения удалите `CAsyncSocket` объект, если он был создан в куче; деструктор автоматически вызывает функцию [Close](#close) . Тип данных сокета описан в статье [сокеты Windows: Фон](../../mfc/windows-sockets-background.md).
...
Дополнительные сведения см. в [разделе сокеты Windows: Использование класса CAsyncSocket](../../mfc/windows-sockets-using-class-casyncsocket.md) и связанных статей, а также [API Windows Sockets 2](/windows/win32/WinSock/windows-sockets-start-page-2).
...
Аргумент *lpSockAddr* является параметр результата, который заполняется адрес подключения сокета, как известные. `Accept`используется с такими типами сокетов на основе соединений, как SOCK_STREAM.
...
- FD_READ хотят получить уведомление о готовности к чтению.

- FD_WRITE хочет получать уведомления, когда данные доступны для чтения.

- FD_OOB хочет получить уведомление о поступлении данных, находящихся за пределами диапазона.

- FD_ACCEPT хочет получить уведомления о входящих подключениях.

- FD_CONNECT хочет получить уведомления о результатах подключения.

- FD_CLOSE хочет получить уведомление при закрытии сокета одноранговым узлом.
...
Эта функция используется для указания того, какие функции уведомления обратного вызова MFC будут вызываться для сокета. `AsyncSelect`автоматически задает для этого сокета режим неблокировки. Дополнительные сведения см. в статье [сокеты Windows: Уведомления](../../mfc/windows-sockets-socket-notifications.md)сокета.

##  <a name="attach"></a>CAsyncSocket:: Attach

Вызовите эту функцию члена, чтобы присоединить обработчик хсоккет `CAsyncSocket` к объекту.
...
Сетевой адрес, разделенный цифрой, например "128.56.22.8". Передача строки null для этого параметра указывает на то `CAsyncSocket` , что экземпляр должен прослушивать активность клиента во всех сетевых интерфейсах.
...
- ВСАЕАДДРИНУСЕ указанный адрес уже используется. (См. параметр сокета SO_REUSEADDR в разделе [сетсоккопт](#setsockopt).)
...
Эта подпрограммы используется для неподключенных датаграмм или сокета потока перед последующим `Connect` вызовом или `Listen` . Прежде чем он сможет принимать запросы на подключение, сокет сервера прослушивания должен выбрать номер порта и сделать его известным сокетам Windows, `Bind`вызвав. `Bind`устанавливает локальное сопоставление (адрес узла или номер порта) сокета путем назначения локального имени неименованному сокету.
...
После создания объекта необходимо вызвать его `Create` функцию члена, чтобы создать структуру данных сокета и привязать ее адрес. (На стороне сервера при обмене данными с сокетами Windows, когда сокет для прослушивания создает сокет для использования в `Accept` вызове, этот сокет не `Create` вызывается.)
...
Для `CAsyncSocket`, но не для `CSocket` `Close` , семантика зависит от параметров сокета SO_LINGER и SO_DONTLINGER. Дополнительные сведения см. в разделе Функция `GetSockOpt`члена.
...
Если сокет не привязан, уникальные значения назначаются в локальную ассоциацию системой, а сокет помечается как привязанный. Обратите внимание, что если поле адреса структуры имени равно нулю, `Connect` возвращает ноль. Чтобы получить расширенные сведения об ошибке, `GetLastError` вызовите функцию члена.

Для сокетов потока (тип SOCK_STREAM) активное подключение инициируется для инородного узла. Когда вызов сокета завершается успешно, сокет готов к отправке и получению данных.

Для сокета датаграммы (тип SOCK_DGRAM) устанавливается назначение по умолчанию, которое будет использоваться при последующих `Send` вызовах и. `Receive`

##  <a name="create"></a>CAsyncSocket:: Create

Вызовите функцию-член после создания объекта сокета, чтобы создать сокет Windows и присоединить его. `Create`
...
- FD_WRITE хотят получить уведомление о готовности к написанию.

- FD_OOB хочет получить уведомление о поступлении данных, находящихся за пределами диапазона.

- FD_ACCEPT хочет получить уведомления о входящих подключениях.

- FD_CONNECT хочет получить уведомление о завершенном подключении.

- FD_CLOSE хочет получить уведомление о замыкании сокета.

*лпсзсоккаддресс*<br/>
Указатель на строку, содержащую сетевой адрес подключенного сокета, разделенный цифрой, например "128.56.22.8". Передача строки null для этого параметра указывает на то `CAsyncSocket` , что экземпляр должен прослушивать активность клиента во всех сетевых интерфейсах.
...
- SOCK_DGRAM поддерживает датаграммы без установления соединения, ненадежные пакеты фиксированной (обычно небольшой) максимальной длины. Использует протокол UDP для семейства Интернет-адресов.
    ...
    > [!NOTE]
    >  Функция- `CSocket` член принимает ссылку на новый пустой объект в качестве параметра. `Accept` Этот объект необходимо создать перед вызовом метода `Accept`. Помните, что если объект сокета выходит за пределы области действия, соединение закрывается. Не вызывайте `Create` этот новый объект сокета.
...
Дополнительные сведения о потоках и сокетах датаграмм см. в статье [сокеты Windows: ](../../mfc/windows-sockets-background.md) Фоновые [и сокеты Windows: Порты и адреса](../../mfc/windows-sockets-ports-and-socket-addresses.md) сокетов и [API Windows Sockets 2](/windows/win32/WinSock/windows-sockets-start-page-2).

##  <a name="detach"></a>CAsyncSocket::D етач

Вызовите эту функцию члена, чтобы отсоединить маркер сокета в элементе данных *m_hSocket* от `CAsyncSocket` объекта и установить *m_hSocket* в значение null.
...
Этот вызов особенно удобен в том случае `Connect` , если был выполнен вызов без `Bind` первого выполнения. Этот вызов предоставляет единственные средства, с помощью которых можно определить локальную связь, установленную системой.
...
Для `GetSockOpt`поддерживаются следующие параметры. Тип определяет тип данных, адресованных *лпоптионвалуе*. Параметр TCP_NODELAY использует уровень IPPROTO_TCP; все остальные параметры используют уровень SOL_SOCKET.
...
|SO_RCVBUF|ssNoversion|Размер буфера для получения.|
...
Параметры Berkeley Software Distribution (BSD) не поддерживаются `GetSockOpt` для:
...
Вызов `GetSockOpt` с неподдерживаемым параметром приведет к `GetLastError`возвращению кода ошибки всаенопротупт.
...
- СИОКАТМАРК определяет, были ли считаны все данные по внешнему каналу. Это относится только к сокету типа SOCK_STREAM, настроенному для интерактивного приема любых данных, не исходящих из аппаратного контроллера управления (SO_OOBINLINE). Если данные вне диапазона не ожидают чтения, операция возвращает ненулевое значение. В противном случае она возвращает 0, `Receive` а `ReceiveFrom` Следующая или выполняемая на сокете данные будут извлекать некоторые или все сведения, предшествующие "Mark"; приложение должно использовать операцию сиокатмарк, чтобы определить, остаются ли данные. Если имеются обычные данные, предшествующие "срочные" (внештатные) данные, они будут получены по порядку. (Обратите внимание `Receive` , `ReceiveFrom` что или никогда не будет смешивать нестандартные и обычные данные в одном вызове.) Параметр *лпаргумент* указывает на в, `DWORD` где `IOCtl` хранится результат.
...
`Listen`При отсутствии доступных портов (дескрипторов) попытки продолжать работать рационально. Он будет принимать подключения до тех пор, пока очередь не будет очищена. Если порты становятся доступными, последующий вызов `Listen` или `Accept` приведет к повторному заполнению очереди на текущую или последнюю "невыполненную работу", если это возможно, и возобновить прослушивание входящих подключений.
...
Дополнительные сведения см. в [разделе сокеты Windows: Уведомления](../../mfc/windows-sockets-socket-notifications.md)сокета.
...
MFC поддерживает данные вне диапазона, но пользователям класса `CAsyncSocket` не рекомендуется использовать их. Проще всего создать второй сокет для передачи таких данных. Дополнительные сведения о нестандартных данных см. в разделе [сокеты Windows: Уведомления](../../mfc/windows-sockets-socket-notifications.md)сокета.

##  <a name="onreceive"></a>CAsyncSocket:: OnReceive

Вызывается платформой для уведомления этого сокета о том, что в буфере есть данные, которые можно извлечь `Receive` , вызвав функцию-член.
...
Вызывается платформой для уведомления сокета о том, что теперь он может отправлять данные `Send` путем вызова функции члена.
...
Вызовите эту функцию, чтобы скопировать `CAsyncSocket` существующий объект в `CAsyncSocket` другой объект.

##  <a name="operator_socket"></a>СОКЕТ CAsyncSocket:: operator

Этот оператор используется для получения маркера `CAsyncSocket` сокета объекта.
...
- MSG_PEEK Просмотр входящих данных. Данные копируются в буфер, но не удаляются из входной очереди.

- MSG_OOB обработка данных вне диапазона.
...
- Указан ВСАЕОПНОТСУПП MSG_OOB, но тип сокета не является SOCK_STREAM.

- ВСАЕШУТДОВН. Работа сокета была завершена; вызов `Receive` через сокет после `ShutDown` вызова с параметром *нхов* , равным 0 или 2, невозможен.
...
- ВСАЕИНВАЛ `Bind`. сокет не привязан к.
...
Для сокетов типа SOCK_STREAM, так как в настоящее время доступен объем информации, доступной до размера переданного буфера, возвращается. Если сокет настроен для интерактивного приема данных по внештатному каналу (параметр сокета SO_OOBINLINE), а непрочтенные данные не считываются, возвращаются только данные, не являющиеся внешними. Приложение может использовать `IOCtlSIOCATMARK` параметр или [онаутофбанддата](#onoutofbanddata) , чтобы определить, остаются ли недоступные для чтения данные.

Для сокетов датаграмм данные извлекаются из первой поставленной в очередь датаграммы вплоть до размера переданного буфера. Если размер датаграммы превышает указанный буфер, то буфер заполняется первой частью датаграммы, лишние данные теряются и `Receive` возвращается значение SOCKET_ERROR с кодом ошибки, равным всаемсгсизе. Если на сокете недоступны входящие данные, возвращается значение SOCKET_ERROR с кодом ошибки ВСАЕВАУЛДБЛОКК. Функцию обратного вызова [OnReceive](#onreceive) можно использовать, чтобы определить, когда поступит больше данных.

Если сокет имеет тип SOCK_STREAM, а удаленная сторона правильно отключила подключение, то `Receive` завершается немедленно с полученным 0 байт. Если подключение было сброшено, `Receive` произойдет сбой с ошибкой всаеконнресет.
...
- ВСАЕНОТКОНН. сокет не подключен (только для SOCK_STREAM).

- ВСАЕНОТСОКК. дескриптор не является сокетом.

- Указан ВСАЕОПНОТСУПП MSG_OOB, но тип сокета не является SOCK_STREAM.

- ВСАЕШУТДОВН. Работа сокета была завершена; вызов `ReceiveFrom` через сокет после `ShutDown` вызова с параметром *нхов* , равным 0 или 2, невозможен.
...
Для сокетов датаграмм данные извлекаются из первой поставленной в очередь датаграммы вплоть до размера переданного буфера. Если размер датаграммы превышает указанный буфер, то буфер заполняется первой частью сообщения, лишние данные теряются и `ReceiveFrom` возвращается значение SOCKET_ERROR с кодом ошибки, равным всаемсгсизе.

Если *лпсоккаддр* имеет ненулевое значение, а сокет имеет тип SOCK_DGRAM, то сетевой адрес сокета, который отправил данные, копируется в соответствующую структуру [SOCKADDR](/windows/win32/winsock/sockaddr-2) . Значение, на которое указывает *лпсоккаддрлен* , инициализируется до размера этой структуры и изменяется при возврате для указания фактического размера адреса, хранящегося в ней. Если на сокете отсутствуют входящие данные, `ReceiveFrom` вызов ожидает поступления данных, если только сокет не блокируется. В этом случае возвращается значение SOCKET_ERROR с кодом ошибки, равным ВСАЕВАУЛДБЛОКК. Обратный `OnReceive` вызов можно использовать, чтобы определить, когда поступит больше данных.

Если сокет имеет тип SOCK_STREAM, а удаленная сторона правильно отключила подключение, то `ReceiveFrom` завершается немедленно с полученным 0 байт.
...
- ВСАЕШУТДОВН. Работа сокета была завершена; вызов `ReceiveFromEx` через сокет после `ShutDown` вызова с параметром *нхов* , равным 0 или 2, невозможен.
...
Для сокетов датаграмм данные извлекаются из первой поставленной в очередь датаграммы вплоть до размера переданного буфера. Если размер датаграммы превышает указанный буфер, то буфер заполняется первой частью сообщения, лишние данные теряются и `ReceiveFromEx` возвращается значение SOCKET_ERROR с кодом ошибки, равным всаемсгсизе.

Если *лпсоккаддр* имеет ненулевое значение, а сокет имеет тип SOCK_DGRAM, то сетевой адрес сокета, который отправил данные, копируется в соответствующую структуру [SOCKADDR](/windows/win32/winsock/sockaddr-2) . Значение, на которое указывает *лпсоккаддрлен* , инициализируется до размера этой структуры и изменяется при возврате для указания фактического размера адреса, хранящегося в ней. Если на сокете отсутствуют входящие данные, `ReceiveFromEx` вызов ожидает поступления данных, если только сокет не блокируется. В этом случае возвращается значение SOCKET_ERROR с кодом ошибки, равным ВСАЕВАУЛДБЛОКК. Обратный `OnReceive` вызов можно использовать, чтобы определить, когда поступит больше данных.

Если сокет имеет тип SOCK_STREAM, а удаленная сторона правильно отключила подключение, то `ReceiveFromEx` завершается немедленно с полученным 0 байт.
...
- MSG_OOB отправляют нестандартные данные (только SOCK_STREAM).
...
Обратите внимание, что для сокета датаграмм успешное `Send` завершение не указывает на то, что данные были успешно доставлены.
...
Обратите внимание, что успешное `SendTo` завершение не означает, что данные были успешно доставлены.
...
Чтобы отправить вещание (только на SOCK_DGRAM), адрес в параметре *лпсоккаддр* должен быть создан с помощью специального IP-адреса INADDR_BROADCAST (определенного в файле заголовка Windows Sockets в формате Winsock. H) вместе с требуемым номером порта. Или, если параметр *лпсзостаддресс* имеет значение null, сокет настраивается для вещания. Как правило, широковещательная датаграмма должна превысить размер, при котором может произойти фрагментация, что означает, что часть данных датаграммы (за исключением заголовков) не должна превышать 512 байт.
...
Обратите внимание, что успешное `SendToEx` завершение не означает, что данные были успешно доставлены.
...
- Истекло время ожидания подключения ВСАЕНЕТРЕСЕТ, когда SO_KEEPALIVE установлен.
...
- ВСАЕНОТКОНН подключение было сброшено при установке SO_KEEPALIVE.
...
Чтобы сообщить о реализации сокетов Windows, что `Bind` вызов на сокете не должен быть запрещен, так как нужный адрес уже используется другим сокетом, приложение должно установить параметр сокета SO_REUSEADDR для сокета перед выдачей `Bind` вызов метода. Обратите внимание, что параметр интерпретируется только во время `Bind` вызова: поэтому он не нужен (но является безопасным) для установки параметра на сокете, который не должен быть привязан к существующему адресу, и установки или сброса параметра `Bind` после вызова не влияет на этот или любой другой сокет.

Приложение может запросить, что реализация сокетов Windows позволяет использовать пакеты с поддержкой проверки активности для подключений по протоколу TCP, включив параметр сокета SO_KEEPALIVE. Реализация сокетов Windows не должна поддерживать использование проверки активности. Если это так, точная семантика зависит от реализации, но должна соответствовать разделу 4.2.3.6 RFC 1122: «Требования к узлам Интернета — уровни связи». Если соединение отбрасывается как результат "Проверка активности", код ошибки ВСАЕНЕТРЕСЕТ возвращается к любым выполняемым в сокете вызовам, а последующие вызовы будут завершаться с помощью ВСАЕНОТКОНН.

Параметр TCP_NODELAY отключает алгоритм Nagle. Алгоритм Nagle используется для сокращения числа небольших пакетов, отправляемых узлом путем буферизации неподтвержденной отправки данных до тех пор, пока не будет отправлен пакет полного размера. Однако для некоторых приложений этот алгоритм может снизить производительность, и TCP_NODELAY можно использовать для его отключения. Средства записи приложений не должны устанавливать TCP_NODELAY, если только это не будет хорошо понятным и желательным, поскольку Настройка TCP_NODELAY может оказать значительное негативное влияние на производительность сети. TCP_NODELAY является единственным поддерживаемым параметром сокета, который использует IPPROTO_TCP уровня; все остальные параметры используют уровень SOL_SOCKET.
...
|SO_DONTLINGER|BOOL|Не блокируйте `Close` Ожидание отправки неотправленных данных. Установка этого параметра эквивалентна установке параметра SO_LINGER с `l_onoff` нулевым значением.|
...
Параметры Berkeley Software Distribution (BSD) не поддерживаются `SetSockOpt` для:
...
Обратите `ShutDown` внимание, что не закрывает сокет, а ресурсы, подключенные к сокету, `Close` не будут освобождены до вызова метода. Приложение не должно полагаться на возможность повторного использования сокета после его завершения работы. В частности, реализация сокетов Windows не требуется для поддержки использования `Connect` на таком сокете.
...
- `FD_CONNECT`: Требуется получить уведомление о завершенном подключении.

- `FD_CLOSE`: Требуется получить уведомление о замыкании сокета.
...
Возвращает `TRUE` значение " `FALSE` успешно" в случае сбоя.
