...
|[CAsyncSocket:: Attach](#attach)|Присоединяет маркер сокета к объекту `CAsyncSocket`.|
...
|[CAsyncSocket::D етач](#detach)|Отсоединяет маркер сокета от объекта `CAsyncSocket`.|
...
|Имя|Description|
|----------|-----------------|
|[CAsyncSocket:: OnAccept](#onaccept)|Уведомляет сокет прослушивания о том, что он может принимать ожидающие запросы на подключение, вызывая `Accept`.|
...
|[CAsyncSocket:: OnReceive](#onreceive)|Уведомляет сокет прослушивания о том, что данные для извлечения выдаются путем вызова `Receive`.|
...
|Имя|Description|
|----------|-----------------|
|[CAsyncSocket:: operator =](#operator_eq)|Присваивает новое значение объекту `CAsyncSocket`.|
|[СОКЕТ CAsyncSocket:: operator](#operator_socket)|Этот оператор используется для получения маркера СОКЕТа объекта `CAsyncSocket`.|

### <a name="public-data-members"></a>Открытые элементы данных

|Имя|Description|
|----------|-----------------|
|[CAsyncSocket:: m_hSocket](#m_hsocket)|Указывает маркер СОКЕТа, присоединенный к этому объекту `CAsyncSocket`.|

## <a name="remarks"></a>Remarks

Класс `CAsyncSocket` инкапсулирует API функций Windows Socket, предоставляя объектно-ориентированную абстракцию для программистов, желающих использовать сокеты Windows совместно с MFC.
...
Чтобы использовать объект `CAsyncSocket`, вызовите его конструктор, а затем вызовите функцию [CREATE](#create) , чтобы создать базовый маркер сокета (тип `SOCKET`), за исключением принятых сокетов. Для сокета сервера вызывается функция-член [Listen](#listen) , а для сокета клиента вызывается функция-член [Connect](#connect) . Сокет сервера должен вызвать функцию [Accept](#accept) при получении запроса на соединение. Используйте оставшиеся функции `CAsyncSocket` для выполнения обмена данными между сокетами. После завершения удалите объект `CAsyncSocket`, если он был создан в куче. деструктор автоматически вызывает функцию [Close](#close) . Тип данных СОКЕТа описан в статье [Windows Sockets: Background](../../mfc/windows-sockets-background.md).
...
- ВСАЕИНВАЛ `Listen` не был вызван перед принятием.
...
Эта подпрограммы извлекает первое подключение в очереди ожидающих подключений, создает новый сокет с теми же свойствами, что и этот сокет, и прикрепляет его к *рконнектедсоккет*. Если в очереди отсутствуют ожидающие подключения, `Accept` возвращает ноль, а `GetLastError` возвращает ошибку. Принятый сокет ( *рконнектедсоккет)* нельзя использовать для приема дополнительных соединений. Исходный сокет остается открытым и прослушивается.

Аргумент *лпсоккаддр* — это параметр результата, который заполняется адресом соединяющего сокета, как известно на коммуникационном уровне. `Accept` используется с такими типами сокетов на основе соединений, как SOCK_STREAM.

##  <a name="asyncselect"></a>CAsyncSocket:: Асинкселект
...
Эта функция используется для указания того, какие функции уведомления обратного вызова MFC будут вызываться для сокета. `AsyncSelect` автоматически устанавливает этот сокет в неблокирующий режим. Дополнительные сведения см. в статье [сокеты Windows: уведомления сокетов](../../mfc/windows-sockets-socket-notifications.md).

##  <a name="attach"></a>CAsyncSocket:: Attach

Вызовите эту функцию члена, чтобы присоединить обработчик *хсоккет* к объекту `CAsyncSocket`.
...
Сетевой адрес, разделенный цифрой, например "128.56.22.8". Передача значения NULL для этого параметра указывает на то, что экземпляр `CAsyncSocket` должен прослушивать активность клиентов во всех сетевых интерфейсах.
...
Эта подпрограммы используется для неподключенных датаграмм или сокета потока, прежде чем последующие `Connect` или вызовы `Listen`. Прежде чем он сможет принимать запросы на подключение, сокет сервера прослушивания должен выбрать номер порта и сделать его известным сокетам Windows, вызвав `Bind`. `Bind` устанавливает локальное сопоставление (адрес узла или номер порта) сокета, назначая локальное имя неименованному сокету.
...
После создания объекта необходимо вызвать его `Create` функцию члена, чтобы создать структуру данных СОКЕТа и привязать ее адрес. (На стороне сервера при взаимодействии с сокетами Windows, когда сокет для прослушивания создает сокет для использования в вызове `Accept`, для этого сокета не нужно вызывать `Create`.)
...
Эта функция освобождает дескриптор сокета, чтобы дальнейшие ссылки на него завершились с ошибкой ВСАЕНОТСОКК. Если это последняя ссылка на базовый сокет, связанная информация об именовании и данные, поставленные в очередь, отбрасываются. Деструктор объекта сокета вызывает `Close`.

Для `CAsyncSocket`, но не для `CSocket`, семантика `Close` зависят от параметров сокета SO_LINGER и SO_DONTLINGER. Дополнительные сведения см. в разделе функция члена `GetSockOpt`.
...
Если сокет не привязан, уникальные значения назначаются в локальную ассоциацию системой, а сокет помечается как привязанный. Обратите внимание, что если поле адреса структуры имени равно нулю, `Connect` возвратит ноль. Чтобы получить расширенные сведения об ошибке, вызовите функцию члена `GetLastError`.
...
Для сокета датаграммы (тип SOCK_DGRAM) устанавливается назначение по умолчанию, которое будет использоваться при последующих вызовах `Send` и `Receive`.

##  <a name="create"></a>CAsyncSocket:: Create

Вызовите функцию-член `Create` после создания объекта сокета, чтобы создать сокет Windows и присоединить его.
...
*лпсзсоккаддресс*<br/>
Указатель на строку, содержащую сетевой адрес подключенного сокета, разделенный цифрой, например "128.56.22.8". Передача значения NULL для этого параметра указывает на то, что экземпляр `CAsyncSocket` должен прослушивать активность клиентов во всех сетевых интерфейсах.
...
##  <a name="detach"></a>CAsyncSocket::D етач

Вызовите эту функцию члена, чтобы отсоединить маркер СОКЕТа в *m_hSocketном* члене данных от `CAsyncSocket` объекта и задать для *m_hSocket* значение null.
...
Возвращает указатель на объект `CAsyncSocket`.
...
Указатель на объект `CAsyncSocket` или значение NULL, если к *хсоккет*не присоединен `CAsyncSocket` объект.

### <a name="remarks"></a>Remarks

Если при задании маркера СОКЕТа объект `CAsyncSocket` не присоединен к этому обработчику, функция-член возвращает значение NULL.
...
Если определенная функция-член указывает, что произошла ошибка, для получения соответствующего кода ошибки следует вызвать `GetLastError`. Список применимых кодов ошибок см. в описании отдельных функций-членов.
...
##  <a name="getpeername"></a>CAsyncSocket:: Жетпирнаме
...
*рпираддресс*<br/>
Ссылка на объект `CString`, который получает IP-адрес с точками в виде чисел.
...
##  <a name="getpeernameex"></a>CAsyncSocket:: Жетпирнамикс
...
##  <a name="getsockname"></a>CAsyncSocket:: Жетсоккнаме
...
Этот вызов особенно полезен, если `Connect` был выполнен вызов без предварительного `Bind`. Этот вызов предоставляет единственные средства, с помощью которых можно определить локальную связь, установленную системой.

Для работы с IPv6-адресами используйте [CAsyncSocket:: жетсоккнамикс](#getsocknameex)

##  <a name="getsocknameex"></a>CAsyncSocket:: Жетсоккнамикс
...
##  <a name="getsockopt"></a>CAsyncSocket:: Жетсоккопт
...
`GetSockOpt` извлекает текущее значение параметра сокета, связанного с сокетом любого типа, в любом состоянии и сохраняет результат в *лпоптионвалуе*. Параметры влияют на операции сокета, например маршрутизацию пакетов, передачу данных по внештатному каналу и т. д.

Для `GetSockOpt`поддерживаются следующие параметры. Тип определяет тип данных, адресованных *лпоптионвалуе*. Параметр TCP_NODELAY использует IPPROTO_TCP уровня; все остальные параметры используют SOL_SOCKET уровня.
...
Вызов `GetSockOpt` с неподдерживаемым параметром приведет к возвращению кода ошибки ВСАЕНОПРОТУПТ из `GetLastError`.
...
- ФИОНБИО включает или отключает неблокирующий режим на сокете. Параметр *лпаргумент* указывает на `DWORD`, который является ненулевым, если включен режим без блокировки, и нуль, если он должен быть отключен. Если `AsyncSelect` был выдан на сокете, любая попытка использовать `IOCtl` для переключения сокета в режим блокировки приведет к сбою с ВСАЕИНВАЛ. Чтобы вернуть сокет в режим блокировки и предотвратить ошибку ВСАЕИНВАЛ, приложение должно сначала отключить `AsyncSelect`, вызвав `AsyncSelect` с параметром *Левент* , равным 0, а затем вызвать `IOCtl`.

- ФИОНРЕАД определяет максимальное число байтов, которое может быть считано одним `Receive` вызовом из этого сокета. Параметр *лпаргумент* указывает на `DWORD`, в котором `IOCtl` сохраняет результат. Если этот сокет имеет тип SOCK_STREAM, ФИОНРЕАД возвращает общий объем данных, которые могут быть считаны в одном `Receive`. Обычно это то же самое, что и общий объем данных, поставленных в очередь на сокете. Если этот сокет имеет тип SOCK_DGRAM, ФИОНРЕАД возвращает размер первой датаграммы, поставленной в очередь на сокете.

- СИОКАТМАРК определяет, были ли считаны все данные по внешнему каналу. Это относится только к сокету типа SOCK_STREAM, которая настроена для интерактивного приема любых данных вне диапазона (SO_OOBINLINE). Если данные вне диапазона не ожидают чтения, операция возвращает ненулевое значение. В противном случае возвращается 0, а следующая `Receive` или `ReceiveFrom`, выполненные на сокете, извлекают некоторые или все данные, предшествующие "Mark". чтобы определить, остаются ли данные, приложение должно использовать операцию СИОКАТМАРК. Если имеются обычные данные, предшествующие "срочные" (внештатные) данные, они будут получены по порядку. (Обратите внимание, что `Receive` или `ReceiveFrom` в одном вызове никогда не будут смешиваться и обычные данные.) Параметр *лпаргумент* указывает на `DWORD`, в котором `IOCtl` сохраняет результат.
...
- ВСАЕОПНОТСУПП. ссылочный сокет не относится к типу, поддерживающему операцию `Listen`.

### <a name="remarks"></a>Remarks

Чтобы принимать подключения, сначала создается сокет с `Create`, невыполненной работой для входящих подключений указывается `Listen`, а затем эти подключения принимаются с `Accept`. `Listen` применяется только к сокетам, поддерживающим соединения, то есть к типу SOCK_STREAM. Этот сокет помещается в пассивный режим, в котором входящие подключения подтверждаются и ожидаются в очереди процессом.

Эта функция обычно используется серверами (или любым приложением, которым требуется принимать подключения), которые могут одновременно иметь несколько запросов на подключение. Если запрос на подключение поступает с очередью, клиент получит сообщение об ошибке с указанием ВСАЕКОННРЕФУСЕД.

`Listen` пытается продолжить работу, если нет доступных портов (дескрипторов). Он будет принимать подключения до тех пор, пока очередь не будет очищена. Если порты становятся доступными, то при последующем вызове `Listen` или `Accept` будет повторно заполнена очередь до текущей или самой последней "невыполненной работы", если это возможно, и возобновить прослушивание входящих подключений.

##  <a name="m_hsocket"></a>CAsyncSocket:: m_hSocket

Содержит маркер СОКЕТа для сокета, инкапсулированного этим объектом `CAsyncSocket`.
...
*нерроркоде*<br/>
Самая последняя ошибка на сокете. Следующие коды ошибок относятся к функции члена `OnAccept`:
...
*нерроркоде*<br/>
Самая последняя ошибка на сокете. Следующие коды ошибок относятся к функции члена `OnClose`:
...
*нерроркоде*<br/>
Самая последняя ошибка на сокете. Следующие коды ошибок относятся к функции члена `OnConnect`:
...
> [!NOTE]
>  В [CSocket](../../mfc/reference/csocket-class.md)функция уведомления `OnConnect` никогда не вызывается. Для соединений просто вызывается `Connect`, который будет возвращать, когда соединение завершается (успешно или по ошибке). Обработка уведомлений о соединении — это сведения о реализации MFC.
...
##  <a name="onoutofbanddata"></a>CAsyncSocket:: Онаутофбанддата
...
*нерроркоде*<br/>
Самая последняя ошибка на сокете. Следующие коды ошибок относятся к функции члена `OnOutOfBandData`:
...
MFC поддерживает данные вне диапазона, но пользователям класса `CAsyncSocket` не рекомендуется их использовать. Проще всего создать второй сокет для передачи таких данных. Дополнительные сведения о нестандартных данных см. в разделе [сокеты Windows: уведомления сокетов](../../mfc/windows-sockets-socket-notifications.md).

##  <a name="onreceive"></a>CAsyncSocket:: OnReceive

Вызывается платформой для уведомления этого сокета о том, что в буфере есть данные, которые можно извлечь путем вызова функции-члена `Receive`.
...
*нерроркоде*<br/>
Самая последняя ошибка на сокете. Следующие коды ошибок относятся к функции члена `OnReceive`:
...
Вызывается платформой для уведомления сокета о том, что теперь он может отправлять данные путем вызова функции члена `Send`.
...
*нерроркоде*<br/>
Самая последняя ошибка на сокете. Следующие коды ошибок относятся к функции члена `OnSend`:
...
Присваивает новое значение объекту `CAsyncSocket`.
...
*rSrc*<br/>
Ссылка на существующий объект `CAsyncSocket`.

### <a name="remarks"></a>Remarks

Вызовите эту функцию, чтобы скопировать существующий объект `CAsyncSocket` в другой объект `CAsyncSocket`.

##  <a name="operator_socket"></a>СОКЕТ CAsyncSocket:: operator

Этот оператор используется для получения маркера СОКЕТа объекта `CAsyncSocket`.
...
*нфлагс*<br/>
Указывает способ вызова. Семантика этой функции определяется параметрами сокета и параметром *нфлагс* . Второй вариант создается путем объединения любого из следующих значений с C++ оператором **или** :
...
- ВСАЕШУТДОВН. Работа сокета была завершена; невозможно вызвать `Receive` на сокете после вызова `ShutDown` с параметром *нхов* , равным 0 или 2.

- ВСАЕВАУЛДБЛОКК. сокет помечается как неблокируемая, а операция `Receive` блокируется.
...
Для сокетов типа SOCK_STREAM, сколько информации в настоящее время доступно вплоть до размера переданного буфера, возвращается. Если сокет настроен для интерактивного приема нестандартных данных (параметр сокета SO_OOBINLINE), а непрочтенные данные не считываются, возвращаются только данные, не являющиеся внешними. Приложение может использовать параметр `IOCtlSIOCATMARK` или [онаутофбанддата](#onoutofbanddata) , чтобы определить, будут ли доступны дополнительные данные для чтения.

Для сокетов датаграмм данные извлекаются из первой поставленной в очередь датаграммы вплоть до размера переданного буфера. Если датаграмма превышает указанный буфер, буфер заполняется первой частью датаграммы, лишние данные теряются, а `Receive` возвращает значение SOCKET_ERROR с кодом ошибки, установленным в ВСАЕМСГСИЗЕ. Если на сокете недоступны входящие данные, возвращается значение SOCKET_ERROR с кодом ошибки, равным ВСАЕВАУЛДБЛОКК. Функцию обратного вызова [OnReceive](#onreceive) можно использовать, чтобы определить, когда поступит больше данных.

Если сокет имеет тип SOCK_STREAM и удаленная сторона успешно отключила подключение, `Receive` будет выполняться немедленно с полученным 0 байт. Если подключение было сброшено, `Receive` завершится ошибкой с ошибкой ВСАЕКОННРЕСЕТ.

`Receive` следует вызывать только один раз для каждого вызова [CAsyncSocket:: OnReceive](#onreceive) .

### <a name="example"></a>Пример

  См. пример для [CAsyncSocket:: OnReceive](#onreceive).

##  <a name="receivefrom"></a>CAsyncSocket:: Рецеивефром
...
- ВСАЕШУТДОВН. Работа сокета была завершена; невозможно вызвать `ReceiveFrom` на сокете после вызова `ShutDown` с параметром *нхов* , равным 0 или 2.

- ВСАЕВАУЛДБЛОКК. сокет помечается как неблокируемая, а операция `ReceiveFrom` блокируется.
...
Для сокетов типа SOCK_STREAM, сколько информации в настоящее время доступно вплоть до размера переданного буфера, возвращается. Если сокет настроен для интерактивного приема нестандартных данных (параметр сокета SO_OOBINLINE), а непрочтенные данные не считываются, возвращаются только данные, не являющиеся внешними. Приложение может использовать параметр `IOCtlSIOCATMARK` или `OnOutOfBandData`, чтобы определить, будут ли доступны дополнительные данные для чтения. Параметры *лпсоккаддр* и *лпсоккаддрлен* игнорируются для сокетов SOCK_STREAM.

Для сокетов датаграмм данные извлекаются из первой поставленной в очередь датаграммы вплоть до размера переданного буфера. Если датаграмма превышает указанный буфер, буфер заполняется первой частью сообщения, лишние данные теряются, а `ReceiveFrom` возвращает значение SOCKET_ERROR с кодом ошибки, установленным в ВСАЕМСГСИЗЕ.

Если *лпсоккаддр* имеет ненулевое значение и сокет относится к типу SOCK_DGRAM, то сетевой адрес сокета, который отправил данные, копируется в соответствующую структуру [SOCKADDR](/windows/win32/winsock/sockaddr-2) . Значение, на которое указывает *лпсоккаддрлен* , инициализируется до размера этой структуры и изменяется при возврате для указания фактического размера адреса, хранящегося в ней. Если на сокете отсутствуют входящие данные, вызов `ReceiveFrom` ожидает поступления данных, если только сокет не блокируется. В этом случае возвращается значение SOCKET_ERROR с кодом ошибки, установленным в ВСАЕВАУЛДБЛОКК. Чтобы определить, когда поступит больше данных, можно использовать обратный вызов `OnReceive`.

Если сокет имеет тип SOCK_STREAM и удаленная сторона успешно отключила подключение, `ReceiveFrom` будет выполняться немедленно с полученным 0 байт.

##  <a name="receivefromex"></a>CAsyncSocket:: Рецеивефромекс
...
- ВСАЕШУТДОВН. Работа сокета была завершена; невозможно вызвать `ReceiveFromEx` на сокете после вызова `ShutDown` с параметром *нхов* , равным 0 или 2.

- ВСАЕВАУЛДБЛОКК. сокет помечается как неблокируемая, а операция `ReceiveFromEx` блокируется.
...
Для сокетов датаграмм данные извлекаются из первой поставленной в очередь датаграммы вплоть до размера переданного буфера. Если датаграмма превышает указанный буфер, буфер заполняется первой частью сообщения, лишние данные теряются, а `ReceiveFromEx` возвращает значение SOCKET_ERROR с кодом ошибки, установленным в ВСАЕМСГСИЗЕ.

Если *лпсоккаддр* имеет ненулевое значение и сокет относится к типу SOCK_DGRAM, то сетевой адрес сокета, который отправил данные, копируется в соответствующую структуру [SOCKADDR](/windows/win32/winsock/sockaddr-2) . Значение, на которое указывает *лпсоккаддрлен* , инициализируется до размера этой структуры и изменяется при возврате для указания фактического размера адреса, хранящегося в ней. Если на сокете отсутствуют входящие данные, вызов `ReceiveFromEx` ожидает поступления данных, если только сокет не блокируется. В этом случае возвращается значение SOCKET_ERROR с кодом ошибки, установленным в ВСАЕВАУЛДБЛОКК. Чтобы определить, когда поступит больше данных, можно использовать обратный вызов `OnReceive`.

Если сокет имеет тип SOCK_STREAM и удаленная сторона успешно отключила подключение, `ReceiveFromEx` будет выполняться немедленно с полученным 0 байт.
...
- ВСАЕШУТДОВН. Работа сокета была завершена; невозможно вызвать `Send` на сокете после вызова `ShutDown` с параметром *нхов* , равным 1 или 2.
...
`Send` используется для записи исходящих данных в подключенном потоке или сокетах датаграмм. Для сокетов датаграмм необходимо соблюдать осторожность, чтобы превысить максимальный размер пакета IP-адресов базовых подсетей, который определяется элементом `iMaxUdpDg` в структуре [всадата](/windows/win32/api/winsock2/ns-winsock2-wsadata) , возвращаемой `AfxSocketInit`. Если данные слишком длинны для передачи атомарным образом через базовый протокол, то ошибка ВСАЕМСГСИЗЕ возвращается через `GetLastError`и никакие данные не передаются.

Обратите внимание, что для сокета датаграммы успешное завершение `Send` не указывает на то, что данные были успешно доставлены.

В `CAsyncSocket` объектов типа SOCK_STREAM число записанных байтов может быть в диапазоне от 1 до запрошенной длины в зависимости от доступности буфера как на локальном, так и на инородном узлах.
...
- ВСАЕШУТДОВН. Работа сокета была завершена; невозможно вызвать `SendTo` на сокете после вызова `ShutDown` с параметром *нхов* , равным 1 или 2.
...
`SendTo` используется в сокетах датаграммы и потока и используется для записи исходящих данных на сокете. Для сокетов датаграмм необходимо соблюдать осторожность, чтобы превысить максимальный размер пакета IP-адресов базовых подсетей, который определяется элементом `iMaxUdpDg` в структуре [всадата](/windows/win32/api/winsock2/ns-winsock2-wsadata) , заполненной [афкссоккетинит](../../mfc/reference/application-information-and-management.md#afxsocketinit). Если данные слишком длинны для передачи атомарным образом через базовый протокол, возвращается ошибка ВСАЕМСГСИЗЕ и данные не передаются.
...
##  <a name="sendtoex"></a>CAsyncSocket:: Сендтоекс
...
- ВСАЕШУТДОВН. Работа сокета была завершена; невозможно вызвать `SendToEx` на сокете после вызова `ShutDown` с параметром *нхов* , равным 1 или 2.
...
`SendToEx` используется в сокетах датаграммы и потока и используется для записи исходящих данных на сокете. Для сокетов датаграмм необходимо соблюдать осторожность, чтобы превысить максимальный размер пакета IP-адресов базовых подсетей, который определяется элементом `iMaxUdpDg` в структуре [всадата](/windows/win32/api/winsock2/ns-winsock2-wsadata) , заполненной [афкссоккетинит](../../mfc/reference/application-information-and-management.md#afxsocketinit). Если данные слишком длинны для передачи атомарным образом через базовый протокол, возвращается ошибка ВСАЕМСГСИЗЕ и данные не передаются.
...
##  <a name="setsockopt"></a>CAsyncSocket:: Сетсоккопт
...
SO_LINGER управляет действием, выполняемым, когда неотправленные данные помещаются в очередь на сокете, а для закрытия сокета вызывается функция `Close`.
...
Чтобы сообщить о реализации сокетов Windows, что вызов `Bind` на сокете не должен быть запрещен, так как нужный адрес уже используется другим сокетом, приложение должно установить параметр сокета SO_REUSEADDR для сокета перед выдачей вызова `Bind`. Обратите внимание, что параметр интерпретируется только во время вызова `Bind`. Поэтому он не нужен (но является безопасным) для установки параметра на сокете, который не привязан к существующему адресу, и установки или сброса параметра после вызова `Bind` не влияет на этот или любой другой сокет.
...
Для `SetSockOpt`поддерживаются следующие параметры. Тип определяет тип данных, адресованных *лпоптионвалуе*.

|Значение|Тип|Значение|
|-----------|----------|-------------|
|SO_BROADCAST|BOOL|Разрешить передачу широковещательных сообщений на сокет.|
|SO_DEBUG|BOOL|Записать отладочную информацию.|
|SO_DONTLINGER|BOOL|Не блокируйте `Close`, ожидающие отправки неотправленных данных. Установка этого параметра эквивалентна установке SO_LINGER с параметром `l_onoff` значение 0.|
|SO_DONTROUTE|BOOL|Не маршрутизировать: прямая отправка в интерфейс.|
|SO_KEEPALIVE|BOOL|Отправка проверки активности.|
|SO_LINGER|`struct LINGER`|Ожидание на `Close`, если отсутствуют неотправленные данные.|
...
Этот метод выделяет маркер сокета. Он не вызывает [CAsyncSocket:: BIND](#bind) для привязки сокета к указанному адресу, поэтому необходимо вызвать `Bind` позже, чтобы привязать сокет к указанному адресу. Чтобы задать параметр сокета перед его привязкой, можно использовать [CAsyncSocket:: сетсоккопт](#setsockopt) .
