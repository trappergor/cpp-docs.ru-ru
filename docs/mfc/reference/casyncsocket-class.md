...
Дополнительные сведения см. в разделе [сокеты Windows: Использование класса CAsyncSocket](../../mfc/windows-sockets-using-class-casyncsocket.md) и связанных статей., а также [Windows Sockets 2 API](/windows/desktop/WinSock/windows-sockets-start-page-2).
...
*lpSockAddr*<br/>
Указатель на [SOCKADDR](/windows/desktop/winsock/sockaddr-2) структуры, получающей адрес подключения сокета, как известные в сети. Точный формат *lpSockAddr* аргумент определяется семейство адресов, определяются, когда сокет был создан. Если *lpSockAddr* и/или *lpSockAddrLen* равны NULL, то данные об удаленном адресе допущенный сокет не возвращаются.
...
- WSAEFAULT *lpSockAddrLen* аргумент слишком мал (меньше, чем размер [SOCKADDR](/windows/desktop/winsock/sockaddr-2) структуры).
...
Эта функция используется для указания, какие функции MFC обратного вызова уведомления будет вызываться для сокета. `AsyncSelect` автоматически устанавливает этот сокет в неблокирующем режиме. Дополнительные сведения см. в статье [сокеты Windows: Сокета уведомления](../../mfc/windows-sockets-socket-notifications.md).
...
*lpSockAddr*<br/>
Указатель на [SOCKADDR](/windows/desktop/winsock/sockaddr-2) структуру, содержащую адрес, связываемый с этим сокетом.
...
- WSAEFAULT *nSockAddrLen* аргумент слишком мал (меньше, чем размер [SOCKADDR](/windows/desktop/winsock/sockaddr-2) структуры).
...
*lpSockAddr*<br/>
Указатель на [SOCKADDR](/windows/desktop/winsock/sockaddr-2) структуру, содержащую адрес подключенного сокета.
...
Дополнительные сведения о stream и датаграмм сокетов, см. в статьях [сокеты Windows: Фон](../../mfc/windows-sockets-background.md) и [сокеты Windows: Порты и адреса сокета](../../mfc/windows-sockets-ports-and-socket-addresses.md) и [Windows Sockets 2 API](/windows/desktop/WinSock/windows-sockets-start-page-2).
...
*lpSockAddr*<br/>
Указатель на [SOCKADDR](/windows/desktop/winsock/sockaddr-2) структуры, который получает имя однорангового узла сокета.
...
*lpSockAddr*<br/>
Указатель на [SOCKADDR](/windows/desktop/winsock/sockaddr-2) структуры, получающей адрес сокета.
...
Дополнительные сведения см. в разделе [сокеты Windows: Сокета уведомления](../../mfc/windows-sockets-socket-notifications.md).
...
MFC поддерживает внешнее данные, но пользователи класса `CAsyncSocket` не рекомендуется использовать его. Простой способ — создать второй сокет для передачи таких данных. Дополнительные сведения о данных из внешнего, см. в разделе [сокеты Windows: Сокета уведомления](../../mfc/windows-sockets-socket-notifications.md).
...
Вызов этой функции-члена для получения датаграммы и сохранения исходный адрес в [SOCKADDR](/windows/desktop/winsock/sockaddr-2) структуры или в *rSocketAddress*.
...
*lpSockAddr*<br/>
Указатель на [SOCKADDR](/windows/desktop/winsock/sockaddr-2) структура, которая содержит адрес источника при возврате.
...
Вызов этой функции-члена для получения датаграммы и сохранения исходный адрес в [SOCKADDR](/windows/desktop/winsock/sockaddr-2) структуры или в *rSocketAddress* (обрабатывает IPv6-адресов).
...
`Send` используется для записи выходных данных на подключенном поток или сокеты датаграмм. Для сокета датаграмм, будьте осторожны не должно превышать максимальный размер пакета IP-адрес базовой подсетей, который определяется `iMaxUdpDg` элемент в [WSADATA](/windows/desktop/api/winsock2/ns-winsock2-wsadata) структура, возвращенная `AfxSocketInit`. Если данные слишком длинное для прохождения базового протокола атомарным образом, WSAEMSGSIZE возвращается ошибка через `GetLastError`, и данные не передаются.
...
*lpSockAddr*<br/>
Указатель на [SOCKADDR](/windows/desktop/winsock/sockaddr-2) структуру, содержащую адрес сокета целевой объект.
...
- WSAEFAULT *lpBuf* или *lpSockAddr* параметров не являются частью адресном пространстве пользователя или *lpSockAddr* аргумент слишком мал (меньше, чем размер [SOCKADDR](/windows/desktop/winsock/sockaddr-2) структуры).
...
`SendTo` используется сокеты датаграмм или поток и используется для записи выходных данных на сокете. Для сокета датаграмм, будьте осторожны не должно превышать максимальный размер пакета IP-адрес базовой подсетей, который определяется `iMaxUdpDg` элемент в [WSADATA](/windows/desktop/api/winsock2/ns-winsock2-wsadata) структуры для заполнения [AfxSocketInit](../../mfc/reference/application-information-and-management.md#afxsocketinit). Если данные слишком длинное для прохождения базового протокола атомарным образом, будет возвращена ошибка WSAEMSGSIZE и данные не передаются.
...
`SendToEx` используется сокеты датаграмм или поток и используется для записи выходных данных на сокете. Для сокета датаграмм, будьте осторожны не должно превышать максимальный размер пакета IP-адрес базовой подсетей, который определяется `iMaxUdpDg` элемент в [WSADATA](/windows/desktop/api/winsock2/ns-winsock2-wsadata) структуры для заполнения [AfxSocketInit](../../mfc/reference/application-information-and-management.md#afxsocketinit). Если данные слишком длинное для прохождения базового протокола атомарным образом, будет возвращена ошибка WSAEMSGSIZE и данные не передаются.
...
- `FD_OOB`: Хотите получать уведомление о появлении-каналу данных.
