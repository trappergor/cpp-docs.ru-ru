...
|[CAsyncSocket::ShutDown](#shutdown)|Отключает `Send` и/или `Receive` вызывает для сокета.|  
...
|[CAsyncSocket::OnSend](#onsend)|Уведомляет сокета, что его можно отправить данные путем вызова `Send`.|  
...
### <a name="remarks"></a>Примечания  
 ...
 Аргумент *lpSockAddr* является параметр результата, который заполняется с использованием адреса подключения сокета, как известно, уровня коммуникаций. `Accept` используется с типами сокета на основе подключения, например **SOCK_STREAM**.  
...
### <a name="remarks"></a>Примечания  
 Эта процедура используется в несвязанной датаграмм или сокета потока перед последующих `Connect` или `Listen` вызовов. Прежде чем он может принимать запросы на подключение, прослушивающего сокета сервера необходимо выберите номер порта и сделать ее известных Windows Sockets, вызвав `Bind`. `Bind` Устанавливает локальную ассоциацию (номер адреса и порта узла) сокета, назначив гнездо неименованные локальное имя.  
...
### <a name="remarks"></a>Примечания  
 После создания объекта, необходимо вызвать его `Create` функции-члена для создания **СОКЕТА** структуры данных и привязать ее адрес. (На стороне сервера Windows Sockets связи, когда прослушивающего сокета создает сокет для использования в `Accept` вызова, не нужно вызывать `Create` для этого сокета.)  
...
### <a name="remarks"></a>Примечания  
 Эта функция освобождает дескриптор сокета, чтобы дополнительные ссылки на него завершится с ошибкой **WSAENOTSOCK**. Если это последняя ссылка на основном сокете, связанной с ними информации об именах и очереди данных удаляются. Вызывает деструктор объекта сокета `Close` для вас.  
  
 Для `CAsyncSocket`, но не для `CSocket`, семантика `Close` затронутые параметры сокетов **SO_LINGER** и **SO_DONTLINGER**. Дополнительные сведения см. в разделе функции-члена `GetSockOpt`.  
...
- **WSAENOBUFS** нет места в буфере. Сокет не подключен.  
...
### <a name="remarks"></a>Примечания  
 ...
 Для сокета датаграмм (тип **SOCK_DGRAM**), задать назначение по умолчанию, который будет использоваться на последующих `Send` и `Receive` вызовов.  
  
##  <a name="create"></a>  CAsyncSocket::Create  
 Вызовите `Create` после создания объекта сокета Создание сокетов Windows и присоединение его функции-члена.  
...
- **WSAENOBUFS** нет места в буфере. Не удается создать сокет.  
...
### <a name="remarks"></a>Примечания  
 `Create` вызовы [сокета](#socket) и в случае успеха, он вызывает [привязки](#bind) привязке сокета по указанному адресу. Поддерживаются следующие типы сокета:  
...
- **SOCK_DGRAM** поддерживает датаграммы — пакеты без установления соединения, ненадежные фиксированной (обычно малой) максимальной длиной. Использует протокол UDP (User Datagram) для семейства адресов Интернета.  
    ...
    > [!NOTE]
    >  `Accept` Функции-члена принимает ссылку на новый, пустой `CSocket` объект в качестве параметра. Этот объект необходимо создать перед вызовом метода `Accept`. Имейте в виду, что если этот объект сокета идет области видимости, соединение закрывается. Не следует вызывать `Create` для этого нового объекта сокета.  
  
> [!IMPORTANT]
> `Create` — **не** потокобезопасным.  При вызове он в многопоточной среде которых он может быть применено одновременно различными потоками, не забудьте защитить каждый вызов с мьютекс или другие блокировки синхронизации.  
...
##  <a name="detach"></a>  CAsyncSocket::Detach  
 Вызовите эту функцию-член для отсоединения **СОКЕТА** обработки в *m_hSocket* элемент данных из `CAsyncSocket` и задайте *m_hSocket* для **значение NULL** .  
...
- **WSAEINVAL** сокета не привязано к адрес с `Bind`.  
  
### <a name="remarks"></a>Примечания  
 Этот вызов особенно полезна при `Connect` вызова без это `Bind` , этот вызов предоставляет единственное средство, с помощью которого можно определить локальную ассоциацию, которое было задано системой.  
...
- **WSAEINVAL** *lCommand* не является допустимой команды, или *lpArgument* не является допустимым параметром для *lCommand*, или команда не применяется к тип сокета указано.  
...
- **WSAEINVAL** сокета не были связаны со `Bind` или уже подключен.  
...
> [!NOTE]
>  В [CSocket](../../mfc/reference/csocket-class.md), `OnConnect` функция уведомления никогда не вызывается. Для соединений, можно просто вызвать `Connect`, который возвращает при завершении соединения (успешно или с ошибкой). Обработки уведомления соединения является реализации MFC.  
 ...
 *nFlags*  
 Указывает способ, в котором был сделан вызов. Семантика этой функции определяется параметрами сокета и *nFlags* параметра. Последний создается путем объединения любой из следующих значений с C++ **или** оператор:  
...
- **WSAEINVAL** сокета не были связаны со `Bind`.  
...
### <a name="remarks"></a>Примечания  
 ...
 Для сокеты датаграмм данные извлекаются из первой датаграммы из очереди, вплоть до размер предоставленного буфера. Если датаграмма превышает размер буфера, предоставленного, буфер заполняется в первой части датаграмма, избыточные данные будут потеряны, и `Receive` возвращает значение **SOCKET_ERROR** с кодом ошибки присвоено  **WSAEMSGSIZE**. Если входящие данные недоступны на сокет, значение **SOCKET_ERROR** возвращается с кодом ошибки **WSAEWOULDBLOCK**. [OnReceive](#onreceive) функция обратного вызова можно использовать для определения того, при поступлении дополнительных данных.  
...
- **WSAESHUTDOWN** сокета была завершена; невозможно вызвать `Send` на сокете после `ShutDown` был вызван с *nHow* в значение 1 или 2.  
...
### <a name="remarks"></a>Примечания  
 `Send` используется для записи выходных данных на подключенный поток или сокеты датаграмм. Для сокеты датаграмм, необходимо соблюдать осторожность не должно превышать максимальный размер пакета IP базовой подсетей, которая задана по **iMaxUdpDg** элемент в [WSADATA](../../mfc/reference/wsadata-structure.md) структуры, возвращенный `AfxSocketInit`. Если данные слишком много атомарным образом проходить через базовый протокол ошибки **WSAEMSGSIZE** возвращаться через `GetLastError`, и данные не передаются.  
  
 Обратите внимание, что датаграммы сокета успешному `Send` не указывает данные был успешно доставлен.  
...
- **WSAEFAULT** *lpBuf* или *lpSockAddr* параметров не являются частью адресного пространства пользователя, или *lpSockAddr* аргумент слишком мал (меньше, чем размер из [SOCKADDR](../../mfc/reference/sockaddr-structure.md) структуры).  
 ...
 **SO_LINGER** действие, выполняемое при неотправленные данные в очереди на сокете элементов управления и `Close` функция, вызываемая для закрытия сокета.  
 ...
 Для информирования реализация Windows Sockets, `Bind` вызов на сокет не должны быть запрещены, так как нужный адрес уже используется другой сокета, приложение должно задать **SO_REUSEADDR** параметра сокета для сокета перед выдачей `Bind` вызова. Обратите внимание, что параметр интерпретируется только во время `Bind` вызова: он необязателен таким образом (но опасное) для задания параметра на сокете, которой не должен быть привязан к существующего адреса, и установка или Сброс параметров после `Bind` вызов не влияет на это или какому сокету.  
