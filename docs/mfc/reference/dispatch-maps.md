...
|Отправить макрос с картой|Description|
|-|-|
...
Если класс, производный от `CCmdTarget`в программе, поддерживает OLE-автоматизацию, этот класс должен предоставлять карту диспетчеризации для предоставления своих методов и свойств.
...
Аргумент *втретвал* имеет тип VarType. Следующие возможные значения для этого аргумента взяты из перечисления `VARENUM`:
...
Аргумент *втспарамс* представляет собой разделенный пробелами список значений из констант `VTS_*`. Одно или несколько из этих значений, разделенных пробелами (а не запятыми), задает список параметров функции. Например,

[!code-cpp[NVC_MFCAutomation#14](../../mfc/codesnippet/cpp/dispatch-maps_2.cpp)]

Указывает список, содержащий короткое целое число, за которым следует указатель на короткое целое число.

Константы `VTS_` и их значения приведены ниже.
...
|VTS_R4|**float**|
|VTS_R8|**double**|
|VTS_CY|`const CY` либо `CY*`|
|VTS_DATE|DATE|
|VTS_BSTR|LPCSTR|
|VTS_DISPATCH|лпдиспатч|
|VTS_SCODE|SCODE|
|VTS_BOOL|BOOL|
|VTS_VARIANT|`const VARIANT*` либо `VARIANT&`|
|VTS_UNKNOWN|лпункновн|
|VTS_PI2|__короткий\*__|
|VTS_PI4|__длинный\*__|
|VTS_PR4|__\* с плавающей запятой__|
|VTS_PR8|__Double\*__|
|VTS_PCY|`CY*`|
|VTS_PDATE|`DATE*`|
|VTS_PBSTR|`BSTR*`|
...
Определяет свойство, доступ к которому осуществляется с помощью отдельных функций-членов `Get` и `Set`.
...
*втспарамс*<br/>
Строка с разделенными пробелами `VTS_*` типов параметров Variant, по одному для каждого параметра.
