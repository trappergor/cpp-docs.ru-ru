...
|name|Описание|
|----------|-----------------|
|[Цинтернетфиле:: Цинтернетфиле](#cinternetfile)|Создает объект `CInternetFile`.|
...
`CInternetFile` Функции элементов,`LockRange` ,и`Duplicate` не реализованы для .`CInternetFile` `UnlockRange` `Open` При вызове этих функций для `CInternetFile` объекта вы получите [кнотсуппортедексцептион](../../mfc/reference/cnotsupportedexception-class.md).
...
При обработке исключений `Abort` отличается от [закрытия](#close) двумя важными способами. Во-первых `Abort` , функция не создает исключение в случае сбоев, поскольку оно игнорирует сбои. Во вторых `Abort` , не выполняет **утверждение** , если файл не был открыт или был закрыт ранее.
...
Если файл был открыт для записи, вызывается неявный вызов функции [flush](#flush) , чтобы гарантировать запись всех буферизованных данных на узел. По завершении `Close` использования файла следует вызвать метод.
...
Функция помещает полученную строку в память, на которую ссылается параметр *ПСТР* . Он прекращает чтение символов при переходе в максимальное количество символов, определяемое *Nмакс*. Буфер всегда получает завершающий нуль-символ.
...
*Nиз*<br/>
Относительная ссылка для смещения. Должно иметь одно из следующих значений:
...
Базовые API-интерфейсы WinInet не выполняют буферизацию, поэтому выбирайте размер буфера, позволяющий приложению эффективно считывать данные независимо от объема считываемых данных. Если при каждом вызове метода [Read](#read) обычно используется большой ааунт данных (например, четыре или более килобайта), то буфер не нужен. Однако при вызове для `Read` получения небольших фрагментов данных или при использовании метода [ReadString](#readstring) для считывания отдельных строк за раз буфер чтения повышает производительность приложения.
