...
|name|Описание|
|----------|-----------------|
|[Кдравингманажер:: Кдравингманажер](#cdrawingmanager)|Создает объект `CDrawingManager`.|
...
|name|Описание|
|----------|-----------------|
|[CDrawingManager::CreateBitmap_32](#createbitmap_32)|32 создает независимый от устройства точечный рисунок (DIB), который приложения могут записывать непосредственно.|
...
|[CDrawingManager::SmartMixColors](#smartmixcolors)|Объединяет два цвета на основе взвешенного соотношения.|
...
##  <a name="cdrawingmanager"></a>Кдравингманажер:: Кдравингманажер
...
##  <a name="createbitmap_32"></a>Кдравингманажер:: CreateBitmap_32
...
|-|-|
|Параметр|Описание|
|*size*|окне Параметр [ксизе](../../atl-mfc-shared/reference/csize-class.md) , указывающий размер точечного рисунка.|
|*пбитс*|заполняет Указатель на указатель данных, получающий расположение битовых значений DIB.|
|*массив*|Маркер исходного точечного рисунка|
|*клртранспарент*|Значение RGB, указывающее прозрачный цвет исходного растрового изображения.|
...
##  <a name="drawalpha"></a>Кдравингманажер::D Равалфа
...
##  <a name="drawellipse"></a>Кдравингманажер::D Равеллипсе
...
##  <a name="drawgradientring"></a>Кдравингманажер::D Равградиентринг
...
##  <a name="drawline_cdrawingmanager__drawlinea"></a>Кдравингманажер::D Равлине, Кдравингманажер::D Равлинеа
...
|-|-|
|Параметр|Описание|
|*x1*|окне Координата x, с которой начинается строка.|
|*y1*|окне Координата y, с которой начинается строка.|
|*штук*|окне Координата x, на которой заканчивается линия.|
|*Y2*|окне Координата y, на которой заканчивается линия.|
|*клрлине*|окне Цвет линии.|

### <a name="remarks"></a>Примечания

Этот метод завершается ошибкой, если *клрлине* равен-1.

##  <a name="drawrect"></a>Кдравингманажер::D Раврект
...
##  <a name="drawshadow"></a>Кдравингманажер::D Равшадов
...
*бригхтшадов*<br/>
окне Логический параметр, который указывает, как рисуется тень. Если *бригхтшадов* имеет `TRUE`значение `DrawShadow` , рисует тень с правой стороны прямоугольника.
...
Можно указать два допустимых точечных рисунка для нижних и правых теней с помощью параметров *пбмпсавеботтом* и *пбмпсаверигхт*. Если эти объекты [CBitmap](../../mfc/reference/cbitmap-class.md) имеют присоединенный объект GDI, `DrawShadow` будут использовать эти растровые изображения в качестве теней. Если параметры не имеют присоединенного объекта GDI, `DrawShadow` рисует тень и присоединяет растровые изображения к параметрам. `CBitmap` В будущих вызовах `DrawShadow`можно предоставить эти точечные рисунки для ускорения процесса рисования. Дополнительные сведения о классах `CBitmap` и объектах GDI см. в разделе [графические объекты](../../mfc/graphic-objects.md).

Если один из этих параметров имеет `NULL`значение `DrawShadow` , автоматически рисует тень.

Если задать для *бригхтшадов* значение false, тень будет отображаться под областью и слева от прямоугольной области.

### <a name="example"></a>Пример

В следующем примере показано, `DrawShadow` как использовать метод `CDrawingManager` класса. Этот фрагмент кода является частью демонстрационного [примера на листе Prop](../../overview/visual-cpp-samples.md).

[!code-cpp[NVC_MFC_PropSheetDemo#1](../../mfc/reference/codesnippet/cpp/cdrawingmanager-class_1.cpp)]

##  <a name="fill4colorsgradient"></a>Кдравингманажер:: Fill4ColorsGradient
...
*бхорз*<br/>
окне Логический параметр, указывающий, `Fill4ColorsGradient` является ли цвет цветом горизонтальным или вертикальным градиентом. Значение TRUE указывает горизонтальный градиент.
...
##  <a name="fillgradient"></a>Кдравингманажер:: Филлградиент
...
В следующем примере показано, `FillGradient` как использовать метод `CDrawingManager` класса. Этот фрагмент кода является частью демонстрационного [примера MS Office 2007](../../overview/visual-cpp-samples.md).

[!code-cpp[NVC_MFC_MSOffice2007Demo#12](../../mfc/reference/codesnippet/cpp/cdrawingmanager-class_2.cpp)]

##  <a name="fillgradient2"></a>Кдравингманажер:: FillGradient2
...
В следующем примере показано, `FillGradient2` как использовать метод `CDrawingManager` класса. Этот фрагмент кода является частью [примера новых элементов управления](../../overview/visual-cpp-samples.md).

[!code-cpp[NVC_MFC_NewControls#37](../../mfc/reference/codesnippet/cpp/cdrawingmanager-class_3.cpp)]

##  <a name="grayrect"></a>Кдравингманажер:: Грайрект
...
##  <a name="highlightrect"></a>Кдравингманажер:: Хигхлигхтрект
...
##  <a name="hlstorgb_one"></a>Кдравингманажер:: HLStoRGB_ONE
...
Этот метод и `CDrawingManager::HLStoRGB_TWO` метод выполняют одну и ту же операцию, но для параметра *H* требуются разные значения. В этом методе *H* — это процентная доля круга. В методе H — это значение градуса от 0 до 360, которое представляет собой красный цвет. `CDrawingManager::HLStoRGB_TWO` Например `HLStoRGB_ONE`, значение 0,25 для *H* эквивалентно значению 90 с `HLStoRGB_TWO`.

##  <a name="hlstorgb_two"></a>Кдравингманажер:: HLStoRGB_TWO
...
##  <a name="hsvtorgb"></a>Кдравингманажер:: Хсвторгб
...
|-|-|
|Параметр|Описание|
|*H*|окне Число от 0 до 360, указывающее цветовой тон цвета.|
|*S*|окне Число от 0 до 1, указывающее насыщенность цвета.|
|*V*|окне Число от 0 до 1, указывающее значение цвета.|
...
##  <a name="huetorgb"></a>Кдравингманажер:: Хуеторгб
...
##  <a name="mirrorrect"></a>Кдравингманажер:: Мирроррект
...
##  <a name="pixelalpha"></a>Кдравингманажер::P Икселалфа
...
##  <a name="prepareshadowmask"></a>Кдравингманажер::P Репарешадовмаск
...
##  <a name="rgbtohsl"></a>Кдравингманажер:: Ргбтохсл
...
|-|-|
|Параметр|Описание|
|*rgb*|окне Цвет в значениях RGB.|
|*H*|заполняет Указатель на двойное значение, где метод сохраняет цветовой тон для цвета.|
|*S*|заполняет Указатель на двойное значение, где метод сохраняет насыщенность цвета.|
|*L*|заполняет Указатель на двойное значение, где метод сохраняет освещение цвета.|
...
##  <a name="rgbtohsv"></a>Кдравингманажер:: Ргбтохсв
...
##  <a name="setalphapixel"></a>Кдравингманажер:: Сеталфапиксел
...
##  <a name="setpixel"></a>Кдравингманажер:: SetPixel
...
|-|-|
|Параметр|Описание|
|*пбитс*|окне Указатель на битовые значения точечного рисунка.|
|*cx*|окне Общая ширина точечного рисунка.|
|*CY*|окне Общая высота точечного рисунка.|
|*x*|окне Координата x пикселя в растровом изображении, который необходимо изменить.|
|*y*|окне Координата y пикселя в растровом изображении, который необходимо изменить.|
|*color*|окне Новый цвет для пикселя, определяемый предоставленными координатами.|
...
|-|-|
|Параметр|Описание|
|*color1*|окне Первый цвет для смешивания.|
|*color2*|окне Второй цвет для смешивания.|
|*дбллумратио*|окне Отношение для яркости нового цвета. `SmartMixColors`Умножает яркость смешанного цвета на это отношение перед определением окончательного цвета.|
|*K1*|окне Взвешенное соотношение для первого цвета.|
|*априорной оценкой K2*|окне Взвешенное соотношение для второго цвета.|
...
##  <a name="drawrotated"></a>Кдравингманажер::D Равротатед
