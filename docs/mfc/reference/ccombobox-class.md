...
|name|Описание|
|----------|-----------------|
|[CComboBox::AddString](#addstring)|Добавляет строку в конец списка в поле со списком или в отсортированном положении для полей со списком в стиле CBS_SORT.|
...
|[CComboBox:: Copy](#copy)|Копирует текущее выделение (при наличии) в буфер обмена в формате CF_TEXT.|
|[CComboBox:: Create](#create)|Создает поле со списком и прикрепляет его к `CComboBox` объекту.|
|[CComboBox:: Cut](#cut)|Удаляет (вырезает) текущее выделение (при наличии) в элементе управления "поле ввода" и копирует удаленный текст в буфер обмена в формате CF_TEXT.|
...
|[CComboBox::P вставить](#paste)|Вставляет данные из буфера обмена в элемент управления "поле ввода" в текущей позиции курсора. Данные вставляются только в том случае, если буфер обмена содержит данные в формате CF_TEXT.|
...
|[CComboBox:: Сетекстендедуи](#setextendedui)|Выбирает либо пользовательский интерфейс по умолчанию, либо расширенный пользовательский интерфейс для поля со списком, имеющего стиль CBS_DROPDOWN или CBS_DROPDOWNLIST.|
...
`CComboBox` Объект можно создать либо из шаблона диалогового окна, либо непосредственно в коде. В обоих случаях `CComboBox` сначала вызовите конструктор для `CComboBox` создания объекта. Затем вызовите функцию [создания](#create) члена, чтобы создать элемент управления и присоединить его к `CComboBox` объекту.

Если требуется работать с сообщениями уведомлений Windows, отправленными полем со списком, родительскому элементу (обычно это `CDialog`класс, производный от), добавьте запись схемы сообщения и функцию-член обработчика сообщений в родительский класс для каждого сообщения.

Каждая запись схемы сообщений имеет следующий вид:

**При\_** _уведомлении_ **(** _ID_, _мемберфксн_ **)**

где `id` указывает идентификатор дочернего окна элемента управления "поле со списком", отправляющего `memberFxn` уведомление, и является именем родительской функции-члена, написанной для работы с уведомлением.

Прототип родительской функции выглядит следующим образом:

**afx_msg** `void` `memberFxn` **( );**

Порядок, в котором будут отправляться определенные уведомления, нельзя прогнозировать. В частности, уведомление CBN_SELCHANGE может возникнуть либо до, либо после уведомления CBN_CLOSEUP.
...
- ON_CBN_DBLCLK. пользователь дважды щелкает строку в поле со списком. Это сообщение уведомления отправляется только для поля со списком, имеющего стиль CBS_SIMPLE. Для поля со списком, имеющего стиль [CBS_DROPDOWN](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) или [CBS_DROPDOWNLIST](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) , двойной щелчок не может произойти, так как один щелчок скрывает окно списка.

- ON_CBN_DROPDOWN поле со списком раскрывающегося списка раскрывается (сделать видимым). Это сообщение уведомления может появляться только для поля со списком, имеющего стиль CBS_DROPDOWN или CBS_DROPDOWNLIST.

- ON_CBN_EDITCHANGE, что пользователь выполнил действие, которое могло изменить текст в элементе управления поля со списком. В отличие от сообщения CBN_EDITUPDATE, это сообщение отправляется после того, как Windows обновляет экран. Он не отправляется, если поле со списком имеет стиль CBS_DROPDOWNLIST.

- ON_CBN_EDITUPDATE. элемент управления "Правка" в поле со списком будет отображать измененный текст. Это сообщение уведомления отправляется после того, как элемент управления отформатирует текст, но перед отображением текста. Он не отправляется, если поле со списком имеет стиль CBS_DROPDOWNLIST.

- ON_CBN_ERRSPACE. поле со списком не может выделить достаточно памяти для удовлетворения конкретного запроса.

- ON_CBN_SELENDCANCEL (Windows 3,1 и более поздние версии) Указывает, что выбор пользователя должен быть отменен. Пользователь щелкает элемент, а затем щелкает другое окно или элемент управления, чтобы скрыть список полей со списком. Это сообщение уведомления отправляется перед сообщением уведомления CBN_CLOSEUP, чтобы указать, что выбор пользователя следует игнорировать. Сообщение уведомления CBN_SELENDCANCEL или CBN_SELENDOK отправляется, даже если сообщение уведомления CBN_CLOSEUP не отправляется (как в случае поля со списком в стиле CBS_SIMPLE).

- ON_CBN_SELENDOK пользователь выбирает элемент, а затем либо нажимает клавишу ВВОД, либо нажимает клавишу со стрелкой вниз, чтобы скрыть поле со списком. Это сообщение уведомления отправляется перед сообщением CBN_CLOSEUP, чтобы указать, что выбор пользователя должен считаться допустимым. Сообщение уведомления CBN_SELENDCANCEL или CBN_SELENDOK отправляется, даже если сообщение уведомления CBN_CLOSEUP не отправляется (как в случае поля со списком в стиле CBS_SIMPLE).

- ON_CBN_KILLFOCUS. поле со списком теряет фокус ввода.

- ON_CBN_SELCHANGE выбор в списке поля со списком будет изменяться в результате щелчка в списке или изменения выделенного фрагмента с помощью клавиш со стрелками. При обработке этого сообщения текст в элементе управления "поле со списком" может быть получен только с помощью `GetLBText` или другой аналогичной функции. `GetWindowText`нельзя использовать.
...
Если возвращаемое значение больше или равно 0, то это Отсчитываемый от нуля индекс строки в списке. Если возникает ошибка, возвращается значение CB_ERR. Если недостаточно места для хранения новой строки, возвращается значение CB_ERRSPACE.

### <a name="remarks"></a>Примечания

Если список не был создан с использованием стиля [CBS_SORT](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) , строка добавляется в конец списка. В противном случае строка вставляется в список, а список сортируется.
...
Указывает относительное расположение двух элементов, описанных в `COMPAREITEMSTRUCT` структуре. Может принимать любое из следующих значений:
...
По умолчанию эта функция члена не выполняет никаких действий. При создании поля со списком, рисуемого владельцем с использованием стиля LBS_SORT, необходимо переопределить эту функцию-член, чтобы помочь платформе в сортировке новых элементов, добавленных в список.
...
`CComboBox` Объект создается в два этапа. Сначала вызовите конструктор, а затем вызовите метод `Create`, который создает поле со списком Windows и присоединяет его `CComboBox` к объекту.

Когда `Create` выполняется, Windows отправляет сообщения [WM_NCCREATE](../../mfc/reference/cwnd-class.md#onnccreate), [WM_CREATE](../../mfc/reference/cwnd-class.md#oncreate), [WM_NCCALCSIZE](../../mfc/reference/cwnd-class.md#onnccalcsize)и [WM_GETMINMAXINFO](../../mfc/reference/cwnd-class.md#ongetminmaxinfo) в поле со списком.

Эти сообщения по умолчанию обрабатываются функциями члена [оннккреате](../../mfc/reference/cwnd-class.md#onnccreate), [OnCreate](../../mfc/reference/cwnd-class.md#oncreate), [оннккалксизе](../../mfc/reference/cwnd-class.md#onnccalcsize)и [онжетминмаксинфо](../../mfc/reference/cwnd-class.md#ongetminmaxinfo) в `CWnd` базовом классе. Чтобы расширить обработку сообщений по умолчанию, создайте производный класс `CComboBox`от, добавьте к новому классу схему сообщений и переопределите предыдущие функции-члены обработчика сообщений. Переопределение `OnCreate`, например, для выполнения необходимой инициализации нового класса.
...
- WS_VSCROLL для добавления вертикальной прокрутки списка в поле со списком

- WS_HSCROLL Добавление горизонтальной прокрутки для списка в поле со списком

- WS_GROUP к элементам управления Group

- WS_TABSTOP для включения поля со списком в порядок табуляции
...
Удаляет (вырезает) текущее выделение (если оно есть) в элементе управления "поле ввода" в виде поля со списком и копирует удаленный текст в буфер обмена в формате CF_TEXT.
...
Вызывается платформой, когда пользователь удаляет элемент из объекта рисования `CComboBox` владельцем или удаляет поле со списком.
...
- Файл DDL_SYSTEM является системным файлом.

- DDL_DIRECTORY. имя, заданное параметром *лпсзвилдкард* , указывает каталог.

- Файл DDL_ARCHIVE был архивирован.

- DDL_DRIVES включает все диски, соответствующие имени, заданному параметром *лпсзвилдкард*.
...
Если возвращаемое значение больше или равно 0, то это Отсчитываемый от нуля индекс последнего имени файла, добавленного в список. Если возникает ошибка, возвращается значение CB_ERR. Если недостаточно места для хранения новых строк, возвращается значение CB_ERRSPACE.
...
Вызовите функцию- член,чтобынайтипервуюстрокусписка(вполесосписком),совпадающуюсострокой,указаннойв`FindStringExact` лпсзфинд.
...
Если поле со списком было создано с использованием стиля рисования, но без стиля [CBS_HASSTRINGS](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) , `FindStringExact` пытается сопоставить значение даублеворд со значением *лпсзфинд*.
...
Эта функция члена эмулирует функциональность сообщения [CB_GETCOMBOBOXINFO](/windows/win32/Controls/cb-getcomboboxinfo) , как описано в Windows SDK.
...
Число элементов. Возвращаемое значение счетчика больше значения индекса последнего элемента (индекс отсчитывается от нуля). Это CB_ERR, если возникает ошибка.
...
В первой перегрузке объект [CString](../../atl-mfc-shared/using-cstring.md) , содержащий текст баннера подсказки, если он существует; в противном случае — объект,имеющийнулевуюдлину.`CString`
...
Этот метод отправляет сообщение [CB_GETCUEBANNER](/windows/win32/Controls/cb-getcuebanner) , описанное в Windows SDK.
...
Отсчитываемый от нуля индекс текущего выбранного элемента в списке поля со списком или CB_ERR, если элемент не выбран.
...
Вызовите функцию-член, чтобы получить экранные координаты видимого (раскрывающегося списка) поля со списком в раскрывающемся списке. `GetDroppedControlRect`
...
Вызовите функцию-член, чтобы определить, является ли список раскрывающегося списка поля со списком видимым (выводится вниз). `GetDroppedState`
...
32-разрядное значение, содержащее начальную точку в слове с низким порядком и позиции первого невыделенного символа после конца выделенного слова в высоком порядке. Если эта функция используется в поле со списком без элемента управления Edit, возвращается CB_ERR.
...
Вызовите функцию-член, чтобы определить, имеет ли поле со списком пользовательский интерфейс по умолчанию или расширенный пользовательский интерфейс. `GetExtendedUI`
...
- При щелчке статического элемента управления окно списка отображается только для полей со списком, имеющих стиль [CBS_DROPDOWNLIST](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) .
...
32-разрядное значение можно задать с помощью параметра *двитемдата* вызова функции-члена [сетитемдата](#setitemdata) . Используйте функцию `GetItemDataPtr` -член, если извлекаемое 32-разрядное значение является указателем (**void** <strong>\*</strong>).
...
Вызовите функцию-член, чтобы получить высоту элементов списка в поле со списком. `GetItemHeight`
...
*ниндекс*<br/>
Указывает компонент поля со списком, высота которого необходимо получить. Если параметр *ниндекс* имеет значение-1, то высота элемента управления Edit-Control (или статического текста) в поле со списком извлекается. Если поле со списком имеет стиль [CBS_OWNERDRAWVARIABLE](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) , *ниндекс* Указывает отсчитываемый от нуля индекс элемента списка, высота которого нужно получить. В противном случае *ниндекс* должен иметь значение 0.

### <a name="return-value"></a>Возвращаемое значение

Высота (в пикселях) указанного элемента в поле со списком. Если возникает ошибка, возвращается значение CB_ERR.
...
Длина строки (в байтах), исключая завершающего нуль-символа. Если *ниндекс* не указывает допустимый индекс, возвращается значение CB_ERR.
...
Длина строки в байтах, за исключением завершающего нуль символа. Если *ниндекс* не указывает допустимый индекс, возвращается значение CB_ERR.
...
Этот метод отправляет сообщение [CB_GETMINVISIBLE](/windows/win32/Controls/cb-setminvisible) , описанное в Windows SDK.
...
Вызовите эту функцию перед добавлением большого числа элементов в часть `CComboBox`списка.
...
Отсчитываемый от нуля индекс позиции, в которую была вставлена строка. Если возникает ошибка, возвращается значение CB_ERR. Если недостаточно места для хранения новой строки, возвращается значение CB_ERRSPACE.
...
Ненулевое значение в случае успеха. Если вызывается для поля со списком со стилем [CBS_DROPDOWNLIST](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) или для поля со списком без элемента управления "поле ввода", возвращаемое значение — CB_ERR.

### <a name="remarks"></a>Примечания

Если поле со списком не имеет стиля [CBS_AUTOHSCROLL](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles), Установка предельного размера текста, превышающего размер элемента управления поля ввода, не будет действовать.
...
Отсчитываемый от нуля индекс выбранного элемента, если строка найдена. Если поиск завершился неудачно, возвращается значение CB_ERR, а текущее выделение не изменяется.
...
Обратите внимание `SelectString` , `FindString` что функции-члены и находят строку, `SelectString` но функция-член также выбирает строку.
...
Этот метод отправляет сообщение [CB_SETCUEBANNER](/windows/win32/Controls/cb-setcuebanner) , описанное в Windows SDK.
...
- При щелчке статического элемента управления окно списка отображается только для полей со списком, имеющих стиль CBS_DROPDOWNLIST.
...
Если ширина списка меньше этого значения, горизонтальная полоса прокрутки будет горизонтально прокручивать элементы списка. Если ширина окна списка равна или больше этого значения, горизонтальная полоса прокрутки скрыта или, если поле со списком имеет стиль [CBS_DISABLENOSCROLL](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) , отключено.
...
Используйте функцию `SetItemDataPtr` -член, если 32-разрядный элемент является указателем.
...
Вызовите функцию-член, чтобы задать высоту элементов списка в поле со списком или высоту части поля ввода (или статического элемента управления) в поле со списком. `SetItemHeight`
...
Этот метод отправляет сообщение [CB_SETMINVISIBLE](/windows/win32/Controls/cb-setminvisible) , описанное в Windows SDK.
...
Эта функция-член не влияет на поле со списком, созданное с использованием стиля [CBS_SIMPLE](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) .
