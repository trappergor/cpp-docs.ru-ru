...
### <a name="remarks"></a>Примечания  
 ...
 Когда **создать** выполняет Windows отправляет [WM_NCCREATE](../../mfc/reference/cwnd-class.md#onnccreate), [WM_CREATE](../../mfc/reference/cwnd-class.md#oncreate), [WM_NCCALCSIZE](../../mfc/reference/cwnd-class.md#onnccalcsize), и [WM_ GETMINMAXINFO](../../mfc/reference/cwnd-class.md#ongetminmaxinfo) сообщений в поле со списком.  
...
### <a name="parameters"></a>Параметры  
 `nIndex`  
 Указывает компонент поле со списком имеет высоту которого требуется получить. Если `nIndex` параметр имеет значение -1, извлекается высоты элемента управления редактирования (или статического текста) часть поле со списком. Если поле со списком имеет [CBS_OWNERDRAWVARIABLE](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) стиль `nIndex` указывает отсчитываемый от нуля индекс элемента списка, который имеет высоту которого требуется получить. В противном случае `nIndex` должно быть равно 0.  
...
### <a name="parameters"></a>Параметры  
 ...
 Если поле со списком имеет [CBS_OWNERDRAWVARIABLE](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) стиля, `nIndex` указывает отсчитываемый от нуля индекс, высоту которого будет задано; в противном случае — элемент списка `nIndex` должно быть 0 и высоту список всех элементов будет значение.  
