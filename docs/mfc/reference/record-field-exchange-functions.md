...
ms.locfileid: "71095835"
---
# <a name="record-field-exchange-functions"></a>Функции обмена данными полями записей

В этом разделе перечислены функции обмена полями записей (RFX, групповые RFX и DFX), используемые для автоматизации перемещения данных между объектом Recordset и его источником данных, а также для выполнения других операций с данными.
...
Если вы не реализовали многострочную выборку строк в классах на основе ODBC или используете классы на основе DAO (устарели), ClassWizard переопределит `DoFieldExchange` `CRecordset` функцию-член функции или `CDaoRecordset` , вызвав функции RFX (для Классы ODBC) или функции DFX (для классов DAO) для каждого элемента данных поля в наборе записей.
...
Дополнительные сведения о том, как используются эти функции, см. в [статье Обмен полями записи: Принцип работы RFX (ODBC)](../../data/odbc/record-field-exchange-how-rfx-works.md). Дополнительные сведения о групповой выборке строк см. в статье [набор записей. Пакетное получение строк (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md).

Для столбцов данных, которые можно привязать динамически, можно также вызывать функции RFX или DFX самостоятельно, как описано в статье [набор записей. Динамическая привязка столбцов данных (ODBC)](../../data/odbc/recordset-dynamically-binding-data-columns-odbc.md). Кроме того, можно написать собственные подпрограммы RFX и DFX, как описано в техническом примечании [43](../../mfc/tn043-rfx-routines.md) (для ODBC) и техническом примечании [53](../../mfc/tn053-custom-dfx-routines-for-dao-database-classes.md) (для DAO).

Пример функций RFX и функции групповой RFX, отображаемых в функциях `DoFieldExchange` и `DoBulkFieldExchange` , см. в разделе [RFX_Text](#rfx_text) and [RFX_Text_Bulk] #rfx_text_bulk). Функции DFX очень похожи на функции RFX.
...
|[RFX_Date](#rfx_date)|Передает данные о времени и датах с помощью [CTime](../../atl-mfc-shared/reference/ctime-class.md) или TIMESTAMP_STRUCT.|
...
Передает массивы байтов между элементами `CRecordset` данных поля объекта и столбцами записи в источнике данных ODBC типа SQL_BINARY, SQL_VARBINARY или SQL_LONGVARBINARY.
...
*Сохраняется*<br/>
Указатель на объект класса [кфиелдексчанже](cfieldexchange-class.md). Этот объект содержит сведения, определяющие контекст для каждого вызова функции. Дополнительные сведения об операциях, которые `CFieldExchange` могут указываться в объекте, см [. в статье Обмен полями записи. Принцип работы RFX](../../data/odbc/record-field-exchange-how-rfx-works.md).
...
Передает логические данные между элементами `CRecordset` данных поля объекта и столбцами записи в источнике данных ODBC типа SQL_BIT.
...
Передает отдельные байты между элементами `CRecordset` данных поля объекта и столбцами записи в источнике данных ODBC типа SQL_TINYINT.
...
Передает `CTime` или TIMESTAMP_STRUCT данные между элементами `CRecordset` данных поля объекта и столбцами записи в источнике данных ODBC Type SQL_DATE, SQL_TIME или SQL_TIMESTAMP.
...
Передает данные типа **Double с плавающей запятой** между элементами `CRecordset` данных поля объекта и столбцами записи в источнике данных ODBC типа SQL_DOUBLE.
...
Передает целочисленные данные между элементами `CRecordset` данных поля объекта и столбцами записи в источнике данных ODBC типа SQL_SMALLINT.
...
Передает длинные целочисленные данные между элементами `CRecordset` данных поля объекта и столбцами записи в источнике данных ODBC типа SQL_INTEGER.
...
Передает данные больших двоичных объектов (BLOB) с помощью класса [CLongBinary](clongbinary-class.md) между элементами `CRecordset` данных поля объекта и столбцами записи в источнике данных ODBC типа SQL_LONGVARBINARY или SQL_LONGVARCHAR.
...
Передает данные с плавающей запятой между элементами `CRecordset` данных поля объекта и столбцами записи в источнике данных ODBC типа SQL_REAL.
...
Передает `CString` данные между элементами `CRecordset` данных поля объекта и столбцами записи в источнике данных ODBC Type SQL_LONGVARCHAR, SQL_CHAR, SQL_VARCHAR, SQL_DECIMAL или SQL_NUMERIC.
...
*Сохраняется*<br/>
Указатель на объект класса `CFieldExchange`. Этот объект содержит сведения, определяющие контекст для каждого вызова функции. Дополнительные сведения об операциях, которые `CFieldExchange` могут указываться в объекте, см [. в статье Обмен полями записи. Принцип работы RFX](../../data/odbc/record-field-exchange-how-rfx-works.md).
...
*нколумнтипе*<br/>
Используется в основном для параметров. Целое число, указывающее тип данных параметра. Тип является типом данных ODBC в формате **SQL_XXX**.

*нскале*<br/>
Задает масштаб для значений ODBC типа SQL_DECIMAL или SQL_NUMERIC. *нскале* полезен только при задании значений параметров. Дополнительные сведения см. в разделе "точность, масштаб, Длина и размер дисплея" в приложении г из *справочника по программированию для ODBC SDK*.

### <a name="remarks"></a>Примечания

Данные в источнике данных всех этих типов сопоставляются с набором записей и `CString` из него.

### <a name="example"></a>Пример

В `RFX_Text`этом примере показано несколько вызовов. Обратите также внимание на два `CFieldExchange::SetFieldType`вызова. Для параметров необходимо написать вызов `SetFieldType` и его вызов RFX. Вызов выходного столбца и связанные с ним вызовы RFX обычно записываются мастером кода.
...
Передает несколько строк байтовых данных из столбца источника данных ODBC в соответствующий массив в производном от него `CRecordset`объекте.
...
*Сохраняется*<br/>
Указатель на объект [кфиелдексчанже](cfieldexchange-class.md) . Этот объект содержит сведения, определяющие контекст для каждого вызова функции. Дополнительные сведения см. в статье [обмен полями записи. Принцип работы RFX](../../data/odbc/record-field-exchange-how-rfx-works.md).
...
*пргленгсс*<br/>
Указатель на массив длинных целых чисел. Этот массив будет хранить длину в байтах каждого значения в массиве, на которое указывает *пргбитевалс*. Обратите внимание, что значение SQL_NULL_DATA будет сохранено, если соответствующий элемент данных содержит значение null. Дополнительные сведения см. в описании функции `SQLBindCol` ODBC API в *справочнике программиста по пакету SDK для ODBC*.
...
> [!NOTE]
>  Обмен полями с массовыми записями передает данные только из источника данных в объект набора записей. Чтобы сделать набор записей обновляемым, необходимо использовать функцию `SQLSetPos`ODBC API.
...
Передает несколько строк логических данных из столбца источника данных ODBC в соответствующий массив в объекте, `CRecordset`производном от.
...
*пргленгсс*<br/>
Указатель на массив длинных целых чисел. Этот массив будет хранить длину в байтах каждого значения в массиве, на которое указывает *пргбулвалс*. Обратите внимание, что значение SQL_NULL_DATA будет сохранено, если соответствующий элемент данных содержит значение null. Дополнительные сведения см. в описании функции `SQLBindCol` ODBC API в *справочнике программиста по пакету SDK для ODBC*.
...
Передает несколько строк из одного байта из столбца источника данных ODBC в соответствующий массив в объекте, `CRecordset`производном от.
...
Передает несколько строк данных TIMESTAMP_STRUCT из столбца источника данных ODBC в соответствующий массив в объекте, `CRecordset`производном от.
...
*пргленгсс*<br/>
Указатель на массив длинных целых чисел. Этот массив будет хранить длину в байтах каждого значения в массиве, на которое указывает *пргтсвалс*. Обратите внимание, что значение SQL_NULL_DATA будет сохранено, если соответствующий элемент данных содержит значение null. Дополнительные сведения см. в описании функции `SQLBindCol` ODBC API в *справочнике программиста по пакету SDK для ODBC*.

### <a name="remarks"></a>Примечания

Столбец источника данных может иметь тип ODBC SQL_DATE, SQL_TIME или SQL_TIMESTAMP. Набор записей должен определять элемент данных поля типа pointer в TIMESTAMP_STRUCT.
...
Передает несколько строк данных двойной точности с плавающей запятой из столбца источника данных ODBC в соответствующий массив в производном от него `CRecordset`объекте.
...
*пргленгсс*<br/>
Указатель на массив длинных целых чисел. Этот массив будет хранить длину в байтах каждого значения в массиве, на которое указывает *пргдблвалс*. Обратите внимание, что значение SQL_NULL_DATA будет сохранено, если соответствующий элемент данных содержит значение null. Дополнительные сведения см. в описании функции `SQLBindCol` ODBC API в *справочнике программиста по пакету SDK для ODBC*.
...
Передает несколько строк длинных целочисленных данных из столбца источника данных ODBC в соответствующий массив в объекте, `CRecordset`производном от.
...
*пргленгсс*<br/>
Указатель на массив длинных целых чисел. Этот массив будет хранить длину в байтах каждого значения в массиве, на которое указывает *прглонгвалс*. Обратите внимание, что значение SQL_NULL_DATA будет сохранено, если соответствующий элемент данных содержит значение null. Дополнительные сведения см. в описании функции `SQLBindCol` ODBC API в *справочнике программиста по пакету SDK для ODBC*.
...
Передает несколько строк данных с плавающей запятой из столбца источника данных ODBC в соответствующий массив в производном от него `CRecordset`объекте.
...
*пргленгсс*<br/>
Указатель на массив длинных целых чисел. Этот массив будет хранить длину в байтах каждого значения в массиве, на которое указывает *пргфлтвалс*. Обратите внимание, что значение SQL_NULL_DATA будет сохранено, если соответствующий элемент данных содержит значение null. Дополнительные сведения см. в описании функции `SQLBindCol` ODBC API в *справочнике программиста по пакету SDK для ODBC*.
...
Передает несколько строк символьных данных из столбца источника данных ODBC в соответствующий массив в производном от него `CRecordset`объекте.
...
*пргленгсс*<br/>
Указатель на массив длинных целых чисел. Этот массив будет хранить длину в байтах каждого значения в массиве, на которое указывает *пргстрвалс*. Эта длина исключает завершающий символ null. Обратите внимание, что значение SQL_NULL_DATA будет сохранено, если соответствующий элемент данных содержит значение null. Дополнительные сведения см. в описании функции `SQLBindCol` ODBC API в *справочнике программиста по пакету SDK для ODBC*.
...
Необходимо вручную написать вызовы в `DoBulkFieldExchange` переопределении. В этом примере показан вызов `RFX_Text_Bulk`, а также `RFX_Long_Bulk`вызов для передачи данных. Этим вызовам предшествует вызов [кфиелдексчанже:: сетфиелдтипе](CFieldExchange::SetFieldType.md). Обратите внимание, что для параметров необходимо вызвать функции RFX вместо функций, выполняющих функции групповой RFX.
...
Данные сопоставляются между типами DAO_BYTES в DAO и типы [CByteArray](cbytearray-class.md) в наборе записей.
...
> [!NOTE]
>  Можно управлять двойным буферизацией данных по умолчанию с помощью параметра [CDaoRecordset:: m_bCheckCacheForDirtyFields](cdaorecordset-class.md#m_bcheckcachefordirtyfields).

### <a name="remarks"></a>Примечания

Данные сопоставляются между типами DAO_BOOL в DAO и типа BOOL в наборе записей.
...
Данные сопоставляются между типами DAO_BYTES в DAO и Type BYTE в наборе записей.
...
Данные сопоставляются между типами DAO_CURRENCY в DAO и Type [COleCurrency](colecurrency-class.md) в наборе записей.
...
Данные сопоставляются между типами DAO_DATE в DAO и Type [COleDateTime](../../atl-mfc-shared/reference/coledatetime-class.md) в наборе записей.
...
Данные сопоставляются между типами DAO_R8 в DAO и имеют тип **Double float** в наборе записей.
...
Данные сопоставляются между типами DAO_I4 в DAO и имеют тип **Long** в наборе записей.
...
`DFX_LongBinary`предоставляется для обеспечения совместимости с классами ODBC MFC. Функция `DFX_LongBinary` передает данные большого двоичного объекта (BLOB) с помощью класса `CLongBinary` между элементами данных поля объекта [CDaoRecordset](cdaorecordset-class.md) и столбцами записи в источнике данных. Данные сопоставляются между типами DAO_BYTES в DAO и Type [CLongBinary](clongbinary-class.md) в наборе записей.
...
Данные сопоставляются между типами DAO_I2 в DAO и имеют **короткий** тип в наборе записей.
...
Данные сопоставляются между типами DAO_R4 в DAO и типа **float** в наборе записей.
...
Данные сопоставляются между типами DAO_CHAR в DAO (или, если определен символ _UNICODE, DAO_WCHAR) и тип [CString](../../atl-mfc-shared/reference/cstringt-class.md) в наборе записей.  n

### <a name="example"></a>Пример

В `DFX_Text`этом примере показано несколько вызовов. Обратите также внимание на два вызова [кдаофиелдексчанже:: сетфиелдтипе](cdaofieldexchange-class.md#setfieldtype). Необходимо написать первый вызов метода `SetFieldType` и его **DFX** вызов. Второй вызов и связанные с ними **DFX** вызовы обычно записываются мастером кода, который создал класс.
