...
### <a name="remarks"></a>Примечания  
 ...
 Если передается значение NULL, либо для *pOpenStream* или *lpszFileName*, конструктор вызывает `CInvalidArgException*`.  
...
### <a name="remarks"></a>Примечания  
 Он имеет значение NULL, если файл никогда не был открыт или закрыт.  
...
### <a name="remarks"></a>Примечания  
 ...
 Этот метод создает исключение `CInvalidArgException*` Если указать значение NULL для *lpsz* параметра.  
