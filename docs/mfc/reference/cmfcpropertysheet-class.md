---
title: Класс CMFCPropertySheet | Документация Майкрософт
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
|`CMFCPropertySheet::PreTranslateMessage`|Преобразует сообщения окна перед их диспетчеризацией в [TranslateMessage](http://msdn.microsoft.com/library/windows/desktop/ms644955) и [DispatchMessage](http://msdn.microsoft.com/library/windows/desktop/ms644934) функции Windows. (Переопределяет `CPropertySheet::PreTranslateMessage`.)|  
...
4.  Если вы создаете таблицу свойств в стиле панели инструментов Microsoft Outlook, вызовите [CMFCPropertySheet::SetIconsList](#seticonslist) способ связывания списка изображений со страницами свойств.  
...
### <a name="parameters"></a>Параметры  
 [in] *Физ_страница*  
 Указатель на объект страницы. Этот параметр не может иметь значение NULL.  
...
### <a name="parameters"></a>Параметры  
 [in] *pCategory*  
 Указатель на родительский узел дерева, или значение NULL, должен быть сопоставлен указанную страницу узел верхнего уровня. Вызовите [CMFCPropertySheet::AddTreeCategory](#addtreecategory) метод, чтобы получить этот указатель.  
  
 [in] *Физ_страница*  
 Указатель на объект страницы свойств.  
  
 [in] *nIconNum*  
 Отсчитываемый от нуля индекс значка или -1, если значок не используется. Значок отображается рядом с страницу свойств элемента управления дерева, если страницы не выбран. Значение по умолчанию — -1.  
  
 [in] *nSelIconNum*  
 Отсчитываемый от нуля индекс значка или -1, если значок не используется. Значок отображается рядом со страницей свойств элемента управления дерева, при выборе страницы. Значение по умолчанию — -1.  
...
### <a name="parameters"></a>Параметры  
 ...
 [in] *pParentCategory*  
 Указатель на родительский узел дерева, или значение NULL, должен быть сопоставлен указанную страницу узел верхнего уровня. Установите этот параметр с [CMFCPropertySheet::AddTreeCategory](#addtreecategory) метод.  
...
### <a name="parameters"></a>Параметры  
 [in] *pszCaption*  
 Строка, содержащая заголовок листа свойств. Не может принимать значение NULL.  
  
 [in] *nIDCaption*  
 Идентификатор ресурса, содержащее заголовок листа свойств.  
  
 [in] *pParentWnd*  
 Указатель на родительское окно свойств, или значение NULL, если родительское окно главное окно приложения. Значение по умолчанию имеет значение NULL.  
  
 [in] *iSelectPage*  
 Отсчитываемый от нуля индекс страницы свойство top. Значение по умолчанию — 0.  
...
### <a name="parameters"></a>Параметры  
 [in] *Физ_страница*  
 Указатель на объект page свойство, представляющее страницу свойств enabled.  
...
### <a name="parameters"></a>Параметры  
 ...
 [in] *rectHeader*  
 Ограничивающий прямоугольник, который указывает, где для рисования заголовка.  
...
### <a name="parameters"></a>Параметры  
 [in] *pCategory*  
 Указатель на категорию (узел), чтобы удалить.  
...
### <a name="parameters"></a>Параметры  
 [in] *Физ_страница*  
 Указатель на объект страницы свойство, представляющее страницу свойств для удаления. Не может принимать значение NULL.  
  
 [in] *nPage*  
 Отсчитываемый от нуля индекс, которую требуется удалить.  
...
### <a name="parameters"></a>Параметры  
 [in] *uiImageListResID*  
 Идентификатор ресурса из списка изображений.  
...
### <a name="parameters"></a>Параметры  
 [in] *поиска*  
 Одно из значений перечисления, указывающее внешний вид страницы свойств. Стиль по умолчанию для страницы свойств является `CMFCPropertySheet::PropSheetLook_Tabs`. Дополнительные сведения см. в таблице в разделе "Примечания" в этом разделе.  
