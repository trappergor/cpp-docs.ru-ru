...
*лпсзлокале*<br/>
Строковое выражение, используемое для указания порядка сортировки при создании *лпсздестнаме*. Если опустить этот аргумент, приняв значение `dbLangGeneral` по умолчанию (см. ниже), языковой стандарт новой базы данных будет таким же, как и у старой базы данных. Доступны следующие значения:
...
> [!CAUTION]
>  Функция `CompactDatabase` -член не будет правильно преобразовывать полную базу данных Microsoft Access из одной версии в другую. Преобразуется только формат данных. Объекты, определяемые Microsoft Access, такие как формы и отчеты, не преобразуются. Однако данные правильно преобразуются.
...
Вызовите эту функцию-член, чтобы создать новый объект рабочей области DAO и связать его `CDaoWorkspace` с объектом MFC.
...
1. Вызовите функцию `Create` члена объекта, чтобы создать базовую рабочую область DAO. Необходимо указать имя рабочей области.
...
- AFX_DAO_SECONDARY_INFO первичную информацию и: Версия, порядок сортировки, время ожидания запроса

- AFX_DAO_ALL_INFO основную и дополнительную информацию, а также: Подключение
...
Вызовите эту функцию-член, чтобы получить определяемое пользователем имя объекта рабочей области DAO, `CDaoWorkspace` лежащего в основе объекта.
...
- AFX_DAO_SECONDARY_INFO первичную информацию и: Имя пользователя

- AFX_DAO_ALL_INFO основную и дополнительную информацию, а также: Изоляция Одбктранс
...
*Nвыходные*<br/>
Действие, выполняемое во время бездействия обработки. В настоящее время единственным допустимым `dbFreeLocks`действием является.
...
Хотя многие `CDaoWorkspace` функции-члены могут вызываться только после открытия рабочей области, следующие функции-члены, работающие с ядром СУБД, доступны после создания C++ объекта, но перед вызовом `Open`:
...
*лпсздефаултусер*<br/>
Имя пользователя по умолчанию. Имя пользователя может содержать 1-20 символов и включать буквы, цифры, пробелы и символы, за исключением: "(кавычки),/(косая черта)," \ (обратная косая черта \[ \] ) ", (скобки),: &#124; (двоеточие), ( pipe), \< (знак "меньше"), > (знак "больше"), + (знак "плюс"), = (знак равенства),; (точка с запятой),, (запятая), (вопросительный \* знак), (звездочка), начальные пробелы и управляющие символы (от ASCII 00 до ASCII 31). Связанные сведения см. в разделе "свойство UserName" справки DAO.
