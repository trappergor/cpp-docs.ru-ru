...
|Имя|Описание:|  
|----------|-----------------|  
|[CDaoWorkspace::CDaoWorkspace](#cdaoworkspace)|Создает объект рабочей области. После этого вызвать `Create` или `Open`.|  
...
|[CDaoWorkspace::SetDefaultPassword](#setdefaultpassword)|Задает пароль, который компонент database engine использует при создании объекта рабочей области без определенного пароля.|  
...
-   Откройте существующую рабочую область в коллекции рабочих областей.  
 ...
 Завершение сеанса рабочей области, закрыв рабочей области с [закрыть](#close) функции-члена. `Close` Закрывает все базы данных, которые не закрыты ранее, выполняется откат всех незафиксированных транзакций.  
...
### <a name="remarks"></a>Примечания  
 Закрытие объекта открыть рабочую область освобождает объект базового DAO и, если рабочей области является членом коллекции рабочих областей, удаляет его из коллекции. Вызов `Close` является хорошим стилем программирования.  
...
### <a name="remarks"></a>Примечания  
 Транзакция состоит из серии изменений базы данных или ее структуру, начиная с вызова [BeginTrans](#begintrans). Если после завершения транзакции, либо выполняющее ее фиксацию или откат (отменить изменения) с [отката](#rollback). По умолчанию без транзакций обновления записей фиксируются немедленно. Вызов `BeginTrans` вызывает обновлений приверженность отложено до вызова `CommitTrans`.  
  
> [!CAUTION]
>  В одной рабочей области транзакции всегда являются глобальными в рабочую область и не ограничиваются только одной базы данных или набора записей. При выполнении операций на более чем одной базы данных или набора записей в рабочей области транзакции, `CommitTrans` фиксации все ожидающие обновления, и `Rollback` восстанавливает все операции с этими базами данных и наборы записей.  
 ...
 *nOptions*  
 Указывает один или несколько параметров для целевой базы данных, *lpszDestName*. Если этот аргумент опущен, приняв значения по умолчанию, *lpszDestName* будут иметь шифрования одной и той же версии, *lpszSrcName*. Можно объединять **dbEncrypt** или **dbDecrypt** параметр с одним из параметров версии, с помощью оператора побитового или. Возможные значения, которые задают формат базы данных, а не версию ядра базы данных, являются:  
...
2.  Вызов объекта `Create` функции-члена для создания базовой рабочей области DAO. Необходимо указать имя рабочей области.  
...
3.  При необходимости вызовите функцию [Append](#append) Если вы хотите добавить в коллекцию рабочих областей ядро базы данных рабочей области. Без добавления ее можно работать с рабочей областью.  
 ...
 После `Create` вызов, объект рабочей области находится в открытом состоянии, готов к использованию. Не следует вызывать `Open` после `Create`. Не следует вызывать `Create` Если рабочая область уже существует в коллекции рабочих областей. `Create` Инициализирует ядро базы данных, если он еще не был инициализирован для вашего приложения.  
...
> [!NOTE]
>  Вызов `Idle` не является обязательным для баз данных, созданных в версии 3.0 базы данных Microsoft Jet. Используйте `Idle` только для баз данных, созданных в более ранних версиях.  
...
-   Откройте существующий `CDaoWorkspace` объект, является членом коллекции рабочих областей, по имени. Передайте допустимое имя для существующего объекта рабочей области.  
 ...
 `Open` помещает объект рабочей области в открытом состоянии, а также инициализирует ядро базы данных, если он еще не был инициализирован для вашего приложения.  
  
 Хотя многие `CDaoWorkspace` члена функции могут вызываться только после открытия рабочей области, следующие функции-члены, которые работают в компоненте database engine, доступные после создания экземпляра объекта C++, но перед вызовом `Open`:  
...
> [!CAUTION]
>  В один объект рабочей области транзакции всегда являются глобальными в рабочую область и не ограничиваются только одной базы данных или набора записей. При выполнении операций на более чем одной базы данных или набора записей в рабочей области транзакции, `Rollback` восстанавливает все операции во всех этих баз данных и наборы данных.  
  
 Если объект рабочей области закрыть без сохранения или отката все отложенные транзакции, откат этих транзакций автоматически. При вызове метода [CommitTrans](#committrans) или `Rollback` без предварительного вызова функции [BeginTrans](#begintrans), возникает ошибка.  
  
> [!NOTE]
>  При запуске транзакции, компонент database engine записывает операции, его в файле, который хранится в каталоге, указанном в переменной среды TEMP на рабочей станции. Если файл журнала транзакций исчерпывает доступное хранилище на диске TEMP, компонент database engine приведет к MFC создаст `CDaoException` (ошибка DAO 2004 г.). На этом этапе при вызове метода `CommitTrans`фиксируются неопределенное количество операций, но остальные незавершенные действия будут утеряны, и операция должна быть перезапущена. Вызов `Rollback` освобождает журнал транзакций и выполняет откат всех операций в транзакции.  
...
1.  Создать `CDaoWorkspace` объекта, но не нужно вызывать `Create`.  
  
2.  Вызовите `SetDefaultPassword` и при желании [SetDefaultUser](#setdefaultuser).  
  
3.  Вызовите `Create` для этого объекта в рабочей области или последующие условия, без указания пароля.  
...
### <a name="parameters"></a>Параметры  
 *lpszDefaultUser*  
 Имя пользователя по умолчанию. Имя пользователя может иметь длину от 1 до 20 символов и включать буквы, диакритические знаки, цифры, пробелы и символы, за исключением: "(кавычки) / (косая черта), \ (обратная косая черта), \[ \] (скобки): (двоеточие), &#124; () Вертикальная черта), \< (меньше-знак "больше"), > (больше-знак "больше"), + (плюс) = (равно входа), (точку с запятой), (запятая) (вопросительный знак) * (звездочка), начальные пробелы и управляющие символы (коды ASCII от 00 до ASCII 31). Дополнительные сведения см. в разделе «Свойство имени пользователя» справки DAO.  
...
3.  Вызовите `Create` для этого объекта в рабочей области или последующие условия, без указания имени пользователя.  
