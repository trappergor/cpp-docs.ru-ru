...
### <a name="parameters"></a>Параметры  
 [in] *bVertDock*  
 Значение TRUE, чтобы указать, что элемент управления панели Цвет закреплен по вертикали; Значение FALSE, чтобы указать, что элемент управления панели Цвет закреплена горизонтально.  
...
### <a name="parameters"></a>Параметры  
 ...
 [in] *src*  
 Существующий `CMFCColorBar` объект, который необходимо скопировать в новый `CMFCColorBar` объекта.  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[in] *bSquareButtons*|Значение TRUE, чтобы указать, что фигуры кнопок на элементе управления полосы цвета квадрата; в противном случае — значение FALSE. Значение по умолчанию — TRUE.|  
|[in] *bCenterButtons*|Значение TRUE, чтобы указать, что содержимое на поверхности нажимаемой кнопки управления цветовой полосы выравнивается по центру; в противном случае — значение FALSE. Значение по умолчанию — TRUE.|  
...
### <a name="parameters"></a>Параметры  
 ...
 [in] *nColumns*  
 Число столбцов в элементе управления цветовой полосы. Значение по умолчанию — 0.  
  
 [in] *nRowsDockHorz*  
 Число строк в элементе управления цветовой панели, когда она закреплена горизонтально. Значение по умолчанию — 0.  
  
 [in] *nColDockVert*  
 Число столбцов в элементе управления цветовой панели, когда она закреплена по вертикали. Значение по умолчанию — 0.  
...
### <a name="parameters"></a>Параметры  
 [in] *pParentWnd*  
 Указатель на родительское окно. Не может принимать значение NULL.  
  
 [in] *rect*  
 Ограничивающий прямоугольник, указывающий, куда нарисуйте элемент управления цветовой полосы.  
  
 [in] *nID*  
 Идентификатор элемента управления.  
  
 [in] *nColumns*  
 Оптимальное количество столбцов в элементе управления цветовой полосы. Этот метод изменяет это число в соответствии с указанным палитры цветов. Значение по умолчанию равно -1, это означает, что этот параметр не указан.  
  
 [in] *pPalette*  
 Указатель на палитру цветов, или значение NULL. Если этот параметр имеет значение NULL, этот метод вычисляет размер элемента управления цветовой полосы, как при указанном 20 цветов. Значение по умолчанию имеет значение NULL.  
...
### <a name="parameters"></a>Параметры  
 ...
 [in] *bEnable*  
 Значение TRUE для включения автоматической кнопки; Значение FALSE, чтобы отключить автоматическое кнопку. Значение по умолчанию — TRUE.  
...
### <a name="parameters"></a>Параметры  
 ...
 [in] *bAltColorDlg*  
 True, если [CMFCColorDialog](../../mfc/reference/cmfccolordialog-class.md) диалоговом окне; Значение FALSE, чтобы отображать стандартный [CColorDialog](../../mfc/reference/ccolordialog-class.md) диалоговое окно. Значение по умолчанию — TRUE.  
  
 [in] *bEnable*  
 Значение TRUE, чтобы включить кнопку; Значение FALSE, чтобы отключить кнопку. Значение по умолчанию — TRUE.  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[in] *bVertDock*|Значение TRUE, чтобы выполнить вычисления для элемента управления по вертикали закрепленной цветовую шкалу; в противном случае выполните вычисления для горизонтально пристыкованного элемента управления.|  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[in] *nNumColumns*|Если элемент управления цветовой полосы содержит цвета документа, количество столбцов для отображения в сетке цветов документа. В противном случае это значение не используется.|  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[in] *pPalette*|Указатель на объект в палитре, или значение NULL. Если этот параметр имеет значение NULL, этот метод использует палитры по умолчанию операционной системы.|  
|[in] *arColors*|Массив цветов.|  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[in] *pButton*|Указатель на элемент управления, находящийся на панели инструментов.|  
...
### <a name="parameters"></a>Параметры  
 ...
 Значение TRUE, чтобы отключить элемент пользовательского интерфейса, если обработчик не определен в схеме сообщений; в противном случае — значение FALSE.  
...
### <a name="parameters"></a>Параметры  
 [in] *colorDefault*  
 Цвет, который выбран по умолчанию, когда откроется диалоговое окно «цвет».  
  
 [out] *colorRes*  
 Цвет, который пользователь выбрал.  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[in] *основного контроллера домена*|Указатель на контекст устройства кнопки родительского текущего элемента управления цветовой полосы.|  
...
### <a name="parameters"></a>Параметры  
 [in] *lpszCaption*  
 Заголовок, отображаемый, когда элемент управления панели Цвет не прикреплен.  
  
 [in] *lstDocColors*  
 Список цветов, заменяющий цвета текущего документа.  
  
 [in] *bShowWhenDocked*  
 Значение TRUE для отображения цвета документа при присоединении цветовая панель управления; в противном случае — значение FALSE. Значение по умолчанию — FALSE.  
