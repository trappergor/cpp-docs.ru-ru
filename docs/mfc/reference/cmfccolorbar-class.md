...
### <a name="remarks"></a>Примечания  
 Когда пользователь выбирает новый цвет, платформа отправляет идентификатор команды сообщения WM_COMMAND, чтобы уведомить родительский `CMFCColorBar` объекта.  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[in] *основного контроллера домена*|Указатель на контекст устройства кнопки родительского элемента цветовой шкалы.|  
...
### <a name="remarks"></a>Примечания  
 Этот метод отправляет сообщение WM_SETMESSAGESTRING владельца элемента цветовой шкалы.  
