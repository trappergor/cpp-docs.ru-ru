...
|[Кдаокуеридеф:: Close](#close)|Закрывает объект QueryDef. Уничтожение C++ объекта по завершении работы с ним.|
|[Кдаокуеридеф:: Create](#create)|Создает базовый объект DAO QueryDef. Используйте QueryDef в качестве временного запроса или вызовите `Append`, чтобы сохранить его в базе данных.|
...
|[Кдаокуеридеф:: m_pDatabase](#m_pdatabase)|Указатель на объект `CDaoDatabase`, с которым связана QueryDef. Объект QueryDef может быть сохранен в базе данных или нет.|
...
1. Во всех случаях сначала создайте объект `CDaoQueryDef`, указав указатель на объект [CDaoDatabase](../../mfc/reference/cdaodatabase-class.md) , которому принадлежит запрос.
...
1. Затем выполните следующие действия в зависимости от того, что вам нужно:
   ...
   - Чтобы создать новый сохраненный запрос, вызовите функцию [-член объекта](#create) QueryDef, указав имя запроса. Затем вызовите [append](#append) , чтобы сохранить запрос, добавив его в коллекцию QueryDef базы данных. `Create` помещает QueryDef в открытое состояние, поэтому после вызова `Create` вы не вызываете `Open`.
...
- Вызов функции-члена `Execute` объекта для непосредственного выполнения запроса действия или запроса к серверу SQL

Объект QueryDef можно использовать для любого типа запросов, включая SELECT, Action, перекрестный, DELETE, Update, Append, Make-Table, Definition, а также запросы SQL Pass, Union и массовы. Тип запроса определяется содержимым предоставленной инструкции SQL. Сведения о типах запросов см. в разделе функции элементов `Execute` и [GetType](#gettype) . Наборы записей обычно используются для запросов, возвращающих строки, обычно с помощью инструкции **SELECT... ИЗ** ключевых слов. `Execute` наиболее часто используется для выполнения операций с массовыми операциями. Дополнительные сведения см. в разделе [EXECUTE](#execute) and [CDaoRecordset](../../mfc/reference/cdaorecordset-class.md).

## <a name="querydefs-and-recordsets"></a>QueryDef и наборы записей

Чтобы использовать объект QueryDef для создания объекта `CDaoRecordset`, обычно создается или открывается объект QueryDef, как описано выше. Затем создайте объект набора записей, передав указатель на объект QueryDef при вызове [CDaoRecordset:: Open](../../mfc/reference/cdaorecordset-class.md#open). Передаваемый объект QueryDef должен находиться в открытом состоянии. Дополнительные сведения см. в разделе Class [CDaoRecordset](../../mfc/reference/cdaorecordset-class.md).

Нельзя использовать QueryDef для создания набора записей (наиболее распространенный способ использования для объекта QueryDef), если он не находится в открытом состоянии. Переведите QueryDef в открытое состояние, вызвав либо `Open`, либо `Create`.
...
##  <a name="append"></a>Кдаокуеридеф:: Append
...
`Append` сохраняет QueryDef в базе данных, добавляя объект в коллекцию QueryDef базы данных. Объект QueryDef можно использовать в качестве временного объекта, не добавляя его, но если его нужно сохранить, необходимо вызвать `Append`.
...
##  <a name="canupdate"></a>Кдаокуеридеф:: Канупдате
...
##  <a name="cdaoquerydef"></a>Кдаокуеридеф:: Кдаокуеридеф
...
- Если объект представляет новый QueryDef для сохранения, вызовите функцию-член [CREATE](#create) объекта. При этом объект добавляется в коллекцию QueryDef базы данных. Затем вызовите `CDaoQueryDef` функции члена, чтобы задать атрибуты объекта. Наконец, вызовите [append](#append).

- Если объект представляет временный QueryDef (не сохраняемый в базе данных), вызовите `Create`, передав пустую строку для имени запроса. После вызова `Create`инициализируйте объект QueryDef, указав его атрибуты напрямую. Не вызывайте `Append`.
...
По завершении работы с объектом QueryDef вызовите его [закрытую](#close) функцию Member. Если у вас есть указатель на QueryDef, используйте оператор **Delete** для уничтожения C++ объекта.

##  <a name="close"></a>Кдаокуеридеф:: Close
...
Закрытие объекта QueryDef освобождает базовый объект DAO, но не уничтожает сохраненный объект DAO QueryDef или объект C++ `CDaoQueryDef`. Это не то же самое, что [CDaoDatabase::D елетекуеридеф](../../mfc/reference/cdaodatabase-class.md#deletequerydef), который удаляет QueryDef из коллекции QueryDef базы данных в DAO (если не является временным QueryDef).

##  <a name="create"></a>Кдаокуеридеф:: Create
...
Если инструкция SQL не указана в параметре *lpszSQL*, то нельзя выполнить запрос с `Execute` но можно использовать его для создания набора записей. В этом случае MFC использует инструкцию SQL набора записей по умолчанию.

##  <a name="execute"></a>Кдаокуеридеф:: Execute
...
- `dbInconsistent` непоследовательных обновлений.

- `dbConsistent` последовательных обновлений.

- `dbSQLPassThrough` SQL Pass. Вызывает передачу инструкции SQL в базу данных ODBC для обработки.
...
`Execute` не работает для запросов, возвращающих записи, таких как запросы SELECT. `Execute` часто используется для запросов с массовыми операциями, таких как **Обновление**, **Вставка**или **Выбор в**, или для операций языка описания данных (DDL).
...
##  <a name="getconnect"></a>Кдаокуеридеф:: соединение
...
##  <a name="getdatecreated"></a>Кдаокуеридеф:: Жетдатекреатед
...
##  <a name="getdatelastupdated"></a>Кдаокуеридеф:: Жетдателаступдатед
...
##  <a name="getfieldcount"></a>Кдаокуеридеф:: Жетфиелдкаунт
...
`GetFieldCount` удобно использовать для цикла по всем полям в QueryDef. Для этой цели используйте `GetFieldCount` в сочетании с [жетфиелдинфо](#getfieldinfo).

##  <a name="getfieldinfo"></a>Кдаокуеридеф:: Жетфиелдинфо
...
*FieldInfo*<br/>
Ссылка на объект `CDaoFieldInfo`, который возвращает запрошенную информацию.
...
##  <a name="getname"></a>Кдаокуеридеф:: Name
...
##  <a name="getodbctimeout"></a>Кдаокуеридеф:: Жетодбктимеаут
...
##  <a name="getparametercount"></a>Кдаокуеридеф:: GetParameterCount
...
`GetParameterCount` удобно использовать для прохода по всем параметрам в QueryDef. Для этой цели используйте `GetParameterCount` в сочетании с [GetParameterInfo](#getparameterinfo).

Дополнительные сведения см. в разделах "объект параметров", "Коллекция параметров" и "объявление параметров (SQL)" в справке DAO.

##  <a name="getparameterinfo"></a>Кдаокуеридеф:: GetParameterInfo
...
##  <a name="getparamvalue"></a>Кдаокуеридеф:: Жетпарамвалуе
...
##  <a name="getrecordsaffected"></a>Кдаокуеридеф:: Жетрекордсаффектед
...
##  <a name="getreturnsrecords"></a>Кдаокуеридеф:: Жетретурнсрекордс
...
##  <a name="getsql"></a>Кдаокуеридеф:: Жетскл
...
##  <a name="gettype"></a>Кдаокуеридеф:: GetType
...
- перекрестное `dbQCrosstab`

- `dbQDelete` удалить

- `dbQUpdate` Update

- Добавление `dbQAppend`

- `dbQMakeTable` Make-Table

- `dbQDDL` определение данных

- `dbQSQLPassThrough` сквозной

- Объединение `dbQSetOperation`
...
##  <a name="isopen"></a>Кдаокуеридеф:: OnOpen

Вызовите эту функцию-член, чтобы определить, открыт ли в данный момент объект `CDaoQueryDef`.
...
Ненулевое значение, если объект `CDaoQueryDef` открыт в данный момент; в противном случае — 0.
...
##  <a name="m_pdatabase"></a>Кдаокуеридеф:: m_pDatabase
...
##  <a name="m_pdaoquerydef"></a>Кдаокуеридеф:: m_pDAOQueryDef
...
##  <a name="open"></a>Кдаокуеридеф:: Open
...
##  <a name="setconnect"></a>Кдаокуеридеф:: Сетконнект
...
Перед выполнением QueryDef, представляющего транзитный запрос SQL к источнику данных ODBC, задайте строку подключения с `SetConnect` и вызовите [сетретурнсрекордс](#setreturnsrecords) , чтобы указать, будет ли запрос возвращать записи.
...
##  <a name="setname"></a>Кдаокуеридеф:: SetName
...
Имена QueryDef являются уникальными, определяемыми пользователем именами. Можно вызвать `SetName` перед добавлением объекта QueryDef в коллекцию QueryDef.

##  <a name="setodbctimeout"></a>Кдаокуеридеф:: Сетодбктимеаут
...
Эта функция члена позволяет переопределить количество секунд по умолчанию перед выполнением последующих операций в подключенном источнике данных "время ожидания". Время ожидания операции может истекает из-за проблем с доступом к сети, чрезмерного времени обработки запросов и т. д. Вызовите `SetODBCTimeout` перед выполнением запроса с этим объектом QueryDef, если необходимо изменить значение времени ожидания запроса. (Так как ODBC повторно использует соединения, значение времени ожидания одинаково для всех клиентов в одном соединении.)

Значение времени ожидания запроса по умолчанию составляет 60 секунд.

##  <a name="setparamvalue"></a>Кдаокуеридеф:: Сетпарамвалуе
...
Укажите значение, устанавливаемое в качестве объекта `COleVariant`. Сведения о настройке требуемого значения и типа в объекте `COleVariant` см. в разделе Class [COleVariant](../../mfc/reference/colevariant-class.md).

##  <a name="setreturnsrecords"></a>Кдаокуеридеф:: Сетретурнсрекордс
...
В этом случае необходимо создать QueryDef и задать его свойства с помощью других функций-членов `CDaoQueryDef`. Описание внешних баз данных см. в разделе [сетконнект](#setconnect).

##  <a name="setsql"></a>Кдаокуеридеф:: Сетскл
