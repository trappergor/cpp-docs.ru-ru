...
> [!NOTE]
>  Если вы используете мастер классов, мастер определяет **перечисления** значение `CRecordView::IDD`, он указан в объявлении класса и использует его в списке инициализации членов для конструктора.  
...
### <a name="remarks"></a>Примечания  
 Реализация по умолчанию вызывает соответствующий `Move` функцию-член `CRecordset` объект, связанный с представлением записи.  
