...
> [!NOTE]
>  Если вы используете мастер классов, мастер определяет `enum` значение `CDaoRecordView::IDD` в объявлении класса и использует его в инициализации членов списка для конструктора.  
