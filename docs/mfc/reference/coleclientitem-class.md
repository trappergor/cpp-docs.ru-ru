...
### <a name="remarks"></a>Примечания  
 ...
 Если *bUndoable* — **FALSE**, контейнер следует отключить команда undo, фактически удаляя состояния отмены контейнера, так как он указывает, что последняя операция, выполненная на сервере не отменяемом.  
...
### <a name="remarks"></a>Примечания  
 `Release` вызывается методом `COleClientItem` деструктор.  
...
### <a name="remarks"></a>Примечания  
 Вызовите `Run` функцию-член, чтобы запустить приложение сервера перед активацией элемента. Это делается автоматически [активировать](#activate) и [DoVerb](#doverb), поэтому он обычно не требуется для вызова этой функции. Эта функция вызывается, если это необходимо для запуска сервера, чтобы задать атрибут элемента, например [SetExtent](#setextent), перед выполнением [DoVerb](#doverb).  
