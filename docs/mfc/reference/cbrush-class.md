...
|name|Описание|
|----------|-----------------|
|[Кбруш:: Кбруш](#cbrush)|Создает объект `CBrush`.|

### <a name="public-methods"></a>Открытые методы

|name|Описание|
|----------|-----------------|
|[CBrush::CreateBrushIndirect](#createbrushindirect)|Инициализирует кисть стилем, цветом и шаблоном, указанным в структуре [логбруш](/windows/win32/api/wingdi/ns-wingdi-logbrush) .|
...
|[CBrush::CreateHatchBrush](#createhatchbrush)|Инициализирует кисть с указанным заштрихованным шаблоном и цветом.|
...
|[Кбруш:: FromHandle](#fromhandle)|Возвращает указатель на `CBrush` объект при наличии маркера для объекта Windows `HBRUSH` .|
...
Чтобы использовать `CBrush` объект, `CBrush` создайте объект и передайте его в любую `CDC` функцию-член, для которой требуется кисть.
...
- HS_CROSS горизонтальная и вертикальная клетчатая

- Клетчатая HS_DIAGCROSSа на 45 градусов
...
*нусаже*<br/>
Указывает, должны `bmiColors[]` ли поля структуры данных [битмапинфо](/windows/win32/api/wingdi/ns-wingdi-bitmapinfo) (часть "упакованного" DIB ") содержать явные значения RGB или индексы в текущей логической палитре. Параметр должен иметь одно из следующих значений:
...
- В первой версии для получения маркера рисунка DIB вы вызываете функцию Windows `GlobalAlloc` для выделения блока глобальной памяти, а затем заполняете память упакованным DIB.
...
- [Креатедибпаттернбруш](/windows/win32/api/wingdi/nf-wingdi-createdibpatternbrush) (Эта функция предоставляется только для совместимости с приложениями, написанными для более ранних версий Windows, чем 3,0 `CreateDIBPatternBrushPt` ; используйте функцию.)
...
Когда приложение завершило использовать кисть `CreateSolidBrush`, созданную, она должна выбрать кисть вне контекста устройства.
...
Когда приложение завершило использовать кисть `CreateSysColorBrush`, созданную, она должна выбрать кисть вне контекста устройства.
...
Если объект еще не присоединен к обработчику, создается и прикрепляется временный `CBrush` объект. `CBrush` Этот временный `CBrush` объект действителен только до тех пор, пока приложение не пойдет в цикле событий в следующий раз. В настоящее время все временные графические объекты будут удалены. Иными словами, временный объект действителен только во время обработки одного сообщения окна.
...
Вызовите эту функцию члена, чтобы `LOGBRUSH` получить структуру.
...
Например, вызовите `GetLogBrush` метод, чтобы сопоставить определенный цвет или шаблон точечного рисунка.
...
Этот оператор используется для получения подключенного маркера `CBrush` GDI Windows объекта.
