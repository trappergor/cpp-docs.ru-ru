...
|name|Описание|
|----------|-----------------|
|[Кдаофиелдексчанже:: m_nOperation](#m_noperation)|Операция DFX, выполняемая текущим вызовом функции `DoFieldExchange` члена набора записей.|
...
Вызовите `SetFieldType` метод `CDaoRecordset` в `DoFieldExchange` переопределении класса.
...
Обычно перед каждой группой вызовов DFX, связанных с элементами данных поля или элементами данных параметров, должен предшествовать вызов `SetFieldType`. Параметр *нфиелдтипе* каждого `SetFieldType` вызова определяет тип элементов данных, представленных `SetFieldType` вызовами функции DFX, которые следуют за вызовом.
