...
### <a name="remarks"></a>Примечания  
 Создании `CScrollBar` объекта в два этапа. Во-первых, вызывает конструктор, который создает `CScrollBar` объекта; затем вызвать `Create`, который создает и инициализирует полосу прокрутки связанного Windows и прикрепляет его к `CScrollBar` объекта.  
