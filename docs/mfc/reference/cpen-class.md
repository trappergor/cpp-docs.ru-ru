 ...
 *pLogBrush*  
 Указывает `LOGBRUSH` структуры. Если *nPenStyle* — **PS_COSMETIC**, *lbColor* членом `LOGBRUSH` структура задает цвет пера и *lbStyle*членом `LOGBRUSH` структуры должно быть присвоено **BS_SOLID**. Если *nPenStyle* — **PS_GEOMETRIC**, все члены, которые необходимо использовать для указания атрибутов кисти пера.  
 ...
 *pLogBrush*  
 Указывает на [LOGBRUSH](http://msdn.microsoft.com/library/windows/desktop/dd145035) структуры. Если *nPenStyle* — **PS_COSMETIC**, **lbColor** членом `LOGBRUSH` структура задает цвет пера и *lbStyle*членом `LOGBRUSH` структуры должно быть присвоено **BS_SOLID**. Если **nPenStyle** — **PS_GEOMETRIC**, все члены, которые необходимо использовать для указания атрибутов кисти пера.  
...
-   Если *nPenStyle* — **PS_COSMETIC** и **PS_USERSTYLE**, записи в *lpStyle* массива укажите длину штрихов и пробелов в Стиль единиц. Стиль определяются устройства, в котором используется перо, чтобы нарисовать линию.  
  
-   Если *nPenStyle* — **PS_GEOMETRIC** и **PS_USERSTYLE**, записи в *lpStyle* массива укажите длину штрихов и пробелов в логические устройства.  
