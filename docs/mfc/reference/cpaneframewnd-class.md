...
[in] [out] *pParentWnd* указывает родительского фрейма окна области. Это значение не должно быть значение NULL.

[in] [out] *pContext* указывает контекст определяемые пользователем.
