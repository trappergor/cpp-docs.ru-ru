...
|[CWnd::OnHelpFinder](#onhelpfinder)|Обработка команд ID_HELP_FINDER и ID_DEFAULT_HELP.|  
|[CWnd::OnHelpIndex](#onhelpindex)|Обрабатывает ID_HELP_INDEX-команда и предоставляет раздел справки по умолчанию.|  
...
### <a name="parameters"></a>Параметры  
 ...
 [in] *nAdjustType*  
 Используйте `CWnd::adjustBorder` для вычисления координаты окна без стиль WS_EX_CLIENTEDGE; в противном случае используйте `CWnd::adjustOutside`.  
...
### <a name="parameters"></a>Параметры  
 *bKeys*  
 Значение TRUE для отмены всплывающие подсказки при нажатии клавиши и текст строки состояния присвоено значение по умолчанию. в противном случае — значение FALSE.  
 ...
 *Проверьте*  
 Указывает действие, предпринимаемое. Если *проверьте* отлично от нуля, `CheckDlgButton` функция-член устанавливает флажок рядом с кнопкой, если значение равно 0, флажок будет удалено. Для кнопок с тремя состояниями Если *проверьте* значение 2, неопределенное состояние кнопки.  
...
##  <a name="checkradiobutton"></a>  CWnd::CheckRadioButton  
 Выбирает (добавляет флажок, чтобы) данного переключателя в группе и очищает (удаляет флажок напротив) все остальные переключатели в группе.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Идентифицирует дочернего окна, содержащего точку. Он имеет значение NULL, если заданная точка находится за пределами клиентской области. Если точка находится в пределах области клиента, но не внутри любого дочернего окна `CWnd` возвращается.  
...
### <a name="remarks"></a>Примечания  
 `ClientToScreen` Функция-член использует клиентские координаты в `POINT` или `RECT` структуры или `CPoint` или `CRect` объекта, на который указывает *lpPoint* или *lpRect* для вычисления новые координаты экрана; Затем он заменяет координаты в структуре новые координаты. Новый экран координаты указываются относительно верхнего левого угла отображаемого системы.  
...
### <a name="parameters"></a>Параметры  
 ...
 [in] *lpszWindowName*  
 Указатель на строку с завершающим нулем, содержащего окно отображаемое имя; в противном случае значение NULL для не отображаемое имя окна.  
...
> [!WARNING]
> `CWnd::PreCreateWindow` Теперь назначает hMenu членом его `CREATESTRUCT` параметр **это** указателя, если меню имеет значение NULL, и стиль содержит WS_CHILD. На правильную работу убедитесь, что элемент управления диалогового окна имеет идентификатор, который не имеет значение NULL.  
>   
>  Это изменение устраняет потенциальный сбой в сценариях взаимодействия с управляемым и машинным. Оператора ТРАССИРОВКИ в `CWnd::Create` предупреждения разработчиков о проблеме.  
...
##  <a name="createcaret"></a>  CWnd::CreateCaret  
 Создает новую форму системного курсора и курсор во владение утверждений.  
...
### <a name="parameters"></a>Параметры  
 *pszClass*  
 Эта строка может содержать OLE «краткое имя» (ProgID) для класса, например, «CIRC3. Circ3Ctrl.1». Имя должно соответствовать тем же именем, зарегистрированные с помощью элемента управления. Кроме того строка может содержать строковая форма CLSID, заключенное в скобки, например, «{9DBAFCCF-592F-101B-85CE-00608CEC297B}». В любом случае `CreateControl` преобразует строку в соответствующий идентификатор класса.  
 ...
 *pParentWnd*  
 Указывает родительскому окну элемента управления. Оно не должно быть NULL.  
  
 *nID*  
 Указывает идентификатор элемента управления.  
  
 *pPersist*  
 Указатель на [CFile](../../mfc/reference/cfile-class.md) содержащий постоянное состояние для элемента управления. Значение по умолчанию — NULL, указывающее, что элемент управления инициализирует себя без восстановления свое состояние из любого из постоянного хранилища. Если значение не NULL, он должен быть указателем на `CFile`-производный объект, который содержит постоянных данных элемента управления в виде потока или хранилища. Удалось, эти данные были сохранены в предыдущей активации клиента. `CFile` Может содержать другие данные, но не устанавливается на первый байт постоянные данные во время вызова для чтения и записи указатель `CreateControl`.  
  
 *bStorage*  
 Указывает, является ли данные в *pPersist* должен интерпретироваться как IStorage или IStream данные. Если данные в *pPersist* — это хранилище *bStorage* необходимо указать значение TRUE. Если данные в *pPersist* представляет собой поток *bStorage* должно быть значение FALSE. Значение по умолчанию — FALSE.  
...
- WS_GROUP указывает первый элемент управления из группы элементов управления. Пользователь может изменить фокус от одного элемента управления в группе к другому с помощью клавиш направления. Все элементы управления, определенной с помощью стиля WS_GROUP после первого элемента управления принадлежать одной группе. Следующий элемент управления с использованием стиля WS_GROUP завершает группу и запускает следующей группы.  
...
### <a name="parameters"></a>Параметры  
 *dwExStyle*  
 Побитовое сочетание (OR) [расширенные стили окна](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles); в противном случае значение NULL по умолчанию расширенный стиль окна.  
...
>  Это изменение устраняет потенциальный сбой в сценариях взаимодействия с управляемым и машинным. Объект `TRACE` инструкции в `CWnd::Create` предупреждения разработчиков о проблеме.  
 ...
 Значение по умолчанию расширенный стиль окна — WS_EX_LEFT. Стиль окна по умолчанию — WS_OVERLAPPED.  
...
##  <a name="creategraycaret"></a>  CWnd::CreateGrayCaret  
 Создает серый прямоугольник для системного курсора и курсор во владение утверждений.  
...
### <a name="remarks"></a>Примечания  
 ...
 Граница окна ширины или высоты в системе может быть получена с [GetSystemMetrics](http://msdn.microsoft.com/library/windows/desktop/ms724385) функции Windows с индексами SM_CXBORDER и SM_CYBORDER. Использование границы окна Ширина или высота гарантирует, что курсор будет отображаться с высоким разрешением экрана.  
...
##  <a name="createsolidcaret"></a>  CWnd::CreateSolidCaret  
 Создает сплошной прямоугольник для системного курсора и курсор во владение утверждений.  
...
- Файлы DDL_READONLY только для чтения.  
...
- Флаг DDL_POSTMSGS LB_DIR. Если установлен флаг LB_DIR, Windows размещает сообщения, созданные `DlgDirList` в очередь приложения; в противном случае они отправляются напрямую к процедуре диалогового окна.  
  
- DDL_DRIVES диски. Если установлен флаг DDL_DRIVES, флаг DDL_EXCLUSIVE устанавливается автоматически. Таким образом, чтобы создать список каталогов, включающий дисков и файлов, необходимо вызвать `DlgDirList` дважды: один раз с DDL_DRIVES флаг набор и один раз с использованием флагов для оставшейся части списка.  
  
- Монопольный DDL_EXCLUSIVE бит. Если монопольного бит задан, включаются только файлы указанного типа; в противном случае перечислены обычные файлы и файлы указанного типа.  
 ...
 *LpPathSpec* параметр имеет следующий вид:  
 ...
 В этом примере *диск* — буква диска, *каталога* является допустимых имен каталогов, и *filename* имеет допустимое имя файла, который должен содержать по крайней мере один символ подстановки. Подстановочные знаки — это вопросительный знак (*) соответствует любому символу, а звездочка ( **\***), то есть соответствует любому количеству символов.  
...
- Флаг DDL_POSTMSGS CB_DIR. Если установлен флаг CB_DIR, Windows размещает сообщения, созданные `DlgDirListComboBox` в очередь приложения; в противном случае они отправляются напрямую к процедуре диалогового окна.  
...
### <a name="remarks"></a>Примечания  
 Только то окно, которое вызывает `DragAcceptFiles` с *bAccept* присвоено значение TRUE, параметр идентификации как обработать сообщение Windows WM_DROPFILES. Например, в приложении MDI Если `CMDIFrameWnd` окна указатель используется в `DragAcceptFiles` вызов только функции `CMDIFrameWnd` окно получает сообщение WM_DROPFILES. Это сообщение не отправляется ко всем открытым `CMDIChildWnd` windows. Для `CMDIChildWnd` окна, чтобы это сообщение, необходимо вызвать `DragAcceptFiles` с `CMDIChildWnd` окна указателя.  
...
### <a name="remarks"></a>Примечания  
 Поддержка Active Accessibility MFC по умолчанию достаточно для стандартных окон и элементов управления, включая элементы управления ActiveX; Тем не менее если ваш `CWnd`-производный класс содержит элементы пользовательского интерфейса nonwindowed, MFC предоставляет невозможно узнать о них. В этом случае необходимо переопределить соответствующий [функции-члены Active Accessibility](http://msdn.microsoft.com/en-us/68af04ac-4eb9-4b7d-b33f-c45512097a74) в вашем классе, и необходимо вызвать метод `EnableActiveAccessibility` в конструктор класса.  
...
- SB_BOTH включает или отключает стрелки полосы прокрутки по горизонтальной и вертикальной связанные с окном.  
  
- SB_HORZ включает или отключает стрелки горизонтальной полосы прокрутки связанные с окном.  
...
- Обе стрелки прокрутки позволяет ESB_ENABLE_BOTH панели (по умолчанию).  
...
### <a name="parameters"></a>Параметры  
 *bEnable*  
 Указывает, включен ли всплывающая подсказка. Значение TRUE включает элемент управления; FALSE отключает элемент управления.  
  
### <a name="return-value"></a>Возвращаемое значение  
 Значение TRUE, если включены всплывающие подсказки. в противном случае — значение FALSE.  
...
>  Некоторые окна, такие как [CToolBar](../../mfc/reference/ctoolbar-class.md), предоставляют встроенные реализации [OnToolHitTest](#ontoolhittest).  
 ...
 Просто вызов `EnableToolTips` не достаточно отображать всплывающие подсказки для элементов управления ваш ребенок, если родительское окно является производным от `CFrameWnd`. Это вызвано `CFrameWnd` предоставляет обработчик по умолчанию для уведомления TTN_NEEDTEXT. Если ваш родительское окно не является производным от `CFrameWnd`, который, если это диалоговое окно или представление формы, средство советы для своего ребенка, элементы управления будут отображаться неправильно, необходимо предоставить обработчик для уведомления TTN_NEEDTEXT средство подсказки. В разделе [подсказки](../../mfc/tool-tips-in-windows-not-derived-from-cframewnd.md).  
  
 Всплывающие подсказки по умолчанию, для windows с `EnableToolTips` отсутствует текст, связанный с ними. Чтобы получить текст для отображения всплывающей подсказки, уведомления TTN_NEEDTEXT отправляется родительскому окну элемента управления всплывающей подсказки непосредственно перед отображением подсказки. Если отсутствует обработчик для этого сообщения для назначения либо значение для *pszText* членом `TOOLTIPTEXT` структуры, не будет не текст, отображаемый для всплывающей подсказки.  
...
### <a name="parameters"></a>Параметры  
 *bEnable*  
 Указывает, является ли заданное окно, чтобы включить или отключить. Если этот параметр имеет значение TRUE, окно будет включена. Если этот параметр имеет значение FALSE, то окна будут отключены.  
...
##  <a name="endpaint"></a>  CWnd::EndPaint  
 Отмечает завершение рисования в заданном окне.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Значение TRUE, если ресурс диалогового окна выполняется; в противном случае — значение FALSE.  
 ...
 *lpszWindowName*  
 Указывает нулем строка, указывающая имя окна (заголовок окна). Если *lpWindowName* имеет значение NULL, все окна имена совпадают.  
  
### <a name="return-value"></a>Возвращаемое значение  
 Идентифицирует окно с заданным именем класса и имя окна. Он имеет значение NULL, если не найдено ни одного такого окна.  
 ...
 *lpszWindow*  
 Указатель на строку, завершающуюся значением null, указывающее имя окна (заголовок окна). Если этот параметр имеет значение NULL, все окна имена совпадают.  
  
### <a name="return-value"></a>Возвращаемое значение  
 Если функция выполняется успешно, возвращаемое значение является указателем на объект window, с указанного класса, а также имена окна. Если функция завершается с ошибкой, возвращается значение NULL.  
...
### <a name="parameters"></a>Параметры  
 *bInvert*  
 Указывает, является ли `CWnd` записанная флеш-память или вернуть в исходное состояние. `CWnd` Записанная флеш-память из одного состояния, если другие *bInvert* имеет значение TRUE. Если *bInvert* имеет значение FALSE, окно возвращается в исходное состояние (активное или неактивное).  
 ...
 *BInvert* параметр должен иметь значение FALSE, только если окно получает фокус ввода и больше не будет мигать; оно должно быть значение TRUE при последующих вызовах во время ожидания, чтобы получить фокус ввода.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Активное окно или значение NULL, если ни одного окна была активна во время вызова. Указатель может быть временным. Его не требуется сохранять для дальнейшего использования.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Если функция выполняется успешно, возвращаемое значение является указателем на объект window предка. Если функция завершается с ошибкой, возвращается значение NULL.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Идентифицирует окно, захваченный мышью. Он имеет значение NULL, если ни одно окно захват мыши.  
...
### <a name="parameters"></a>Параметры  
 *lpRect*  
 Указывает на [структура RECT](../../mfc/reference/rect-structure1.md) или `CRect` объект для получения клиентских координат. `left` И `top` элементов будет 0. `right` И `bottom` будет содержать элементы, ширина и высота окна.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Идентифицирует окно, которому принадлежит буфер обмена, если функция выполнена успешно. В противном случае возвращается значение NULL.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Идентифицирует окно, в данный момент отвечает за отображение в буфер обмена, в случае успешного выполнения; в противном случае — значение NULL (например, если используется не средство просмотра).  
...
### <a name="return-value"></a>Возвращаемое значение  
 Указатель на [IUnknown](http://msdn.microsoft.com/library/windows/desktop/ms680509) интерфейса элемента управления OLE, представленным этим `CWnd` объекта. Если этот объект не представляет элемента управления OLE, возвращается значение NULL.  
...
### <a name="remarks"></a>Примечания  
 ...
 Контекст устройства, принадлежащие к `CWnd` класс возвращается посредством `GetDC` CS_CLASSDC, CS_OWNDC или CS_PARENTDC задано стиля в функции-члена `WNDCLASS` структуры при регистрации класса.  
...
- DCX_CACHE Возвращает контекст устройства из кэша, а не в окне OWNDC или CLASSDC. Переопределяет CS_OWNDC и CS_CLASSDC.  
...
- Исключает DCX_EXCLUDERGN отсеченная область определяется *prgnClip* из видимой области контекст возвращаемой устройства.  
  
- DCX_INTERSECTRGN пересекается с вырезанной определяется *prgnClip* в видимой области контекст возвращаемой устройства.  
...
- DCX_PARENTCLIP использует видимой области родительского окна и игнорирует битов стиля WS_CLIPCHILDREN и WS_PARENTDC родительского окна. Это значение задает начало координат контекст устройства в верхний левый угол `CWnd` окна.  
...
### <a name="remarks"></a>Примечания  
 ...
 Чтобы получить контекст кэшированных устройства, приложение должно указывать [DCX_CACHE](http://msdn.microsoft.com/library/windows/desktop/dd144873). Если не указан DCX_CACHE и окно не является ни CS_OWNDC, ни [CS_CLASSDC](http://msdn.microsoft.com/library/windows/desktop/ms633576), эта функция возвращает значение NULL.  
  
 Возвращенный контекст устройства с особенности [GetDCEx](http://msdn.microsoft.com/library/windows/desktop/dd144873) функционировать, если CS_CLASSDC [CS_OWNDC](http://msdn.microsoft.com/library/windows/desktop/ms633576), или [CS_PARENTDC](http://msdn.microsoft.com/library/windows/desktop/ms633576) стиль был указан в [ Например, WNDCLASS](http://msdn.microsoft.com/library/windows/desktop/ms633576) структуры при регистрации класса.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Если функция выполнена успешно; отрисовки целевого объекта для указанного окна из контекста устройства в противном случае — значение NULL.  
 ...
 *bOnlyPerm*  
 Указывает, может ли быть временного окна должен быть возвращен. Если значение равно TRUE, могут быть возвращены только постоянное окно; Если значение равно FALSE, функция может вернуть временного окна. Дополнительные сведения о временных windows в разделе [технические Примечание 3](../../mfc/tn003-mapping-of-windows-handles-to-objects.md).  
  
### <a name="return-value"></a>Возвращаемое значение  
 Указатель на `CWnd` объект или значение NULL, если не найдено ни одного дочернего окна.  
...
### <a name="return-value"></a>Возвращаемое значение  
 ...
 При возникновении ошибок, например возникновения нечисловых знаков и превышение максимальной выше, `GetDlgItemInt` копирует 0 в расположении, указанном *lpTrans*. Если ошибок нет, *lpTrans* получает ненулевое значение. Если *lpTrans* имеет значение NULL, `GetDlgItemInt` выдает предупреждающих сообщений об ошибках.  
...
### <a name="remarks"></a>Примечания  
 ...
 Если *bSigned* имеет значение TRUE, `GetDlgItemInt` проверяет знак минус (-) в начале текста и преобразует текст в число со знаком. В противном случае она создает значение без знака.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Указатель на окно с текущим местоположением, или значение NULL, если нет фокуса окна.  
...
### <a name="remarks"></a>Примечания  
 Этот метод действует только окна обрабатывает сообщение WM_GETFONT. Многие классы MFC, которые являются производными от `CWnd` обрабатывают такие сообщения, поскольку они привязаны к классу стандартных окон, включающий обработчик сообщений для сообщения WM_GETFONT. С помощью этого метода, классы, производные от `CWnd` необходимо определить метод обработчика для сообщений WM_GETFONT.  
...
### <a name="parameters"></a>Параметры  
 *bBigIcon*  
 Указывает 32 x 32 пикселя значок, если задано значение TRUE; Указывает 16 пикселей по 16 пикселей значок, если значение равно FALSE.  
...
### <a name="parameters"></a>Параметры  
 *pcrKey*  
 Указатель на значение COLORREF, который получает ключ цвета прозрачности для использования при составлении многослойного окна. Все пиксели, созданный при помощи окна в этот цвет будет прозрачной. Это может быть значение NULL, если аргумент не требуется.  
  
 *pbAlpha*  
 Указатель БАЙТОВ, который получает значение альфа-канал, используемый для описания прозрачности многослойного окна. Если переменная ссылается *pbAlpha* равно 0, окно является полностью прозрачным. Если переменная ссылается *pbAlpha* равно 255, окно является непрозрачным. Это может быть значение NULL, если аргумент не требуется.  
  
 *pdwFlags*  
 Указатель на значение DWORD, принимающий флаг слоев. Это может быть значение NULL, если аргумент не требуется. Полный список возможных значений см. в разделе [GetLayeredWindowAttributes](http://msdn.microsoft.com/library/windows/desktop/ms633508).  
...
### <a name="return-value"></a>Возвращаемое значение  
 Идентифицирует меню. Это значение равно NULL, если `CWnd` меню не имеет. Возвращаемое значение не определено, если `CWnd` является дочерним окном.  
...
##  <a name="getmenubarinfo"></a>  CWnd::GetMenuBarInfo  
 Извлекает сведения о указанную строку меню.  
 ...
 *где Bпредыдущие_параметры*  
 Указывает способ поиска группу элементов управления в диалоговом окне функции. Значение TRUE, если функция выполняет поиск предыдущего элемента управления в группе; Если значение равно FALSE, он выполняет поиск следующего элемента управления в группе.  
  
 *pCurSiteOrWnd*  
 Идентифицирует `COleControlSiteOrWnd` элемента управления. Дополнительные сведения о `COleControlSiteOrWnd`, в разделе **примечания**.  
...
### <a name="remarks"></a>Примечания  
 Группы элементов управления начинается с элемента управления, который был создан с [WS_GROUP](styles-used-by-mfc.md#window-styles) стиль и заканчивается на последний элемент управления, который не был создан с использованием стиля WS_GROUP.  
...
|Без окон элемента управления ActiveX|Содержит не HWND. `m_pSite` Членом `COleControlSiteOrWnd` указывает на элемент управления `COleControlSite`и `m_hWnd` член имеет значение NULL.|  
|Стандартное окно|Содержит только HWND. `m_hWnd` Членом `COleControlSiteOrWnd` равно HWND окна и `m_pSite` член имеет значение NULL.|  
 ...
 *pCurSiteOrWnd*  
 Идентифицирует `COleControlSiteOrWnd` элемента управления. Дополнительные сведения о `COleControlSiteOrWnd`, в разделе [CWnd::GetNextDlgGroupItem](#getnextdlggroupitem).  
  
 *где Bпредыдущие_параметры*  
 Указывает, как диалоговое окно поиска функции. Значение TRUE, если функция выполняет поиск предыдущего элемента управления в диалоговое окно. Если значение равно FALSE, он выполняет поиск следующего элемента управления.  
  
### <a name="return-value"></a>Возвращаемое значение  
 Указатель на предыдущую или следующую элемент управления, к которому стиль WS_TABSTOP, при успешном выполнении функция-член.  
...
### <a name="parameters"></a>Параметры  
 *nFlag*  
 Определяет, возвращает ли функция указатель к следующему окну или предыдущее окно. Это может быть либо GW_HWNDNEXT, возвращающий окне, которое следует за `CWnd` объекта в списке диспетчера окон или GW_HWNDPREV, который возвращает предыдущее окно в списке диспетчера окон.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Дескриптор окна, буфер обмена, открыть, если функция выполнена успешно; в противном случае — значение NULL.  
...
### <a name="remarks"></a>Примечания  
 `GetParent` Функция возвращает указатель для непосредственного родителя (если он существует). Напротив [GetParentOwner](#getparentowner) функция возвращает указатель непосредственная родительского или владельца окна, которое не является дочерним окном (не имеет стиль WS_CHILD). Если у вас есть дочернее окно в дочернее окно `GetParent` и `GetParentOwner` возвращать разные результаты.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Указатель на окно фрейма, в случае успешного выполнения; в противном случае — значение NULL.  
...
### <a name="remarks"></a>Примечания  
 `GetParentOwner` Возвращает указатель непосредственная родительского или владельца окна, которое не является дочерним окном (не имеет стиль WS_CHILD). Можно задать текущее окно-владелец [SetOwner](#setowner). По умолчанию родительского окна является его владельцем.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Возвращает дескриптор окна для окна. Возвращает NULL, если `CWnd` не прикрепленный к окну или если он используется со значением NULL `CWnd` указателя.  
 ...
 *pWndTop*  
 Указатель, находящегося в верхней части окна. Может иметь значение NULL.  
  
### <a name="return-value"></a>Возвращаемое значение  
 Указатель безопасного владельца указанного окна.  
  
### <a name="remarks"></a>Примечания  
 Безопасный владельцем является окном первого не дочерний родительский *pParent*. Если *pParent* имеет значение NULL, в главном окне потока (извлекаются через [AfxGetMainWnd](../../mfc/reference/application-information-and-management.md#afxgetmainwnd)) используется для нахождения владельца.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Элемент управления полоса прокрутки того же уровня, или значение NULL, если нет.  
  
### <a name="remarks"></a>Примечания  
 Эта функция-член не обрабатывает полосы прокрутки, созданные при установке bits WS_HSCROLL или WS_VSCROLL во время создания окна. `CWnd` Реализация этой функции просто возвращает значение NULL. Производные классы, такие как `CView`, реализации описанных функций.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Если сообщение извлечено все значения, возвращается значение TRUE. В противном случае — FALSE.  
...
### <a name="parameters"></a>Параметры  
 *bRevert*  
 Задает действие, выполняемое. Если *bRevert* имеет значение FALSE, `GetSystemMenu` возвращает дескриптор копию оконного меню в настоящий момент. Эта копия изначально идентична меню элемента управления, но может быть изменена. Если *bRevert* имеет значение TRUE, `GetSystemMenu` восстанавливает меню управления состояние по умолчанию. Предыдущей, возможно, изменение, управление меню, если, удаляется. В этом случае возвращаемое значение не определено.  
  
### <a name="return-value"></a>Возвращаемое значение  
 Идентифицирует копию элемента управления меню, если *bRevert* имеет значение FALSE. Если *bRevert* имеет значение TRUE, возвращаемое значение не определено.  
...
### <a name="remarks"></a>Примечания  
 Окно верхнего уровня — окно, которое является дочерним для рабочего стола. Если `CWnd` не имеет вложенные окна, эта функция возвращает значение NULL.  
 ...
 *lpRect*  
 ...
 Присвойте этому параметру значение NULL, чтобы определить, существует ли в пределах области обновления `CWnd`. Если *lpRect* имеет значение NULL, `GetUpdateRect` функция-член возвращает ненулевое значение, если существует область обновления и 0, если он не поддерживает. Это обеспечивает способ определить, завершилась ли сообщения WM_PAINT из области, недопустимый. Не устанавливайте этот параметр в значение NULL в Windows версии 3.0 и более ранних версий.  
...
### <a name="remarks"></a>Примечания  
 Если `CWnd` был создан с CS_OWNDC стиль и сопоставление режим не режим MM_TEXT, `GetUpdateRect` функция-член предоставляет прямоугольника в логических координатах. В противном случае `GetUpdateRect` дает прямоугольника в клиентских координатах. Если ни один регион обновления не `GetUpdateRect` задает прямоугольник, быть пустым (задает все координаты 0).  
  
 *BErase* параметр указывает, является ли `GetUpdateRect` необходимо стереть фон области обновления. Если *bErase* имеет значение TRUE и обновление области не является пустым, будут удалены в фоновом режиме. Стирание фона, `GetUpdateRect` отправляет [WM_ERASEBKGND](#onerasebkgnd) сообщения.  
 ...
 *bErase*  
 Определяет, будет удалена в фоновом режиме и получаются неклиентской областей дочерних окон. Если значение равно FALSE, рисование не выполняется.  
  
### <a name="return-value"></a>Возвращаемое значение  
 Указывает короткое целое флаг, указывающий тип результирующей области. Значение может принимать одно из следующих:  
  
- SIMPLEREGION области имеет перекрывающиеся границы отсутствуют.  
  
- COMPLEXREGION области имеет перекрывающиеся границы.  
  
- NULLREGION области является пустым.  
  
- Ни один регион создания ошибки.  
...
##  <a name="getwindow"></a>  CWnd::GetWindow  
 Возвращает указатель в окно запрошено, или значение NULL, если нет.  
...
- Идентифицирует GW_CHILD `CWnd` первого дочернего окна.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Число дочерних окон, связанных с `CWnd` объекта.  
  
##  <a name="getwindowdc"></a>  CWnd::GetWindowDC  
 Извлекает контекст отображения для всего окна, включая заголовок окна, меню и полосы прокрутки.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Идентифицирует контекст отображения для заданного окна, если функция выполнена успешно; в противном случае — значение NULL.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Количество безоконных дочерних окон, связанных с `CWnd` объекта.  
...
### <a name="remarks"></a>Примечания  
 `flags` Членом [WINDOWPLACEMENT](../../mfc/reference/windowplacement-structure.md) структуры, полученные при помощи этой функции всегда равно 0. Если `CWnd` развернуто, `showCmd` членом `WINDOWPLACEMENT` — SW_SHOWMAXIMIZED. Если окно свернуто, это SW_SHOWMINIMIZED. В противном случае — SW_SHOWNORMAL.  
...
- COMPLEXREGION области имеет более одного прямоугольника.  
...
### <a name="remarks"></a>Примечания  
 Флаги MF_HILITE и MF_UNHILITE можно использовать только с помощью этой функции-члена; не может использоваться с [CMenu::ModifyMenu](../../mfc/reference/cmenu-class.md#modifymenu) функции-члена.  
...
### <a name="remarks"></a>Примечания  
 Клиентской области помечен для прорисовки при следующем [WM_PAINT](#onpaint) возникает сообщение. Области могут также проверяться перед инициализацией сообщения WM_PAINT по [ValidateRect](#validaterect) или [ValidateRgn](#validatergn) функции-члена.  
  
 *BErase* параметр указывает, является ли фон в пределах области обновления для удаления при обработке в область обновления. Если *bErase* имеет значение TRUE, фон удаляется при [BeginPaint](#beginpaint) вызывается функция-член; Если *bErase* имеет значение FALSE, фон не изменяется. Если *bErase* имеет значение TRUE для любой части области обновления, фон всей области не только в данной части будут удалены.  
...
### <a name="parameters"></a>Параметры  
 *lpRect*  
 Указывает на `CRect` объекта или [структура RECT](../../mfc/reference/rect-structure1.md) , содержащий прямоугольника (в клиентских координатах) добавляется в область обновления. Если *lpRect* имеет значение NULL, всю клиентскую область добавляется к области.  
  
 *bErase*  
 Указывает, является ли фон в пределах области обновления для удаления.  
  
### <a name="remarks"></a>Примечания  
 Недействительная прямоугольник, а также других областей в область обновления помечен для прорисовки при следующей [WM_PAINT](#onpaint) отправляется сообщение. Недействительная области накапливаются в область обновления до обработки при следующем вызове WM_PAINT регион или область проверяется с [ValidateRect](#validaterect) или [ValidateRgn](#validatergn) члена функция.  
  
 *BErase* параметр указывает, является ли фон в пределах области обновления для удаления при обработке в область обновления. Если *bErase* имеет значение TRUE, фон удаляется при [BeginPaint](#beginpaint) вызывается функция-член; Если *bErase* имеет значение FALSE, фон не изменяется. Если *bErase* имеет значение TRUE для любой части области обновления, фон весь регион будут удалены, не только в данной части.  
...
### <a name="parameters"></a>Параметры  
 *pRgn*  
 Указатель на [CRgn](../../mfc/reference/crgn-class.md) объект, который определяет область для добавления в область обновления. Предполагается, что область имеет клиентские координаты. Если этот параметр имеет значение NULL, всю клиентскую область добавляется в область обновления.  
  
 *bErase*  
 Указывает, является ли фон в пределах области обновления для удаления.  
  
### <a name="remarks"></a>Примечания  
 Недопустимая область, а также других областей в область обновления помечен для прорисовки при [WM_PAINT](#onpaint) затем отправляется сообщение. Недействительная области накапливаются в область обновления до области обрабатывается при отправке сообщения WM_PAINT рядом или область проверяется с [ValidateRect](#validaterect) или [ValidateRgn](#validatergn) члена функция.  
 ...
 *pvRet*  
 Адрес переменной, которая будет будет принимать значение свойства или возвращаемое значение. Он должен соответствовать типу, определяемое *vtRet*.  
  
 *pbParamInfo*  
 Указатель на строку, завершающуюся значением null байтов, указывающих типы параметров после *pbParamInfo*. Возможные значения см. в разделе «Примечания» `COleDispatchDriver::InvokeHelper`.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Значение TRUE, если `CWnd` сенсорный ввод поддержка; в противном случае — значение FALSE.  
...
### <a name="remarks"></a>Примечания  
 Окно может иметь состояние видимости, обозначенном бит стиля WS_VISIBLE. Когда этот бит стиля устанавливается с помощью вызова [ShowWindow](#showwindow) функция-член, отображается окно и последующие рисования в окно отображается при условии, что окно имеет набор бит стиля.  
  
 Любое изображение в окно, в которой WS_VISIBLE стиль будет отображаться не в том случае, если окно охватывается других окон или отсекаются родительского окна.  
  
### <a name="example"></a>Пример  
 [!code-cpp[NVC_MFCWindowing#103](../../mfc/reference/codesnippet/cpp/cwnd-class_43.cpp)]  
  
##  <a name="iszoomed"></a>  CWnd::IsZoomed  
 Определяет, является ли `CWnd` полноэкранном режиме.  
...
##  <a name="lockwindowupdate"></a>  CWnd::LockWindowUpdate  
 Отключает рисования в заданном окне.  
...
### <a name="parameters"></a>Параметры  
 *pwndTo*  
 Идентифицирует окно, в который преобразуются точек. Если этот параметр имеет значение NULL, точки, преобразуются в экранных координатах.  
 ...
 *nCount*  
 Указывает количество `POINT` структур в массиве, на который указывает *lpPoint*.  
 ...
 *lpszCaption*  
 Указывает на `CString` объект или строка, заканчивающаяся, используемый для заголовка окна сообщения. Если *lpszCaption* имеет значение NULL, «Ошибка» используется заголовок по умолчанию.  
...
### <a name="remarks"></a>Примечания  
 ...
 Если *nFlags* отлично от нуля, `ModifyStyle` вызывает функцию Windows API [SetWindowPos](http://msdn.microsoft.com/library/windows/desktop/ms633545) и на экран окно путем объединения *nFlags* с помощью следующие четыре стиля флаги:  
...
> [!NOTE]
>  Для некоторых стилей в некоторых элементах управления (ES_READONLY стиль в элементе управления редактированием, например) `ModifyStyle` не может правильно изменить стиль, так как элемент управления может потребоваться выполнить специальные внутренней обработки. В этих случаях соответствующего сообщения, чтобы изменить стиль будет доступен (EM_SETREADONLY в тот пример).  
  
### <a name="example"></a>Пример  
 [!code-cpp[NVC_MFCWindowing#105](../../mfc/reference/codesnippet/cpp/cwnd-class_45.cpp)]  
  
##  <a name="modifystyleex"></a>  CWnd::ModifyStyleEx  
 Вызовите эту функцию-член для изменения расширенный стиль окна.  
...
### <a name="remarks"></a>Примечания  
 ...
 Если *nFlags* отлично от нуля, `ModifyStyleEx` вызывает функцию Windows API [SetWindowPos](http://msdn.microsoft.com/library/windows/desktop/ms633545) и на экран окно путем объединения *nFlags* с помощью следующие четыре стиля флаги:  
 ...
 *bRepaint*  
 Указывает, является ли `CWnd` является повторное окрашивание. Если значение равно TRUE, `CWnd` получает [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) сообщений в его [OnPaint](#onpaint) обработчик сообщений обычным образом. Если этот параметр имеет значение FALSE, перерисовка любого типа, не происходит. Это относится к клиентской области, для неклиентской области (включая заголовок и полосами прокрутки) и с любой частью родительского окна, которые были выявлены в результате использования `CWnd`переместить. Если этот параметр имеет значение FALSE, приложение должно явно приводят к недействительности или перерисовывает все части `CWnd` и родительское окно, которое требуется перерисовка.  
...
- Нажмите кнопку WA_ACTIVE, окно активируется по какой-либо метод, отличный от мыши (например, с помощью интерфейса клавиатуры для выбора окна).  
 ...
 *bMinimized*  
 Указывает состояние свернутое `CWnd` активируется или деактивируется. Значение TRUE указывает, что окно свернуто.  
  
 Значение TRUE, если `CWnd` активируется; в противном случае деактивации.  
...
### <a name="parameters"></a>Параметры  
 *bActive*  
 Указывает, является ли `CWnd` активации или деактивации. Значение TRUE указывает `CWnd` активируется. Значение FALSE указывает `CWnd` становится неактивным.  
  
 *dwThreadID*  
 Указывает значение идентификатор потока. Если *bActive* имеет значение TRUE, *dwThreadID* определяет поток, который владеет `CWnd` быть активным. Если *bActive* имеет значение FALSE, *dwThreadID* определяет поток, который владеет `CWnd` время активации.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Значение TRUE, если поддерживается внешнего свойства. Значение FALSE, если это не так.  
...
##  <a name="onaskcbformatname"></a>  CWnd::OnAskCbFormatName  
 Платформа вызывает эту функцию-член, если в буфере обмена содержатся данные дескриптора для CF_OWNERDISPLAY форматирования (то есть, когда владелец буфера обмена будет отображаться содержимое буфера обмена).  
...
### <a name="remarks"></a>Примечания  
 ...
 Переопределить эту функцию-член и скопировать в указанный буфер, не превышающее максимальное число байтов, указанное имя формата CF_OWNERDISPLAY.  
...
- Элемент пользовательского интерфейса UIS_CLEAR state (заданные *nUIElement*) должен быть скрыт.  
  
- Элемент пользовательского интерфейса UIS_INITIALIZE state (заданные *nUIElement*) должны изменяться в зависимости от последнего входного события. Дополнительные сведения см. в разделе **примечания** раздел [WM_CHANGEUISTATE](http://msdn.microsoft.com/library/windows/desktop/ms646342).  
  
- Элемент пользовательского интерфейса UIS_SET state (заданные *nUIElement*) должны быть видимыми.  
...
- Индикаторы UISF_HIDEFOCUS фокус.  
 ...
 *pLResult*  
 Указатель на значение, возвращаемое из процедуре родительского окна. Этот указатель будет равно NULL, если ожидается, не возвращая значения.  
...
##  <a name="onclose"></a>  CWnd::OnClose  
 Платформа вызывает эту функцию-член в качестве сигнала, `CWnd` или завершения приложения.  
...
|[in] *bOpacity*|Значение TRUE, если новый цвет смешивается с непрозрачности; Значение FALSE, если это не так.|  
...
### <a name="remarks"></a>Примечания  
 `OnCommand` выполняет сопоставление сообщений для уведомлений элемента управления, ON_COMMAND записи и вызывает соответствующую функцию-член.  
...
### <a name="remarks"></a>Примечания  
 Если в поле со списком или список создается с [CBS_SORT](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) или [LBS_SORT](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) стиля, Windows отправляет владельцем поле со списком или список WM_COMPAREITEM сообщение каждый раз, когда приложение добавляет новый элемент.  
...
##  <a name="oncompositionchanged"></a>  CWnd::OnCompositionChanged  
 Эта функция-член вызывается платформой для всех окон верхнего уровня при композиции диспетчера окон рабочего стола (DWM) включен или отключен.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Возвращает значение TRUE, если принимающее приложение успешно принимает данные. В противном случае возвращает FALSE.  
...
- CTLCOLOR_BTN Button-элемент управления  
  
- Диалоговое окно «CTLCOLOR_DLG»  
  
- Элемент управления CTLCOLOR_EDIT Edit  
  
- Элемент управления CTLCOLOR_LISTBOX списков  
...
### <a name="remarks"></a>Примечания  
 ...
 Чтобы изменить цвет фона элемента управления редактированием однострочный, задать для дескриптора кисти CTLCOLOR_EDIT и CTLCOLOR_MSGBOX коды сообщений, и вызовите метод [CDC::SetBkColor](../../mfc/reference/cdc-class.md#setbkcolor) функции в ответ на CTLCOLOR_EDIT кода.  
...
##  <a name="ondevicechange"></a>  CWnd::OnDeviceChange  
 Платформа вызывает эту функцию-член для уведомления приложения или драйвера устройства об изменении конфигурации оборудования компьютера или устройства.  
...
### <a name="remarks"></a>Примечания  
 Для устройств, которые обеспечивают управляемое программное обеспечение возможностей, таких как извлечение и блокировки операционная система обычно отправляет сообщение DBT_DEVICEREMOVEPENDING позволяют приложениям и драйверы устройств закончить связь правильно их использование устройства.  
  
 Если операционная система принудительно удаляет устройства, он не может отправить сообщение DBT_DEVICEQUERYREMOVE перед этим действием.  
...
##  <a name="ondevmodechange"></a>  CWnd::OnDevModeChange  
 Платформа вызывает эту функцию-член для всех верхнего уровня `CWnd` объектов, когда пользователь изменяет параметры режима устройства.  
...
### <a name="remarks"></a>Примечания  
 Приложений, которые обрабатывают сообщения WM_DEVMODECHANGE может повторно инициализировать свои параметры режима устройства. Приложения, использующие Windows `ExtDeviceMode` функцией для сохранения и восстановления параметры устройства обычно не обрабатывать эту функцию.  
 ...
 *szRequiredThumbnailSize*  
 Указывает размер целевой эскиз. Если следует игнорировать *bIsThumbnail* имеет значение FALSE.  
  
 *bIsThumbnail*  
 Указывает, является ли этот метод вызывается для преобразованного в значок эскиза или интерактивный просмотр (Просмотр).  
  
 *bAlphaChannelSet*  
 [out] Присвойте ему значение TRUE, если реализация инициализирует альфа-канала растрового изображения, выбранного в *контроллера домена*.  
  
### <a name="remarks"></a>Примечания  
 Переопределите этот метод в производном классе и рисования в заданном контексте устройства для настройки эскиза и считывания. Если *bThumbnail* имеет значение TRUE, *szRequiredThumbnailSize* можно игнорировать. В этом случае следует иметь в виду, рисуется растровое изображение полного размера (то есть, битовая карта, охватывает всю клиентскую область). Контекст устройства ( *контроллера домена*) поставляется с выбранной 32 бита растрового изображения. Реализация по умолчанию отправляет WM_PRINT это окно с флагами PRF_CLIENT, PRF_CHILDREN и PRF_NONCLIENT.  
...
##  <a name="onenable"></a>  CWnd::OnEnable  
 Платформа вызывает эту функцию-член, когда приложение изменяет состояние выполнения `CWnd` объекта.  
...
### <a name="parameters"></a>Параметры  
 *bEnable*  
 Указывает, является ли `CWnd` объекта была включена или отключена. Этот параметр имеет значение TRUE, если `CWnd` была включена; равно FALSE, если `CWnd` был отключен.  
...
### <a name="parameters"></a>Параметры  
 *ломаный*  
 Указывает, выполняется ли сеанс заканчивается. Он имеет значение TRUE, если сеанс завершен; в противном случае — значение FALSE.  
  
### <a name="remarks"></a>Примечания  
 `OnEndSession` Информирует вызов `CWnd` объекта фактически окончания сеанса.  
  
 Если *ломаный* имеет значение TRUE, Windows может завершить в любое время после всех приложений возвратили может обрабатывать этот вызов. Следовательно, имеют приложения выполнять все задачи, необходимые для завершения в `OnEndSession`.  
...
##  <a name="onenteridle"></a>  CWnd::OnEnterIdle  
 Платформа вызывает эту функцию-член для информирования процедуры главного окна приложения, что модального диалогового окна или меню переходит в состояние простоя.  
...
### <a name="remarks"></a>Примечания  
 ...
 Если `hbrBackground` член имеет значение NULL, переопределенная версия `OnEraseBkgnd` следует удалить цвет фона. Ваша версия также должны быть выровнены источника предполагаемого кисть `CWnd` координаты путем вызова метода [UnrealizeObject](http://msdn.microsoft.com/library/windows/desktop/dd145164) для кисти, а затем выбрав кисти.  
  
 Переопределенный `OnEraseBkgnd` должны возвращать ненулевое значение, в ответ на WM_ERASEBKGND, если он обрабатывает сообщение и будут удалены в фоновом режиме; это означает, что необходимо дальнейшая стирание. Если возвращается значение 0, окно останется помеченным как нуждающийся в будут уничтожены. (Как правило, это означает, что `fErase` членом `PAINTSTRUCT` структуры будет равен TRUE.)  
  
 Windows предполагается, что фон вычисляется с помощью режима сопоставления режим MM_TEXT. Если контекст устройства используется любой другой режим сопоставления, области удалены может оказаться в видимой части клиентской области.  
  
> [!NOTE]
>  Это функция-член вызывается платформой, чтобы разрешить приложению обработать сообщение Windows. Параметры, передаваемые в функцию, отражают параметры, полученные платформой при поступлении сообщения. При вызове реализации базового класса этой функции эта реализация будет использовать параметры, изначально переданные с сообщением, а не параметры, передаваемые в функции.  
  
##  <a name="onexitmenuloop"></a>  CWnd::OnExitMenuLoop  
 Эта функция-член вызывается платформой при выходе из модального цикла меню.  
...
- Нет DLGC_UNDEFPUSHBUTTON кнопка обработки по умолчанию. (Приложение может использовать этот флаг с DLGC_BUTTON для указывают обрабатывает входные данные кнопки, но использует для обработки кнопка по умолчанию в системе.)  
...
- Сообщений DLGC_WANTCHARS WM_CHAR.  
  
- Все DLGC_WANTMESSAGE ввод с клавиатуры. Приложение передает сообщение в элемент управления.  
  
- Клавиша DLGC_WANTTAB TAB.  
...
##  <a name="onhelpfinder"></a>  CWnd::OnHelpFinder  
 Обработка команд ID_HELP_FINDER и ID_DEFAULT_HELP.  
...
##  <a name="onhelpindex"></a>  CWnd::OnHelpIndex  
 Обрабатывает ID_HELP_INDEX-команда и предоставляет раздел справки по умолчанию.  
...
### <a name="remarks"></a>Примечания  
 Если меню не активна, при нажатии клавиши F1, WM_HELP помещается в окно, связанное с меню; в противном случае WM_HELP отправляется в окно, в котором фокус клавиатуры. Если ни одно окно имеет фокус клавиатуры, WM_HELP отправляется активной в настоящее время.  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[in] *nHotKeyId*|Идентификатор для сочетания клавиш, которая создала сообщение. Если сообщение было создано системные сочетания клавиш, этот параметр будет иметь одно из следующих значений:<br /><br /> Сочетания клавиш - IDHOT_SNAPDESKTOP - desktop оснастка была нажата.<br />Сочетания клавиш - IDHOT_SNAPWINDOW - окно оснастки была нажата.|  
|[in] *nKey1*|Побитовое сочетание (OR) флагов, которое указывает ключи, которые были нажаты в сочетании с ключом, определяется *nKey2* параметра. Допустимые значения:<br /><br /> -Был удерживании MOD_ALT - либо клавиша ALT.<br />-Был удерживании MOD_CONTROL - либо клавишу CTRL.<br />-Был удерживании MOD_SHIFT - либо клавишу SHIFT.<br />-Был удерживании MOD_WIN - или ключ WINDOWS. Эти ключи обозначены логотипом Microsoft Windows.|  
...
- SB_PAGELEFT один страница прокрутки влево.  
  
- SB_PAGERIGHT один страница прокрутки вправо.  
  
- SB_RIGHT прокрутки в правом крае.  
  
- Прокрутите SB_THUMBPOSITION абсолютное положение. Текущее положение определяется *nPos* параметра.  
  
- Перетащите SB_THUMBTRACK полосы прокрутки для указанной позиции. Текущее положение определяется *nPos* параметра.  
 ...
 *pScrollBar*  
 Если сообщения прокрутки взята из элемента управления полосы прокрутки, содержит указатель на элемент управления. Если пользователь щелкнул полосу прокрутки окна, этот параметр имеет значение NULL. Указатель может быть временным. Его не требуется сохранять для дальнейшего использования.  
  
### <a name="remarks"></a>Примечания  
 Код SB_THUMBTRACK полосы прокрутки обычно используется приложениями, которые предоставляют отзыв, пока полосы прокрутки.  
...
- SB_PAGEDOWN Scroll одну страницу вниз.  
  
- SB_PAGEUP Scroll одну страницу вверх.  
  
- Прокрутите SB_THUMBPOSITION абсолютное положение. Текущая позиция предоставляется в *nPos*.  
  
- Прокрутка SB_TOP левый верхний угол.  
 ...
 *bSysMenu*  
 Значение TRUE, если во всплывающем меню в меню элемента управления; в противном случае — значение FALSE.  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[in] *uFlag*|Этот флаг может содержать следующие значения:<br /><br /> Новое устройство - GIDC_ARRIVAL - объект добавлен в систему.<br />Устройство - GIDC_REMOVAL - объект был удален из системы.|  
...
##  <a name="oninputlangchange"></a>  CWnd::OnInputLangChange  
 Этот член вызывается платформой для верхнему затронутому окну после изменения языка ввода приложения.  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[in] *nCharSet*|Кодировка нового языкового стандарта. Дополнительные сведения см. в разделе *азиатских языков* параметр [LOGFONT](http://msdn.microsoft.com/library/windows/desktop/dd145037) структуры.|  
...
##  <a name="oninputlangchangerequest"></a>  CWnd::OnInputLangChangeRequest  
 Платформа вызывает этот член для окну с фокусом, когда пользователь выбирает новый язык ввода.  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[in] *nFlags*|Побитовое сочетание (OR) флагов, которое указывает, был выбран новый язык из предыдущего или следующего языкового стандарта в списке установленных языковых стандартов, или что раскладка клавиатуры новый язык ввода может использоваться с кодировкой системы.<br /><br /> Возможные значения: INPUTLANGCHANGE_BACKWARD, INPUTLANGCHANGE_FORWARD и INPUTLANGCHANGE_SYSCHARSET.|  
|[in] *nLocaleId*|Идентификатор языка ввода. Дополнительные сведения см. в разделе [языка константы и строки кода](http://msdn.microsoft.com/library/windows/desktop/dd318693).|  
...
##  <a name="onkeydown"></a>  CWnd::OnKeyDown  
 Эта функция-член вызывается платформой при нажатии несистемной клавиши.  
...
|15|Переход состояния (1, если ключ освобождается, 0, если при нажатии клавиши).|  
 ...
 Сообщения об WM_KEYDOWN перехода ключ разряд (15) равен 0, а код контекста разряд (13) равно 0.  
...
|15|Переход состояния (1, если ключ освобождается, 0, если при нажатии клавиши).|  
 ...
 Сообщения об WM_KEYUP перехода ключ разряд (15) — 1, код контекста разряд (13) равно 0.  
...
### <a name="parameters"></a>Параметры  
 *pNewWnd*  
 Задает указатель на окно, который получает фокус ввода (может иметь значение NULL или может быть временным).  
...
### <a name="parameters"></a>Параметры  
 *bActivate*  
 Значение TRUE, если дочерний активирована и значение FALSE, если он отключен.  
...
### <a name="remarks"></a>Примечания  
 Переопределить эту функцию-член и заполните `MEASUREITEMSTRUCT` структуру данных, на который указывает *lpMeasureItemStruct* и возврат; это сообщает Windows размеры элемента управления и позволяет обрабатывать взаимодействие пользователей с Windows управлять правильно.  
 ...
 Система Windows инициирует вызов `OnMeasureItem` для владельца поля со списком и списки, созданные с использованием стиля OWNERDRAWFIXED перед отправкой [WM_INITDIALOG](http://msdn.microsoft.com/library/windows/desktop/ms645428) сообщения. В результате когда владелец получает этот вызов, Windows еще не определено высоты и ширины шрифта, используемого в элементе управления; вызовы функций и вычислений, которые требуются эти значения должны происходить в функцию main приложения или библиотеки.  
 ...
 `OnMeasureItem` вызывается только в том случае, если создается класс элемента управления во время выполнения, или он создается с использованием стиля LBS_OWNERDRAWVARIABLE или CBS_OWNERDRAWVARIABLE. Если элемент управления создается с помощью диалогового окна редактора `OnMeasureItem` не будет вызван. Это вызвано [WM_MEASUREITEM](http://msdn.microsoft.com/library/windows/desktop/bb775925) сообщение отправляется на раннем этапе процесса создания элемента управления. Если с помощью подкласс `DDX_Control`, `SubclassDlgItem`, или `SubclassWindow`, создания подкласса обычно возникает после завершения процесса создания. Таким образом, нет возможности обрабатывать [WM_MEASUREITEM](http://msdn.microsoft.com/library/windows/desktop/bb775925) сообщение в элементе управления `OnChildNotify` функция, которая является механизмом, MFC использует для реализации ON_WM_MEASUREITEM_REFLECT.  
 ...
 *nFlags*  
 Содержит флаг MF_POPUP, если меню является всплывающим меню. Он содержит флаг MF_SYSMENU, если меню элемента управления меню.  
...
- Элемент MF_CHECKED проверяется.  
...
- Элемент MF_OWNERDRAW — это рисуемый владельцем элемент.  
  
- Элемент MF_POPUP содержит всплывающего меню.  
  
- Элемент MF_SEPARATOR является разделителем пункта меню.  
  
- MF_SYSMENU элемент содержится в меню «Управление».  
  
 *hSysMenu*  
 Если *nFlags* содержит MF_SYSMENU, определяет меню, связанное с сообщением. Если *nFlags* содержит MF_POPUP, обозначает дескриптор главного меню. Если *nFlags* не содержит ни MF_SYSMENU, ни MF_POPUP, он не используется.  
...
### <a name="remarks"></a>Примечания  
 Реализация по умолчанию передает это сообщение в родительское окно, перед выполнением действий. Если родительское окно возвращает значение TRUE, обработка прерывается.  
...
##  <a name="onmousehover"></a>  CWnd::OnMouseHover  
 Платформа вызывает эту функцию-член, когда курсор перемещается над клиентской областью окна в течение периода времени, указанного в предыдущем вызове [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265).  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[in] *nFlags*|Побитовое сочетание (OR) флагов, которое указывает, при нажатии клавиши-модификаторы. Например флаг MK_CONTROL указывает, что нажата клавиша CTRL.|  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[in] *nFlags*|Побитовое сочетание (OR) флагов, которое указывает, при нажатии клавиши-модификаторы. Например флаг MK_CONTROL указывает, что нажата клавиша CTRL.<br /><br /> Список флагов см. в разделе подзаголовка «Параметры сообщения» в [о ввода мыши](http://msdn.microsoft.com/library/windows/desktop/ms645601).|  
|[in] *zDelta*|Указывает расстояние, на которые поворачивается колесико мыши, выраженное в кратные или подразделения WHEEL_DELTA, имеющей значение 120. Положительное значение указывает, что был колесика вправо; отрицательное значение указывает, что был колесика слева.|  
...
##  <a name="onmousemove"></a>  CWnd::OnMouseMove  
 Эта функция-член вызывается платформой при перемещении курсора мыши.  
...
### <a name="remarks"></a>Примечания  
 Если указатель мыши не записан, получает сообщение WM_MOUSEMOVE `CWnd` объекта под курсором мыши; в противном случае отправкой сообщения окна, захвативший мышь.  
 ...
 *ZDelta* параметра должно быть кратно WHEEL_DELTA, который составляет 120. Это значение является пороговое значение для действия, которые должны выполняться, и одна такая операция (например, прокрутки вперед на одно деление) должно выполняться для каждого изменений.  
  
 Чтобы предусмотреть wheels с высоким разрешением, например колеса свободно поворот с не деления WHEEL_DELTA было присвоено 120. Колесо высоким разрешением отправляет больше сообщений за поворота, но каждое сообщение имеет меньшее значение смещения. Чтобы использовать такие колесико мыши, либо добавьте входящего *zDelta* значения до достижения WHEEL_DELTA (, чтобы получить отклик для заданного дельта поворот) или прокручивать строки частично в ответ на частые сообщения. Можно также выбрать уровень детализации прокрутки и накапливать детали, пока не будет достигнут WHEEL_DELTA.  
...
##  <a name="onmove"></a>  CWnd::OnMove  
 Платформа вызывает эту функцию-член после `CWnd` объект был перемещен.  
...
### <a name="parameters"></a>Параметры  
 *bActive*  
 Указывает, когда строки заголовка или значок должно быть изменено для указания активного и неактивного состояния. *BActive* параметр имеет значение TRUE, если активного заголовка или значок должна отображаться. Он имеет значение FALSE для неактивного заголовка или значок.  
...
### <a name="remarks"></a>Примечания  
 Реализация по умолчанию отображает строку заголовка и заголовка текста в их активные цвета Если *bActive* имеет значение TRUE и их неактивные цветов Если *bActive* имеет значение FALSE.  
...
### <a name="remarks"></a>Примечания  
 ...
 `rgrc[1]` И `rgrc[2]` прямоугольники допустимы только если *bCalcValidRects* имеет значение TRUE. В этом случае `rgrc[1]` прямоугольник содержит координаты окна, прежде чем он был перемещен или изменения размера. `rgrc[2]` Прямоугольник содержит координаты клиентской области окна, прежде чем был перемещен в окне. Все координаты указываются относительно родительского окна или экрана.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Ненулевое значение, если создается неклиентской области. Он равен 0, если произошла ошибка; `Create` функция возвратит **сбоя** в этом случае.  
...
|[in] *точки*|Объект [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) , указывающий *x* и *y* координаты указателя относительно верхнего левого угла экрана.|  
...
##  <a name="onncmousemove"></a>  CWnd::OnNcMouseMove  
 Эта функция-член вызывается платформой при перемещении курсора в пределах неклиентской области.  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[in] *bIsRendering*|Значение TRUE, если диспетчер окон рабочего стола (DWM) отрисовка разрешена неклиентской области окна. Значение FALSE, если отключена подготовки к просмотру.|  
...
|---------------|-----------------|  
|[in] *nHitTest*|Значение проверки нажатия, возвращенное [CWnd::DefWindowProc](#defwindowproc) функцию в результате обработки [WM_NCHITTEST](http://msdn.microsoft.com/library/windows/desktop/ms645618) сообщения.|  
|[in] *nButton*|Значение XBUTTON1, если первая кнопка мыши X или XBUTTON2 Если второй нажатии кнопку «X».|  
|[in] *точки*|Объект [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) , указывающий *x* и *y* координаты указателя относительно верхнего левого угла экрана.|  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[in] *nKey*|Побитовое сочетание (OR) флагов, которое указывает, при нажатии клавиши-модификаторы. Например флаг MK_CONTROL указывает, что нажата клавиша CTRL.<br /><br /> Список флагов см. в разделе подзаголовка «Параметры сообщения» в [о ввода мыши](http://msdn.microsoft.com/library/windows/desktop/ms645601).|  
 ...
 *pResult*  
 Указатель на переменную LRESULT для хранения код результата, если сообщение обработано.  
...
### <a name="remarks"></a>Примечания  
 ...
 Переопределите в производном классе для обработки сообщения WM_NOTIFY эта функция-член. Переопределение не будет обрабатывать схему сообщений, если базовый класс `OnNotify` вызывается.  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[in] *pWnd*|Указатель на `CWnd` , представляющий окно отправку [WM_NOTIFY](http://msdn.microsoft.com/library/windows/desktop/bb775583) сообщения.<br /><br /> Этот параметр является указатель на элемент управления, если *nCommand* параметр является NF_QUERY или указатель на родительское окно элемента управления, если *nCommand* — NF_REQUERY.|  
|[in] *nCommand*|Значение команды, которое специализирует WM_NOTIFY-сообщение. Допустимые значения:<br /><br /> -NF_QUERY-<br />     Сообщение является запрос, чтобы определить, следует ли использовать в сообщении WM_NOTIFY структуры формата ANSI или Юникод. Это сообщение отправляется из элемента управления своему родительскому окну при создании элемента управления и в ответ на форме NF_REQUERY этого сообщения.<br />-NF_REQUERY-<br />     Сообщение — это запрос для элемента управления для отправки формы NF_QUERY сообщения родительского окна. Этот запрос отправляется из родительского окна с запросом управления повторный запрос родительского о тип структуры, для использования в сообщении WM_NOTIFY. Если *nCommand* NF_REQUERY — параметр, возвращаемое значение является результатом операции повторного запроса.|  
  
### <a name="return-value"></a>Возвращаемое значение  
  
|Возвращаемое значение|Значение|  
|------------------|-------------|  
|NFR_ANSI|ANSI структуры можно использовать в WM_NOTIFY сообщения, отправляемые с помощью элемента управления.|  
|NFR_UNICODE|Следует использовать Юникод структур в WM_NOTIFY сообщений, отправляемых с помощью элемента управления.|  
|0|Произошла ошибка.|  
  
### <a name="remarks"></a>Примечания  
 Этот метод получает [WM_NOTIFYFORMAT](http://msdn.microsoft.com/library/windows/desktop/bb775584) уведомлений, описанный в Windows SDK. WM_NOTIFY-сообщение отправляется из стандартного элемента управления своему родительскому окну, а также от родительского окна стандартного элемента управления.  
...
### <a name="remarks"></a>Примечания  
 ...
 Приложения должны проверять любой необходимости перерисовки внутренней или обновления, просмотрев его внутренние структуры данных для каждого сообщения WM_PAINT, так как сообщения WM_PAINT может быть вызвано недопустимым области и вызов `RedrawWindow`функцию-член с RDW_INTERNALPAINT значение флага.  
  
 Внутреннего сообщения WM_PAINT отправляется только один раз, операционной системой Windows. После внутренних WM_PAINT сообщения, отправляемые в окно путем `UpdateWindow` функция-член, последующие сообщения WM_PAINT не будет отправлено или опубликованы, пока окно становится недействительным, или пока не `RedrawWindow` функция-член вызывается снова с RDW_INTERNALPAINT набор флагов.  
...
##  <a name="onpaintclipboard"></a>  CWnd::OnPaintClipboard  
 Владелец буфера обмена `OnPaintClipboard` функция-член вызывается средством просмотра буфера обмена, когда владелец буфера обмена размещает данные в буфер обмена в формате CF_OWNERDISPLAY и клиентской области просмотра буфера требуется перерисовка.  
...
### <a name="remarks"></a>Примечания  
 Чтобы определить ли требуется перерисовка всю клиентскую область или только с частью, владелец буфер обмена должен сравнить размеры области рисования `rcpaint` членом `PAINTSTRUCT` структуры с размерами, заданными в последней [ OnSizeClipboard](#onsizeclipboard) вызова функции-члена.  
...
### <a name="remarks"></a>Примечания  
 ...
 `OnPaletteChanged` Функция-член вызывается для всех окон верхнего уровня и перекрывающихся, включая измененного системной палитры и вызвало отправку сообщения WM_PALETTECHANGED. Если любого дочернего окна использует цветовую палитру, это сообщение должны быть переданы его.  
...
##  <a name="onpaletteischanging"></a>  CWnd::OnPaletteIsChanging  
 Платформа вызывает эту функцию-член для оповещения приложения, что приложение собирается реализовать свою логическую палитру.  
...
### <a name="parameters"></a>Параметры  
 *message*  
 Указывает событие, для которого был оповещен родительский и идентификатор дочернего окна. Данное событие является слова низкого порядка *сообщение*. Если событие является WM_CREATE или WM_DESTROY старшего слова *сообщение* идентификатор дочернего окна; в противном случае старшее слово не определено. События (младшее слово *сообщение*) может иметь любое из следующих значений:  
  
- Идет создание WM_CREATE дочернего окна.  
  
- WM_DESTROY дочернее окно уничтожается.  
  
- WM_LBUTTONDOWN пользователь имеет навести курсор мыши на дочернем окне и нажатии левой кнопки мыши.  
  
- WM_MBUTTONDOWN пользователь имеет навести курсор мыши на дочернем окне и нажата средняя кнопка мыши.  
  
- WM_RBUTTONDOWN пользователь имеет навести курсор мыши на дочернем окне и нажатии правой кнопки мыши.  
  
 *lParam*  
 Если событие (младшее слово) *сообщение* WM_CREATE или WM_DESTROY, *lParam* указывает дескриптор окна дочернего окна; в противном случае *lParam* содержит x и y координаты курсора. Координата x находится в младшее слово и — координату по оси y в word высокого порядка.  
...
### <a name="remarks"></a>Примечания  
 ...
 Все дочерние окна, кроме тех, которые имеют [WS_EX_NOPARENTNOTIFY](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles) стиль отправить это сообщение их родительского окна. По умолчанию дочерние окна в диалоговом окне со стилем WS_EX_NOPARENTNOTIFY Если дочернее окно, созданный путем вызова метода без этого стиля [CreateEx](#createex) функции-члена.  
  
> [!NOTE]
>  Это функция-член вызывается платформой, чтобы разрешить приложению обработать сообщение Windows. Параметры, передаваемые в функцию, отражают параметры, полученные платформой при поступлении сообщения. При вызове реализации базового класса этой функции эта реализация будет использовать параметры, изначально переданные с сообщением, а не параметры, передаваемые в функции.  
  
##  <a name="onpowerbroadcast"></a>  CWnd::OnPowerBroadcast  
 Эта функция-член вызывается платформой при возникновении события управления питанием.  
 ...
 *NPowerEvent* параметр указывает события, такие как низкого заряда батареи, состояние питания изменилась, разрешение на приостановку операции запрошенного или не, операция автоматически возобновляется после события, система Приостановка операции или операции возобновляет работу после приостановки. *NEventData* параметр обычно не используется. Дополнительные сведения см. в разделе *wParam* и *lParam* параметры [WM_POWERBROADCAST](http://msdn.microsoft.com/library/windows/desktop/aa373247) сообщения.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Двойное значение, содержащее дескриптор курсора или значка в младшее слово. Курсор или значок должен быть совместим с разрешением драйвер дисплея. Если приложение возвращает NULL, система отображает курсор по умолчанию. Возвращаемое значение по умолчанию имеет значение NULL.  
...
### <a name="remarks"></a>Примечания  
 Если любое приложение возвращает 0, не окончания сеанса Windows. Windows перестает вызова `OnQueryEndSession` как только одно приложение возвращает 0, а также отправляет [WM_ENDSESSION](#onendsession) сообщение со значением FALSE параметра для любого приложения, который уже был возвращен ненулевое значение.  
...
- Индикаторы UISF_HIDEFOCUS фокуса скрыты.  
...
- Задать MK_LBUTTON, если оставить кнопку мыши не работает.  
  
- MK_MBUTTON значение, если нажата средняя кнопка мыши.  
  
- MK_RBUTTON значение, если нажата правая кнопка мыши.  
...
##  <a name="onsessionchange"></a>  CWnd::OnSessionChange  
 Платформа вызывает эту функцию-член для оповещения приложения об изменении состояния сеанса.  
...
### <a name="remarks"></a>Примечания  
 Реализация по умолчанию вызывает родительское окно `OnSetCursor` перед обработкой. Если родительское окно возвращает значение TRUE, дальнейшая обработка прерывается. Вызов родительское окно имеет родительского окна контролировать параметры курсора в дочернем окне.  
 ...
 Если *nHitTest* — HTERROR и *сообщение* является сообщением предшествовали мыши `MessageBeep` вызвать функцию-член.  
  
 *Сообщение* имеет значение 0, если `CWnd` переходит в режим меню.  
...
### <a name="parameters"></a>Параметры  
 *pOldWnd*  
 Содержит `CWnd` объект, который теряет фокус (может иметь значение NULL). Указатель может быть временным. Его не требуется сохранять для дальнейшего использования.  
...
-   Используйте ON_WININICHANGE при запуске Windows NT 3.51 или более ранней версии. Это сообщение больше не используется.  
 ...
 Требуется только один из этих макросов в схему сообщений. Чтобы написать программу, которая работает под Windows 95/98 и Windows NT 4.0, необходимо напишите обработчик для ON_WM_SETTINGCHANGE. В группе Windows NT 3.51 обработчиком будут вызваны `OnSettingChange` и *uFlags* и всегда будет равно нулю.  
...
### <a name="parameters"></a>Параметры  
 *bShow*  
 Указывает отображается ли окно. Он имеет значение TRUE, если отображается окно; он имеет значение FALSE, если окно скрывается.  
...
- Закрывает SW_PARENTCLOSING родительскому окну (вносятся преобразованного в значок) или всплывающее окно скрывается.  
  
- Открывается SW_PARENTOPENING родительскому окну (отображается) или отображается всплывающее окно.  
...
##  <a name="onsize"></a>  CWnd::OnSize  
 Эта функция-член вызывается платформой, после изменения размера окна.  
...
- Свернутый SIZE_MINIMIZED окна.  
  
- Окно SIZE_RESTORED был изменен, но SIZE_MINIMIZED ни SIZE_MAXIMIZED применяется.  
  
- SIZE_MAXHIDE сообщение отправляется для всех всплывающих окон, если некоторые другие окно развернуто.  
  
- SIZE_MAXSHOW сообщение отправляется для всех всплывающих окон, при других окон был восстановлен до первоначального размера.  
...
- GWL_STYLE nonextended стилей окон изменились.  
...
##  <a name="onsyscolorchange"></a>  CWnd::OnSysColorChange  
 Эта функция-член вызывается платформой для всех окон верхнего уровня при внесении изменений в параметра системного цвета.  
...
- Активировать SC_HOTKEY `CWnd` объекта, связанного с сочетания клавиш, определяемый приложением. Слова низкого порядка *lParam* идентифицирует HWND окна для активации.  
  
- SC_HSCROLL прокручиваться по горизонтали.  
  
- SC_KEYMENU получить меню до нажатия клавиши.  
  
- SC_MAXIMIZE (или SC_ZOOM) развернуть `CWnd` объекта.  
  
- SC_MINIMIZE (или SC_ICON) свернуть `CWnd` объекта.  
  
- SC_MOUSEMENU получить щелкните меню с помощью мыши.  
...
- Восстановление SC_RESTORE окно для обычного положения и размера.  
  
- Выполняет SC_SCREENSAVE экрана заставки приложения указан в разделе [boot] системы. INI-файл.  
  
- Размер SC_SIZE `CWnd` объекта.  
  
- Выполнение SC_TASKLIST или активировать приложение диспетчера задач Windows.  
...
- Активировать SC_HOTKEY окно, связанное с сочетания клавиш, определяемый приложением. Слова низкого порядка *lParam* определяет окно для активации.  
  
- Выполнить SC_SCREENSAVE сохранения экрана приложение, указанное в разделе рабочего стола или панели управления.  
...
### <a name="remarks"></a>Примечания  
 ...
 В сообщениях WM_SYSCOMMAND, четыре младших бита *nID* параметр используются операционной системой Windows. Когда приложение проверяет значение *nID*, его необходимо объединить значение 0xFFF0 с *nID* значение с помощью битовой операции- и оператор, чтобы получить правильный результат.  
  
 Пункты меню в меню управления могут быть изменены с `GetSystemMenu`, `AppendMenu`, `InsertMenu`, и `ModifyMenu` функции-члены. Приложения, изменяющие меню «Управление» необходимо обработать WM_SYSCOMMAND сообщения и сообщения WM_SYSCOMMAND, не обработанных приложением должны быть переданы на `OnSysCommand`. Все команды значения, добавленные в приложении должны обрабатываться приложением и не могут быть переданы в `OnSysCommand`.  
  
 Приложения можно выполнять любые команды системы в любое время путем передачи сообщения WM_SYSCOMMAND `OnSysCommand`.  
...
- IDABORT пользователь щелчок на настраиваемую кнопку Abort.  
  
- IDCANCEL пользователь щелчок на настраиваемую кнопку "Отмена".  
  
- IDCLOSE пользователя закрыт карта обучения.  
  
- IDHELP пользователь щелчок на настраиваемую кнопку справки Windows.  
  
- IDIGNORE пользователь щелчок на настраиваемую кнопку пропустить.  
  
- Пользователь IDOK щелчок настраиваемую кнопку "ОК".  
  
- IDNO пользователь нажал настраиваемую кнопку Нет.  
  
- IDRETRY пользователь щелчок на настраиваемую кнопку "Повторить".  
  
- HELP_TCARD_DATA пользователь щелчок настраиваемую кнопку. *DwActionData* параметр содержит целое число типа long, указанным автором справки.  
  
- HELP_TCARD_NEXT пользователь щелчок настраиваемую кнопку Далее.  
  
- Другое приложение HELP_TCARD_OTHER_CALLER запросил карты обучения.  
  
- IDYES пользователь щелчок настраиваемую кнопку "Да".  
...
### <a name="remarks"></a>Примечания  
 Эта функция вызывается только в том случае, если приложение было запущено карту обучения справки Windows. Приложение инициирует карта обучения, указав при вызове команды HELP_TCARD [WinHelp](../../mfc/reference/cwinapp-class.md#winhelp) функции.  
...
### <a name="remarks"></a>Примечания  
 У любого приложения, что изменения системного времени это сообщение отправляется всем окнам верхнего уровня. WM_TIMECHANGE сообщение отправляется всем окнам верхнего уровня, приложение может использовать [SendMessage](http://msdn.microsoft.com/library/windows/desktop/ms644950) Windows работать с его *hwnd* равным HWND_BROADCAST.  
...
- *lpszText* = LPSTR_TEXTCALLBACK указатель на строку, будет отображаться в указанном окне  
...
### <a name="return-value"></a>Возвращаемое значение  
 Значение TRUE, если приложение обрабатывает Windows сенсорный ввод; в противном случае — значение FALSE.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Значение TRUE, если приложение процессы Windows touch входных данных; в противном случае — значение FALSE.  
...
|[in] *nFlags*|Меню, который был удален. В настоящее время может быть только меню Окно MF_SYSMENU.|  
...
- Элемент пользовательского интерфейса UIS_INITIALIZE state (заданные *nUIElement*) должны изменяться в зависимости от последнего входного события. Дополнительные сведения см. в разделе **примечания** раздел [WM_UPDATEISTATE](http://msdn.microsoft.com/library/windows/desktop/ms646361).  
...
##  <a name="onvkeytoitem"></a>  CWnd::OnVKeyToItem  
 Если `CWnd` объекта, которому принадлежит поле со списком с [LBS_WANTKEYBOARDINPUT](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) стиль списка будет отправлять сообщения WM_VKEYTOITEM в ответ на сообщение WM_KEYDOWN.  
...
##  <a name="onvscrollclipboard"></a>  CWnd::OnVScrollClipboard  
 Владелец буфера обмена `OnVScrollClipboard` функция-член вызывается буфер обмена, когда данные из буфера обмена имеет формат CF_OWNERDISPLAY и события в буфер обмена вертикальной полосы прокрутки.  
...
### <a name="remarks"></a>Примечания  
 ...
 Для окна с [WS_OVERLAPPED](styles-used-by-mfc.md#window-styles) или [WS_THICKFRAME](styles-used-by-mfc.md#window-styles) стиля, реализация по умолчанию отправляет [WM_GETMINMAXINFO](http://msdn.microsoft.com/library/windows/desktop/ms632626) сообщение в окно. Это делается для проверки новый размер и положение окна и применять стили CS_BYTEALIGNCLIENT и CS_BYTEALIGN клиента. Приложение может переопределить эту функцию, не вызвав его базовый класс.  
...
### <a name="remarks"></a>Примечания  
 ...
 WM_WININICHANGE сообщение отправляется всем окнам верхнего уровня, приложение может использовать [SendMessage](http://msdn.microsoft.com/library/windows/desktop/ms644950) Windows работать с его *hwnd* равным HWND_BROADCAST.  
  
 Если приложение изменяет множество различных разделов в WIN. INI-ФАЙЛЕ, в то же время, приложение должно отправлять одно сообщение WM_WININICHANGE с *lpszSection* присваивается значение NULL. В противном случае приложение должно отправлять WM_WININICHANGE каждый раз упрощает изменение WIN. INI-ФАЙЛЕ.  
...
|---------------|-----------------|  
|[in] *nFlags*|Побитовое сочетание (OR) флагов, которое указывает, при нажатии клавиши-модификаторы. Например флаг MK_CONTROL указывает, что нажата клавиша CTRL.|  
|[in] *nButton*|Значение XBUTTON1, если была нажата кнопка первого Microsoft Поворачивайте X или XBUTTON2 Если второй X кнопка была нажата.|  
|[in] *точки*|Объект [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) , указывающий *x* и *y* координаты указателя относительно левого верхнего угла клиентской области.|  
...
|---------------|-----------------|  
|[in] *nFlags*|Побитовое сочетание (OR) флагов, которое указывает, при нажатии клавиши-модификаторы. Например флаг MK_CONTROL указывает, что нажата клавиша CTRL.|  
|[in] *nButton*|Значение XBUTTON1 Если и первая кнопка Microsoft Поворачивайте X был дважды щелкнут или XBUTTON2 Если второй кнопку «X» был дважды щелкнут.|  
|[in] *точки*|Объект [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) , указывающий *x* и *y* координаты указателя относительно левого верхнего угла клиентской области.|  
...
##  <a name="operator_hwnd"></a>  CWnd::operator HWND  
 Этот оператор используется для получения дескриптора `CWnd` объекта.  
...
> [!WARNING]
> `CWnd::PreCreateWindow` Теперь назначает hMenu членом *cs* для **это** указателя, если меню имеет значение NULL, и стиль содержит WS_CHILD. На правильную работу убедитесь, что элемент управления диалогового окна имеет идентификатор, который не имеет значение NULL.  
>   
>  Это изменение устраняет потенциальный сбой в сценариях взаимодействия с управляемым и машинным. Оператора ТРАССИРОВКИ в `CWnd::Create` предупреждения разработчиков о проблеме.  
  
 Невозможно напрямую вызвать эту функцию.  
  
 Реализация по умолчанию эта функция проверяет имя класса окна NULL и замещает соответствующее значение по умолчанию. Переопределить эту функцию-член для изменения `CREATESTRUCT` структуру перед созданием окна.  
...
- PRF_CLIENT прорисовки клиентской области окна.  
  
- PRF_ERASEBKGND стереть фон перед рисованием окна.  
  
- PRF_NONCLIENT отрисовки неклиентской области окна.  
...
-   Если указан PRF_CHECKVISIBLE и окно не отображается, не требуется.  
  
-   Если указано PRF_NONCLIENT отрисовки неклиентской области в контексте данного устройства.  
  
-   Если указано PRF_ERASEBKGND отправки окна [WM_ERASEBKGND](http://msdn.microsoft.com/library/windows/desktop/ms648055) сообщения.  
  
-   Если указано PRF_CLIENT отправки окна [WM_PRINTCLIENT](http://msdn.microsoft.com/library/windows/desktop/dd145217) сообщения.  
  
-   Если PRF_CHILDREN отправки каждого видимые дочерние окна [WM_PRINT](http://msdn.microsoft.com/library/windows/desktop/dd145216) сообщения.  
  
-   Если PRF_OWNED, отправьте сообщение WM_PRINT каждого видимым собственное окно.  
 ...
 *prgnUpdate*  
 Определяет область обновления. Если оба *prgnUpdate* и *lpRectUpdate* имеют значение NULL, всю клиентскую область добавляется в область обновления.  
  
 *flags*  
 Следующие флаги, используемые для недействительным окно.  
  
- RDW_ERASE вызывает окно, чтобы получить [WM_ERASEBKGND](http://msdn.microsoft.com/library/windows/desktop/ms648055) сообщения при окрашивание окна. Также необходимо указать флаг RDW_INVALIDATE; в противном случае RDW_ERASE не оказывает влияния.  
  
- RDW_FRAME вызывает любой части неклиентской области окна, которое пересекается с областью обновления для получения [WM_NCPAINT](http://msdn.microsoft.com/library/windows/desktop/dd145212) сообщения. Также необходимо указать флаг RDW_INVALIDATE; в противном случае RDW_FRAME не оказывает влияния.  
  
- Вызывает RDW_INTERNALPAINT [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) сообщения должны быть учтены окна независимо от ли окно содержит недопустимый регион.  
  
- Сделать недействительным RDW_INVALIDATE *lpRectUpdate* или *prgnUpdate* (только один может не иметь значение NULL). Если оба имеют значение NULL, все окно становится недействительным.  
...
- RDW_NOINTERNALPAINT подавляет все ожидающие внутренние сообщения WM_PAINT. Этот флаг не влияет на сообщения WM_PAINT, возникающие в результате недопустимый областей.  
  
- Проверяет RDW_VALIDATE *lpRectUpdate* или *prgnUpdate* (только один может не иметь значение NULL). Если оба имеют значение NULL, будет проверен всего содержимого окна. Этот флаг не влияет на внутренние сообщения WM_PAINT.  
...
- RDW_ERASENOW вызывает затронутых windows (в соответствии с флагами RDW_ALLCHILDREN и RDW_NOCHILDREN) для получения сообщений WM_NCPAINT и WM_ERASEBKGND, при необходимости, до выполнения возврата функцией. Сообщения WM_PAINT откладываются.  
  
- RDW_UPDATENOW вызывает затронутых windows (в соответствии с флагами RDW_ALLCHILDREN и RDW_NOCHILDREN) для получения сообщений WM_NCPAINT, WM_ERASEBKGND и WM_PAINT, при необходимости, до выполнения возврата функцией.  
  
 По умолчанию windows затронутых `RedrawWindow` функции зависят от того, имеет ли заданное окно WS_CLIPCHILDREN стиль. Дочерние окна WS_CLIPCHILDREN windows не затрагиваются. Однако этих окон, которые не являются WS_CLIPCHILDREN windows, рекурсивно проверить или недействительным, пока не будет обнаружен WS_CLIPCHILDREN окна. Следующие флаги управления windows, подвержены `RedrawWindow` функции:  
  
- Включает в себя RDW_ALLCHILDREN дочерних окон, если таковая имеется, в операции следует.  
  
- Исключает RDW_NOCHILDREN дочерних окон, если таковые имеются, следует операции.  
 ...
 *pResult*  
 Результат, созданный дочернее окно, возвращенных родительского окна. Может иметь значение NULL.  
  
### <a name="return-value"></a>Возвращаемое значение  
 Значение TRUE, если сообщение было переслано; в противном случае — значение FALSE.  
  
### <a name="remarks"></a>Примечания  
 Это вспомогательная функция, которая отражает *сообщения* с источником.  
...
##  <a name="reflectlastmsg"></a>  CWnd::ReflectLastMsg  
 Эта функция-член вызывается платформой, чтобы отразить последнее сообщение дочернему окну.  
...
- `CWnd::reposDefault` Выполняет компоновку панелей элементов управления. *lpRectParam* не используется и может иметь значение NULL.  
  
- `CWnd::reposQuery` Макет панели элементов управления не выполняется; Вместо этого *lpRectParam* инициализируется размер клиентской области, как если бы макета фактически выполнена.  
 ...
 *lpRectClient*  
 Указывает на [структура RECT](../../mfc/reference/rect-structure1.md) содержащий доступные клиентской области. Если значение равно NULL, будет использоваться клиентской области окна.  
...
##  <a name="runmodalloop"></a>  CWnd::RunModalLoop  
 Вызовите эту функцию-член для извлечения, преобразования или отправлять сообщения, пока не [ContinueModal](#continuemodal) возвращает значение FALSE.  
...
- Send не MLF_NOIDLEMSG [WM_ENTERIDLE](http://msdn.microsoft.com/library/windows/desktop/ms645422) сообщений в родительский объект.  
  
- MLF_NOKICKIDLE не отправлять сообщения WM_KICKIDLE окна.  
...
### <a name="remarks"></a>Примечания  
 ...
 Если *lpRect* имеет значение NULL, положения всех дочерних окон в окне смещаются на величину, указанную в *xAmount* и *yAmount*и любой недопустимый (неокрашенные) областей `CWnd` также смещения. `ScrollWindow` Когда выполняется быстрее *lpRect* имеет значение NULL.  
  
 Если *lpRect* не равно NULL, позиции дочерних окон — это не измененные с недопустимым области `CWnd` не смещения. Во избежание проблем обновления при *lpRect* является not NULL, вызовите `UpdateWindow` функции-члена для отрисовки `CWnd` перед вызовом `ScrollWindow`.  
 ...
 *lpRectScroll*  
 Указывает на [структура RECT](../../mfc/reference/rect-structure1.md) , задающий части клиентской области, чтобы выполнить прокрутку. Если этот параметр имеет значение NULL, всю клиентскую область прокрутки.  
 ...
 *prgnUpdate*  
 Определяет область, в который изменен для хранения области недопустимой в результате выполнения прокрутки. Этот параметр может иметь значение NULL.  
  
 *lpRectUpdate*  
 Указывает `RECT` структуры, который получит границы прямоугольника, недопустимой в результате выполнения прокрутки. Этот параметр может иметь значение NULL.  
...
- Делает недействительным SW_INVALIDATE области определяется *prgnUpdate* после прокрутки.  
  
- Выполняет прокрутку SW_SCROLLCHILDREN всех дочерних окон, которые пересекают прямоугольника, на который указывает *lpRectScroll* на количество пикселей, заданное в *dx* и *dy*. Windows отправляет [WM_MOVE](http://msdn.microsoft.com/library/windows/desktop/ms632631) сообщений для всех дочерних окон, которые пересекают *lpRectScroll*, даже если они не перемещаются. При прокрутке дочернее окно и прямоугольника курсор пересекает прямоугольник прокрутки положения курсора.  
  
### <a name="return-value"></a>Возвращаемое значение  
 Возвращает значение SIMPLEREGION (прямоугольной области недействительным), COMPLEXREGION (непрямоугольного Недопустимая область; перекрывающиеся прямоугольники) или NULLREGION (Недопустимая область), если функция выполнена успешно; в противном случае возвращается ошибка.  
...
### <a name="remarks"></a>Примечания  
 ...
 Если окно [WS_CLIPCHILDREN](http://msdn.microsoft.com/library/windows/desktop/ms632679) стиль, возвращенный областях, указанных в *prgnUpdate* и *lpRectUpdate* представляют общей области прокручиваемого окна, которое необходимо обновить, включая все области в дочерних окон, которые необходимо обновить.  
  
 Если [SW_SCROLLCHILDREN](http://msdn.microsoft.com/library/windows/desktop/bb787593) установлен флаг, Windows не будут обновлены надлежащим образом экрана Если прокрутить частью дочернего окна. Часть прокручиваемого дочернее окно, которое находится за пределами исходного прямоугольника, не удаляются и не будет правильно перерисовываться в его новое место назначения. Используйте [DeferWindowPos](http://msdn.microsoft.com/library/windows/desktop/ms632681) функции Windows для перемещения дочерних окон, которые не находятся в полностью *lpRectScroll* прямоугольник. Если установлен флаг SW_SCROLLCHILDREN и прямоугольника курсор пересекает прямоугольник прокрутки положения курсора.  
  
 Все координаты ввода-вывода (для *lpRectScroll*, *lpRectClip*, *lpRectUpdate*, и *prgnUpdate*) считаются в координаты клиента, независимо от того, имеет ли окно CS_OWNDC или CS_CLASSDC стиль класса. Используйте [LPtoDP](http://msdn.microsoft.com/library/windows/desktop/dd145042) и [DPtoLP](http://msdn.microsoft.com/library/windows/desktop/dd162474) функций Windows для преобразования в логических координатах при необходимости.  
 ...
 *bDeep*  
 Задает уровень, на котором выполняется поиск. Если значение равно TRUE, выполнять рекурсивный поиск всех дочерних объектов; Если значение равно FALSE, поиск только непосредственные дочерние элементы.  
  
 *bOnlyPerm*  
 Указывает ли временные windows получит сообщение. Значение TRUE, если временные windows может получать сообщения. При значении FALSE только постоянные windows сообщение. Дополнительные сведения о временных windows в разделе [технические Примечание 3](../../mfc/tn003-mapping-of-windows-handles-to-objects.md).  
  
### <a name="remarks"></a>Примечания  
 Если *bDeep* имеет значение FALSE, сообщение отправляется только непосредственные дочерние элементы окна; в противном случае сообщение отправляется для всех окон-потомков.  
  
 Если *bDeep* и *bOnlyPerm* имеют значение TRUE, поиск продолжается ниже временные windows. В этом случае только постоянные windows во время поиска в сообщении. Если *bDeep* имеет значение FALSE, сообщение отправляется только непосредственные дочерние элементы окна.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Указатель на объект window, появлялось всем вводом мыши. Если окно не возвращается значение NULL. Возвращенный указатель может быть временным и не требуется сохранять для дальнейшего использования.  
...
### <a name="remarks"></a>Примечания  
 ...
 При захвате мыши не WM_NCHITTEST или WM_SETCURSOR сообщения отправляются на активное окно.  
...
### <a name="remarks"></a>Примечания  
 ...
 Эта функция-член отправляет сообщение WM_DRAWCLIPBOARD окно. Так как дескриптор следующее окно в цепочке просмотра буфера еще не был возвращен, приложение не следует передавать в сообщении WM_DRAWCLIPBOARD, получаемые во время вызова `SetClipboardViewer`.  
 ...
 *bSigned*  
 Определяет, подписанные или неподписанные целочисленное значение. Если этот параметр имеет значение TRUE, *nValue* подписан. Если этот параметр имеет значение TRUE и *nValue* меньше 0, знак "минус" входа размещается перед первой цифрой в строке. Если этот параметр имеет значение FALSE, *nValue* не подписан.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Указатель на объект window, ранее имевший фокус ввода. Если окно не возвращается значение NULL. Возвращенный указатель может быть временным и не должны храниться.  
 ...
 *bRedraw*  
 Значение TRUE, если для окна для перерисовки сразу после его обрабатывает сообщение WM_SETFONT; в противном случае — значение FALSE.  
  
### <a name="remarks"></a>Примечания  
 Этот метод действует только окна обрабатывает сообщение WM_SETFONT. Многие классы MFC, которые являются производными от `CWnd` обрабатывают такие сообщения, поскольку они привязаны к классу стандартных окон, включающий обработчик сообщений для сообщения WM_SETFONT. С помощью этого метода, классы, производные от `CWnd` необходимо определить метод обработчика для сообщений WM_SETFONT.  
...
### <a name="parameters"></a>Параметры  
 *crKey*  
 Указатель на значение COLORREF, задает ключ цвета прозрачности для использования при составлении многослойного окна. Все пиксели, созданный при помощи окна в этот цвет будет прозрачной. Чтобы создать COLORREF, используйте макрос RGB.  
...
### <a name="parameters"></a>Параметры  
 *pMenu*  
 Определяет новое меню. Если этот параметр имеет значение NULL, меню будет удален.  
...
### <a name="parameters"></a>Параметры  
 *pOwnerWnd*  
 Определяет новый владелец объекта window. Если этот параметр имеет значение NULL, владелец не имеет объект окна.  
...
### <a name="parameters"></a>Параметры  
 *bRedraw*  
 Указывает состояние флага перерисовку. Если этот параметр имеет значение TRUE, установлен флаг перерисовку; Если значение равно FALSE, снят флаг.  
 ...
 *bRedraw*  
 Указывает, следует ли заново полосу прокрутки, с учетом новой позиции. Если *bRedraw* имеет значение TRUE, перерисовке полосы прокрутки. Если он имеет значение FALSE, не перерисовке. По умолчанию перерисовке полосы прокрутки.  
 ...
 *bRedraw*  
 Указывает, следует ли повторное окрашивание полосы прокрутки и отобразит новое положение полосы прокрутки. Если этот параметр имеет значение TRUE, перерисовки полосы прокрутки; Если значение равно FALSE, полосы прокрутки не разрешено.  
 ...
 *bRedraw*  
 Указывает, является ли полоса прокрутки необходимости перерисовки в соответствии с изменениями. Если *bRedraw* имеет значение TRUE, перерисовке полосы прокрутки; Если значение равно FALSE, не перерисовывается полосы прокрутки.  
...
### <a name="remarks"></a>Примечания  
 ...
 Если вызов `SetScrollRange` сразу за вызов [SetScrollPos](#setscrollpos) функции-члена *bRedraw* параметр в `SetScrollPos` функция-член должен иметь значение 0 для предотвращения полосы прокрутки из отрисовывается дважды.  
...
### <a name="remarks"></a>Примечания  
 Указанное значение интервала, и каждый раз при истечении интервала времени, система отправляет сообщение WM_TIMER Установка очереди сообщений установки приложения или передает сообщения определяется приложением `TimerProc` функции обратного вызова.  
...
### <a name="example"></a>Пример  
 В этом примере используется `CWnd::SetTimer`, `CWnd::OnTimer`, и `CWnd::KillTimer` для обработки сообщения WM_TIMER. Первый таймер настроен для отправки сообщения WM_TIMER каждые 2 секунды в основную область окна `OnStartTimer`. `OnTimer` Обработчик событий обрабатывает сообщения WM_TIMER для фрейма главного окна. Этот метод вызывает динамика звуковой сигнал каждые 2 секунды. Второй таймер сообщение передается функции обратного вызова каждые 3,75 секунд. `OnStopTimer` Останавливает обоих таймеров, вызвав `CWnd::KillTimer` для каждого идентификатора таймера.  
...
### <a name="parameters"></a>Параметры  
 *pWndInsertAfter*  
 Идентифицирует `CWnd` объекта, который предшествует (быть выше, чем) это `CWnd` объекта в Z-порядке. Этот параметр может иметь указатель `CWnd` или указатель на один из следующих значений:  
  
- `wndBottom` Перевод окна в нижней части Z-порядка. Если этот `CWnd` является поверх всех окон, теряет состояние переднего плана; система переведет это окно в нижней части других окон.  
  
- `wndTop` Перевод окна в верхней части Z-порядка.  
  
- `wndTopMost` Перевод окна над всеми отношению к другим окнам. Окно поддерживает его перемещенное даже в том случае, если он отключен.  
...
- Отправляет SWP_FRAMECHANGED WM_NCCALCSIZE сообщения в окно, даже если не изменяется размер окна. Если этот флаг не указан, WM_NCCALCSIZE отправляется только при изменении размера окна.  
...
- SWP_NOREDRAW не будут перерисовываться изменения. Если этот флаг установлен, перерисовка любого типа, не происходит. Это относится к клиентской области заголовка неклиентской области (включая заголовок и полосами прокрутки) и любой части родительского окна, обнаруженных в результате перемещать окно. Если этот флаг установлен, приложение необходимо явно сделать недействительными или перерисовки части окна и родительское окно, которое требуется перерисовка.  
  
- SWP_NOREPOSITION то же, что SWP_NOOWNERZORDER.  
  
- SWP_NOSENDCHANGING запрещает прием сообщения WM_WINDOWPOSCHANGING окна.  
  
- Сохраняет SWP_NOSIZE текущий размер (не учитывает *cx* и *cy* параметров).  
  
- Сохраняет SWP_NOZORDER текущий порядок (не учитывает *pWndInsertAfter*).  
...
### <a name="remarks"></a>Примечания  
 ...
 Если указано SWP_NOACTIVATE ни SWP_NOZORDER (то есть, когда приложение запрашивает, окно одновременно активации и помещен в указанном порядке Z), значение, заданное в *pWndInsertAfter* используется только в следующих обстоятельствах:  
...
-   Это окно не активного окна.  
 ...
 В версиях Windows 3.1 и более поздних версий windows можно перемещена в верхнюю часть Z-порядка и блокировки, задав свои стили WS_EX_TOPMOST. Самое верхнее окно поддерживает его перемещенное даже при отключении. Например при выборе команды WinHelp поверх делает верхнее окно справки, который затем остается видимым после возврата в приложение.  
  
 Чтобы создать поверх всех окон, вызовите `SetWindowPos` с *pWndInsertAfter* параметра равно **& wndTopMost**, или задать стиль WS_EX_TOPMOST при создании окна.  
  
 Если Z-порядка содержит все окна со стилем WS_EX_TOPMOST, перемещать окно, на которых **& wndTopMost** значение помещается в верхней части всех окон, отличных от переднего плана, но находится ниже любого окна переднего плана. Когда приложение активирует неактивного окна без WS_EX_TOPMOST бит, окна перемещается выше все окна не переднего плана, но ниже любого окна переднего плана.  
 ...
 *bRedraw*  
 Значение TRUE, если операционная система перерисовывает окна после настройки области; в противном случае не делает. Как правило, *bRedraw* значение TRUE, если окно является видимым. Если задано значение TRUE, система отправляет сообщения WM_WINDOWPOSCHANGING и WM_WINDOWPOSCHANGED окно.  
...
### <a name="parameters"></a>Параметры  
 *bShow*  
 Указывает, будут ли всплывающие окна отображаться или скрываться. Если этот параметр имеет значение TRUE, отображаются все скрытые всплывающие окна. Если этот параметр имеет значение FALSE, будут скрыты все видимые всплывающего окна.  
 ...
 *bShow*  
 Указывает, будет ли Windows показывает или скрывает полосу прокрутки. Если этот параметр имеет значение TRUE, отображается полоса прокрутки; в противном случае полоса прокрутки будет скрыта.  
...
- Активирует SW_RESTORE и отображение окна. Если окно сворачивать или разворачивать, Windows восстанавливает его исходный размер и положение.  
...
- SW_SHOWMINNOACTIVE Отображение окна в виде значка. Окно, которое в данный момент активна остается активным.  
  
- SW_SHOWNA Отображение окна в ее текущем состоянии. Окно, которое в данный момент активна остается активным.  
  
- SW_SHOWNOACTIVATE Отображение окна в его последней размеров и положения. Окно, которое в данный момент активна остается активным.  
  
- Активирует SW_SHOWNORMAL и отображение окна. Если окно сворачивать или разворачивать, Windows восстанавливает его исходный размер и положение.  
...
##  <a name="unlockwindowupdate"></a>  CWnd::UnlockWindowUpdate  
 Вызовите эту функцию-член для разблокировки окна, заблокированного с помощью `CWnd::LockWindowUpdate`.  
...
### <a name="parameters"></a>Параметры  
 *bSaveAndValidate*  
 Флаг, указывающий, является ли диалоговое окно, которое инициализируется (FALSE) или данных извлекается (TRUE).  
...
### <a name="remarks"></a>Примечания  
 Если дочерний элемент управления не имеет обработчика и *bDisableIfNoHndler* имеет значение TRUE, дочерний элемент управления будет недоступен.  
...
### <a name="parameters"></a>Параметры  
 *pDCDst*  
 Указатель на контекст устройства для экрана. Он используется для сопоставления цветов палитры, при обновлении содержимого окна. Если *pDCDst* имеет значение NULL, палитра по умолчанию будет использоваться.  
  
 Если *pDCSrc* имеет значение NULL, *pDCDst* должен иметь значение NULL.  
  
 *pptDst*  
 Указатель на `POINT` структуры, указав новое положение экрана многослойного окна. Если текущая позиция не меняется, *pptDst* может иметь значение NULL.  
  
 *psize*  
 Указатель на `SIZE` структуру, которая указывает новый размер многослойного окна. Если размер окна не меняется, *psize* может иметь значение NULL.  
  
 Если *pDCSrc* имеет значение NULL, *psize* должен иметь значение NULL.  
  
 *pDCSrc*  
 Указатель на Контроллере домена для рабочей области, определяющий многослойного окна. Если не изменить форму и визуального контекста окна, *pDCSrc* может иметь значение NULL.  
  
 *pptSrc*  
 Указатель на `POINT` структура, которая задает расположение слоя в контексте устройства.  
...
### <a name="remarks"></a>Примечания  
 `UpdateWindow` Функции-члена отправляет сообщения WM_PAINT напрямую, минуя очереди приложения. Если область обновления пуст, WM_PAINT не отправляется.  
...
### <a name="parameters"></a>Параметры  
 *lpRect*  
 Указывает на [CRect](../../atl-mfc-shared/reference/crect-class.md) объекта или [структура RECT](../../mfc/reference/rect-structure1.md) , содержащий клиентских координат прямоугольника, удаляемый из области обновления. Если *lpRect* имеет значение NULL, выполняется проверка всего содержимого окна.  
...
### <a name="remarks"></a>Примечания  
 ...
 Windows по-прежнему создавать сообщения WM_PAINT, пока не проверяется в текущую область обновления.  
...
### <a name="parameters"></a>Параметры  
 *pRgn*  
 Указатель на [CRgn](../../mfc/reference/crgn-class.md) объект, который определяет область, которая определяет область удаляется из области обновления. Если этот параметр имеет значение NULL, всю клиентскую область удаляется.  
...
### <a name="return-value"></a>Возвращаемое значение  
 Указатель на объект window, в которой находится точка. Он имеет значение NULL, если ни одно окно уже существует в заданной точке. Возвращенный указатель может быть временным и не требуется сохранять для дальнейшего использования.  
 ...
 *bЗарегистрируйтесь участия*  
 Значение TRUE указывает регистрации поддержки технологии касания Windows Значение FALSE в противном случае.  
