...
|[CWnd::OpenClipboard](#openclipboard)|Открывает буфер обмена. Другие приложения не смогут изменять буфер обмена, пока Windows [CloseClipboard](/windows/desktop/api/winuser/nf-winuser-closeclipboard) функция вызывается.|
...
|[CWnd::OnDestroyClipboard](#ondestroyclipboard)|Вызывается, когда буфер обмена очищается путем вызова Windows [EmptyClipboard](/windows/desktop/api/winuser/nf-winuser-emptyclipboard) функции.|
...
`CWnd` Класс и механизм схемы сообщений скрыть `WndProc` функции. Входящие сообщения уведомления Windows автоматически перенаправляются через схему сообщений соответствующим **на**<em>сообщение</em> `CWnd` функций-членов. Необходимо переопределить **на**<em>сообщение</em> функции-члена для обработки конкретного сообщения члена в ваших производных классах.
...
Возвращает значение S_OK в случае успешного выполнения, код ошибки COM. в случае сбоя. См. в разделе **возвращаемые значения** в [IAccessible::accDoDefaultAction](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-accdodefaultaction) в Windows SDK.

### <a name="remarks"></a>Примечания

Эта функция является частью MFC [Active Accessibility](/windows/desktop/WinAuto/microsoft-active-accessibility) поддержки.

Переопределите эту функцию в вашей `CWnd`-производного класса для выполнения действие объекта по умолчанию. Дополнительные сведения см. в разделе [IAccessible::accDoDefaultAction](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-accdodefaultaction) в пакете Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::accHitTest](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-acchittest) в пакете Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::accNavigate](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-accnavigate) в пакете Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::accSelect](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-accselect) в пакете Windows SDK.
...
*lpPaint*<br/>
Указывает на [PAINTSTRUCT](/windows/desktop/api/winuser/ns-winuser-tagpaintstruct) структуры, для получения сведений о рисования.
...
Эта функция просто вызывает Win32 [BringWindowToTop](/windows/desktop/api/winuser/nf-winuser-bringwindowtotop) функции. Вызовите [SetWindowPos](#setwindowpos) функции, чтобы изменить положение окна в Z-порядке. Функция `BringWindowToTop` не изменяет стиль окна, чтобы сделать его окном верхнего уровня. Дополнительные сведения см. в разделе [Какова разница между различаются HWND_TOP и HWND_TOPMOST](https://devblogs.microsoft.com/oldnewthing/?p=33263)
...
*lpClientRect*<br/>
[in, out] Указатель на структуру прямоугольника. Во входных данных эта структура содержит клиентской прямоугольной области. После завершения метода эта структура содержит прямоугольной области окна, который может содержать указанной клиентской области.

*nAdjustType*<br/>
[in] Используйте `CWnd::adjustBorder` для которого требуется вычислить координаты окна без стиля WS_EX_CLIENTEDGE; в противном случае используйте `CWnd::adjustOutside`.
...
`CheckDlgButton` Функция отправляет [BM_SETCHECK](/windows/desktop/Controls/bm-setcheck) сообщение для указанной кнопки.
...
`CheckRadioButton` Функция отправляет [BM_SETCHECK](/windows/desktop/Controls/bm-setcheck) сообщение на указанный переключатель.
...
*lpPoint*<br/>
Указывает на [структура POINT](/windows/desktop/api/windef/ns-windef-tagpoint) или `CPoint` , содержащей клиентские координаты для преобразования.

*lpRect*<br/>
Указывает на [структура RECT](/windows/desktop/api/windef/ns-windef-tagrect) или `CRect` , содержащей клиентские координаты для преобразования.
...
*lpszClassName*<br/>
[in] Указатель на заканчивающуюся нулем строку, содержащую имя класса окна зарегистрированных системы; или имя класса окна стандартные системные.

*lpszWindowName*<br/>
[in] Указатель на заканчивающуюся нулем строку, содержащего окно отображаемое имя; в противном случае имеет значение NULL для окна не отображаемое имя.
...
*pContext*<br/>
[in] Указатель на [CCreateContext](../../mfc/reference/ccreatecontext-structure.md) структуру, которая используется для настройки архитектуру представления документов для приложения.
...
Точечный рисунок должен ранее созданные [CBitmap::CreateBitmap](../../mfc/reference/cbitmap-class.md#createbitmap) функции-члена [CreateDIBitmap](/windows/desktop/api/wingdi/nf-wingdi-createdibitmap) функции Windows, или [CBitmap::LoadBitmap](../../mfc/reference/cbitmap-class.md#loadbitmap) функция-член.
...
*rect*<br/>
Задает размер и положение элемента управления. Может быть либо [CRect](../../atl-mfc-shared/reference/crect-class.md) объекта или [структура RECT](/windows/desktop/api/windef/ns-windef-tagrect).

*PPT*<br/>
Указывает на [структура POINT](/windows/desktop/api/windef/ns-windef-tagpoint) или `CPoint` , содержащий в верхний левый угол элемента управления.

*pSize*<br/>
Указывает на [размер](/windows/desktop/api/windef/ns-windef-tagsize) структуры или `CSize` , содержащий размер элемента управления
...
Если `CWnd` уничтожения — дочернее окно и не имеет [WS_EX_NOPARENTNOTIFY](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles) в стиле набора, то [WM_PARENTNOTIFY ](/previous-versions/windows/desktop/inputmsg/wm-parentnotify) сообщение отправляется в родительский объект.
...
`DlgDirList` отправляет [LB_RESETCONTENT](/windows/desktop/Controls/lb-resetcontent) и [LB_DIR](/windows/desktop/Controls/lb-dir) сообщения в поле со списком. Он заполняет поле со списком определяется *nIDListBox* имена всех файлов, соответствующих указанному по пути *lpPathSpec*.

*LpPathSpec* параметр имеет следующий вид:

`[drive:] [ [\u]directory[\idirectory]...\u] [filename]`

В этом примере *диск* — это буква диска, *directory* — это имя допустимого каталога, и *filename* является допустимым именем файла, который должен содержать по крайней мере один символ. Подстановочные знаки вопроса ( **?** ), что означает соответствует любому символу, а звездочка (<strong>\*</strong>), то есть соответствует любому количеству символов.
...
`DlgDirListComboBox` отправляет [CB_RESETCONTENT](/windows/desktop/Controls/cb-resetcontent) и [CB_DIR](/windows/desktop/Controls/cb-dir) сообщения в поле со списком. Он заполняет поле списка в поле со списком, определяемое *nIDComboBox* имена всех файлов, соответствующих указанному по пути *lpPathSpec*.

*LpPathSpec* параметр имеет следующий вид:

`[drive:] [ [\u]directory[\idirectory]...\u] [filename]`

В этом примере *диск* — это буква диска, *directory* — это имя допустимого каталога, и *filename* является допустимым именем файла, который должен содержать по крайней мере один символ. Подстановочные знаки вопроса ( **?** ), какие соответствует любому символу, а звездочка (<strong>\*</strong>), что означает соответствует любому количеству символов.
...
`DlgDirSelect` отправляет [LB_GETCURSEL](/windows/desktop/Controls/lb-getcursel) и [LB_GETTEXT](/windows/desktop/Controls/lb-gettext) сообщения в поле со списком.
...
`DlgDirSelectComboBox` отправляет [CB_GETCURSEL](/windows/desktop/Controls/cb-getcursel) и [CB_GETLBTEXT](/windows/desktop/Controls/cb-getlbtext) сообщения в поле со списком.
...
*lprcFrom*<br/>
Указатель на [RECT](/previous-versions/dd162897\(v=vs.85\)) структуры, указывающий расположение и размер значка или свернутого окна.

*lprcTo*<br/>
Указатель на [RECT](/previous-versions/dd162897\(v=vs.85\)) структуру, указав расположение и размер восстановленной окна
...
Эта функция-член эмулирует работу функции [DrawAnimatedRects](/windows/desktop/api/winuser/nf-winuser-drawanimatedrects), как описано в пакете Windows SDK.
...
Эта функция-член эмулирует работу функции [DrawCaption](/windows/desktop/api/winuser/nf-winuser-drawcaption), как описано в пакете Windows SDK.
...
Переопределить [OnToolHitTest](#ontoolhittest) для предоставления [TOOLINFO](/windows/desktop/api/commctrl/ns-commctrl-tagtoolinfoa) структуры или структур для окна.

> [!NOTE]
> Некоторые окна, такие как [CToolBar](../../mfc/reference/ctoolbar-class.md), встроенные реализации [OnToolHitTest](#ontoolhittest).

См. в разделе [TOOLINFO](/windows/desktop/api/commctrl/ns-commctrl-tagtoolinfoa) в пакете SDK для Windows, Дополнительные сведения об этой структуре.
...
*lpPaint*<br/>
Указывает на [PAINTSTRUCT](/windows/desktop/api/winuser/ns-winuser-tagpaintstruct) структуру, содержащую рисования сведений, получаемых методом [BeginPaint](#beginpaint) функция-член.
...
*lpszClass*<br/>
Указатель на заканчивающуюся нулем строку, указывающее имя класса или класса atom, созданные после предыдущего вызова [RegisterClass](/windows/desktop/api/winuser/nf-winuser-registerclassa) или [RegisterClassEx](/windows/desktop/api/winuser/nf-winuser-registerclassexa).
...
Этот метод эмулирует работу функции [FlashWindowEx](/windows/desktop/api/winuser/nf-winuser-flashwindowex), как описано в пакете Windows SDK.
...
Возвращает значение S_OK в случае успешного выполнения, код ошибки COM. в случае сбоя. См. в разделе **возвращаемые значения** в [IAccessible::get_accChild](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accchild) в Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::get_accChild](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accchild) в пакете Windows SDK.
...
Возвращает значение S_OK в случае успешного выполнения, код ошибки COM. в случае сбоя. См. в разделе **возвращаемые значения** в [IAccessible::get_accChildCount](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accchildcount) в Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::get_accChildCount](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accchildcount) в пакете Windows SDK.
...
Возвращает значение S_OK в случае успешного выполнения, код ошибки COM. в случае сбоя. См. в разделе **возвращаемые значения** в [IAccessible::get_accDefaultAction](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accdefaultaction) в Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::get_accDefaultAction](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accdefaultaction) в пакете Windows SDK.
...
Возвращает значение S_OK в случае успешного выполнения, код ошибки COM. в случае сбоя. См. в разделе **возвращаемые значения** в [IAccessible::get_accDescription](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accdescription) в Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::get_accDescription](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accdescription) в пакете Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::get_accFocus](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accfocus) в пакете Windows SDK.
...
Возвращает значение S_OK в случае успешного выполнения, код ошибки COM. в случае сбоя. См. в разделе **возвращаемые значения** в [IAccessible::get_accHelp](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_acchelp) в Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::get_accHelp](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_acchelp) в пакете Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::get_accHelpTopic](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_acchelptopic) в пакете Windows SDK.
...
Возвращает значение S_OK в случае успешного выполнения, код ошибки COM. в случае сбоя. См. в разделе **возвращаемые значения** в [IAccessible::get_accKeyboardShortcut](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_acckeyboardshortcut) в Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::get_accKeyboardShortcut](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_acckeyboardshortcut) в пакете Windows SDK.
...
Возвращает значение S_OK в случае успешного выполнения, код ошибки COM. в случае сбоя. См. в разделе **возвращаемые значения** в [IAccessible::get_accName](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accname) в Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::get_accName](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accname) в пакете Windows SDK.
...
Возвращает значение S_OK в случае успешного выполнения, код ошибки COM. в случае сбоя. См. в разделе **возвращаемые значения** в [IAccessible::get_accParent](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accparent) в Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::get_accParent](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accparent) в пакете Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::get_accRole](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accrole) в пакете Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::get_accSelection](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accselection) в пакете Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::get_accState](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accstate) в пакете Windows SDK.
...
Возвращает значение S_OK в случае успешного выполнения, код ошибки COM. в случае сбоя. См. в разделе **возвращаемые значения** в [IAccessible::get_accValue](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accvalue) в Windows SDK.
...
Дополнительные сведения см. в разделе [IAccessible::get_accValue](/windows/desktop/api/oleacc/nf-oleacc-iaccessible-get_accvalue) в пакете Windows SDK.
...
*lpRect*<br/>
Указывает на [структура RECT](/windows/desktop/api/windef/ns-windef-tagrect) или `CRect` объект для получения клиентских координат. `left` И `top` элементов будет 0. `right` И `bottom` будет содержать члены, ширину и высоту окна.
...
Указатель на [IUnknown](/windows/desktop/api/unknwn/nn-unknwn-iunknown) интерфейса элемента управления OLE, представленный этим `CWnd` объекта. Если этот объект не представляет элемент управления OLE, возвращается значение NULL.
...
Указатель интерфейса, возвращаемый `GetControlUnknown` не является подсчетом ссылок. Не вызывайте [IUnknown::Release](/windows/desktop/api/unknwn/nf-unknwn-iunknown-release) указателя, если не был вызван ранее [IUnknown::AddRef](/windows/desktop/api/unknwn/nf-unknwn-iunknown-addref) на нем.
...
Возвращает указатель на сообщение, обрабатываемое этим окном в настоящий момент. Должен вызываться только когда **на**<em>сообщение</em> функция-член обработчика сообщений.
...
Возвращает указатель на [MSG](/windows/desktop/api/winuser/ns-winuser-tagmsg) структуру, содержащую сообщение окна обрабатывает в настоящий момент. Должен вызываться только когда **на**<em>сообщение</em> обработчика.
...
Эта функция, которая представляет собой расширение для [GetDC](/windows/desktop/api/winuser/nf-winuser-getdc) функции, имеет контролировать способ и необходимость обрезается контекста устройства для окна приложения.
...
Он отправляет [WM_GETTEXT](/windows/desktop/winmsg/wm-gettext) сообщение элементу управления.
...
Эта функция-член эмулирует работу функции [о функции](/windows/desktop/api/winuser/nf-winuser-getmenubarinfo), как описано в пакете Windows SDK.
...
Эта функция-член эмулирует работу функции [GetScrollBarInfo](/windows/desktop/api/winuser/nf-winuser-getscrollbarinfo), как описано в пакете Windows SDK.
...
*lpRect*<br/>
Указывает на `CRect` объекта или [структура RECT](/windows/desktop/api/windef/ns-windef-tagrect) который используется для отправки клиентских координат элемента обновление, которое включает в область обновления.
...
`flags` Членом [WINDOWPLACEMENT](/windows/desktop/api/winuser/ns-winuser-tagwindowplacement) структуры, полученные при помощи этой функции всегда равно 0. Если `CWnd` развернуто, `showCmd` членом `WINDOWPLACEMENT` является SW_SHOWMAXIMIZED. Если окно свернуто, то SW_SHOWMINIMIZED. В противном случае это SW_SHOWNORMAL.
...
*lpRect*<br/>
Указывает на `CRect` объекта или [структура RECT](/windows/desktop/api/windef/ns-windef-tagrect) , получат экранные координаты верхнего левого и правого нижнего углов.
...
Эта функция-член вызывает [WM_GETTEXT](/windows/desktop/winmsg/wm-gettext) сообщение должно отправляться `CWnd` объекта.
...
Эта функция-член вызывает [WM_GETTEXTLENGTH](/windows/desktop/winmsg/wm-gettextlength) сообщение должно отправляться `CWnd` объекта.
...
*lpRect*<br/>
Указывает на `CRect` объекта или [структура RECT](/windows/desktop/api/windef/ns-windef-tagrect) , содержащий прямоугольник (в координатах клиентской области окна) для добавления в область обновления. Если *lpRect* имеет значение NULL, всю клиентскую область добавляется к области.
...
*lpMsg*<br/>
Указывает на [MSG](/windows/desktop/api/winuser/ns-winuser-tagmsg) структуру, содержащую сообщения для проверки.
...
*lpPoint*<br/>
Указатель на массив [структура POINT](/windows/desktop/api/windef/ns-windef-tagpoint) , содержащие набор точек, для преобразования.
...
Этот метод использует [MessageBox](/windows/desktop/api/winuser/nf-winuser-messagebox) функции, как определено в пакете Windows SDK. Этот метод возвращает результат вызова этой функции.
...
*lpRect*<br/>
[CRect](../../atl-mfc-shared/reference/crect-class.md) объекта или [структура RECT](/windows/desktop/api/windef/ns-windef-tagrect) , который указывает новый размер и положение.
...
Эта функция-член эмулирует работу функции [NotifyWinEvent](/windows/desktop/api/winuser/nf-winuser-notifywinevent), как описано в пакете Windows SDK.
...
|*nDevice*|[in] Устройство ввода, вызвавшего событие ввода. Список возможных значений см. в разделе устройств в разделе *uDevice* раздел *lParam* параметр [WM_APPCOMMAND](/windows/desktop/inputdev/wm-appcommand).|
|*nKey*|[in] Указывает виртуальный ключи, которые не работают, например клавишу CTRL или левую кнопку мыши. Список возможных значений см. в разделе реестра *dwKeys* раздел *lParam* параметр [WM_APPCOMMAND](/windows/desktop/inputdev/wm-appcommand). Дополнительные сведения см. в разделе «Параметры сообщения» подзаголовка в [о ввод от мыши](/windows/desktop/inputdev/about-mouse-input).|

### <a name="remarks"></a>Примечания

Этот метод получает [WM_APPCOMMAND](/windows/desktop/inputdev/wm-appcommand) уведомлений, который описан в пакете Windows SDK.
...
Реализация по умолчанию реагирует вызовом [ReleaseCapture](/windows/desktop/api/winuser/nf-winuser-releasecapture) функции Windows. Переопределите эта функция-член в производном классе для обработки других режимов.
...
Окно получает это сообщение, даже если он вызывает [ReleaseCapture](/windows/desktop/api/winuser/nf-winuser-releasecapture) сам. Приложения не следует пытаться установить захват мыши в ответ на это сообщение. При получении этого сообщения, окно перерисовываться, при необходимости, чтобы отразить новое состояние захвата мыши.
...
- Элемент пользовательского интерфейса UIS_INITIALIZE state (определяется *nUIElement*) должны изменяться в зависимости от последнее событие ввода. Дополнительные сведения см. в разделе **"Примечания"** раздел [WM_CHANGEUISTATE](/windows/desktop/menurc/wm-changeuistate).
...
Эта функция-член эмулирует функциональные возможности [WM_CHANGEUISTATE](/windows/desktop/menurc/wm-changeuistate) сообщения, как описано в пакете Windows SDK.
...
Вызывается, когда поле со списком с [LBS_WANTKEYBOARDINPUT](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) стиля отправляет его владельцем [WM_CHARTOITEM](/windows/desktop/Controls/wm-chartoitem) сообщение в ответ на [WM_CHAR](#onchar) сообщения.
...
Этот метод получает [WM_DWMNCRENDERINGCHANGED](/windows/desktop/dwm/wm-dwmcolorizationcolorchanged) сообщение уведомления, который описан в пакете Windows SDK.
...
Эта функция-член в производном классе для обработки переопределить [WM_COMMAND](/windows/desktop/menurc/wm-command) сообщения. Переопределение не будет обрабатывать схему сообщений, если базовый класс `OnCommand` вызывается.
...
Содержит длинный указатель на [COMPAREITEMSTRUCT](/windows/desktop/api/winuser/ns-winuser-tagcompareitemstruct) структура данных, содержащая список идентификаторов и данных, предоставляемую приложением, для двух элементов в поле со списком или списка.
...
Этот метод получает [WM_DWMCOMPOSITIONCHANGED](/windows/desktop/dwm/wm-dwmcompositionchanged) уведомлений, который описан в пакете Windows SDK.
...
*lpCreateStruct*<br/>
Указывает на [CREATESTRUCT](/windows/desktop/api/winuser/ns-winuser-tagcreatestructa) структуру, содержащую сведения о `CWnd` создаваемого объекта.
...
Указывает, длинный указатель на [DELETEITEMSTRUCT](/windows/desktop/api/winuser/ns-winuser-tagdeleteitemstruct) структуру данных, содержащую сведения об элементе поле удаленный список.
...
Платформа вызывает эту функцию-член для владельца буфера обмена, когда буфер обмена очищается путем вызова [EmptyClipboard](/windows/desktop/api/winuser/nf-winuser-emptyclipboard) функции Windows.
...
- [DBT_DEVICEARRIVAL](/windows/desktop/DevIO/dbt-devicearrival) устройство было добавлено и теперь доступна.

- [DBT_DEVICEQUERYREMOVE](/windows/desktop/DevIO/dbt-devicequeryremove) запрашивается разрешение на удаление устройства. Любое приложение может отклонить этот запрос и отменить удаление.

- [DBT_DEVICEQUERYREMOVEFAILED](/windows/desktop/DevIO/dbt-devicequeryremovefailed) запрос для удаления устройства был отменен.

- [DBT_DEVICEREMOVEPENDING](/windows/desktop/DevIO/dbt-deviceremovepending) устройство является удалением. Не может быть отказано.

- [DBT_DEVICEREMOVECOMPLETE](/windows/desktop/DevIO/dbt-deviceremovecomplete) устройство было удалено.

- [DBT_DEVICETYPESPECIFIC](/windows/desktop/DevIO/dbt-devicetypespecific) событий конкретного устройства.

- [DBT_CONFIGCHANGED](/windows/desktop/DevIO/dbt-configchanged) текущей конфигурации был изменен.
...
`itemAction` Членом [DRAWITEMSTRUCT](/windows/desktop/api/winuser/ns-winuser-tagdrawitemstruct) структура определяет операции рисования, который должен выполняться. Данные в этот элемент позволяет владелец элемента управления определить, какие рисования действий не требуется.
...
Этот метод получает [WM_ENTERSIZEMOVE](/windows/desktop/winmsg/wm-entersizemove) уведомлений, который описан в пакете Windows SDK.

Окно вводит скользящее или изменения размера модального цикла, когда пользователь щелкает заголовок или границы изменения размера окна или окна передает [WM_SYSCOMMAND](/windows/desktop/menurc/wm-syscommand) сообщение [CWnd::DefWindowProc](#defwindowproc) функции и *wParam* параметр это сообщение указывает SC_MOVE или SC_SIZE.
...
Этот метод получает [WM_EXITSIZEMOVE](/windows/desktop/winmsg/wm-exitsizemove) уведомлений, который описан в пакете Windows SDK.
...
*lpMMI*<br/>
Указывает на `MINMAXINFO` структуру, содержащую сведения об окнах развернуто, его размер отслеживания минимального и максимального размеров и положения. Дополнительные сведения об этой структуре, см. в разделе [MINMAXINFO](/windows/desktop/api/winuser/ns-winuser-tagminmaxinfo) структуры.
...
|*nKey2*|[in] Виртуального кода клавиши сочетания клавиш.|

### <a name="remarks"></a>Примечания

Этот метод получает [WM_HOTKEY](/windows/desktop/inputdev/wm-hotkey) уведомлений, который описан в пакете Windows SDK. Это сообщение помещается в верхней части очереди сообщений, связанный с потоком, который регистрировал клавиш. Используйте [RegisterHotKey](/windows/desktop/api/winuser/nf-winuser-registerhotkey) функции для регистрации сочетания клавиш во всей системе.
...
Этот метод получает [WM_INPUT_DEVICE_CHANGE](/windows/desktop/inputdev/wm-input-device-change) уведомлений, который описан в пакете Windows SDK. Представляет собой сообщение универсальные устройства ввода.
...
Этот метод получает [WM_INPUTLANGCHANGE](/windows/desktop/winmsg/wm-inputlangchange) сообщение уведомления, который описан в пакете Windows SDK.
...
Указывает на [MEASUREITEMSTRUCT](/windows/desktop/api/winuser/ns-winuser-tagmeasureitemstruct) структура данных, которая содержит размеры рисуемого владельцем элемента управления.
...
Windows инициирует вызов `OnMeasureItem` владельца поля со списком и списки, создан со стилем OWNERDRAWFIXED перед отправкой [WM_INITDIALOG](/windows/desktop/dlgbox/wm-initdialog) сообщения. Таким образом когда владелец получает этот вызов, Windows еще не определено высоту и ширину шрифта, используемого в элементе управления; вызовы функций и вычисления, которым требуются эти значения должно выполняться в основной функции приложения или библиотеки.
...
Этот метод получает [WM_MENUDRAG](/windows/desktop/menurc/wm-menudrag) уведомлений, который описан в пакете Windows SDK.
...
|Возвращаемое значение|Значение|
|------------------|-------------|
|MNGO_NOERROR|Указатель интерфейса, который поддерживает операции перетаскивания и перетащите возвращается в `pvObj` членом [MENUGETOBJECTINFO](/windows/desktop/api/winuser/ns-winuser-tagmenugetobjectinfo) структуры. В настоящее время только [IDropTarget](/windows/desktop/api/oleidl/nn-oleidl-idroptarget) интерфейс поддерживается.|
|MNGO_NOINTERFACE|Интерфейс перетаскивания и перетащите не поддерживается.|

### <a name="remarks"></a>Примечания

Этот метод получает [WM_MENUGETOBJECT](/windows/desktop/menurc/wm-menugetobject) уведомлений, который описан в пакете Windows SDK.
...
Этот метод получает [WM_MENURBUTTONUP](/windows/desktop/menurc/wm-menurbuttonup) уведомлений, который описан в пакете Windows SDK. [WM_MENURBUTTONUP](/windows/desktop/menurc/wm-menurbuttonup) сообщений позволяет приложению предоставлять контекстное меню для элемента, указанного в сообщении меню.
...
Этот метод получает [WM_MOUSEHOVER](/windows/desktop/inputdev/wm-mousehover) уведомлений, который описан в пакете Windows SDK.
...
|*pt*|[in] Объект [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) , указывающий *x* и *y* координаты указателя относительно левого верхнего угла клиентской области.|

### <a name="remarks"></a>Примечания

Этот метод получает [WM_MOUSEHWHEEL](/windows/desktop/inputdev/wm-mousehwheel) сообщение уведомления, который описан в пакете Windows SDK. Это сообщение отправляется в окно с фокусом, когда колесо горизонтальной прокрутки поворачивается или крутится.
...
Этот метод получает [WM_MOUSELEAVE](/windows/desktop/inputdev/wm-mouseleave) уведомлений, который описан в пакете Windows SDK.
...
*lpRect*<br/>
Адрес [CRect](../../atl-mfc-shared/reference/crect-class.md) или [структура RECT](/windows/desktop/api/windef/ns-windef-tagrect) , содержащий координаты элемента.
...
*lpncsp*<br/>
Указывает на [NCCALCSIZE_PARAMS](/windows/desktop/api/winuser/ns-winuser-tagnccalcsize_params) структуру данных, содержащую сведения, которые приложение может использовать для вычисления новый размер и положение `CWnd` прямоугольника (в том числе клиентской области, границы, заголовок, полосы прокрутки и т. д.) .
...
*lpCreateStruct*<br/>
Указывает на [CREATESTRUCT](/windows/desktop/api/winuser/ns-winuser-tagcreatestructa) структуры данных для `CWnd`.
...
Один из проверки нажатия мыши значений перечисления. См. в разделе [WM_NCHITTEST](/windows/desktop/inputdev/wm-nchittest) список значений.
...
*nHitTest*<br/>
Указывает [код проверки нажатия](#onnchittest). Проверку нажатия является тест, который определяет расположение курсора. См. в разделе [WM_NCHITTEST](/windows/desktop/inputdev/wm-nchittest) список значений.
...
Этот метод получает [WM_NCMOUSEHOVER](/windows/desktop/inputdev/wm-ncmousehover) уведомлений, который описан в пакете Windows SDK.
...
Этот метод получает [WM_NCMOUSELEAVE](/windows/desktop/inputdev/wm-ncmouseleave) уведомлений, который описан в пакете Windows SDK.
...
Этот метод получает [WM_DWMNCRENDERINGCHANGED](/windows/desktop/dwm/wm-dwmncrenderingchanged) уведомлений, который описан в пакете Windows SDK.
...
Этот метод получает [WM_XBUTTONDBLCLK](/windows/desktop/inputdev/wm-xbuttondblclk) уведомлений, который описан в пакете Windows SDK. Это сообщение передается в окно, в которой находится курсор. Если мышь захвачена окна, это сообщение не публикуется.
...
Этот метод получает [WM_NCXBUTTONDOWN](/windows/desktop/inputdev/wm-ncxbuttondown) уведомлений, который описан в пакете Windows SDK. Это сообщение передается в окно, в которой находится курсор. Если мышь захвачена окна, это сообщение не публикуется.
...
Этот метод получает [WM_NCXBUTTONUP](/windows/desktop/inputdev/wm-ncxbuttonup) уведомлений, который описан в пакете Windows SDK. Это сообщение передается в окно, в которой находится курсор. Если мышь захвачена окна, это сообщение не публикуется.
...
Эта функция-член вызывается платформой, когда клавишу со стрелкой вправо или влево используется для переключения панели меню или системного меню.
...
Этот метод получает [WM_UNINITMENUPOPUP](/windows/desktop/menurc/wm-uninitmenupopup) уведомлений, который описан в пакете Windows SDK. В ответ на это сообщение, приложение может задать `hmenuNext` членом [MDINEXTMENU](/windows/desktop/api/winuser/ns-winuser-tagmdinextmenu) структуры, чтобы указать в меню, чтобы переключиться в режим и `hwndNext` члена, чтобы указать размер окна для получения сообщений уведомления меню .
...
Этот метод получает [WM_NOTIFYFORMAT](/windows/desktop/Controls/wm-notifyformat) уведомлений, который описан в пакете Windows SDK. Сообщения WM_NOTIFY, отправляются в общем элементе управления своему родительскому окну, а также от родительского окна для общего элемента управления.
...
`OnPaintClipboard` следует использовать [GlobalLock](/windows/desktop/api/winbase/nf-winbase-globallock) функцию Windows для блокировки памяти, содержащий `PAINTSTRUCT` данные структуры и разблокировки памяти [GlobalUnlock](/windows/desktop/api/winbase/nf-winbase-globalunlock) Windows работать до ее завершения.
...
Эта функция-член вызывается платформой, при завершении пользователем сеанса Windows или когда приложение вызывает [ExitWindows](/windows/desktop/api/winuser/nf-winuser-exitwindows) функции Windows.
...
Эта функция-член эмулирует функциональные возможности [WM_QUERYUISTATE](/windows/desktop/menurc/wm-queryuistate) сообщения, как описано в пакете Windows SDK.
...
Этот метод получает [WM_INPUT](/windows/desktop/inputdev/wm-appcommand) уведомлений, который описан в пакете Windows SDK.
...
Если это не переопределено, `OnRegisteredMouseWheel` направляет сообщение в соответствующем окне (родительского окна с фокусом) и вызовы [WM_MOUSEWHEEL](/windows/desktop/inputdev/wm-mousewheel) обработчик для этого окна.
...
Владелец буфер обмена подготавливаться к просмотру данных в любых форматах, он может создать и передать дескриптор для каждого формата данных в буфер обмена, путем вызова [SetClipboardData](/windows/desktop/api/winuser/nf-winuser-setclipboarddata) функции Windows. Это гарантирует, что буфер обмена содержит допустимые данные, несмотря на то, что приложение, к просмотру данных уничтожается. Приложение должно вызвать [OpenClipboard](#openclipboard) перед вызовом функции-члена [SetClipboardData](/windows/desktop/api/winuser/nf-winuser-setclipboarddata) функции Windows и вызов [CloseClipboard](/windows/desktop/api/winuser/nf-winuser-closeclipboard) Windows После этого функция.
...
Получатель должен отображать данные в этом формате и передать его в буфер обмена, путем вызова [SetClipboardData](/windows/desktop/api/winuser/nf-winuser-setclipboarddata) функции Windows.
...
Этот метод получает [WM_WTSSESSION_CHANGE](/windows/desktop/TermServ/wm-wtssession-change) уведомлений, который описан в пакете Windows SDK.
...
В рамках `OnSizeClipboard`, приложение должно использовать [GlobalLock](/windows/desktop/api/winbase/nf-winbase-globallock) функцию Windows для блокировки памяти, содержащий структуру RECT данных. Разверните приложение-разблокировки памяти [GlobalUnlock](/windows/desktop/api/winbase/nf-winbase-globalunlock) Windows работать до дает или возвращает элемент управления.
...
*lpStyleStruct*<br/>
Указывает на [STYLESTRUCT](/windows/desktop/api/winuser/ns-winuser-stylestruct) структуру, содержащую новые стили для окна. Приложение может проверить стили, но его можно не изменять их.
...
*lpStyleStruct*<br/>
Указывает на [STYLESTRUCT](/windows/desktop/api/winuser/ns-winuser-stylestruct) структуру, содержащую новые стили для окна. Приложения можно проверить стили и изменить их.
...
Он указывает виртуального кода клавиши ключа меню для элемента управления. (Список стандартные коды виртуальных клавиш, см. в разделе Winuser.h)

Если код контекста равно 0, можно передать WM_SYSCHAR [WM_SYSCHAR](/windows/desktop/menurc/wm-syschar) сообщение [TranslateAccelerator](/windows/desktop/api/winuser/nf-winuser-translateacceleratora) функции Windows, которая будет обрабатывать его, как будто это обычный послание вместо системы клавиши знака. Это позволяет сочетания клавиш для использования с активным окном, даже если окна не имеет фокус ввода.
...
Если код контекста равно 0, WM_SYSKEYDOWN сообщение получено по `OnSysKeyDown` могут передаваться [TranslateAccelerator](/windows/desktop/api/winuser/nf-winuser-translateacceleratora) функции Windows, которая будет обрабатывать его, как будто это обычный послание вместо сообщения системного ключа. Это позволяет сочетания клавиш для использования с активным окном, даже если окна не имеет фокус ввода.
...
Если код контекста равно 0, WM_SYSKEYUP сообщение получено по `OnSysKeyUp` могут передаваться [TranslateAccelerator](/windows/desktop/api/winuser/nf-winuser-translateacceleratora) функции Windows, которая будет обрабатывать его, как будто это обычный послание вместо сообщения системного ключа. Благодаря этому сочетаний клавиш (клавиши) для использования с активным окном, даже если окна не имеет фокус ввода.
...
См. в разделе [TOOLINFO](/windows/desktop/api/commctrl/ns-commctrl-tagtoolinfoa), в пакете Windows SDK, Дополнительные сведения о структуре.
...
Этот метод получает [WM_UNINITMENUPOPUP](/windows/desktop/menurc/wm-uninitmenupopup) уведомлений, который описан в пакете Windows SDK.
...
- Элемент пользовательского интерфейса UIS_INITIALIZE state (определяется *nUIElement*) должны изменяться в зависимости от последнее событие ввода. Дополнительные сведения см. в разделе **"Примечания"** раздел [WM_UPDATEISTATE](/windows/desktop/menurc/wm-updateuistate).
...
Эта функция-член эмулирует функциональные возможности [WM_UPDATEUISTATE](/windows/desktop/menurc/wm-updateuistate) сообщения, как описано в пакете Windows SDK.
...
Этот метод получает [WM_DWMWINDOWMAXIMIZEDCHANGE](/windows/desktop/dwm/wm-dwmwindowmaximizedchange) сообщение уведомления, который описан в пакете Windows SDK.
...
*lpwndpos*<br/>
Указывает на [WINDOWPOS](/windows/desktop/api/winuser/ns-winuser-tagwindowpos) структуру данных, содержащую сведения о новый размер и положение окна.

### <a name="remarks"></a>Примечания

Реализация по умолчанию отправляет [WM_SIZE](/windows/desktop/winmsg/wm-size) и [WM_MOVE](/windows/desktop/winmsg/wm-move) сообщения в окне. Эти сообщения отправляются не в том случае, если приложение обрабатывает `OnWindowPosChanged` вызывается без вызова своего базового класса. Более эффективно для выполнения любого перемещения или размер обработку изменений во время вызова `OnWindowPosChanged` без вызова своего базового класса.
...
Приложения можно предотвратить изменения в окно, при установке или снятии соответствующие биты в **флаги** членом [WINDOWPOS](/windows/desktop/api/winuser/ns-winuser-tagwindowpos) структуры.

Для окна с [WS_OVERLAPPED](styles-used-by-mfc.md#window-styles) или [WS_THICKFRAME](styles-used-by-mfc.md#window-styles) стиль, реализация по умолчанию отправляет [WM_GETMINMAXINFO](/windows/desktop/winmsg/wm-getminmaxinfo) сообщение в окно. Это делается для проверки новый размер и положение окна и для соблюдения CS_BYTEALIGNCLIENT и CS_BYTEALIGN стили клиента. Приложение может переопределить эту функцию, не вызвав его базового класса.
...
Этот метод получает [WM_XBUTTONDBLCLK](/windows/desktop/inputdev/wm-xbuttondblclk) уведомлений, который описан в пакете Windows SDK. Если указатель мыши не фиксируются, публикации сообщения в окно под курсором. В противном случае сообщение публикуется для окна, мышь захвачена.
...
Этот метод получает [WM_XBUTTONDOWN](/windows/desktop/inputdev/wm-xbuttondown) уведомлений, который описан в пакете Windows SDK. Если указатель мыши не фиксируются, публикации сообщения в окно под курсором. В противном случае сообщение публикуется для окна, мышь захвачена.
...
Этот метод получает [WM_XBUTTONUP](/windows/desktop/inputdev/wm-xbuttonup) уведомлений, который описан в пакете Windows SDK. Если указатель мыши не фиксируются, публикации сообщения в окно под курсором. В противном случае сообщение публикуется для окна, мышь захвачена.
...
Другие приложения, не сможете изменить в буфер обмена, пока [CloseClipboard](/windows/desktop/api/winuser/nf-winuser-closeclipboard) вызове функции Windows.

Текущий `CWnd` объект не станет владельца буфера обмена до [EmptyClipboard](/windows/desktop/api/winuser/nf-winuser-emptyclipboard) вызове функции Windows.
...
*pMsg*<br/>
Указывает на [MSG](/windows/desktop/api/winuser/ns-winuser-tagmsg) структуру, содержащую сообщение для обработки.
...
Эта функция-член эмулирует работу функции [PrintWindow](/windows/desktop/api/winuser/nf-winuser-printwindow), как описано в пакете Windows SDK.
...
*lpRectUpdate*<br/>
Указывает на [структура RECT](/windows/desktop/api/windef/ns-windef-tagrect) содержит координаты прямоугольника обновления. Этот параметр учитывается, если *prgnUpdate* содержит дескриптор допустимый регион.
...
- RDW_FRAME приводит к любой части неклиентской области окна, которая пересекает область обновления для получения [WM_NCPAINT](/windows/desktop/gdi/wm-ncpaint) сообщения. Необходимо также указать флаг RDW_INVALIDATE; в противном случае RDW_FRAME не оказывает влияния.

- Вызывает RDW_INTERNALPAINT [WM_PAINT](/windows/desktop/gdi/wm-paint) сообщение для публикации в окно, независимо от того, является ли окно содержит недопустимый регион.
...
Когда `RedrawWindow` требуется сделать недействительной часть окно рабочего стола, используется функция-член этого окна не получает [WM_PAINT](/windows/desktop/gdi/wm-paint) сообщения. Для обновления рабочего стола, приложение должно использовать [CWnd::ValidateRgn](#validatergn), [CWnd::InvalidateRgn](#invalidatergn), [CWnd::UpdateWindow](#updatewindow), или [RedrawWindow](/windows/desktop/api/winuser/nf-winuser-redrawwindow)
...
*lpRectParam*<br/>
Указывает на [структура RECT](/windows/desktop/api/windef/ns-windef-tagrect); использование которого зависит от значения *снять пометку со*.

*lpRectClient*<br/>
Указывает на [структура RECT](/windows/desktop/api/windef/ns-windef-tagrect) содержащий доступных клиентской области. Если значение равно NULL, будет использоваться клиентской области окна.
...
*lpPoint*<br/>
Указывает на [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) объекта или [структура POINT](/windows/desktop/api/windef/ns-windef-tagpoint) , содержащий координаты экрана для преобразования.

*lpRect*<br/>
Указывает на [CRect](../../atl-mfc-shared/reference/crect-class.md) объекта или [структура RECT](/windows/desktop/api/windef/ns-windef-tagrect) , содержащий координаты экрана для преобразования.
...
*lpRect*<br/>
Указывает на [CRect](../../atl-mfc-shared/reference/crect-class.md) объекта или [структура RECT](/windows/desktop/api/windef/ns-windef-tagrect) , указывающий часть клиентской области, чтобы прокручивать. Если *lpRect* имеет значение NULL, может прокручиваться всей клиентской области. Курсор перемещен в том случае, если курсор прямоугольник пересекается прямоугольник прокрутки.
...
Обнаруженные области `ScrollWindow` функция-член не разрешено, но вместе с текущим `CWnd` область обновления объекта. В конце концов получит приложение [WM_PAINT](/windows/desktop/gdi/wm-paint) сообщение, уведомляющее, области требуется перерисовка. Обновление области непокрытая в то же время, прокрутка выполняется путем, вызовите [UpdateWindow](#updatewindow) сразу после вызова функции-члена `ScrollWindow`.
...
*lpRectScroll*<br/>
Указывает на [структура RECT](/windows/desktop/api/windef/ns-windef-tagrect) , указывающий часть клиентской области, чтобы прокручивать. Если этот параметр имеет значение NULL, может прокручиваться всей клиентской области.
...
Эта функция аналогична [ScrollWindow](/windows/desktop/api/winuser/nf-winuser-scrollwindow) функция некоторых дополнительных функциях.
...
Когда `CWnd` больше не требуются все команды мыши, приложение должно вызвать [ReleaseCapture](/windows/desktop/api/winuser/nf-winuser-releasecapture) функцию, чтобы другие windows может получать ввод от мыши.
...
`SetDlgItemInt` отправляет [WM_SETTEXT](/windows/desktop/winmsg/wm-settext) сообщений для заданного элемента управления.
...
`SetDlgItemText` отправляет [WM_SETTEXT](/windows/desktop/winmsg/wm-settext) сообщений для заданного элемента управления.
...
`SetFocus` Отправляет функция-член [WM_KILLFOCUS](/windows/desktop/inputdev/wm-killfocus) сообщения окна, которое теряет фокус ввода и [WM_SETFOCUS](/windows/desktop/inputdev/wm-setfocus) сообщения окна, которое получает фокус ввода. Она также активирует окна или его родителя.
...
*lpwndpl*<br/>
Указывает на [WINDOWPLACEMENT](/windows/desktop/api/winuser/ns-winuser-tagwindowplacement) структуры, указывающее новое состояние отображения и позиций.
...
Эта функция приводит к [WM_SETTEXT](/windows/desktop/winmsg/wm-settext) сообщения этому окну.
...
Только одно окно одновременно могут быть заблокированы с помощью `LockWindowUpdate`. См. в разделе [CWnd::LockWindowUpdate](#lockwindowupdate) или функцию Win32 [LockWindowUpdate](/windows/desktop/api/winuser/nf-winuser-lockwindowupdate) Дополнительные сведения о блокировке windows.
...
Обновляет клиентскую область, отправляя [WM_PAINT](/windows/desktop/gdi/wm-paint) сообщение, если области обновления не является пустым.
...
*lpRect*<br/>
Указывает на [CRect](../../atl-mfc-shared/reference/crect-class.md) объекта или [структура RECT](/windows/desktop/api/windef/ns-windef-tagrect) , содержащий клиент координаты прямоугольника, который необходимо удалить из области обновления. Если *lpRect* имеет значение NULL, проверяются все окно.

### <a name="remarks"></a>Примечания

[BeginPaint](#beginpaint) функция-член автоматически проверяет всю клиентскую область. Ни `ValidateRect` ни [ValidateRgn](#validatergn) следует вызвать функцию-член, если часть области обновления должна быть проверена перед [WM_PAINT](/windows/desktop/gdi/wm-paint) формировании следующего.
...
[BeginPaint](#beginpaint) функция-член автоматически проверяет всю клиентскую область. Ни [ValidateRect](#validaterect) ни `ValidateRgn` следует вызвать функцию-член, если часть области обновления должны быть проверены до следующей [WM_PAINT](/windows/desktop/gdi/wm-paint) формируется сообщение.
...
*point*<br/>
Указывает [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) объекта или [ТОЧКИ](/windows/desktop/api/windef/ns-windef-tagpoint) структура данных, которая определяет точку, для проверки.
