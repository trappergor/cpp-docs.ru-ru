...
### <a name="remarks"></a>Примечания  
 ...
 При обработке исключений, `CFile::Abort` отличается от `CFile::Close` в двух важных аспектах. Во-первых, `Abort` функция не вызывают исключение при сбое, поскольку сбои будут пропускаться `Abort`. Во-вторых, `Abort` не будет **ASSERT** Если файл не открыт или закрыт ранее.  
  
 Если вы использовали **новый** для выделения `CFile` объекта в куче, необходимо удалить его после закрытия файла. `Abort` Задает `m_hFile` для `CFile::hFileNull`.  
...
|Значение|Описание:|  
|-----------|-----------------|  
|`CFile::modeCreate`|Создает новый файл, если файл не существует. Если файл уже существует, перезаписать и нулевой длины по умолчанию значение.|  
...
### <a name="remarks"></a>Примечания  
 ...
 Если вы использовали **новый** для выделения `CFile` объекта в куче, необходимо удалить его после закрытия файла. `Close` Задает `m_hFile` для `CFile::hFileNull`.  
...
### <a name="remarks"></a>Примечания  
 ...
 Хотя `CFile` конструктор вызовет исключение в случае ошибки `Open` вернет **FALSE** причин ошибок. `Open` по-прежнему можно инициализировать [CFileException](../../mfc/reference/cfileexception-class.md) объект для описания ошибки, однако. Если вы не предоставите *pError* параметра, или если передается **NULL** для *pError*, `Open` вернет **FALSE** и создает исключение `CFileException`. Если передать указатель в существующий `CFileException`, и `Open` возникает ошибка, функция будет заполнить его подробное описание этой ошибки. В ни один из вариантов будут `Open` создаст исключение.  
