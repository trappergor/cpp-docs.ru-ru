...
Используйте `CFile` и его производных классов для общего назначения дискового ввода-вывода. Используйте `ofstream` или другого Майкрософт `iostream` классы для форматированный текст, отправляемый файл на диске.
...
Если файл еще не закрыт перед удалением объекта, деструктор закрывает его для вас.

При обработке исключений, `CFile::Abort` отличается от `CFile::Close` два важных отличия. Во-первых, `Abort` функция не создается исключение в случае сбоев, так как сбои будут пропускаться `Abort`. Во-вторых, `Abort` не **ASSERT** Если файл не был открыт, или был ранее закрыт.
...
|Значение|Описание|
|-----------|-----------------|
|`CFile::modeCreate`|Создает новый файл, если файл не существует. Если файл уже существует, он перезаписываются и задано значение нулевой длины.|
|`CFile::modeNoTruncate`|Создает новый файл, если файл не существует; в противном случае, если файл уже существует, он подключен к `CFile` объекта.|

Выберите соответствующие параметры кэширования файлов, описанные далее. По умолчанию система использует схему общего назначения кэширования, которая недоступна в виде параметра.
...
|`CFile::osRandomAccess`|Файловый кэш оптимизирован для случайного доступа. Не используйте этот параметр и параметром последовательного сканирования.|
...
|`CFile::osWriteThrough`|Записи операции будут выполняться без задержки.|
...
Конструктор по умолчанию инициализирует члены, но не присоединяет файл к `CFile` объекта. После использования этого конструктора, используйте [CFile::Open](#open) метод, чтобы открыть файл и присоединить его к `CFile` объекта.
...
Если `CFile` объект успешно открывает указанный файл, он будет закрыть этот файл автоматически при `CFile` уничтожении объекта; в противном случае необходимо явно закрыть файл после он больше не присоединен к `CFile` объекта.
...
Использование `Flush` не гарантирует списание `CArchive` буферов. Если вы используете архив, вызвать [CArchive::Flush](../../mfc/reference/carchive-class.md#flush) первого.
...
Версия нестатических `GetStatus` извлекает сведения о состоянии из открытого файла, связанного с данной `CFile` объекта.  Статическую версию `GetStatus` получает состояние файла из заданного пути к файлу без фактически при открытии файла. Эта версия удобен для тестирования права на доступ и существование файла.
...
После разблокирования консоли в регионе с помощью `UnlockRange` функция-член, диапазон байтов должен точно соответствовать регион, который ранее был заблокирован. `LockRange` Функция не слияния смежных областей. Если два заблокированных раздела являются смежными, каждый регион необходимо разблокировать отдельно.
...
`m_hFile` — это открытая переменная типа целое число без знака. Он содержит `CFile::hFileNull`, пустой файл операционной системы — независимый от индикатора, если дескриптор не была назначена.

Использование `m_hFile` не рекомендуется, так как значение элемента зависит от производного класса. `m_hFile` выполняется открытый член для удобства в поддержке неполиморфных использование класса.
...
*lpszFileName*<br/>
Строка, содержащая путь к требуемому файлу. Путь может быть относительным, абсолютное или сетевое имя (UNC).
...
Два `Open` функции являются «безопасных» методы для открытия файла, где сбоя — это обычный, ожидаемое условие.

Хотя `CFile` конструктор создает исключение в случае ошибки `Open` возвращает значение FALSE для условий ошибок. `Open` по-прежнему можно инициализировать [CFileException](../../mfc/reference/cfileexception-class.md) объект для описания ошибки, тем не менее. Если вы не указали *pError* параметра, или если передается значение NULL *pError*, `Open` возвращает значение FALSE и не выдавал `CFileException`. Если передать указатель в существующий `CFileException`, и `Open` возникает ошибка, функция заполняет его сведения, описывающие эту ошибку. `Open` не вызывает исключение в любом случае.
...
Количество байтов, переданных в буфер. Для всех `CFile` классы, возвращаемое значение может быть меньше, чем *nCount* если достигнут конец файла.
...
`Remove` не приведет к удалению каталога.

`Remove` Функция-член вызывает исключение, если подключенный файл открыт или если файл не может быть удален. Эта функция является эквивалентом команды DEL.
...
Невозможно переименовать каталоги. Эта функция является эквивалентом команды REN.
...
Указатель файла можно задать в позицию после конца файла. В противном случае не увеличивать размер файла, пока запись в файле.
...
Вызывайте эту функцию, чтобы указать путь к файлу. Например, если путь к файлу недоступен, когда [CFile](../../mfc/reference/cfile-class.md) объект создан, вызовите `SetFilePath` его.
...
Когда делается вызов к `SetStatus` при попытке изменить только атрибуты файла и `m_mtime` член структуры состояния файла имеет ненулевое значение, атрибуты также могут быть затронуты (изменение метка может иметь побочные эффекты, атрибуты времени). Если вы хотите только изменить атрибуты файла, сначала задайте `m_mtime` член структуры состояния файла нуля, а затем отправить вызов `SetStatus`.
...
Также см. в примерах [CFile::CFile](#cfile) и [CFile::Open](#open).
