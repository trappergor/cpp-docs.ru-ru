...
|[CListCtrl:: Create](#create)|Создает элемент управления "список" и прикрепляет его `CListCtrl` к объекту.|
...
В дополнение к значку и метке каждый элемент может отображать сведения в столбцах справа от значка и метки. Этот элемент управления (и, `CListCtrl` следовательно, класс) доступен только для программ, работающих под управлением Windows 95/98 и Windows NT версии 3,51 и более поздних версий.
...
Текущее представление определяется стилем текущего представления списка элемента управления. Дополнительные сведения об этих стилях и их использовании см. [в разделе using CListCtrl: Изменение стилей](../../mfc/changing-list-control-styles.md)элемента управления "список".
...
- Представления виртуального списка
   ...
   Если этот параметр включен, элемент управления поддерживает до DWORD элементов. Это можно сделать, поместив дополнительные издержки на управление данными элементов в приложении. За исключением выбора элементов и информации о фокусе, все сведения об элементе должны управляться приложением. Дополнительные сведения см. в [разделе using CListCtrl: Элементы управления](../../mfc/virtual-list-controls.md)виртуального списка.
...
Сведения об использовании этих новых расширенных стилей см. [в разделе using CListCtrl: Изменение стилей](../../mfc/changing-list-control-styles.md)элемента управления "список".
...
*нкоде*<br/>
Задает стиль выравнивания для элементов. Может принимать одно из следующих значений:

- LVA_ALIGNLEFT выровняйте элементы вдоль левого края окна.

- LVA_ALIGNTOP выдает элементы по верхнему краю окна.

- LVA_DEFAULT выравнивает элементы в соответствии с текущими стилями выравнивания в представлении списка (значение по умолчанию).
...
Эта функция члена эмулирует функциональность сообщения [LVM_CANCELEDITLABEL](/windows/win32/Controls/lvm-canceleditlabel) , как описано в Windows SDK.
...
Создает элемент управления "список" и прикрепляет его `CListCtrl` к объекту.
...
*rect*<br/>
Задает размер и расположение элемента управления "список". Это может быть либо `CRect` объект, либо структура [Rect](/previous-versions/dd162897\(v=vs.85\)) .

*ппарентвнд*<br/>
Указывает родительское окно элемента управления "список", `CDialog`обычно. Оно не должно иметь значение NULL.
...
Построение создается `CListCtrl` в два этапа. Сначала вызовите конструктор, а затем вызовите метод `Create`, который создает элемент управления представления списка и присоединяет его `CListCtrl` к объекту.
...
`CImageList` Объект является постоянным и его необходимо удалить по завершении. Например:
...
Элемент `itemAction` структуры [DRAWITEMSTRUCT](/windows/win32/api/winuser/ns-winuser-drawitemstruct) определяет выполняемое действие рисования.
...
Эта функция члена эмулирует функциональность сообщения [LVM_ENABLEGROUPVIEW](/windows/win32/Controls/lvm-enablegroupview) , как описано в Windows SDK.
...
*пколумн*<br/>
Адрес структуры [LVCOLUMN](/windows/win32/api/commctrl/ns-commctrl-lvcolumnw) , указывающей сведения для извлечения и получения сведений о столбце. `mask` Член указывает, какие атрибуты столбца следует извлечь. Если член задает значение LVCF_TEXT, то `pszText` элемент должен содержать адрес буфера, который получает текст элемента, а `cchTextMax` член должен задавать размер буфера. `mask`
...
Этот метод отправляет сообщение [LVM_GETEMPTYTEXT](/windows/win32/Controls/lvm-getemptytext) , описанное в Windows SDK.
...
Индекс группы, состояние которой равно LVGS_FOCUSED, если такая группа существует; в противном случае — значение-1.

### <a name="remarks"></a>Примечания

Этот метод отправляет сообщение [LVM_GETFOCUSEDGROUP](/windows/win32/Controls/lvm-getfocusedgroup) , описанное в Windows SDK. Дополнительные сведения см. в описании значения `state` LVGS_FOCUSED элемента структуры [лвграуп](/windows/win32/api/commctrl/ns-commctrl-lvgroup) .
...
Этот метод отправляет сообщение [LVM_GETGROUPCOUNT](/windows/win32/Controls/lvm-getgroupcount) , описанное в Windows SDK-->.
...
Эта функция члена эмулирует функциональность сообщения [LVM_GETGROUPINFO](/windows/win32/Controls/lvm-getgroupinfo) , как описано в Windows SDK.
...
|*пграуп*|заполняет Указатель на структуру [лвграуп](/windows/win32/api/commctrl/ns-commctrl-lvgroup) , которая получает сведения о группе, указанной параметром *ииндекс* .<br /><br /> Вызывающий объект отвечает за инициализацию членов структуры [лвграуп](/windows/win32/api/commctrl/ns-commctrl-lvgroup) . Задайте для `mask` элемента размер структуры, а флаги элемента — для указания извлекаемых данных. `cbSize`|
...
Этот метод отправляет сообщение [LVM_GETGROUPINFOBYINDEX](/windows/win32/controls/lvm-getgroupinfobyindex) , описанное в Windows SDK-->.
...
Эта функция члена эмулирует функциональность сообщения [LVM_GETGROUPMETRICS](/windows/win32/Controls/lvm-getgroupmetrics) , как описано в Windows SDK.
...
Этот метод отправляет сообщение [LVM_GETGROUPRECT](/windows/win32/Controls/lvm-getgrouprect) , описанное в Windows SDK.
...
|*двмаск*|окне Маска, указывающая значение состояния, которое необходимо получить для указанной группы. Дополнительные сведения см. в `mask` описании члена структуры [лвграуп](/windows/win32/api/commctrl/ns-commctrl-lvgroup) .|
...
Этот метод отправляет сообщение [LVM_GETGROUPSTATE](/windows/win32/Controls/lvm-getgroupstate) , описанное в Windows SDK. Дополнительные сведения см. в описании макроса [ListView_GetGroupState](/windows/win32/api/commctrl/nf-commctrl-listview_getgroupstate) .
...
*нимажелист*<br/>
Значение, указывающее список изображений для извлечения. Может принимать одно из следующих значений:

- LVSIL_NORMAL в списке изображений с большими значками.

- LVSIL_SMALL Image List с мелкими значками.

- LVSIL_STATE Image List с изображениями состояний.
...
Эта функция члена эмулирует функциональность сообщения [LVM_GETINSERTMARK](/windows/win32/Controls/lvm-getinsertmark) , как описано в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_GETINSERTMARKCOLOR](/windows/win32/Controls/lvm-getinsertmarkcolor) , как описано в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_GETINSERTMARKRECT](/windows/win32/Controls/lvm-getinsertmarkrect) , как описано в Windows SDK.
...
Извлекает значение 32-разрядного приложения, связанное с элементом, заданным `nItem`параметром.
...
|Параметр|Описание|
|---------------|-----------------|
|*питеминдекс*|окне Указатель на структуру [лвитеминдекс](/windows/win32/api/commctrl/ns-commctrl-lvitemindex) для родительского элемента подэлемента.<br /><br /> Вызывающий объект отвечает за выделение и задание элементов структуры [лвитеминдекс](/windows/win32/api/commctrl/ns-commctrl-lvitemindex) . Этот параметр не может иметь значение NULL.|
...
|*прект*|заполняет Указатель на структуру [Rect](/previous-versions/dd162897\(v=vs.85\)) , которая получает сведения об ограничивающем прямоугольнике подэлемента.<br /><br /> Вызывающий объект отвечает за выделение структуры [Rect](/previous-versions/dd162897\(v=vs.85\)) . Этот параметр не может иметь значение NULL.|
...
Этот метод отправляет сообщение [LVM_GETITEMINDEXRECT](/windows/win32/Controls/lvm-getitemindexrect) , описанное в Windows SDK. Дополнительные сведения см. в разделе [ListView_GetItemIndexRect Macro](/windows/win32/api/commctrl/nf-commctrl-listview_getitemindexrect).
...
*нкоде*<br/>
Часть элемента представления списка, для которой необходимо получить ограничивающий прямоугольник. Может принимать одно из следующих значений:
...
Этот метод отправляет сообщение [LVM_GETITEMSPACING](/windows/win32/Controls/lvm-getitemspacing) , описанное в Windows SDK.
...
Состояние элемента задается `state` членом структуры [лвитем](/windows/win32/api/commctrl/ns-commctrl-lvitemw) , как описано в Windows SDK. При указании или изменении состояния `stateMask` элемента элемент указывает, какие биты состояния необходимо изменить.
...
|Параметр|Описание|
|---------------|-----------------|
|*питеминдекс*|[вход, выход] Указатель на структуру [лвитеминдекс](/windows/win32/api/commctrl/ns-commctrl-lvitemindex) , описывающую элемент, где начинается поиск, или-1, чтобы найти первый элемент, соответствующий флагам в параметре *нфлагс* .<br /><br /> Если этот метод выполнен успешно, `LVITEMINDEX` структура описывает элемент, найденный в результате поиска.|
|*нфлагс*|окне Побитовое сочетание (или) флагов, определяющих способ выполнения поиска.<br /><br /> Поиск может зависеть от индекса, состояния или внешнего вида целевого элемента или от физической позиции целевого элемента относительно элемента, указанного параметром *питеминдекс* . Дополнительные сведения см. в описании параметра *flags* в сообщении [LVM_GETNEXTITEMINDEX](/windows/win32/controls/lvm-getnextitemindex) .|
...
Этот метод отправляет сообщение [LVM_GETNEXTITEMINDEX](/windows/win32/controls/lvm-getnextitemindex) , описанное в Windows SDK.
...
Можно использовать `GetNextSelectedItem` в цикле прямой итерации, если исходное расположение устанавливается с `GetFirstSelectedItemPosition`вызовом.
...
Эта функция члена эмулирует функциональность сообщения [LVM_GETOUTLINECOLOR](/windows/win32/Controls/lvm-getoutlinecolor) , как описано в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_GETSELECTEDCOLUMN](/windows/win32/Controls/lvm-getselectedcolumn) , как описано в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_GETTILEINFO](/windows/win32/Controls/lvm-gettileinfo) , как описано в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_GETTILEVIEWINFO](/windows/win32/Controls/lvm-gettileviewinfo) , как описано в Windows SDK.
...
Эта функция члена реализует поведение сообщения Win32 [LVM_GETTOOLTIPS](/windows/win32/Controls/lvm-gettooltips), как описано в Windows SDK. Реализация `GetToolTips` MFC`CToolTipCtrl` возвращает объект, который используется элементом управления "список", а не маркером элемента управления ToolTip.
...
Эта функция члена эмулирует функциональность сообщения [LVM_GETVIEW](/windows/win32/Controls/lvm-getview) , как описано в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_HASGROUP](/windows/win32/Controls/lvm-hasgroup) , как описано в Windows SDK.
...
*фиттестинфо*<br/>
`LVHITTESTINFO` Адрес структуры, содержащей точку для проверки попадания и принимающая сведения о результатах проверки попадания.

*pt*<br/>
Проверяемая точка.

*пфлагс*<br/>
Указатель на целое число, получающее сведения о результатах теста. См. Описание `flags` члена структуры [лвхиттестинфо](/windows/win32/api/commctrl/ns-commctrl-lvhittestinfo) в Windows SDK.
...
Можно использовать значения LVHT_ABOVE, LVHT_BELOW, LVHT_TOLEFT и LVHT_TORIGHT элемента структуры `flag` , чтобы определить, следует ли прокручивать содержимое элемента управления "представление списка". Два из этих флагов можно комбинировать, например, если они расположены выше и слева от клиентской области.

Можно проверить значение LVHT_ONITEM элемента структуры `flag` , чтобы определить, находится ли данная позиция над элементом представления списка. Это значение является операцией побитового или для значений LVHT_ONITEMICON, LVHT_ONITEMLABEL и LVHT_ONITEMSTATEICON `flag` элемента структуры.
...
*пколумн*<br/>
`LVCOLUMN` Адрес структуры, содержащей атрибуты нового столбца.
...
Эта функция члена эмулирует функциональность сообщения [LVM_INSERTGROUP](/windows/win32/Controls/lvm-insertgroup) , как описано в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_INSERTGROUPSORTED](/windows/win32/Controls/lvm-insertgroupsorted) , как описано в Windows SDK.
...
Вызов этого метода может привести к тому, что сообщение LVM_INSERTITEM будет отправлено в окно управления. Связанный обработчик сообщений для элемента управления может не задать текст элемента при определенных условиях (например, с помощью стилей окон, таких как LVS_OWNERDRAW). Дополнительные сведения об этих условиях см. в разделе [LVM_INSERTITEM](/windows/win32/Controls/lvm-insertitem) в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_INSERTMARKHITTEST](/windows/win32/Controls/lvm-insertmarkhittest) , как описано в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_ISGROUPVIEWENABLED](/windows/win32/Controls/lvm-isgroupviewenabled) , как описано в Windows SDK.
...
|Параметр|Описание|
|---------------|-----------------|
|*номер*|окне Отсчитываемый от нуля индекс элемента в текущем элементе управления "представление списка".|
...
Этот метод отправляет сообщение [LVM_ISITEMVISIBLE](/windows/win32/Controls/lvm-isitemvisible) , описанное в Windows SDK.
...
|Параметр|Описание|
|---------------|-----------------|
|*id*|окне Уникальный идентификатор элемента.|
...
Этот метод отправляет сообщение [LVM_MAPIDTOINDEX](/windows/win32/controls/lvm-mapidtoindex) , описанное в Windows SDK.
...
|Параметр|Описание|
|---------------|-----------------|
|*номер*|окне Отсчитываемый от нуля индекс элемента.|
...
Этот метод отправляет сообщение [LVM_MAPINDEXTOID](/windows/win32/Controls/lvm-mapindextoid) , описанное в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_MOVEGROUP](/windows/win32/Controls/lvm-movegroup) , как описано в Windows SDK.
...
Этот метод эмулирует функциональность сообщения [LVM_MOVEITEMTOGROUP](/windows/win32/Controls/lvm-moveitemtogroup) , как описано в Windows SDK.
...
Указанные элементы на самом деле не перерисовывается, пока окно представления списка не получит сообщение WM_PAINT. Для немедленного перерисовки вызовите функцию Windows [упдатевиндов](/windows/win32/api/winuser/nf-winuser-updatewindow) после использования этой функции.
...
Эта функция члена эмулирует функциональность сообщения [LVM_REMOVEALLGROUPS](/windows/win32/Controls/lvm-removeallgroups) , как описано в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_REMOVEGROUP](/windows/win32/Controls/lvm-removegroup) , как описано в Windows SDK.
...
*size*<br/>
`CSize` Объект, указывающий величину горизонтальной и вертикальной прокрутки в пикселях. Элемент size делится на высоту (в пикселях) строки элемента управления "список", и элемент управления прокручивается по результирующему количеству строк. `y`
...
*Кредит*<br/>
Цвет фона для установки или значение CLR_NONE без цвета фона. Элементы управления "представление списка" с фоновыми цветами перерисуются значительно быстрее, чем без фоновых цветов. Дополнительные сведения см. в разделе [COLORREF](/windows/win32/gdi/colorref) в Windows SDK.
...
*плвбкимаже*<br/>
`LVBKIMAGE` Адрес структуры, содержащий новые сведения о фоновом изображении.
...
*пколумн*<br/>
Адрес структуры [LVCOLUMN](/windows/win32/api/commctrl/ns-commctrl-lvcolumnw) , которая содержит атрибуты нового столбца, как описано в Windows SDK. `mask` Элемент структуры указывает, какие атрибуты столбца задаются. Если член задает значение LVCF_TEXT, то `pszText` элемент структуры является адресом `cchTextMax` строки, завершающейся нулем, а элемент структуры игнорируется. `mask`
...
*cx*<br/>
Новая ширина столбца. Может быть либо LVSCW_AUTOSIZE, либо LVSCW_AUTOSIZE_USEHEADER, как описано в разделе [LVM_SETCOLUMNWIDTH](/windows/win32/Controls/lvm-setcolumnwidth) в Windows SDK.
...
Этот метод отправляет сообщение [LVM_SETGROUPINFO](/windows/win32/Controls/lvm-setgroupinfo) , описанное в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_SETGROUPMETRICS](/windows/win32/Controls/lvm-setgroupmetrics) , как описано в Windows SDK.
...
Активный курсор, видимый только при включении выделения при наведении, отображается, когда курсор передается над любым элементом представления списка. Выделение при наведении включено путем установки расширенного стиля LVS_EX_TRACKSELECT.
...
*нимажелисттипе*<br/>
Тип списка изображений. Может принимать одно из следующих значений:
...
Эта функция члена эмулирует функциональность сообщения [LVM_SETINFOTIP](/windows/win32/Controls/lvm-setinfotip) , как описано в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_SETINSERTMARK](/windows/win32/Controls/lvm-setinsertmark) , как описано в Windows SDK.
...
`COLORREF` Возвращает структуру, содержащую предыдущий цвет.

### <a name="remarks"></a>Примечания

Эта функция члена эмулирует функциональность сообщения [LVM_SETINSERTMARKCOLOR](/windows/win32/Controls/lvm-setinsertmarkcolor) , как описано в Windows SDK.
...
*питем*<br/>
Адрес структуры [лвитем](/windows/win32/api/commctrl/ns-commctrl-lvitemw) , содержащей новые атрибуты элемента, как описано в Windows SDK. Элементы структуры `iItem` и `iSubItem` определяют элемент или подэлемент `mask` , а член структуры определяет, какие атрибуты задаются. Дополнительные сведения `mask` об элементе см. в разделе **Примечания**.
...
`iItem` Элементы и параметров`iSubItem` Structure и нитем и нсубитем определяют элемент и подэлемент, атрибуты которого должны быть заданы. `LVITEM`

Элемент структуры и параметр нмаск определяют, какие атрибуты элементов должны быть установлены: `mask` `LVITEM`

- LVIF_TEXT элемент или параметр *лпсзитем* является адресом строки `cchTextMax` , завершающейся нулем, элемент игнорируется. `pszText`

- LVIF_STATE. параметр `state` Member или нстатемаск указывает, какие состояния элементов следует изменить, а параметр Member или nсведения содержит значения этих состояний `stateMask` .
...
Эта функция члена реализует поведение макроса Win32, [ListView_SetItemCountEx](/windows/win32/api/commctrl/nf-commctrl-listview_setitemcountex), как описано в разделе сдканд Windows, чтобы вызывать только виртуальные представления списка.
...
Этот метод отправляет сообщение [LVM_SETITEMINDEXSTATE](/windows/win32/Controls/lvm-setitemindexstate) , описанное в Windows SDK.
...
Если элемент управления представления списка имеет стиль LVS_AUTOARRANGE, представление списка упорядочивается после того, как задается позиция элемента.
...
Задает цвет границы элемента управления "представление списка", если установлен расширенный стиль окна [LVS_EX_BORDERSELECT](/windows/win32/Controls/list-view-window-styles) .
...
Эта функция члена эмулирует функциональность сообщения [LVM_SETOUTLINECOLOR](/windows/win32/Controls/lvm-setoutlinecolor) , как описано в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_SETSELECTEDCOLUMN](/windows/win32/Controls/lvm-setselectedcolumn) , как описано в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_SETTILEINFO](/windows/win32/Controls/lvm-settileinfo) , как описано в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_SETTILEVIEWINFO](/windows/win32/Controls/lvm-settileviewinfo) , как описано в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_SETVIEW](/windows/win32/Controls/lvm-setview) , как описано в Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения [LVM_SORTGROUPS](/windows/win32/Controls/lvm-sortgroups) , как описано в Windows SDK.
...
Этот метод отправляет сообщение [LVM_SORTITEMS](/windows/win32/Controls/lvm-sortitems) , описанное в Windows SDK.
...
int CALLBACK CompareFunc(LPARAM lParam1,
    ...
    LPARAM lParamSort);
```
Это сообщение похоже на [LVM_SORTITEMS](/windows/win32/Controls/lvm-sortitems), за исключением типа данных, передаваемых в функцию сравнения. В [LVM_SORTITEMS](/windows/win32/Controls/lvm-sortitems), *lParam1* и *lParam2* — это значения сравниваемых элементов. В [LVM_SORTITEMSEX](/windows/win32/Controls/lvm-sortitemsex) *lParam1* — это текущий индекс первого сравниваемого элемента, а *lParam2* — текущий индекс второго элемента. Вы можете отправить сообщение [LVM_GETITEMTEXT](/windows/win32/Controls/lvm-getitemtext) , чтобы получить дополнительные сведения об элементе.

Функция сравнения должна возвращать отрицательное значение, если первый элемент должен предшествовать второму, положительное значение, если первый элемент должен следовать за вторым, или равняться нулю, если два элемента равны.

> [!NOTE]
>  В процессе сортировки содержимое представления списка нестабильно. Если функция обратного вызова отправляет все сообщения в элемент управления "список", отличный от [LVM_GETITEM](/windows/win32/Controls/lvm-getitem), результаты будут непредсказуемыми.

Этот метод отправляет сообщение [LVM_SORTITEMSEX](/windows/win32/Controls/lvm-sortitemsex) , описанное в Windows SDK.
