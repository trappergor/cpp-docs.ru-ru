...
|[COleServerItem:: Жетембедсаурцедата](#getembedsourcedata)|Возвращает данные CF_EMBEDSOURCE для объекта OLE.|
|[COleServerItem:: Жетитемнаме](#getitemname)|Возвращает имя элемента. Используется только для связанных элементов.|
|[COleServerItem:: Жетлинксаурцедата](#getlinksourcedata)|Возвращает данные CF_LINKSOURCE для объекта OLE.|
|[COleServerItem:: Жетобжектдескриптордата](#getobjectdescriptordata)|Возвращает данные CF_OBJECTDESCRIPTOR для объекта OLE.|
...
Чтобы использовать `COleServerItem`, создайте класс из него и реализуйте функции-члены [OnDraw](#ondraw) и [Serialize](../../mfc/reference/cobject-class.md#serialize) . `OnDraw` Функция предоставляет представление метафайла элемента, что позволяет отображать его при открытии приложением-контейнером составного документа. `Serialize` Функцияпредоставляетсобственноепредставлениеэлемента,позволяяпередаватьвнедренныйэлементмеждусерверным`CObject` и контейнерным приложением. [OnGetExtent](#ongetextent) обеспечивает естественный размер элемента в контейнере, позволяя контейнеру изменять размер элемента.

Дополнительные сведения о серверах и связанных разделах см. в статье [серверы: Реализация сервера](../../mfc/servers-implementing-a-server.md) и "Создание приложения-контейнера или сервера" в контейнерах статьи [: Дополнительные функции](../../mfc/containers-advanced-features.md).
...
`COleServerItem` Создает объект и добавляет его в коллекцию элементов документа серверного документа.
...
Функция-член [онжетклипбоарддата](#ongetclipboarddata) используется для создания объекта [COleDataSource](../../mfc/reference/coledatasource-class.md) , содержащего данные OLE-элемента в поддерживаемых форматах. Затем функция помещает `COleDataSource` объект в буфер обмена с помощью функции [COleDataSource:: сетклипбоард](../../mfc/reference/coledatasource-class.md#setclipboard) . `COleDataSource` Объект включает в себя собственные данные элемента и его представление в формате CF_METAFILEPICT, а также данные любых форматов преобразования, которые вы выбрали для поддержки. Для работы этой функции члена необходимо реализовать [сериализацию](../../mfc/reference/cobject-class.md#serialize) и [OnDraw](#ondraw) .
...
Переопределите эту функцию, если требуется поместить форматы в `COleDataSource` объект до или после этих форматов, `CopyToClipboard`предоставляемых.
...
Если вы хотите, чтобы серверное приложение предлагало данные в различных форматах во время операций обмена данными, зарегистрируйте эти `COleDataSource` форматы с помощью объекта, возвращаемого этой функцией. Например, если вы хотите предоставить CF_TEXT представление объекта OLE для буфера обмена или операции перетаскивания, зарегистрируйте формат с `COleDataSource` объектом, который возвращает эта функция, а затем `OnRenderXxxData` переопределите функцию-член на Укажите данные.

##  <a name="getdocument"></a>COleServerItem:: a Document
...
Это позволяет получить доступ к серверному документу, переданному в качестве `COleServerItem` аргумента в конструктор.
...
*лпстгмедиум*<br/>
Указатель на структуру [стгмедиум](/windows/win32/api/objidl/ns-objidl-ustgmedium~r1) , которая будет принимать данные CF_EMBEDSOURCE для элемента OLE.

### <a name="remarks"></a>Примечания

Этот формат включает в себя собственные данные элемента. Для правильной работы этой функции `Serialize` необходимо реализовать функцию члена.
...
*лпстгмедиум*<br/>
Указатель на структуру [стгмедиум](/windows/win32/api/objidl/ns-objidl-ustgmedium~r1) , которая будет принимать данные CF_LINKSOURCE для элемента OLE.
...
*лпстгмедиум*<br/>
Указатель на структуру [стгмедиум](/windows/win32/api/objidl/ns-objidl-ustgmedium~r1) , которая будет принимать данные CF_OBJECTDESCRIPTOR для элемента OLE.
...
Реализация по умолчанию `OnDraw` вызывает, когда дваспект равен DVASPECT_CONTENT; в противном случае происходит сбой.

Переопределите эту функцию, чтобы предоставить данные представления для аспектов, отличных от DVASPECT_CONTENT, таких как DVASPECT_ICON или DVASPECT_THUMBNAIL.

##  <a name="ongetclipboarddata"></a>COleServerItem:: Онжетклипбоарддата
...
Вызовы `COleServerDoc::OnShowDocument( FALSE )`по умолчанию. Функция также уведомляет контейнер о том, что элемент OLE был скрыт. Переопределите эту функцию, если требуется выполнить специальную обработку при скрытии элемента OLE.
...
Если *лпстгмедиум*-> *тимед* имеет значение TYMED_NULL, стгмедиум должен выделяться и заполняться, как указано в *лпформатетк-> тимед*. Если не TYMED_NULL, СТГМЕДИУМ следует заполнять данными.
...
##  <a name="onsetdata"></a>COleServerItem:: Онсетдата
...
Если приложение-контейнер было написано с Библиотека Microsoft Foundation Class, эта функция вызывается при вызове функции-члена [сетекстент](../../mfc/reference/coleclientitem-class.md#setextent) соответствующего `COleClientItem` объекта. Реализация по умолчанию устанавливает для элемента [m_sizeExtent](#m_sizeextent) указанный размер, если *НДРАВАСПЕКТ* имеет значение DVASPECT_CONTENT; в противном случае возвращается значение 0. Переопределите эту функцию для выполнения особой обработки при изменении размера элемента.
...
Реализация по умолчанию вызывает [упдателинк](../../mfc/reference/coleclientitem-class.md#updatelink) для `COleClientItem` всех объектов в документе.
