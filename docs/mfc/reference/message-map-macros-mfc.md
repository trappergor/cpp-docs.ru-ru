...
В файле реализации (. cpp), который определяет функции-члены для класса, запустите схему сообщений с помощью макроса BEGIN_MESSAGE_MAP, а затем добавьте записи макросов для каждой из функций обработчика сообщений и завершите схему сообщений с помощью END_MESSAGE_MAP. макровирусах.
...
Дополнительные сведения о реализации карт сообщений для классов шаблонов см. в [разделе как Создайте схему сообщений для класса](../how-to-create-a-message-map-for-a-template-class.md)шаблона.
...
> [!NOTE]
>  Если вы объявили любой член после DECLARE_MESSAGE_MAP, для них необходимо указать новый тип доступа (**открытый**, **частный**или **защищенный**).
...
Когда целевой объект Command получает сообщение WM_COMMAND Windows с указанным идентификатором, ON_COMMAND будет вызывать функцию `memberFxn` -член для его обработки.

Используйте ON_COMMAND для преобразования одной команды в функцию-член. Используйте [ON_COMMAND_RANGE](#on_command_range) для преобразования диапазона идентификаторов команд в одну функцию-член. Только одна запись схемы сообщений может соответствовать указанному ИДЕНТИФИКАТОРу команды. То есть нельзя сопоставлять команду более чем с одним обработчиком. Дополнительные сведения и примеры см. в [разделах обработка сообщений и сопоставление](../../mfc/message-handling-and-mapping.md).
...
Расширенная форма обработчиков командных сообщений доступна для расширенных применений. Макрос ON_COMMAND_EX используется для таких обработчиков сообщений и предоставляет надмножество функций [ON_COMMAND](message-map-macros-mfc.md#on_command) . Расширенные функции-члены обработчика команд принимают один параметр, UINT, содержащий идентификатор команды, и возвращают логическое значение. Чтобы указать, что команда была обработана, возвращаемое значение должно быть равно TRUE. в противном случае маршрутизация продолжит другие целевые объекты команды.

Дополнительные сведения см. в техническом примечании [TN006: Схемы сообщений] tm006-Message-maps.md).
...
В схеме сообщений должен содержаться ровно один оператор ON_CONTROL макроопределения для каждого управляющего сообщения уведомления, которое должно быть сопоставлено функции обработчика сообщений.
...
В схеме сообщений должен быть ровно один оператор ON_MESSAGE макро для каждого определяемого пользователем сообщения, которое должно быть сопоставлено функции обработчика сообщений.
...
Маршрутизирует команды через интерфейс `IOleCommandTarget`диспетчеризации команд.
...
Регистервиндовмессаже используется для определения нового сообщения окна, которое гарантированно уникально в пределах всей системы. При наличии класса CWinThread необходимо использовать ON_REGISTERED_THREAD_MESSAGE вместо ON_REGISTERED_MESSAGE.
...
ON_THREAD_MESSAGE необходимо использовать вместо ON_MESSAGE при наличии `CWinThread` класса. Определяемые пользователем сообщения — это любые сообщения, не являющиеся стандартными сообщениями Windows WM_MESSAGE. В схеме сообщений должен быть ровно один оператор ON_THREAD_MESSAGE макро для каждого определяемого пользователем сообщения, которое должно быть сопоставлено функции обработчика сообщений.
...
В схеме сообщений должен содержаться ровно один оператор ON_UPDATE_COMMAND_UI макроопределения для каждой команды обновления пользовательского интерфейса, которая должна быть сопоставлена с функцией обработчика сообщений.
...
Используйте ON_COMMAND_RANGE для преобразования диапазона идентификаторов команд в одну функцию-член. Используйте [ON_COMMAND](#on_command) для преобразования одной команды в функцию-член. Только одна запись схемы сообщений может соответствовать указанному ИДЕНТИФИКАТОРу команды. То есть нельзя сопоставлять команду более чем с одним обработчиком. Дополнительные сведения о сопоставлении диапазонов сообщений см. в разделе [обработчики для диапазонов карт сообщений](../../mfc/handlers-for-message-map-ranges.md).
