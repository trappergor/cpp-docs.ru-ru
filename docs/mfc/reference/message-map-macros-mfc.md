...
### <a name="remarks"></a>Примечания  
 Он указывает, какая функция будет обрабатывать сообщения команды из объекта команды пользовательского интерфейса, например пункта меню или панели инструментов кнопки.  
...
> [!NOTE]
>  Помимо определяемых пользователем сообщений `ON_MESSAGE` обрабатывает сообщения Windows, реже. Дополнительные сведения см. статью базы знаний [99848: INFO: используйте макрос ON_MESSAGE() карты менее типичных сообщений](http://go.microsoft.com/fwlink/?linkId=192022).  
...
### <a name="remarks"></a>Примечания  
 Должен быть ровно один `ON_UPDATE_COMMAND_UI` инструкции макроса на карте сообщения для каждой команды обновления пользовательского интерфейса, который должен быть сопоставлен с функцией обработчик сообщений.  
