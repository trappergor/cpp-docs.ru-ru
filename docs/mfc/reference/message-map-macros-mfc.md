---
title: Макросы сопоставления (MFC) сообщения | Документация Майкрософт
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
Расширенную форму обработчики сообщений команда доступна для сложных вариантов использования. On_command_ex-макрос используется для таких обработчиков сообщений и предоставляет надмножество функциональности [ON_COMMAND] (#on_command).  Функции-члены расширенного обработчика команд принимать один параметр, целое число без знака, содержащее идентификатор команды и возвращают Логическое значение. Возвращаемое значение должно быть значение TRUE для

Этот макрос сопоставляет сообщения команды функцию расширенный элемент обработчика команд.
