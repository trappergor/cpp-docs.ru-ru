...
|[ON_THREAD_MESSAGE](#on_thread_message)|Указывает, какая функция будет обрабатывает определяемое пользователем сообщение при наличии класса `CWinThread`.|
...
В файле реализации (. cpp), который определяет функции-члены для класса, запустите схему сообщений с помощью макроса BEGIN_MESSAGE_MAP, добавьте записи макросов для каждой из функций обработчика сообщений и завершите схему сообщений с помощью END_MESSAGE_MAP макровирусах.
...
Объявляет, что класс определяет схему сообщения. Каждый класс, производный от `CCmdTarget`в программе, должен предоставить схему сообщений для работы с сообщениями.
...
> [!NOTE]
>  Если после DECLARE_MESSAGE_MAP объявляется любой член, для них необходимо указать новый тип доступа (**открытый**, **частный**или **защищенный**).
...
Когда целевой объект Command получает сообщение Windows WM_COMMAND с указанным ИДЕНТИФИКАТОРом, ON_COMMAND вызывает функцию-член, `memberFxn` для ее обработки.
...
Направляет команды через интерфейс диспетчеризации команд `IOleCommandTarget`.
...
`IOleCommandTarget` позволяет контейнеру получать команды, которые происходят в пользовательском интерфейсе DocObject, и позволяет контейнеру отправлять одни и те же команды (например, создать, открыть, сохранить и распечатать в меню файл), а также копировать, вставлять, отменять и т. д. в меню Правка) в DocObject.

`IOleCommandTarget` проще `IDispatch`OLE-автоматизации. `IOleCommandTarget` полностью опирается на стандартный набор команд, которые редко имеют аргументы, и сведения о типе не участвуют (безопасность типов также уменьшается для аргументов команды). Если нужно отправить команды с аргументами, используйте [колесервердок:: онексеколекмд](coleserverdoc-class.md#onexecolecmd).
