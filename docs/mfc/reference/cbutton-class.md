...
|name|Описание|
|----------|-----------------|
|[Кбуттон:: Кбуттон](#cbutton)|Создает объект `CButton`.|

### <a name="public-methods"></a>Открытые методы

|name|Описание|
|----------|-----------------|
|[Кбуттон:: Create](#create)|Создает элемент управления "кнопка Windows" и прикрепляет его `CButton` к объекту.|
|[Кбуттон::D Равитем](#drawitem)|Переопределение для рисования рисуемого `CButton` владельцем объекта.|
...
Обычные кнопки — это флажок, переключатель и кнопка. Объект `CButton` может стать любым из этих элементов в соответствии со [стилем кнопки](../../mfc/reference/styles-used-by-mfc.md#button-styles), заданным в его инициализации функцией [создания](#create) члена.

Кроме того, класс [кбитмапбуттон](../../mfc/reference/cbitmapbutton-class.md) , производный `CButton` от, поддерживает создание элементов управления "Кнопка" с растровыми изображениями вместо текста. `CBitmapButton` Может иметь отдельные точечные рисунки для состояний кнопки "вверх", "вниз", "с сортировкой" и "отключено".

Элемент управления "Кнопка" можно создать либо из шаблона диалогового окна, либо непосредственно в коде. В обоих случаях `CButton` сначала вызовите конструктор для `CButton` создания `Create` объекта. Затем вызовите функцию члена, чтобы создать элемент управления "кнопка `CButton` Windows" и присоединить его к объекту.
...
Создает элемент управления "кнопка Windows" и прикрепляет его `CButton` к объекту.
...
*rect*<br/>
Задает размер и расположение элемента управления "Кнопка". Это может быть либо `CRect` объект, `RECT` либо структура.

*ппарентвнд*<br/>
Задает родительское окно элемента управления "Кнопка", `CDialog`обычно. Оно не должно иметь значение NULL.
...
`CButton` Объект создается в два этапа. Сначала вызовите конструктор, а затем вызовите метод `Create`, который создает элемент управления "кнопка Windows" и прикрепляет его `CButton` к объекту.
...
Для кнопки, рисуемой владельцем, установлен стиль BS_OWNERDRAW. Переопределите эту функцию-член, чтобы реализовать Рисование для рисуемого `CButton` владельцем объекта. Приложение должно восстановить все объекты интерфейса графических устройств (GDI), выбранные для контекста вывода, указанного в *лпдравитемструкт* , перед завершением функции-члена.

См. также значения стилей [BS_](../../mfc/reference/styles-used-by-mfc.md#button-styles) .
...
Возвращает стили кнопок для данного `CButton` объекта. Эта функция возвращает только значения стиля [BS_](../../mfc/reference/styles-used-by-mfc.md#button-styles) , а не другие стили окна.
...
Возвращаемое значение из элемента управления "Кнопка", созданного с помощью стиля BS_AUTOCHECKBOX, BS_AUTORADIOBUTTON, BS_AUTO3STATE, BS_CHECKBOX, BS_RADIOBUTTON или BS_3STATE, имеет одно из следующих значений:
...
Если у кнопки есть другой стиль, возвращаемое значение — BST_UNCHECKED.
...
Эта функция члена эмулирует функциональность сообщения BCM_GETIDEALSIZE, как описано в разделе [кнопки](/windows/win32/controls/buttons) Windows SDK.
...
Эта функция члена эмулирует функциональность сообщения BCM_GETIMAGELIST, как описано в разделе [кнопки](/windows/win32/controls/buttons) Windows SDK.
...
Этот метод следует использовать только с элементами управления, чей стиль кнопки — BS_COMMANDLINK или BS_DEFCOMMANDLINK.

Этот метод отправляет сообщение [BCM_GETNOTE](/windows/win32/Controls/bcm-getnote) , описанное в Windows SDK.
...
Этот метод отправляет сообщение [BCM_GETNOTELENGTH](/windows/win32/Controls/bcm-getnotelength) , описанное в Windows SDK.
...
Этот метод следует использовать только с элементами управления, чей стиль кнопки — BS_SPLITBUTTON или BS_DEFSPLITBUTTON.
...
Этот метод отправляет сообщение [BCM_GETSPLITINFO](/windows/win32/Controls/bcm-getsplitinfo) , описанное в Windows SDK.
...
Побитовое сочетание стилей разворачивающейся кнопки. Дополнительные сведения см. в `uSplitStyle` описании члена структуры [BUTTON_SPLITINFO](/windows/win32/api/commctrl/ns-commctrl-button_splitinfo) .
...
Элемент управления Button с стилем кнопки BS_3STATE или BS_AUTO3STATE создает флажок с третьим состоянием с именем неопределенное состояние. Неопределенное состояние указывает, что флажок не установлен и не снят.
...
Эта функция члена эмулирует функциональность сообщения BCM_GETTEXTMARGIN, как описано в разделе [кнопки](/windows/win32/controls/buttons) Windows SDK.
...
В отличие от [кбитмапбуттон](../../mfc/reference/cbitmapbutton-class.md), которая использует четыре точечных рисунка для `SetBitmap` каждой кнопки, использует только одно растровое изображение для каждой кнопки. При нажатии кнопки точечный рисунок будет перемещен вниз и вправо.
...
Используйте функцию `GetButtonStyle` члена для получения стиля кнопки. Слово полного стиля кнопки "полный" является стилем для определенной кнопки.
...
*nДополнительные*<br/>
Задает состояние проверки. Этот параметр может быть одним из следующих:

|Значение|Значение|
|-----------|-------------|
|BST_UNCHECKED|Установите для кнопки состояние "снят".|
|BST_CHECKED|Установите состояние кнопки "установлен".|
|BST_INDETERMINATE|Задайте для кнопки состояние неопределенное. Это значение можно использовать, только если у кнопки есть стиль BS_3STATE или BS_AUTO3STATE.|
...
В отличие от [кбитмапбуттон](../../mfc/reference/cbitmapbutton-class.md), в котором для каждой кнопки используется четыре `SetCursor` точечных рисунка, в каждой кнопке используется только один курсор. При нажатии кнопки курсор перемещается вниз и вправо.
...
|Параметр|Описание|
|---------------|-----------------|
|*фдропдовн*|окне Значение TRUE, чтобы задать состояние BST_DROPDOWNPUSHED; в противном случае — значение FALSE.|
...
Этот метод отправляет сообщение [BCM_SETDROPDOWNSTATE](/windows/win32/Controls/bcm-setdropdownstate) , описанное в Windows SDK.

### <a name="example"></a>Пример

В следующем примере кода определяется переменная *m_splitButton*, которая используется для программного доступа к элементу управления "разворачивающаяся кнопка". Эта переменная используется в следующем примере.
...
Устанавливает состояние текущего элемента управления "Кнопка" в `elevation required`значение, которое необходимо для отображения значка безопасности с повышенными привилегиями.
...
|Параметр|Описание|
|---------------|-----------------|
|*фелеватионрекуиред*|окне Значение true, `elevation required` чтобы задать состояние; в противном случае — значение false.|
...
В отличие от [кбитмапбуттон](../../mfc/reference/cbitmapbutton-class.md), в котором для каждой кнопки используется четыре `SetIcon` точечных рисунка, в каждой кнопке используется только один значок. При нажатии кнопки значок отображается, чтобы сдвинуться вниз и вправо.
...
Эта функция члена эмулирует функциональность сообщения BCM_SETIMAGELIST, как описано в разделе [кнопки](/windows/win32/controls/buttons) Windows SDK.
...
Этот метод отправляет сообщение [BCM_SETNOTE](/windows/win32/Controls/bcm-setnote) , описанное в Windows SDK.
...
Этот метод инициализирует `mask` член структуры [BUTTON_SPLITINFO](/windows/win32/api/commctrl/ns-commctrl-button_splitinfo) с `himlGlyph` флагом BCSIF_GLYPH и элементом с параметром *чглиф* , а затем отправляет эту структуру в сообщение [BCM_GETSPLITINFO](/windows/win32/Controls/bcm-getsplitinfo) это описано в Windows SDK.
...
Этот метод инициализирует `mask` член структуры [BUTTON_SPLITINFO](/windows/win32/api/commctrl/ns-commctrl-button_splitinfo) с `himlGlyph` флагом BCSIF_IMAGE и элементом с параметром *псплитимажелист* , а затем отправляет эту структуру в [BCM_GETSPLITINFO](/windows/win32/Controls/bcm-getsplitinfo) сообщение, описанное в Windows SDK.
...
Этот метод отправляет сообщение [BCM_SETSPLITINFO](/windows/win32/Controls/bcm-setsplitinfo) , описанное в Windows SDK.
...
Этот метод инициализирует `mask` член структуры [BUTTON_SPLITINFO](/windows/win32/api/commctrl/ns-commctrl-button_splitinfo) с `size` флагом BCSIF_SIZE и элементом с параметром *псизе* , а затем отправляет эту структуру в сообщение [BCM_GETSPLITINFO](/windows/win32/Controls/bcm-getsplitinfo) , которое описывается в Windows SDK.
...
|Параметр|Описание|
|---------------|-----------------|
|*усплитстиле*|окне Побитовое сочетание стилей разворачивающейся кнопки. Дополнительные сведения см. в `uSplitStyle` описании члена структуры [BUTTON_SPLITINFO](/windows/win32/api/commctrl/ns-commctrl-button_splitinfo) .|
...
Стили разворачивающейся кнопки определяют выравнивание, пропорции и графический формат, с помощью которого Windows рисует значок разворачивающейся кнопки. Дополнительные сведения см. в `uSplitStyle` описании члена структуры [BUTTON_SPLITINFO](/windows/win32/api/commctrl/ns-commctrl-button_splitinfo) .

Этот метод инициализирует `mask` член структуры [BUTTON_SPLITINFO](/windows/win32/api/commctrl/ns-commctrl-button_splitinfo) с `uSplitStyle` флагом BCSIF_STYLE и элементом с параметром *усплитстиле* , а затем отправляет эту структуру в [BCM_GETSPLITINFO](/windows/win32/Controls/bcm-getsplitinfo) сообщение, описанное в Windows SDK.
...
В следующем примере кода задается стиль стрелки раскрывающегося списка для кнопки разделения. Стиль BCSS_ALIGNLEFT отображает стрелку в левой части кнопки, а стиль BCSS_STRETCH удерживает пропорции стрелки раскрывающегося списка при изменении размера кнопки.
...
Эта функция члена эмулирует функциональность сообщения BCM_SETTEXTMARGIN, как описано в разделе [кнопки](/windows/win32/controls/buttons) Windows SDK.
