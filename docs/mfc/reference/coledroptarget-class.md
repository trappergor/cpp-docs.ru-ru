...
|name|Описание|
|----------|-----------------|
|[Коледроптаржет:: Коледроптаржет](#coledroptarget)|Создает объект `COleDropTarget`.|
...
Результат, который пользователь выбрал для операции удаления по умолчанию на основе текущего состояния ключа. Это может быть DROPEFFECT_NONE. Эффекты перетаскивания обсуждаются в разделе "Примечания".
