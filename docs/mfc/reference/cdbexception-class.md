...
- **AFX_SQL_ERROR_EMPTY_COLUMN_LIST** предпринята попытка открыть таблицу (или Вы дали невозможно идентифицировать как вызов процедуры или **ВЫБЕРИТЕ** инструкции), но нет столбцов, указанных в обмен полями записей (RFX) вызовы функции вашей `DoFieldExchange` переопределения.  
...
- **AFX_SQL_ERROR_MULTIPLE_ROWS_AFFECTED** Вы вызвали `CRecordset::Update` или `Delete` не уникальный ключ в таблице и изменить несколько записей.  
...
- **AFX_SQL_ERROR_NO_ROWS_AFFECTED** Вы вызвали `CRecordset::Update` или `Delete`, но во время начала операции записи не найден.  
...
-   Текст сообщения об ошибке возвращаются в *szErrorMsg* параметр `SQLError` функции. Это сообщение состоит из нескольких имен в квадратных скобках. При передаче ошибки из источника для пользователя, каждый компонент ODBC (источник данных, драйвер, диспетчер драйверов) добавляет собственное имя. Эта информация помогает выявить источник ошибки. Пример: [Microsoft] [драйвер ODBC для SQL Server] [SQL Server]  
 ...
 Платформа интерпретирует строку ошибки и помещает его компонентов в **m_strStateNativeOrigin**; Если **m_strStateNativeOrigin** содержит сведения для несколько ошибок, разделенных ошибки символы перевода строки. Платформа помещает текст буквенно-цифровые ошибки в **m_strError**.  
