...
|[Кдаотабледеф:: OutAttribute](#getattributes)|Возвращает значение, которое указывает одну или несколько характеристик объекта `CDaoTableDef`.|
|[Кдаотабледеф:: соединение](#getconnect)|Возвращает значение, которое предоставляет сведения об источнике таблицы.|
|[Кдаотабледеф:: Жетдатекреатед](#getdatecreated)|Возвращает дату и время создания базовой таблицы, лежащей в основе объекта `CDaoTableDef`.|
...
|[Кдаотабледеф:: Сетаттрибутес](#setattributes)|Задает значение, которое указывает одну или несколько характеристик объекта `CDaoTableDef`.|
...
Вы управляете определением таблицы с помощью объекта `CDaoTableDef`. Например, администратор может сделать следующее:

- Изучите структуру полей и индексов любой локальной, присоединенной или внешней таблицы в базе данных.

- Вызовите функции членов `SetConnect` и `SetSourceTableName` для присоединенных таблиц и используйте функцию члена `RefreshLink` для обновления соединений с присоединенными таблицами.

- Вызовите функцию члена `CanUpdate`, чтобы определить, можно ли изменять определения полей в таблице.

- Получение или установка условий проверки с помощью `GetValidationRule` и `SetValidationRule`, а также функций членов `GetValidationText` и `SetValidationText`.

- Используйте функцию-член `Open` для создания объекта `CDaoRecordset` типа "Таблица — динамический набор" или "моментальный снимок".
...
1. Во всех случаях сначала создайте объект `CDaoTableDef`, указав указатель на объект [CDaoDatabase](../../mfc/reference/cdaodatabase-class.md) , которому принадлежит таблица.
...
1. Затем выполните следующие действия в зависимости от того, что вам нужно:
   ...
   - Вызовите метод [append](#append) , чтобы сохранить таблицу, добавив ее в коллекцию tabledef базы данных. `Create` помещает объект tabledef в открытое состояние, поэтому после вызова `Create` вы не вызываете `Open`.
...
Чтобы использовать открытый или созданный объект tabledef, создайте и откройте объект `CDaoRecordset`, указав имя объекта tabledef со значением `dbOpenTable` в параметре *нопентипе* .

Чтобы использовать объект tabledef для создания объекта `CDaoRecordset`, обычно создается или открывается объект tabledef, как описано выше, затем создается объект набора записей, передающий указатель на объект tabledef при вызове [CDaoRecordset:: Open](../../mfc/reference/cdaorecordset-class.md#open). Передаваемый объект tabledef должен находиться в открытом состоянии. Дополнительные сведения см. в разделе Class [CDaoRecordset](../../mfc/reference/cdaorecordset-class.md).
...
##  <a name="append"></a>Кдаотабледеф:: Append
...
##  <a name="canupdate"></a>Кдаотабледеф:: Канупдате

Вызовите эту функцию-член, чтобы определить, можно ли изменить определение таблицы, лежащего в основе объекта `CDaoTableDef`.
...
По умолчанию вновь созданную таблицу, которая является базовым объектом `CDaoTableDef`, можно обновить, а присоединенную таблицу базового объекта `CDaoTableDef` обновить нельзя. Объект `CDaoTableDef` может быть обновляемым, даже если результирующий набор записей невозможно обновить.

Связанные сведения см. в разделе "обновляемое свойство" справки DAO.

##  <a name="cdaotabledef"></a>Кдаотабледеф:: Кдаотабледеф
...
После создания объекта необходимо вызвать функцию члена [CREATE](#create) или [Open](#open) . После завершения работы с объектом необходимо вызвать его [закрытую](#close) функцию Member и уничтожить объект `CDaoTableDef`.

##  <a name="close"></a>Кдаотабледеф:: Close
...
Как правило, после вызова `Close`удаляется объект tabledef, если он был выделен с помощью **New**.

После вызова `Close`можно вызвать метод [Open](#open) . Это позволяет повторно использовать объект tabledef.

Дополнительные сведения см. в разделе «метод закрытия» справки DAO.

##  <a name="create"></a>Кдаотабледеф:: Create
...
##  <a name="createfield"></a>Кдаотабледеф:: Креатефиелд
...
Объект `DAOField` (OLE) создается и добавляется к коллекции Fields объекта `DAOTableDef` (OLE). Помимо использования для проверки свойств объекта, можно также использовать `CDaoFieldInfo` для создания входного параметра для создания новых полей в объекте tabledef. Первая версия `CreateField` проще в использовании, но если требуется более точный контроль, можно использовать вторую версию `CreateField`, которая принимает параметр `CDaoFieldInfo`.

При использовании версии `CreateField`, которая принимает параметр `CDaoFieldInfo`, необходимо тщательно задать каждый из следующих элементов структуры `CDaoFieldInfo`:
...
Остальные члены `CDaoFieldInfo` должны иметь значение **0**, false или пустую строку, в зависимости от элемента, или может возникнуть `CDaoException`.

Связанные сведения см. в разделе «метод Креатефиелд» справки DAO.

##  <a name="createindex"></a>Кдаотабледеф:: CreateIndex
...
- `m_strName` должно быть указано имя.
...
Остальные элементы будут пропущены, если задано значение FALSE. Кроме того, во время создания индекса элемент `m_lDistinctCount` игнорируется.

##  <a name="deletefield"></a>Кдаотабледеф::D Елетефиелд
...
##  <a name="deleteindex"></a>Кдаотабледеф::D Елетеиндекс
...
##  <a name="getattributes"></a>Кдаотабледеф:: OutAttribute

Для объекта `CDaoTableDef` возвращаемое значение указывает характеристики таблицы, представленной объектом `CDaoTableDef`, и может быть суммой этих констант:
...
Возвращает значение, которое указывает одну или несколько характеристик объекта `CDaoTableDef`.
...
##  <a name="getconnect"></a>Кдаотабледеф:: соединение
...
Для `CDaoTableDef` объекта, представляющего присоединенную таблицу, объект `CString` состоит из одной или двух частей (спецификатор типа базы данных и путь к базе данных).
...
Если пароль является обязательным, но не указан, драйвер ODBC отображает диалоговое окно входа при первом обращении к таблице и снова, если соединение закрыто и повторно открыто. Если присоединенная таблица имеет атрибут `dbAttachSavePWD`, то при повторном открытии таблицы запрос на вход не отображается.

Дополнительные сведения см. в разделе «свойство Connect» справки DAO.

##  <a name="getdatecreated"></a>Кдаотабледеф:: Жетдатекреатед

Вызовите эту функцию, чтобы определить дату и время создания таблицы, лежащей в основе объекта `CDaoTableDef`.
...
Значение типа, содержащее дату и время создания таблицы, лежащей в основе объекта `CDaoTableDef`.
...
##  <a name="getdatelastupdated"></a>Кдаотабледеф:: Жетдателаступдатед

Вызовите эту функцию, чтобы определить дату и время последнего обновления таблицы, лежащей в `CDaoTableDef` объекте.
...
Значение типа, содержащее дату и время последнего обновления таблицы, лежащей в `CDaoTableDef` объекте.
...
##  <a name="getfieldcount"></a>Кдаотабледеф:: Жетфиелдкаунт
...
##  <a name="getfieldinfo"></a>Кдаотабледеф:: Жетфиелдинфо
...
- Имя, тип, размер, атрибуты `AFX_DAO_PRIMARY_INFO` (по умолчанию). Используйте этот параметр для обеспечения максимальной производительности.
...
- `AFX_DAO_ALL_INFO` первичной и дополнительной информации, плюс: правило проверки, текст проверки, значение по умолчанию
...
##  <a name="getindexcount"></a>Кдаотабледеф:: Жетиндекскаунт
...
##  <a name="getindexinfo"></a>Кдаотабледеф:: Жетиндексинфо
...
- Имя `AFX_DAO_PRIMARY_INFO`, сведения о поле, поля. Используйте этот параметр для обеспечения максимальной производительности.

- `AFX_DAO_SECONDARY_INFO` основную информацию, а также: первичный, уникальный, кластеризованный, пропуск значений NULL, обязательный, внешний
...
##  <a name="getname"></a>Кдаотабледеф:: Name
...
##  <a name="getrecordcount"></a>Кдаотабледеф:: Жетрекордкаунт

Вызовите эту функцию-член, чтобы узнать, сколько записей находится в объекте `CDaoTableDef`.
...
Вызов `GetRecordCount` для `CDaoTableDef` объекта типа "Таблица" отражает приблизительное количество записей в таблице и задействуется немедленно, когда записи таблицы добавляются и удаляются. Откат транзакций будет отображаться как часть счетчика записей, пока не будет вызван метод [кдаоворкспаце:: компактдатабасе](../../mfc/reference/cdaoworkspace-class.md#compactdatabase). Объект `CDaoTableDef` без записей имеет значение свойства счетчик записей, равное 0. При работе с присоединенными таблицами или базами данных ODBC `GetRecordCount` всегда возвращает значение-1.

Связанные сведения см. в разделе «свойство RecordCount» справки DAO.

##  <a name="getsourcetablename"></a>Кдаотабледеф:: Жетсаурцетабленаме
...
Объект `CString`, указывающий имя источника присоединенной таблицы или пустую строку, если собственная таблица данных.
...
##  <a name="getvalidationrule"></a>Кдаотабледеф:: Жетвалидатионруле
...
Объект `CString`, проверяющий данные в поле при изменении или добавлении в таблицу.

### <a name="remarks"></a>Remarks

Правила проверки используются в связи с операциями обновления. Если объект tabledef содержит правило проверки, обновления для этого tabledef должны соответствовать предварительно заданным условиям перед изменением данных. Если изменение не соответствует критериям, выдается исключение, содержащее значение [жетвалидатионтекст](#getvalidationtext) . Для объекта `CDaoTableDef` этот `CString` доступен только для чтения для присоединенной таблицы, а для чтения и записи — для базовой таблицы.

Связанные сведения см. в разделе "свойство ValidationRule" справки DAO.

##  <a name="getvalidationtext"></a>Кдаотабледеф:: Жетвалидатионтекст
...
Объект `CString`, указывающий текст, отображаемый при вводе пользователем данных, не соответствующих правилу проверки.

### <a name="remarks"></a>Remarks

Для объекта `CDaoTableDef` этот `CString` доступен только для чтения для присоединенной таблицы, а для чтения и записи — для базовой таблицы.

Связанные сведения см. в разделе «свойство Валидатионтекст» справки DAO.

##  <a name="isopen"></a>Кдаотабледеф:: OnOpen

Вызовите эту функцию-член, чтобы определить, открыт ли в данный момент объект `CDaoTableDef`.
...
Ненулевое значение, если объект `CDaoTableDef` открыт. в противном случае — 0.

### <a name="remarks"></a>Remarks

##  <a name="m_pdatabase"></a>Кдаотабледеф:: m_pDatabase

Содержит указатель на объект [CDaoDatabase](../../mfc/reference/cdaodatabase-class.md) для этой таблицы.

### <a name="remarks"></a>Remarks

##  <a name="m_pdaotabledef"></a>Кдаотабледеф:: m_pDAOTableDef

Содержит указатель на интерфейс OLE для объекта DAO tabledef, лежащего в основе объекта `CDaoTableDef`.

### <a name="remarks"></a>Remarks

Используйте этот указатель, если требуется прямой доступ к интерфейсу DAO.

##  <a name="open"></a>Кдаотабледеф:: Open
...
##  <a name="refreshlink"></a>Кдаотабледеф:: Рефрешлинк
...
Вы изменяете сведения о соединении для присоединенной таблицы, вызывая [сетконнект](#setconnect) для соответствующего объекта `CDaoTableDef`, а затем используя функцию члена `RefreshLink` для обновления информации. При вызове `RefreshLink`свойства присоединенной таблицы не изменяются.
...
##  <a name="setattributes"></a>Кдаотабледеф:: Сетаттрибутес

Задает значение, которое указывает одну или несколько характеристик объекта `CDaoTableDef`.
...
*латтрибутес*<br/>
Характеристики таблицы, представленной объектом `CDaoTableDef`, и могут быть суммой этих констант:
...
При задании нескольких атрибутов их можно объединить, суммируя соответствующие константы с помощью оператора побитового или. При установке `dbAttachExclusive` в неприсоединенной таблице создается исключение. Объединение следующих значений также приводит к возникновению исключения:
...
##  <a name="setconnect"></a>Кдаотабледеф:: Сетконнект

Для объекта `CDaoTableDef`, представляющего присоединенную таблицу, объект строки состоит из одной или двух частей (спецификатор типа базы данных и путь к базе данных).
...
|-------------------|---------------|----------|
|База данных с помощью ядра СУБД Jet|"[`database`];"|"`drive`:\\\ *путь*\\\ *filename*. ФОРМАТЕ|
...
|Excel 97|"Excel 8,0;"|"`drive`:\\\ *путь*\ *имя файла*. XLS|
|Импорт HTML|"Импорт HTML;"|"`drive`:\\\ *путь*\ *имя файла*"|
|Экспорт HTML|"Экспорт HTML;"|"`drive`:\\\ *путь*"|
|текст|"Text;"|"диск:\\\пас"|
|ODBC|СОЕДИНЕНИЯ БАЗА данных = `database`; UID = *пользователь*; PWD = *пароль*; DSN = *источник данных;* LOGINTIMEOUT = *секунды;* " (Это может быть не полная строка подключения для всех серверов. это лишь пример. Очень важно, чтобы между параметрами не было пробелов.)|None|
|Exchange|Сообщения<br /><br /> МАПИЛЕВЕЛ = *FolderPath*;<br /><br /> [TABLETYPE={ 0 &#124; 1 };]<br /><br /> [Профиль = *профиль*;]<br /><br /> [PWD = *пароль*;]<br /><br /> [База данных = `database`;] "|*"диск*:\\\ *путь*\\\ *filename*. ФОРМАТЕ|

> [!NOTE]
>  Таблица Btrieve больше не поддерживается в DAO 3,5.

В строках подключения необходимо использовать двойную обратную косую черту (\\\\). Если вы изменили свойства существующего соединения с помощью `SetConnect`, необходимо впоследствии вызвать [рефрешлинк](#refreshlink). Если вы инициализируют свойства соединения с помощью `SetConnect`, вам не нужно вызывать `RefreshLink`, но если вы решите это сделать, сначала добавьте объект tabledef.
...
Можно задать строку подключения для `CDaoTableDef` объекта, указав исходный аргумент для `Create` функции-члена. Чтобы определить тип, путь, идентификатор пользователя, пароль или источник данных ODBC для базы данных, можно проверить параметр. Дополнительные сведения см. в документации по конкретному драйверу.

Дополнительные сведения см. в разделе «свойство Connect» справки DAO.

##  <a name="setname"></a>Кдаотабледеф:: SetName
...
##  <a name="setsourcetablename"></a>Кдаотабледеф:: Сетсаурцетабленаме

Вызовите эту функцию-член, чтобы указать имя присоединенной таблицы или имя базовой таблицы, на которой основан объект `CDaoTableDef`, так как он существует в исходном источнике данных.
...
##  <a name="setvalidationrule"></a>Кдаотабледеф:: Сетвалидатионруле
...
Проверка поддерживается только для баз данных, использующих ядро СУБД Microsoft Jet. Выражение не может ссылаться на определяемые пользователем функции, агрегатные функции в домене, агрегатные функции SQL или запросы. Правило проверки для объекта `CDaoTableDef` может ссылаться на несколько полей в этом объекте.
...
##  <a name="setvalidationtext"></a>Кдаотабледеф:: Сетвалидатионтекст

Вызовите эту функцию-член, чтобы задать текст исключения правила проверки для объекта `CDaoTableDef` с базовой таблицей, поддерживаемой ядром СУБД Microsoft Jet.
