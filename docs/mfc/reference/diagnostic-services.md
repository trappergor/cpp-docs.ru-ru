...
### <a name="remarks"></a>Примечания  
 Эта функция используется для отключения отслеживания на фрагменты кода, известно правильно выделения блоков.  
...
### <a name="remarks"></a>Примечания  
 На ней также проверяются от исходного размера, выделенного указанного размера. Если функция возвращает ненулевое значение, порядковый номер выделения возвращается в `plRequestNumber`. Это число представляет порядок, в котором был выделен блок относительно всех других **новый** распределения.  
