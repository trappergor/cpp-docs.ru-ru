...
-   функции диагностики объектов.  
 ...
 Эти макросы и функции доступны для всех классов, производных от `CObject` , в отладочной и окончательной версиях MFC. Однако все, кроме указанных DEBUG_NEW и ПРОВЕРИТЬ ничего не делать в окончательной версии.  
...
|||  
|-|-|  
|[ASSERT](#assert)|Выводит сообщение, а затем прерывает выполнение программы, если указанное выражение принимает значение FALSE в отладочной версии библиотеки.|  
...
|[DEBUG_ONLY](#debug_only)|Аналогично ASSERT, но не проверяет значение выражения; Это полезно для кода, который должен выполняться только в режиме отладки.|  
...
|[AfxCheckError](#afxcheckerror)|Глобальная переменная, используемая для проверки переданных SCODE ли считается ошибкой, и если да, вызывается соответствующая ошибка.|  
...
### <a name="remarks"></a>Примечания  
 ...
 В окончательной версии MFC УТВЕРЖДЕНИЯ не вычисляет выражение и таким образом, не будут прерваны программы. Если выражение должно вычисляться независимо от среды, используйте макрос VERIFY вместо ASSERT.  
...
### <a name="remarks"></a>Примечания  
 Вызовы ASSERT_VALID `AssertValid` функции-члена объекта передана в качестве аргумента.  
  
 В версии MFC ASSERT_VALID не выполняет никаких действий. В отладочной версии проверки указателя, проверяет на равенство значению NULL, а вызывает объекта собственные `AssertValid` функции-члены. Если любой из этих тестов завершается с ошибкой, так же, как отображается предупреждающее сообщение [ASSERT](#assert).  
...
### <a name="remarks"></a>Примечания  
 Используя DEBUG_NEW везде в программе, которые обычно используются **новый** оператор для выделения памяти в куче.  
  
 В режиме отладки (когда **_DEBUG** символ определен), DEBUG_NEW отслеживает отслеживания объекта файла и номер строки для каждого объекта, которому выделяется память. Затем, при использовании [CMemoryState::DumpAllObjectsSince](cmemorystate-structure.md#dumpallobjectssince) функции-члена каждого объекта выделяется с помощью DEBUG_NEW указывается имя файла и номер строки, где было выполнено выделение.  
  
 Чтобы использовать DEBUG_NEW, вставьте следующую директиву в исходные файлы:  
  
 [!code-cpp[NVC_MFCCObjectSample#14](../../mfc/codesnippet/cpp/diagnostic-services_1.cpp)]  
  
 После добавления этой директивы препроцессора будет вставлять DEBUG_NEW везде, где используется **новый**, и MFC делает все остальное. При компиляции окончательной версии программы DEBUG_NEW простой **новый** операции и имя файла и номере строки не создаются.  
  
> [!NOTE]
>  В предыдущих версиях MFC (4.1 и более ранних версий), необходимые для размещения `#define` инструкции после всех инструкций, которые вызваны макросы IMPLEMENT_DYNCREATE или IMPLEMENT_SERIAL. Это больше не требуется.  
...
### <a name="remarks"></a>Примечания  
 Эти макросы предназначена для повышения проверки параметров. Макросы препятствуют дальнейшей обработке неверные параметры в коде. В отличие от макрос ASSERT УБЕДИТЕСЬ, что макросы исключение дополнение к созданию утверждения.  
  
 Макросы работать двумя способами, в зависимости от конфигурации проекта. Макросы вызвать ASSERT и затем создания исключения, если утверждение не выполняется. Таким образом конфигураций отладки (то есть, где определен _DEBUG) макросы обеспечивают утверждение и исключение во время в конфигурациях выпуска, создаваемых макросы только исключение (ASSERT не вычисляет выражение в конфигурацию выпуска).  
  
 Макрос ENSURE_ARG ведет себя как УБЕДИТЕСЬ, что макрос.  
  
 ENSURE_VALID вызывает макрос ASSERT_VALID (который оказывает влияние только в отладочных построениях). Кроме того ENSURE_VALID вызывает исключение, если указатель имеет значение NULL. NULL тест выполняется в конфигурации отладки и выпуска.  
  
 Если любой из этих проверок завершается ошибкой, предупреждающее сообщение отображается так же, как ASSERT. Макрос создает исключение недопустимого аргумента при необходимости.  
### <a name="requirements"></a>Требования  
 **Заголовок:** afx.h  
...
### <a name="remarks"></a>Примечания  
 Информация используется макрос ASSERT и ПРОВЕРИТЬ. Мастер приложения и код мастеров поместите макрос в файлах исходного кода, которые они создают.  
...
### <a name="remarks"></a>Примечания  
 В разделе [ATLTRACE2](../../atl/reference/debugging-and-error-reporting-macros.md#atltrace2) описание ТРАССИРОВКИ. ТРАССИРОВКА и ATLTRACE2 имеют одинаковое поведение.  
...
### <a name="remarks"></a>Примечания  
 `AfxDump` вызывает объект `Dump` функция-член и отправляет сведения в расположение указаны в `afxDump` переменной. `AfxDump` доступен только в отладочной версии MFC.  
...
- `delayFreeMemDF` Задержка при освобождении памяти. Пока приложение освобождает блок памяти, распределителя не возвращает эту память для операционной системы. Это разместите нагрузки максимальный объем памяти в приложении.  
...
- AFX_STACK_DUMP_TARGET_TRACE отправляет выходные данные с помощью параметра [ТРАССИРОВКИ](#trace) макрос. Макрос TRACE создает выходные данные в отладочных построениях. он не создает выходные данные в сборках выпуска. Кроме того ТРАССИРОВКА может быть перенаправлен к другим целевым объектам, помимо отладчик.  
  
- AFX_STACK_DUMP_TARGET_DEFAULT отправляет выходные данные дампа для целевого объекта по умолчанию. Для отладочной сборки результат выводится в макрос TRACE. В сборке выпуска результат выводится в буфер обмена.  
  
- AFX_STACK_DUMP_TARGET_CLIPBOARD отправляет выходные данные только в буфер обмена. Как обычный текст, используя формат буфера обмена CF_TEXT данные помещаются в буфер обмена.  
  
- AFX_STACK_DUMP_TARGET_BOTH отправляет выходные данные в буфер обмена, а макрос TRACE, одновременно.  
...
### <a name="parameters"></a>Параметры  
 [in] *bDump*  
 Значение TRUE указывает, что включена дамп утечки памяти; Значение FALSE указывает, что дамп утечки памяти будет отключена.  
 ...
 *bObject*  
 Значение TRUE, если для выделения `CObject`-производного объекта; в противном случае — значение FALSE.  
  
 *lRequestNumber*  
 Порядковый номер выделения памяти.  
  
 Обратите внимание, что AFXAPI, соглашение о вызовах подразумевает, что вызываемого метода необходимо удалить параметры из стека.  
 ...
 *pContext*  
 Указывает дополнительные данные, которые вызывающий объект может предоставить функцию итерации. Этот указатель может иметь значение NULL.  
