...
|||
|-|-|
|[ASSERT](#assert)|Выводит сообщение, а затем прерывает программу, если указанное выражение принимает значение FALSE в отладочной версии библиотеки.|
...
|[DEBUG_ONLY](#debug_only)|Аналогично ASSERT, но не проверяет значение выражения; полезен для кода, который должен выполняться только в режиме отладки.|
|[ОБЕСПЕЧЕНИЕ и ENSURE_VALID](#ensure)|Используйте для проверки правильности данных.|
...
|[AfxCheckError](#afxcheckerror)|Глобальная переменная, используемая для проверки переданного SCODE, чтобы проверить, является ли она ошибкой, и если да, выдаст соответствующую ошибку.|
...
*classname*<br/>
Имя класса, `CObject`производного от.
...
Параметр *объект* должен быть указателем на объект и может быть константой. Объект, на который указывает, и класс должен `CObject` поддерживать сведения о классе времени выполнения. Например, чтобы убедиться, что `pDocument` является указателем на объект `CMyDoc` класса или любого из его производных, можно выполнить код:

[!code-cpp[NVC_MFCDocView#194](../../mfc/codesnippet/cpp/diagnostic-services_3.cpp)]

`ASSERT_KINDOF` Использование макроса точно так же, как и написание кода:
...
*Объект*<br/>
Указывает объект класса, производного от `CObject` , который имеет переопределяющую версию `AssertValid` функции-члена.

### <a name="remarks"></a>Примечания

ASSERT_VALID вызывает `AssertValid` функцию члена объекта, переданного в качестве аргумента.

В окончательной версии MFC ASSERT_VALID не выполняет никаких действий. В отладочной версии он проверяет указатель, проверяет значение NULL и вызывает собственные `AssertValid` функции члена объекта. Если какой-либо из этих тестов завершается неудачей, предупреждающее сообщение отображается так же, как и [Assert](#assert).
...
В режиме отладки (если определен символ **_DEBUG** ) DEBUG_NEW отслеживает имя файла и номер строки для каждого объекта, который он выделяет. Затем при использовании функции-члена [CMemoryState::D умпаллобжектссинце](cmemorystate-structure.md#dumpallobjectssince) каждый объект, выделенный с помощью DEBUG_NEW, отображается с именем файла и номером строки, где она была выделена.

Чтобы использовать DEBUG_NEW, вставьте в исходные файлы следующую директиву:

[!code-cpp[NVC_MFCCObjectSample#14](../../mfc/codesnippet/cpp/diagnostic-services_1.cpp)]

После вставки этой директивы препроцессор будет вставлять DEBUG_NEW везде, где вы используете **New**, а MFC сделает все остальное. При компиляции окончательной версии программы DEBUG_NEW разрешается в простую **новую** операцию, а сведения о имени файла и номере строки не формируются.

> [!NOTE]
>  В предыдущих версиях MFC (4,1 и более ранних версий) требовалось разместить `#define` инструкцию после всех инструкций, которые вызывали макросы IMPLEMENT_DYNCREATE или IMPLEMENT_SERIAL. Это больше не требуется.
...
В режиме отладки (при определении символа **_DEBUG** ) DEBUG_ONLY оценивает свой аргумент.
...
В сборке выпуска DEBUG_ONLY не вычисляет свой аргумент. Это полезно при наличии кода, который должен выполняться только в отладочных сборках.
...
### <a name="ensure"></a>ОБЕСПЕЧЕНИЕ и ENSURE_VALID
...
Макросы ведут себя двумя способами в соответствии с конфигурацией проекта. Макрос вызывает ASSERT, а затем вызывает исключение, если утверждение не выполняется. Таким же, в конфигурациях отладки (то есть если определен _DEBUG) макросы создают утверждение и исключение в конфигурациях выпуска, макросы создают только исключение (ASSERT не вычисляет выражение в конфигурациях выпуска).
...
`afxDump`— Это предопределенный объект [CDumpContext](../../mfc/reference/cdumpcontext-class.md) , позволяющий передавать `CDumpContext` информацию в окно вывода отладчика или в терминал отладки. Обычно в качестве `CObject::Dump`параметра `afxDump` указывается.
...
`AfxDump`вызывает функцию `Dump` члена объекта и отправляет информацию в расположение, указанное `afxDump` переменной. `AfxDump`доступен только в отладочной версии MFC.

Код программы не должен вызывать метод `AfxDump`, но вместо этого следует `Dump` вызывать функцию члена соответствующего объекта.
...
Если это ошибка, функция создает исключение. Если переданный SCODE имеет значение E_OUTOFMEMORY, функция создает исключение [CMemoryException](../../mfc/reference/cmemoryexception-class.md) , вызывая [афкссровмеморексцептион](exception-processing.md#afxthrowmemoryexception). В противном случае функция выдает исключение [COleException](../../mfc/reference/coleexception-class.md) , вызывая [афкссроволиксцептион](exception-processing.md#afxthrowoleexception).
...
|`+ 181 bytes`|Смещение в байтах от адреса прототипа функции (в данном случае `void AfxDumpStack(unsigned long)`) к возвращаемому адресу (в данном `00427D55`случае).|
...
- Файл IMAGEHLP. DLL должна быть указана в пути. Если эта библиотека DLL отсутствует, функция отобразит сообщение об ошибке. Сведения о наборе функций, предоставляемых IMAGEHLP, см. в разделе [Библиотека справки](/windows/win32/Debug/image-help-library) по изображениям.
...
*бобжект*<br/>
Значение true, если выделение предназначено `CObject`для производного объекта; в противном случае — значение false.
...
*PFN*<br/>
Указывает на функцию итерации для выполнения для каждого объекта. Аргументы функции являются указателем `CObject` на и пустым указателем на дополнительные данные, которые вызывающий объект передает в функцию.
