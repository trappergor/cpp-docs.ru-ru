...
Кроме того, для проверки соответствия между TYPEDESC и данным идентификатором GUID предоставляется глобальная функция `AfxOleTypeMatchGuid`.

##  <a name="px_blob"></a>PX_Blob

Вызовите эту функцию в функции-члене `DoPropExchange` элемента управления, чтобы сериализовать или инициализировать свойство, которое хранит данные больших двоичных объектов (BLOB).
...
*ппкс*<br/>
Указатель на объект [кпропексчанже](../../mfc/reference/cpropexchange-class.md) (обычно передается в качестве параметра `DoPropExchange`).
...
Обратите внимание, что `PX_Blob` выделит память с помощью API Windows [GlobalAlloc](/windows/win32/api/winbase/nf-winbase-globalalloc) при загрузке свойств типа больших двоичных объектов. Вы несете ответственность за освобождение этой памяти. Таким образом, деструктор элемента управления должен вызывать [GlobalFree](/windows/win32/api/winbase/nf-winbase-globalfree) для всех дескрипторов свойств типа больших двоичных объектов, чтобы освободить память, выделенную для элемента управления.

##  <a name="px_bool"></a>PX_Bool

Вызовите эту функцию в функции-члене `DoPropExchange` элемента управления для сериализации или инициализации свойства типа BOOL.
...
Вызовите эту функцию в функции-члене `DoPropExchange` элемента управления для сериализации или инициализации свойства типа OLE_COLOR.
...
Вызывайте эту функцию в функции-члене `DoPropExchange` элемента управления для сериализации или инициализации свойства типа **Currency**.
...
Вызовите эту функцию в функции-члене `DoPropExchange` элемента управления для сериализации или инициализации свойства пути к данным типа [кдатапаспроперти](../../mfc/reference/cdatapathproperty-class.md).
...
Вызовите эту функцию в функции-члене `DoPropExchange` элемента управления для сериализации или инициализации свойства типа **Double**.
...
Вызовите эту функцию в функции-члене `DoPropExchange` элемента управления для сериализации или инициализации свойства типа Font.
...
*шрифтов*<br/>
Ссылка на объект `CFontHolder`, содержащий свойство Font.

*пфонтдеск*<br/>
Указатель на структуру `FONTDESC`, содержащую значения, используемые при инициализации состояния по умолчанию свойства Font, в случае если *пфонтдиспамбиент* имеет значение null.

*пфонтдиспамбиент*<br/>
Указатель на интерфейс `IFontDisp` шрифта, используемый при инициализации состояния по умолчанию для свойства Font.
...
Значение свойства считывается или записывается в `font`, при необходимости ссылка на `CFontHolder`. Если указаны *пфонтдеск* и *пфонтдиспамбиент* , они используются для инициализации значения свойства по умолчанию при необходимости. Эти значения используются, если по какой либо причине происходит сбой процесса сериализации элемента управления. Как правило, для *пфонтдеск* передается значение NULL и значение окружения, возвращаемое `COleControl::AmbientFont` для *пфонтдиспамбиент*. Обратите внимание, что объект Font, возвращаемый `COleControl::AmbientFont`, должен быть освобожден вызовом функции-члена `IFontDisp::Release`.

##  <a name="px_float"></a>PX_Float

Вызовите эту функцию в функции-члене `DoPropExchange` элемента управления для сериализации или инициализации свойства типа **float**.
...
Вызывайте эту функцию в функции-члене `DoPropExchange` элемента управления для сериализации или инициализации свойства, представленного объектом, имеющего производный от `IUnknown`интерфейс.
...
Вызовите эту функцию в функции-члене `DoPropExchange` элемента управления для сериализации или инициализации свойства типа **Long**.
...
Вызовите эту функцию в функции-члене `DoPropExchange` элемента управления для сериализации или инициализации свойства изображения элемента управления.
...
Вызовите эту функцию в функции-члене `DoPropExchange` элемента управления для сериализации или инициализации свойства типа **Short**.
...
Вызовите эту функцию в функции-члене `DoPropExchange` элемента управления для сериализации или инициализации свойства типа **ulong**.
...
Вызовите эту функцию в функции-члене `DoPropExchange` элемента управления, чтобы сериализовать или инициализировать свойство типа **без знака Short**.
...
Вызовите эту функцию в функции-члене `DoPropExchange` элемента управления для сериализации или инициализации свойства символьной строки.
...
Вызывайте эту функцию в функции-члене `DoPropExchange` элемента управления для инициализации свойства Font путем преобразования свойств, связанных со шрифтами элемента управления VBX.
...
Эта функция должна использоваться только элементом управления OLE, который предназначен для непосредственной замены элемента управления VBX. Когда среда разработки Visual Basic преобразует форму, содержащую элемент управления VBX, для использования соответствующего заменяющего элемента управления OLE, он вызывает функцию `IDataObject::SetData` элемента управления, передавая набор свойств, содержащий данные свойства элемента управления VBX. Эта операция, в свою очередь, вызывает вызов функции `DoPropExchange` элемента управления. `DoPropExchange` может вызвать `PX_VBXFontConvert` для преобразования свойств шрифта элемента управления VBX (например, "FontName", "FontSize" и т. д.) в соответствующие компоненты свойства Font элемента управления OLE.

`PX_VBXFontConvert` следует вызывать только в том случае, если элемент управления фактически преобразуется из приложения формы VBX. Пример:
