...
## <a name="remarks"></a>Примечания  
 ...
 Когда извлекается меню, платформа ищет и вызывает каждый `ON_UPDATE_COMMAND_UI` обработчик, вызывает каждый обработчик `CCmdUI` члена функции, такие как `Enable` и `Check`, и платформа затем соответствующим образом отображает каждый элемент меню.  
