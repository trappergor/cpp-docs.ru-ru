...
|[Поддержка CCmdUI:: Сетрадио](#setradio)|Как и функция члена, но работает с группами переключателей. `SetCheck`|
...
|[CCmdUI::m_pSubMenu](#m_psubmenu)|Указывает на вложенное меню, представленное `CCmdUI` объектом.|
...
Когда меню извлекается, платформа ищет и вызывает каждый обработчик ON_UPDATE_COMMAND_UI, каждый обработчик вызывает `CCmdUI` функции-члены, такие как `Enable` и `Check`, а платформа соответствующим образом отображает каждый пункт меню.
...
|Элемент пользовательского интерфейса|Разрешить|сетчекк|сетрадио|SetText|
|--------------------------|------------|--------------|--------------|-------------|
|Элемент меню|Включает или отключает|Проверяет или снимает|Проверяет с помощью точки|Задает текст элемента|
|Кнопка панели инструментов|Включает или отключает|Выборки, отменить выборки или неопределенные|Аналогично `SetCheck`|(Неприменимо)|
|Панель состояния|Делает текст видимым или невидимым|Задает выступающее или нормальную границу|Аналогично `SetCheck`|Задает текст панели|
|Обычная кнопка в`CDialogBar`|Включает или отключает|Флажок "устанавливает или снимает"|Аналогично `SetCheck`|Задает текст кнопки|
...
Значение null, если элемент является меню или не `CWnd` является объектом.
...
Эта функция-член действует `SetCheck`аналогично, за исключением того, что она работает с элементами пользовательского интерфейса, работающими как часть группы переключателей. Извлечение других элементов в группе выполняется автоматически, если только сами элементы не поддерживают поведение группы.
