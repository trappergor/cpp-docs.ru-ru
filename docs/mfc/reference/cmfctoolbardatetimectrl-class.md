...
### <a name="return-value"></a>Возвращаемое значение  
 `TRUE` Если кнопки обрабатывает сообщения WM_COMMAND или `FALSE` для указания обработки сообщения с панели инструментов родительского.  
