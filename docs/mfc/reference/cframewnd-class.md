...
|[CFrameWnd:: Create](#create)|Вызовите метод, чтобы создать и инициализировать окно фрейма Windows `CFrameWnd` , связанное с объектом.|
|[CFrameWnd:: CreateView](#createview)|Создает представление внутри фрейма, который не является производным `CView`от.|
...
|[CFrameWnd:: Ендмодалстате](#endmodalstate)|Завершает модальное состояние окна фрейма. Включает все окна, отключенные с `BeginModalState`помощью.|
...
|[CFrameWnd:: Инитиалупдатефраме](#initialupdateframe)|Приводит к вызову функции члена,принадлежащейвсемпредставлениямвокнефрейма.`OnInitialUpdate`|
...
|[CFrameWnd:: RecalcLayout](#recalclayout)|Перемещает панели `CFrameWnd` элементов управления объекта.|
...
|[CFrameWnd:: Ректдефаулт](#rectdefault)|Передайте этот `CRect` статический параметр в качестве параметра при `CFrameWnd` создании объекта, чтобы разрешить Windows выбирать исходный размер и расположение окна.|

## <a name="remarks"></a>Примечания

Чтобы создать полезное окно фрейма для приложения, создайте класс, производный `CFrameWnd`от. Добавьте переменные члена в производный класс для хранения данных, относящихся к вашему приложению. Реализуйте в производном классе функции-члены обработчика сообщений и схему сообщений, чтобы указать, что происходит, когда сообщения направляются в окно.
...
Перед вызовом `Create` `LoadFrame`или необходимо создать объект окна фрейма в куче с помощью C++ оператора **New** . Перед вызовом `Create`можно также зарегистрировать класс окна с глобальной функцией [афксрегистервндкласс](../../mfc/reference/application-information-and-management.md#afxregisterwndclass) , чтобы задать стили значков и классов для фрейма.
...
- Окно `CFrameWnd` фрейма отслеживает текущее активное представление, которое не зависит от активного окна Windows или текущего фокуса ввода. При повторной активации кадра активное представление уведомляется путем вызова `CView::OnActivateView`.

- Сообщения команд и многие распространенные сообщения с уведомлениями о кадрах, включая `OnSetFocus`те `OnHScroll`, которые `OnVScroll` обрабатываются `CFrameWnd` функциями `CWnd`, и, делегируются окном фрейма в текущее активное представление.
...
- Окно `CFrameWnd` фрейма содержит необязательную таблицу сочетаний клавиш, которая автоматически преобразует сочетания клавиш.

- Окно фрейма содержит необязательный идентификатор справки, `LoadFrame` заданный с помощью, который используется для контекстной справки. `CFrameWnd` Окно фрейма — это основная Orchestrator семимодал состояний, таких как контекстно-зависимая справка (SHIFT + F1) и режимы предварительного просмотра.
...
Дополнительные сведения о см `CFrameWnd`. в разделе [окна](../../mfc/frame-windows.md)с рамками.
...
`CFrameWnd` Создает объект, но не создает видимое окно фрейма.
...
Вызовите метод, чтобы создать и инициализировать окно фрейма Windows `CFrameWnd` , связанное с объектом.
...
*лпсзкласснаме*<br/>
Указывает на строку символов, завершающуюся нулем, которая именует класс Windows. Именем класса может быть любое имя, `AfxRegisterWndClass` зарегистрированное в глобальной функции `RegisterClass` или функции Windows. Если значение равно null, использует стандартные `CFrameWnd` атрибуты по умолчанию.
...
`CFrameWnd` Создайте объект в два этапа. Сначала вызовите конструктор, который `CFrameWnd` создает объект, а затем вызовите `Create`, который создаст окно фрейма Windows и `CFrameWnd` присоединяет его к объекту. `Create`Инициализирует имя класса и имя окна окна и регистрирует значения по умолчанию для соответствующего стиля, родительского и связанного меню.

`LoadFrame` Используйте`Create` вместо для загрузки окна фрейма из ресурса вместо указания его аргументов.
...
Используйте эту функцию-член для создания представлений, которые не `CView`являются производными внутри рамки. После вызова `CreateView`необходимо вручную задать для представления значение активно и сделать его видимым; эти задачи не `CreateView`выполняются автоматически.
...
- AFX_IDW_DOCKBAR_TOP закрепляется на верхней стороне окна фрейма.

- AFX_IDW_DOCKBAR_BOTTOM закрепление в нижней части окна фрейма.
...
- CBRS_ALIGN_TOP позволяет закреплять в верхней части клиентской области.

- CBRS_ALIGN_BOTTOM позволяет закреплять в нижней части клиентской области.

- CBRS_ALIGN_LEFT позволяет закреплять в левой части клиентской области.

- CBRS_ALIGN_RIGHT позволяет закреплять в правой части клиентской области.

- CBRS_ALIGN_ANY позволяет закреплять на любой стороне клиентской области.
...
Вызовите эту функцию-член для получения указателя на текущий `CDocument` объект, присоединенный к текущему активному представлению.
...
Вызовите эту функцию-член, чтобы получить указатель на активное представление (если таковое имеется), присоединенное `CFrameWnd`к окну фрейма ().
...
Это приводит к тому, что все представления в этом окне `OnInitialUpdate` фрейма получат свои вызовы.
...
`CFrameWnd` Создайте объект в два этапа. Сначала следует вызвать конструктор, который создает `CFrameWnd` объект, а затем вызвать метод `LoadFrame`, который загружает окно фрейма Windows и связанные ресурсы и прикрепляет окно фрейма к `CFrameWnd` объекту. Параметр *нидресаурце* указывает меню, таблицу сочетаний клавиш, значок и строковый ресурс заголовка окна фрейма.

Используйте функцию `Create` члена, `LoadFrame` а не в том случае, если необходимо указать все параметры создания окна фрейма.

Платформа вызывает `LoadFrame` , когда создает фрейм окна с помощью объекта шаблона документа.

Платформа использует аргумент *пконтекст* для указания объектов, которые должны быть подключены к окну фрейма, включая все содержащиеся в нем объекты представления. При вызове `LoadFrame`аргумент *пконтекст* можно установить в значение null.

##  <a name="m_bautomenuenable"></a>CFrameWnd:: m_bAutoMenuEnable

Если этот элемент данных включен (по умолчанию), пункты меню, не имеющие обработчиков ON_UPDATE_COMMAND_UI или ON_COMMAND, будут автоматически отключены, когда пользователь выберет меню.
...
Элементы меню, имеющие обработчик ON_COMMAND, но без обработчика ON_UPDATE_COMMAND_UI, будут включены автоматически.
...
Этот элемент данных упрощает реализацию необязательных команд на основе текущего выбора и сокращает потребность в написании обработчиков ON_UPDATE_COMMAND_UI для включения и отключения пунктов меню.
...
к схеме сообщений `CFrameWnd` класса, а также добавьте запись ускорителя, обычно SHIFT + F1, чтобы включить эту функцию-член.

Если приложение является контейнером OLE, помещает все элементы на месте, содержащиеся в объекте окна фрейма, `OnContextHelp` в режим справки. Курсор превращается в стрелку и вопросительный знак, и пользователь может переместить указатель мыши и нажать левую кнопку мыши, чтобы выбрать диалоговое окно, окно, меню или кнопку команды. Эта функция члена вызывает функцию `WinHelp` Windows с контекстом справки объекта под курсором.
...
Переопределите эту функцию, чтобы переопределить значения, `CCreateContext` переданные в объекте, или изменить способ создания элементов управления в основной клиентской области окна фрейма. Члены, которые можно переопределить, описаны в классе [ккреатеконтекст.](../../mfc/reference/ccreatecontext-structure.md) `CCreateContext`

> [!NOTE]
>  Не заменяйте значения, `CREATESTRUCT` переданные в структуре. Они предназначены только для использования в информационных целях. Если необходимо переопределить исходный прямоугольник окна, например переопределить `CWnd` функцию члена [PreCreateWindow](../../mfc/reference/cwnd-class.md#precreatewindow).
...
*пкмдуи*<br/>
Указатель на объект [Поддержка CCmdUI](../../mfc/reference/ccmdui-class.md) , представляющий меню, вызвавшее команду обновления. Обработчик обновлений вызывает функцию `CCmdUI` -член [Enable](../../mfc/reference/ccmdui-class.md#enable) объекта через *пкмдуи* для обновления пользовательского интерфейса.
...
Реализация по умолчанию этой функции-члена вызывает `CWnd` функцию `RepositionBars` -член для изменения расположения всех панелей элементов управления в кадре, а также в основном клиентском окне (обычно `CView` это или мдиклиент).
...
Передайте этот `CRect` статический параметр в качестве параметра при создании окна, позволяющего Windows выбрать исходный размер и расположение окна.
...
*бнотифи*<br/>
Указывает, должно ли представление получать уведомления об активации. Если значение равно `OnActivateView` true, вызывается для нового представления; значение false, если нет.
...
Вызовите эту функцию-член, чтобы применить сведения о `CDockState` состоянии, хранящиеся в объекте, к панелям управления окна фрейма.
...
*нпрогресспос*<br/>
Задает устанавливаемое расположение. Он должен находиться в диапазоне, заданном `SetProgressBarRange`параметром.
