...
По умолчанию READYSTATE_UNINITIALIZED состояние инициализации

В настоящее время загрузки элемента управления READYSTATE_LOADING его свойства

READYSTATE_LOADED управления был инициализирован

Элемент управления READYSTATE_INTERACTIVE имеет достаточно данных, чтобы быть интерактивным, но не только асинхронные данных еще загружается.
...
POINTERINACTIVE_ACTIVATEONENTRY объекта должно быть активировано, когда указатель мыши перемещается на его во время операции перемещения указателя мыши на месте.

Операции перемещения POINTERINACTIVE_DEACTIVATEONLEAVE, объект следует деактивировать, когда мышь покидает объекта во время мыши.
...
Объект VIEWSTATUS_OPAQUE полностью непрозрачен. Если этот бит не задано, объект содержит прозрачные части. Этот бит применяется только к аспекты, связанные с содержимым, а не к DVASPECT_ICON или DVASPECT_DOCPRINT.

Объект VIEWSTATUS_SOLIDBKGND имеет непрозрачный фон (состоящему в сплошным цветом, а не узором). Этот бит имеет смысл только в том случае, если VIEWSTATUS_OPAQUE устанавливается и применяется только к аспекты, связанные с содержимым, а не к DVASPECT_ICON или DVASPECT_DOCPRINT.

Объект VIEWSTATUS_DVASPECTOPAQUE поддерживает DVASPECT_OPAQUE. Все методы IViewObjectEx, которые принимают вид рисования, как параметр может быть вызван этот аспект.

Объект VIEWSTATUS_DVASPECTTRANSPARENT поддерживает DVASPECT_TRANSPARENT. Все `IViewObjectEx` методы, которые принимают вид рисования, как параметр может быть вызван этот аспект.
