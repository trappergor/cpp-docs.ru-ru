...
Платформа передает указатель на этот объект `CPropExchange` в функцию `DoPropExchange` элемента управления. Если вы использовали мастер для создания начальных файлов для элемента управления, функция `DoPropExchange` элемента управления вызывает `COleControl::DoPropExchange`. Версия базового класса обменивается свойствами запасов элемента управления. Вы изменяете версию производного класса на свойства Exchange, добавленные в элемент управления.

`CPropExchange` можно использовать для сериализации свойств элемента управления или инициализации свойств элемента управления при загрузке или создании элемента управления. Функции-члены `ExchangeProp` и `ExchangeFontProp` `CPropExchange` могут хранить свойства и загружать их с разных носителей.
...
*пфонтдиспамбиент*<br/>
Указатель на интерфейс `IFontDisp` шрифта, используемый для инициализации состояния по умолчанию для свойства Font.
...
Если свойство Font загружается с носителя на элемент управления, характеристики шрифта извлекаются из среднего, а объект `CFontHolder`, на который ссылается *Font* , инициализируется с ними. Если свойство Font хранится, характеристики в объекте Font записываются на носитель.
...
*ппунк*<br/>
Указатель на переменную, содержащую указатель на интерфейс `IUnknown` свойства (эта переменная обычно является членом класса).
