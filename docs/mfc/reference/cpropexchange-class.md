...
|[CPropExchange::ExchangePersistentProp](#exchangepersistentprop)|Обменивает свойство между элементом управления и файлом.|
|[CPropExchange::ExchangeProp](#exchangeprop)|Обменивается свойствами любого встроенного типа.|
...
Платформа передает указатель на этот `CPropExchange` объект в `DoPropExchange` функцию элемента управления. Если для создания начальных файлов для элемента управления использовался мастер, вызывается `DoPropExchange` `COleControl::DoPropExchange`функция элемента управления. Версия базового класса обменивается свойствами запасов элемента управления. Вы изменяете версию производного класса на свойства Exchange, добавленные в элемент управления.

`CPropExchange`может использоваться для сериализации свойств элемента управления или инициализации свойств элемента управления при загрузке или создании элемента управления. `ExchangeFontProp` Функции `ExchangeProp` -членыимогутсохранятьсвойстваизагружатьихсразныхносителей.`CPropExchange`

Дополнительные сведения об использовании `CPropExchange`см. в статье [элементы управления ActiveX в MFC. Страницы](../../mfc/mfc-activex-controls-property-pages.md)свойств.
...
##  <a name="exchangeblobprop"></a>Кпропексчанже:: Ексчанжеблобпроп
...
Функции `CArchivePropExchange::ExchangeBlobProp`, `CResetPropExchange::ExchangeBlobProp`и переопределяютэтучистовиртуальнуюфункцию.`CPropsetPropExchange::ExchangeBlobProp`

##  <a name="exchangefontprop"></a>Кпропексчанже:: Ексчанжефонтпроп
...
Функции `CArchivePropExchange::ExchangeFontProp`, `CResetPropExchange::ExchangeFontProp`и переопределяютэтучистовиртуальнуюфункцию.`CPropsetPropExchange::ExchangeFontProp`

##  <a name="exchangepersistentprop"></a>Кпропексчанже:: Ексчанжеперсистентпроп
...
Функции `CArchivePropExchange::ExchangePersistentProp`, `CResetPropExchange::ExchangePersistentProp`и переопределяютэтучистовиртуальнуюфункцию.`CPropsetPropExchange::ExchangePersistentProp`

##  <a name="exchangeprop"></a>Кпропексчанже:: Ексчанжепроп
...
*втпроп*<br/>
Символ, указывающий тип свойства, для которого выполняется обмен. Доступны следующие значения:
...
Функции `CArchivePropExchange::ExchangeProp`, `CResetPropExchange::ExchangeProp`и переопределяютэтучистовиртуальнуюфункцию.`CPropsetPropExchange::ExchangeProp`

##  <a name="exchangeversion"></a>Кпропексчанже:: Ексчанжеверсион
...
##  <a name="getversion"></a>Кпропексчанже::/версия
...
##  <a name="isasynchronous"></a>Кпропексчанже:: в асинхронном режиме
...
##  <a name="isloading"></a>Кпропексчанже:: Load
