...
|[CListBox:: Чартоитем](#chartoitem)|Переопределение для предоставления настраиваемой обработки WM_CHAR для списков, рисуемых владельцем, которые не имеют строк.|
...
|[CListBox:: Вкэйтоитем](#vkeytoitem)|Переопределение для предоставления настраиваемой обработки WM_KEYDOWN для списков с набором стилей LBS_WANTKEYBOARDINPUT.|
...
Список можно создать либо из шаблона диалогового окна, либо непосредственно в коде. Чтобы создать его напрямую, `CListBox` создайте объект, а затем вызовите функцию [создания](#create) члена, чтобы создать элемент управления списка Windows и `CListBox` присоединить его к объекту. Чтобы использовать список в шаблоне диалогового окна, объявите переменную списка в классе диалогового окна, а затем используйте `DDX_Control` в `DoDataExchange` функции класса диалогового окна, чтобы подключить переменную-член к элементу управления. (это делается автоматически при добавлении управляющей переменной в класс диалогового окна.)

Построение может быть одношаговым процессом в классе, производном от `CListBox`. Напишите конструктор для производного класса и вызовите `Create` его в конструкторе.
...
где `id` указывает идентификатор дочернего окна элемента управления "список", отправляющего уведомление `memberFxn` , и является именем родительской функции-члена, написанной для работы с уведомлением.
...
- ON_LBN_DBLCLK. пользователь дважды щелкает строку в поле со списком. Только список, имеющий стиль [LBS_NOTIFY](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) , будет отсылать это сообщение уведомления.

- ON_LBN_ERRSPACE. окно списка не может выделить достаточно памяти для удовлетворения запроса.

- ON_LBN_KILLFOCUS. поле со списком теряет фокус ввода.

- ON_LBN_SELCANCEL. Выбор текущего списка отменен. Это сообщение отправляется только в том случае, если у окна списка есть стиль LBS_NOTIFY.

- ON_LBN_SELCHANGE выбор в списке изменился. Это уведомление не отправляется, если выбор изменяется функцией-членом [CListBox:: сеткурсел](#setcursel) . Это уведомление относится только к списку, имеющему стиль LBS_NOTIFY. Сообщение уведомления LBN_SELCHANGE отправляется для списка с множественным выбором каждый раз, когда пользователь нажимает клавишу со стрелкой, даже если выделение не изменяется.

- ON_LBN_SETFOCUS. окно списка получает фокус ввода.

- ON_WM_CHARTOITEM. список, рисуемый владельцем, не содержит строк, получает сообщение WM_CHAR.

- ON_WM_VKEYTOITEM. список с LBS_WANTKEYBOARDINPUT стилем получает сообщение WM_KEYDOWN.
...
При создании `CListBox` объекта в окне может потребоваться `CListBox` уничтожить объект. При создании `CListBox` объекта в стеке он уничтожается автоматически. При создании `CListBox` объекта в куче с помощью **новой** функции необходимо вызвать метод **Delete** для объекта, чтобы уничтожить его, когда пользователь закроет родительское окно.

При выделении памяти в `CListBox` объекте `CListBox` Переопределите деструктор, чтобы удалить выделение.
...
Отсчитываемый от нуля индекс строки в списке. Если возникает ошибка, возвращается значение LB_ERR. Если недостаточно места для хранения новой строки, возвращается значение LB_ERRSPACE.

### <a name="remarks"></a>Примечания

Если список не был создан с использованием стиля [LBS_SORT](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) , строка добавляется в конец списка. В противном случае строка вставляется в список, а список сортируется. Если список был создан с использованием стиля LBS_SORT, но не [LBS_HASSTRINGS](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) стиля, платформа сортирует список по одному или нескольким вызовам `CompareItem` функции члена.
...
Вызывается структурой, когда родительское окно списка получает сообщение WM_CHARTOITEM из списка.
...
Сообщение WM_CHARTOITEM отправляется списком при получении сообщения WM_CHAR, но только в том случае, если список удовлетворяет всем указанным условиям:

- Является списком, рисуемым владельцем.

- Не задан стиль [LBS_HASSTRINGS](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) .
...
`CListBox` Объект создается в два этапа. Сначала вызовите конструктор `ClistBox` , а затем вызовите метод `Create`, который инициализирует список окон и `CListBox`присоединяет его к.
...
Описание`COMPAREITEMSTRUCT` структуры см. в разделе [CWnd:: онкомпареитем](../../mfc/reference/cwnd-class.md#oncompareitem) .

### <a name="remarks"></a>Примечания

По умолчанию эта функция члена не выполняет никаких действий. При создании списка, рисуемого владельцем, с использованием стиля LBS_SORT необходимо переопределить эту функцию-член, чтобы помочь платформе в сортировке новых элементов, добавленных в список.
...
*rect*<br/>
Задает размер и расположение окна списка. Может быть либо `CRect` объектом, `RECT` либо структурой.
...
`CListBox` Объект создается в два этапа. Сначала вызовите конструктор, а затем вызовите метод `Create`, который инициализирует список окон и присоединяет его `CListBox` к объекту.

Когда `Create` выполняется, Windows отправляет сообщения [WM_NCCREATE](../../mfc/reference/cwnd-class.md#onnccreate), [WM_CREATE](../../mfc/reference/cwnd-class.md#oncreate), [WM_NCCALCSIZE](../../mfc/reference/cwnd-class.md#onnccalcsize)и [WM_GETMINMAXINFO](../../mfc/reference/cwnd-class.md#ongetminmaxinfo) в элемент управления "список".

Эти сообщения по умолчанию обрабатываются функциями члена [оннккреате](../../mfc/reference/cwnd-class.md#onnccreate), [OnCreate](../../mfc/reference/cwnd-class.md#oncreate), [оннккалксизе](../../mfc/reference/cwnd-class.md#onnccalcsize)и [онжетминмаксинфо](../../mfc/reference/cwnd-class.md#ongetminmaxinfo) в `CWnd` базовом классе. Чтобы расширить обработку сообщений по умолчанию, создайте производный класс `CListBox`от, добавьте к новому классу схему сообщений и переопределите предыдущие функции-члены обработчика сообщений. Переопределение `OnCreate`, например, для выполнения необходимой инициализации нового класса.
...
Вызывается платформой, когда пользователь удаляет элемент из объекта рисования `CListBox` владельцем или удаляет окно списка.
...
Описание`DELETEITEMSTRUCT` структуры см. в разделе [CWnd:: онделетеитем](../../mfc/reference/cwnd-class.md#ondeleteitem) .
...
*атрибутов*<br/>
Может быть любым сочетанием **перечисляемых** значений, описанным в `CFile::GetStatu`разделе [s](../../mfc/reference/cfile-class.md#getstatus), или любым сочетанием следующих значений:
...
Отсчитываемый от нуля индекс последнего имени файла, добавленного в список. Если возникает ошибка, возвращается значение LB_ERR. Если недостаточно места для хранения новых строк, возвращается значение LB_ERRSPACE.
...
Если список был создан с использованием стиля рисования, но без стиля [LBS_HASSTRINGS](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) , то `FindStringExact` функция-член пытается сопоставить значение даублеворд со значением *лпсзфинд*.
...
Отсчитываемый от нуля индекс текущего выбранного элемента, если он является списком с единственным выбором. Это LB_ERR, если ни один элемент не выбран в данный момент.
...
Значение, связанное с элементом, или LB_ERR, если возникает ошибка.
...
*ниндекс*<br/>
Указывает отсчитываемый от нуля индекс элемента в списке. Этот параметр используется только в том случае, если список имеет стиль LBS_OWNERDRAWVARIABLE. в противном случае значение должно быть равно 0.

### <a name="return-value"></a>Возвращаемое значение

Высота элементов списка в пикселях. Если список имеет стиль [LBS_OWNERDRAWVARIABLE](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) , возвращаемое значение является высотой элемента, указанного параметром *ниндекс*. При возникновении ошибки возвращается значение LB_ERR.
...
Количество элементов в столбце `CListBox` объекта.

### <a name="remarks"></a>Примечания

Эта функция члена эмулирует функциональность сообщения [LB_GETLISTBOXINFO](/windows/win32/Controls/lb-getlistboxinfo) , как описано в Windows SDK.
...
Положительное число, если выбран указанный элемент; в противном случае — 0. Если возникает ошибка, возвращается значение LB_ERR.
...
Число выбранных элементов в поле со списком. Если список является списком с единственным выбором, возвращается значение LB_ERR.
...
Длина строки (в байтах), исключая завершающего нуль-символа. Если *ниндекс* не указывает допустимый индекс, возвращается значение LB_ERR.
...
Длина строки в символах, за исключением завершающего нуль символа. Если *ниндекс* не указывает допустимый индекс, возвращается значение LB_ERR.
...
В случае успеха максимальное число элементов, которое может храниться в списке, до того, как потребуется перераспределение памяти, в противном случае LB_ERRSPACE, что означает недостаточное количество памяти.
...
Отсчитываемый от нуля индекс позиции, в которую была вставлена строка. Если возникает ошибка, возвращается значение LB_ERR. Если недостаточно места для хранения новой строки, возвращается значение LB_ERRSPACE.

### <a name="remarks"></a>Примечания

В отличие от функции-члена [AddString](#addstring), `InsertString` не приводит к сортировке списка с [LBS_SORT](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) стилем.
...
*баутсиде*<br/>
Ссылка на переменную BOOL, которая будет иметь значение TRUE, если *PT* находится за пределами клиентской области ближайшего элемента списка, значение false, если *PT* находится внутри клиентской области ближайшего элемента списка.
...
По умолчанию эта функция члена не выполняет никаких действий. Переопределите эту функцию-член и заполните `MEASUREITEMSTRUCT` структуру, чтобы информировать окна измерений списка. Если список создается с использованием стиля [LBS_OWNERDRAWVARIABLE](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) , платформа вызывает эту функцию-член для каждого элемента в списке. В противном случае этот член вызывается только один раз.

Дополнительные сведения об использовании стиля [LBS_OWNERDRAWFIXED](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) в списке `SubclassDlgItem` `CWnd`, рисуемом владельцем, созданном с помощью функции-члена, см. в обсуждении в [техническом примечании 14](../../mfc/tn014-custom-controls.md).
...
Индекс выбранного элемента, если поиск прошел успешно. Если поиск завершился неудачно, возвращается значение LB_ERR, а текущее выделение не изменяется.

### <a name="remarks"></a>Примечания

При необходимости список будет прокручиваться, чтобы отобразить выбранный элемент.

Эту функцию-член нельзя использовать с списком, имеющим стиль [LBS_MULTIPLESEL](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) .
...
Используйте функцию-член для поиска строки без выбора элемента. `FindString`
...
Задает ширину (в пикселях) всех столбцов в списке с многостолбцовым списком (созданном с использованием стиля [LBS_MULTICOLUMN](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) ).
...
Чтобы ответить на вызов `SetHorizontalExtent`, список должен быть определен с использованием стиля [WS_HSCROLL](../../mfc/reference/styles-used-by-mfc.md#window-styles) .
...
Если список имеет стиль [LBS_OWNERDRAWVARIABLE](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) , эта функция задает высоту элемента, указанного параметром *ниндекс*. В противном случае эта функция задает высоту всех элементов в списке.
...
Вызывается структурой, когда родительское окно списка получает сообщение WM_VKEYTOITEM из списка.
...
Сообщение WM_VKEYTOITEM отправляется списком при получении сообщения WM_KEYDOWN, но только в том случае, если список соответствует обоим следующим условиям:

- Имеет установленный стиль [LBS_WANTKEYBOARDINPUT](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) .
