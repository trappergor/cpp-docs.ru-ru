 ...
 *pfnRequest*  
 Указатель на функцию-член, который обрабатывает `OnRequestEdit` уведомления для этого свойства. Данная функция должна иметь возвращаемый тип BOOL и **BOOL\***  параметра. Эту функцию следует присвоить параметру значение TRUE, чтобы разрешить изменение этого свойства и значение FALSE, чтобы запретить. Данная функция должна возвращать значение TRUE, чтобы указать, что уведомление было обработано. в противном случае — значение FALSE.  
