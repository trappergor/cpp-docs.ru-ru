...
##  <a name="domodal"></a>  CPrintDialogEx::DoModal  
 Эта функция вызывается для отображения окна свойств печати Windows и разрешить пользователю выбирать различные параметры печати, например число копий, диапазон страниц и ли должно быть по копиям.  
