...
|[CToolBarCtrl:: Create](#create)|Создает элемент управления Toolbar и прикрепляет его к объекту `CToolBarCtrl`.|
|[CToolBarCtrl:: Креатикс](#createex)|Создает элемент управления ToolBar с указанными расширенными стилями Windows и прикрепляет его к объекту `CToolBarCtrl`.|
...
|[CToolBarCtrl:: Сетбуттонструктсизе](#setbuttonstructsize)|Задает размер структуры `TBBUTTON`.|
...
Этот элемент управления (и, следовательно, класс `CToolBarCtrl`) доступен только для программ, работающих под управлением Windows 95/98 и Windows NT версии 3,51 и более поздних версий.
...
`CToolBarCtrl` объекты содержат несколько важных внутренних структур данных: список растровых изображений кнопок или список изображений, список строк меток для кнопок и список структур `TBBUTTON`, которые связывают изображение и (или) строку с положением, стилем, состоянием и ИДЕНТИФИКАТОРом команды кнопки. На каждый элемент этих структур данных ссылается индекс, начинающийся с нуля. Прежде чем можно будет использовать объект `CToolBarCtrl`, необходимо настроить эти структуры данных. Список строк можно использовать только для меток кнопок; нельзя получить строки с панели инструментов.

Чтобы использовать объект `CToolBarCtrl`, обычно выполняются следующие действия.

1. Создайте объект `CToolBarCtrl`.

1. Вызовите [CREATE](#create) , чтобы создать общий элемент управления панели инструментов Windows и присоединить его к объекту `CToolBarCtrl`. Укажите стиль панели инструментов с помощью стилей, например TBSTYLE_TRANSPARENT для прозрачной панели инструментов или TBSTYLE_DROPDOWN для панели инструментов, которая поддерживает кнопки с раскрывающимся списком.
...
1. Если вам нужны подсказки для кнопки на панели инструментов в окне-владельце, которая не является `CFrameWnd`, необходимо обработать сообщения TTN_NEEDTEXT в окне владелец панели инструментов, как описано в разделе [обработка уведомлений подсказок](../../mfc/handling-tool-tip-notifications.md). Если родительское окно панели инструментов является производным от `CFrameWnd`, всплывающие подсказки отображаются без каких-либо дополнительных усилий, поскольку `CFrameWnd` предоставляет обработчик по умолчанию.
...
*пбитмап*<br/>
Указатель на объект `CBitmap`, содержащий изображение кнопки или изображения для добавления.
...
Вы можете использовать Windows API [креатемаппедбитмап](/windows/win32/api/commctrl/nf-commctrl-createmappedbitmap) для отображения цветов перед добавлением точечного рисунка на панель инструментов. При передаче указателя на объект `CBitMap` необходимо убедиться, что растровое изображение не уничтожается до тех пор, пока не будет удалена панель инструментов.
...
*лпбуттонс*<br/>
Адрес массива структур `TBBUTTON`, содержащих сведения о добавляемых кнопках. Число элементов в массиве должно совпадать с кнопками, заданными параметром *ннумбуттонс*.
...
Указатель *лпбуттонс* указывает на массив структур `TBBUTTON`. Каждая структура `TBBUTTON` связывает добавленную кнопку со стилем кнопки, изображением и/или строкой, ИДЕНТИФИКАТОРом команды, состоянием и определяемыми пользователем данными:
...
- `idCommand`
   ...
   Идентификатор команды, связанный с кнопкой. Этот идентификатор отправляется в сообщении WM_COMMAND при выборе кнопки. Если элемент `fsStyle` имеет значение TBSTYLE_SEP, этот элемент должен быть равен нулю.
...
Не следует передавать объект `CString` в эту функцию, так как в `CString`не может быть более одного символа null.
...
Создает элемент управления Toolbar и прикрепляет его к объекту `CToolBarCtrl`.
...
Создание `CToolBarCtrl` выполняется в два этапа. Сначала вызовите конструктор, а затем вызовите метод `Create`, который создает элемент управления Toolbar и прикрепляет его к объекту `CToolBarCtrl`. Применить следующие стили окна к элементу управления ToolBar.
...
Чтобы использовать расширенные стили панели инструментов, вызовите [сетекстендедстиле](#setextendedstyle) после вызова `Create`. Чтобы создать панель инструментов с расширенными стилями окна, вызовите метод [CToolBarCtrl:: креатикс](#createex) вместо `Create`.
...
Создает элемент управления (дочернее окно) и связывает его с объектом `CToolBarCtrl`.
...
Используйте `CreateEx` вместо [CREATE](#create) для применения расширенных стилей Windows, заданных в **WS_EX_е**расширенного стиля Windows. `CreateEx` создает элемент управления с расширенными стилями Windows, заданным параметром *двексстиле*. Установка расширенных стилей, относящихся к элементу управления с помощью [сетекстендедстиле](#setextendedstyle). Например, используйте `CreateEx`, чтобы задать такие стили, как WS_EX_CONTEXTHELP, но используйте `SetExtendedStyle` для установки таких стилей, как TBSTYLE_EX_DRAWDDARROWS. Дополнительные сведения см. в статье стили, описанные в разделе [Расширенные стили панели инструментов](/windows/win32/Controls/toolbar-extended-styles) в Windows SDK.
...
*лпбуттон*<br/>
Адрес структуры `TBBUTTON`, которая должна получить копию сведений о кнопке. Сведения о структуре `TBBUTTON` см. в разделе [CToolBarCtrl:: аддбуттонс](#addbuttons) .
...
Эта функция члена реализует поведение сообщения Win32 [TB_GETDISABLEDIMAGELIST](/windows/win32/Controls/tb-getdisabledimagelist), как описано в Windows SDK. Реализация `GetDisabledImageList` MFC использует объект `CImageList`, содержащий изображения кнопки элемента управления ToolBar, а не маркер списка изображений.
...
Извлекает метрики объекта `CToolBarCtrl`.
...
*птбм*<br/>
Указатель на структуру [тбметрикс](/windows/win32/api/commctrl/ns-commctrl-tbmetrics) объекта `CToolBarCtrl`.
...
*PowerPoint*<br/>
Указатель на структуру [Point](/previous-versions/dd162805\(v=vs.85\)) , содержащую координату x проверки попадания в элементе `x` и координату y проверки попадания в элементе `y`. Координаты задаются относительно клиентской области панели инструментов.
...
*лпбуттон*<br/>
Адрес структуры `TBBUTTON`, содержащей сведения о вставляемой кнопке. Описание структуры `TBBUTTON` см. в разделе [CToolBarCtrl:: аддбуттонс](#addbuttons) .
...
|Идентификатор точечного рисунка|Description|
|---------------|-----------------|
...
*нсизе*<br/>
Размер (в байтах) структуры `TBBUTTON`.

### <a name="remarks"></a>Remarks

Если вы хотите сохранить дополнительные данные в структуре `TBBUTTON`, можно либо получить новую структуру от `TBBUTTON`, добавить необходимые элементы, либо создать новую структуру, содержащую `TBBUTTON`ную структуру в качестве первого элемента. Затем следует вызвать эту функцию, чтобы указать элементу управления ToolBar размер новой структуры.

Дополнительные сведения о структуре `TBBUTTON` см. в разделе [CToolBarCtrl:: аддбуттонс](#addbuttons) .
...
*двдтфлагс*<br/>
Сочетание одного или нескольких флагов DT_, заданных в функции Win32 `DrawText`, которые указывают, как будет нарисован текст кнопки. Это значение передается в `DrawText` при прорисовке текста кнопки.
...
Эта функция члена реализует поведение сообщения Win32 [TB_SETDRAWTEXTFLAGS](/windows/win32/Controls/tb-setdrawtextflags), как описано в Windows SDK. Эта функция члена задает флаги в функции Win32 `DrawText`, которая рисует текст в указанном прямоугольнике в соответствии с установленными флагами.
...
Реализация `SetHotImageList` MFC использует объект `CImageList`, содержащий изображения активной кнопки элемента управления ToolBar, а не маркер списка изображений. Кнопка "горячая" отображается при выделении указателя над ним.
...
Реализация `SetImageList` MFC использует объект `CImageList`, содержащий изображения кнопки элемента управления ToolBar, а не маркер списка изображений.
...
Задает метрики объекта `CToolBarCtrl`.
...
*Приводится*<br/>
Указатель на `CWnd` или `CWnd`производный объект, который будет новым владельцем окна для элемента управления ToolBar.
...
Задает визуальный стиль объекта `CToolBarCtrl`.
