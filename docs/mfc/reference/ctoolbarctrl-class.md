...
|[CToolBarCtrl:: Креатикс](#createex)|Создает элемент управления ToolBar с указанными расширенными стилями Windows и прикрепляет его `CToolBarCtrl` к объекту.|
...
Этот элемент управления (и, `CToolBarCtrl` следовательно, класс) доступен только для программ, работающих под управлением Windows 95/98 и Windows NT версии 3,51 и более поздних версий.
...
`CToolBarCtrl`объекты содержат несколько важных внутренних структур данных: список растровых изображений кнопок или список изображений, список строк меток для кнопок и список структур, связывающих изображение и ( `TBBUTTON` или) строку с расположением, стилем, состоянием и идентификатором команды. кнопки. На каждый элемент этих структур данных ссылается индекс, начинающийся с нуля. Прежде чем можно будет использовать `CToolBarCtrl` объект, необходимо настроить эти структуры данных. Список строк можно использовать только для меток кнопок; нельзя получить строки с панели инструментов.

Для использования `CToolBarCtrl` объекта, как правило, выполняются следующие действия.

1. `CToolBarCtrl` Создайте объект.
...
1. Если вам нужны подсказки для кнопки на панели инструментов в окне владельца, которое не `CFrameWnd`является, необходимо обрабатывать сообщения TTN_NEEDTEXT в окне владелец панели инструментов, как описано в разделе [Обработка всплывающих уведомлений](../../mfc/handling-tool-tip-notifications.md). Если родительское окно панели инструментов является производным от `CFrameWnd`, всплывающие подсказки отображаются без каких-либо дополнительных усилий, `CFrameWnd` так как предоставляет обработчик по умолчанию.
...
Прозрачная панель инструментов позволяет отображать на панели инструментов клиент с помощью. Чтобы создать прозрачную панель инструментов, используйте стили TBSTYLE_FLAT и TBSTYLE_TRANSPARENT. Прозрачные панели инструментов Отслеживание горячей замены; Это значит, что при наведении указателя мыши на кнопку на панели инструментов внешний вид кнопки изменяется. Панели инструментов, созданные только с помощью стиля TBSTYLE_FLAT, будут содержать непрозрачные кнопки.
...
##  <a name="addbitmap"></a>CToolBarCtrl:: Аддбитмап
...
##  <a name="addbuttons"></a>CToolBarCtrl:: Аддбуттонс
...
- `idCommand`
   ...
   Идентификатор команды, связанный с кнопкой. Этот идентификатор отправляется в сообщении WM_COMMAND при выборе кнопки. `fsStyle` Если элемент имеет значение TBSTYLE_SEP, этот элемент должен быть равен нулю.
...
##  <a name="addstrings"></a>CToolBarCtrl:: Аддстрингс
...
Не следует передавать `CString` объект в эту функцию, поскольку невозможно иметь более одного символа NULL `CString`в.
...
##  <a name="changebitmap"></a>CToolBarCtrl:: Чанжебитмап
...
|Параметр|Описание|
|---------------|-----------------|
|*идбуттон*|окне Идентификатор команды для получения нового точечного рисунка.|
|*ибитмап*|окне Отсчитываемый от нуля индекс изображения в списке изображений текущего элемента управления ToolBar.|
...
Этот метод отправляет сообщение [TB_CHANGEBITMAP](/windows/win32/Controls/tb-changebitmap) , описанное в Windows SDK.
...
##  <a name="checkbutton"></a>CToolBarCtrl:: Чеккбуттон
...
##  <a name="commandtoindex"></a>CToolBarCtrl:: Коммандтоиндекс
...
*rect*<br/>
При необходимости задает размер и расположение элемента управления ToolBar. Это может быть либо объект [крект](../../atl-mfc-shared/reference/crect-class.md) , либо структура [Rect](/previous-versions/dd162897\(v=vs.85\)) .
...
Построение создается `CToolBarCtrl` в два этапа. Сначала вызовите конструктор, а затем вызовите `Create`метод, который создает элемент управления Toolbar и прикрепляет его `CToolBarCtrl` к объекту. Применить следующие стили окна к элементу управления ToolBar.
...
Чтобы использовать расширенные стили панели инструментов, вызовите метод [SetExtendedStyle](#setextendedstyle) после вызова метода `Create`. Чтобы создать панель инструментов с расширенными стилями окна, вызовите метод [CToolBarCtrl:: креатикс](#createex) вместо `Create`.
...
##  <a name="createex"></a>CToolBarCtrl:: Креатикс
...
##  <a name="customize"></a>CToolBarCtrl:: Настройка
...
##  <a name="deletebutton"></a>CToolBarCtrl::D Елетебуттон
...
##  <a name="enablebutton"></a>CToolBarCtrl:: Енаблебуттон
...
##  <a name="getanchorhighlight"></a>CToolBarCtrl:: Жетанчорхигхлигхт
...
##  <a name="getbitmap"></a>CToolBarCtrl::/Bitmap
...
##  <a name="getbitmapflags"></a>CToolBarCtrl:: Жетбитмапфлагс
...
Значение UINT с установленным флагом TBBF_LARGE, если отображение может поддерживать большие точечные рисунки панели инструментов. в противном случае снимите флажок.
...
##  <a name="getbutton"></a>CToolBarCtrl:: "Кнопка"
...
*лпбуттон*<br/>
`TBBUTTON` Адрес структуры, получающей копию сведений о кнопке. Сведения о структуре см. в `TBBUTTON` разделе [CToolBarCtrl:: аддбуттонс](#addbuttons) .

### <a name="return-value"></a>Возвращаемое значение

Ненулевое значение в случае успеха, иначе — 0.

##  <a name="getbuttoncount"></a>CToolBarCtrl:: Жетбуттонкаунт
...
##  <a name="getbuttoninfo"></a>CToolBarCtrl:: Жетбуттонинфо
...
##  <a name="getbuttonsize"></a>CToolBarCtrl:: Жетбуттонсизе
...
##  <a name="getbuttontext"></a>CToolBarCtrl:: Жетбуттонтекст
...
|Параметр|Описание|
|---------------|-----------------|
|*идбуттон*|окне Идентификатор для кнопки, отображаемый текст которой извлекается.|
...
Этот метод отправляет сообщение [TB_GETBUTTONTEXT](/windows/win32/Controls/tb-getbuttontext) , описанное в Windows SDK.

##  <a name="getcolorscheme"></a>CToolBarCtrl:: Жетколорсчеме
...
|Параметр|Описание|
|---------------|-----------------|
|*лпколорсчеме*|заполняет Указатель на структуру [колорсчеме](/windows/win32/api/commctrl/ns-commctrl-colorscheme) , которая получает сведения о цветовой схеме. При возврате из этого метода структура описывает цвет выделения и цвет тени элемента управления ToolBar.|
...
Этот метод отправляет сообщение [TB_GETCOLORSCHEME](/windows/win32/Controls/tb-getcolorscheme) , описанное в Windows SDK.

##  <a name="getdisabledimagelist"></a>CToolBarCtrl:: Жетдисабледимажелист
...
Эта функция члена реализует поведение сообщения Win32 [TB_GETDISABLEDIMAGELIST](/windows/win32/Controls/tb-getdisabledimagelist), как описано в Windows SDK. Реализация `GetDisabledImageList` MFC`CImageList` использует объект, содержащий изображения кнопки элемента управления ToolBar, а не маркер списка изображений.

##  <a name="getdroptarget"></a>CToolBarCtrl:: Жетдроптаржет
...
##  <a name="getextendedstyle"></a>CToolBarCtrl:: Жетекстендедстиле
...
##  <a name="gethotimagelist"></a>CToolBarCtrl:: Жесотимажелист
...
##  <a name="gethotitem"></a>CToolBarCtrl:: Жесотитем
...
##  <a name="getimagelist"></a>CToolBarCtrl::/ImageList
...
##  <a name="getinsertmark"></a>CToolBarCtrl:: Жетинсертмарк
...
##  <a name="getinsertmarkcolor"></a>CToolBarCtrl:: Жетинсертмаркколор
...
##  <a name="getitemrect"></a>CToolBarCtrl:: Жетитемрект
...
*лпрект*<br/>
Адрес структуры [Rect](/previous-versions/dd162897\(v=vs.85\)) или объекта [крект](../../atl-mfc-shared/reference/crect-class.md) , который получает координаты ограничивающего прямоугольника.
...
##  <a name="getmaxsize"></a>CToolBarCtrl:: Жетмакссизе
...
##  <a name="getmaxtextrows"></a>CToolBarCtrl:: Жетмакстекстровс
...
##  <a name="getmetrics"></a>CToolBarCtrl:: не метрики
...
*птбм*<br/>
Указатель на структуру `CToolBarCtrl` [тбметрикс](/windows/win32/api/commctrl/ns-commctrl-tbmetrics) объекта.

### <a name="remarks"></a>Примечания

Эта функция члена эмулирует функциональность сообщения [TB_GETMETRICS](/windows/win32/Controls/tb-getmetrics) , как описано в Windows SDK.

##  <a name="getpadding"></a>CToolBarCtrl:: Padding
...
|Параметр|Описание|
|---------------|-----------------|
|*пнхорзпаддинг*|заполняет Целое число, получающее горизонтальное заполнение элемента управления Toolbar в пикселях.|
|*пнвертпаддинг*|заполняет Целое число, получающее вертикальное заполнение элемента управления Toolbar в пикселях.|
...
Этот метод отправляет сообщение [TB_GETPADDING](/windows/win32/Controls/tb-getpadding) , описанное в Windows SDK.

##  <a name="getpressedimagelist"></a>CToolBarCtrl:: Жетпресседимажелист
...
Этот метод отправляет сообщение [TB_GETPRESSEDIMAGELIST](/windows/win32/Controls/tb-getpressedimagelist) , описанное в Windows SDK.

##  <a name="getrect"></a>CToolBarCtrl:: коrect
...
*лпрект*<br/>
Указатель на структуру [Rect](/previous-versions/dd162897\(v=vs.85\)) для получения сведений о ограничивающем прямоугольнике.
...
##  <a name="getstate"></a>CToolBarCtrl:: State
...
##  <a name="getstyle"></a>CToolBarCtrl:: штриховка
...
##  <a name="gettooltips"></a>CToolBarCtrl:: подсказки
...
*PowerPoint*<br/>
Указатель на структуру [Point](/previous-versions/dd162805\(v=vs.85\)) , которая содержит координату x проверки попадания в `x` элементе и координату y проверки `y` попадания в элементе. Координаты задаются относительно клиентской области панели инструментов.
...
##  <a name="hidebutton"></a>CToolBarCtrl:: Хидебуттон
...
##  <a name="indeterminate"></a>CToolBarCtrl:: неопределенное
...
##  <a name="insertbutton"></a>CToolBarCtrl:: Инсертбуттон
...
*лпбуттон*<br/>
Адрес структуры, `TBBUTTON` содержащей сведения о вставляемой кнопке. Описание структуры см. в `TBBUTTON` разделе [CToolBarCtrl:: аддбуттонс](#addbuttons) .
...
##  <a name="insertmarkhittest"></a>CToolBarCtrl:: Инсертмаркхиттест
...
*PowerPoint*<br/>
Указатель на структуру [Point](/previous-versions/dd162805\(v=vs.85\)) , содержащую координаты проверки попадания относительно клиентской области панели инструментов.
...
##  <a name="isbuttonchecked"></a>CToolBarCtrl:: Исбуттончеккед
...
##  <a name="isbuttonenabled"></a>CToolBarCtrl:: Исбуттоненаблед
...
##  <a name="isbuttonhidden"></a>CToolBarCtrl:: Исбуттонхидден
...
##  <a name="isbuttonhighlighted"></a>CToolBarCtrl:: Исбуттонхигхлигхтед
...
##  <a name="isbuttonindeterminate"></a>CToolBarCtrl:: Исбуттониндетерминате
...
##  <a name="isbuttonpressed"></a>CToolBarCtrl:: Исбуттонпрессед
...
##  <a name="loadimages"></a>CToolBarCtrl:: Лоадимажес
...
##  <a name="mapaccelerator"></a>CToolBarCtrl:: Мапакцелератор
...
##  <a name="markbutton"></a>CToolBarCtrl:: Маркбуттон
...
##  <a name="movebutton"></a>CToolBarCtrl:: Мовебуттон
...
##  <a name="pressbutton"></a>CToolBarCtrl::P Рессбуттон
...
Этот метод отправляет сообщение [TB_PRESSBUTTON](/windows/win32/Controls/tb-pressbutton) , описанное в Windows SDK.

##  <a name="replacebitmap"></a>CToolBarCtrl:: Реплацебитмап
...
|Параметр|Описание|
|---------------|-----------------|
|*преплацебитмап*|окне Указатель на структуру [тбреплацебитмап](/windows/win32/api/commctrl/ns-commctrl-tbreplacebitmap) , описывающую заменяемый точечный рисунок и новый точечный рисунок.|
...
Этот метод отправляет сообщение [TB_REPLACEBITMAP](/windows/win32/Controls/tb-replacebitmap) , описанное в Windows SDK.
...
- РАЗДЕЛ РЕЕСТРА HKEY_CLASSES_ROOT
...
##  <a name="setanchorhighlight"></a>CToolBarCtrl:: Сетанчорхигхлигхт
...
##  <a name="setbitmapsize"></a>CToolBarCtrl:: Сетбитмапсизе
...
##  <a name="setbuttoninfo"></a>CToolBarCtrl:: Сетбуттонинфо
...
##  <a name="setbuttonsize"></a>CToolBarCtrl:: Сетбуттонсизе
...
##  <a name="setbuttonstructsize"></a>CToolBarCtrl:: Сетбуттонструктсизе
...
*нсизе*<br/>
Размер `TBBUTTON` структуры в байтах.

### <a name="remarks"></a>Примечания

Если вы хотите хранить в `TBBUTTON` структуре дополнительные данные, можно либо получить новую структуру из `TBBUTTON`, добавить необходимые элементы, либо создать `TBBUTTON` новую структуру, содержащую структуру в качестве первого элемента. Затем следует вызвать эту функцию, чтобы указать элементу управления ToolBar размер новой структуры.

Дополнительные сведения `TBBUTTON` о структуре см. в разделе [CToolBarCtrl:: аддбуттонс](#addbuttons) .

##  <a name="setbuttonwidth"></a>CToolBarCtrl:: Сетбуттонвидс
...
##  <a name="setcmdid"></a>CToolBarCtrl:: Сеткмдид
...
##  <a name="setcolorscheme"></a>CToolBarCtrl:: Сетколорсчеме
...
|Параметр|Описание|
|---------------|-----------------|
|*лпколорсчеме*|окне Указатель на структуру [колорсчеме](/windows/win32/api/commctrl/ns-commctrl-colorscheme) , описывающую цвет выделения и цвет тени элемента управления ToolBar.|

### <a name="remarks"></a>Примечания

Этот метод не действует, если задана визуальная тема Windows Vista.

Этот метод отправляет сообщение [TB_SETCOLORSCHEME](/windows/win32/Controls/tb-setcolorscheme) , описанное в Windows SDK.
...
##  <a name="setdisabledimagelist"></a>CToolBarCtrl:: Сетдисабледимажелист
...
*пимажелист*<br/>
Указатель на `CImageList` объект, содержащий изображения, которые будут использоваться элементом управления Toolbar для вывода отключенных изображений кнопки.
...
Эта функция члена реализует поведение сообщения Win32 [TB_SETDISABLEDIMAGELIST](/windows/win32/Controls/tb-setdisabledimagelist), как описано в Windows SDK. Реализация `SetDisabledImageList` MFC`CImageList` использует объект, содержащий отключенные изображения кнопки элемента управления ToolBar, а не маркер списка изображений.

##  <a name="setdrawtextflags"></a>CToolBarCtrl:: Сетдравтекстфлагс
...
*двмаск*<br/>
Сочетание одного или нескольких флагов DT_, заданных в функции Win32 [DrawText](/windows/win32/api/winuser/nf-winuser-drawtext), которое указывает, какие биты в *двдтфлагс* будут использоваться при рисовании текста.

*двдтфлагс*<br/>
Сочетание одного или нескольких флагов DT_, заданных в функции `DrawText`Win32, которые указывают, как будет нарисован текст кнопки. Это значение передается `DrawText` в при прорисовке текста кнопки.
...
Эта функция члена реализует поведение сообщения Win32 [TB_SETDRAWTEXTFLAGS](/windows/win32/Controls/tb-setdrawtextflags), как описано в Windows SDK. Эта функция – член задает флаги в функции `DrawText`Win32, которая рисует текст в указанном прямоугольнике, отформатированный в соответствии с установленными флагами.

##  <a name="setextendedstyle"></a>CToolBarCtrl:: Сетекстендедстиле
...
##  <a name="sethotimagelist"></a>CToolBarCtrl:: Сесотимажелист
...
*пимажелист*<br/>
Указатель на `CImageList` объект, содержащий изображения, которые будут использоваться элементом управления Toolbar для показа изображений активной кнопки.
...
Реализация `SetHotImageList` MFC`CImageList` использует объект, содержащий изображения активной кнопки элемента управления ToolBar, а не маркер списка изображений. Кнопка "горячая" отображается при выделении указателя над ним.

##  <a name="sethotitem"></a>CToolBarCtrl:: Сесотитем
...
##  <a name="setimagelist"></a>CToolBarCtrl:: Сетимажелист
...
*пимажелист*<br/>
Указатель на `CImageList` объект, содержащий изображения, которые будут использоваться элементом управления Toolbar для отображения изображений кнопок в их состоянии по умолчанию.
...
Реализация `SetImageList` MFC`CImageList` использует объект, содержащий изображения кнопки элемента управления ToolBar, а не маркер списка изображений.

##  <a name="setindent"></a>CToolBarCtrl:: Сетиндент
...
##  <a name="setinsertmark"></a>CToolBarCtrl:: Сетинсертмарк
...
##  <a name="setinsertmarkcolor"></a>CToolBarCtrl:: Сетинсертмаркколор
...
##  <a name="setmaxtextrows"></a>CToolBarCtrl:: Сетмакстекстровс
...
##  <a name="setmetrics"></a>CToolBarCtrl:: Сетметрикс
...
Эта функция члена эмулирует функциональность сообщения [TB_SETMETRICS](/windows/win32/Controls/tb-setmetrics) , как описано в Windows SDK.

##  <a name="setowner"></a>CToolBarCtrl:: Сетовнер
...
*Приводится*<br/>
Указатель на `CWnd` объект, `CWnd`производный от или, который будет новым владельцем окна для элемента управления ToolBar.

### <a name="remarks"></a>Примечания

Окно-владелец — это окно, которое получает уведомления с панели инструментов.

##  <a name="setpadding"></a>CToolBarCtrl:: Сетпаддинг
...
|Параметр|Описание|
|---------------|-----------------|
|*нхорзпаддинг*|окне Задает горизонтальное заполнение элемента управления Toolbar в пикселях.|
|*нвертпаддинг*|окне Задает вертикальное заполнение элемента управления Toolbar в пикселях.|
...
Этот метод отправляет сообщение [TB_SETPADDING](/windows/win32/Controls/tb-setpadding) , описанное в Windows SDK.
...
##  <a name="setpressedimagelist"></a>CToolBarCtrl:: Сетпресседимажелист
...
|Параметр|Описание|
|---------------|-----------------|
|*иимажеид*|окне Отсчитываемый от нуля индекс списка изображений. Если используется только один список изображений, присвойте этому параметру значение нуль.|
|*пимажелист*|окне Указатель на [CImageList](../../mfc/reference/cimagelist-class.md) , содержащий новый список изображений.|
...
Этот метод отправляет сообщение [TB_SETPRESSEDIMAGELIST](/windows/win32/Controls/tb-setpressedimagelist) , описанное в Windows SDK.
...
##  <a name="setrows"></a>CToolBarCtrl:: Сетровс
...
*лпрект*<br/>
Указывает на объект [крект](../../atl-mfc-shared/reference/crect-class.md) или структуру [Rect](/previous-versions/dd162897\(v=vs.85\)) , которая будет принимать новый ограничивающий прямоугольник панели инструментов.
...
Обратите внимание, `SetRows` что функция – член всегда выбирает макет, который приводит к наименьшему размеру панели инструментов. Создание панели инструментов с стилем TBSTYLE_WRAPABLE и последующее изменение размера элемента управления приведет к простому применению метода, описанного выше, с учетом ширины элемента управления.

Эту функцию можно вызывать только для панелей инструментов, созданных с использованием стиля TBSTYLE_WRAPABLE.
...
Эта функция особенно полезна, если требуется задать более одного состояния кнопки. Чтобы просто установить одно состояние, используйте одну из следующих функций члена: [Енаблебуттон](#enablebutton), [чеккбуттон](#checkbutton), [Хидебуттон](#hidebutton), [неопределенный](#indeterminate)или [прессбуттон](#pressbutton).
...
##  <a name="settooltips"></a>CToolBarCtrl:: Сеттултипс
...
Эта функция члена эмулирует функциональность сообщения [TB_SETWINDOWTHEME](/windows/win32/Controls/tb-setwindowtheme) , как описано в Windows SDK.
