...
Класс содержит открытые члены данных, которые можно использовать для определения причины исключения. `CDaoException` объекты формируются и создаются функциями членов классов базы данных DAO.
...
`GetErrorInfo` сохраняет сведения в элементе данных `m_pErrorInfo` объекта исключения. Краткое описание возвращаемых сведений см. в разделе [m_pErrorInfo](#m_perrorinfo). Если перехватится исключение типа `CDaoException` вызванное MFC, то элемент `m_pErrorInfo` уже будет заполнен. При выборе непосредственного вызова DAO необходимо вручную вызвать функцию члена `GetErrorInfo` объекта исключения для заполнения `m_pErrorInfo`. Более подробное описание см. в разделе Структура [CDaoErrorInfo](../../mfc/reference/cdaoerrorinfo-structure.md) .
...
Содержит указатель на структуру `CDaoErrorInfo`, которая предоставляет сведения об объекте ошибки DAO, полученном последним путем вызова [жетерроринфо](#geterrorinfo).

### <a name="remarks"></a>Заметки

Этот объект содержит следующие сведения:

|CDaoErrorInfo, элемент|Информация|Значение|
|--------------------------|-----------------|-------------|
...
Полные сведения о данных, содержащихся в объекте `CDaoErrorInfo`, см. в разделе Структура [CDaoErrorInfo](../../mfc/reference/cdaoerrorinfo-structure.md) .
...
Это код OLE. Это значение редко необходимо использовать, поскольку в большинстве случаев более конкретные сведения об ошибках MFC или DAO доступны в других `CDaoException`ных элементах данных.
