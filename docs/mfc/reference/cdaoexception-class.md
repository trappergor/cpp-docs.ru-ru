---
title: Класс CDaoException | Документация Майкрософт
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
2.  Вызовите объект исключения [GetErrorCount](#geterrorcount) функция-член для определения, сколько ошибок объекты находятся в коллекцию ошибок для ядра СУБД. (Обычно имеет только один, если вы не используете источник данных ODBC.)  
  
3.  Вызовите объект исключения [GetErrorInfo](#geterrorinfo) функция-член для извлечения одного объекта ошибки одновременно, по индексу в коллекции, с помощью объекта исключения. Считать объект исключения прокси-сервер для одного объекта ошибки DAO.  
  
4.  Проверить текущую [CDaoErrorInfo](../../mfc/reference/cdaoerrorinfo-structure.md) структуру, в которой `GetErrorInfo` возвращает в [m_pErrorInfo](#m_perrorinfo) данные-член. Ее члены предоставляют сведения об этой ошибке DAO.  
  
5.  В случае с источником данных ODBC повторите шаги 3 и 4 при необходимости для дополнительные объекты ошибок.  
  
6.  Если вы создан объект исключения в куче, удалите его, используя **удалить** оператора после завершения.  
