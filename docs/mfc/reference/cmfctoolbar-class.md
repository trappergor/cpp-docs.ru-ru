...
ms.locfileid: "78870003"
---
# <a name="cmfctoolbar-class"></a>Класс CMFCToolBar

Класс `CMFCToolBar` похож на [класс CToolBar](../../mfc/reference/ctoolbar-class.md), но обеспечивает дополнительную поддержку функций пользовательского интерфейса. Они включают плоские панели инструментов, панели инструментов с наиболее часто используемыми изображениями, крупные значки, кнопки пролистывания страниц, заблокированные панели инструментов, элементы управления главной панели, текст под изображениями, фоновые изображения и панели инструментов с вкладками. Класс `CMFCToolBar` также содержит встроенную поддержку пользовательской настройки панелей инструментов и меню, перетаскивания между панелями элементов и меню, кнопки поля со списком, кнопки редактируемых полей, различных палитр и кнопок сворачивания.
...
|[CMFCToolBar:: Креатикс](#createex)|Создает объект `CMFCToolBar`, который использует дополнительные параметры стиля, например крупные значки.|
...
|[CMFCToolBar:: Фромхандлеперманент](#fromhandlepermanent)|Получает указатель на объект `CMFCToolBar`, содержащий данный маркер окна.|
...
|[CMFCToolBar:: "Кнопка"](#getbutton)|Возвращает указатель на объект `CMFCToolBarButton` с указанным индексом кнопки панели инструментов.|
...
|[CMFCToolBar:: Жеткустомизебуттон](#getcustomizebutton)|Получает указатель на объект `CMFCCustomizeButton`, связанный с панелью инструментов.|
...
|[CMFCToolBar:: Лоадтулбарекс](#loadtoolbarex)|Загружает панель инструментов из ресурсов приложения с помощью вспомогательного класса `CMFCToolBarInfo`, позволяя приложению использовать крупные изображения.|
...
Чтобы внедрить объект `CMFCToolBar` в приложение, выполните следующие действия.

1. Добавьте объект `CMFCToolBar` в главное окно фрейма.
...
Переопределите этот метод, чтобы обеспечить собственный динамический макет в классах, производных от `CMFCToolbar`.
...
Метод [CMFCToolBar:: аджустлайаут](#adjustlayout) вызывает этот метод для повторного вычисления размера, если родительский элемент панели инструментов не является объектом `CMFCReBar`.
...
Этот метод вычисляет размер панели инструментов с помощью метода `CMFCToolBar::CalcLayout`. Он передает флаг LM_STRETCH для параметра *двмоде* , если *бстретч* имеет значение true. Он передает LM_HORZ флаг, если *бхорз* имеет значение true.
...
Переопределите этот метод в классе, производном от `CMFCToolBar`, чтобы обеспечить собственное вычисление высоты.
...
Если `CanBeRestored` возвращает значение TRUE, то на странице **панели инструментов** диалогового окна Настройка активируется кнопка **Сброс** для выбранной панели инструментов.
...
Можно переопределить метод `CanBeRestored`, чтобы настроить это поведение в производных классах.
...
Объект `CMFCToolBar` поддерживает внутренний список кнопок на панели инструментов. Вызовите эту функцию для получения индекса кнопки в списке, заданной ИДЕНТИФИКАТОРом команды кнопки.
...
Создает объект `CMFCToolBar`, который использует дополнительные параметры стиля, например крупные значки.
...
*ркбордерс*<br/>
окне Объект `CRect`, указывающий ширину границ окна панели инструментов.
...
В следующем примере показано, как использовать метод `CreateEx` класса `CMFCToolBar`. Этот фрагмент кода является частью [примера демонстрационной версии IE](../../overview/visual-cpp-samples.md).
...
Этот метод расширяет реализацию базового класса [CBasePane:: енабледоккинг](../../mfc/reference/cbasepane-class.md#enabledocking), устанавливая для элемента данных `CBasePane::m_dwControlBarStyle` значение AFX_CBRS_FLOAT. Затем этот метод передает *двалигнмент* реализации базового класса.
...
Получает указатель на объект `CMFCToolBar`, содержащий данный маркер окна.
...
Указатель на объект `CMFCToolBar`, который содержит заданный маркер окна, или значение NULL, если соответствующий объект `CMFCToolBar` не существует.
...
Константная ссылка на объект [класса коблист](../../mfc/reference/coblist-class.md) , содержащий коллекцию объектов `CMFCToolBar`.
...
Метод `GetButtonInfo` находит кнопку на панели инструментов по указанному индексу и получает идентификатор команды, стиль и индекс изображения кнопки.
...
Получает указатель на объект `CMFCCustomizeButton`, связанный с панелью инструментов.
...
Указатель на объект `CMFCCustomizeButton`, связанный с панелью инструментов.
...
Можно вызвать метод [CMFCToolBar:: исексисткустомизебуттон](#isexistcustomizebutton) , чтобы определить, содержит ли панель инструментов допустимый объект `CMFCCustomizeButton`.
...
Когда элемент управления `CMFCToolBar` получает фокус ввода, он сохраняет в окне маркер, который потерял фокус, чтобы его можно было восстановить позже.
...
Объект `CRect`, к которому *лпрект* Points, имеет значение 0, если кнопка по указанному индексу не существует.

### <a name="example"></a>Пример

В следующем примере показано, как использовать метод `GetItemRect` класса `CMFCToolBar`. Этот фрагмент кода является частью [примера демонстрационной версии IE](../../overview/visual-cpp-samples.md).
...
Если этот метод возвращает ненулевое значение, можно получить указатель на объект родительского фрейма с помощью метода `CMFCToolBar::GetCommandTarget`. Пример использования этого метода см. в примере Висуалстудиодемо.
...
Указатель на объект `CMFCToolBarButton`, который существует по указанному индексу, или значение NULL, если такого объекта не существует.

### <a name="remarks"></a>Remarks

Платформа вызывает этот метод при обновлении клиентской области, связанной с кнопкой на панели инструментов. Он вызывает метод [CWnd:: инвалидатерект](../../mfc/reference/cwnd-class.md#invalidaterect) с клиентским прямоугольником объекта `CMFCToolBarButton`, который существует по указанному индексу.
...
Переопределите этот метод в классе, производном от `CMFCToolBar`, и возвратите значение TRUE, если панель инструментов может отображать кнопки с дополнительным размером границы и возвращать значение FALSE в противном случае. Реализация по умолчанию возвращает значение TRUE.
...
Этот метод получает указатель на [структуру MSG](/windows/win32/api/winuser/ns-winuser-msg) , вызывая `CWnd::GetCurrentMessage`. Затем он сравнивает HWND кнопки с элементами `MSG::lParam` и `MSG::hwnd`, чтобы определить, была ли кнопка источником команды.
...
В следующем примере показано, как использовать метод `LoadToolBar` в классе `CMFCToolBar`. Этот фрагмент кода является частью [примера демонстрационной версии IE](../../overview/visual-cpp-samples.md).
...
Загружает панель инструментов из ресурсов приложения с помощью вспомогательного класса `CMFCToolBarInfo`, позволяя приложению использовать крупные изображения.
...
*params*<br/>
окне Ссылка на объект `CMFCToolBarInfo`, содержащий идентификаторы ресурсов для изображений панели инструментов.
...
Реализация по умолчанию не выполняет никаких действий. Переопределите `OnReset` в классе, производном от `CMFCToolBar`, если на панели инструментов есть фиктивные кнопки, которые необходимо заменить, когда панель инструментов возвращается в исходное состояние.
...
*стртттекст*<br/>
заполняет Ссылка на объект `CString`, который получает текст подсказки.
...
*кнопка*<br/>
окне Ссылка на `CMFCToolBarButton` для вставки.
...
В следующем примере показано, как использовать метод `ReplaceButton` в классе `CMFCToolBar`. Этот фрагмент кода является частью [примера демонстрационной версии IE](../../overview/visual-cpp-samples.md).
...
*лсткоммандс*<br/>
окне Ссылка на объект `CList`, содержащий коллекцию команд.
...
Этот класс использует класс `CMFCCmdUsageCount` для наблюдения за количеством команд. Дополнительные сведения об этом классе см. в разделе [класс CMFCCmdUsageCount](../../mfc/reference/cmfccmdusagecount-class.md).
...
*лсткоммандс*<br/>
окне Ссылка на объект `CList`, содержащий команды, которые не могут быть выполнены пользователем.
...
Ненулевое значение, если метод выполнен. в противном случае 0, если указанный объект `CMFCToolBarImages` является недопустимым или имеет размер изображения, отличающийся от используемого по умолчанию размера изображения панели инструментов.
...
Этот метод создает ошибку утверждения в сборках отладки, если указанный объект `CMFCToolBarImages` является недопустимым или имеет размер изображения, отличающийся от используемого по умолчанию размера изображения панели инструментов.
