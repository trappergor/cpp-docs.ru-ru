...
## <a name="remarks"></a>Примечания  
 ...
 `CDocument` Отправка документа по почте, если присутствует почтовых служб (MAPI) поддерживает. `COleDocument` обновил [OnFileSendMail](#onfilesendmail) для правильной обработки составных документов. Дополнительные сведения см. в статьях [MAPI](../../mfc/mapi.md) и [поддержка MAPI в MFC](../../mfc/mapi-support-in-mfc.md)...  
...
### <a name="parameters"></a>Параметры  
 *pCmdUI*  
 Указатель на `CCmdUI` структуру, которая представляет меню, вызвавшего команду update. Обработчик вызывает метод обновления `Enable` функцию-член `CCmdUI` структуры через *pCmdUI* для обновления пользовательского интерфейса.  
