...
*лпабк*<br/>
Указывает на массив структур [ABC](/windows/win32/api/wingdi/ns-wingdi-abc) , которые получают ширину символов при возврате функции. Этот массив должен содержать по меньшей мере `ABC` столько структур, сколько символов в диапазоне, указанном параметрами *нфирстчар* и *нластчар* .
...
Чтобы преобразовать ширину ABC в единицы проектирования шрифта, приложение должно создать шрифт, высота которого (как `lfHeight` указано в элементе структуры [LOGFONT](/windows/win32/api/wingdi/ns-wingdi-logfontw) ), равна `ntmSizeEM` значению, хранящемуся в элементе [невтекстметрик](/windows/win32/api/wingdi/ns-wingdi-newtextmetricw) дереве. (Значение `ntmSizeEM` члена можно получить, вызвав функцию Windows [EnumFontFamilies](/windows/win32/api/wingdi/nf-wingdi-enumfontfamiliesw) .)
...
*лпабк*<br/>
Указатель на массив [ABC](/windows/win32/api/wingdi/ns-wingdi-abc) , принимающий ширину символов. Этот массив должен содержать по меньшей мере `ABC` столько структур, сколько индексов глифов, заданных параметром *CGI* .
...
*lpmat2*<br/>
Указывает на структуру [MAT2](/windows/win32/api/wingdi/ns-wingdi-mat2) , содержащую матрицу преобразования для символа. Этот параметр не может быть равен NULL, даже если для *нформат*задано значение GGO_NATIVE.
...
*лппоинтс*<br/>
Указывает на массив структур данных [Point](/windows/win32/api/windef/ns-windef-point) или `CPoint` объектов, где размещаются конечные точки линии и контрольные точки кривой.
...
Функция `GetPixel` члена имеет две формы. Первый принимает два значения координат; второй принимает либо структуру [Point](/windows/win32/api/windef/ns-windef-point) , либо объект [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) .
...
Дополнительные сведения см. в разделе [PolyLine](/windows/win32/api/wingdi/nf-wingdi-polyline) в Windows SDK.
...
Текущее состояние ограничивающего прямоугольника, если функция выполнена успешно. Как и *Флаги*, возвращаемое значение может быть сочетанием значений **DCB_** :
