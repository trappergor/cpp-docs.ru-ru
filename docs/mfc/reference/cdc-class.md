...
|Имя|Description|
|----------|-----------------|
|[CDC:: Абортдок](#abortdoc)|Завершает текущее задание печати, удаляя все, что приложение записало на устройство с момента последнего вызова функции члена `StartDoc`.|
...
|[CDC:: аркто](#arcto)|Рисование эллиптической дуги. Эта функция аналогична `Arc`, за исключением того, что текущая позицией обновляется.|
|[CDC:: Attach](#attach)|Присоединяет контекст устройства Windows к этому объекту `CDC`.|
...
|[CDC::D Елетедк](#deletedc)|Удаляет контекст устройства Windows, связанный с данным объектом `CDC`.|
|[CDC::D Елететемпмап](#deletetempmap)|Вызывается обработчиком `CWinApp` времени простоя для удаления любого временного объекта `CDC`, созданного `FromHandle`. Также отсоединяет контекст устройства.|
|[CDC::D етач](#detach)|Отсоединяет контекст устройства Windows от этого объекта `CDC`.|
...
|[CDC::D Равтекст](#drawtext)|Рисует форматированный текст в указанном прямоугольнике.|
...
|[CDC:: Енддок](#enddoc)|Завершает задание печати, запущенное функцией-членом `StartDoc`.|
...
|[CDC:: Жеткуррентбитмап](#getcurrentbitmap)|Возвращает указатель на текущий выбранный объект `CBitmap`.|
|[CDC:: Жеткуррентбруш](#getcurrentbrush)|Возвращает указатель на текущий выбранный объект `CBrush`.|
|[CDC:: Жеткуррентфонт](#getcurrentfont)|Возвращает указатель на текущий выбранный объект `CFont`.|
|[CDC:: Жеткуррентпалетте](#getcurrentpalette)|Возвращает указатель на текущий выбранный объект `CPalette`.|
|[CDC:: Жеткуррентпен](#getcurrentpen)|Возвращает указатель на текущий выбранный объект `CPen`.|
...
|[CDC::P Лайметафиле](#playmetafile)|Воспроизводит содержимое указанного метафайла на заданном устройстве. В улучшенной версии `PlayMetaFile` отображается изображение, хранящееся в заданном метафайле расширенного формата. Метафайл может воспроизводиться любое количество раз.|
...
|[CDC:: Релеасеаттрибдк](#releaseattribdc)|Освобождает `m_hAttribDC`, контекст устройства атрибута.|
|[CDC:: Релеасеаутпутдк](#releaseoutputdc)|Освобождает `m_hDC`, выходной контекст устройства.|
|[CDC:: Ресетдк](#resetdc)|Обновляет контекст устройства `m_hAttribDC`.|
|[CDC:: Ресторедк](#restoredc)|Восстанавливает контекст устройства в предыдущее состояние, сохраненное с помощью `SaveDC`.|
...
|[CDC:: Селектстоккобжект](#selectstockobject)|Выбирает одно из стандартных геоакций, кистей или шрифтов, предоставляемых Windows.|
...
|[CDC:: Сетаутпутдк](#setoutputdc)|Задает `m_hDC`, контекст вывода устройства.|
|[CDC:: SetPixel](#setpixel)|Устанавливает пиксел в указанной точке в ближайшее приближение указанного цвета.|
|[CDC:: Сетпикселв](#setpixelv)|Задает пиксель с заданными координатами до ближайшей аппроксимации указанного цвета. `SetPixelV` работает быстрее, чем `SetPixel`, поскольку ему не нужно возвращать значение цвета, нарисованное на момент рисования.|
...
|Имя|Description|
|----------|-----------------|
|[CDC:: m_hAttribDC](#m_hattribdc)|Контекст атрибута-устройства, используемый данным объектом `CDC`.|
|[CDC:: m_hDC](#m_hdc)|Контекст вывода устройства, используемый данным объектом `CDC`.|

## <a name="remarks"></a>Remarks

Объект `CDC` предоставляет функции-члены для работы с контекстом устройства, например дисплеем или принтером, а также членами для работы с контекстом экрана, связанным с клиентской областью окна.

Все рисование с помощью функций-членов объекта `CDC`. Класс предоставляет функции-члены для операций контекста устройства, работы с инструментами рисования, выбора объектов GDI и работы с цветами и палитрами. Он также предоставляет функции элементов для получения и установки атрибутов рисования, сопоставления, работы с окном просмотра, работы с областью окна, преобразования координат, работы с регионами, обрезки, рисования линий и рисования простых фигур, эллипсов и многоугольников. Функции-члены также предоставляются для рисования текста, работы со шрифтами, использования escape-символов принтера, прокрутки и воспроизведения метафайлов.

Чтобы использовать объект `CDC`, создайте его, а затем вызовите его функции-члены, которые используют параллельные функции Windows, использующие контексты устройств.

> [!NOTE]
>  В Windows 95/98 все экранные координаты ограничены 16 битами. Таким образом, значение **int** , передаваемое функции-члену `CDC`, должно находиться в диапазоне от-32768 до 32767.

Для конкретных применений библиотека Microsoft Foundation Class предоставляет несколько классов, производных от `CDC`. `CPaintDC` инкапсулирует вызовы `BeginPaint` и `EndPaint`. `CClientDC` управляет контекстом вывода, связанным с клиентской областью окна. `CWindowDC` управляет контекстом экрана, связанным со всем окном, включая его фрейм и элементы управления. `CMetaFileDC` связывает контекст устройства с метафайлом.

`CDC` предоставляет две функции-члены- [Layout](#getlayout) и [сетлайаут](#setlayout)для обратного разметки контекста устройства, который не наследует его макет из окна. Ориентация справа налево необходима для приложений, написанных для языков и региональных параметров, таких как арабский или иврит, в которых макет символов не является европейским стандартом.

`CDC` содержит два контекста устройств, [m_hDC](#m_hdc) и [m_hAttribDC](#m_hattribdc), которые при создании объекта `CDC` ссылаются на одно и то же устройство. `CDC` направляет все выходные вызовы GDI в `m_hDC` и большинство вызовов GDI атрибута для `m_hAttribDC`. (Пример вызова атрибута `GetTextColor`, тогда как `SetTextColor` является выходным вызовом.)

Например, платформа использует эти два контекста устройств для реализации объекта `CMetaFileDC`, который будет передавать выходные данные в метафайл при чтении атрибутов физического устройства. Предварительный просмотр реализуется в платформе аналогичным образом. Вы также можете использовать два контекста устройств аналогичным образом в коде, зависящем от приложения.

Иногда может потребоваться информация о метриках как в `m_hDC`, так `m_hAttribDC` в контекстах устройств. Эта возможность обеспечивается следующими парами функций:
...
##  <a name="abortdoc"></a>CDC:: Абортдок
...
Если приложение обнаруживает ошибку печати или отмененную операцию печати, не следует пытаться завершить операцию с помощью функций-членов [енддок](#enddoc) или `AbortDoc` класса `CDC`. GDI автоматически прерывает операцию, прежде чем возвращать значение ошибки.

Если приложение отображает диалоговое окно, позволяющее пользователю отменить операцию печати, необходимо вызвать `AbortDoc` перед уничтожением диалогового окна.

Если для запуска задания печати использовался диспетчер печати, то при вызове `AbortDoc` удаляется все задание из очереди — принтер ничего не получает. Если диспетчер печати не использовался для запуска задания печати, данные могли быть отправлены на принтер до вызова `AbortDoc`. В этом случае драйвер принтера будет сбрасывать принтер (по возможности) и закрыл задание печати.

### <a name="example"></a>Пример

  См. пример для [CDC:: стартдок](#startdoc).

##  <a name="abortpath"></a>CDC:: Абортпас
...
##  <a name="addmetafilecomment"></a>CDC:: Аддметафилекоммент
...
##  <a name="alphablend"></a>CDC:: Алфабленд
...
##  <a name="anglearc"></a>CDC:: Англеарк
...
*птенд*<br/>
Задает координаты x и y точки, определяющей конечную точку дуги (в логических единицах). Эта точка не обязательно должна находиться в точности на дуги. Для этого параметра можно передать либо структуру `POINT`, либо объект `CPoint`.
...
*птенд*<br/>
Задает координаты x и y точки, определяющей конечную точку дуги (в логических единицах). Эта точка не обязательно должна находиться в точности на дуги. Для этого параметра можно передать либо `POINT` структуру данных, либо объект `CPoint`.
...
Используйте эту функцию члена для присоединения *HDC* к объекту `CDC`.
...
Параметр *HDC* хранится как в `m_hDC`, так и в контексте устройства вывода, а в `m_hAttribDC`— в контексте атрибута устройства.

##  <a name="beginpath"></a>CDC:: Бегинпас
...
После открытия скобки пути приложение может начать вызывать функции рисования GDI для определения точек, находящихся в пути. Приложение может закрыть открывающую скобку пути, вызвав функцию члена `EndPath`. Когда приложение вызывает `BeginPath`, все предыдущие пути отбрасываются.
...
*псркдк*<br/>
Указатель на объект `CDC`, определяющий контекст устройства, из которого будет скопирован точечный рисунок. Он должен иметь значение NULL, если *двроп* указывает операцию точечной операции, которая не включает источник.
...
Приложение может выстроить области окна или клиента на границах байтов, чтобы гарантировать, что операции `BitBlt` выполняются в прямоугольниках с выровняйтем по байтам. (Задайте флаги CS_BYTEALIGNWINDOW или CS_BYTEALIGNCLIENT при регистрации классов окон.)

`BitBlt` операции над прямоугольниками с выкрашенными по байтам значительно быстрее, чем `BitBlt` операции с прямоугольниками, не поддерживающими округление байтов. Если вы хотите задать стили классов, такие как выравнивание байтов для собственного контекста устройства, необходимо зарегистрировать класс окна, а не полагаться на классы Microsoft Foundation для вас. Используйте глобальную функцию [афксрегистервндкласс](../../mfc/reference/application-information-and-management.md#afxregisterwndclass).
...
Если точечные рисунки назначения, источника и шаблона имеют разные форматы цвета, функция `BitBlt` преобразует растровые изображения исходного кода и шаблона в соответствии с назначением. Цвет переднего плана и фона целевого растрового изображения используются при преобразовании.

Когда функция `BitBlt` преобразует монохромное растровое изображение в цвет, оно устанавливает белые биты (1) в цвет фона, а черные биты (0) — в цвет переднего плана. Используются цвет переднего плана и фона конечного контекста устройства. Чтобы преобразовать цвет в монохромный режим, `BitBlt` устанавливает Пиксели, соответствующие цвету фона, белым, и устанавливает все остальные Пиксели в черные. `BitBlt` использует цвет переднего плана и фона контекста устройства цвета для преобразования цветов в монохромные.

Обратите внимание, что не все контексты устройств поддерживают `BitBlt`. Чтобы проверить, поддерживает ли данный контекст устройства `BitBlt`, используйте функцию-член `GetDeviceCaps` и укажите индекс РАСТЕРКАПС.
...
*птстарт*<br/>
Задает координаты x и y точки, определяющей начальную точку аккорда (в логических единицах). Эта точка не обязательно должна находиться точно в соответствии с аккордом. Для этого параметра можно передать либо структуру `POINT`, либо объект `CPoint`.
...
Рисунок, рисуемый функцией `Chord`, расширяется до, но не включает в себя правую и нижнюю координаты. Это означает, что высота фигуры имеет значение *y2* - *Y1* , а ширина фигуры равна *x2* - *x1*.
...
Функция закрывает рисунок, рисуя линию от текущей позиции до первой точки фигуры (обычно это точка, заданная в последнем вызове функции-члена `MoveTo`), и соединяет линии с помощью стиля соединения линий. Если фигура закрывается с помощью функции-члена `LineTo` вместо `CloseFigure`, для создания угла вместо объединения используются конечные точки. `CloseFigure` следует вызывать только при наличии скобки открытого пути в контексте устройства.

Рисунок в пути открыт, если он явно не закрыт с помощью этой функции. (Фигура может быть открыта, даже если текущая и начальная точки фигуры совпадают.) Любая линия или кривая, добавленная в путь после `CloseFigure`, запускает новую фигуру.

##  <a name="createcompatibledc"></a>CDC:: Креатекомпатибледк
...
##  <a name="createdc"></a>CDC:: Креатедк
...
*лпсздривернаме*<br/>
Указывает на строку, завершающуюся нулем, которая указывает имя файла (без расширения) драйвера устройства (например, "EPSON"). Для этого параметра также можно передать объект `CString`.

*лпсздевиценаме*<br/>
Указывает на строку, завершающуюся нулем, которая указывает имя конкретного устройства, которое должно поддерживаться (например, "EPSON FX-80"). Параметр *лпсздевиценаме* используется, если модуль поддерживает более одного устройства. Для этого параметра также можно передать объект `CString`.

*лпсзаутпут*<br/>
Указывает на строку, завершающуюся нулем, которая указывает имя файла или устройства для физического выходного носителя (порт File или Output). Для этого параметра также можно передать объект `CString`.

*лпинитдата*<br/>
Указывает на структуру `DEVMODE`, содержащую данные инициализации для драйвера устройства. Функция Windows `DocumentProperties` извлекает эту структуру, заполненную для данного устройства. Параметр *лпинитдата* должен иметь значение null, если драйвер устройства использует инициализацию по умолчанию (если таковая имеется), указанную пользователем с помощью панели управления.
...
*лпсздривернаме*<br/>
Указывает на строку, завершающуюся нулем, которая указывает имя файла (без расширения) драйвера устройства (например, "EPSON"). Для этого параметра можно передать объект `CString`.

*лпсздевиценаме*<br/>
Указывает на строку, завершающуюся нулем, которая указывает имя конкретного устройства, которое должно поддерживаться (например, "EPSON FX-80"). Параметр *лпсздевиценаме* используется, если модуль поддерживает более одного устройства. Для этого параметра можно передать объект `CString`.

*лпсзаутпут*<br/>
Указывает на строку, завершающуюся нулем, которая указывает имя файла или устройства для физического выходного носителя (файла или порта). Для этого параметра можно передать объект `CString`.

*лпинитдата*<br/>
Указывает на данные инициализации драйвера устройства для конкретных устройств. Параметр *лпинитдата* должен иметь значение null, если драйвер устройства использует инициализацию по умолчанию (если таковая имеется), указанную пользователем с помощью панели управления. Сведения об инициализации для конкретного устройства см. в разделе `CreateDC`.
...
##  <a name="deletedc"></a>CDC::D Елетедк
...
Функция члена `DeleteDC` удаляет контексты устройств Windows, связанные с `m_hDC` в текущем объекте `CDC`. Если этот `CDC` объект является последним активным контекстом устройства для данного устройства, устройство получает уведомления, а все системные ресурсы, используемые устройством, освобождаются.

Приложение не должно вызывать `DeleteDC`, если объекты были выбраны в контексте устройства. Прежде чем удалить объекты, их необходимо выбрать из контекста устройства.
...
Функция `DeleteDC` обычно используется для удаления контекстов устройств, созданных с помощью [креатедк](#createdc), [CREATE](#createic)или [креатекомпатибледк](#createcompatibledc).

### <a name="example"></a>Пример

  См. пример для [кпринтдиалог:: жетпринтердк](../../mfc/reference/cprintdialog-class.md#getprinterdc).

##  <a name="deletetempmap"></a>CDC::D Елететемпмап

Автоматически вызывается обработчиком `CWinApp` времени простоя, `DeleteTempMap` удаляет все временные `CDC` объекты, созданные `FromHandle`, но не уничтожает дескрипторы контекста устройства (`hDC`), временно связанные с объектами `CDC`.
...
Вызовите эту функцию, чтобы отсоединить `m_hDC` (контекст устройства вывода) от объекта `CDC` и задать для обоих `m_hDC` и `m_hAttribDC` значение NULL.
...
Вызовите его в цикле, как показано в примере расположения мыши, чтобы дать визуальную обратную связь. При вызове `DrawDragRect`предыдущий прямоугольник удаляется и рисуется новый. Например, при перетаскивании прямоугольника на экране `DrawDragRect` удаляет исходный прямоугольник и перерисовывает новый в новом положении. По умолчанию `DrawDragRect` выводит прямоугольник с помощью кисти полутонов для устранения мерцания и создания внешнего вида плавно перемещающегося прямоугольника.
...
*нфлагс*<br/>
Флаги, указывающие тип рисуемой границы. См. раздел `DrawEdge` в Windows SDK для таблицы значений параметра. Для диагональных линий флаги BF_RECT задают конечную точку вектора, ограниченного параметром Rectangle.
...
##  <a name="drawframecontrol"></a>CDC::D Равфрамеконтрол
...
Этот код выводит область захвата размера в правом нижнем углу окна. Он подходит для обработчика `OnPaint` диалогового окна, в котором нет стилей и обычно не содержит других элементов управления (например, строки состояния), которые могут дать ему захват размера.

[!code-cpp[NVC_MFCDocView#34](../../mfc/codesnippet/cpp/cdc-class_6.cpp)]

##  <a name="drawicon"></a>CDC::D Равикон

Отображает значок на устройстве, представленном текущим объектом `CDC`.
...
##  <a name="drawtext"></a>CDC::D Равтекст
...
*нкаунт*<br/>
Указывает число символов в строке. Если *нкаунт* имеет значение-1, то предполагается, что *лпсзстринг* является длинным указателем на строку, завершающуюся нулем, а `DrawText` автоматически выполняет подсчет символов.
...
Эта функция члена использует выбранный шрифт, цвет текста и цвет фона для рисования текста в контексте устройства. Если не используется формат DT_NOCLIP, `DrawText` обрезает текст, чтобы текст не отображался за пределами данного прямоугольника. Предполагается, что все форматирование имеет несколько строк, если не задан формат DT_SINGLELINE.

Если выбранный шрифт слишком велик для указанного прямоугольника, функция-член `DrawText` не будет пытаться заменить шрифт меньшего размера.
...
Если установлен флаг выравнивания текста TA_UPDATECP (см. раздел [CDC:: сеттексталигн](#settextalign)), `DrawText` будет отображать текст, начиная с текущей позиции, а не слева от данного прямоугольника. `DrawText` не будет переносить текст при установке флага TA_UPDATECP (то есть флаг DT_WORDBREAK не будет оказывать никакого влияния).

Цвет текста может быть задан с помощью [CDC:: сеттекстколор](#settextcolor).

##  <a name="drawtextex"></a>CDC::D Равтекстекс
...
Фигура, изображенная этой функцией, расширяется до, но не включает, правую и нижнюю координаты. Это означает, что высота фигуры имеет значение *y2* - *Y1* , а ширина фигуры равна *x2* - *x1*.

Если ширина или высота ограничивающего прямоугольника равна 0, эллипс не рисуется.

##  <a name="enddoc"></a>CDC:: Енддок
...
Если приложение обнаруживает ошибку печати или отмененную операцию печати, не следует пытаться завершить операцию с помощью `EndDoc` или [абортдок](#abortdoc). GDI автоматически прерывает операцию, прежде чем возвращать значение ошибки.
...
Обратите внимание, что новые возможности C++ Microsoft Visual позволяют использовать обычную функцию в качестве функции, передаваемой в `EnumObjects`. Адрес, передаваемый `EnumObjects`, является указателем на функцию, экспортированную с помощью **экспорта** , и с соглашением о вызовах Pascal. В приложениях, защищенных в режиме защиты, не нужно создавать эту функцию с помощью функции Windows МакепроЦинстанце или освобождать функцию после использования с функцией ФрипроЦинстанце Windows.
...
Для компиляции программ Microsoft Foundation обычно используются параметры компилятора/GA и/жес. Параметр компилятора/GW не используется с классами Microsoft Foundation. (Если вы используете функцию Windows `MakeProcInstance`, необходимо явно привести возвращаемый указатель функции от ФАРПРОК к типу, который требуется в этом API.) Интерфейсы регистрации обратного вызова теперь являются типобезопасными (необходимо передать указатель на функцию, указывающий на правильный тип функции для конкретного обратного вызова).
...
Для программирования на Win32 `CDC` теперь предоставляет шесть функций-членов, которые заменяют соответствующие escape-последовательности принтера:
...
*лпрект*<br/>
Задает прямоугольник. Также может быть объектом `CRect`.
...
Запрещает прорисовку в недопустимых областях окна, исключая обновленную область в окне из области обрезки, связанной с объектом `CDC`.
...
##  <a name="extfloodfill"></a>CDC:: Екстфлудфилл
...
Эта функция-член обеспечивает большую гибкость, чем `FloodFill`, так как в *нфиллтипе*можно указать тип заливки.
...
`ExtFloodFill`поддерживаются только контексты устройств памяти и устройства, поддерживающие технологию растровой изображения. Дополнительные сведения см. в описании функции члена [жетдевицекапс](#getdevicecaps) .
...
*str*<br/>
Объект `CString`, содержащий указанные символы для рисования.
...
Если *ноптионс* равен 0, а *лпрект* имеет значение null, функция записывает текст в контекст устройства без использования прямоугольной области. По умолчанию текущая позиция не используется и не обновляется функцией. Если приложению необходимо обновить текущую точку при вызове `ExtTextOut`, приложение может вызвать `CDC` функцию члена [сеттексталигн](#settextalign) с параметром *нфлагс* , имеющим значение TA_UPDATECP. Если этот флаг установлен, Windows игнорирует *x* и *y* при последующих вызовах `ExtTextOut` и использует текущую точку. Когда приложение использует TA_UPDATECP для обновления текущей позицией, `ExtTextOut` устанавливает текущую точку в конец предыдущей строки текста или в расположение, указанное последним элементом массива, на который указывает *лпдксвидсс*, в зависимости от того, какое значение больше.

##  <a name="fillpath"></a>CDC:: Филлпас
...
##  <a name="fillrect"></a>CDC:: Филлрект
...
Необходимо либо создать кисть с помощью функций члена [Кбруш](../../mfc/reference/cbrush-class.md) [креатехатчбруш](../../mfc/reference/cbrush-class.md#createhatchbrush), [креатепаттернбруш](../../mfc/reference/cbrush-class.md#createpatternbrush)и [креатесолидбруш](../../mfc/reference/cbrush-class.md#createsolidbrush), либо получить с помощью функции `GetStockObject` Windows.

При заполнении указанного прямоугольника `FillRect` не включает правую и нижнюю стороны прямоугольника. GDI заполняет прямоугольник вплоть до, но не включает, правый столбец и нижнюю строку независимо от текущего режима сопоставления. `FillRect` сравнивает значения `top`, `bottom`, `left`и `right` элементов указанного прямоугольника. Если `bottom` меньше или равно `top`или если `right` меньше или равно `left`, прямоугольник не рисуется.

`FillRect` похожа на [CDC:: филлсолидрект](#fillsolidrect); Однако `FillRect` принимает кисть и, следовательно, может использоваться для заполнения прямоугольника сплошным цветом, раскрашенным цветом, заштрихованными кистями или шаблоном. `FillSolidRect` использует только сплошные цвета (обозначенные параметром COLORREF). `FillRect` обычно медленнее, чем `FillSolidRect`.

##  <a name="fillrgn"></a>CDC:: Филлргн
...
Кисть необходимо создать либо с помощью `CBrush`ных функций `CreateHatchBrush`, `CreatePatternBrush`, `CreateSolidBrush`или получения `GetStockObject`.

### <a name="example"></a>Пример

  См. пример для [кргн:: креатераундректргн](../../mfc/reference/crgn-class.md#createroundrectrgn).

##  <a name="fillsolidrect"></a>CDC:: Филлсолидрект
...
*лпрект*<br/>
Задает ограничивающий прямоугольник (в логических устройствах). Для этого параметра можно передать либо указатель на структуру данных [Rect](/windows/win32/api/windef/ns-windef-rect) , либо объект `CRect`.
...
`FillSolidRect` очень похожа на [CDC:: филлрект](#fillrect); Однако `FillSolidRect` использует только сплошные цвета (обозначенные параметром COLORREF), а `FillRect` принимает кисть и, следовательно, может использоваться для заливки прямоугольника сплошным цветом, раскрашенным цветом, заштрихованными кистями или шаблоном. `FillSolidRect` обычно выполняется быстрее `FillRect`.

> [!NOTE]
>  При вызове `FillSolidRect`цвет фона, ранее заданный с помощью [сетбкколор](#setbkcolor), задается цветом, указанным в *CLR*.

##  <a name="flattenpath"></a>CDC:: Флаттенпас
...
##  <a name="floodfill"></a>CDC:: Флудфилл
...
Предполагается, что область ограничена, как указано в *крколор*. Функция `FloodFill` начинается с точки, заданной *x* и *y* , и продолжается во всех направлениях до границы цвета.

Функция-член `FloodFill` поддерживает только контексты устройств памяти и устройства, поддерживающие технологию растровой изображения. Дополнительные сведения о функции RC_BITBLT см. в разделе функция элемента `GetDeviceCaps`.

Функция `ExtFloodFill` предоставляет аналогичные возможности, но большую гибкость.
...
*лпрект*<br/>
Указывает на структуру [Rect](/windows/win32/api/windef/ns-windef-rect) или объект [крект](../../atl-mfc-shared/reference/crect-class.md) , который содержит логические координаты левого верхнего и правого нижнего угла прямоугольника. Для этого параметра также можно передать объект `CRect`.
...
Если координата `bottom` прямоугольника меньше или равна `top`или если `right` меньше или равна `left`, прямоугольник не рисуется.

Граница, рисуемая `FrameRect`, находится в том же положении, что и граница, нарисованная функцией-членом `Rectangle`, используя одинаковые координаты (если `Rectangle` использует перо, которое равно 1 логическому устройству). Внутренняя часть прямоугольника не заполняется `FrameRect`.
...
*пргн*<br/>
Указывает на объект `CRgn`, который определяет область, которая должна быть заключена в границу. Координаты для данного региона указываются в логических единицах.

*пбруш*<br/>
Указывает на объект `CBrush`, который определяет кисть, используемую для рисования границы.
...
##  <a name="getaspectratiofilter"></a>CDC:: Жетаспектратиофилтер
...
##  <a name="getbkmode"></a>CDC:: Жетбкмоде
...
##  <a name="getboundsrect"></a>CDC:: Жетбаундсрект
...
##  <a name="getbrushorg"></a>CDC:: Жетбрушорг
...
##  <a name="getcharacterplacement"></a>CDC:: Жетчарактерплацемент
...
*лпабк*<br/>
Указывает на массив структур [ABC](/windows/win32/api/wingdi/ns-wingdi-abc) , которые получают ширину символов при возврате функции. Этот массив должен содержать по крайней мере столько `ABC` структур, сколько символов в диапазоне, указанном параметрами *нфирстчар* и *нластчар* .
...
Когда функция-член `GetCharABCWidths` получает отрицательные значения ширины "A" или "C" для символа, этот символ включает в себя зависание или перестает отвечать.

Чтобы преобразовать ширину ABC в единицы проектирования шрифта, приложение должно создать шрифт, высота которого (как указано в `lfHeight` элемента структуры [LOGFONT](/windows/win32/api/wingdi/ns-wingdi-logfontw) ) равна значению, хранящемуся в `ntmSizeEM` элементе структуры [невтекстметрик](/windows/win32/api/wingdi/ns-wingdi-newtextmetricw) . (Значение элемента `ntmSizeEM` можно получить, вызвав функцию Windows [EnumFontFamilies](/windows/win32/api/wingdi/nf-wingdi-enumfontfamiliesw) .)
...
##  <a name="getcharabcwidthsi"></a>CDC:: Жетчарабквидсси
...
*лпабк*<br/>
Указатель на массив [ABC](/windows/win32/api/wingdi/ns-wingdi-abc) , принимающий ширину символов. Этот массив должен содержать по крайней мере столько структур `ABC`, сколько индексов глифов задается параметром *CGI* .
...
##  <a name="getcharwidth"></a>CDC:: Жетчарвидс

Извлекает ширину отдельных символов в последовательных группах символов из текущего шрифта с помощью `m_hAttribDC`, контекста входного устройства.
...
##  <a name="getcharwidthi"></a>CDC:: Жетчарвидси
...
##  <a name="getclipbox"></a>CDC:: Жетклипбокс
...
##  <a name="getcoloradjustment"></a>CDC:: Жетколораджустмент
...
##  <a name="getcurrentbitmap"></a>CDC:: Жеткуррентбитмап
...
Указатель на объект `CBitmap`, если он успешно выполнен; в противном случае — NULL.

### <a name="remarks"></a>Remarks

Эта функция члена может возвращать временные объекты.

##  <a name="getcurrentbrush"></a>CDC:: Жеткуррентбруш
...
Указатель на объект `CBrush`, если он успешно выполнен; в противном случае — NULL.

### <a name="remarks"></a>Remarks

Эта функция члена может возвращать временные объекты.

##  <a name="getcurrentfont"></a>CDC:: Жеткуррентфонт
...
Указатель на объект `CFont`, если он успешно выполнен; в противном случае — NULL.

### <a name="remarks"></a>Remarks

Эта функция члена может возвращать временные объекты.

##  <a name="getcurrentpalette"></a>CDC:: Жеткуррентпалетте
...
Указатель на объект `CPalette`, если он успешно выполнен; в противном случае — NULL.

### <a name="remarks"></a>Remarks

Эта функция члена может возвращать временные объекты.

##  <a name="getcurrentpen"></a>CDC:: Жеткуррентпен
...
Указатель на объект `CPen`, если он успешно выполнен; в противном случае — NULL.

### <a name="remarks"></a>Remarks

Эта функция члена может возвращать временные объекты.

##  <a name="getcurrentposition"></a>CDC:: Жеткуррентпоситион
...
Текущее расположение как объект `CPoint`.

### <a name="remarks"></a>Remarks

Текущую точку можно задать с помощью функции члена `MoveTo`.

##  <a name="getdcbrushcolor"></a>CDC:: Жетдкбрушколор
...
##  <a name="getdcpencolor"></a>CDC:: Жетдкпенколор
...
##  <a name="getdevicecaps"></a>CDC:: Жетдевицекапс
...
##  <a name="getfontdata"></a>CDC:: Жетфонтдата
...
Иногда приложение может использовать функцию-член `GetFontData` для сохранения шрифта TrueType с документом. Для этого приложение определяет, можно ли внедрить шрифт, а затем извлекает весь файл шрифта, указывая 0 для параметров *двтабле*, *двоффсет*и *cbData* .

Приложения могут определить, можно ли внедрить шрифт, проверив элемент `otmfsType` структуры [аутлинетекстметрик](/windows/win32/api/wingdi/ns-wingdi-outlinetextmetricw) . Если задан бит 1 из `otmfsType`, встраивание для шрифта не разрешено. Если бит 1 снят, шрифт может быть внедрен. Если задан бит 2, то внедрение будет доступно только для чтения.

Если приложение пытается использовать эту функцию для получения сведений для шрифта, отличного от TrueType, функция-член `GetFontData` возвращает значение – 1.

##  <a name="getfontlanguageinfo"></a>CDC:: Жетфонтлангуажеинфо
...
##  <a name="getglyphoutline"></a>CDC:: Жетглифаутлине
...
##  <a name="gethalftonebrush"></a>CDC:: Жесалфтонебруш
...
![Подробные сведения об отливке пера](../../mfc/reference/media/vc318s1.gif "Сведения о штрихе сглаживающего пера")

##  <a name="getkerningpairs"></a>CDC:: Жеткернингпаирс
...
*лпкрнпаир*<br/>
Указывает на массив структур `KERNINGPAIR`, которые получают пары кернинга при возврате функции. Этот массив должен содержать по меньшей мере столько структур, сколько указано в *нпаирс*. Если этот параметр имеет значение NULL, функция возвращает общее число пар кернинга для шрифта.
...
##  <a name="getlayout"></a>CDC:: AutoLayout
...
##  <a name="getmapmode"></a>CDC:: Жетмапмоде
...
Описание режимов сопоставления см. в описании функции-члена `SetMapMode`.

> [!NOTE]
>  При вызове [сетлайаут](#setlayout) для изменения макета контроллера домена с письмом справа налево `SetLayout` автоматически изменяет режим сопоставления на MM_ISOTROPIC. Следовательно, любой последующий вызов `GetMapMode` будет возвращать MM_ISOTROPIC.

##  <a name="getmiterlimit"></a>CDC:: Жетмитерлимит
...
##  <a name="getnearestcolor"></a>CDC:: Жетнеарестколор
...
##  <a name="getoutlinetextmetrics"></a>CDC:: Жетаутлинетекстметрикс
...
*лпотм*<br/>
Указывает на структуру `OUTLINETEXTMETRIC`. Если этот параметр имеет значение NULL, функция возвращает размер буфера, необходимого для получения сведений о метрике.
...
Структура [аутлинетекстметрик](/windows/win32/api/wingdi/ns-wingdi-outlinetextmetricw) содержит большую часть сведений о метрике шрифта, предоставляемых в формате TrueType, включая структуру [текстметрик](/windows/win32/api/wingdi/ns-wingdi-textmetricw) . Последние четыре элемента структуры `OUTLINETEXTMETRIC` являются указателями на строки. Приложения должны выделить место для этих строк в дополнение к пространству, требуемому для других элементов. Поскольку ограничение на размер строк не ограничено, простейшим методом выделения памяти является получение требуемого размера путем указания значения NULL для *лпотм* при первом вызове функции `GetOutlineTextMetrics`.

##  <a name="getoutputcharwidth"></a>CDC:: Жетаутпутчарвидс
...
##  <a name="getoutputtabbedtextextent"></a>CDC:: Жетаутпуттаббедтекстекстент
...
Текущая область отсечения не смещает ширину и высоту, возвращаемые функцией `GetOutputTabbedTextExtent`.
...
##  <a name="getoutputtextextent"></a>CDC:: Жетаутпуттекстекстент
...
##  <a name="getoutputtextmetrics"></a>CDC:: Жетаутпуттекстметрикс

Извлекает метрики для текущего шрифта с помощью `m_hDC`, контекста выходного устройства.
...
*лппоинтс*<br/>
Указывает на массив структур данных [точки](/windows/win32/api/windef/ns-windef-point) или `CPoint` объектов, где размещаются конечные точки линии и контрольные точки кривой.
...
Если параметр *нкаунт* имеет ненулевое значение, число перечисленных точек. Если *нкаунт* имеет значение 0, общее количество точек в пути (и `GetPath` ничего не записывает в буферы). Если *нкаунт* не равно нулю и меньше числа точек в пути, возвращаемое значение равно-1.

### <a name="remarks"></a>Remarks

Контекст устройства должен содержать закрытый путь. Точки пути возвращаются в логических координатах. Точки хранятся по пути в координатах устройства, поэтому `GetPath` изменяет точки из координат устройства на логические координаты с помощью обратного текущего преобразования. Функцию члена `FlattenPath` можно вызвать до `GetPath`, чтобы преобразовать все кривые в контуре в сегменты линии.

### <a name="example"></a>Пример

  См. пример для [CDC:: бегинпас](#beginpath).

##  <a name="getpixel"></a>CDC:: с точкой
...
Функция члена `GetPixel` имеет две формы. Первый принимает два значения координат; второй принимает либо структуру [Point](/windows/win32/api/windef/ns-windef-point) , либо объект [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) .

##  <a name="getpolyfillmode"></a>CDC:: Жетполифиллмоде
...
Описание режимов заливки многоугольников см. в описании функции-члена `SetPolyFillMode`.
...
Режим рисования. Список значений режима рисования см. в описании функции-члена `SetROP2`.
...
##  <a name="getsafehdc"></a>CDC:: Жетсафехдк
...
##  <a name="getstretchbltmode"></a>CDC:: Жетстретчблтмоде
...
Режим растяжения по точечному рисунку определяет способ удаления информации из растровых изображений, которые растягиваются или сжимаются функцией-членом `StretchBlt`.
...
##  <a name="gettabbedtextextent"></a>CDC:: Жеттаббедтекстекстент
...
Текущая область отсечения не смещает ширину и высоту, возвращаемые функцией `GetTabbedTextExtent`.
...
##  <a name="gettextalign"></a>CDC:: Жеттексталигн
...
Флаги выравнивания текста определяют, как `TextOut` и `ExtTextOut` функции-члены выравнивать строку текста относительно начальной точки строки. Флаги выравнивания текста не обязательно являются одиночными битовыми флагами и могут быть равны 0. Чтобы проверить, установлен ли флаг, приложение должно выполнить следующие действия:
...
##  <a name="gettextcharacterextra"></a>CDC:: Жеттекстчарактерекстра
...
##  <a name="gettextcolor"></a>CDC:: Жеттекстколор
...
##  <a name="gettextextent"></a>CDC:: Жеттекстекстент
...
По умолчанию `GetTextExtent` предполагает, что текст, для которого извлекается измерение, задается вдоль горизонтальной линии (то есть escape-символ равен 0). При создании шрифта, указывающего на ненулевую escape-последовательность, необходимо явно преобразовать угол текста, чтобы получить размеры строки.

Текущая область отсечения не влияет на ширину и высоту, возвращаемые `GetTextExtent`.

Поскольку некоторые устройства не размещают символы в обычных массивах ячеек (то есть они выполняют кернинг), сумма экстентов символов в строке может не совпадать с экстентом строки.

##  <a name="gettextextentexpointi"></a>CDC:: Жеттекстекстентекспоинти
...
##  <a name="gettextextentpointi"></a>CDC:: Жеттекстекстентпоинти
...
##  <a name="gettextface"></a>CDC:: Жеттекстфаце
...
##  <a name="gettextmetrics"></a>CDC:: Жеттекстметрикс
...
##  <a name="getviewportext"></a>CDC:: Жетвиевпортекст
...
Экстенты x и y (в единицах устройства) в качестве объекта `CSize`.

##  <a name="getviewportorg"></a>CDC:: Жетвиевпорторг
...
Источник окна просмотра (в координатах устройства) в качестве объекта `CPoint`.

##  <a name="getwindow"></a>CDC::/Window
...
Указатель на объект `CWnd` в случае успеха; в противном случае — NULL.
...
##  <a name="getwindowext"></a>CDC:: Жетвиндовекст
...
Экстенты x и y (в логических единицах) в качестве объекта `CSize`.

##  <a name="getwindoworg"></a>CDC:: Жетвиндоворг
...
Источник окна (в логических координатах) в качестве объекта `CPoint`.

##  <a name="getworldtransform"></a>CDC:: Жетворлдтрансформ
...
##  <a name="gradientfill"></a>CDC:: Градиентфилл
...
##  <a name="graystring"></a>CDC:: Грайстринг
...
*лпфнаутпут*<br/>
Указывает адрес процедуры-экземпляра функции обратного вызова, предоставляемой приложением, которая будет рисовать строку. Дополнительные сведения см. в описании [функции обратного вызова](callback-functions-used-by-mfc.md#graystring)Windows `OutputFunc`. Если этот параметр имеет значение NULL, система использует функцию Windows `TextOut` для рисования строки, а *лпдата* считается длинным указателем на строку символов для вывода.
...
*нвидс*<br/>
Задает ширину (в логических единицах) прямоугольника, охватывающего строку. Если *нвидс* имеет значение 0, `GrayString` вычисляет ширину области, предполагая, что *лпдата* является указателем на строку.

*нхеигхт*<br/>
Задает высоту (в логических единицах) прямоугольника, охватывающего строку. Если *нхеигхт* имеет значение 0, `GrayString` вычисляет высоту области, предполагая, что *лпдата* является указателем на строку.

### <a name="return-value"></a>Возвращаемое значение

Ненулевое значение, если строка рисуется, или 0, если функция `TextOut` или предоставляемая приложением функция вывода вернула 0, или если недостаточно памяти для создания точечного рисунка памяти для затемнения.

### <a name="remarks"></a>Remarks

Функция затемняет текст, независимо от выбранной кисти и фона. Функция члена `GrayString` использует текущий выбранный шрифт. Перед использованием этой функции необходимо выбрать режим сопоставления MM_TEXT.

Приложение может нарисовать затемненные (серые) строки на устройствах, поддерживающих сплошной серый цвет, без вызова функции-члена `GrayString`. Системный цвет COLOR_GRAYTEXT является сплошным серым цветом системы, используемым для рисования отключенного текста. Приложение может вызвать функцию `GetSysColor` Windows для получения значения цвета COLOR_GRAYTEXT. Если цвет отличается от 0 (черный), приложение может вызвать функцию-член `SetTextColor`, чтобы задать цвет текста в значении цвета, а затем нарисовать строку напрямую. Если полученный цвет является черным, приложение должно вызвать `GrayString` для затемнения (серого) текста.

Если *лпфнаутпут* имеет значение null, GDI использует функцию [текста](/windows/win32/api/wingdi/nf-wingdi-textoutw) Windows, а *лпдата* — в качестве дальнего указателя на символ для вывода. Если выводимые символы не могут обрабатываться функцией-членом `TextOut` (например, строка хранится в виде точечного рисунка), приложение должно предоставить собственную выходную функцию.

Также обратите внимание, что все функции обратного вызова должны перехватывать исключения Microsoft Foundation перед возвратом в Windows, так как исключения не могут быть вызваны через границы обратного вызова. Дополнительные сведения об исключениях см. в статье [исключения](../../mfc/exception-handling-in-mfc.md).

Функция обратного вызова, передаваемая `GrayString`, должна использовать соглашение о вызовах `__stdcall` и должно быть экспортировано с помощью `__declspec`.

Если платформа находится в режиме предварительного просмотра, вызов функции-члена `GrayString` преобразуется в `TextOut` вызов, а функция обратного вызова не вызывается.
...
##  <a name="intersectcliprect"></a>CDC:: Интерсектклипрект
...
*лпрект*<br/>
Задает прямоугольник. Для этого параметра можно передать либо объект `CRect`, либо указатель на структуру `RECT`.
...
##  <a name="invertrect"></a>CDC:: Инвертрект
...
*лпрект*<br/>
Указывает на `RECT`, содержащий логические координаты прямоугольника для инвертирования. Для этого параметра также можно передать объект `CRect`.

### <a name="remarks"></a>Remarks

Инверсия — это логическая операция NOT, которая отражает биты каждого пикселя. При отображении монохромных экранов функция делает белые Пиксели черными и черными пикселами белым цветом. При отображении цвета инверсия зависит от того, как создаются цвета для отображения. Вызов `InvertRect` дважды с одним прямоугольником восстанавливает предыдущие цвета.
...
##  <a name="invertrgn"></a>CDC:: Инвертргн
...
Ненулевое значение, если объект `CDC` является контроллером домена принтера; в противном случае — 0.
...
*point*<br/>
Задает конечную точку для линии. Для этого параметра можно передать либо структуру `POINT`, либо объект `CPoint`.
...
##  <a name="lptohimetric"></a>CDC:: Лптохиметрик
...
*лпсизе*<br/>
Указывает на структуру `SIZE` или на объект `CSize`.
...
Контекст устройства атрибута для данного объекта `CDC`.
...
По умолчанию этот контекст устройства равен `m_hDC`. В общем случае `CDC` вызовы GDI, которые запрашивают сведения из контекста устройства, направляются в `m_hAttribDC`. Дополнительные сведения об использовании этих двух контекстов устройств см. в описании класса [CDC](../../mfc/reference/cdc-class.md) .

##  <a name="m_hdc"></a>CDC:: m_hDC

Контекст устройства вывода для данного объекта `CDC`.
...
По умолчанию `m_hDC` равно `m_hAttribDC`, контекст другого устройства, заключенный в `CDC`. В общем случае `CDC` вызовы GDI, которые создают выходные данные, переходят в контекст устройства `m_hDC`. Можно инициализировать `m_hDC` и `m_hAttribDC`, чтобы они указывали на различные устройства. Дополнительные сведения об использовании этих двух контекстов устройств см. в описании класса [CDC](../../mfc/reference/cdc-class.md) .
...
*point*<br/>
Задает новую точку. Для этого параметра можно передать либо структуру `POINT`, либо объект `CPoint`.

### <a name="return-value"></a>Возвращаемое значение

Координаты x и y предыдущего положения в виде объекта `CPoint`.

### <a name="example"></a>Пример

  См. пример для [крект:: центерпоинт](../../atl-mfc-shared/reference/crect-class.md#centerpoint).

##  <a name="offsetcliprgn"></a>CDC:: Оффсетклипргн
...
##  <a name="offsetviewportorg"></a>CDC:: Оффсетвиевпорторг
...
Предыдущее исходное окно просмотра (в координатах устройства) в качестве объекта `CPoint`.

##  <a name="offsetwindoworg"></a>CDC:: Оффсетвиндоворг
...
Предыдущий источник окна (в логических координатах) в качестве объекта `CPoint`.

##  <a name="operator_hdc"></a>CDC:: operator HDC

Этот оператор используется для получения маркера контекста устройства объекта `CDC`.
...
Не все контексты устройств поддерживают функцию `PatBlt`. Чтобы определить, поддерживает ли контекст устройства `PatBlt`, вызовите функцию-член `GetDeviceCaps` с индексом РАСТЕРКАПС и проверьте возвращаемое значение для флага RC_BITBLT.
...
*лпрект*<br/>
Задает ограничивающий прямоугольник. Для этого параметра можно передать либо объект `CRect`, либо указатель на структуру `RECT`.
...
*птенд*<br/>
Задает конечную точку дуги. Эта точка не обязательно должна находиться в точности на дуги. Для этого параметра можно передать либо структуру `POINT`, либо объект `CPoint`.
...
Фигура, изображенная этой функцией, расширяется до, но не включает в себя правую и нижнюю координаты. Это означает, что высота фигуры имеет значение *y2* - *Y1* , а ширина фигуры равна *x2* - *x1*. Ширина и высота ограничивающего прямоугольника должны быть больше 2 единиц и меньше 32 767 единиц.

### <a name="example"></a>Пример

[!code-cpp[NVC_MFCDocView#37](../../mfc/codesnippet/cpp/cdc-class_9.cpp)]

##  <a name="playmetafile"></a>CDC::P Лайметафиле
...
*лпбаундс*<br/>
Указывает на `RECT`ную структуру или объект `CRect`, содержащий координаты ограничивающего прямоугольника, используемого для вывода изображения. Координаты указываются в логических единицах.
...
Текущая позицией не используется и не обновляется функцией `PolyBezier`. Фигура не заполнена. Эта функция рисует линии с помощью текущего пера.

##  <a name="polybezierto"></a>CDC::P Олибезиерто
...
##  <a name="polydraw"></a>CDC::P Олидрав
...
Эта функция может использоваться для нарисовать непересекающиеся цифры вместо последовательных вызовов `CDC::MoveTo`, `CDC::LineTo`и `CDC::PolyBezierTo` функций элементов. Линии и сплайны рисуются с помощью текущего пера, а фигуры не заполнены. Если имеется активный путь, запущенный путем вызова функции-члена `CDC::BeginPath`, `PolyDraw` добавляет к пути. Точки, содержащиеся в массиве *лппоинтс* и в *лптипес* , указывают, является ли каждая точка частью `CDC::MoveTo`, `CDC::LineTo`или операции `CDC::BezierTo`. Также можно закрывать фигуры. Эта функция обновляет текущую точку.

### <a name="example"></a>Пример

  См. пример для [CDC:: бегинпас](#beginpath).

##  <a name="polygon"></a>CDC::P олигон
...
*лппоинтс*<br/>
Указывает на массив точек, указывающих вершины многоугольника. Каждая точка в массиве является структурой `POINT` или объектом `CPoint`.
...
Текущий режим заполнения многоугольников можно получить или задать с помощью функций-членов `GetPolyFillMode` и `SetPolyFillMode`.

### <a name="example"></a>Пример

[!code-cpp[NVC_MFCDocView#38](../../mfc/codesnippet/cpp/cdc-class_10.cpp)]

##  <a name="polyline"></a>CDC::P олилине
...
*лппоинтс*<br/>
Указывает на массив `POINT`ных структур или `CPoint` объектов для подключения.
...
Линии выводятся из первой точки через последующие точки с использованием текущего пера. В отличие от функции члена `LineTo`, функция `Polyline` не использует и не обновляет текущую точку.

Дополнительные сведения см. в разделе [Ломаная](/windows/win32/api/wingdi/nf-wingdi-polyline) линия в Windows SDK.

##  <a name="polylineto"></a>CDC::P Олилинето
...
##  <a name="polypolygon"></a>CDC::P Олиполигон
...
*лппоинтс*<br/>
Указывает на массив `POINT`ных структур или `CPoint` объектов, определяющих вершины многоугольников.
...
Каждый многоугольник, указанный в вызове функции `PolyPolygon`, должен быть закрыт. В отличие от многоугольников, созданных функцией члена `Polygon`, многоугольники, созданные `PolyPolygon`, не закрываются автоматически.

Функция создает два или более многоугольников. Для создания одного многоугольника приложение должно использовать функцию-член `Polygon`.

Текущий режим заполнения многоугольников можно получить или задать с помощью функций-членов `GetPolyFillMode` и `SetPolyFillMode`.

##  <a name="polypolyline"></a>CDC::P Олиполилине
...
##  <a name="ptvisible"></a>CDC::P Твисибле
...
*point*<br/>
Задает точку для проверки логических координат. Для этого параметра можно передать либо структуру `POINT`, либо объект `CPoint`.

### <a name="return-value"></a>Возвращаемое значение

Ненулевое значение, если указанная точка находится внутри вырезанной области; в противном случае — 0.

##  <a name="queryabort"></a>CDC:: Куеряборт
...
##  <a name="realizepalette"></a>CDC:: Реализепалетте
...
*лпрект*<br/>
Задает прямоугольник в логических единицах. Для этого параметра можно передать либо объект `CRect`, либо указатель на структуру `RECT`.
...
Прямоугольник расширяется до, но не включает, правую и нижнюю координаты. Это означает, что высота прямоугольника равна *y2* - *Y1* , а ширина прямоугольника равна *x2* - *x1*. Ширина и высота прямоугольника должны быть больше 2 единиц и меньше 32 767 единиц.
...
*лпрект*<br/>
Указывает на `RECT`ную структуру или объект `CRect`, содержащий логические координаты указанного прямоугольника.
...
##  <a name="releaseattribdc"></a>CDC:: Релеасеаттрибдк

Вызовите эту функцию члена, чтобы присвоить `m_hAttribDC` значение NULL.
...
Это не приводит к возникновению `Detach`. Только контекст устройства вывода прикрепляется к объекту `CDC`, и только он может быть отсоединен.

##  <a name="releaseoutputdc"></a>CDC:: Релеасеаутпутдк

Вызовите эту функцию члена, чтобы присвоить элементу `m_hDC` значение NULL.
...
Эта функция-член не может быть вызвана, если контекст выходного устройства присоединен к объекту `CDC`. Используйте функцию члена `Detach`, чтобы отсоединить контекст выходного устройства.

##  <a name="resetdc"></a>CDC:: Ресетдк

Вызовите эту функцию члена, чтобы обновить контекст устройства, упакованный объектом `CDC`.
...
*лпдевмоде*<br/>
Указатель на структуру `DEVMODE` Windows.
...
Контекст устройства обновляется на основе сведений, указанных в структуре `DEVMODE` Windows. Эта функция члена сбрасывает только контекст устройства атрибута.

Приложение, как правило, использует функцию-член `ResetDC`, когда окно обрабатывает сообщение `WM_DEVMODECHANGE`. Эту функцию-член также можно использовать для изменения ориентации бумаги или лотков бумаги при печати документа.
...
##  <a name="restoredc"></a>CDC:: Ресторедк
...
`RestoreDC` восстанавливает контекст устройства путем выталкивания сведений о состоянии из стека, созданного предыдущими вызовами функции-члена `SaveDC`.

Стек может содержать сведения о состоянии для нескольких контекстов устройств. Если контекст, заданный параметром *нсаведдк* , находится не вверху стека, `RestoreDC` удаляет все сведения о состоянии между контекстом устройства, указанным в *нсаведдк* , и верхней частью стека. Удаленные данные теряются.

##  <a name="roundrect"></a>CDC:: Раундрект
...
*лпрект*<br/>
Задает ограничивающий прямоугольник в логических единицах. Для этого параметра можно передать либо объект `CRect`, либо указатель на структуру `RECT`.

*point*<br/>
Координата x *точки* задает ширину эллипса для рисования скругленных углов (в логических единицах). Координата y *точки* указывает высоту эллипса для рисования скругленных углов (в логических единицах). Для этого параметра можно передать либо структуру `POINT`, либо объект `CPoint`.
...
Рисунок, который рисует эта функция, расширяется до, но не включает в себя правую и нижнюю координаты. Это означает, что высота фигуры имеет значение *y2* - *Y1* , а ширина фигуры равна *x2* - *x1*. Высота и ширина ограничивающего прямоугольника должны быть больше 2 единиц и меньше 32 767 единиц.

### <a name="example"></a>Пример

[!code-cpp[NVC_MFCDocView#40](../../mfc/codesnippet/cpp/cdc-class_12.cpp)]

##  <a name="savedc"></a>CDC:: Саведк
...
После этого сохраненный контекст устройства можно будет восстановить с помощью `RestoreDC`.

`SaveDC` можно использовать любое количество раз для сохранения любого числа состояний контекста устройства.

##  <a name="scaleviewportext"></a>CDC:: Скалевиевпортекст
...
##  <a name="scalewindowext"></a>CDC:: Скалевиндовекст
...
Предыдущие экстенты окна (в логических единицах) как объект `CSize`.
...
##  <a name="scrolldc"></a>CDC:: Скроллдк
...
Если *лпректупдате* имеет значение null, Windows не выполняет вычисление прямоугольника обновления. Если оба *пргнупдате* и *лпректупдате* имеют значение null, Windows не вычислит регион обновления. Если *пргнупдате* не равно null, Windows предполагает, что он содержит допустимый указатель на регион, обнаруженный процессом прокрутки (определяется функцией-членом `ScrollDC`). Регион обновления, возвращенный в *лпректупдате* , можно передать в `CWnd::InvalidateRgn`, если это необходимо.

Приложение должно использовать функцию-член `ScrollWindow` класса `CWnd`, когда необходимо прокручивать всю клиентскую область окна. В противном случае он должен использовать `ScrollDC`.

##  <a name="selectclippath"></a>CDC:: Селектклиппас
...
##  <a name="selectcliprgn"></a>CDC:: Селектклипргн
...
Указатель на заменяемый объект. Это указатель на объект одного из классов, производных от `CGdiObject`, например `CPen`, в зависимости от используемой версии функции. Если возникает ошибка, возвращается значение NULL. Эта функция может возвращать указатель на временный объект. Этот временный объект действителен только во время обработки одного сообщения Windows. Дополнительные сведения см. в разделе `CGdiObject::FromHandle`.

Версия функции-члена, которая принимает параметр Region, выполняет ту же задачу, что и функция-член `SelectClipRgn`. Его возвращаемое значение может быть любым из следующих:
...
Класс `CDC` предоставляет пять версий, специализированных для определенных типов объектов GDI, включая перья, кисти, шрифты, точечные рисунки и регионы. Вновь выбранный объект заменяет предыдущий объект того же типа. Например, если *объект* в общей версии `SelectObject` указывает на объект [кпен](../../mfc/reference/cpen-class.md) , функция заменяет текущее перо на перо, заданное *объект*.

Приложение может выбрать точечный рисунок только в контекстах устройства памяти и только в одном контексте устройства памяти за раз. Формат точечного рисунка должен быть либо монохромным, либо совместимым с контекстом устройства; Если это не так, `SelectObject` возвращает ошибку.

Для Windows 3,1 и более поздних версий функция `SelectObject` возвращает одно и то же значение, независимо от того, используется ли он в метафайле. В предыдущих версиях Windows `SelectObject` вернул ненулевое значение для успешного выполнения и 0 для сбоя, если он использовался в метафайле.

##  <a name="selectpalette"></a>CDC:: Селектпалетте
...
*ппалетте*<br/>
Определяет логическую палитру для выбора. Эта палитра уже должна быть создана с помощью функции члена `CPalette` [креатепалетте](../../mfc/reference/cpalette-class.md#createpalette).
...
Указатель на объект `CPalette`, определяющий логическую палитру, замененную палитрой, заданной параметром *ппалетте*. Если возникает ошибка, она имеет значение NULL.
...
##  <a name="selectstockobject"></a>CDC:: Селектстоккобжект
...
- SYSTEM_FONT системный шрифт. По умолчанию Windows использует системный шрифт для рисования меню, элементов управления диалоговых окон и другого текста. Однако лучше не полагаться на SYSTEM_FONT для получения шрифта, используемого в диалоговых окнах и окнах. Вместо этого используйте функцию `SystemParametersInfo` с параметром SPI_GETNONCLIENTMETRICS, чтобы получить текущий шрифт. `SystemParametersInfo` учитывает текущую тему и предоставляет сведения о шрифтах для заголовков, меню и диалоговых окон сообщений.
...
Указывает результат функции `SetAbortProc`. Некоторые из приведенных ниже значений являются более вероятной, чем другие, но все это возможно.
...
Обратите внимание, что функции Microsoft C++ Visual упрощают создание функции обратного вызова, передаваемой в `SetAbortProc`. Адрес, передаваемый функции-члену `EnumObjects`, — это указатель на функцию, экспортируемую с помощью `__declspec(dllexport)` и с соглашением о вызове `__stdcall`.
...
Направление по умолчанию — против часовой стрелки. Функция `SetArcDirection` задает направление рисования следующих функций:
...
##  <a name="setattribdc"></a>CDC:: Сетаттрибдк
...
Эта функция члена не присоединяет контекст устройства к объекту `CDC`. К объекту `CDC` прикрепляется только контекст выходного устройства.

##  <a name="setbkcolor"></a>CDC:: Сетбкколор
...
##  <a name="setbkmode"></a>CDC:: Сетбкмоде
...
##  <a name="setbrushorg"></a>CDC:: Сетбрушорг
...
*point*<br/>
Задает координаты x и y нового источника. Каждое значение должно находиться в диапазоне 0-7. Для этого параметра можно передать либо структуру `POINT`, либо объект `CPoint`.
...
Координаты по умолчанию для источника кисти: (0, 0). Чтобы изменить происхождение кисти, вызовите функцию `UnrealizeObject` для объекта `CBrush`, вызовите `SetBrushOrg`, а затем вызовите функцию-член `SelectObject`, чтобы выбрать кисть в контексте устройства.

Не используйте `SetBrushOrg` с объектами `CBrush` запасов.
...
Значения коррекции цвета используются для настройки входного цвета исходного растрового изображения для вызовов функции-члена `CDC::StretchBlt`, когда задан режим передачи ПОЛУТОНОВ.

##  <a name="setdcbrushcolor"></a>CDC:: Сетдкбрушколор
...
##  <a name="setdcpencolor"></a>CDC:: Сетдкпенколор
...
##  <a name="setgraphicsmode"></a>CDC:: Сетграфиксмоде
...
##  <a name="setlayout"></a>CDC:: Сетлайаут
...
Обычно для окна не вызывается `SetLayout`. Вместо этого вы управляете макетом справа налево в окне, устанавливая [Расширенные стили окон](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles) , например WS_EX_RTLREADING. Контекст устройства, например принтер или метафайл, не наследует этот макет. Единственный способ задать контекст устройства для разметки справа налево — вызвать `SetLayout`.
...
В некоторых случаях, например с несколькими точечными рисунками, может потребоваться сохранить макет слева направо. В таких случаях следует подготовить изображение, вызвав `BitBlt` или `StretchBlt`, а затем установить флаг элемента управления Bitmap для *двлайаут* в значение LAYOUT_BITMAPORIENTATIONPRESERVED.
...
##  <a name="setmapmode"></a>CDC:: Сетмапмоде
...
- MM_ISOTROPIC логические устройства преобразуются в произвольные единицы с одинаковыми масштабируемыми осями; то есть 1 единица вдоль оси x равна 1 единице вдоль оси y. Используйте функции элементов `SetWindowExt` и `SetViewportExt`, чтобы указать нужные единицы и ориентацию осей. При необходимости GDI вносит необходимые изменения, чтобы гарантировать, что единицы x и y будут иметь одинаковый размер.
...
##  <a name="setmapperflags"></a>CDC:: Сетмапперфлагс
...
##  <a name="setmiterlimit"></a>CDC:: Сетмитерлимит
...
##  <a name="setoutputdc"></a>CDC:: Сетаутпутдк
...
Эта функция-член может быть вызвана только в том случае, если контекст устройства не был присоединен к объекту `CDC`. Эта функция члена задает `m_hDC`, но не присоединяет контекст устройства к объекту `CDC`.
...
*point*<br/>
Задает логические координаты x и y устанавливаемой точки. Для этого параметра можно передать либо структуру `POINT`, либо объект `CPoint`.
...
Не все устройства поддерживают функцию `SetPixel`. Чтобы определить, поддерживает ли устройство `SetPixel`, вызовите функцию-член `GetDeviceCaps` с индексом РАСТЕРКАПС и проверьте возвращаемое значение для флага RC_BITBLT.

##  <a name="setpixelv"></a>CDC:: Сетпикселв
...
Точка должна быть в области обрезки и видимой части поверхности устройства. Не все устройства поддерживают функцию члена. Дополнительные сведения см. в описании возможности RC_BITBLT в функции члена `CDC::GetDeviceCaps`. `SetPixelV` работает быстрее, чем `SetPixel`, поскольку ему не нужно возвращать значение цвета, нарисованное на момент рисования.

##  <a name="setpolyfillmode"></a>CDC:: Сетполифиллмоде
...
##  <a name="setstretchbltmode"></a>CDC:: Сетстретчблтмоде

Задает режим растяжения по точечному рисунку для функции-члена `StretchBlt`.
...
- TA_UPDATECP Обновляет текущую точку по оси x после каждого вызова функции вывода текста. Новая позиции находится в правой части ограничивающего прямоугольника для текста. Если этот флаг установлен, координаты, указанные в вызовах функции-члена `TextOut`, игнорируются.
...
Функции-члены `TextOut` и `ExtTextOut` используют эти флаги при размещении строки текста на дисплее или на устройстве. Флаги определяют связь между определенной точкой и прямоугольником, ограничивающим текст. Координаты этой точки передаются в качестве параметров функции члена `TextOut`. Прямоугольник, ограничивающий текст, формируется смежными ячейками символов в текстовой строке.

##  <a name="settextcharacterextra"></a>CDC:: Сеттекстчарактерекстра
...
*нчарекстра*<br/>
Указывает объем дополнительного пространства (в логических единицах), добавляемого к каждому символу. Если текущий режим сопоставления не `MM_TEXT`, *нчарекстра* преобразуется и округляется до ближайшего пикселя.
...
##  <a name="settextcolor"></a>CDC:: Сеттекстколор
...
Если устройство не может представить указанный цвет, система устанавливает цвет текста, равный ближайшему физическому цвету. Цвет фона для символа задается `SetBkColor` и `SetBkMode` функциями-членами.

### <a name="example"></a>Пример

  См. пример для [CWnd:: онктлколор](../../mfc/reference/cwnd-class.md#onctlcolor).

##  <a name="settextjustification"></a>CDC:: Сеттекстжустификатион
...
*нбреакекстра*<br/>
Указывает общее дополнительное пространство, добавляемое в строку текста (в логических единицах). Если текущий режим сопоставления не `MM_TEXT`, значение, заданное этим параметром, преобразуется в текущий режим сопоставления и округляется до ближайшей единицы устройства.
...
После вызова функции-члена `SetTextJustification` вызов функции вывода текста (например, `TextOut`) распределяет заданный дополнительный пробел равномерно между заданным числом символов разрыва. Символ разрыва обычно является символом пробела (ASCII 32), но может быть определен шрифтом в качестве другого символа.

Функция-член `GetTextExtent` обычно используется с `SetTextJustification`. `GetTextExtent` вычислит ширину данной строки перед выравниванием. Приложение может определить, сколько пространства следует указать в параметре *нбреакекстра* , вычитая значение, возвращаемое `GetTextExtent`, из ширины строки после выравнивания.

Функцию `SetTextJustification` можно использовать для выровнять линию, содержащую несколько запусков, в разных шрифтах. В этом случае строка должна быть создана поэтапно путем согласования и записи каждого запуска по отдельности.
...
После того как каждая строка будет согласована, этот термин ошибки необходимо очистить, чтобы предотвратить его встраивание в следующую строку. Термин можно очистить путем вызова `SetTextJustification` с параметром *нбреакекстра* , имеющим значение 0.

##  <a name="setviewportext"></a>CDC:: Сетвиевпортекст
...
Если задан режим MM_ISOTROPIC, приложение должно вызвать функцию-член `SetWindowExt` перед вызовом `SetViewportExt`.

### <a name="example"></a>Пример

  См. пример для [CView:: онпрепаредк](../../mfc/reference/cview-class.md#onpreparedc).

##  <a name="setviewportorg"></a>CDC:: Сетвиевпорторг
...
*point*<br/>
Указывает источник окна просмотра. Значения должны находиться в диапазоне от системы координат устройства. Для этого параметра можно передать либо структуру `POINT`, либо объект `CPoint`.

### <a name="return-value"></a>Возвращаемое значение

Предыдущий источник окна просмотра (в координатах устройства) в качестве объекта `CPoint`.

### <a name="remarks"></a>Remarks

В окне просмотра, а также окно контекста устройства, определяется, как точки сопоставления GDI в логической системе координат указывают на точки в системе координат фактического устройства. Иными словами, они определяют, как GDI преобразует логические координаты в координаты устройства.

Источник окна просмотра отмечает точку в системе координат устройства, к которой GDI сопоставляет исходное окно, точку в логической системе координат, заданную функцией-членом `SetWindowOrg`. GDI сопоставляет все остальные точки, следуя тому же процессу, который требуется для сопоставления исходного окна с исходным окном просмотра. Например, все точки в окружности вокруг точки в источнике окна будут находиться в окружности вокруг точки в начале окна просмотра. Аналогичным образом все точки в строке, проходящей через начало координат окна, будут находиться в строке, проходящей через точку просмотра.

### <a name="example"></a>Пример

  См. пример для [CView:: онпрепаредк](../../mfc/reference/cview-class.md#onpreparedc).

##  <a name="setwindowext"></a>CDC:: Сетвиндовекст
...
Предыдущие экстенты окна (в логических единицах) как объект `CSize`. При возникновении ошибки координаты x и y возвращаемого `CSize` объекта устанавливаются в 0.
...
##  <a name="setwindoworg"></a>CDC:: Сетвиндоворг
...
*point*<br/>
Задает логические координаты нового источника окна. Для этого параметра можно передать либо структуру `POINT`, либо объект `CPoint`.

### <a name="return-value"></a>Возвращаемое значение

Предыдущий источник окна как объект `CPoint`.

### <a name="remarks"></a>Remarks

В окне вместе с окном просмотра контекста устройства определяется, как точки сопоставления GDI в логической системе координат указывают на точки в системе координат устройства.

Источник окна отмечает точку в логической системе координат, с которой GDI сопоставляет источник окна просмотра, точку в системе координат устройства, указанную функцией `SetWindowOrg`. GDI сопоставляет все остальные точки, следуя тому же процессу, который требуется для сопоставления исходного окна с исходным окном просмотра. Например, все точки в окружности вокруг точки в источнике окна будут находиться в окружности вокруг точки в начале окна просмотра. Аналогичным образом все точки в строке, проходящей через начало координат окна, будут находиться в строке, проходящей через точку просмотра.

##  <a name="setworldtransform"></a>CDC:: Сетворлдтрансформ
...
##  <a name="startdoc"></a>CDC:: Стартдок

Информирует драйвер устройства о том, что начинается новое задание печати и все последующие `StartPage` и `EndPage` вызовы должны быть помещены в очередь в том же задании, пока не будет вызван вызов `EndDoc`.
...
Система отключает `ResetDC` функцию члена между вызовами функций `StartPage` и `EndPage`.

### <a name="example"></a>Пример

  См. пример для [CDC:: стартдок](#startdoc).

##  <a name="stretchblt"></a>CDC:: Стретчблт
...
Не все устройства поддерживают функцию `StretchBlt`. Чтобы определить, поддерживает ли устройство `StretchBlt`, вызовите функцию-член `GetDeviceCaps` с индексом РАСТЕРКАПС и проверьте возвращаемое значение для флага RC_STRETCHBLT.

##  <a name="strokeandfillpath"></a>CDC:: Строкеандфиллпас
...
Контекст устройства должен содержать закрытый путь. Функция-член `StrokeAndFillPath` имеет тот же результат, что и закрытие всех открытых фигур в контуре и обводки и заливка контура отдельно, за исключением того, что заполненная область не пересекается с обводкой, даже если перо является широким.

##  <a name="strokepath"></a>CDC:: Строкепас
...
##  <a name="tabbedtextout"></a>CDC:: Таббедтекстаут
...
Размеры строки (в логических единицах) в качестве объекта `CSize`.
...
Если *нтабпоситионс* имеет значение 1, то позиции табуляции разделяются расстоянием, заданным первым значением в массиве *лпнтабстоппоситионс* . Если массив *лпнтабстоппоситионс* содержит более одного значения, для каждого значения в массиве задается позиция табуляции вплоть до числа, заданного параметром *нтабпоситионс*. Параметр *нтаборигин* позволяет приложению вызывать функцию `TabbedTextOut` несколько раз для одной строки. Если приложение вызывает функцию более одного раза, если для *нтаборигин* задано одно и то же значение каждый раз, функция разворачивает все вкладки относительно позиции, заданной параметром *нтаборигин*.

По умолчанию текущая позиция не используется и не обновляется функцией. Если приложению необходимо обновить текущую точку при вызове функции, приложение может вызвать функцию члена [сеттексталигн](#settextalign) с параметром *нфлагс* , для которого задано значение TA_UPDATECP. Если этот флаг установлен, Windows игнорирует параметры *x* и *y* при последующих вызовах `TabbedTextOut`, вместо этого использует текущую точку.

##  <a name="textout"></a>CDC:: текстовое поле
...
Если приложению необходимо обновить текущую точку при вызове `TextOut`, приложение может вызвать `SetTextAlign` функцию члена с параметром *нфлагс* , для которого задано значение TA_UPDATECP. Если этот флаг установлен, Windows игнорирует параметры *x* и *y* при последующих вызовах `TextOut`, вместо этого использует текущую точку.

### <a name="example"></a>Пример

  См. пример для [CDC:: бегинпас](#beginpath).

##  <a name="transparentblt"></a>CDC:: Транспарентблт
...
`TransparentBlt` допускает прозрачность; то есть цвет RGB, заданный *клртранспарент* , отображается прозрачным для перемещения.

Дополнительные сведения см. в разделе [транспарентблт](/windows/win32/api/wingdi/nf-wingdi-transparentblt) в Windows SDK.

##  <a name="updatecolors"></a>CDC:: Упдатеколорс
...
Неактивное окно с реализованной логической палитрой может вызвать `UpdateColors` в качестве альтернативы перерисовки клиентской области при изменении системной палитры.
...
Функция-член `UpdateColors` обычно обновляет клиентскую область быстрее, чем перерисовка области. Однако поскольку функция выполняет преобразование цвета на основе цвета каждого пикселя перед изменением системной палитры, каждый вызов этой функции приводит к утрате некоторой точности цвета.

##  <a name="widenpath"></a>CDC:: Виденпас
