...
### <a name="example"></a>Пример  
 В этом примере показано использование функции `CArchive` оператора вставки << с `int` и `long` типов.  
...
### <a name="remarks"></a>Примечания  
 ...
 Если вы использовали `IMPLEMENT_SERIAL` макрос в реализации класса, то операторы извлечения, перегруженные для `CObject` вызовите защищенный **ReadObject** функция (указатель ненулевое значение, класс среды выполнения). Эта функция, в свою очередь, вызывает `Serialize` функции класса.  
