...
|Название|Описание|
|----------|-----------------|
|[CRecordset:: CRecordset](#crecordset)|Создает объект `CRecordset`. Производный класс должен предоставить конструктор, который вызывает этот метод.|

### <a name="public-methods"></a>Открытые методы

|Название|Описание|
|----------|-----------------|
|[CRecordset:: AddNew](#addnew)|Подготовка к добавлению новой записи. Вызовите `Update`, чтобы завершить добавление.|
|[CRecordset:: Канаппенд](#canappend)|Возвращает ненулевое значение, если новые записи можно добавить в набор записей с помощью функции члена `AddNew`.|
...
|[CRecordset:: Канрестарт](#canrestart)|Возвращает ненулевое значение, если можно вызвать `Requery` для повторного выполнения запроса набора записей.|
...
|[CRecordset:: Исбоф](#isbof)|Возвращает ненулевое значение, если набор записей был помещен перед первой записью. Нет текущей записи.|
|[CRecordset:: isDeleted](#isdeleted)|Возвращает ненулевое значение, если набор записей расположен на удаленной записи.|
|[CRecordset:: Исеоф](#iseof)|Возвращает ненулевое значение, если набор записей был помещен после последней записи. Нет текущей записи.|
...
|[CRecordset:: OnOpen](#isopen)|Возвращает ненулевое значение, если `Open` был вызван ранее.|
|[CRecordset:: Move](#move)|Размещает набор записей по заданному числу записей из текущей записи в любом направлении.|
|[CRecordset:: MoveFirst](#movefirst)|Позиционирует текущую запись в первой записи в наборе записей. Сначала протестируйте `IsBOF`.|
|[CRecordset:: MoveLast](#movelast)|Позиционирует текущую запись по последней записи или последнему набору строк. Сначала протестируйте `IsEOF`.|
|[Метод CRecordset:: MoveNext](#movenext)|Позиционирует текущую запись на следующую запись или на следующий набор строк. Сначала протестируйте `IsEOF`.|
|[CRecordset:: Мовепрев](#moveprev)|Позиционирует текущую запись по предыдущей записи или к предыдущему набору строк. Сначала протестируйте `IsBOF`.|
...
|[CRecordset:: Update](#update)|Завершает операцию `AddNew` или `Edit`, сохраняя новые или измененные данные в источнике данных.|
...
|[CRecordset:: m_pDatabase](#m_pdatabase)|Содержит указатель на объект `CDatabase`, через который набор записей соединяется с источником данных.|
|[CRecordset:: m_strFilter](#m_strfilter)|Содержит `CString`, указывающий предложение `WHERE` язык SQL (SQL). Используется в качестве фильтра для выбора только тех записей, которые удовлетворяют определенным условиям.|
|[CRecordset:: m_strSort](#m_strsort)|Содержит `CString`, указывающий предложение SQL `ORDER BY`. Используется для управления порядком сортировки записей.|

## <a name="remarks"></a> Примечания

Известные как "наборы записей", `CRecordset` объекты обычно используются в двух формах: динамических подмножеств данных и моментальных снимках. Динамический набор синхронизируется с обновлениями данных, сделанными другими пользователями. Моментальный снимок является статическим представлением данных. Каждая форма представляет набор записей, фиксированных во время открытия набора записей, но при прокрутке к записи в динамическом наборе он отражает изменения, внесенные в запись другими пользователями или другими наборами записей в приложении.
...
Для работы с любым типом набора записей обычно необходимо, чтобы класс набора записей, зависящий от приложения, был производным от `CRecordset`. Наборы записей выбирают записи из источника данных, а затем можно:
...
Чтобы использовать класс, откройте базу данных и создайте объект набора записей, передав конструктору указатель на объект `CDatabase`. Затем вызовите функцию-член `Open` набора записей, где можно указать, является ли объект динамическим или моментальным снимком. При вызове `Open` выбираются данные из источника данных. После открытия объекта Recordset используйте его функции-члены и элементы данных для прокрутки записей и работы с ними. Доступные операции зависят от того, является ли объект динамическим или моментальным снимком, является ли он обновляемым или доступным только для чтения (это зависит от возможностей источника данных ODBC) и от того, реализована ли многострочная выборка строк. Чтобы обновить записи, которые могли быть изменены или добавлены с момента вызова `Open`, вызовите функцию члена `Requery` объекта. Вызовите функцию члена `Close` объекта и удалите объект после завершения работы с ним.

В производном классе `CRecordset` для поддержки чтения и обновления полей записей используется обмен полями записей (RFX) или обмен полями групповой записи (групповой RFX).
...
Чтобы увидеть только что добавленную запись, необходимо вызвать функцию " [Обновление](#requery) члена". Поля записи изначально имеют значение null. (В терминологии базы данных значение NULL означает, что значение не имеет значения и не совпадает со значением C++NULL в.) Чтобы завершить операцию, необходимо вызвать функцию [обновления](#update) члена. `Update` сохраняет изменения в источнике данных.

> [!NOTE]
>  При реализации групповой выборки строк нельзя вызвать `AddNew`. Это приведет к неудачному утверждению. Хотя класс `CRecordset` не предоставляет механизм для обновления строк данных, можно написать собственные функции с помощью функции API ODBC `SQLSetPos`. Дополнительные сведения о массовой выборке строк см. в статье [Набор записей: пакетная выборка строк (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md).

`AddNew` готовит новую пустую запись с помощью элементов данных поля набора записей. После вызова `AddNew`задайте нужные значения в элементах данных поля набора записей. (Для этой цели не нужно вызывать функцию [изменения](#edit) члена; используйте `Edit` только для существующих записей.) При последующем вызове `Update`измененные значения в элементах данных поля сохраняются в источнике данных.

> [!CAUTION]
>  При прокрутке до новой записи перед вызовом `Update`новая запись теряется и предупреждение не выдается.

Если источник данных поддерживает транзакции, можно сделать `AddNew` вызовом части транзакции. Дополнительные сведения о транзакциях см. в разделе класс [CDatabase](../../mfc/reference/cdatabase-class.md). Обратите внимание, что перед вызовом `AddNew`необходимо вызвать метод [CDatabase:: примеры BeginTrans](../../mfc/reference/cdatabase-class.md#begintrans) .

> [!NOTE]
>  Для динамических подмножеств данных новые записи добавляются в набор записей в качестве последней записи. Добавленные записи не добавляются в моментальные снимки. для обновления набора записей необходимо вызвать `Requery`.

Не допускается вызов `AddNew` для набора записей, `Open` функция-член которой не была вызвана. При вызове `AddNew` для набора записей, к которому не может быть добавлен `CDBException`, возникает исключение. Чтобы определить, можно ли обновить набор записей путем вызова [канаппенд](#canappend).
...
##  <a name="canappend"></a>CRecordset:: Канаппенд
...
##  <a name="canbookmark"></a>CRecordset:: Канбукмарк
...
Эта функция не зависит от параметра `CRecordset::useBookmarks` в параметре *двоптионс* функции [Open](#open) Member. `CanBookmark` указывает, поддерживаются ли закладки для данного драйвера ODBC и типа курсора. `CRecordset::useBookmarks` указывает, будут ли доступны закладки, если они поддерживаются.
...
##  <a name="canrestart"></a>CRecordset:: Канрестарт

Определяет, допускает ли набор записей перезапуск запроса (для обновления записей), вызывая функцию-член `Requery`.
...
##  <a name="canscroll"></a>CRecordset:: Канскролл
...
##  <a name="cantransact"></a>CRecordset:: Кантрансакт
...
##  <a name="canupdate"></a>CRecordset:: Канупдате
...
Набор записей может быть доступен только для чтения, если базовый источник данных доступен только для чтения или если вы указали `CRecordset::readOnly` в параметре *двоптионс* при открытии набора записей.

##  <a name="checkrowseterror"></a>CRecordset:: Чеккровсетеррор
...
`CheckRowsetError` вызывается автоматически в операции перехода по курсору, например `Open`, `Requery`или любой другой операции `Move`. Ему передается возвращаемое значение функции API ODBC `SQLExtendedFetch`. В следующей таблице перечислены возможные значения для параметра *нреткоде* .
...
|SQL_INVALID_HANDLE|Сбой функции из-за недопустимого обработчика среды, маркера соединения или маркера инструкции. Это указывает на ошибку программирования. Дополнительные сведения из `SQLError`недоступны.|
|SQL_STILL_EXECUTING|Функция, которая была запущена асинхронно, по-прежнему выполняется. Обратите внимание, что по умолчанию MFC никогда не передает это значение в `CheckRowsetError`; MFC будет продолжать вызов `SQLExtendedFetch`, пока он больше не вернет SQL_STILL_EXECUTING.|
...
ХСТМТ ODBC и вся память, выделенная для набора записей, освобождается. Как правило, после вызова `Close`удаляется C++ объект набора записей, если он был выделен с помощью **New**.

После вызова `Close`можно вызвать метод `Open`. Это позволяет повторно использовать объект Recordset. Альтернативой является вызов `Requery`.
...
*пдатабасе*<br/>
Содержит указатель на объект `CDatabase` или значение NULL. Если не задано значение NULL и функция члена `Open` объекта `CDatabase` не была вызвана для подключения к источнику данных, набор записей пытается открыть его для вас во время собственного вызова `Open`. При передаче значения NULL объект `CDatabase` создается и подключается, используя сведения об источнике данных, которые вы указали при наследовании класса Recordset с помощью ClassWizard.

### <a name="remarks"></a>Заметки

Можно либо использовать `CRecordset` напрямую, либо получить класс, зависящий от приложения, от `CRecordset`. ClassWizard можно использовать для получения классов набора записей.
...
Передайте значение NULL конструктору набора записей, чтобы объект `CDatabase` был автоматически создан и подключен. Это полезная Краткая форма, которая не требует создания и подключения объекта `CDatabase` перед созданием набора записей.
...
##  <a name="delete"></a>CRecordset::D удалить
...
После успешного удаления элементы данных поля набора записей устанавливаются в значение null, и для перемещения удаленной записи необходимо явно вызвать одну из `Move` функций. После перемещения удаленной записи к ней невозможно вернуться. Если источник данных поддерживает транзакции, можно сделать `Delete` вызовом части транзакции. Дополнительные сведения см. в описании [транзакции статьи (ODBC)](../../data/odbc/transaction-odbc.md).

> [!NOTE]
>  При реализации групповой выборки строк нельзя вызвать `Delete`. Это приведет к неудачному утверждению. Хотя класс `CRecordset` не предоставляет механизм для обновления строк данных, можно написать собственные функции с помощью функции API ODBC `SQLSetPos`. Дополнительные сведения о массовой выборке строк см. в статье [Набор записей: пакетная выборка строк (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md).

> [!CAUTION]
>  Набор записей должен быть обновляемым, и при вызове `Delete`в наборе записей должна быть действительная запись. в противном случае возникает ошибка. Например, если удалить запись, но не прокручивать до новой записи перед вызовом `Delete`, `Delete` выдаст исключение [кдбексцептион](../../mfc/reference/cdbexception-class.md).

В отличие от [AddNew](#addnew) и [Edit](#edit), вызов `Delete` не должен сопровождаться вызовом [Update](#update). Если `Delete` вызов завершается неудачей, элементы данных поля остаются без изменений.

### <a name="example"></a>Пример

В этом примере показан набор записей, созданный на кадре функции. В примере предполагается существование `m_dbCust`, переменная-член типа `CDatabase` уже подключена к источнику данных.

[!code-cpp[NVC_MFCDatabase#18](../../mfc/codesnippet/cpp/crecordset-class_2.cpp)]

##  <a name="dobulkfieldexchange"></a>CRecordset::D Обулкфиелдексчанже
...
Если многострочная выборка строк не реализована, платформа вызывает функцию [DoFieldExchange](#dofieldexchange). Для реализации групповой выборки строк необходимо указать параметр `CRecordset::useMultiRowFetch` параметра *двоптионс* в функции [Open](#open) Member.

> [!NOTE]
> `DoBulkFieldExchange` доступен, только если используется класс, производный от `CRecordset`. Если объект набора записей был создан непосредственно из `CRecordset`, необходимо вызвать функцию-член [GetFieldValue](#getfieldvalue) для получения данных.

Блочный обмен полями записей (групповой RFX) аналогичен обмену полями записей (RFX). Данные автоматически передаются из источника данных в объект набора записей. Однако нельзя вызвать `AddNew`, `Edit`, `Delete`или `Update` для передачи изменений обратно в источник данных. Класс `CRecordset` в настоящее время не предоставляет механизм для обновления строк данных. Однако можно написать собственные функции с помощью функции API ODBC `SQLSetPos`.

Обратите внимание, что ClassWizard не поддерживает обмен полями с массовыми записями. Таким образом, необходимо переопределить `DoBulkFieldExchange` вручную, написав вызовы функций массовых RFX. Дополнительные сведения об этих функциях см. в разделе [функции обмена полями записи](../../mfc/reference/record-field-exchange-functions.md).
...
##  <a name="dofieldexchange"></a>CRecordset::D Офиелдексчанже
...
Если реализуется групповая выборка строк, платформа вызывает [добулкфиелдексчанже](#dobulkfieldexchange). Для реализации групповой выборки строк необходимо указать параметр `CRecordset::useMultiRowFetch` параметра *двоптионс* в функции [Open](#open) Member.

> [!NOTE]
> `DoFieldExchange` доступен, только если используется класс, производный от `CRecordset`. Если объект набора записей был создан непосредственно из `CRecordset`, необходимо вызвать функцию-член [GetFieldValue](#getfieldvalue) для получения данных.
...
Дополнительные примеры и подробные сведения о `DoFieldExchange`см. в статье [Обмен данными с полями записей: принципы работы RFX](../../data/odbc/record-field-exchange-how-rfx-works.md). Общие сведения о RFX см. в статье об [обмене данными с полем записи](../../data/odbc/record-field-exchange-rfx.md).
...
`Edit` сохраняет значения элементов данных набора записей. При вызове `Edit`, внесении изменений, а затем повторном вызове `Edit` значения записей восстанавливаются до первого вызова `Edit`.
...
Если источник данных поддерживает транзакции, можно сделать `Edit` вызовом части транзакции. Обратите внимание, что перед вызовом `Edit` и после открытия набора записей следует вызвать метод [CDatabase:: примеры BeginTrans](../../mfc/reference/cdatabase-class.md#begintrans) . Также обратите внимание, что вызов [CDatabase:: CommitTrans](../../mfc/reference/cdatabase-class.md#committrans) не является заменой для вызова `Update` для завершения операции `Edit`. Дополнительные сведения о транзакциях см. в разделе класс [CDatabase](../../mfc/reference/cdatabase-class.md).

В зависимости от текущего режима блокировки обновляемая запись может быть заблокирована `Edit` до тех пор, пока не будет вызвана `Update` или выполнена прокрутка к другой записи или она может быть заблокирована только во время вызова `Edit`. Режим блокировки можно изменить с помощью [сетлоккингмоде](#setlockingmode).

Предыдущее значение текущей записи восстанавливается при прокрутке до новой записи перед вызовом `Update`. При вызове `Edit` для набора записей, который не может быть обновлен, или при отсутствии текущей записи выдается `CDBException`.
...
##  <a name="flushresultset"></a>CRecordset:: Флушресултсет
...
Следует вызывать `FlushResultSet` только после завершения работы с курсором на текущем результирующем наборе. Обратите внимание, что при извлечении следующего результирующего набора путем вызова `FlushResultSet`курсор не является допустимым для этого результирующего набора. После вызова `FlushResultSet`необходимо вызвать функцию члена [MoveNext](#movenext) .

Если предопределенный запрос использует выходной параметр или параметры ввода-вывода, необходимо вызвать метод `FlushResultSet`, пока он не вернет `FALSE` (значение 0), чтобы получить эти значения параметров.

`FlushResultSet` вызывает функцию API ODBC `SQLMoreResults`. Если `SQLMoreResults` возвращает SQL_ERROR или SQL_INVALID_HANDLE, то `FlushResultSet` выдаст исключение. Дополнительные сведения о `SQLMoreResults`см. в Windows SDK.

Если требуется вызвать `FlushResultSet`, хранимая процедура должна иметь привязанные поля.

### <a name="example"></a>Пример

В следующем коде предполагается, что `COutParamRecordset` является производным `CRecordset`ным объектом на основе предопределенного запроса с входным параметром и выходным параметром и наличием нескольких результирующих наборов. Обратите внимание на структуру переопределения [DoFieldExchange](#dofieldexchange) .

[!code-cpp[NVC_MFCDatabase#21](../../mfc/codesnippet/cpp/crecordset-class_5.cpp)]

[!code-cpp[NVC_MFCDatabase#22](../../mfc/codesnippet/cpp/crecordset-class_6.cpp)]

##  <a name="getbookmark"></a>CRecordset:: onbookmark
...
Чтобы определить, поддерживаются ли закладки в наборе записей, вызовите [канбукмарк](#canbookmark). Чтобы сделать закладки доступными, если они поддерживаются, необходимо задать параметр `CRecordset::useBookmarks` в параметре *двоптионс* функции [Open](#open) Member.

> [!NOTE]
>  Если закладки не поддерживаются или недоступны, вызов `GetBookmark` приведет к возникновению исключения. Закладки не поддерживаются для наборов записей с последовательным входом.

`GetBookmark` присваивает значение закладки для текущей записи объекту `CDBVariant`. Чтобы вернуться к этой записи в любое время после перехода на другую запись, вызовите [сетбукмарк](#setbookmark) с соответствующим объектом `CDBVariant`.

> [!NOTE]
>  После выполнения определенных операций с набором записей закладки могут стать недействительными. Например, при вызове `GetBookmark`, за которым следует `Requery`, возможно, вы не сможете вернуться к записи с помощью `SetBookmark`. Вызовите метод [CDatabase:: жетбукмаркперсистенце](../../mfc/reference/cdatabase-class.md#getbookmarkpersistence) , чтобы проверить, можно ли безопасно вызвать `SetBookmark`.

Дополнительные сведения о закладках и навигации по наборам записей см. в статьях [набор записей: закладки и абсолютные позиции (ODBC)](../../data/odbc/recordset-bookmarks-and-absolute-positions-odbc.md) и [Recordset: прокрутка (ODBC)](../../data/odbc/recordset-scrolling-odbc.md).

##  <a name="getdefaultconnect"></a>CRecordset:: Жетдефаултконнект
...
`CString`, содержащая строку подключения по умолчанию.
...
Если вам нужна строка инструкции SQL для собственного использования, вызовите метод `GetSQL`, который возвращает инструкцию SQL, используемую для выбора записей набора записей при его открытии. Вы можете изменить строку SQL по умолчанию в переопределении класса `GetDefaultSQL`. Например, можно указать вызов предопределенного запроса с помощью инструкции **Call** . (Обратите внимание, что при изменении `GetDefaultSQL`необходимо также изменить `m_nFields` в соответствии с количеством столбцов в источнике данных.)
...
Можно найти поле по имени или по индексу. Значение поля можно сохранить либо в объекте `CDBVariant`, либо в объекте `CString`.

При реализации групповой выборки строк текущая запись всегда размещается на первой записи набора строк. Чтобы использовать `GetFieldValue` для записи в заданном наборе строк, необходимо сначала вызвать функцию-член [сетровсеткурсорпоситион](#setrowsetcursorposition) , чтобы переместить курсор в нужную строку в этом наборе строк. Затем вызовите `GetFieldValue` для этой строки. Для реализации групповой выборки строк необходимо указать параметр `CRecordset::useMultiRowFetch` параметра *двоптионс* в функции [Open](#open) Member.

`GetFieldValue` можно использовать для динамической выборки полей во время выполнения, а не для их статической привязки во время разработки. Например, если объект набора записей был объявлен непосредственно из `CRecordset`, то для получения данных поля необходимо использовать `GetFieldValue`. Обмен полями записей (RFX) или многоблочный обмен полями записей (групповой RFX) не реализован.

> [!NOTE]
>  Если объект набора записей объявляется без наследования от `CRecordset`, не следует загружать библиотеку курсоров ODBC. Библиотека курсоров требует, чтобы набор записей имел по крайней мере один привязанный столбец. Однако при использовании `CRecordset` напрямую ни один из столбцов не привязан. Функции члена [CDatabase:: опенекс](../../mfc/reference/cdatabase-class.md#openex) и [CDatabase:: Open](../../mfc/reference/cdatabase-class.md#open) контролируют, будет ли загружена библиотека курсоров.

`GetFieldValue` вызывает функцию API ODBC `SQLGetData`. Если драйвер выводит значение SQL_NO_TOTAL для фактической длины значения поля, `GetFieldValue` создает исключение. Дополнительные сведения о `SQLGetData`см. в Windows SDK.

### <a name="example"></a>Пример

В следующем примере кода показаны вызовы `GetFieldValue` для объекта Recordset, объявленного непосредственно из `CRecordset`.

[!code-cpp[NVC_MFCDatabase#23](../../mfc/codesnippet/cpp/crecordset-class_7.cpp)]

> [!NOTE]
>  В отличие от класса DAO `CDaoRecordset`, `CRecordset` не имеет `SetFieldValue` функции-члена. Если объект создается непосредственно из `CRecordset`, он фактически доступен только для чтения.
...
##  <a name="getodbcfieldcount"></a>CRecordset:: Жетодбкфиелдкаунт
...
##  <a name="getodbcfieldinfo"></a>CRecordset:: Жетодбкфиелдинфо
...
##  <a name="getrecordcount"></a>CRecordset:: Жетрекордкаунт
...
> [!CAUTION]
>  Число записей сохраняется как «высокий знак воды», а самая старшая запись, еще не отображаемая при перемещении по записям. Общее число записей известно только после того, как пользователь переместился за последнюю запись. В целях повышения производительности счетчик не обновляется при вызове `MoveLast`. Чтобы самостоятельно подсчитать записи, вызовите `MoveNext` несколько раз, пока `IsEOF` не вернет ненулевое значение. Добавление записи с помощью `CRecordset:AddNew` и `Update` увеличивает число; Удаление записи с помощью `CRecordset::Delete` уменьшает число.

##  <a name="getrowsetsize"></a>CRecordset:: Жетровсетсизе
...
Для реализации групповой выборки строк необходимо указать параметр `CRecordset::useMultiRowFetch` в параметре *двоптионс* функции [Open](#open) Member. Чтобы изменить параметр для размера набора строк, вызовите [сетровсетсизе](#setrowsetsize).

Дополнительные сведения о массовой выборке строк см. в статье [Набор записей: пакетная выборка строк (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md).

##  <a name="getrowsfetched"></a>CRecordset:: Жетровсфетчед
...
Это полезно при реализации групповой выборки строк. Размер набора строк обычно указывает, сколько строк будет извлечено из выборки; Однако общее число строк в наборе записей также влияет на количество строк, которые будут извлечены в наборе строк. Например, если набор записей содержит 10 записей с параметром размера набора строк 4, то цикл по набору записей путем вызова `MoveNext` приведет к тому, что будет создан последний набор строк, содержащий только 2 записи.

Для реализации групповой выборки строк необходимо указать параметр `CRecordset::useMultiRowFetch` в параметре *двоптионс* функции [Open](#open) Member. Чтобы указать размер набора строк, вызовите [сетровсетсизе](#setrowsetsize).
...
##  <a name="getrowstatus"></a>CRecordset:: Жетровстатус
...
Дополнительные сведения см. в описании функции API ODBC `SQLExtendedFetch` в Windows SDK.

##  <a name="getstatus"></a>CRecordset::/Status
...
`CRecordset` пытается отвестить индекс, но в некоторых обстоятельствах это может оказаться невозможным. Описание см. в разделе [жетрекордкаунт](#getrecordcount) .
...
- `m_lCurrentRecord` содержит Отсчитываемый от нуля индекс текущей записи в наборе записей, если он известен. Если индекс не может быть определен, этот элемент содержит AFX_CURRENT_RECORD_UNDEFINED (-2). Если `IsBOF` имеет значение TRUE (пустой набор записей или попытка прокрутки перед первой записью), `m_lCurrentRecord` имеет значение AFX_CURRENT_RECORD_BOF (-1). Если используется первая запись, то для нее устанавливается значение 0, вторая запись 1 и т. д.

- `m_bRecordCountFinal` ненулевое значение, если общее число записей в наборе записей определено. Как правило, это необходимо сделать, начав с начала набора записей и вызывая `MoveNext` до тех пор, пока `IsEOF` не вернет ненулевое значение. Если этот элемент равен нулю, то число записей, возвращенное `GetRecordCount`, если оно не равно-1, будет считаться числом записей с высоким уровнем подложки.

##  <a name="getsql"></a>CRecordset:: Жетскл
...
**Константная** ссылка на `CString`, которая содержит инструкцию SQL.

### <a name="remarks"></a>Заметки

Как правило, это инструкция SQL **SELECT** . Строка, возвращаемая `GetSQL`, доступна только для чтения.

Строка, возвращаемая `GetSQL`, обычно отличается от строки, которая могла быть передана набору записей в параметре *lpszSQL* в функцию-член `Open`. Это обусловлено тем, что набор записей формирует полную инструкцию SQL на основе того, что вы передали в `Open`, что вы указали в параметре ClassWizard, что вы могли указать в `m_strFilter` и `m_strSort` элементы данных, а также какие параметры, которые могли быть указаны. Дополнительные сведения о том, как набор записей формирует эту инструкцию SQL, см. в статье [набор записей: как наборы записей выбирают записи (ODBC)](../../data/odbc/recordset-how-recordsets-select-records-odbc.md).

> [!NOTE]
>  Вызывайте эту функцию члена только после вызова [Open](#open).

##  <a name="gettablename"></a>CRecordset:: имя_таблицы
...
**Константная** ссылка на `CString`, которая содержит имя таблицы, если набор записей основан на таблице; в противном случае — пустая строка.

### <a name="remarks"></a>Заметки

`GetTableName` допустимы только в том случае, если набор записей основан на таблице, а не на соединении нескольких таблиц или в предопределенном запросе (хранимой процедуре). Имя доступно только для чтения.

> [!NOTE]
>  Вызывайте эту функцию члена только после вызова [Open](#open).

##  <a name="isbof"></a>CRecordset:: Исбоф

Возвращает ненулевое значение, если набор записей был помещен перед первой записью. Нет текущей записи.
...
Результат `IsDeleted` зависит от многих факторов, таких как тип набора записей, возможность обновления набора записей, независимо от того, был ли указан параметр `CRecordset::skipDeletedRecords` при открытии набора записей, были ли пакеты драйверов удалены и есть ли несколько пользователей.

Дополнительные сведения о `CRecordset::skipDeletedRecords` и упаковке драйвера см. в разделе Функция [Open](#open) Member.

> [!NOTE]
>  При реализации групповой выборки строк не следует вызывать `IsDeleted`. Вместо этого вызовите функцию члена [жетровстатус](#getrowstatus) . Дополнительные сведения о массовой выборке строк см. в статье [Набор записей: пакетная выборка строк (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md).

##  <a name="iseof"></a>CRecordset:: Исеоф

Возвращает ненулевое значение, если набор записей был помещен после последней записи. Нет текущей записи.
...
Вызывайте эту функцию члена при прокрутке записи до записи, чтобы узнать, не выходит ли вы за последнюю запись набора записей. Можно также использовать `IsEOF`, чтобы определить, содержит ли набор записей какие-либо записи или пуст. Сразу после вызова `Open`, если набор записей не содержит записей, `IsEOF` возвращает ненулевое значение. При открытии набора записей, имеющего по крайней мере одну запись, первая запись является текущей, а `IsEOF` возвращает 0.

Если последняя запись является текущей записью при вызове `MoveNext`, `IsEOF` будет впоследствии возвращать ненулевое значение. Если `IsEOF` возвращает ненулевое значение и вызывается `MoveNext`, возникает ошибка. Если `IsEOF` возвращает ненулевое значение, текущая запись не определена, и любое действие, требующее текущей записи, приведет к ошибке.

### <a name="example"></a>Пример

См. пример для [исбоф](#isbof).

##  <a name="isfielddirty"></a>CRecordset:: Исфиелддирти
...
Данные всех элементов данных "грязных" полей будут передаваться в запись источника данных, когда текущая запись обновляется путем вызова функции-члена [обновления](#update) `CRecordset` (после вызова метода `Edit` или `AddNew`).

> [!NOTE]
>  Эта функция-член неприменима к наборам записей, использующим групповые выборке строк. При реализации групповой выборки строк `IsFieldDirty` всегда возвращает значение FALSE и приведет к неудачному утверждению. Дополнительные сведения о массовой выборке строк см. в статье [Набор записей: пакетная выборка строк (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md).

Вызов `IsFieldDirty` приведет к сбросу результатов предыдущих вызовов [сетфиелддирти](#setfielddirty) , так как состояние "грязного" поля повторно вычисляется. В `AddNew` случае, если текущее значение поля отличается от значения псевдо null, поле состояние устанавливается как «грязный». В `Edit` случае, если значение поля отличается от кэшированного значения, поле состояние устанавливается как «грязный».
...
##  <a name="isfieldnull"></a>CRecordset:: Исфиелднулл
...
Вызовите эту функцию-член, чтобы определить, помечен ли указанный элемент данных поля набора записей как null. (В терминологии базы данных значение NULL означает, что значение не имеет значения и не совпадает со значением C++NULL в.) Если элемент данных поля помечен как имеющий значение null, он интерпретируется как столбец текущей записи, для которой нет значения.

> [!NOTE]
>  Эта функция-член неприменима к наборам записей, использующим групповые выборке строк. При реализации групповой выборки строк `IsFieldNull` всегда возвращает значение FALSE и приведет к неудачному утверждению. Дополнительные сведения о массовой выборке строк см. в статье [Набор записей: пакетная выборка строк (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md).

`IsFieldNull` реализуется с помощью [DoFieldExchange](#dofieldexchange).

##  <a name="isfieldnullable"></a>CRecordset:: Исфиелднуллабле
...
Использование значения NULL для первого аргумента функции приведет к применению функции только к `outputColumn` полям, а не `param` полям. Например, вызов

[!code-cpp[NVC_MFCDatabase#26](../../mfc/codesnippet/cpp/crecordset-class_10.cpp)]

задаст значения NULL только для `outputColumn` полей; `param` поля не затрагиваются.

Для работы с полями `param` необходимо указать фактический адрес отдельных `param`, с которыми вы хотите работать, например:

[!code-cpp[NVC_MFCDatabase#27](../../mfc/codesnippet/cpp/crecordset-class_11.cpp)]

Это означает, что нельзя задать для всех полей `param` значение NULL, как это можно сделать с `outputColumn` полями.

`IsFieldNullable` реализуется с помощью [DoFieldExchange](#dofieldexchange).

##  <a name="isopen"></a>CRecordset:: OnOpen
...
Обычно нет необходимости напрямую обращаться к ХСТМТ, но может потребоваться для прямого выполнения инструкций SQL. Функция члена `ExecuteSQL` класса `CDatabase` предоставляет пример использования `m_hstmt`.
...
Конструктор для класса набора записей должен инициализировать `m_nFields` с правильным числом. Если не реализована многострочная выборка строк, ClassWizard записывает эту инициализацию при ее использовании для объявления класса набора записей. Его также можно записать вручную.
...
Столбцы можно привязывать динамически, как описано в статье «набор записей: динамическая привязка столбцов данных». В этом случае необходимо увеличить число в `m_nFields`, чтобы отразить число вызовов функций RFX или групповой RFX в `DoFieldExchange` или `DoBulkFieldExchange` функции члена для динамически привязанных столбцов.
...
Содержит указатель на объект `CDatabase`, через который набор записей соединяется с источником данных.

### <a name="remarks"></a>Заметки

Эта переменная задается двумя способами. Как правило, при создании объекта набора записей передается указатель на уже подключенный объект `CDatabase`. Если вместо этого передается значение NULL, `CRecordset` создает объект `CDatabase` и подключает его. В любом случае `CRecordset` сохраняет указатель в этой переменной.

Обычно не нужно напрямую использовать указатель, хранящийся в `m_pDatabase`. Однако при написании собственных расширений для `CRecordset`может потребоваться использовать указатель. Например, может потребоваться указатель, если вы создаете собственный `CDBException`. Или это может потребоваться, если нужно сделать что-то с помощью того же объекта `CDatabase`, например выполнение транзакций, установка времени ожидания или вызов `ExecuteSQL` функции-члена класса `CDatabase` для непосредственного выполнения инструкций SQL.

##  <a name="m_strfilter"></a>CRecordset:: m_strFilter

После создания объекта Recordset, но перед вызовом функции-члена `Open` используйте этот элемент данных для хранения `CString`, содержащего предложение SQL **WHERE** .

### <a name="remarks"></a>Заметки

Набор записей использует эту строку для ограничения (или фильтрации) записей, которые она выбирает во время `Open` или `Requery` вызова. Это полезно для выбора подмножества записей, например "все менеджеры, основанные на Калифорнии" ("штат = CA"). Синтаксис ODBC SQL для предложения **WHERE** :
...
После создания объекта Recordset, но перед вызовом функции-члена `Open` используйте этот элемент данных для хранения `CString`, содержащего предложение SQL **ORDER BY** .

### <a name="remarks"></a>Заметки

Набор записей использует эту строку для сортировки записей, которые она выбирает во время `Open` или `Requery` вызова. Эту функцию можно использовать для сортировки набора записей по одному или нескольким столбцам. Синтаксис ODBC SQL для предложения **ORDER BY** :
...
*вфетчтипе*<br/>
Определяет набор строк, который `Move` будет извлекать. Дополнительные сведения см. в разделе "Заметки".

### <a name="remarks"></a>Заметки

Если передать значение 0 для *nrows*, `Move` обновляет текущую запись; `Move` завершит любой текущий `AddNew` или режим `Edit` и восстановит значение текущей записи до вызова `AddNew` или `Edit`.

> [!NOTE]
>  При перемещении по набору записей удаленные записи не пропускаются. Дополнительные сведения см. в разделе [CRecordset:: IsDeleted](#isdeleted) . При открытии `CRecordset` с установленным параметром `skipDeletedRecords` `Move` утверждает, если параметр *nrows* имеет значение 0. Такое поведение предотвращает обновление строк, удаленных другими клиентскими приложениями, использующими одни и те же данные. Описание `skipDeletedRecords`см. в описании параметра *элемент dwoption* в разделе [Open](#open) .

`Move` перемещает набор записей по наборам строк. Основываясь на значениях *nrows* и *вфетчтипе*, `Move` извлекает соответствующий набор строк, а затем делает первую запись в этом наборе строк текущей записью. Если вы не реализовали групповые выборке строк, то размер набора строк всегда равен 1. При выборке набора строк `Move` напрямую вызывает функцию-член [чеккровсетеррор](#checkrowseterror) для выполнения любых ошибок, возникающих в результате выборки.

В зависимости от передаваемых значений `Move` эквивалентна другим функциям-членам `CRecordset`. В частности, значение *вфетчтипе* может указывать на функцию-член, которая более интуитивно понятна и часто является предпочтительным методом перемещения текущей записи.
...
> [!NOTE]
>  Для наборов записей прямого доступа `Move` допустимо только со значением SQL_FETCH_NEXT для *вфетчтипе*.
...
> [!CAUTION]
>  Вызов любой из функций `Move` вызывает исключение, если набор записей не содержит записей. Чтобы определить, имеются ли записи в наборе записей, вызовите `IsBOF` и `IsEOF`.
...
##  <a name="movenext"></a>Метод CRecordset:: MoveNext
...
> [!NOTE]
>  Также рекомендуется вызвать `IsEOF` перед вызовом `MoveNext`. Например, если прокрутить после конца набора записей, `IsEOF` вернет ненулевое значение; При последующем вызове `MoveNext` будет создано исключение.
...
##  <a name="moveprev"></a>CRecordset:: Мовепрев
...
> [!NOTE]
>  Также рекомендуется вызвать `IsBOF` перед вызовом `MovePrev`. Например, если вы выполнили прокрутку перед началом набора записей, `IsBOF` вернет ненулевое значение; При последующем вызове `MovePrev` будет создано исключение.
...
##  <a name="onsetoptions"></a>CRecordset:: Онсетоптионс
...
Вызовите `OnSetOptions`, чтобы задать параметры (используемые для выбора) для указанной инструкции ODBC. Платформа вызывает эту функцию члена, чтобы задать начальные параметры для набора записей. `OnSetOptions` определяет поддержку в источнике данных прокручиваемых курсоров и для параллелизма курсоров и задает соответствующие параметры набора записей. (В то время как `OnSetOptions` используется для операций выбора, `OnSetUpdateOptions` используется для операций обновления.)
...
##  <a name="onsetupdateoptions"></a>CRecordset:: Онсетупдатеоптионс
...
Вызовите `OnSetUpdateOptions`, чтобы задать параметры (используемые при обновлении) для указанной инструкции ODBC. Платформа вызывает эту функцию-член после того, как создаст ХСТМТ для обновления записей в наборе записей. (В то время как `OnSetOptions` используется для операций выбора, `OnSetUpdateOptions` используется для операций обновления.) `OnSetUpdateOptions` определяет поддержку в источнике данных прокручиваемых курсоров и для параллелизма курсоров и задает соответствующие параметры набора записей.
...
- `CRecordset::forwardOnly` набор записей только для чтения с использованием только прямой прокрутки.

   Для `CRecordset`значение по умолчанию — `CRecordset::snapshot`. Механизм по умолчанию позволяет визуальным C++ мастерам взаимодействовать с `CDaoRecordset`ми `CRecordset` ODBC и DAO, которые имеют разные значения по умолчанию.
...
- `CRecordset::appendOnly` не допускают `Edit` или `Delete` в наборе записей. Разрешить только `AddNew`. Этот параметр является взаимоисключающим с `CRecordset::readOnly`.

- `CRecordset::readOnly` открыть набор записей как "только для чтения". Этот параметр является взаимоисключающим с `CRecordset::appendOnly`.

- `CRecordset::optimizeBulkAdd` использовать подготовленную инструкцию SQL для оптимизации одновременного добавления нескольких записей. Применяется только в том случае, если не используется функция API ODBC `SQLSetPos` для обновления набора записей. Первое обновление определяет, какие поля помечены как "грязные". Этот параметр является взаимоисключающим с `CRecordset::useMultiRowFetch`.

- `CRecordset::useMultiRowFetch` реализовать групповую выборку строк, чтобы разрешить получение нескольких строк в одной операции выборки. Это расширенная функция, предназначенная для повышения производительности; Однако ClassWizard не поддерживает обмен полями с массовыми записями. Этот параметр является взаимоисключающим с `CRecordset::optimizeBulkAdd`. Обратите внимание, что при указании `CRecordset::useMultiRowFetch`параметр `CRecordset::noDirtyFieldCheck` будет включен автоматически (двойная буферизация будет недоступна); для наборов записей последовательного доступа параметр `CRecordset::useExtendedFetch` будет включен автоматически. Дополнительные сведения о массовой выборке строк см. в статье [Набор записей: пакетная выборка строк (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md).

- `CRecordset::skipDeletedRecords` пропустить все удаленные записи при переходе по набору записей. Это приведет к снижению производительности в определенных относительных выборках. Этот параметр является недопустимым для наборов записей с последовательным входом. При вызове [Move](#move) с параметром *nrows* , установленным в значение 0, и `CRecordset::skipDeletedRecords` параметре `Move` будет утверждать. Обратите внимание, что `CRecordset::skipDeletedRecords` похож на *упаковку драйвера*, т. е. удаленные строки удаляются из набора записей. Однако если пакеты драйверов записываются, то будут пропущены только те записи, которые вы удалите. записи, удаленные другими пользователями, не будут пропускаться, пока открыт набор записей. `CRecordset::skipDeletedRecords` пропустит строки, удаленные другими пользователями.

- `CRecordset::useBookmarks` могут использовать закладки в наборе записей, если они поддерживаются. Закладки замедляют получение данных, но улучшают производительность при навигации по данным. Недопустимо использовать в наборах записей последовательного доступа. Дополнительные сведения см. в статье [набор записей: закладки и абсолютные позиции (ODBC)](../../data/odbc/recordset-bookmarks-and-absolute-positions-odbc.md).

- `CRecordset::noDirtyFieldCheck` отключить автоматическую проверку измененных полей (двойная буферизация). Это повысит производительность. Однако необходимо вручную пометить поля как "грязные", вызвав функции `SetFieldDirty` и `SetFieldNull` элементов. Обратите внимание, что двойная буферизация в классе `CRecordset` похожа на двойную буферизацию в классе `CDaoRecordset`. Однако в `CRecordset`нельзя включить двойную буферизацию для отдельных полей. Вы либо включите его для всех полей, либо отключите его для всех полей. Обратите внимание, что если указать параметр `CRecordset::useMultiRowFetch`, `CRecordset::noDirtyFieldCheck` будет включен автоматически. Однако `SetFieldDirty` и `SetFieldNull` нельзя использовать для наборов записей, реализующих многострочную выборку строк.

- `CRecordset::executeDirect` не использовать подготовленную инструкцию SQL. Для повышения производительности укажите этот параметр, если функция члена `Requery` никогда не будет вызываться.

- `CRecordset::useExtendedFetch` реализовать `SQLExtendedFetch` вместо `SQLFetch`. Это предназначено для реализации групповой выборки строк для наборов записей с последовательным входом. Если указать параметр `CRecordset::useMultiRowFetch` в наборе записей последовательного доступа, `CRecordset::useExtendedFetch` будет включен автоматически.

- `CRecordset::userAllocMultiRowBuffers` пользователь будет выделять буферы хранилища для данных. Используйте этот параметр вместе с `CRecordset::useMultiRowFetch`, если требуется выделить собственное хранилище. в противном случае платформа автоматически выделит необходимое хранилище. Дополнительные сведения см. в статье [набор записей: групповое извлечение записей (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md). Обратите внимание, что указание `CRecordset::userAllocMultiRowBuffers` без указания `CRecordset::useMultiRowFetch` приведет к неудачному утверждению.

### <a name="return-value"></a>Возвращаемое значение

Ненулевое значение, если объект `CRecordset` был успешно открыт; в противном случае 0, если метод [CDatabase:: Open](../../mfc/reference/cdatabase-class.md#open) (если вызывается) возвращает значение 0.
...
Соединение этого набора записей с источником данных зависит от способа создания набора записей перед вызовом `Open`. Если передать объект [CDatabase](../../mfc/reference/cdatabase-class.md) в конструктор набора записей, который не был подключен к источнику данных, эта функция-член использует [жетдефаултконнект](#getdefaultconnect) , чтобы попытаться открыть объект базы данных. Если значение NULL передается конструктору набора записей, конструктор конструирует объект `CDatabase` и `Open` пытается подключиться к объекту базы данных. Дополнительные сведения о закрытии набора записей и подключении в этих различных обстоятельствах см. в разделе [Close](#close).

> [!NOTE]
>  Доступ к источнику данных через объект `CRecordset` всегда является общим. В отличие от класса `CDaoRecordset` нельзя использовать объект `CRecordset` для открытия источника данных с монопольным доступом.
...
Обычная процедура заключается в передаче значения NULL в `Open`; в этом случае `Open` вызывает функцию [GetDefaultSQL](#getdefaultsql). Если используется производный класс `CRecordset`, `GetDefaultSQL` присваивает имена таблиц, указанных в ClassWizard. Вместо этого можно указать другие сведения в параметре `lpszSQL`.

Что угодно, `Open` формирует конечную строку SQL для запроса (строка может содержать предложения **WHERE** и **ORDER BY** , добавленные в переданную строку `lpszSQL`), а затем выполняет запрос. Вы можете проверить созданную строку, вызвав [жетскл](#getsql) после вызова `Open`. Дополнительные сведения о том, как набор записей формирует инструкцию SQL и выбирает записи, см. в статье [набор записей: как наборы записей выбирают записи (ODBC)](../../data/odbc/recordset-how-recordsets-select-records-odbc.md).
...
В следующих примерах кода показаны различные формы вызова `Open`.

[!code-cpp[NVC_MFCDatabase#16](../../mfc/codesnippet/cpp/crecordset-class_15.cpp)]

##  <a name="refreshrowset"></a>CRecordset:: Рефрешровсет
...
Чтобы использовать `RefreshRowset`, необходимо реализовать групповую выборку строк, указав параметр `CRecordset::useMulitRowFetch` в функции [Open](#open) Member.

`RefreshRowset` вызывает функцию API ODBC `SQLSetPos`. Параметр *влокктипе* задает состояние блокировки строки после выполнения `SQLSetPos`. В следующей таблице описаны возможные значения для *влокктипе*.

|влокктипе|Описание|
|---------------|-----------------|
|SQL_LOCK_NO_CHANGE (значение по умолчанию)|Драйвер или источник данных гарантирует, что строка находится в том же заблокированном или разблокированном состоянии, что и до вызова `RefreshRowset`.|
...
Если попытка перестроения набора записей завершается ошибкой, набор записей закрывается. Перед вызовом `Requery`можно определить, можно ли выполнить повторный запрос к набору записей, вызвав функцию члена `CanRestart`. `CanRestart` не гарантирует, что `Requery` будет выполнена.

> [!CAUTION]
>  Вызовите `Requery` только после вызова [Open](#open).
...
##  <a name="setabsoluteposition"></a>CRecordset:: Сетабсолутепоситион
...
Кроме того, можно передавать отрицательные значения в `SetAbsolutePosition`. В этом случае расположение набора записей вычисляется с конца набора записей. Например, `SetAbsolutePosition( -1 )` перемещает указатель текущей записи на последнюю запись в наборе записей.

> [!NOTE]
>  Абсолютное положение не предназначено для использования в качестве номера записи-заместителя. Закладки по-прежнему являются рекомендуемым способом удержания и возврата в заданную точку, так как при удалении предыдущих записей изменяется ее расположение. Кроме того, нельзя гарантировать, что заданная запись будет иметь ту же абсолютное положение, если набор записей снова создается повторно, так как порядок отдельных записей в наборе записей не гарантируется, если он не создан с помощью инструкции SQL с предложением **ORDER BY** предложение.
...
##  <a name="setbookmark"></a>CRecordset:: Сетбукмарк
...
> [!NOTE]
>  Если закладки не поддерживаются или недоступны, вызов `SetBookmark` приведет к возникновению исключения. Закладки не поддерживаются для наборов записей с последовательным входом.

Чтобы сначала получить закладку для текущей записи, вызовите метод «@ [Bookmark](#getbookmark)», который сохраняет значение закладки в объект `CDBVariant`. Позже можно будет вернуться к этой записи, вызвав `SetBookmark`, используя сохраненное значение закладки.

> [!NOTE]
>  После выполнения определенных операций с набором записей следует проверить сохраняемость закладки перед вызовом `SetBookmark`. Например, если вы получаете закладку с `GetBookmark` а затем вызываете `Requery`, закладка может стать недействительной. Вызовите метод [CDatabase:: жетбукмаркперсистенце](../../mfc/reference/cdatabase-class.md#getbookmarkpersistence) , чтобы проверить, можно ли безопасно вызвать `SetBookmark`.

Дополнительные сведения о закладках и навигации по наборам записей см. в статьях [набор записей: закладки и абсолютные позиции (ODBC)](../../data/odbc/recordset-bookmarks-and-absolute-positions-odbc.md) и [Recordset: прокрутка (ODBC)](../../data/odbc/recordset-scrolling-odbc.md).

##  <a name="setfielddirty"></a>CRecordset:: Сетфиелддирти
...
> [!NOTE]
>  Эта функция-член неприменима к наборам записей, использующим групповые выборке строк. Если вы реализовали групповую выборку строк, `SetFieldDirty` приведет к неудачному утверждению. Дополнительные сведения о массовой выборке строк см. в статье [Набор записей: пакетная выборка строк (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md).

Платформа помечает измененные элементы данных поля, чтобы убедиться, что они будут записаны в запись источника данных с помощью механизма обмена полями записей (RFX). Изменение значения поля обычно устанавливает поле как "грязное" автоматически, поэтому вам редко приходится вызывать `SetFieldDirty` самостоятельно, но иногда требуется обеспечить явное обновление или вставку столбцов независимо от значения в данных поля. участниками.
...
##  <a name="setfieldnull"></a>CRecordset:: Сетфиелднулл
...
> [!NOTE]
>  При установке параметров в значение NULL вызов `SetFieldNull` до открытия набора записей приводит к утверждению. В этом случае вызовите [сетпарамнулл](#setparamnull).

`SetFieldNull` реализуется с помощью [DoFieldExchange](#dofieldexchange).

##  <a name="setlockingmode"></a>CRecordset:: Сетлоккингмоде
...
- `optimistic` Оптимистическая блокировка фиксирует обновляемую запись только во время вызова `Update`.

- `pessimistic` пессимистичная Блокировка блокирует запись сразу после вызова `Edit` и сохраняет ее до тех пор, пока не завершится вызов `Update` или не перейдете к новой записи.

### <a name="remarks"></a>Заметки

Эту функцию-член следует вызывать, если необходимо указать, какие из двух стратегий блокировки записей использует набор записей для обновлений. По умолчанию режим блокировки набора записей `optimistic`. Это можно сделать более осторожным, `pessimistic` стратегия блокировки. Вызовите `SetLockingMode` после создания и открытия объекта Recordset, но перед вызовом `Edit`.

##  <a name="setparamnull"></a>CRecordset:: Сетпарамнулл
...
##  <a name="setrowsetcursorposition"></a>CRecordset:: Сетровсеткурсорпоситион
...
Чтобы использовать `SetRowsetCursorPosition`, необходимо реализовать групповую выборку строк, указав параметр `CRecordset::useMultiRowFetch` параметра *двоптионс* в функции [Open](#open) Member.

`SetRowsetCursorPosition` вызывает функцию API ODBC `SQLSetPos`. Параметр *влокктипе* задает состояние блокировки строки после выполнения `SQLSetPos`. В следующей таблице описаны возможные значения для *влокктипе*.

|влокктипе|Описание|
|---------------|-----------------|
|SQL_LOCK_NO_CHANGE (значение по умолчанию)|Драйвер или источник данных гарантирует, что строка находится в том же заблокированном или разблокированном состоянии, что и до вызова `SetRowsetCursorPosition`.|
|SQL_LOCK_EXCLUSIVE|Драйвер или источник данных блокирует строку в монопольном режиме. Не все источники данных поддерживают этот тип блокировки.|
|SQL_LOCK_UNLOCK|Драйвер или источник данных разблокирует строку. Не все источники данных поддерживают этот тип блокировки.|

Дополнительные сведения о `SQLSetPos`см. в Windows SDK. Дополнительные сведения о массовой выборке строк см. в статье [Набор записей: пакетная выборка строк (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md).

##  <a name="setrowsetsize"></a>CRecordset:: Сетровсетсизе
...
Эта виртуальная функция-член указывает, сколько строк вы хотите получить во время одной выборки при использовании групповой выборки строк. Для реализации групповой выборки строк необходимо задать параметр `CRecordset::useMultiRowFetch` в параметре *двоптионс* функции [Open](#open) Member.

> [!NOTE]
>  Вызов `SetRowsetSize` без реализации групповой выборки строк приведет к неудачному утверждению.

Вызовите `SetRowsetSize` перед вызовом `Open`, чтобы изначально задать размер набора строк для набора записей. Размер набора строк по умолчанию при реализации групповой выборки строк равен 25.

> [!NOTE]
>  Будьте внимательны при вызове `SetRowsetSize`. Если вы вручную выделили хранилище для данных (как указано параметром `CRecordset::userAllocMultiRowBuffers` параметра Двоптионс в `Open`), следует проверить необходимость перераспределения буферов хранилища после вызова `SetRowsetSize`, но перед выполнением курсора Операция навигации.
...
Завершает операцию `AddNew` или `Edit`, сохраняя новые или измененные данные в источнике данных.
...
Вызовите эту функцию-член после вызова функции-члена [AddNew](#addnew) или [Edit](#edit) . Этот вызов необходим для завершения операции `AddNew` или `Edit`.
...
Если источник данных поддерживает транзакции, можно сделать вызов `Update` (и соответствующего `AddNew` или `Edit` вызов) транзакции. Дополнительные сведения о транзакциях см. в описании [транзакции статьи (ODBC)](../../data/odbc/transaction-odbc.md).

> [!CAUTION]
>  При вызове `Update` без предварительного вызова `AddNew` или `Edit``Update` создает исключение `CDBException`. При вызове `AddNew` или `Edit`необходимо вызвать `Update` перед вызовом операции `Move` или перед закрытием набора записей или соединения с источником данных. В противном случае изменения будут потеряны без уведомления.

Дополнительные сведения об обработке ошибок `Update` см. в статье [набор записей: как набор записей обновляет записи (ODBC)](../../data/odbc/recordset-how-recordsets-update-records-odbc.md).
