...
Результат `IsDeleted` зависит от многих факторов, таких как тип набора записей, возможность обновления набора записей, независимо от того, был ли указан параметр `CRecordset::skipDeletedRecords` при открытии набора записей, были ли пакеты драйверов удалены и существует ли несколько пользователей.
...
Если класс набора записей содержит какие – либо элементы данных параметров, конструктор класса должен инициализировать `m_nParams` с правильным числом. Значение `m_nParams` по умолчанию равно 0. При добавлении элементов данных параметров (которые необходимо выполнить вручную) необходимо вручную добавить инициализацию в конструктор класса, чтобы отразить количество параметров (которые должны быть не меньше, чем количество заполнителей "" в `m_strFilter` или `m_strSort` строке).
...
> [!NOTE]
>  Для наборов записей прямого доступа `Move` допустимы только со значением SQL_FETCH_NEXT для *вфетчтипе*.
...
> [!NOTE]
>  Будьте внимательны при вызове `SetRowsetSize`. Если вы вручную выделили хранилище для данных (как указано параметром `CRecordset::userAllocMultiRowBuffers` параметра Двоптионс в `Open`), следует проверить необходимость перераспределения буферов хранилища после вызова `SetRowsetSize`, но перед выполнением любой операции перехода курсора.
