...
|Параметр|Описание|
|---------------|-----------------|
|*iItem*|окне Отсчитываемый от нуля индекс элемента заголовка, стиль которого HDF_SPLITBUTTON. Дополнительные сведения см. в `fmt` описании члена структуры [хдитем](/windows/win32/api/commctrl/ns-commctrl-hditemw) .|
