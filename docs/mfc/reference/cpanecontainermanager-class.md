...
### <a name="remarks"></a>Примечания  
 ...
 `CPaneContainerManager` Класс хранит указатель на корень двоичного дерева, построенный из [CPaneContainer](../../mfc/reference/cpanecontainer-class.md) объектов.  
  
## <a name="example"></a>Пример  
 Следующий пример демонстрирует получение ссылки на `CPaneContainerManager` объект. Этот фрагмент кода является частью [задать размер области пример](../../visual-cpp-samples.md).  
