...
Для поддержки исключений баз данных классы баз данных предоставляют два класса исключений: `CDBException` и `CDaoException`, а также глобальные функции для поддержки типов исключений.
...
*exception_object_pointer_name*<br/>
Задает имя для указателя на объект Exception, который будет создан с помощью макроса. Для доступа к объекту исключения в блоке `CATCH_ALL` можно использовать имя указателя. Эта переменная объявлена для вас.

### <a name="remarks"></a>Remarks

Код обработки исключений может опрашивать объект исключения, если это уместно, чтобы получить дополнительные сведения о конкретной причине исключения. Вызов макроса `THROW_LAST` для сдвига обработки на следующий внешний кадр исключения. Если вы используете **CATCH_ALL**, завершите блок **try** с помощью макроса END_CATCH_ALL.
...
*лпсзарчивенаме*<br/>
Указывает на строку, содержащую имя объекта `CArchive`, вызвавшего исключение (если доступно).
...
Эта функция обычно вызывается сразу после того, как `AfxMessageBox` сообщил пользователю об ошибке.
...
Сведения об исключениях, связанных с классами MFC DAO, см. в разделе `CDaoException` классов в этой книге, а также в статье [исключения: исключения базы данных](../../mfc/exceptions-database-exceptions.md).
...
*файле*<br/>
Указатель на объект `CDatabase`, представляющий соединение с источником данных, с которым связано исключение.
...
Платформа вызывает `AfxThrowDBException` при получении ODBC РЕТКОДЕ из вызова функции API ODBC и интерпретирует РЕТКОДЕ как исключительную ситуацию, а не как предполагаемую ошибку. Например, операция доступа к данным может завершиться ошибкой из-за ошибки чтения с диска.
...
`AfxAbort` вызывается внутренне функциями-членами MFC при возникновении неустранимой ошибки, например неперехваченного исключения, которое не может быть обработано. Вы можете вызвать `AfxAbort` в редких случаях, когда возникает фатальная ошибка, из-за которой невозможно выполнить восстановление.
