...
> [!NOTE]
>  **CATCH_ALL** блока определяется как C++ область, разделенные символами фигурные скобки. При объявлении переменных в этой области, они доступны только в пределах этой области.
...
> [!NOTE]
>  **AND_CATCH** блока определяется как C++ область (разделенные символами фигурные скобки). При объявлении переменных в этой области, помните, что они доступны только внутри этой области. Это также относится к *exception_object_pointer_name* переменной.
...
> [!NOTE]
>  **AND_CATCH_ALL** блока определяется как C++ область (разделенные символами фигурные скобки). При объявлении переменных в этой области, помните, что они доступны только внутри этой области.
...
Помечает конец последнего <strong>CATCH_ALL88 или ** AND_CATCH_ALL</strong> блока.
