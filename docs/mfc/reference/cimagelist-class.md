...
|[CImageList:: Attach](#attach)|Присоединяет список изображений к объекту `CImageList`.|
|[CImageList:: Бегиндраг](#begindrag)|Начинает перетаскивать изображение.|
|[CImageList:: Copy](#copy)|Копирует изображение в объект `CImageList`.|
|[CImageList:: Create](#create)|Инициализирует список изображений и прикрепляет его к объекту `CImageList`.|
|[CImageList::D Елетеимажелист](#deleteimagelist)|Удаляет список изображений.|
|[CImageList::D Елететемпмап](#deletetempmap)|Вызывается обработчиком времени простоя [CWinApp](../../mfc/reference/cwinapp-class.md) для удаления любого временного объекта `CImageList`, созданного `FromHandle`.|
|[CImageList::D етач](#detach)|Отсоединяет объект списка изображений от объекта `CImageList` и возвращает маркер в список изображений.|
...
|[CImageList:: Фромхандлеперманент](#fromhandlepermanent)|Возвращает указатель на `CImageList` объект при наличии маркера для списка изображений. Если объект `CImageList` не присоединен к маркеру, возвращается значение NULL.|
...
|[CImageList:: Жетсафехандле](#getsafehandle)|Возвращает `m_hImageList`.|
...
Этот элемент управления (и, следовательно, класс `CImageList`) доступен только для программ, работающих под управлением Windows 95/98 и Windows NT версии 3,51 и более поздних версий.

Дополнительные сведения об использовании `CImageList`см. в разделе [элементы управления](../../mfc/controls-mfc.md) и [Использование CImageList](../../mfc/using-cimagelist.md).
...
Вызовите эту функцию, чтобы присоединить список изображений к объекту `CImageList`.
...
##  <a name="begindrag"></a>CImageList:: Бегиндраг
...
*нфлагс*<br/>
Указывает тип создаваемого списка изображений. Этот параметр может быть сочетанием следующих значений, но может включать только одно из значений `ILC_COLOR`.
...
Создание `CImageList` выполняется в два этапа. Сначала вызовите конструктор, а затем вызовите `Create`, который создает список изображений и присоединяет его к объекту `CImageList`.

### <a name="example"></a>Пример

[!code-cpp[NVC_MFC_CImageList#7](../../mfc/reference/codesnippet/cpp/cimagelist-class_5.cpp)]

##  <a name="deleteimagelist"></a>CImageList::D Елетеимажелист
...
##  <a name="deletetempmap"></a>CImageList::D Елететемпмап

Автоматически вызывается обработчиком `CWinApp` времени простоя, `DeleteTempMap` удаляет все временные `CImageList` объекты, созданные [FromHandle](#fromhandle), но не уничтожает дескрипторы (`hImageList`), временно связанные с объектами `ImageList`.
...
##  <a name="detach"></a>CImageList::D етач
...
##  <a name="dragenter"></a>CImageList::D Ражентер
...
##  <a name="dragleave"></a>CImageList::D Раглеаве
...
##  <a name="dragmove"></a>CImageList::D Рагмове
...
Эта функция обычно вызывается в ответ на WM_MOUSEMOVE сообщение. Чтобы начать операцию перетаскивания, используйте функцию-член `BeginDrag`.

### <a name="example"></a>Пример

[!code-cpp[NVC_MFC_CImageList#4](../../mfc/reference/codesnippet/cpp/cimagelist-class_8.cpp)]

##  <a name="dragshownolock"></a>CImageList::D Рагшовнолокк
...
##  <a name="drawex"></a>CImageList::D Равекс
...
##  <a name="drawindirect"></a>CImageList::D Равиндирект
...
##  <a name="enddrag"></a>CImageList:: Енддраг
...
##  <a name="extracticon"></a>CImageList:: Екстрактикон
...
Указатель на объект `CImageList` в случае успеха; в противном случае — NULL.

### <a name="remarks"></a>Remarks

Если `CImageList` еще не присоединен к этому обработчику, создается и прикрепляется временный объект `CImageList`. Этот временный объект `CImageList` действителен только до тех пор, пока приложение не найдет время простоя в цикле событий, во время которого все временные объекты будут удалены.

### <a name="example"></a>Пример

[!code-cpp[NVC_MFC_CImageList#13](../../mfc/reference/codesnippet/cpp/cimagelist-class_13.cpp)]

##  <a name="fromhandlepermanent"></a>CImageList:: Фромхандлеперманент
...
Если объект `CImageList` не присоединен к маркеру, возвращается значение NULL.

### <a name="example"></a>Пример

[!code-cpp[NVC_MFC_CImageList#14](../../mfc/reference/codesnippet/cpp/cimagelist-class_14.cpp)]

##  <a name="getbkcolor"></a>CImageList:: Жетбкколор
...
Значение цвета RGB цвета фона объекта `CImageList`.

### <a name="example"></a>Пример

  См. пример для [CImageList:: сетбкколор](#setbkcolor).

##  <a name="getdragimage"></a>CImageList:: Жетдрагимаже
...
*лппоинт*<br/>
Адрес структуры [точек](/previous-versions/dd162805\(v=vs.85\)) , которая получает текущую позицию перетаскивания.

*лппоинсотспот*<br/>
Адрес структуры `POINT`, получающей смещение изображения перетаскивания относительно позиции перетаскивания.
...
##  <a name="getimagecount"></a>CImageList:: Жетимажекаунт
...
##  <a name="getimageinfo"></a>CImageList:: Жетимажеинфо
...
Структура `IMAGEINFO` содержит сведения о изображении в списке изображений.

##  <a name="getsafehandle"></a>CImageList:: Жетсафехандле

Вызовите эту функцию, чтобы получить элемент данных `m_hImageList`.
...
Элемент данных `m_hImageList` является открытой переменной типа ХИМАЖЕЛИСТ.

### <a name="example"></a>Пример

[!code-cpp[NVC_MFC_CImageList#23](../../mfc/reference/codesnippet/cpp/cimagelist-class_16.cpp)]

##  <a name="operator_himagelist"></a>CImageList:: operator ХИМАЖЕЛИСТ

Используйте этот оператор, чтобы получить присоединенный обработчик объекта `CImageList`.
...
В случае успеха — обработчик списка изображений, представленного объектом `CImageList`; в противном случае — NULL.
...
Версия, возвращающая значение **int** , возвращает отсчитываемый от нуля индекс изображения в случае успеха. в противном случае — 1.
...
##  <a name="setbkcolor"></a>CImageList:: Сетбкколор
...
##  <a name="setdragcursorimage"></a>CImageList:: Сетдрагкурсоримаже
...
Поскольку функции перетаскивания используют новый образ во время операции перетаскивания, следует использовать функцию Windows [шовкурсор](/windows/win32/api/winuser/nf-winuser-showcursor) , чтобы скрыть фактический курсор мыши после вызова `CImageList::SetDragCursorImage`. В противном случае система может иметь два курсора мыши в течение операции перетаскивания.

##  <a name="setimagecount"></a>CImageList:: Сетимажекаунт

Вызовите эту функцию-член, чтобы сбросить количество образов в объекте `CImageList`.
...
##  <a name="setoverlayimage"></a>CImageList:: Сетоверлайимаже
