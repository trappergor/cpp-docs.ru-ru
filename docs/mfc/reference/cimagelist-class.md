...
|[CImageList::D Елететемпмап](#deletetempmap)|Вызывается обработчиком времени простоя [CWinApp](../../mfc/reference/cwinapp-class.md) для удаления любого временного `CImageList` объекта, `FromHandle`созданного.|
...
|[CImageList:: Фромхандлеперманент](#fromhandlepermanent)|Возвращает указатель на `CImageList` объект при наличии маркера для списка изображений. `CImageList` Если объект не присоединен к маркеру, возвращается значение null.|
...
Этот элемент управления (и, `CImageList` следовательно, класс) доступен только для программ, работающих под управлением Windows 95/98 и Windows NT версии 3,51 и более поздних версий.
...
Вызовите эту функцию, чтобы присоединить список изображений `CImageList` к объекту.
...
Эта функция создает временный список изображений, который используется для перетаскивания. Изображение объединяет указанное изображение и его маску с текущим курсором. В ответ на последующие сообщения WM_MOUSEMOVE можно переместить изображение перетаскивания с помощью `DragMove` функции члена. Чтобы завершить операцию перетаскивания, можно использовать `EndDrag` функцию члена.
...
|Значение|Значение|
|-----------|-------------|
|ILCF_MOVE|Исходное изображение копируется в индекс конечного образа. Эта операция приводит к получению нескольких экземпляров заданного образа. По умолчанию используется ILCF_MOVE.|
...
|Значение|Значение|
|-----------|-------------|
|ILC_COLOR|Используйте поведение по умолчанию, если не задан ни один из других флагов ILC_COLOR *. Как правило, значение по умолчанию — ILC_COLOR4; но для старых драйверов дисплея значение по умолчанию — ILC_COLORDDB.|
...
Построение создается `CImageList` в два этапа. Сначала вызовите конструктор, а затем вызовите метод `Create`, который создает список изображений и прикрепляет его `CImageList` к объекту.
...
Вызывается автоматически `CWinApp` обработчиком времени простоя, `DeleteTempMap` удаляет все временные `CImageList` объекты, созданные [FromHandle](#fromhandle), но не уничтожает дескрипторы ( `hImageList`), `ImageList` временно связанные с объект.
...
Вызовите эту функцию, чтобы отсоединить объект списка изображений `CImageList` от объекта.
...
Эта функция обычно вызывается в ответ на сообщение WM_MOUSEMOVE. Чтобы начать операцию перетаскивания, используйте `BeginDrag` функцию-член.
...
|ILD_NORMAL|Рисует изображение, используя цвет фона для списка изображений. Если цвет фона является значением CLR_NONE, изображение прозрачно рисуется с помощью маски.|
...
*ргбфоре*<br/>
Цвет переднего плана изображения по умолчанию CLR_DEFAULT. Этот параметр может быть значением RGB, определенным приложением, или одним из следующих значений:
...
Если объект не присоединен к обработчику, создается и подключается временный `CImageList`. `CImageList` Этот временный `CImageList` объект действителен только до тех пор, пока приложение не найдет время простоя в цикле событий, во время которого все временные объекты будут удалены.
...
`CImageList` Если объект не присоединен к маркеру, возвращается значение null.
...
Значение цвета RGB цвета фона `CImageList` объекта.
...
Вызовите эту функцию, чтобы `m_hImageList` получить элемент данных.
...
Элемент `m_hImageList` данных является открытой переменной типа химажелист.
...
*парчиве*<br/>
Указатель на `CArchive` объект, из которого считывается список изображений.
...
*Кредит*<br/>
Цвет фона для установки. Это может быть CLR_NONE. В этом случае изображения прозрачно рисуются с помощью маски.
