...
|[CToolBar:: Create](#create)|Создает панель инструментов Windows и прикрепляет ее к объекту `CToolBar`.|
|[CToolBar:: Креатикс](#createex)|Создает объект `CToolBar` с дополнительными стилями для внедренного объекта `CToolBarCtrl`.|
...
1. Вызовите функцию [CREATE](#create) (или [креатикс](#createex)), чтобы создать панель инструментов Windows и присоединить ее к объекту `CToolBar`.
...
Чтобы задать отдельную кнопку или стиль разделителя, не указывая на массив, вызовите [жетбуттонстиле](#getbuttonstyle) для получения стиля, а затем вызовите [сетбуттонстиле](#setbuttonstyle) вместо `SetButtons`. `SetButtonStyle` наиболее полезна, если нужно изменить стиль кнопки во время выполнения.
...
Чтобы создать кнопку флажка, присвойте ей стиль TBBS_CHECKBOX или используйте функцию-член `SetCheck` объекта `CCmdUI` в обработчике ON_UPDATE_COMMAND_UI. Вызов `SetCheck` превращает кнопку в флажок. Передайте `SetCheck` аргумента 0, значение 1 для параметра Checked или 2 для неопределенного.

Чтобы создать переключатель, вызовите функцию-член [сетрадио](../../mfc/reference/ccmdui-class.md#setradio) объекта [Поддержка CCmdUI](../../mfc/reference/ccmdui-class.md) из обработчика ON_UPDATE_COMMAND_UI. Передайте `SetRadio` аргумента 0 для проверки или ненулевого параметра. Чтобы обеспечить взаимоисключающее поведение группы переключателей, необходимо иметь обработчики ON_UPDATE_COMMAND_UI для всех кнопок в группе.
...
Эта функция-член создает панель инструментов Windows (дочернее окно) и связывает ее с объектом `CToolBar`.
...
Вызовите эту функцию, чтобы создать панель инструментов Windows (дочернее окно) и связать ее с объектом `CToolBar`.
...
Используйте `CreateEx`, а не [CREATE](#create), если определенные стили должны присутствовать во время создания встроенного элемента управления панели инструментов. Например, задайте для параметра *двктрлстиле* значение &#124; TBSTYLE_FLAT TBSTYLE_TRANSPARENT, чтобы создать панель инструментов, похожую на панели инструментов Internet Explorer 4.
...
Эта функция-член конструирует объект `CToolBar` и задает размеры по умолчанию.
...
Эта функция-член заполняет структуру `RECT`, адрес которой содержится в *лпрект* , с координатами кнопки или разделителя, заданными параметром *ниндекс*.
...
Используйте `GetToolBarCtrl`, чтобы воспользоваться возможностями стандартного элемента управления панели инструментов Windows и воспользоваться преимуществами [CToolBarCtrl](../../mfc/reference/ctoolbarctrl-class.md) поддержки для настройки панели инструментов.
...
Вызовите эту функцию члена, чтобы загрузить точечный рисунок, заданный `lpszResourceName` или `nIDResource`.
...
Например, вызовите `SetBitmap`, чтобы изменить изображение точечного рисунка после того, как пользователь выберет действие над документом, который изменяет действие кнопки.
