...
### <a name="remarks"></a>Примечания  
 Если *bActivate* имеет значение TRUE, элемент активизирован; Если значение равно FALSE, он неактивен.  
 ...
 *bLarger*  
 Если значение равно TRUE, *lprc* используется для указания прямоугольник отображения текста и получает соответствующие прямоугольника окна. Если значение равно FALSE, *lprc* используется для указания прямоугольник окна, и он получает соответствующий прямоугольник отображения текста.  
...
### <a name="parameters"></a>Параметры  
 *pParentWnd*  
 Указывает родительского окна элемента управления всплывающей подсказки, обычно `CDialog`. Оно не должно быть NULL.  
...
### <a name="remarks"></a>Примечания  
 Создании `CToolTipCtrl` в два этапа. Во-первых, вызовите конструктор для создания `CToolTipCtrl` объекта, а затем вызвать `Create` Создание управления всплывающей подсказки и присоединить его к `CToolTipCtrl` объекта.  
...
|TTS_NOPREFIX|Этот стиль предотвращает удаление из строкового амперсанд (&). Если всплывающая подсказка не имеет стиль TTS_NOPREFIX, система автоматически удаляет символы амперсанда, что позволяет приложению использовать ту же строку, элемент меню, а текст элемента управления всплывающей подсказки.|  
  
 Всплывающая подсказка имеет WS_POPUP и WS_EX_TOOLWINDOW стили окна, независимо от их при создании элемента управления.  
  
 Для создания элемента управления всплывающей подсказки с помощью windows расширенных стилей, вызовите [CToolTipCtrl::CreateEx](#createex) вместо `Create`.  
...
### <a name="remarks"></a>Примечания  
 Необходимо вызвать метод `Create` после создания объекта.  
...
|Параметр|Описание:|  
|---------------|-----------------|  
|[out] *lpToolInfo*|Указатель на [TOOLINFO](http://msdn.microsoft.com/library/windows/desktop/bb760256) структуры, который получает сведения о текущем окно всплывающей подсказки.|  
  
### <a name="return-value"></a>Возвращаемое значение  
 Значение TRUE, если данные получены успешно. в противном случае — значение FALSE.  
...
- Получить TTDT_AUTOPOP время подсказки окна остается видимым, если указатель находится в границах это средство ограничивающего прямоугольника.  
  
- Получить TTDT_INITIAL отображается количество раз, когда указатель мыши должен неподвижно находиться внутри ограничивающего прямоугольника инструмента перед подсказки.  
  
- Получить TTDT_RESHOW время, необходимое для последующих окнами подсказка для отображения в качестве указателя перемещает из одного набора инструментов в другое.  
