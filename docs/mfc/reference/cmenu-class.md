...
|[Кмену:: Attach](#attach)|Присоединяет маркер меню Windows к объекту `CMenu`.|
...
|[Кмену:: Креатемену](#createmenu)|Создает пустое меню и прикрепляет его к объекту `CMenu`.|
|[Кмену:: Креатепопупмену](#createpopupmenu)|Создает пустое всплывающее меню и прикрепляет его к объекту `CMenu`.|
...
|[Кмену::D Елететемпмап](#deletetempmap)|Удаляет все временные `CMenu` объекты, созданные функцией члена `FromHandle`.|
|[Кмену::D Естроймену](#destroymenu)|Уничтожает меню, присоединенное к `CMenu` объекту, и освобождает память, занятую меню.|
|[Кмену::D етач](#detach)|Отсоединяет маркер меню Windows от объекта `CMenu` и возвращает маркер.|
...
|[Кмену:: Жетсафехмену](#getsafehmenu)|Возвращает `m_hMenu`, упакованный этим объектом `CMenu`.|
...
|[Кмену:: Лоадмену](#loadmenu)|Загружает ресурс меню из исполняемого файла и прикрепляет его к объекту `CMenu`.|
|[Кмену:: Лоадменуиндирект](#loadmenuindirect)|Загружает меню из шаблона в памяти и прикрепляет его к объекту `CMenu`.|
...
|Имя|Description|
|----------|-----------------|
|[Кмену:: m_hMenu](#m_hmenu)|Задает маркер меню Windows, присоединенного к объекту `CMenu`.|

## <a name="remarks"></a>Remarks

Он предоставляет функции-члены для создания, отслеживания, обновления и уничтожения меню.

Создайте объект `CMenu` в кадре стека в качестве локального, а затем вызовите функции-члены `CMenu`, чтобы управлять новым меню по мере необходимости. Затем вызовите [CWnd:: сетмену](../../mfc/reference/cwnd-class.md#setmenu) , чтобы задать для меню окно, а затем сразу же вызвав функцию-член [отсоединения](#detach) объекта `CMenu`. Функция члена `CWnd::SetMenu` устанавливает меню окна в новое меню, вызывает перерисовку окна, чтобы отразить изменение меню, а также передается право на владение окном. Вызов `Detach` отсоединяет HMENU от объекта `CMenu`, поэтому когда локальная переменная `CMenu` выходит за пределы области видимости, деструктор объекта `CMenu` не пытается уничтожить меню, которое больше не принадлежит. Само меню автоматически уничтожается при уничтожении окна.
...
|------------|-----------------------------------|
|MF_OWNERDRAW|Содержит предоставляемое приложением 32-разрядное значение, которое приложение может использовать для обслуживания дополнительных данных, связанных с элементом меню. Это 32-разрядное значение доступно приложению при обработке WM_MEASUREITEM и WM_DRAWITEM сообщений. Значение хранится в `itemData` члене структуры, предоставляемой этими сообщениями.|
...
*pBmp*<br/>
Указывает на объект `CBitmap`, который будет использоваться в качестве пункта меню.
...
Приложение может указать состояние пункта меню, задав значения в *нфлагс*. Если *нидневитем* указывает всплывающее меню, оно становится частью меню, к которому он добавляется. Если это меню уничтожается, добавленное меню также будет уничтожено. Добавленное меню следует отсоединить от `CMenu` объекта, чтобы избежать конфликта. Обратите внимание, что MF_STRING и MF_OWNERDRAW недопустимы для битовой версии `AppendMenu`.
...
- MF_UNCHECKED выступает в качестве переключателя с MF_CHECKED, чтобы снять флажок рядом с элементом. Когда приложение предоставляет точечные рисунки с галочкой (см. `SetMenuItemBitmaps` функция-член), отображается точечный рисунок галочки выкл.
...
Эта функция не должна вызываться, если меню уже присоединено к `CMenu` объекту. Маркер меню хранится в элементе данных `m_hMenu`.
...
Создает меню и прикрепляет его к объекту `CMenu`.
...
Меню изначально пусто. Элементы меню можно добавить с помощью функции члена `AppendMenu` или `InsertMenu`.
...
Создает всплывающее меню и прикрепляет его к объекту `CMenu`.
...
Меню изначально пусто. Элементы меню можно добавить с помощью функции члена `AppendMenu` или `InsertMenu`. Приложение может добавить всплывающее меню в существующее меню или во всплывающее меню. Функция-член `TrackPopupMenu` может использоваться для отображения этого меню в виде плавающего всплывающего меню и для трассировки выбора во всплывающем меню.
...
Если с пунктом меню связано всплывающее меню, `DeleteMenu` уничтожает маркер во всплывающем меню и освобождает память, используемую во всплывающем меню.
...
Вызывается автоматически обработчиком `CWinApp` времени простоя, удаляет все временные `CMenu` объекты, созданные функцией-членом [FromHandle](#fromhandle) .
...
`DeleteTempMap` отсоединяет объект меню Windows, присоединенный к временному объекту `CMenu`, перед удалением объекта `CMenu`.
...
Меню отсоединяется от объекта `CMenu` перед его уничтожением. Функция Windows `DestroyMenu` автоматически вызывается в деструкторе `CMenu`.
...
Отсоединяет меню Windows от объекта `CMenu` и возвращает маркер.
...
Элемент данных `m_hMenu` имеет значение NULL.
...
Для использования значения MF_BYPOSITION требуется, чтобы приложение использовало правильное `CMenu`. Если используется `CMenu` строки меню, то изменяется пункт меню верхнего уровня (элемент в строке меню). Чтобы задать состояние элемента во всплывающем или вложенном всплывающем меню по положению, приложение должно указать `CMenu` всплывающего меню.

Если в приложении указан флаг MF_BYCOMMAND, Windows проверяет все элементы всплывающего меню, которые являются подчиненными для `CMenu`; Таким образом, если имеются дублирующиеся элементы меню, достаточно использовать `CMenu` строки меню.
...
Возвращает указатель на объект `CMenu`, заданный маркером Windows в меню.
...
Если объект `CMenu` еще не присоединен к объекту меню Windows, создается и прикрепляется временный объект `CMenu`.

Этот временный объект `CMenu` действителен только до тех пор, пока приложение не найдет время простоя в цикле событий, во время которого все временные объекты будут удалены.
...
Идентификатор справки контекста, который в настоящее время связан с `CMenu`, если таковой имеется; в противном случае — ноль.
...
*фбипос*<br/>
Значение, определяющее значение `nIDItem`. По умолчанию `ByPos` имеет значение FALSE, что означает, что Уитем является идентификатором пункта меню. Если параметру `ByPos` не присвоено значение FALSE, то это указывает на позицию пункта меню.
...
- MF_CHECKED выступает в качестве переключателя с MF_UNCHECKED, чтобы установить флажок по умолчанию рядом с элементом. Когда приложение предоставляет точечные рисунки с галочкой (см. `SetMenuItemBitmaps` функция-член), отображается точечный рисунок галочки вкл.
...
- MF_UNCHECKED выступает в качестве переключателя с MF_CHECKED, чтобы снять флажок рядом с элементом. Когда приложение предоставляет точечные рисунки с галочкой (см. `SetMenuItemBitmaps` функция-член), отображается точечный рисунок галочки выкл. Обратите внимание, что значение этой константы равно 0; При использовании этого значения приложение не должно проверять значение 0 для сбоя.
...
*rStr*<br/>
Ссылка на объект `CString`, который получает скопированную строку меню.
...
Возвращает HMENU, упакованный этим объектом `CMenu`, или указатель`CMenu` NULL.
...
Извлекает объект `CMenu` всплывающего меню.
...
Указатель на объект `CMenu`, элемент `m_hMenu` которого содержит маркер для всплывающего меню, если в данной позиции имеется всплывающее меню; в противном случае — NULL. Если объект `CMenu` не существует, создается временный. Возвращаемый указатель `CMenu` не должен храниться.
...
|нфлагс|Интерпретация Лпсзневитем|
|------------|-----------------------------------|
|MF_OWNERDRAW|Содержит предоставляемое приложением 32-разрядное значение, которое приложение может использовать для обслуживания дополнительных данных, связанных с элементом меню. Это 32-разрядное значение доступно приложению в `itemData` члене структуры, предоставляемой [WM_MEASUREITEM](/windows/win32/Controls/wm-measureitem) и [WM_DRAWITEM](/windows/win32/Controls/wm-drawitem) сообщения. Эти сообщения отправляются при первоначальном отображении или изменении пункта меню.|
...
При изменении меню, находящегося в окне (независимо от того, отображается ли окно), приложение должно вызвать `CWnd::DrawMenuBar`.

Если *нидневитем* указывает всплывающее меню, оно становится частью меню, в которое он вставлен. Если это меню уничтожается, вставленное меню также будет уничтожено. Вставленное меню следует отсоединить от `CMenu` объекта, чтобы избежать конфликта.
...
Загружает ресурс меню из исполняемого файла приложения и прикрепляет его к объекту `CMenu`.
...
Загружает ресурс из шаблона меню в памяти и прикрепляет его к объекту `CMenu`.
...
Флаги `mtOption` должны включать MF_END для последнего элемента во всплывающем списке и для последнего элемента в главном списке. Дополнительные флаги см. в описании функции члена `AppendMenu`. Элемент `mtId` должен быть опущен из структуры МЕНУИТЕМТЕМПЛАТЕ, если MF_POPUP указан в `mtOption`.

Пространство, выделенное для структуры МЕНУИТЕМТЕМПЛАТЕ, должно быть достаточно большим, чтобы `mtString` содержать имя элемента меню в виде строки, завершающейся нулем.
...
Указывает HMENUный маркер меню окон, присоединенного к объекту `CMenu`.
...
По умолчанию эта функция члена не выполняет никаких действий. Переопределите эту функцию члена и заполните структуру `MEASUREITEMSTRUCT`, чтобы информировать окна измерений меню.
...
При изменении меню, находящегося в окне (независимо от того, отображается ли окно), приложение должно вызвать `CWnd::DrawMenuBar`. Чтобы изменить атрибуты существующих элементов меню, гораздо быстрее использовать функции-члены `CheckMenuItem` и `EnableMenuItem`.
...
Этот оператор используется для получения маркера объекта `CMenu`.
...
В случае успеха — маркер объекта `CMenu`; в противном случае значение NULL.
...
Он не уничтожает маркер всплывающего меню, поэтому меню можно использовать повторно. Перед вызовом этой функции приложение может вызвать функцию-член `GetSubMenu`, чтобы получить всплывающий объект `CMenu` для повторного использования.
