...
|[Колесервердок:: Жетклиентсите](#getclientsite)|Извлекает указатель на базовый интерфейс `IOleClientSite`.|
...
|[Колесервердок:: Жетдокобжектсервер](#getdocobjectserver)|Переопределите эту функцию, чтобы создать новый объект `CDocObjectServer` и указать, что этот документ является контейнером DocObject.|
...
|[Колесервердок:: OnGetEmbeddedItem](#ongetembeddeditem)|Вызывается для получения `COleServerItem`, представляющей весь документ; используется для получения внедренного элемента. Требуется реализация.|
...
Чтобы использовать `COleServerDoc`, создайте производный от него класс и реализуйте функцию-член [OnGetEmbeddedItem](#ongetembeddeditem) , которая позволяет серверу поддерживать внедренные элементы. Производные классы от `COleServerItem` для реализации элементов в документах и возврата объектов этого класса из `OnGetEmbeddedItem`.
...
Для каждого типа серверного документа, поддерживаемого приложением, необходим один производный класс `COleServerDoc`. Например, если серверное приложение поддерживает листы и диаграммы, необходимы два класса, производных от `COleServerDoc`.
...
Переопределите эту функцию, чтобы создать новый элемент `CDocObjectServer` и вернуть указатель на него.
...
*пдоксите*<br/>
Указатель на интерфейс `IOleDocumentSite`, который будет подключать этот документ к серверу.
...
Стандартная реализация для документа, поддерживающего Докобжектс, просто выделяет новый объект `CDocObjectServer` и возвращает его вызывающему объекту. Пример:
...
Вызовите функцию-член `GetItemClipRect`, чтобы получить координаты обрезкиного прямоугольника элемента, который редактируется на месте.
...
*лпклипрект*<br/>
Указатель на `RECT`ную структуру или объект `CRect`, чтобы получить координаты отсечения прямоугольника элемента.
...
Вызовите функцию члена `GetItemPosition`, чтобы получить координаты элемента, редактируемого на месте.
...
*лппосрект*<br/>
Указатель на `RECT`ную структуру или объект `CRect`, чтобы получить координаты элемента.
...
Ненулевое значение, если объект `COleServerDoc` является документом, представляющим объект, внедренный в контейнер; в противном случае — 0.
...
Вызовите функцию-член `IsInPlaceActive`, чтобы определить, находится ли элемент в настоящий момент в активном состоянии.
...
Ненулевое значение, если объект `COleServerDoc` активен на месте; в противном случае — 0.
...
Когда пользователь выбирает команду Сохранить как в меню файл, `NotifyRename` вызывается реализацией функции-члена [онсаведокумент](../../mfc/reference/cdocument-class.md#onsavedocument) в `COleServerDoc`. Эта функция уведомляет системные библиотеки DLL OLE, которые, в свою очередь, уведомляют контейнеры. В приложениях контейнеров, написанных с помощью библиотека Microsoft Foundation Class, вызывается функция-член [onChange](../../mfc/reference/coleclientitem-class.md#onchange) `COleClientItem`.
...
Когда пользователь выбирает команду Сохранить в меню файл, `NotifySaved` вызывается `COleServerDoc`реализации [онсаведокумент](../../mfc/reference/cdocument-class.md#onsavedocument). Эта функция уведомляет системные библиотеки DLL OLE, которые, в свою очередь, уведомляют контейнеры. В приложениях контейнеров, написанных с помощью библиотека Microsoft Foundation Class, вызывается функция-член [onChange](../../mfc/reference/coleclientitem-class.md#onchange) `COleClientItem`.
...
`COleCmdUI` можно использовать для включения, обновления и задания других свойств команд пользовательского интерфейса DocObject. После инициализации команд их можно выполнять с помощью `OnExecOleCmd`.
...
Реализация по умолчанию отсутствует. Необходимо переопределить эту функцию, чтобы она возвращала элемент, представляющий весь документ. Это возвращаемое значение должно быть объектом класса, производного от `COleServerItem`.
...
*лпректбордер*<br/>
Указатель на `RECT`ную структуру или `CRect` объект, указывающий координаты границы.
...
##  <a name="onsethostnames"></a>Колесервердок:: Онсесостнамес
...
*лппосрект*<br/>
Указатель на `RECT`ную структуру или `CRect` объект, указывающий расположение окна фрейма на месте относительно клиентской области приложения-контейнера.

*лпклипрект*<br/>
Указатель на `RECT`ную структуру или объект `CRect`, который указывает прямоугольник обрезки окна фрейма на месте относительно клиентской области приложения-контейнера.

### <a name="remarks"></a>Remarks

Переопределите эту функцию, чтобы при необходимости обновить коэффициент масштабирования представления.

Эта функция обычно вызывается в ответ на вызов `RequestPositionChange`, хотя контейнер может вызывать изменение позиции для элемента на месте в любое время.
...
*лппосрект*<br/>
Указатель на `RECT`ную структуру или объект `CRect`, содержащий новую позицию элемента.

### <a name="remarks"></a>Remarks

Эта функция обычно вызывается (в сочетании с `UpdateAllItems`) при изменении данных в активном элементе на месте. После этого вызова контейнер может или не может выполнить изменение, вызвав `OnSetItemRects`. Результирующая единица может отличаться от запрошенной.
...
Вызовите функцию члена `ScrollContainerBy` для прокрутки документа контейнера на величину (в пикселях), обозначенную `sizeScroll`.
