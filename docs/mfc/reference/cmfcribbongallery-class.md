...
|[CMFCRibbonGallery::EnableMenuSideBar](#enablemenusidebar)|Включает или отключает боковой панели слева от всплывающего меню.|  
...
|[CMFCRibbonGallery::GetItemToolTip](#getitemtooltip)|Возвращает текст подсказки, который связан с элементом в коллекции.|  
|[CMFCRibbonGallery::GetLastSelectedItem](#getlastselecteditem)|Возвращает индекс последнего элемента в коллекции, выбранное пользователем.|  
...
|[CMFCRibbonGallery::IsMenuResizeEnabled](#ismenuresizeenabled)|Указывает, является ли изменение размера меню включен или отключен.|  
...
### <a name="return-value"></a>Возвращаемое значение  
 Указатель на строку подсказки, присвоенное элементу в галереи ленты. Это может быть `NULL` подсказка назначается этому элементу.  
