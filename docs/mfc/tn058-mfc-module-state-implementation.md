---
title: 'TN058: Реализация состояния модуля MFC | Документация Майкрософт'
ms.custom: ''
ms.date: 06/28/2018
ms.technology:
- cpp-mfc
...
Существует три вида сведения о состоянии MFC: модуль состояния, состояние процесса и потока. Иногда можно объединить эти типы состояния. Например MFC дескриптор карты являются локального модуля и локального потока. Это позволяет двух разных модулях иметь разные карты в каждой из своих потоков.
