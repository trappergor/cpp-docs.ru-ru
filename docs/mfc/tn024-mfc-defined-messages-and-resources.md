 ...
 **Сообщения Windows закрытого MFC**  
 ...
 В редких случаях необходимо, чтобы для обработки одного из этих сообщений необходимо использовать макрос ON_MESSAGE сообщения карты и обработки сообщения в формате универсального LRESULT/WPARAM и LPARAM.  
 ...
 *bDisableIfNoHandler* имеет ненулевое значение, чтобы отключить этот объект пользовательского интерфейса, если обработчик ON_COMMAND ни ON_UPDATE_COMMAND_UI.  
...
|lParam|Не используется (0)|  
 ...
 Возвращаемое значение должно быть ненулевое значение, если FS_SYNCACTIVE — набор и синхронизирует окна его активации с родительского фрейма. `CMiniFrameWnd` возвращает ненулевое значение, если стиль имеет значение MFS_SYNCACTIVE.  
  
 Дополнительные сведения см. в разделе реализации `CMiniFrameWnd`.  
  
## <a name="wmactivatetoplevel"></a>WM_ACTIVATETOPLEVEL  
 Это сообщение отправляется в окно верхнего уровня, при активации или деактивации окна в его «верхнего уровня группы». Окно — часть группы верхнего уровня, если это окно верхнего уровня (нет родительского или владелец), или он принадлежит такое окно. Это сообщение используется аналогично WM_ACTIVATEAPP, но работает в случаях, когда windows, принадлежащих разные процессы не смешанного в одном окне иерархии (обычно в приложениях OLE).  
...
## <a name="rttoolbar-resource-format"></a>Формат ресурса RT_TOOLBAR  
 Панели инструментов по умолчанию, предоставляемые мастером приложений основана на RT_TOOLBAR настраиваемого ресурса, который появился в версии MFC 4.0. Вы можете изменить этот ресурс, с помощью панели инструментов редактора.  
...
## <a name="rtdlginit-resource-format"></a>Формат ресурса RT_DLGINIT  
 ...
 Visual C++ и этот ресурс RT_DLGINIT не требуется использовать связанные компоненты MFC, так как имеются альтернативу API с помощью сведений, ресурс. С помощью Visual C++ позволяет легко для записи, поддерживать и перевести приложение в долгосрочной перспективе.  
  
 Базовая структура ресурсов RT_DLGINIT выглядит следующим образом:  
