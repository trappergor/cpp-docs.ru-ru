---
title: Список примеров панелей инструментов
ms.date: 11/04/2016
helpviewer_keywords:
- sample applications [MFC], toolbars
ms.assetid: 61310e4e-3df7-47c2-8b10-7c6cb45c5d1b
ms.openlocfilehash: 528733668cd00c9c1191dbb6b1109f078d000a99
ms.sourcegitcommit: 5cecccba0a96c1b4ccea1f7a1cfd91f259cc5bde
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 04/01/2019
ms.locfileid: "58770139"
---
# <a name="toolbar-sample-list"></a>Список примеров панелей инструментов

См. следующие примеры программ, которые иллюстрируют использование панелей инструментов MFC:

- [SCRIBBLE](../overview/visual-cpp-samples.md)

- [CTRLBARS](../overview/visual-cpp-samples.md)

- [DOCKTOOL ПОКАЗАНА](../overview/visual-cpp-samples.md)

## <a name="see-also"></a>См. также

[Панели инструментов](../mfc/toolbars.md)
