...
Вы можете либо [Добавление элементов управления в диалоговое окно редактора](../mfc/using-the-dialog-editor-to-add-controls.md) или добавить их вручную, с помощью кода.  
 ...
 В следующем примере показано, как можно объявить `CEdit` объекта в объявлении класса диалоговое окно производного класса, а затем вызвать `Create` функции-члена `OnInitDialog`. Поскольку `CEdit` объект объявлен как внедренный объект, он автоматически создается, когда создается объект диалогового окна, но по-прежнему должны инициализироваться с собственным `Create` функции-члена.  
  
 [!code-cpp[NVC_MFCControlLadenDialog#1](../mfc/codesnippet/cpp/adding-controls-by-hand_1.h)]  
  
 Следующие `OnInitDialog` функция задает прямоугольник, затем вызывает метод `Create` Создание управления редактированием Windows и присоединить ее к неинициализированным `CEdit` объекта.  
