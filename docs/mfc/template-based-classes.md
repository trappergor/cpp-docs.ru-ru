...
###  <a name="_core_typed.2d.pointer_map_usage"></a> Использование карты типизированный указатель  
 ...
 Во втором примере является карты на основе `CMapStringToOb` — он использует ключи строка сопоставляется хранимых указатели на `CMyObject` объектов. Можно использовать те же элементы строго типизированным, описанный в предыдущем абзаце или члены класса можно вызвать `CMapStringToOb`.  
  
> [!NOTE]
>  При указании **класса** или **структуры** тип для *значение* параметра, а не указатель или ссылку на тип класса или структуры, должен иметь конструктор копирования.  
