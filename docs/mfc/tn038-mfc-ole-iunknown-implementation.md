...
3. Для каждого интерфейса, которую нужно поддерживать, используйте макрос BEGIN_INTERFACE_PART и END_INTERFACE_PART в определении класса.

4. В файле реализации BEGIN_INTERFACE_MAP и END_INTERFACE_MAP макросы можно используйте для определения схемы интерфейсов класса.

5. Для каждого поддерживаемого идентификатора интерфейса используйте макрос INTERFACE_PART между макросами BEGIN_INTERFACE_MAP и END_INTERFACE_MAP, чтобы сопоставить IID с конкретной «часть» этого класса.

6. Реализуйте каждый из вложенных классов, представляющих интерфейсы, которые вы поддерживаете.

7. Использовать для доступа к родительскому элементу, method_prologue-макрос `CCmdTarget`-производного объекта.
...
Указанное выше объявление создает класс, производный от `CCmdTarget`. Declare_interface_map-макрос указывает среде, что этот класс будет иметь настраиваемую схему интерфейсов. Кроме того макросы BEGIN_INTERFACE_PART и END_INTERFACE_PART определяют вложенные классы, в данном случае имеют имена CEditObj и cprintobj (буква X используется только для обеспечения отличия вложенных классов от глобальных классов, в которых начинаются с буквы C и интерфейс классы, которые начинается с «I»). Создаются два вложенных элемента этих классов — m_CEditObj и m_CPrintObj соответственно. Макросы автоматически объявляют [AddRef](http://msdn.microsoft.com/library/windows/desktop/ms691379), [выпуска](http://msdn.microsoft.com/library/windows/desktop/ms682317), и [QueryInterface](http://msdn.microsoft.com/library/windows/desktop/ms682521) функции; таким образом можно объявить только функции, характерные для этого интерфейса: EditObject и PrintObject (OLE-макрос STDMETHOD используется, чтобы **_stdcall** и виртуальный ключевые слова, которые предоставляются соответствующим образом для целевой платформы).
...
Используйте этот макрос в любом классе, производном от `CCmdTarget`, который будет иметь схему интерфейсов. Используется в основном так же, как DECLARE_MESSAGE_MAP. Вызов этого макроса следует поместить в определение класса, обычно для этого используется файл заголовка (. (H). Класс с DECLARE_INTERFACE_MAP должен определять схему интерфейсов в файле реализации (. CPP) с макросами BEGIN_INTERFACE_MAP и END_INTERFACE_MAP.
...
*LocalClass* аргумент — имя локального класса, который будет определен. К имени будет автоматически добавляться буква X. Это соглашение об именовании используется для предотвращения конфликтов с глобальными классами, имеющими такое же имя. Кроме того, имя внедренного члена совпадает *localClass* имя, за исключением того, она префикса «m_x».
...
Этот макрос применяется, чтобы сообщить платформе о том, что класс использует агрегатный объект. Он должен указываться между макросами BEGIN_INTERFACE_PART и END_INTERFACE_PART. Агрегатный объект представляет собой отдельный объект, производный от [IUnknown](http://msdn.microsoft.com/library/windows/desktop/ms680509). С помощью статистического выражения и макроса INTERFACE_AGGREGATE, можно сделать все интерфейсы, которые поддерживает статистическое выражение, по-видимому, непосредственно поддерживаться объектом. *TheAggr* аргумент — это просто имя переменной — члена класса, который является производным от [IUnknown](http://msdn.microsoft.com/library/windows/desktop/ms680509) (прямо или косвенно). Все макросы INTERFACE_AGGREGATE должен следовать INTERFACE_PART макросы при помещении в схему интерфейсов.
