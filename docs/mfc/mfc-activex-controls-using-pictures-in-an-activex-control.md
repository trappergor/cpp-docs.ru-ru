---
title: 'Элементы ActiveX в MFC: Использование изображений в элементе управления ActiveX | Документация Майкрософт'
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
6.  В **тип свойства** выберите **IPictureDisp** <strong>\*</strong> для типа свойства.  
...
###  <a name="_core_modifications_to_your_control_project"></a> Модификации проекта элемента управления  
 После внесения необходимых дополнений в проект элемента управления необходимо изменить несколько функций, которые влияют на отрисовку элемента управления ActiveX. Эти функции, `OnResetState`и `OnDraw`, а также функции Get и Set настраиваемого свойства изображения расположены в файле реализации элемента управления. (Обратите внимание, что в этом примере класс элемента управления вызывается `CSampleCtrl`, `CPictureHolder` данные-член вызывается *m_pic*, и имя свойства рисунка является `ControlPicture`.)  
