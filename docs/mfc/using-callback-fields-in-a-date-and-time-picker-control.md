...
> [!NOTE]
>  Поля обратного вызова рассматриваются как действительные поля, чтобы ваше приложение должно быть готово для обработки сообщений уведомления DTN_WMKEYDOWN.  
...
## <a name="handling-the-dtnformatquery-notification"></a>Обработка DTN_FORMATQUERY-уведомление  
 Если элемент управления выполняет синтаксический анализ строки формата, встречается поле обратного вызова, приложение отправляет сообщения уведомления DTN_FORMAT и DTN_FORMATQUERY. Строка поля обратного вызова входит в состав уведомления, можно определить, какое поле обратного вызова выполняется запрос.  
 ...
 После вычисления размера поля текущего обратного вызова необходимо указать значение для поля. Это делается в обработчике DTN_FORMAT-уведомление.  
  
## <a name="handling-the-dtnformat-notification"></a>Обработка DTN_FORMAT-уведомление  
 DTN_FORMAT-уведомление используется приложением для запроса символьной строки, который будет заменен. В следующем примере показано одно из возможных способов:  
