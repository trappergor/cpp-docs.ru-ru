...
|`// use the archive to pass data:`<br /><br /> `arIn >> dwValue;`<br /><br /> - или -<br /><br /> `arOut << dwValue;`6|`// use the archive to pass data:`<br /><br /> `arIn >> dwValue;`<br /><br /> - или -<br /><br /> `arOut << dwValue;`6|  
 ...
 2. Сервер всегда необходимо указать порт, чтобы клиенты могли подключаться. `Create` Вызов иногда также указывает адрес. На стороне клиента используйте параметры по умолчанию, которые попросите MFC, позволяющий использовать любой доступный порт.  
 ...
 5. При вызове `Accept` на стороне сервера можно передать ссылку на объект сокета. Вначале необходимо создать этот объект, но не нужно вызывать `Create` для него. Имейте в виду, что если этот объект сокета идет области видимости, соединение закрывается. MFC подключается новый объект в **СОКЕТА** обработки. Вы можете создать сокет, как показано в стеке или в куче.  
