...
>  Следующее техническое примечание не было обновлено, поскольку сначала оно было включено в электронную документацию. В результате некоторые процедуры и разделы могут быть устаревшими или неверными. Для получения последних сведений рекомендуется выполнить поиск интересующей темы в алфавитном указателе документации в Интернете.  
 ...
 `IOleCommandTarget` Интерфейс включает объекты и их контейнеры для отправки команд друг с другом. Например, в панели инструментов объекта может содержать кнопки для команд `Print`, `Print Preview`, `Save`, `New`, и `Zoom`. Если такого объекта были внедрены в контейнер, который поддерживает `IOleCommandTarget`, объект может включить ее кнопок и пересылки команды обработки, когда пользователь щелкнул их к контейнеру. Если контейнер для печати сам внедренного объекта, его выполнения данного запроса, отправив команду через `IOleCommandTarget` интерфейс внедренного объекта.  
...
## <a name="mfc-ole-command-maps"></a>Схемы команд OLE MFC  
 ...
 При получении приложением MFC, который содержит схему команды OLE команду OLE MFC пытается найти идентификатор команды и группы команд для запрашиваемую команду на карте команды OLE приложения. Если соответствие найдено, сообщения WM_COMMAND отправляется для приложения, содержащего карты команд с Идентификатором запрашиваемую команду. (См. в описании `ON_OLECMD` ниже.) Таким образом команды OLE, отправляется в приложение, преобразуются в сообщения WM_COMMAND с MFC. WM_COMMAND-сообщения затем направляются через схемы сообщений приложения с использованием стандарта MFC [маршрутизация команд](../mfc/command-routing.md) архитектуры.  
  
 В отличие от схемы сообщений MFC OLE команды карты не поддерживается ClassWizard. Разработчики MFC необходимо добавить поддержку карты команды OLE и записи карты команд OLE вручную. Команда OLE, карты, можно добавить MFC активных серверов документ в любом классе, в цепочке WM_COMMAND маршрутизации сообщений во время активного документа является активным на месте в контейнере. Эти классы содержат приложения классы, производные от [CWinApp](../mfc/reference/cwinapp-class.md), [CView](../mfc/reference/cview-class.md), [CDocument](../mfc/reference/cdocument-class.md), и [COleIPFrameWnd](../mfc/reference/coleipframewnd-class.md). В контейнеры активных документов схемы OLE команд можно добавить только к [COleDocObjectItem](../mfc/reference/coledocobjectitem-class.md)-производного класса. Кроме того, в контейнеры активных документов WM_COMMAND отправляются сообщения, только в схеме сообщений в `COleDocObjectItem`-производного класса.  
...
```  
 ...
 Этот макрос отмечает конец карты команд. Используйте этот макрос в файле реализации для класса, содержащего карты команд. Этот макрос всегда должны соответствовать BEGIN_OLECMD_MAP макрос.  
 ...
 *id*  
 Идентификатор сообщения WM_COMMAND отправляется приложения, содержащего карты команд при вызове этой команды OLE.  
  
 ON_OLECMD-макрос в карты команд можно используйте для добавления записи для команды OLE, которые необходимо обработать. При получении команд OLE, они будут преобразованы в указанное сообщение WM_COMMAND и маршрутизируются через схему сообщений приложения, с помощью стандартной архитектуры маршрутизация команд MFC.  
...
1.  В вашей `CView`-производного класса заголовка файла, добавьте макрос DECLARE_OLECMD_MAP к объявлению класса.  
  
    > [!NOTE]
    >  Используйте `CView`-производного класса, так как он является одним из классов в активный документ сервера, который находится в цепочке WM_COMMAND маршрутизации сообщений.  
...
2.  В файле реализации для `CView`-производного класса, добавьте макросы BEGIN_OLECMD_MAP и END_OLECMD_MAP:  
...
3.  Для обработки стандартную команду печати OLE, добавить [ON_OLECMD](reference/message-map-macros-mfc.md#on_olecmd) макрос команды карты, указав идентификатор команды OLE для стандартную команду печати и **ID_FILE_PRINT** для WM_COMMAND идентификатора. **ID_FILE_PRINT** — это стандарт, идентификатор команды print, используемых приложениями MFC, созданный мастером приложений:  
 ...
 Обратите внимание, что один из стандартных макрос команды OLE, определенные в afxdocob.h, может использовать вместо ON_OLECMD-макрос, так как **OLECMDID_PRINT** имеет стандартный идентификатор команды OLE. On_olecmd_print-макрос будет выполнения той же задачи, как показано выше ON_OLECMD-макрос.  
