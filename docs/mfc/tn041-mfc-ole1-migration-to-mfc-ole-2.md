...
В MFC/OLE1 `COleClientItem::GetBounds` и `SetBounds` были использованы для запросов и область элемента управления ( `left` и `top` члены всегда равны нулю). В MFC/OLE 2 это напрямую поддерживается `COleClientItem::GetExtent` и `SetExtent`, что работать с **размер** или `CSize` вместо.
