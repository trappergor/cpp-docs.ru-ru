...
1. Новые поля со списком всех имеющихся элементов меню не соответствуют. Таким образом необходимо создать пункт меню для каждого параметра пера.
   ...
   1. Щелкните правой кнопкой мыши текст, который вы ввели откройте **свойства** окна, а затем изменить идентификатор свойства `ID_PEN_THIN_WIDTH`.

   1. Также необходимо создать обработчик событий для всех элементов меню пера. Щелкните правой кнопкой мыши **э & n пера** пункт меню, который вы только что создали и нажмите кнопку **добавить обработчик событий**. **Мастер обработчика события** отображается.

   1. В **список классов** поле в мастере выберите **CScribbleDoc** и нажмите кнопку **добавлять и редактировать**. При этом создается обработчик событий с именем `CScribbleDoc::OnPenThinWidth`.
...
1. Создайте меню элемента и обработчики событий для толстых пера.
   ...
   1. Щелкните правой кнопкой мыши **толстых пера** пункт меню, который вы только что создали и нажмите кнопку **добавить обработчик событий**. **Мастер обработчика события** отображается.

   1. В **список классов** мастера, выберите **CScribbleDoc** и нажмите кнопку **добавлять и редактировать**. При этом создается обработчик событий с именем `CScribbleDoc::OnPenThickWidth`.
...
1. Каждый документ содержит список штрихов, что пользователь уже рисуется. Каждый штрих определяется `CStroke` объекта. `CStroke` Класса не включает сведения о цвет пера. Таким образом необходимо изменить класс. В scribdoc.h в `CStroke` добавьте следующие строки кода после определения `m_nPenWidth` элемент данных.
...
1. Вы добавили `m_penColor` члена на предыдущем шаге. Теперь создайте обработчик событий для кнопку цвета, который задает элемент.
   ...
   1. В **список классов** поле в мастере выберите **CScribbleDoc** и нажмите кнопку **добавлять и редактировать** кнопки. При этом создается `CScribbleDoc::OnPenColor` заглушки обработчика события.
...
1. Сохраните изменения и затем постройте и запустите приложение. Вы сможете нажмите кнопку цвета, а также изменить цвет пера.
