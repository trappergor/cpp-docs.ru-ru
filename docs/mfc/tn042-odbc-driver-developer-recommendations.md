...
## <a name="odbcs-cursor-library"></a>Библиотека курсоров ODBC  
 ...
 Библиотека классов никогда не используются несколько наборов строк. Таким образом, некоторые `SQLSetPos` операторы всегда применяются к строке 1 набора строк.  
...
## <a name="cdatabases"></a>CDatabases  
 ...
 `SQLGetInfo SQL_DATA_SOURCE_READ_ONLY` должен поддерживаться. Если он возвращает «Y», никакие операции обновления выполняется в источнике данных.  
...
## <a name="crecordsets"></a>CRecordsets  
 `SQLAllocStmt`, `SQLPrepare`, `SQLExecute` (Для `Open` и `Requery`), `SQLExecDirect` (для операций обновления), `SQLFreeStmt` должно поддерживаться. `SQLNumResultCols` и `SQLDescribeCol` будет вызван для результирующего набора в различные моменты времени.  
...
## <a name="forwardonly-cursors"></a>ForwardOnly курсоров  
 Только `SQLFetch` является обязательным для `Move` операций. Обратите внимание, что **forwardOnly** курсоры не поддерживают обновление.  
