...
-   Поддерживает одно или несколько представлений, каждый из которых способен активации на месте. То есть представления должен поддерживать интерфейс `IOleDocumentView` а также интерфейсы `IOleInPlaceObject` и `IOleInPlaceActiveObject` (с помощью контейнера `IOleInPlaceSite` и `IOleInPlaceFrame` интерфейсов).  
