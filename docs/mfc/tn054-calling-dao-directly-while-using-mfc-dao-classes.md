...
## <a name="using-the-dao-ole-interfaces"></a>С помощью интерфейсов OLE DAO  
 ...
 Для многих методов в интерфейсах DAO необходимо манипулировать `BSTR` объекта (длина префикса строка, используемая в OLE-автоматизация). `BSTR` Обычно инкапсулируется в объект **VARIANT** тип данных. Класс MFC `COleVariant` сам наследует от **VARIANT** тип данных. В зависимости от того, является ли построения проекта для ANSI или Юникод, ANSI или Юникод будет возвращать интерфейсы DAO `BSTR`s. Два макроса **V_BSTR** и **V_BSTRT**, полезны для обеспечение интерфейс DAO возвращает `BSTR` ожидаемого типа.  
  
 **V_BSTR** извлечет **bstrVal** членом `COleVariant`. Этот макрос обычно используется, когда нужно передать содержимое `COleVariant` метода интерфейса DAO. В следующем фрагменте кода показаны объявления и фактическое использование для двух методов интерфейса DAO DAOUser, воспользоваться преимуществами **V_BSTR** макрос:  
...
```  
 ...
 Макрос, **V_BSTRT**, будут извлечены ANSI или Юникода **bstrVal** членом `COleVariant` в зависимости от типа сборки (ANSI или Юникод). В следующем коде показано, как извлечь `BSTR` значение из `COleVariant` в `CString`:  
 ...
 **V_BSTRT** макрос вместе с другими методами, чтобы открыть другие типы, которые хранятся в `COleVariant`, показано в образце DAOVIEW. В частности, такое преобразование выполняется в **CCrack::strVARIANT** метод. Этот метод, если это возможно, преобразует значение `COleVariant` в экземпляр `CString`.  
  
## <a name="simple-example-of-a-direct-call-to-dao"></a>Простой пример прямой вызов DAO  
 Могут возникать ситуации, когда необходимо обновить базовой коллекции объектов DAO. Как правило это должен быть необходимо, но это простая процедура, если это необходимо. Пример когда коллекции может потребоваться для обновления — при работе в многопользовательской среде с несколькими пользователями, создание новых tabledefs. В этом случае tabledefs-коллекция может стать неактуальной. Для обновления коллекции, необходимо просто вызвать **обновление** метод определенной коллекции объектов и проверяет наличие ошибок:  
