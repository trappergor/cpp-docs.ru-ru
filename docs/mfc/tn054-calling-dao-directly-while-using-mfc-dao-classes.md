...
Благодаря программированию DAO в MFC упрощается использование DAO за счет обработки множества деталей, поэтому не нужно беспокоиться о некоторых вещах. Сюда входит инициализация OLE, создание и управление объектами DAO (особенно объекты коллекции), проверка ошибок и предоставление строго типизированного, простого интерфейса (без **варианта** или `BSTR` аргументов). Вы можете выполнять прямые вызовы DAO и по-прежнему использовать преимущества этих функций. Весь код должен вызывать `Release` для любых объектов, созданных прямыми вызовами DAO, и *не* изменять какие-либо указатели интерфейса, которые могут использоваться MFC для внутренних целей. Например, не изменяйте элемент *m_pDAORecordset* открытого объекта `CDaoRecordset`, если не понимаете *все* внутренние последствия. Однако можно использовать интерфейс *m_pDAORecordset* для непосредственного вызова DAO для получения коллекции Fields. В этом случае элемент *m_pDAORecordset* не будет изменен. По завершении работы с объектом необходимо просто вызвать метод `Release` для объекта коллекции Fields.
...
Для многих методов в интерфейсах DAO необходимо управлять объектом `BSTR` (строка с префиксом длины, используемой в OLE-автоматизации). Объект `BSTR` обычно инкапсулируется в тип данных **Variant** . Класс MFC, `COleVariant` сам себя, наследуется от типа данных **Variant** . В зависимости от того, строится ли проект для ANSI или Unicode, интерфейсы DAO будут возвращать ANSI или Unicode `BSTR`s. Два макроса, V_BSTR и V_BSTRT, полезны для гарантии того, что интерфейс DAO получает `BSTR` ожидаемого типа.

V_BSTR извлечет элемент *бстрвал* `COleVariant`. Этот макрос обычно используется, когда необходимо передать содержимое `COleVariant` в метод интерфейса DAO. В следующем фрагменте кода показаны объявления и фактическое использование двух методов интерфейса DAO Дааусер, которые используют преимущества макроса V_BSTR:
...
Обратите внимание, что аргумент `VT_BSTRT`, указанный в приведенном выше конструкторе `COleVariant`, обеспечивает наличие `BSTR` ANSI в `COleVariant` при создании версии приложения ANSI и `BSTR` Юникод для версии приложения в Юникоде. Именно именно это делает DAO.

Другой макрос, V_BSTRT, извлекает элемент *БСТРВАЛ* ANSI или unicode в `COleVariant` в зависимости от типа сборки (ANSI или Unicode). В следующем коде показано, как извлечь значение `BSTR` из `COleVariant` в `CString`:
...
Макрос V_BSTRT вместе с другими методами открытия других типов, хранящихся в `COleVariant`, демонстрируется в примере ДАОВИЕВ. В частности, этот перевод выполняется в методе `CCrack::strVARIANT`. По возможности этот метод преобразует значение `COleVariant` в экземпляр `CString`.

## <a name="simple-example-of-a-direct-call-to-dao"></a>Простой пример прямого вызова DAO

Ситуации могут возникнуть, когда необходимо обновить базовые объекты коллекции DAO. Как правило, это не требуется, но при необходимости это простая процедура. Например, если коллекцию может потребоваться обновить, то при работе в многопользовательской среде с несколькими пользователями создаются новые tabledef. В этом случае коллекция tabledef может устареть. Чтобы обновить коллекцию, необходимо просто вызвать метод `Refresh` определенного объекта коллекции и проверить на наличие ошибок:
