...
Здесь переменная-член *m_Checkbox* выполняет ту же цель, что и функция-член @no__t – 1, показанная в строго типизированном [доступе к элементам управления без мастеров кода](../mfc/type-safe-access-to-controls-without-code-wizards.md). Если флажок не установлен, то при нажатии кнопки по-прежнему потребуется обработчик в классе диалогового окна для сообщения BN_CLICKED элемента управления.
