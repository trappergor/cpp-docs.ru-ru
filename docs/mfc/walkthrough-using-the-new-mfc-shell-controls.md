...
- В Visual Studio 2017 и более поздних версиях поддержка MFC является дополнительным компонентом. Чтобы установить его, откройте установщик Visual Studio из меню Пуск в Windows. Найти версию Visual Studio, вы используете и выберите **изменить** кнопки. Убедитесь, что **разработка классических приложений на C++**  проверяется плитки. В разделе **дополнительные компоненты**, проверьте **поддержка MFC** кнопки.
...
Эти действия зависят от используемой версии Visual Studio. Убедитесь, что выбора версий в верхнем левом углу этой страницы правильно настроены.

::: moniker range="vs-2019"

### <a name="to-create-an-mfc-project-in-visual-studio-2019"></a>Для создания проекта MFC в Visual Studio 2019 г.

1. В главном меню выберите **файл** > **New** > **проекта** открыть **создайте новый проект** диалоговое окно поле.

1. Введите в поле поиска в верхней **MFC** и выберите **приложение MFC** из списка результатов. 

1. Нажмите кнопку **Далее**. На следующей странице введите имя для проекта и указать расположение проекта, при необходимости.

1. Выберите **создать** кнопку, чтобы создать проект.

   После **мастер приложений MFC** отображает, используйте следующие параметры:
 
   1. Выберите **тип приложения** слева. Затем выберите **одного документа** и выберите **поддержка архитектуры Document/View**. В разделе **проекта стиля**выберите **Visual Studio**и из **визуальный стиль и цвета** раскрывающегося списка выберите **Office 2007 (синяя тема)**.

   1. На **поддержка составных документов** области выберите **None**.

   1. Не вносите никаких изменений в **свойства шаблона документа** области.
   ...
   1. На **дополнительные функции** области выберите **элементы управления ActiveX**, **распространенных манифеста управления**, и **редактируемую** параметр. Оставьте все остальное, так как это. **Редактируемую** параметр приведет к мастеру создать область слева от окна с `CMFCShellTreeCtrl` внедренных.
...
### <a name="to-create-an-mfc-project-in-visual-studio-2017-or-earlier"></a>Для создания проекта MFC в Visual Studio 2017 или более ранней версии
...
1. В **новый проект** диалогового окна разверните узел **Visual C++** узел в **типы проектов** области и выберите **MFC**. Затем в **шаблоны** области выберите **приложения MFC**. Введите имя проекта, например `MFCShellControls` и нажмите кнопку **ОК**. 
...
Убедитесь, что приложение успешно создан путем создания и выполнения его. Для построения приложения, из **построения** меню выберите **построить решение**. Если сборка приложения прошла успешно, запустите приложение, выбрав **начать отладку** из **Отладка** меню.
