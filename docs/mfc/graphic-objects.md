...
> [!NOTE]
> Класс [CImage](../atl-mfc-shared/reference/cimage-class.md) обеспечивает расширенную поддержку битмейпа.
...
У каждого класса графического объекта в библиотеке есть оператор приведения, который преобразует объект MFC в связанный дескриптор Windows. Полученный дескриптор допустим, пока связанный объект не отсоединит его. Используйте функцию `Detach` члена объекта, чтобы отсоединить ручку.
...
1. Определите графический объект в кадре стека. Инициализируйте объект с помощью функции создания определенного типа, такой как `CreatePen`. Или же инициализируйте объект в конструкторе. Смотрите обсуждение [одноступенчатого и двухэтапного создания,](../mfc/one-stage-and-two-stage-construction-of-objects.md)в котором приводится пример кода.

1. [Выберите объект в контексте текущего устройства,](../mfc/selecting-a-graphic-object-into-a-device-context.md)сохраняя старый графический объект, выбранный ранее.
