...
- **Закрытые виртуальные базовые классы и косвенное наследование**
   ...
   Предыдущие версии компилятора позволяли производному классу вызывать функции-члены его *косвенно производного* `private virtual` базовых классов. Это поведение было неправильным и не соответствовало стандарту языка C++. Компилятор больше не принимает код, написанный таким образом, и выдает в результате ошибку C2280.
...
- **Поддержка упакованных типов в структурах значений.**
   ...
   Теперь можно определить типы значений с помощью полей, которые могут иметь значение null, например `IBox<int>^`, а не **int**. Это означает, что поля могут иметь значение или быть равными **nullptr**.
...
- В заголовок `find_if_not`algorithm> были добавлены пятнадцать новых функций, например `copy_if`, `is_sorted` и \<.
