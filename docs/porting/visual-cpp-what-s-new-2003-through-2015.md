...
- `volatile` ***Переменные-члены не допускают неявно определенных конструкторов и операторов присваивания** Предыдущие версии компилятора допускали автоматическое создание конструкторов копирования и перемещения по умолчанию, а также операторов присваивания копирования и перемещения по умолчанию для класса, содержащего переменные-члены **volatile**.TЭто поведение было неправильным и не соответствовало стандарту языка C++.TТеперь компилятор рассматривает класс с переменными-членами volatile как имеющий нетривиальные конструкторы и операторы присваивания, что делает невозможным автоматическую реализацию этих операторов по умолчанию.WЕсли такой класс является членом объединения (или анонимного объединения внутри класса), конструкторы копирования и перемещения и операторы присваивания копирования и перемещения объединения (или класса, содержащего анонимное объединение) будут неявно определены как удаленные.AПопытка создать или скопировать объединение (или класс, содержащий анонимное объединение), не объявляя их явно, будет являться ошибкой. В результате будет выдана ошибка компилятора C2280.
...
Теперь MFC поддерживает анимацию и графику Direct2D. Библиотека MFC содержит несколько новых функций и классов MFC для поддержки этой функциональности. Существует также два новых пошаговых руководства, описывающих добавление объекта D2D и объекта анимации в проект: В этих пошаговых руководствах, **Пошаговое руководство: Добавление объекта D2D в проект MFC** и  **Добавление анимации в проект MFC**.
...
**Мастер элементов управления ATL.** Мастер элементов управления ATL больше не заполняет поле `ProgID` автоматически. Если элемент управления ATL не имеет программного идентификатора `ProgID`, другие средства могут с ним не работать. Одним из примеров инструмента, требующего элементы управления `ProgID`, является диалоговое окно **Вставка элемента управления ActiveX**. Дополнительные сведения об этом диалоговом окне см. в разделе **Диалоговое окно "Вставка элемента управления ActiveX"**.
...
- `/analyze` (корпоративный анализ кода) добавлен в качестве параметра компилятора.
- `/bigobj` добавлен в качестве параметра компилятора.
- `/clr:pure``/clr:safe` и `/clr:oldSyntax` добавлены. (Впоследствии отмечены как нерекомендуемые в Visual Studio 2015 и удалены в Visual Studio 2017.)
...
- `/EH` (модель обработки исключений) или `/EHs` больше невозможно использовать для перехвата исключения, вызванного не с помощью throw; используйте параметр `/EHa`.
- `/errorReport` (отчет о внутренних ошибках компилятора) добавлен в качестве параметра компилятора.
- `/favor` (оптимизация для 64) добавлен в качестве параметра компилятора.
- `/FA`, `/Fa` (файл листинга) добавлен в качестве параметра компилятора.
- `/FC` (полный путь к файлу исходного кода в папке Diagnostics) добавлен в качестве параметра компилятора.
- `/fp` (определение поведения с плавающей запятой) добавлен в качестве параметра компилятора.
- `/G` (оптимизация под процессор) добавлен в качестве параметра компилятора.
- `/G` (оптимизация под процессор) добавлен в качестве параметра компилятора.
- `/G3``/G4`, `/G5`, `/G6`, `/G7` и `/GB`. Теперь компилятор использует "смешанную модель", которая пытается создать оптимальный выходной файл для всех архитектур.
...
- `/GR` включен теперь по умолчанию.
- `/GS` (проверка безопасности буфера) теперь обеспечивает защиту безопасности для уязвимых параметров указателей. `/GS` включен теперь по умолчанию. `/GS` теперь также работает с функциями, скомпилированными в MSIL с параметром `/clr` (компиляция среды CLR).
- `/homeparams` (копирование параметров регистров в стек) добавлен в качестве параметра компилятора.
- `/hotpatch` (создание образа, допускающего оперативное обновление) добавлен в качестве параметра компилятора.
...
- `/ML` и `/MLd` удалены. Visual C++ больше не поддерживает однопоточную библиотеку CRT со статическим связыванием.
...
- `/Oa` удален, но будет игнорироваться без уведомлений; используйте модификаторы `noalias` или `restrict__declspec`, чтобы указать, как компилятор присваивает псевдоним.
- `/Op` удален. Используйте вместо него `/fp` (определение поведения с плавающей запятой).
- Visual C++ теперь поддерживает OpenMP.
- `/openmp` (включение поддержки OpenMP 2.0) добавлен в качестве параметра компилятора.
- `/Ow` удален, но будет игнорироваться без уведомлений. Используйте модификаторы `noalias` или `restrict__declspec`, чтобы указать, как компилятор присваивает псевдоним.

### <a name="profile-guided-optimizations"></a>Профильная оптимизация

- `/QI0f` удален.
- `/QIfdiv` удален.
- `/QIPF_B` (список ошибок для пошагового выполнения B ЦП) добавлен в качестве параметра компилятора.
- `/QIPF_C` (список ошибок для пошагового выполнения C ЦП) добавлен в качестве параметра компилятора.
- `/QIPF_fr32` (не используйте верхние регистры 96-разрядного формата с плавающей запятой) добавлен в качестве параметра компилятора.
- `/QIPF_noPIC` (создание кода, зависящего от позиции) добавлен в качестве параметра компилятора.
- `/QIPF_restrict_plabels` (предполагается, что функции во время выполнения не создаются) добавлен в качестве параметра компилятора.

### <a name="unicode-support-in-the-compiler-and-linker"></a>Поддержка Юникода в компиляторе и компоновщике

- `/vd` (отключение смещений при выполнении конструктора) теперь позволяет использовать оператор dynamic_cast для создаваемого объекта (/vd2)
- `/YX` удален. Используйте вместо него `/Yc` (создание файла предкомпилированного заголовка) или `/Yu` (использование файла предкомпилированного заголовка). Если удалить `/YX` из конфигураций сборки, ничем его не заменив, это может привести к ускорению сборки.
- `/Zc:forScope` включен теперь по умолчанию.
- `/Zc:wchar_t` включен теперь по умолчанию.
- `/Zd` удален. Отладочная информация, ограниченная номером строки, больше не поддерживается. Используйте вместо него `/Zi` (дополнительные сведения см. в разделе **/Z7, /Zi, /ZI (формат отладочной информации)**).
- `/Zg` теперь можно использовать только для файлов исходного кода C, но не с файлами исходного кода C++.
- `/Zx` (код Itanium, оптимизированный для отладки) добавлен в качестве параметра компилятора.

### <a name="new-language-features"></a>Новые функции языка

- Attributeattribute считается устаревшим.
- `appdomain__declspec` добавлен в качестве модификатора.
- `__clrcall` добавлен в качестве соглашения о вызовах.
...
- `jitintrinsicdeclspec` добавлен в качестве модификатора.
- `noaliasdeclspec` добавлен в качестве модификатора.
- `process__declspec` добавлен в качестве модификатора.
- **abstract**, **override** и **sealed** допустимы для компиляций в машинный код.
- Добавлено ключевое слово **__restrict**.
- `restrictdeclspec` добавлен в качестве модификатора.
...
- `embedded_idl` (директива `#import`) теперь принимает необязательный параметр.
- `fenv_access` pragma
...
- `make_public` pragma
- `managed``unmanaged` обновлен (теперь имеется `push` и `pop`)
...
- `region``endregion` добавлены в качестве директив pragma
- Добавлен предустановленный макрос _VC_NODEFAULTLIB.
- Реализованы макросы с переменным числом аргументов.
- `vtordisp` устарела и будет удалена в одном из следующих выпусков Visual C++.
...
- `/ALLOWISOLATION` (поиск манифеста) добавлен в качестве параметра компилятора.
...
- `/CLRIMAGETYPE` (определение типа образа среды CLR) добавлен в качестве параметра компоновщика.
- `/CLRSUPPORTLASTERROR` (сохранение кода последней ошибки для вызовов PInvoke) добавлен в качестве параметра компоновщика.
- `/CLRTHREADATTRIBUTE` (указание атрибута потока среды CLR) добавлен в качестве параметра компоновщика.
- `/CLRUNMANAGEDCODECHECK` (добавление атрибута SuppressUnmanagedCodeSecurityAttribute) добавлен в качестве параметра компоновщика.
- `/ERRORREPORT` (создание отчетов о внутренних ошибках компоновщика) добавлен в качестве параметра компоновщика.
- `/EXETYPE` удален. Компоновщик больше не поддерживает создание драйверов устройств Windows 95 и Windows 98. Используйте для этого подходящий соответствующий пакет DDK. Ключевое слово EXETYPE больше не является допустимым для файлов определений модулей.
- `/FUNCTIONPADMIN` (создание образа, допускающего оперативное обновление) добавлен в качестве параметра компоновщика.
- `/LTCG` теперь поддерживается в модулях, скомпилированных с использованием `/clr`. `/LTCG` также был обновлен для поддержки профильных оптимизаций.
- `/MANIFEST` (создание манифеста параллельной сборки) добавлен в качестве параметра компоновщика.
- `/MANIFESTDEPENDENCY` (определение зависимостей манифеста) добавлен в качестве параметра компоновщика.
- `/MANIFESTFILE` (именование файла манифеста) добавлен в качестве параметра компоновщика.
- `/MAPINFO:LINES` удален.
- `/NXCOMPAT` (совместимость с предотвращением выполнения данных) добавлен в качестве параметра компоновщика.
- `/PGD` (указание базы данных для профильной оптимизации) добавлен в качестве параметра компоновщика.
- `/PROFILE` (профилировщик средств оценки производительности) добавлен в качестве параметра компоновщика.
- `/SECTION` (указание атрибутов секции) теперь поддерживает отрицание атрибутов и больше не поддерживает атрибуты L или D (связанные с VxD).
- Поддержка Юникода в компиляторе и компоновщике
- `/VERBOSE` (печать сообщений о ходе выполнения) теперь принимает ICF и REF.
- `/VXD` удален. Компоновщик больше не поддерживает создание драйверов устройств Windows 95 и Windows 98. Используйте для этого подходящий соответствующий пакет DDK. Ключевое слово VXD больше не является допустимым для файлов определений модулей.
- `/WS` удален. `/WS` использовался для изменения образов, ориентированных на Windows NT 4.0. Вместо `/WS` можно использовать команду IMAGECFG.exe -R имя файла. IMAGECFG.exe находится на компакт-диске Windows NT 4.0 в каталоге SUPPORT\DEBUG\I386\IMAGECFG.EXE.
- `/WX` (обработка предупреждений компоновщика как ошибки) теперь задокументирован.

### <a name="new-linker-utility-features"></a>Новые функции служебной программы компоновщика

- `/ALLOWISOLATION` добавлен в качестве параметра editbin
...
- `/ERRORREPORT` добавлен в качестве параметра в bscmake.exe, dumpbin.exe, editbin.exe и lib.exe.
- `/LTCG` добавлен в качестве параметра lib.
- `/NXCOMPAT` добавлен в качестве параметра editbin.
- `/RANGE` добавлен в качестве параметра dumpbin.
- `/TLS` добавлен в качестве параметра dumpbin.
- `/WS` удален для editbin. `/WS` использовался для изменения образов, ориентированных на Windows NT 4.0. Вместо `/WS` можно использовать команду IMAGECFG.exe -R имя файла. IMAGECFG.exe находится на компакт-диске Windows NT 4.0 в каталоге SUPPORT\DEBUG\I386\IMAGECFG.EXE.
- Добавлен параметр /WX[:NO] программы lib.

### <a name="new-nmake-features"></a>Новые функции NMAKE

- `/ERRORREPORT` добавлен.
- `/G` добавлен.
...
- `/ERRORREPORT` добавлен в качестве параметра ml.exe и ml64.exe.
...
- Был добавлен typedef `errno_t` с типом integer. `errno_t` используется, когда тип возвращаемого функцией значения или параметр связан с кодами ошибок из `errno`. `errno_t` заменяет `errcode`.
...
- Добавлено пошаговое руководство, в котором объясняется процесс переноса существующего собственного приложения для использования управляемых расширений для C++: Пошаговое руководство. Перенос существующего собственного приложения C++ для взаимодействия с компонентами .NET Framework.
...
- `/arch` добавлен в качестве параметра компилятора.
- `/Gf` устарел и будет удален в следующей версии Visual C++.
- `/G7` добавлен в качестве параметра компилятора.
...
- `implements` теперь задокументирован.
