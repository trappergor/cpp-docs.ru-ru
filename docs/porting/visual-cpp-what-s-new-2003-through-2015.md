...
### <a name="conformance-improvements-in-visual-studio-2015"></a><a name="VS_RTM"></a>Улучшение соответствия в визуальной студии 2015
...
- **Размещаемые операторы new и delete**
   ...
   В оператор **delete** были внесены изменения, чтобы привести его в соответствие со стандартом C++14. Сведения об изменении стандартов можно найти на странице [Освобождение с размером в C++](https://isocpp.org/files/papers/n3778.html). Эти изменения добавляют форму глобального оператора **delete**, принимающего параметр размера. Нарушение изменения заключается в том, что если вы ранее использовали оператора **удалить** с той же подписью (чтобы соответствовать **размещению нового** оператора), вы получите ошибку компилятора (C2956, которая происходит в точке, где размещение **новое** используется, так как это позиция в коде, где компилятор пытается определить соответствующий соответствующий оператор **удаления).**

   Функция `void operator delete(void *, size_t)` была **размещаемым оператором delete**, соответствующим **размещаемой функции new**`void * operator new(size_t, size_t)` в C++11. С помощью сделки размером с C-14 эта функция **удаления** теперь является *обычной функцией определения местоположения (глобальный* оператор **удаления).** Согласно стандарту, если использование **размещаемого оператора new** ищет соответствующую функцию **delete** и находит функции обычного освобождения, программа сформирована некорректно.
   ...
   Затем измените определение **нового** размещения и **удалите,** чтобы `size_t`использовать этот тип в качестве второго аргумента вместо . Вам также нужно будет обновить вызовы для **размещения новых** пройти новый `static_cast<my_type>` тип (например, с помощью для преобразования из значения всей величины) и обновить определение **новых** и **удалить,** чтобы отбросить к типу integer. Для этого не нужно использовать **enum;** тип класса с `size_t` членом также будет работать.
   ...
   Если вы не хотите немедленно обновлять код, можно вернуться к старому поведению с помощью параметра компилятора `/Zc:sizedDealloc-`. Если вы используете эту опцию, функции **удаления** из двух аргументов не существуют и не вызывают конфликта с оператором **удаления размещения.**
...
### <a name="conformance-improvements-in-visual-studio-2015-update-1"></a><a name="VS_Update1"></a>Улучшение соответствия в визуальной студии 2015 Обновление 1
...
### <a name="conformance-improvements-in-visual-studio-2015-update-2"></a><a name="VS_Update2"></a>Улучшение соответствия в визуальной студии 2015 Обновление 2
...
### <a name="conformance-improvements-in-visual-studio-2015-update-3"></a><a name="VS_Update3"></a>Улучшение соответствия в визуальной студии 2015 Обновление 3
...
- make_unique\<T> (args...) и make_unique<Тз>(n)
...
- **Поддержка упакованных типов в структурах значений.**
   ...
   Теперь можно определить типы значений, используя поля, которые могут быть нулевыми, например, `IBox<int>^` в отличие от **int.** Это означает, что поля могут иметь значение или быть **равныnullptr**.
...
- Завершает многолинейные комментарии\* \*(/ /)
...
**Фильтрация списка членов на основе контекста.** Недоступные члены отфильтровываются из списка членов IntelliSense. Например, закрытые члены не отображаются в списке членов, кроме случаев, когда вы изменяете код, реализующий тип. В то время как список участников открыт, можно нажать **Ctrl**+**J,** чтобы удалить один уровень фильтрации (применяется только к текущему окну списка участников). Вы можете нажать **Ctrl**+**J** снова, чтобы удалить текстовую фильтрацию и показать каждому члену.
...
**Отладки.** Процесс отладки для приложений, использующих C++ AMP для ориентации GPU, аналогичен отладке для других приложений C++. Сюда входят новые дополнения параллельной отладки, упомянутые ранее.
...
**Автоматическое развертывание для удаленной отладки.** Развертывание файлов для удаленной отладки в Visual C++ стало еще проще. Опция **«Развертывание»** в меню контекста проекта автоматически копирует на удаленный компьютер файлы, указанные в свойствах конфигурации отладки. Копировать файлы на удаленный компьютер вручную больше не требуется.
...
**Улучшения списка членов.** Список **участников списка** отображается автоматически при вводе кода в редактор кода. Результаты отфильтруются так, чтобы отображались только соответствующие члены. Вы можете управлять логикой фильтрации, используемой в списке членов, в диалоговом окне **Параметры** в разделе **Текстовый редактор** > **C/C++** > **Дополнительно**.

**Семантическая колоритная.** Типы, перечисления, макросы и другие токены C++ теперь по умолчанию имеют раскраску.

**Выделение ссылок.** Теперь при выборе символа выделяются все его экземпляры в текущем файле. Нажмите **Ctrl**+**Shift**+**Up Arrow** или **Ctrl**+**Shift**+**Down Arrow,** чтобы перемещаться среди выделенных ссылок. Вы можете отключить эту функцию в диалоговом окне **Параметры** в разделе **Текстовый редактор** > **C/C++** > **Дополнительно**.
...
- В окне **анализа кода** можно вставить в исходный код прагму, которая подавляет выбранное предупреждение.
...
Теперь, чтобы лучше понять свой код, вы можете создать графы зависимостей для двоичного файла, класса, пространства имен, а также включить файлы в решение. В панели меню выберите**график создания зависимости** **Architecture,** > а затем **для решения** или для **включения файла** для создания графика зависимостей. После создания графа его можно изучить, развернув каждый узел, определить отношения зависимости, перемещаясь между узлами, а также просмотреть исходный код, щелкнув пункт **Просмотр содержимого** в контекстном меню для узла. Для создания графика зависимости для включения файлов, в \*меню ярлыка для \*файла исходного кода .cpp или файла заголовка .cpp, выберите **«Создать график включений файлов».**

#### <a name="architecture-explorer"></a>Обозреватель архитектуры

С помощью **Architecture Explorer**можно изучить ресурсы в своем решении, проектах или файлах. В панели меню выберите **Architecture** > **Architecture** > **Explorer.** Вы можете выбрать нужный узел, например **Представление классов**. В этом случае правая часть окна инструментов развертывается, отображая список пространств имен. Если выбрать пространство имен, появляется новый столбец со списком классов, структур и перечислений в этом пространстве имен. Вы можете продолжить просмотр ресурсов или вернуться в крайний левый столбец для запуска другого запроса. Смотрите **Найти код с архитектурой Explorer**.
...
**авто ключевое слово.** Ключевое слово **auto** имеет новое назначение. Используйте значение по умолчанию ключевого слова **auto** для объявления переменной, тип которой выводится из выражения инициализации в объявлении переменной. Параметр компилятора `/Zc:auto` вызывает либо новое, либо предыдущее значение ключевого слова **auto**.

**Описатель типа decltype.** Разоспектор типа **деклитипа** возвращает тип указанного выражения. Используйте разосpecть типа **деклитипа** в сочетании с **автоматическим** ключевым словом, чтобы объявить тип, который является либо сложным, либо известным только компилятору. Например, используйте это сочетание для объявления функции шаблонов, тип возвращаемого значения которой зависит от типов аргументов его шаблонов. Либо объявите функцию шаблонов, которая вызывает другую функцию, а затем возвращает тип возвращаемого значения вызванной функции.

**Выражения Ламбда.** Лямбда-функции имеют тело функции, но не имеют имени. Лямбда-функции сочетают лучшие характеристики указателей функций и объектов функций. Используйте функцию лямбды сама по себе, в качестве параметра функции шаблона вместо объекта функции, или вместе с **автоматическим** ключевым словом, чтобы объявить переменную, тип которой является lambda.
...
**Объявление static_assert.** **Декларация static_assert** тестирует утверждение программного обеспечения во время компиляции, в отличие от других механизмов утверждения, которые тестируются во время выполнения. Если утверждение не выполняется, не выполняется и компиляция. При этом выводится сообщение о возникшей ошибке.

**Ключевые слова nullptr и __nullptr.** MSVC позволяет использовать ключевое слово **nullpt** с машинным или управляемым кодом. Ключевое слово **nullptr** указывает на то, что ручка объекта, указатель интерьера или тип родного указателя не указывают на объект. Компилятор обрабатывает **nullptr** как управляемый код при использовании параметра компилятора `/clr` и как машинный код, когда параметр `/clr` не используется.
**Ключевое** слово Microsoft __nullptr имеет такое же значение, как **nullptr,** но оно относится только к родному коду. При компиляции машинного кода C/C++ с помощью параметра компилятора `/clr` компилятор не может определить, является ли ключевое слово **nullptr** машинным или управляемым. Чтобы устранить эту неоднозначность, используйте ключевое слово nullptr для обозначения управляемого значения, а **__nullptr** — машинного.
...
**Каталоги VC++.** Параметр каталогов VC++ теперь доступен в двух местах. Вы можете использовать страницы свойств проекта, чтобы задать значения каталогов VC++ для отдельных проектов. Используйте **«Менеджер свойств»** и лист свойств для установки глобальных значений для конфигурации для каталогов VC.
...
**Перейдите к.** Поле **«Перейти к** диалогу» позволяет искать все символы и файлы в проекте, которые соответствуют указанной строке. Результаты уточняются сразу после ввода дополнительных символов в строке поиска. Поле обратной связи **«Результаты»** определяет количество найденных элементов и помогает вам решить, следует ли ограничивать поиск. Поля **Вид/область**, **Расположение** и **Предварительный просмотр** помогают различать элементы с одинаковыми именами. Кроме того, вы можете расширить эту функцию для поддержки других языков программирования.
...
**Дизайнер лент.** **Дизайнер ленты** графический редактор, который позволяет создавать и изменять MFC ленты UI. Окончательный пользовательский интерфейс ленты представлен файлом ресурсов на основе XML (MFCRIBBON-MS). Для существующих приложений можно записать текущий пользовательский интерфейс ленты, временно добавив несколько строк кода и затем вызвав **конструктор лент**. После создания файла ресурсов ленты вы можете заменить рукописный код пользовательского интерфейса ленты на несколько операторов, загружающих ресурс ленты.

**Позвоните иерархии.** Окно **Иерархия вызовов** позволяет перейти ко всем функциям, вызываемым определенной функцией и вызывающих ее.

### <a name="tools"></a>Инструменты

**Мастер класса МФЦ.** В Visual C++ 2010 снова появился популярный мастер классов MFC. Мастер классов MFC позволяет удобно добавлять в проект классы, сообщения и переменные, не редактируя наборы исходных файлов вручную.

**Мастер управления ATL.** Мастер элементов управления ATL больше не заполняет поле `ProgID` автоматически. Если элемент управления ATL не имеет программного идентификатора `ProgID`, другие средства могут с ним не работать. Одним из примеров инструмента, требующего элементы управления `ProgID`, является диалоговое окно **Вставка элемента управления ActiveX**. Для получения дополнительной информации о диалоговом поле **см.**
...
- Диалоговые окна, созданные в приложениях Win32, ATL и MFC, теперь соответствуют рекомендациям по стилю для Windows Vista. При создании проекта с помощью Visual Studio 2008 все диалоговые окна, вставляемые в приложение, будут соответствовать рекомендациям по стилю Windows Vista. При перекомпиляции проекта, созданного с помощью более ранней версии Visual Studio, все существующие диалоговые окна сохранят предыдущий внешний вид. Для получения дополнительной информации о том, как **Dialog Editor**вставить диалоговые коробки в приложение, см.

- Мастер **проекта ATL** теперь имеет возможность зарегистрировать компоненты для всех пользователей. Начиная с Visual Studio 2008 COM-компоненты и библиотеки типов, создаваемые мастером **проектов ATL**, регистрируются в узле HKEY_CURRENT_USER реестра, если только не выбрана **регистрация компонентов для всех пользователей**.
- Мастер **проекта ATL** больше не предоставляет возможность создавать приписываемые проекты ATL. Начиная с Visual Studio 2008, мастер **проекта ATL** не имеет возможности изменить приписываемый статус нового проекта. Все создаваемые мастером проекты ATL стали неопределенными.
...
- В настоящее время **конструктор класса** имеет ограниченную поддержку родного кода СЗ. В более ранних версиях Visual Studio **конструктор классов** работал только с Visual C# и Visual Basic. Теперь пользователи C++ могут использовать **конструктор классов**, но только в режиме для чтения. Дополнительные сведения об использовании **конструктора классов** в C++ см. в разделе **Работа с кодом Visual C++ в конструкторе классов**.
...
- Параметр компилятора `/Wp64` и ключевое слово **__w64** устарели. Параметр компилятора `/Wp64` и ключевое слово **__w64**, отвечающие за обнаружение конфликтов переносимости на 64-разрядные платформы, не рекомендуются к использованию и будут удалены в будущей версии компилятора. Вместо них рекомендуется использовать MSVC, ориентированный на 64-разрядную платформу.
...
- Утомленные варианты компилятора: многие варианты компилятора были уэрокатированы в этом выпуске; для получения дополнительной **информации** см.
...
- Были обновлены вневиальная эвристика функций; для получения дополнительной информаци **inline_depth** **__forceinline** **__inlineи** **__inline**
...
- **dynamic_cast** Оператор имеет нарушение изменений.
...
- **абстрактные,** **переопределить,** и **запечатанные** действительны для родной компиляции.
- **__restrict** добавлено ключевое слово.
- Добавлен параметр `restrictdeclspec`.
- **__thiscall** теперь ключевое слово.
- **__unaligned** ключевое слово теперь документировано.
...
- Были добавлены безопасные версии нескольких функций. Эти функции лучше обрабатывают ошибки и более строго контролируют буферы, что помогает устранить наиболее распространенные бреши безопасности. Новые безопасные версии идентифицируются **_s** суффиксом.
...
- Функции, зависящие от языкового стандарта, теперь имеют версии, которые принимают языковой стандарт в качестве параметра, а не используют текущий языковой стандарт. Эти новые функции имеют **_l** суффикс. Для работы с объектами языковых стандартов добавлено несколько новых функций. К новым функциям относятся `_get_current_locale`, `_create_locale` и `_free_locale`.
...
- **Длинный длинный** тип теперь доступен в качестве **typedef** **__int64** Обратите внимание, что еще не поддержка долго **в** CRT.
