...
Компилятор C++ позволяет создавать уже существующий код, настроенный для компиляции с файлом makefile или без него, и добавлять его в проект [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)]. Дополнительные сведения см. в разделе [Практическое руководство. Создание проекта C++ из существующего кода](../ide/how-to-create-a-cpp-project-from-existing-code.md).  
