...
![Решение&#43;&#43; шпиона](../porting/media/spyxxsolution.PNG "Решение&#43;&#43; шпиона")
...
Следующая проблема `basic_stringbuf` заключается в `freeze` том, что не имеет метода. Метод `freeze` используется для предотвращения утечки памяти в старом `ostream`. Нам не нужно сейчас, когда мы `ostringstream`используем новый . Мы можем удалить вызов `freeze`.

```cpp
//rdbuf()->freeze(0);
```

Следующие две ошибки произошли в соседних строках. Первый жалуется `ends`на использование, `iostream` который является IO манипулятор старой библиотеки, который добавляет нулевой терминатор к строке. Вторая из этих ошибок объясняет, что вывод `str` метода не может быть назначен неконст-указателю.
...
Благодаря применению новой библиотеки потоков `ends` не нужен, так как строка всегда имеет знак завершения NULL, поэтому ее можно удалить. Во втором вопросе проблема заключается в том, что теперь `str()` не возвращаетуказатель в массив символов для строки; он возвращает `std::string` тип. Для устранения второй проблемы следует изменить тип на `LPCSTR` и использовать метод `c_str()` для запроса указателя.
...
Поскольку существует около десяти случаев этой функции все в разных классах, полученных из CWnd, это полезно использовать **Перейти к определению** (клавиатура: **F12**) и **перейти к декларации** (клавиатура: **Ctrl**+**F12**) когда курсор находится на функции в редакторе, чтобы найти их и перейти к ним из окна инструмента **Find Symbol.** Обычно функция **Перейти к определению** является более полезной при поиске. **Перейти к объявлению** позволяет искать объявления, отличные от объявления определяющего класса, например объявления дружественного класса или ссылки вперед.
...
Этот код был написан до того, как в Visual C++ начал поддерживаться встроенный тип bool. В таком коде BOOL был **typedef** для **int**. Тип **Int** является **подписанным** типом, и бит-представление **подписанного Int** заключается в использовании первого бита в качестве бита знака, так что битфилд типа Int может быть истолкован как представляющий 0 или -1, вероятно, не то, что было задумано.
...
Вы можете задаться вопросом, если с помощью стандартного типа **bool** во всем было бы полезно. Многие из старых шаблонов кода, таких как тип BOOL, были изобретены для решения проблем, которые позже были решены в стандартной ки, поэтому переход от BOOL к встроенному типу **bool** является лишь одним из примеров такого изменения, которое вы считаете делать после того, как вы получите ваш код, первоначально запущенный в новой версии.
...
Сообщение LNK1181 появляется потому, что устаревшая версия статической библиотеки для mfc включена во входные данные компоновщика. Это больше не требуется, так как мы можем связать MFC динамически, поэтому нам просто нужно удалить все статические библиотеки MFC из свойства **ввода** в разделе **Linker** свойств проекта. В этом проекте также используется параметр `/NODEFAULTLIB`, который вместо этого перечисляет все зависимости библиотеки.
...
Вот пример кода, который производит это:
...
Еще одна распространенная ошибка, которую вы увидите при преобразовании в Unicode, заключается в этом.
...
Во многих случаях нам было необходимо использовать универсальную версию (макрос) API-интерфейса Win32, например `GetClassName` (вместо `GetClassNameA`). В заявлении обработчика сообщений некоторые сообщения являются специфичными MBCS или Unicode, в этих случаях нам пришлось изменить код, чтобы явно вызвать версию MBCS, потому что мы заменили общеназванные функции на a **и** **W** специфические функции, и добавили макрос для общего имени, которое разрешает **правильное** имя A или **W** в зависимости от того, определяется ли UNICODE.  Во многих частях кода, когда \_мы перешли к определению UNICODE, версия W теперь выбирается даже тогда, когда версия **A** является тем, что требуется.
...
В качестве проверки нашей работы мы должны сбросить **набор символов** для **использования набора символов Multibyte** и убедиться, что код по-прежнему компилируется с помощью MBCS, а также Unicode. Следует отметить, что после внесения всех этих изменений для перекомпилированного приложения нужно было выполнить полное тестирование.
...
Следующим является перенос кода на безопасные версии (версии с **_s** суффиксом) функций CRT. В этом случае общая стратегия заключается в замене функции **_s** версии, а затем, как правило, добавить необходимые дополнительные параметры размера буфера. В большинстве случаев сделать это несложно, так как размер известен. В других случаях, когда размер не доступен сразу, необходимо добавить дополнительные параметры к функции, использующую функцию CRT, или, возможно, изучить использование буфера назначения и посмотреть, каковы соответствующие ограничения размера.
...
Когда вы определяете, что каждый раз, когда буфер является массивом, а не необработанным указателем, его размер определяется из типа массива и используется в качестве параметра размера, и вам не нужно его указывать. Это позволяет уменьшить степень сложности при переработке кода. Вы все еще должны заменить имя функции с **_s** версии, но это часто может быть сделано путем поиска и заменить операцию.
...
Некоторые из распространенных случаев: для `memcpy` при переключении на `memcpy_s` мы часто добавляли размер структуры, в которую выполняется копирование. Аналогичным образом, для большинства строк и буферов размер массива или буфера легко определить с помощью объявления буфера или путем поиска места, в котором изначально располагался буфер. В некоторых ситуациях необходимо определить, насколько большой буфер на самом деле доступен, и если эта информация недоступна в области функции, которую вы изменяете, она должна быть добавлена в качестве дополнительного параметра и код вызова должен быть изменен, чтобы предоставить информацию.
