...
Можно также работать с проектами Visual Studio 2008, но их нельзя использовать без изменений. Для получения подробной информации, **см.**
...
Visual Studio 2008 имеет свою собственную специальную систему сборки для СЗ под названием **VCBuild**. Начиная с Visual Studio 2010 проекты Visual Studio C++ были изменены для использования **MSBuild**. Это означает, что будь то постоянное обновление или мульти-таргетинг вы должны пройти через шаг обновления, чтобы построить visual Studio 2008 проектов в последней версии Visual Studio. Обновленный проект по-прежнему создает двоичные файлы, которые полностью совместимы с двоичными файлами, созданными в интегрированной среде разработки Visual Studio 2008.

Сначала в дополнение к текущей версии Visual Studio необходимо установить Visual Studio 2010 на одном компьютере с Visual Studio 2008. Только Visual Studio 2010 устанавливает скрипты **MSBuild,** которые необходимы для целевых проектов Visual Studio 2008.
...
> [!NOTE]
> Следующие шаги применяются только к многоцелевым сценариям. Если вы собираетесь постоянно модернизировать проект до более позднего набора инструментов, то следующим шагом будет сохранение проекта, открытие проекта в Visual Studio 2019 и решение проблем сборки, которые там появляются.

Если по завершении обновления в журнале ошибок появятся ошибки или предупреждения по проектам, внимательно изучите их. Преобразование с **VCBuild** в **MSBuild** может вызвать проблемы. Вы должны понимать и уметь применять все элементы действий, приведенные в отчете. Для получения дополнительной информации о отчете об обновлении журнала и проблемы, которые могут возникнуть при преобразовании **VCBuild** в **MSBuild**, см. [C++ Native Multi-Targeting](https://blogs.msdn.microsoft.com/vcblog/2009/12/08/c-native-multi-targeting/)
