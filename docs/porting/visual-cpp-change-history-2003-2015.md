...
- **Инкапсуляция FILE**
   ...
   В предыдущих версиях полный тип файла был определен в \<stdio. h >, поэтому пользовательский код может обращаться к файлу и изменять его внутренние свойства. Библиотека была изменена, чтобы скрыть детали реализации. В рамках этого изменения тип FILE теперь определен в \<stdio.h> как непрозрачный, а его члены недоступны извне CRT.
...
- **smallheap**
   ...
   Параметр включения режимов `smallheap` был удален. См. раздел [Link Options](../c-runtime-library/link-options.md).
...
- **Доступ к конструкторам внутри операторов decltype**
   ...
   Чтобы исправить эту ошибку, добавьте в `S2` объявление дружественных отношений для `S`:
...
- **#include: использование спецификатора родительского каталога ".." в пути** (влияет только на `/Wall` `/WX`)
...
- **#pragma optimize () расширяет конец файла заголовка** (влияет только на `/Wall` `/WX`)
...
- **Несоответствие #pragma warning (Push)** и **#pragma warning (POP)** (влияет только на `/Wall` `/WX`)
...
- **Непарное #pragma предупреждение (Push)** (влияет только на `/Wall` `/WX`)
...
- **Файлы предкомпилированного заголовка (PCH) и несоответствующие директивы #include** (влияет только на `/Wall` `/WX`)
...
- **Файлы предкомпилированного заголовка (PCH) и несовпадающие каталоги включаемых файлов** (влияет только на `/Wall` `/WX`)
...
- До того, как компилятор стал совместимым с ISO C++11, следующий код скомпилировался бы, и переменная `x` разрешилась бы в тип **int**:
   ...
   Теперь этот код разрешает `x` типу `std::initializer_list<int>` и вызывает ошибку на следующей строке, которая пытается назначить `x` типа **int**. (По умолчанию преобразование отсутствует.) Чтобы исправить этот код, используйте **int** для замены **Auto**:
...
- В Visual Studio 2013 результат `sizeof(S2)` в x64 — 48, но в предыдущих выпусках он равен 32. Чтобы получить значение 32 в компиляторе C++ в Visual Studio 2013 для x64, добавьте базовый класс-заглушку, обладающий **виртуальной** функцией:
   ...
   Чтобы найти в коде места, которые попытались оптимизировать в более раннем выпуске, используйте компилятор из этого выпуска вместе с параметром компилятора `/W3` и включите on Warning C4370. Например:
...
- `CFileDialog` Class: пользовательские шаблоны для класса `CFileDialog` не могут быть автоматически перенесены в Windows Vista. Их по-прежнему можно использовать, но у них не будет дополнительной функциональности или вида диалоговых окон в стиле Windows Vista.
