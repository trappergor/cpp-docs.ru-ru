...
- **Теперь семейство функций printf и scanf определяется встроенным образом.**
   ...
   Чтобы устранить нерешенные ошибки символа, можно попробовать использовать [dumpbin.exe](../build/reference/dumpbin-reference.md) для изучения символов, определенных в двоичном файле. Попробуйте выполнить следующую команду для просмотра символов, определенных в библиотеке.
...
- **Инкапсуляция FILE**
   ...
   В предыдущих версиях полный тип \<FILE был публично определен в sdio.h>, так что можно было для пользовательского кода, чтобы достичь в FILE и изменить его внутренностях. Библиотека была изменена, чтобы скрыть детали реализации. В рамках этого изменения тип FILE теперь определен в \<stdio.h> как непрозрачный, а его члены недоступны извне CRT.
...
- [Улучшение соответствия в обновлении 1](#VS_Update1)

- [Улучшение соответствия в обновлении 2](#VS_Update2)

- [Улучшение соответствия в обновлении 3](#VS_Update3)
...
- **Правила, применяемые для форвардных деклараций. (Применяется только к C.)**
...
- **alignof теперь является ключевым словом**
   ...
   Теперь для следующего кода создается ошибка C2332: 'класс': отсутствует имя тега. Чтобы исправить код, необходимо переименовать класс или, если класс выполняет ту же работу, что и **выравнивание,** просто замените класс новым ключевым словом.
...
- **__declspec (novtable) декларации должны быть последовательными**
...
- Переменные-члены `volatile` ** не допускают неявно определенных конструкторов и операторов присваивания**
   ...
   В предыдущих версиях компилятора допускалось автоматическое создание конструкторов копирования и перемещения по умолчанию, а также операторов присваивания копирования и перемещения по умолчанию для класса, содержащего переменные-члены **volatile**. Это поведение было неправильным и не соответствовало стандарту языка C++. Теперь компилятор рассматривает класс с **неустойчивыми** переменными членов, чтобы иметь нетривиальных операторов построения и назначения, что предотвращает автоматическое генерируемое выполнение по умолчанию этих операторов. Если такой класс является членом объединения (или анонимного объединения внутри класса), конструкторы копирования и перемещения и операторы присваивания копирования и перемещения объединения (или класса, содержащего анонимное объединение) будут неявно определены как удаленные. Попытка создать или скопировать объединение (или класс, содержащий анонимное объединение), не объявляя их явно, будет являться ошибкой. В результате будет выдана ошибка компилятора C2280.
...
- До того, как компилятор стал совместимым с ISO C++11, следующий код скомпилировался бы, и переменная `x` разрешилась бы в тип **int**:
   ...
   Этот код теперь разрешает `x` `std::initializer_list<int>` сядки с типом и вызывает `x` ошибку на следующей строке, которая пытается назначить для ввода **Int**. (По умолчанию преобразования не существует.) Чтобы исправить этот код, используйте **Int** для замены **авто:**
...
- Макет объектов изменен. В x64 макет объектов класса может отличаться от аналогичного макета в предыдущих выпусках. Если у него есть **виртуальная** функция, но он не имеет базовый класс, который имеет **виртуальную** функцию, объектная модель компилятора вставляет указатель в **виртуальную** таблицу функций после макета члена данных. Это означает, что макет не во всех случаях будет оптимальным. В предыдущих выпусках технология оптимизации x64 предпринимала попытку улучшения макета в соответствии с требованиями пользователя, но так как при работе со сложными кодами она не выполняла свои функции должным образом, в Visual Studio 2013 данная технология была удалена. Рассмотрим для примера такой код:
...
- В Visual Studio 2013 результат `sizeof(S2)` в x64 — 48, но в предыдущих выпусках он равен 32. Чтобы получить значение 32 в компиляторе C++ в Visual Studio 2013 для x64, добавьте базовый класс-заглушку, обладающий **виртуальной** функцией:
   ...
   Чтобы найти места в коде, которые предыдущий релиз попытался бы оптимизировать, используйте компилятор из этого выпуска вместе с опцией `/W3` компилятора и включите предупреждение C4370. Пример:
...
- Когда установлен новый флаг компоновщика `/HIGHENTROPYVA`, Windows 8 обычно при выделении памяти возвращает 64-разрядный адрес. (До Windows 8 такие выделения чаще возвращали адреса, которые составляли менее 2 ГБ.) Это изменение может обнажить ошибки усечения указателей в существующем коде. По умолчанию этот параметр включен. Значение `/HIGHENTROPYVA:NO` отключает это поведение.
...
- C++ 11 17.6.4.3.1 [macro.names]/2 запрещает замену ключевых слов макросами, если включены заголовки стандартной библиотеки C++. Теперь заголовки выдают ошибки компилятора при обнаружении ключевых слов, замененных макросами. (Определение _ALLOW_KEYWORD_MACROS позволяет компилировать такой код, но мы настоятельно не рекомендуем это использование.) В `new` качестве исключения, макроформа допускается по умолчанию, потому что `#pragma push_macro("new")` / `#undef new` / `#pragma pop_macro("new")`заголовки всесторонне защитить себя с помощью . Определение _ENFORCE_BAN_OF_MACRO_NEW делает именно то, что предполагает его имя.
...
- Куча среды выполнения C (CRT), которая используется для new и malloc(), больше не является закрытой. CRT теперь использует кучу процесса. Это означает, что куча не разрушается при разгрузке DLL, поэтому DLL, которые статически связываются с CRT, должны обеспечить очистку памяти, выделенной кодом DLL, прежде чем она будет выгружена.
...
- Последовательность табуляций элементов управления в диалоговых окнах MFC: правильная последовательность нескольких элементов управления в диалоговом окне MFC нарушается, если в нее вставляется элемент управления MFC ActiveX. Это изменение устраняет данную проблему.
   ...
   Например, создайте диалоговое приложение MFC с элементом управления ActiveX и несколькими элементами управления редактированием. Поместите элемент управления ActiveX в середине последовательности табуляции элементов управления редактирования. Запустите приложение, нажмите элемент управления, чей порядок вкладок после управления ActiveX, а затем вкладка. До этого изменения, фокус пошел на контроль изменения после управления ActiveX вместо следующего управления изменениями в порядке вкладки.

- `CFileDialog`Класс: Пользовательские шаблоны для `CFileDialog` класса не могут быть автоматически портированы в Windows Vista. Их по-прежнему можно использовать, но у них не будет дополнительной функциональности или вида диалоговых окон в стиле Windows Vista.
...
[Новые возможности Visual C++ в Visual Studio](../overview/what-s-new-for-visual-cpp-in-visual-studio.md)
