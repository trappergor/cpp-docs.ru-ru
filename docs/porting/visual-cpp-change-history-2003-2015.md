...
- Вы больше не сможете выполнять модульные тесты с использованием C++/CLI из командной строки с помощью mstest.exe. Вместо этого используйте vstest.console.exe. См. раздел [Параметры командной строки для VSTest.Console.exe](/visualstudio/test/vstest-console-options).
...
- **Объявления __declspec(novtable) должны быть согласованы**
   ...
   `__declspec` должны быть согласованы во всех библиотеках. Следующий код теперь приводит к нарушению правила одного определения (ODR):
...
- `CDatabase::GetConnect` (метод) удален. Для большей безопасности строка подключения теперь хранится в зашифрованном виде и расшифровывается только по мере необходимости.  Получить строку можно с помощью метода `CDatabase::Dump`.
...
- Изменились сигнатуры для поддержки обработчиков сообщений. Списки параметров следующих функций были изменены для использования вновь добавленных обработчиков сообщений ON_WM_*:
   ...
   - `CWnd::OnDisplayChange` изменилась с (WPARAM, LPARAM) на (UINT, int, int), чтобы можно было использовать в схеме сообщений новый макрос ON_WM_DISPLAYCHANGE.

   - `CFrameWnd::OnDDEInitiate` изменилась с (WPARAM, LPARAM) на (CWnd*, UINT, UNIT), чтобы можно было использовать в схеме сообщений новый макрос ON_WM_DDE_INITIATE.

   - `CFrameWnd::OnDDEExecute` изменилась с (WPARAM, LPARAM) на (CWnd*, HANDLE), чтобы можно было использовать в схеме сообщений новый макрос ON_WM_DDE_EXECUTE.

   - `CFrameWnd::OnDDETerminate` изменилась с (WPARAM, LPARAM) на (CWnd*), чтобы можно было использовать в схеме сообщений новый макрос ON_WM_DDE_TERMINATE.

   - `CMFCMaskedEdit::OnCut` вместо (WPARAM, LPARAM) теперь не имеет параметров, чтобы можно было использовать в схеме сообщений новый макрос ON_WM_CUT.

   - `CMFCMaskedEdit::OnClear` вместо (WPARAM, LPARAM) теперь не имеет параметров, чтобы можно было использовать в схеме сообщений новый макрос ON_WM_CLEAR.

   - `CMFCMaskedEdit::OnPaste` вместо (WPARAM, LPARAM) теперь не имеет параметров, чтобы можно было использовать в схеме сообщений новый макрос ON_WM_PASTE.

- `#ifdef` (директивы) в файлах заголовков MFC удалены. Многочисленные директивы `#ifdef` в файлах заголовков MFC, относящиеся к неподдерживаемым версиям Windows (WINVER &lt; 0x0501), удалены.
...
- Наряду с обнаружением несоответствия в _ITERATOR_DEBUG_LEVEL (функция, реализованная в Visual C++ 2010) компилятор C++ в Visual Studio 2012 обнаруживает несоответствия библиотеки времени выполнения. Эти несоответствия возникают при смешении параметров компилятора `/MT` (статический выпуск), `/MTd` (статическая отладка), `/MD` (динамический выпуск) и `/MDd` (динамическая отладка).
...
- `CFileStatus` (структура) теперь имеет новый размер: элемент `m_attribute` изменен с BYTE на DWORD (для соответствия значению, возвращенному `GetFileAttributes`).
...
- Невозможно выполнить сборку ATL без зависимости от CRT. В более ранних версиях Visual Studio можно было использовать #define ATL_MIN_CRT, чтобы установить минимальную зависимость проекта ATL от CRT. В Visual C++ 2008 все проекты ATL минимально зависят от CRT независимо от того, определен ли ATL_MIN_CRT.
...
- `CTime` Класс: теперь класс `CTime` принимает даты, начиная с 01/01/1900 г. н. э. вместо 01/01/1970 г. н. э.
...
- `CFileDialog` (класс): пользовательские шаблоны для класса `CFileDialog` невозможно перенести автоматически в Windows Vista. Их по-прежнему можно использовать, но у них не будет дополнительной функциональности или вида диалоговых окон в стиле Windows Vista.

- `CWnd` и `CFrameWnd` (классы): метод `CWnd::GetMenuBarInfo` удален.
...
- `sprintf` теперь печатает знак минус для нуля со знаком.

- `swprintf` изменена для соответствия стандарту, в ней теперь необходимо указывать параметр размера. Форма `swprintf` без параметра размера не рекомендуется.
