...
Эта команда регистрации после сборки нам больше не нужна. Вместо этого мы просто удалим пользовательскую команду сборки и укажите в настройках **Linker** для регистрации вывода.
...
Проблема состоит в том, что `i` объявляется как `UINT`, а `lCount` объявляется как **long**, поэтому имеется несоответствие типов со знаком или без знака. Менять тип `lCount` на `UINT` будет неудобно, так как он получает свое значение из метода `IMtsEventInfo::get_Count`, использующего тип **long**, и не находится в коде пользователя. Поэтому мы добавим приведение в код. C-стиль литые будет делать для численных литой, такие как это, но **static_cast** является рекомендуемым стилем.
...
Чтобы проверить функциональность приложения, мы создали **обслуживаемый компонент** с помощью шаблона проекта ATL, решили добавить поддержку КОМЗ в мастера проекта ATL. Если вы не работали с обслуживаемыми компонентами раньше, это не трудно создать один и получить один зарегистрирован и доступен в системе или сети для других приложений для использования. Приложение COM Spy предназначено для отслеживания активности обслуживаемых компонентов в качестве средства диагностики.
...
Затем мы построили и зарегистрировали его (вам нужно будет запустить Visual Studio в качестве администратора), и активировали его с помощью приложения **Serviced Component** в панели управления Windows. Мы создали проект Windows Forms в C#, перетащили кнопку в форму с панели элементов и дважды щелкнули ее для перехода к обработчику событий щелчка кнопкой мыши. Чтобы создать экземпляр компонента `Dog`, мы добавили следующий код.
