...
Чтобы начать процесс обновления, откройте старую версию проекта, решения или рабочей области в последней версии Visual Studio. Visual Studio создаст новый проект на основе старых параметров проекта. Если старый проект имеет библиотеку или включает пути, которые жестко закодированы в нестандартных местоположениях, возможно, что файлы в этих путях не будут видны компилятору, когда проект использует параметры по умолчанию. Дополнительные сведения см. в разделе [Параметр OutputFile компоновщика](porting-guide-spy-increment.md#linker_output_settings).
...
- Если исходный файл находится в месте, не по умолчанию, вы добавили путь к каталогам проекта?
...
- Типы аргументов в месте вызова соответствуют существующей перегрузке функции? Проверка базовых типов для любых типов в подписи функции и в коде, который вызывает функцию, то, что вы ожидаете, что они будут.
...
В следующей таблице приведены библиотеки, содержимое которых было изменено начиная с Visual Studio 2015. Для обновления нужно добавить новые имена библиотек во второй столбец к библиотекам в первом столбце. Некоторые из этих библиотек являются библиотеками импорта, но это не имеет значения.
...
Вы можете нажать **F12** (**Перейти к определению**), чтобы увидеть, где определяется тип в вопросе.
...
За прошедшие годы в среду выполнения C было внесено множество изменений. Были добавлены многие безопасные версии функций, а некоторые были удалены. Кроме того, как описано ранее в этой статье, реализация Microsoft CRT была рефакторизована в Visual Studio 2015 в новые файлы бинарных файлов и связанных с ними файлов .lib.
...
Постепенно стали появляться безопасные версии функций для среды выполнения C. Хотя старые небезопасные версии по-прежнему доступны, рекомендуется изменить код, чтобы использовать безопасные версии. При наличии небезопасных версий компилятор выдает предупреждение. Вы можете отключить или игнорировать эти предупреждения. Чтобы отключить предупреждение для всех проектов в вашем решении, откройте **View** > **Property Manager,** выберите все проекты, для которых вы хотите отключить предупреждение, а затем нажмите правой кнопкой мыши на выбранные элементы и выберите **Свойства.** В разделе **Свойства конфигурации** > **C/C++** > **Дополнительно** диалогового окна **Страницы свойств** выберите **Отключить некоторые предупреждения**. Нажмите на выпадающую стрелку, а затем нажмите на **Edit**. Введите 4996 в текстовом поле. (Не включайте префикс 'C'.) Для получения дополнительной [информации см. Porting, чтобы использовать Безопасный CRT](porting-guide-spy-increment.md#porting_to_secure_crt).
...
До стандартизации Юникода для представления символов, не включенных в набор символов ASCII, многие программы использовали многобайтовую кодировку (MBCS). В старых проектах MFC кодировка MBCS использовалась по умолчанию, поэтому при обновлении такой программы выводятся предупреждения, где рекомендуется использовать Юникод. Вы можете отключить или проигнорировать такое предупреждение, если считаете переход на Юникод нецелесообразным. Чтобы отключить его для всех проектов в вашем решении, откройте **View** > **Property Manager,** выберите все проекты, для которых вы хотите отключить предупреждение, а затем нажмите правой кнопкой мыши на выбранные элементы и выберите **Свойства.** В диалоговом окне **Страницы свойств** выберите **Свойства конфигурации** > **C/C++** > **Дополнительно**. Откройте раскрывающийся список в свойстве **Отключить некоторые предупреждения** и выберите **Изменить**. Введите 4996 в текстовом поле. (Не включайте префикс 'C'.) Выберите **OK,** чтобы сохранить свойство, а затем выбрать **OK,** чтобы сохранить изменения.

Дополнительные сведения см. в разделе [Перенос из MBCS в Юникод](porting-guide-spy-increment.md#porting_to_unicode). Для получения общей информации о MBCS против Unicode, см [Текст и строки в визуальных СЗ](../text/text-and-strings-in-visual-cpp.md) и [интернационализации](../c-runtime-library/internationalization.md) .

## <a name="see-also"></a>См. также раздел

[Обновление проектов из более ранних версий Визуального КЗ](upgrading-projects-from-earlier-versions-of-visual-cpp.md)<br/>
