...
При наличии исходного кода для библиотеки DLL или статической библиотеки можно выполнить перекомпиляцию с параметром `/ZW` в виде проекта UWP. В этом случае можно добавить ссылку с помощью **Обозреватель решений**и использовать ее в приложениях UWP на C++. В случае с DLL осуществляется связь с библиотекой экспорта.
...
2. Откройте **Свойства проекта** библиотеки DLL и задайте **конфигурацию** для **всех конфигураций**.
...
5. Убедитесь, что файл предкомпилированного заголовка имеет правильное имя. В разделе **предварительно скомпилированные заголовки** измените **файл предкомпилированного заголовка** с *PCH. h* на *stdafx. h*. Если этого не сделать, может появиться следующее сообщение об ошибке.
...
7. Чтобы использовать библиотеку DLL в приложении UWP в том же решении, откройте контекстное меню узла проекта UWP и выберите команду **Добавить**  >  **ссылку**.

   В **Projects**разделе  >  **решение**проектов установите флажок рядом с проектом DLL и нажмите кнопку **ОК** .

8. Включите файлы заголовков библиотеки в файл *PCH. h* приложения UWP.
...
2. Добавьте оператор include для ссылки на файл заголовка в файл *PCH. h* (если он есть) или в любой CPP-файл при необходимости и начните добавлять код, использующий библиотеку.
...
5. Выберите все файлы для добавления из исходного проекта и нажмите кнопку **ОК**. При необходимости повторите эти действия для вложенных папок.

6. На этом этапе может появиться повторяющийся код. Если у вас есть несколько предварительно скомпилированных заголовков (например *, stdafx. h* и *PCH. h*), выберите один из них для сохранения. В него скопируйте необходимый код, например выражения include. Затем удалите другой и в окне свойств проекта в разделе **Предкомпилированные заголовки** проверьте правильность имени файла заголовка.
