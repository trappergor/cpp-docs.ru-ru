...
Компьютер удаленной сборки | Отображает целевой компьютер или устройство, используемые для удаленной сборки, развертывания и отладки. Можно добавить или изменить подключение к целевому компьютеру в разделе **Средства** > **Параметры** > **Кроссплатформенные** > **Диспетчер подключений**. **Visual Studio 2019 версии 16.1** Вы можете указать другой компьютер для отладки на странице [Отладка](debugging-linux.md).
