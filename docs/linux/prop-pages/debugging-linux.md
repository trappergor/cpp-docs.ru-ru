...
--- | ---| ---
Удаленный компьютер для отладки | **Visual Studio 2019 версии 16.1**. Определяет компьютер для отладки программы. Может отличаться от компьютера для удаленной сборки, определенный на странице [Общие](general-linux.md).
