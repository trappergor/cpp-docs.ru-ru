...
## <a name="configure-other-debugging-options-msbuild-based-projects"></a>Настройка других параметров отладки (для проектов на основе MSBuild)
...
## <a name="configure-other-debugging-options-cmake-projects"></a>Настройка других параметров отладки (для проектов CMake)

Вы можете указать дополнительные аргументы командной строки для проекта CMake в файле launch.vs.json. Дополнительные сведения см. в статье [об отладке проекта CMake](cmake-linux-project.md#debug_cmake_project).
...
## <a name="specify-different-machines-for-building-and-debugging"></a><a name="separate_build_debug"></a> Указание разных компьютеров для сборки и отладки
...
### <a name="msbuild-based-projects"></a>Проекты на основе MSBuild
...
В раскрывающемся меню **Удаленный компьютер отладки** перечислены все созданные удаленные подключения. Чтобы добавить новое удаленное подключение, откройте раздел **Средства** > **Параметры** > **Кроссплатформенные** > **Диспетчер подключений** или выполните поиск по строке "диспетчер подключений" на панели **быстрого запуска**. Кроме того, новый удаленный каталог для развертывания можно указать на страницах свойств проекта (**Свойства конфигурации** > **Общие** > **Удаленный каталог развертывания**).
